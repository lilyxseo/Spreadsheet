...
export async function handleDashboardSummaryRequest({ request, env }) {
  ...
  try {
    ...
    const rows = await loadInventoryAnalyticsRows(env);
    return json({ success: true, source: 'supabase', summary: computeInventorySummary(rows), durationMs: Date.now() - startedAt });
  } catch (error) {
