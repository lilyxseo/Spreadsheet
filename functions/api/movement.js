...
const SHEET_RANGE = "Data Movement Barang!A:G";
...
function validateItem(item, index) {
  ...
  const stokLokasiAwal = toNumberOrNull(item?.stok_lokasi_awal);
  const stokAktual = toNumberOrNull(item?.stok_aktual);

  if (!from) return { error: `items[${index}].from wajib diisi` };
  if (!to) return { error: `items[${index}].to wajib diisi` };
  if (!sku) return { error: `items[${index}].sku wajib diisi` };
  if (!namaBarang) return { error: `items[${index}].nama_barang wajib diisi` };
  if (stokLokasiAwal === null) return { error: `items[${index}].stok_lokasi_awal wajib angka` };
  if (stokAktual === null) return { error: `items[${index}].stok_aktual wajib angka` };

  return { value: { from, to, sku, nama_barang: namaBarang, stok_lokasi_awal: stokLokasiAwal, stok_aktual: stokAktual } };
}

export async function onRequestPost({ request, env }) {
  try {
    const SHEET_MAP = { movement: env.SHEET_ID_INVENTORY };
    const sheetId = SHEET_MAP.movement;
    if (!sheetId) return json({ success: false, message: "Sheet tidak ditemukan" }, 400);
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const key = `${result.value.sku}||${result.value.from}||${result.value.to}`.toLowerCase();
      if (dedupe.has(key)) return json({ success: false, message: "Duplikasi item (sku + from + to)" }, 400);
      dedupe.add(key);
      validated.push(result.value);
    }

    const accessToken = await createAccessToken(env);
    const rows = validated.map((item) => [tanggal, item.from, item.to, item.sku, item.nama_barang, item.stok_lokasi_awal, item.stok_aktual]);
    ...
    if (!sheetRes.ok) return json({ success: false, message: result.error?.message || "Gagal append ke Google Sheet", detail: result }, sheetRes.status);

    return json({ success: true, message: "Movement berhasil disimpan", total_items: rows.length, result });
  } catch (err) {
