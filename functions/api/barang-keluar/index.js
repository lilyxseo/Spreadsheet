import { json, token, SHEET_BARANG_KELUAR, mapBarangSheetValues } from '../_barang-ops.js';
...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const mapped = mapBarangSheetValues(values, START_ROW);
    const rows = limitRows(mapped.rows, request);
    return json({ success: true, spreadsheetId, sheetName: SHEET_BARANG_KELUAR, columns: mapped.columns, startRow: START_ROW, data: rows, rows, values: rows.map(row => mapped.columns.map(key => row[key] ?? '')) });
  } catch (err) {
