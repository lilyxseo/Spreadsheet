...
export async function insertRows({env,sheetName,rows}){const access=await token(env);const ss=env.SHEET_ID_2026;const meta=await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${ss}?fields=sheets.properties`,{headers:{Authorization:`Bearer ${access}`}});const m=await meta.json();const sh=(m.sheets||[]).find(s=>String(s.properties?.title||'')===sheetName);if(!sh)throw new Error(`Sheet ${sheetName} tidak ditemukan di SHEET_ID_2026`);const sorted=[...rows].sort((a,b)=>Number(a.rowNumber)-Number(b.rowNumber));for(const item of sorted){const rowNumber=Number(item.rowNumber);const requests=[{insertDimension:{range:{sheetId:sh.properties.sheetId,dimension:'ROWS',startIndex:rowNumber-1,endIndex:rowNumber},inheritFromBefore:rowNumber>2}}];const ins=await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${ss}:batchUpdate`,{method:'POST',headers:{Authorization:`Bearer ${access}`,'Content-Type':'application/json'},body:JSON.stringify({requests})});const insOut=await ins.json();if(!ins.ok)throw new Error(insOut.error?.message||'Gagal insert row');const range=`${sheetName}!A${rowNumber}:I${rowNumber}`;const put=await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${ss}/values/${encodeURIComponent(range)}?valueInputOption=USER_ENTERED`,{method:'PUT',headers:{Authorization:`Bearer ${access}`,'Content-Type':'application/json'},body:JSON.stringify({values:[item.values||[]]})});const putOut=await put.json();if(!put.ok)throw new Error(putOut.error?.message||'Gagal restore row');}}
