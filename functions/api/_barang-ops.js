...
export async function cachedGoogleRead(key,loader,{ttlMs=3*60*1000,force=false}={}){
  const cached=readCache.get(key);
  if(!force&&cached&&Date.now()-cached.updatedAt<ttlMs)return {...cached,fromCache:true};
  if(readRequests.has(key))return readRequests.get(key);
  const pending=Promise.resolve().then(loader).then(value=>{
    const entry={value,updatedAt:Date.now(),version:String(value?.etag||Date.now()),fromCache:false};
...
export function invalidateGoogleRead(key){if(key)readCache.delete(key);else readCache.clear();}
