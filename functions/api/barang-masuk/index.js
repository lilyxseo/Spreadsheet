import { json, token, SHEET_BARANG_MASUK, detectColumnsFromRows, mapBarangSheetValues } from '../_barang-ops.js';

const START_ROW = 1;
const RANGE = `${SHEET_BARANG_MASUK}!A${START_ROW}:ZZ20000`;
...
export async function onRequestGet({ request, env }) {
  try {
    ...
    return json({ success: true, spreadsheetId, sheetName: SHEET_BARANG_MASUK, columns, startRow: 2, data: rows, rows, values: rows.map(row => columns.map(key => row[key] ?? '')) });
  } catch (err) {
