...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const range = `${SHEET_BARANG_MASUK}!A1:I20000`;
    ...
    return json({ success: true, spreadsheetId, sheetName: SHEET_BARANG_MASUK, mode, limit, values, totalRows: Math.max(allValues.length - 1, 0) });
  } catch (err) {
