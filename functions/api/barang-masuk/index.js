import { json, token, SHEET_BARANG_MASUK, parseBarangValuesWithHeader } from '../_barang-ops.js';
...
export async function onRequestGet({ request, env }) {
  try {
    ...
    return json({ success: true, spreadsheetId, sheetName: SHEET_BARANG_MASUK, columns: parsed.columns, startRow: START_ROW, data: rows, rows, values: rows.map(row => parsed.columns.map(key => row[key] ?? '')) });
  } catch (err) {
