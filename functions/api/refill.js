...
export async function onRequestGet({request,env}){
  ...
  try{
    const spreadsheetId=sanitize(env.SHEET_ID_INVENTORY);
...
export async function onRequestPost(ctx){
  ...
  try{
    ...
    const app=await appendToSheet({accessToken,sheetId:sanitize(env.SHEET_ID_INVENTORY),range:'REKAP RPL!A:F',values:historyRows});
