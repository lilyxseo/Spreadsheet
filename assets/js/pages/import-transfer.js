      ...
      for (const src of cdnCandidates) {
        try {
          if (src.endsWith('.mjs')) {
            await import(src);
            if (window.pdfjsLib) {
              pdfWorkerSrc = src.replace('pdf.min.mjs', 'pdf.worker.min.mjs');
              break;
            }
          } else {
            await loadScript(src);
            if (window.pdfjsLib) {
              pdfWorkerSrc = src.replace('pdf.min.js', 'pdf.worker.min.js');
              break;
            }
          }
        } catch (err) {
...
async function parsePdf(file) {
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    for (const item of content.items) {
      ...
      if (bucket) bucket.items.push({ text, x });
      else buckets.push({ y, items: [{ text, x }] });
    }
    buckets
      .sort((a, b) => b.y - a.y)
      .forEach((bucket) => {
        const line = bucket.items
          .sort((a, b) => a.x - b.x)
          .map((x) => x.text)
          .join(' ')
          .trim();
