...
const getField=(r,keys)=>{if(Array.isArray(r)){const idxMap={tanggal:0,date:0,from:1,to:2,sku:3,"nama barang":4,namabarang:4,nama:4,item:4,description:4,qty:5,status:6,pic:7,keterangan:8,waktu:0};for(const k of keys){const idx=idxMap[String(k||"").toLowerCase().trim()];if(Number.isInteger(idx)&&r[idx]!=null)return String(r[idx]);}return "";}const cols=Object.keys(r||{});for(const k of keys){const f=cols.find(c=>String(c||"").toLowerCase().includes(String(k).toLowerCase()));if(f&&r[f]!=null)return String(r[f]);}return "";};
const num=v=>{const n=parseFloat(String(v||"").replace(/[^0-9.-]/g,""));return Number.isFinite(n)?n:0;};
function parseSheetDate(value){if(!value)return null;const str=String(value).trim();if(/^\d{4}-\d{2}-\d{2}/.test(str)){const d=new Date(str);return Number.isNaN(d.getTime())?null:d;}const parts=str.split("/");if(parts.length===3){const a=Number(parts[0]);const b=Number(parts[1]);let y=Number(parts[2]);if(y<100)y+=2000;const d=new Date(y,a-1,b);return Number.isNaN(d.getTime())?null:d;}const d=new Date(str);return Number.isNaN(d.getTime())?null:d;}
