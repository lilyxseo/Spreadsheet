...
const ARCHIVE_SHEETS={"SO JULI 2025":"SO JULI 2025"};
const ARCHIVE_STATE={sheetList:[],selectedSheet:"",cache:{},loadingList:false,loadingData:false,searchInput:"",search:"",searchTimer:null,sortColumn:"",sortDirection:"asc",columnFilter:"all",page:1,pageSize:25,lastLoadedAt:{}};
...
async function fetchArchiveSheetList(){return Object.keys(ARCHIVE_SHEETS);}
async function fetchArchiveSheetData(sheetName){const sourceName=ARCHIVE_SHEETS[sheetName]||sheetName;const raw=await fetchSheet(sourceName);return parseSheet(raw,sourceName);}
async function ensureArchiveList(){if(ARCHIVE_STATE.sheetList.length||ARCHIVE_STATE.loadingList)return;ARCHIVE_STATE.loadingList=true;renderArchivePage();try{ARCHIVE_STATE.sheetList=await fetchArchiveSheetList();}finally{ARCHIVE_STATE.loadingList=false;renderArchivePage();}}
async function selectArchiveSheet(sheetName){ARCHIVE_STATE.selectedSheet=sheetName||"";ARCHIVE_STATE.page=1;if(!sheetName){renderArchivePage();return;}if(ARCHIVE_STATE.cache[sheetName]){renderArchivePage();return;}ARCHIVE_STATE.loadingData=true;renderArchivePage();try{ARCHIVE_STATE.cache[sheetName]=await fetchArchiveSheetData(sheetName);ARCHIVE_STATE.lastLoadedAt[sheetName]=new Date().toISOString();}finally{ARCHIVE_STATE.loadingData=false;renderArchivePage();}}
async function refreshArchive(){const sheet=ARCHIVE_STATE.selectedSheet;if(!sheet){ARCHIVE_STATE.sheetList=[];await ensureArchiveList();return;}delete ARCHIVE_STATE.cache[sheet];ARCHIVE_STATE.loadingData=true;renderArchivePage();try{ARCHIVE_STATE.cache[sheet]=await fetchArchiveSheetData(sheet);ARCHIVE_STATE.lastLoadedAt[sheet]=new Date().toISOString();}finally{ARCHIVE_STATE.loadingData=false;renderArchivePage();}}
function getArchiveColumns(rows){const set=new Set();rows.forEach(r=>Object.keys(r||{}).forEach(k=>{if(k)set.add(k);}));return [...set];}
function getArchiveRows(){const rows=ARCHIVE_STATE.cache[ARCHIVE_STATE.selectedSheet]||[];const cols=getArchiveColumns(rows);const q=clean(ARCHIVE_STATE.search);let out=rows.filter(r=>!q||cols.some(c=>clean(String(r[c]??"")).includes(q)));if(ARCHIVE_STATE.columnFilter!=="all"&&q)out=out.filter(r=>clean(String(r[ARCHIVE_STATE.columnFilter]??"")).includes(q));const col=ARCHIVE_STATE.sortColumn||cols[0]||"";if(col)out=[...out].sort((a,b)=>{const av=a[col],bv=b[col];const an=Number(av),bn=Number(bv),numA=Number.isFinite(an),numB=Number.isFinite(bn);let cmp=0;if(numA&&numB)cmp=an-bn;else cmp=String(av??"").localeCompare(String(bv??""),"id");return ARCHIVE_STATE.sortDirection==="desc"?-cmp:cmp;});return {rows:out,columns:cols};}
function renderArchivePage(){if(!archiveApp)return;if(!ARCHIVE_STATE.sheetList.length&&!ARCHIVE_STATE.loadingList)ensureArchiveList();const selected=ARCHIVE_STATE.selectedSheet;const sourceRows=ARCHIVE_STATE.cache[selected]||[];const {rows,columns}=selected?getArchiveRows():{rows:[],columns:[]};const totalPage=Math.max(1,Math.ceil(rows.length/ARCHIVE_STATE.pageSize));if(ARCHIVE_STATE.page>totalPage)ARCHIVE_STATE.page=totalPage;const start=(ARCHIVE_STATE.page-1)*ARCHIVE_STATE.pageSize;const pageRows=rows.slice(start,start+ARCHIVE_STATE.pageSize);const lastLoaded=selected&&ARCHIVE_STATE.lastLoadedAt[selected]?new Date(ARCHIVE_STATE.lastLoadedAt[selected]).toLocaleString("id-ID"):"-";const sortOps=columns.map(c=>`<option value='${esc(c)}' ${ARCHIVE_STATE.sortColumn===c?"selected":""}>${esc(c)}</option>`).join("");archiveApp.innerHTML=`<div class='archive-layout'><div class='card archive-section'><div class='section-header'><h3 class='page-title'>Arsip</h3><div class='row'><select id='archiveSheetSelect'><option value=''>Pilih Sheet Arsip</option>${ARCHIVE_STATE.sheetList.map(s=>`<option value='${esc(s)}' ${s===selected?"selected":""}>${esc(s)}</option>`).join("")}</select><button id='archiveRefreshBtn' class='btn-ghost'>Refresh Arsip</button></div></div>${ARCHIVE_STATE.loadingList?"<div class='state'>Memuat daftar sheet arsip...</div>":""}</div><div class='card archive-section archive-filter-card'><div class='mv-filters open archive-filters'><input id='archiveSearch' class='search-lg' placeholder='Cari di semua kolom' value='${esc(ARCHIVE_STATE.searchInput)}'/><select id='archiveSortColumn'><option value=''>Urutkan Kolom</option>${sortOps}</select><select id='archiveSortDir'><option value='asc' ${ARCHIVE_STATE.sortDirection==='asc'?'selected':''}>ASC</option><option value='desc' ${ARCHIVE_STATE.sortDirection==='desc'?'selected':''}>DESC</option></select><select id='archiveColumnFilter'><option value='all'>Semua Kolom</option>${columns.map(c=>`<option value='${esc(c)}' ${ARCHIVE_STATE.columnFilter===c?'selected':''}>Filter: ${esc(c)}</option>`).join('')}</select></div></div>${!selected?"<div class='card archive-section'><div class='state'>Pilih sheet arsip untuk melihat data</div></div>":ARCHIVE_STATE.loadingData?"<div class='card archive-section'><div class='state'>Memuat data sheet...</div></div>":`<div class='card archive-section'><div class='grid dashboard archive-summary'><div class='metric'><div class='k'>Total Baris</div><div class='v'>${sourceRows.length}</div></div><div class='metric'><div class='k'>Total Kolom</div><div class='v'>${columns.length}</div></div><div class='metric'><div class='k'>Sheet Aktif</div><div class='v'>${esc(selected)}</div></div><div class='metric'><div class='k'>Terakhir Dimuat</div><div class='v'>${esc(lastLoaded)}</div></div></div></div><div class='card archive-section archive-table-card'>${!rows.length?"<div class='state'>Tidak ada data</div>":`<div class='table-wrap table-wrap-full archive-table-wrap'><table><thead><tr>${columns.map(c=>`<th>${esc(c)}</th>`).join("")}</tr></thead><tbody>${pageRows.map(r=>`<tr>${columns.map(c=>`<td>${esc(r[c]??"")}</td>`).join("")}</tr>`).join("")}</tbody></table></div><div class='mv-pagination'><span>Menampilkan ${rows.length?start+1:0}-${Math.min(start+ARCHIVE_STATE.pageSize,rows.length)} dari ${rows.length}</span><div class='row'><button class='btn-ghost' data-archive-page='-1' ${ARCHIVE_STATE.page<=1?'disabled':''}>Prev</button><button class='btn-ghost' data-archive-page='1' ${ARCHIVE_STATE.page>=totalPage?'disabled':''}>Next</button></div></div>`}</div>`}</div>`;}
