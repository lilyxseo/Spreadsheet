...
function runSearch(){const qRaw=searchInput.value||"";const q=clean(qRaw);lastQuery=qRaw;if(!q){lastResults=[];renderRecentHistory();return renderState("results","Masukkan kata kunci pencarian.");}saveRecentSearch(qRaw);const words=q.split(" ").filter(Boolean);const out=[];for(const it of CACHE_SKU.values()){if(currentFilter!=="Semua"&&!it.sources.has(currentFilter))continue;const skuRaw=String(it.sku||"");const skuN=clean(skuRaw),nameN=clean(it.nama),combined=`${skuN} ${nameN}`;let rank=99;if(skuN===q)rank=1;else if(/^\d{4}$/.test(q)&&skuRaw.replace(/\D/g,"").endsWith(q))rank=2;else if(skuN.includes(q))rank=3;else if(nameN===q)rank=4;else if(words.length&&words.every(w=>combined.includes(w)))rank=5;else if(words.some(w=>combined.includes(w)))rank=6;if(rank<99)out.push({...it,sources:[...it.sources],rank});}lastResults=out.sort((a,b)=>a.rank-b.rank||a.sku.localeCompare(b.sku)).slice(0,50);renderResults(lastResults,qRaw);}
...
function openSearchModal(){prevRouteBeforeSearch=location.pathname||"/";searchModalOpen=true;navigateTo('/search');syncSearchModalUi(true);setTimeout(()=>searchInput?.focus(),20);renderRecentHistory();}
function closeSearchModal(){searchModalOpen=false;syncSearchModalUi(false);if(location.pathname==='/search')navigateTo(prevRouteBeforeSearch==='/search'?'/':prevRouteBeforeSearch);}
function syncSearchModalUi(open){document.body.classList.toggle('search-modal-open',!!open);document.getElementById('page-search')?.classList.toggle('search-modal',!!open);}
...
function getSkuTotals(rows){const map={};rows.forEach(r=>{const sku=normalizeSku(getVal(r,["sku"]));if(!sku)return;map[sku]=(map[sku]||0)+parseNumber(getVal(r,["qty"]));});return map;}
...
const skuNames={};[...DATA["Kartu Stock"]||[],...masuk,...keluar,...rpl,...bulky].forEach(r=>{const sku=normalizeSku(getVal(r,["sku"]));const nama=getSkuName(r);if(!sku)return;if(!skuNames[sku])skuNames[sku]=new Set();if(nama)skuNames[sku].add(nama);});
...
[rpl,bulky].forEach((list,idx)=>list.forEach(r=>{const sku=normalizeSku(getVal(r,["sku"]));if(sku&&!kartuSet.has(sku))rows.push({severity:'Medium',sku,nama:getSkuName(r)||'-',issue:idx===0?'SKU RPL tidak ada di Kartu Stock':'SKU BULKY tidak ada di Kartu Stock',source:idx===0?'RPL':'BULKY',recommendation:'Sinkronkan master SKU ke Kartu Stock.'});}));
...
[...DATA["Kartu Stock"]||[],...masuk,...keluar,...rpl,...bulky].forEach(r=>{const rawSku=String(getVal(r,["sku"])||'').trim();const sku=normalizeSku(rawSku);const nama=getSkuName(r);const source=SHEETS.find(s=>(DATA[s]||[]).includes(r))||'Unknown';if(!sku||sku.length<3)rows.push({severity:'Low',sku:rawSku||'-',nama:nama||'-',issue:'SKU kosong / invalid',source,recommendation:'Isi SKU valid (min 3 karakter alfanumerik).'});if(!nama)rows.push({severity:'Low',sku:rawSku||'-',nama:'-',issue:'Nama barang kosong',source,recommendation:'Lengkapi nama barang.'});});
return rows;}
