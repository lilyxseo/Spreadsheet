...
if(!["/search","/balikan-store"].includes(location.pathname))return;
...
const MOVEMENT_STATE={sessionActive:false,scanInput:"",sessionItems:[],submitting:false,focusKey:""};
let movementHistoryPage=1;
let movementHistoryPageSize=10;
let MOVEMENT_HISTORY_REMOTE={rows:[],error:"",loaded:false};
function canSubmitMovement(){return MOVEMENT_STATE.sessionItems.length>0&&MOVEMENT_STATE.sessionItems.every(it=>String(it.to||"").trim()&&Number.isFinite(it.stokAktual)&&Number(it.stokAktual)>0);}
function getMovementSourceRows(){return getCycleSourceRows();}
function findMovementSourceBySku(sku){const key=clean(sku);return getMovementSourceRows().find(r=>clean(r.sku)===key)||null;}
...
function renderMovementSession(){return `<div class='cc-section'><div class='mv-toolbar'><input id='movementScanInput' class='form-input' placeholder='Scan SKU...' autocomplete='off' value='${esc(MOVEMENT_STATE.scanInput)}'></div></div><div class='cc-section'><h4>Movement Sementara</h4><div class='table-wrap cc-table-wrap cc-session-wrap'><table><thead><tr><th>SKU</th><th>Nama Barang</th><th>From</th><th>To</th><th>Qty Aktual</th><th>Aksi</th></tr></thead><tbody id='mvSessionBody'></tbody></table></div></div>`;}
function renderMovementPage(){if(!movementApp)return;movementApp.innerHTML=`<div class='card cc-card cc-section'><div class='section-header cc-main-header'><div class='cc-action-stack'>${MOVEMENT_STATE.sessionActive?"<button class='btn-ghost' onclick='mvCancelSession()'>Batal Movement</button>":""}<button class='btn-primary' onclick='mvStartSession()' ${MOVEMENT_STATE.sessionActive?"disabled":""}>Mulai Movement</button>${MOVEMENT_STATE.sessionActive?"<button id='mvSubmitBtn' class='btn-primary' onclick='mvSubmitSession()'>Selesai Movement</button>":""}</div></div>${MOVEMENT_HISTORY_REMOTE.error?`<div class='state cc-state state-error'>${esc(MOVEMENT_HISTORY_REMOTE.error)}</div>`:""}${MOVEMENT_STATE.sessionActive?renderMovementSession():renderMovementEmptyState()}</div><div class='card cc-card cc-section mv-history-section'><div class='section-header'><h4>History Movement</h4></div><div class='table-wrap cc-table-wrap'><table><thead><tr><th>Tanggal</th><th>From</th><th>To</th><th>SKU</th><th>Nama Barang</th><th>Stok Lokasi Awal</th><th>Stok Aktual</th><th>Keterangan</th><th>Aksi</th></tr></thead><tbody id='mvHistoryBody'></tbody></table></div><div class='mv-pagination'><span id='mvHistoryInfo'>Menampilkan 0-0 dari 0 data</span><div class='row'><select id='mvHistoryPageSize'><option value='10'>10</option><option value='25'>25</option><option value='50'>50</option></select><button id='mvHistoryPrev' class='btn-ghost'>Prev</button><button id='mvHistoryNext' class='btn-ghost'>Next</button></div></div></div>`;if(window.lucide)window.lucide.createIcons();if(MOVEMENT_STATE.sessionActive){renderMovementSessionTable();const scanInput=document.getElementById('movementScanInput');if(scanInput)scanInput.focus();}renderMovementHistory();}
window.mvStartSession=()=>{MOVEMENT_STATE.sessionActive=true;MOVEMENT_STATE.scanInput="";MOVEMENT_STATE.sessionItems=[];MOVEMENT_STATE.focusKey="";renderMovementPage();};
window.mvCancelSession=()=>{showConfirmModal({title:'Batal Movement',message:'Batalkan movement yang sedang berjalan?',confirmText:'Ya, Batalkan',cancelText:'Kembali',type:'danger',onConfirm:()=>{MOVEMENT_STATE.sessionActive=false;MOVEMENT_STATE.scanInput="";MOVEMENT_STATE.sessionItems=[];MOVEMENT_STATE.focusKey="";MOVEMENT_STATE.submitting=false;renderMovementPage();}});};
function addMovementItem(source){const item={lokasi:source.lokasi,sku:source.sku,nama:source.nama,stokAwal:source.stok_akhir,to:"",stokAktual:null};const existingIdx=MOVEMENT_STATE.sessionItems.findIndex(it=>clean(it.sku)===clean(item.sku)&&clean(it.lokasi)===clean(item.lokasi);if(existingIdx>=0){const key=`${clean(item.sku)}|${clean(item.lokasi)}`;renderMovementSessionTable();const existingRow=document.querySelector(`[data-mv-row-key="${key}"]`);existingRow?.scrollIntoView({behavior:'smooth',block:'center'});existingRow?.querySelector('[data-mvm="to"]')?.focus();toast('SKU sudah ada di list movement.','error');return false;}MOVEMENT_STATE.sessionItems.push(item);renderMovementSessionTable();toast('SKU berhasil ditambahkan.','success');return true;}
function handleMovementScanSubmit(rawValue){if(!MOVEMENT_STATE.sessionActive)return;const sku=cleanScannedSku(rawValue);if(!sku)return;const source=findMovementSourceBySku(sku);if(!source){toast('SKU tidak ditemukan','error');return;}addMovementItem(source);MOVEMENT_STATE.scanInput="";const scanInput=document.getElementById('movementScanInput');if(scanInput){scanInput.value="";scanInput.focus();}}
...
window.mvSubmitSession=async()=>{if(!canSubmitMovement()){toast('Lengkapi tujuan dan qty aktual (>0) semua item.','error');return;}const dup=new Set();for(const it of MOVEMENT_STATE.sessionItems){const key=`${clean(it.sku)}|${clean(it.lokasi)}|${clean(it.to)}`;if(dup.has(key)){toast('Duplikasi SKU + From + To terdeteksi.','error');return;}dup.add(key);}MOVEMENT_STATE.submitting=true;renderMovementSessionTable();try{for(const it of MOVEMENT_STATE.sessionItems){const tanggal=formatTanggal(new Date());const from=it.lokasi;const to=String(it.to||"").trim();const qty=Number(it.stokAktual);const rawRole=window.currentUser?.role||"";const normalizedRole=normalizeRole(rawRole);const pic=APP_CONFIG.PIC_BY_ROLE[normalizedRole]||APP_CONFIG.PIC_BY_ROLE.inventory||"ABI";const payload={tanggal,from,to,sku:it.sku,namaBarang:it.nama,qty,pic,stokDiLokasiAwal:Number(it.stokAwal),stokAktual:Number(it.stokAktual)};const res=await fetch('/api/movement/in',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal menyimpan barang masuk');logActivitySafe({action:'CREATE_MOVEMENT',module:'Movement',detail:`[CREATE] Movement\nSKU: ${it.sku}\nQty: ${qty}\nStatus: Movement`,status:'SUCCESS',metadata:{inventorySheet:'Movement',inventorySpreadsheet:'SHEET_ID_INVENTORY',syncedTo2026:true,tanggal,from,to,sku:it.sku,namaBarang:it.nama,qty,status:'Movement',pic,keterangan:'INTERNAL STOCK TRANSFER'}});}toast('Movement berhasil disimpan','success');MOVEMENT_STATE.sessionActive=false;MOVEMENT_STATE.scanInput="";MOVEMENT_STATE.sessionItems=[];MOVEMENT_STATE.focusKey="";MOVEMENT_HISTORY_REMOTE.loaded=false;await ensureMovementHistoryLoaded();if(typeof syncData==='function')syncData();}catch(err){toast(err?.message||'Gagal menyimpan barang masuk','error');}finally{MOVEMENT_STATE.submitting=false;renderMovementPage();}};
document.addEventListener('input',e=>{if(e.target?.id==='movementScanInput'){MOVEMENT_STATE.scanInput=e.target.value||"";return;}if(e.target?.id==='mvHistoryPageSize'){movementHistoryPageSize=Number(e.target.value)||10;movementHistoryPage=1;renderMovementHistory();return;}if(e.target?.dataset?.mvm){const idx=Number(e.target.dataset.idx);const row=MOVEMENT_STATE.sessionItems[idx];if(!row)return;if(e.target.dataset.mvm==='to')row.to=e.target.value||"";if(e.target.dataset.mvm==='akt'){const n=Number(e.target.value);row.stokAktual=Number.isFinite(n)?n:null;}const submitBtn=document.getElementById('mvSubmitBtn');if(submitBtn)submitBtn.disabled=!canSubmitMovement()||MOVEMENT_STATE.submitting;}});
document.addEventListener('keydown',e=>{if(e.target?.id==='movementScanInput'&&e.key==='Enter'){e.preventDefault();handleMovementScanSubmit(e.target.value||"");}});
document.addEventListener('click',e=>{if(e.target?.id==='ccHistoryPrev'){cycleHistoryPage=Math.max(1,cycleHistoryPage-1);renderCycleHistory();return;}if(e.target?.id==='ccHistoryNext'){cycleHistoryPage+=1;renderCycleHistory();return;}if(e.target?.id==='mvHistoryPrev'){movementHistoryPage=Math.max(1,movementHistoryPage-1);renderMovementHistory();return;}if(e.target?.id==='mvHistoryNext'){movementHistoryPage+=1;renderMovementHistory();return;}const btn=e.target.closest('[data-mvm-action]');if(!btn)return;const action=btn.dataset.mvmAction;if(action==='remove'){const idx=Number(btn.dataset.idx);MOVEMENT_STATE.sessionItems.splice(idx,1);renderMovementSessionTable();return;}});
