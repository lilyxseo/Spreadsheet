...
import { analyzeWarnings } from "./warning-engine.js";
...
const ANOMALY_CACHE_KEY="warningCacheV2";
const ANOMALY_STATE={page:1,pageSize:25,rows:[],filtered:[],q:"",severity:"all",type:"all",rendered:false,isWarningCalculating:false,warningProgress:null,warningDataVersion:"",warningLastCalculatedAt:0,_searchDebounce:null};
function warningVersion(){const lists=[DATA["Kartu Stock"]||[],getBarangKeluarRows(),getBarangMasukRows()];return lists.map(list=>`${list.length}:${list.at(-1)?.rowNumber||""}`).join("|");}
function sevClass(s){return s==='Critical'?'b-high':s==='Warning'?'b-medium':'b-low';}
function warningCacheGet(){try{return JSON.parse(localStorage.getItem(ANOMALY_CACHE_KEY)||"null");}catch(_){return null;}}
function warningCacheSet(rows,version){try{localStorage.setItem(ANOMALY_CACHE_KEY,JSON.stringify({rows,version,updatedAt:Date.now()}));}catch(_){}}
function setAnomalyLoading(active,text="Memeriksa data terbaru…"){ANOMALY_STATE.isWarningCalculating=active;const el=document.getElementById("anomalyLoading");if(el){el.classList.toggle("hidden",!active);el.textContent=text;}}
...
function filteredWarnings(){const q=clean(ANOMALY_STATE.q);return ANOMALY_STATE.rows.filter(r=>(ANOMALY_STATE.severity==='all'||r.severity===ANOMALY_STATE.severity)&&(ANOMALY_STATE.type==='all'||r.type===ANOMALY_STATE.type)&&(!q||clean(`${r.sku} ${r.nama} ${r.location} ${r.issue}`).includes(q)));}
function renderAnomalySummary(){const rows=ANOMALY_STATE.rows;const count=t=>rows.filter(r=>t(r)).length;anomalySummary.innerHTML=[["Critical",count(r=>r.severity==='Critical')],["Tidak Balance",count(r=>r.type==='balance')],["Input Tidak Terbaca",count(r=>r.type==='input')],["Nama/SKU Tidak Sesuai",count(r=>r.type==='name')],["Lokasi Mencurigakan",count(r=>r.type==='location')]].map(([k,v])=>`<div class='metric'><div class='k'>${k}</div><div class='v'>${v}</div></div>`).join('');}
function renderAnomalyRowsOnly(){const rows=filteredWarnings();ANOMALY_STATE.filtered=rows;const max=Math.max(1,Math.ceil(rows.length/ANOMALY_STATE.pageSize));ANOMALY_STATE.page=Math.min(ANOMALY_STATE.page,max);const page=rows.slice((ANOMALY_STATE.page-1)*ANOMALY_STATE.pageSize,ANOMALY_STATE.page*ANOMALY_STATE.pageSize);const body=document.querySelector('#anomalyTable tbody');if(body)body.innerHTML=page.map((r,i)=>`<tr><td><span class='badge ${sevClass(r.severity)}'>${r.severity}</span></td><td>${esc(r.sku)}</td><td>${esc(r.nama)}</td><td>${esc(r.location)}</td><td>${esc(r.issue)}${r.count>1?` <strong>(${r.count} row)</strong>`:''}</td><td>${esc(r.impact||'-')}</td><td>${esc(r.suggestion||'-')}</td><td><button class='btn-ghost' onclick='openWarningDetail(${(ANOMALY_STATE.page-1)*ANOMALY_STATE.pageSize+i})'>Detail</button></td></tr>`).join('')||"<tr><td colspan='8'><div class='state'>Tidak ada warning penting.</div></td></tr>";const info=document.getElementById('anomalyInfo');if(info)info.textContent=`Menampilkan ${page.length} dari ${rows.length} data`;renderAnomalySummary();}
function applyAnomalyFilters(reset=false){if(reset)ANOMALY_STATE.page=1;ANOMALY_STATE.severity=anomalySeverity?.value||'all';ANOMALY_STATE.type=anomalyType?.value||'all';renderAnomalyRowsOnly();}
function renderAnomalyPage(){if(!ANOMALY_STATE.rendered){anomalyTable.innerHTML=`<div class='row anomaly-toolbar'><div id='anomalyInfo'></div><div class='row'><button class='btn-ghost' onclick='refreshAnomalyInBackground(true)'>Refresh</button><select id='anomalySize'><option>25</option><option>50</option><option>100</option></select><button class='btn-ghost' onclick='changeAnomalyPage(-1)'>Prev</button><button class='btn-ghost' onclick='changeAnomalyPage(1)'>Next</button></div></div><div id='anomalyLoading' class='state hidden'></div><div class='table-wrap table-wrap-full'><table><thead><tr><th>Severity</th><th>SKU</th><th>Nama Barang</th><th>Lokasi</th><th>Masalah</th><th>Dampak</th><th>Saran</th><th>Aksi</th></tr></thead><tbody></tbody></table></div><dialog id='warningDetail'></dialog>`;document.getElementById('anomalySize').onchange=e=>{ANOMALY_STATE.pageSize=Number(e.target.value);renderAnomalyRowsOnly();};ANOMALY_STATE.rendered=true;const cached=warningCacheGet();if(cached?.rows){ANOMALY_STATE.rows=cached.rows;ANOMALY_STATE.warningLastCalculatedAt=cached.updatedAt;renderAnomalyRowsOnly();}}refreshAnomalyInBackground(false);}
async function refreshAnomalyInBackground(force=false){const version=warningVersion();if(ANOMALY_STATE.isWarningCalculating||(!force&&ANOMALY_STATE.warningDataVersion===version))return;ANOMALY_STATE.warningDataVersion=version;setAnomalyLoading(true);try{const result=await analyzeWarnings({kartu:DATA["Kartu Stock"]||[],keluar:getBarangKeluarRows(),masuk:getBarangMasukRows(),fresh:!!window.__isDataReady,onProgress:p=>{ANOMALY_STATE.warningProgress=p;setAnomalyLoading(true,`Memeriksa data terbaru… ${p.phase} ${p.done}/${p.total}`);}});ANOMALY_STATE.rows=result.rows;ANOMALY_STATE.warningLastCalculatedAt=Date.now();warningCacheSet(result.rows,version);renderAnomalyRowsOnly();}finally{setAnomalyLoading(false);}}
function changeAnomalyPage(step){const max=Math.max(1,Math.ceil(ANOMALY_STATE.filtered.length/ANOMALY_STATE.pageSize));ANOMALY_STATE.page=Math.max(1,Math.min(max,ANOMALY_STATE.page+step));renderAnomalyRowsOnly();}
function openWarningDetail(index){const r=ANOMALY_STATE.filtered[index];if(!r)return;const dialog=document.getElementById('warningDetail');dialog.innerHTML=`<div class='card'><h3>${esc(r.issue)}</h3><p><b>Data ditemukan:</b> ${esc(r.sku)} · ${esc(r.nama)} · ${esc(r.location)}</p><p><b>Data seharusnya:</b> ${esc(Array.isArray(r.expected)?r.expected.join(', '):r.expected||r.suggestion||'-')}</p><p><b>Selisih:</b> ${esc(r.difference??'-')}</p><p><b>Sumber sheet:</b> ${esc(r.source)}</p><p><b>Row terkait:</b> ${esc((r.rowNumbers||[]).filter(Boolean).join(', ')||'-')}</p><p><b>Saran pemeriksaan:</b> ${esc(r.suggestion)}</p><button class='btn-ghost' onclick='this.closest("dialog").close()'>Tutup</button></div>`;dialog.showModal();}
window.renderAnomalyPage=renderAnomalyPage;window.changeAnomalyPage=changeAnomalyPage;window.applyAnomalyFilters=applyAnomalyFilters;window.scheduleAnomalySearch=scheduleAnomalySearch;window.refreshAnomalyInBackground=refreshAnomalyInBackground;window.openWarningDetail=openWarningDetail;
