...
window.balikanSort="default";
window.balikanAutoCheck=localStorage.getItem("balikanAutoCheck")!=="0";
...
balikanSort?.addEventListener("change",e=>{window.balikanSort=e.target.value||"default";renderBalikanTable(false);});
balikanResetBtn?.addEventListener("click",resetBalikanControls);
balikanExportBtn?.addEventListener("click",exportBalikanCsv);
balikanAutoCheck?.addEventListener("change",e=>{window.balikanAutoCheck=e.target.checked;localStorage.setItem("balikanAutoCheck",window.balikanAutoCheck?"1":"0");});
if(balikanAutoCheck)balikanAutoCheck.checked=window.balikanAutoCheck===true;
...
async function loadBalikanSheets(){try{const res=await fetch('/api/balikan-store/sheets');const data=await res.json();if(!res.ok)throw new Error(data?.message||'Gagal memuat daftar sheet');BALIKAN_STATE.sheets=Array.isArray(data?.sheets)?data.sheets:[];if(!window.currentTripSheet&&BALIKAN_STATE.sheets.length)window.currentTripSheet=BALIKAN_STATE.sheets[0];if(window.currentTripSheet)await loadBalikanRows();}catch(err){toast(err?.message||'Gagal memuat daftar sheet','error');}}
...
function ensureBalikanFilterState(){if(!BALIKAN_STATE.filterState)BALIKAN_STATE.filterState={page:1,pageSize:1000,openFilterCol:"",columnFilters:{},rows:[]};return BALIKAN_STATE.filterState;}
function applyBalikanTableFilters(rows){return rows;}
...
function getBalikanSortedRows(rows){const mode=window.balikanSort||"default";const list=[...rows];const cmp=(a,b)=>String(a||"").localeCompare(String(b||""),"id",{sensitivity:"base",numeric:true});if(mode==="sku")list.sort((a,b)=>cmp(a.sku,b.sku));if(mode==="barcode")list.sort((a,b)=>cmp(a.barcode,b.barcode));if(mode==="judul")list.sort((a,b)=>cmp(a.namaBarang,b.namaBarang));if(mode==="check")list.sort((a,b)=>Number(isCheckedValue(b.checked))-Number(isCheckedValue(a.checked)));return list;}
function resetBalikanControls(){window.balikanSort="default";if(balikanSort)balikanSort.value="default";renderBalikanTable(true);}
function exportBalikanCsv(){const rows=getBalikanSortedRows((window.BALIKAN_ROWS||[]).map(r=>({...r})));if(!rows.length){toast("Tidak ada data untuk diexport.","error");return;}const headers=["No","SKU","Barcode","Nama Barang","Qty","Rak Tujuan","Lokasi","Stok Bulky","Stok Retail","Status","Keterangan","Checked"];const escCsv=v=>`"${String(v??"").replace(/"/g,'""')}"`;const lines=[headers.join(",")].concat(rows.map(r=>[r.no,r.sku,r.barcode,r.namaBarang,r.qty,r.rakTujuan,r.lokasi,r.stokBulky,r.stokRetail,r.status,r.keterangan,r.checked].map(escCsv).join(",")));const blob=new Blob([lines.join("\n")],{type:"text/csv;charset=utf-8;"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`balikan-store-${window.currentTripSheet||"data"}.csv`;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(a.href);}
...
function renderBalikanTable(){const rows=getBalikanSortedRows((window.BALIKAN_ROWS||[]).map(r=>({...r})));if(!rows.length){balikanTable.innerHTML='<div class="subtitle">Data kosong.</div>';return;}balikanTable.innerHTML=`<div class="balikan-table-wrapper"><table class="balikan-table"><thead><tr><th class="col-check">Centang</th><th>No</th><th>SKU</th><th>Barcode</th><th>Judul Produk</th><th>Qty</th><th>Rak Tujuan</th><th>Lokasi</th><th>Stok Bulky</th><th>Stok Retail</th><th>Status</th><th>Keterangan</th></tr></thead><tbody></tbody></table></div>`;const tbody=balikanTable.querySelector("tbody");if(!tbody)return;const frag=document.createDocumentFragment();rows.forEach(row=>{const tr=document.createElement("tr");const rowNumber=Number(row.rowNumber);tr.id=`balikan-row-${rowNumber}`;tr.dataset.rowNumber=String(rowNumber);if(BALIKAN_STATE.highlightRowNumber===rowNumber)tr.className="balikan-row-highlight";const tdCheck=document.createElement("td");tdCheck.className="col-check";const checkbox=document.createElement("input");checkbox.type="checkbox";checkbox.className="balikan-check";checkbox.checked=isCheckedValue(row.checked);checkbox.addEventListener("change",()=>{toggleBalikanCheck(window.currentTripSheet,row.rowNumber,checkbox.checked);});tdCheck.appendChild(checkbox);tr.appendChild(tdCheck);[row.no,row.sku,row.barcode,row.namaBarang].forEach(value=>{const td=document.createElement("td");td.textContent=String(value??"");tr.appendChild(td);});const editableFields=['qty','rakTujuan','lokasi','stokBulky','stokRetail','status','keterangan'];editableFields.forEach(field=>{tr.appendChild(createEditableCell(row,field,row[field],{onSave:async({row,field,value,oldValue})=>{await updateBalikanCell(window.currentTripSheet,row,field,value,oldValue);}}));});frag.appendChild(tr);});tbody.replaceChildren(frag);}
...
function openBalikanScanner(){openBarcodeScanner("searchInput",handleBalikanScanResult);}
window.openBalikanScanner=openBalikanScanner;

async function handleBalikanScanResult(decodedText){if(navigator.vibrate)navigator.vibrate(100);const sku=cleanScannedSku(decodedText);const row=(window.BALIKAN_ROWS||[]).find(item=>String(item.sku||"").trim().toLowerCase()===String(sku||"").trim().toLowerCase()||String(item.barcode||"").trim().toLowerCase()===String(sku||"").trim().toLowerCase());if(!row){BALIKAN_STATE.highlightRowNumber=null;toast("SKU/Barcode tidak ditemukan di Balikan Store: "+sku,"error");renderBalikanTable();return;}BALIKAN_STATE.highlightRowNumber=Number(row.rowNumber);if(window.balikanAutoCheck)await toggleBalikanCheck(window.currentTripSheet,Number(row.rowNumber),true);renderBalikanTable();setTimeout(()=>{const el=document.querySelector(`[data-row-number="${row.rowNumber}"]`);if(el)el.scrollIntoView({behavior:"smooth",block:"center"});},150);showToast(window.balikanAutoCheck?"SKU berhasil discan dan dicentang: "+sku:"SKU berhasil discan: "+sku,"success");}
