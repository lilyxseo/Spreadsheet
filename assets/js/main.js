...
function buildLocationRows(){const skuMap=new Map();for(const row of (DATA["Kartu Stock"]||[])){const sku=String(getVal(row,["sku"])||"").trim();if(!sku)continue;const nama=getVal(row,["nama barang","nama","item","description"])||"-";const locRaw=String(getLocationValue(row)||"").trim()||"Tanpa Lokasi";const loc=locRaw||"Tanpa Lokasi";if(!(/[58]/.test(loc)))continue;const qty=parseNumber(getVal(row,["stok akhir","qty","closing stock","ending stock","saldo akhir"]));if(qty<=0)continue;const key=`${clean(loc)}__${clean(sku)}`;if(!skuMap.has(key))skuMap.set(key,{lokasi:loc,sku,nama,qty:0,type:getTypeValue(row)});const it=skuMap.get(key);it.qty+=qty;}const grouped={};for(const it of skuMap.values()){if(it.qty<=0)continue;if(!grouped[it.lokasi])grouped[it.lokasi]={lokasi:it.lokasi,skus:[],totalQty:0};grouped[it.lokasi].skus.push(it);grouped[it.lokasi].totalQty+=it.qty;}return Object.values(grouped).filter(g=>g.totalQty>0&&g.skus.length>0).map(g=>{const jumlahSku=g.skus.length,skuKosong=0;const invalid=clean(g.lokasi)==='tanpa lokasi';const status=invalid?'Kosong / Tidak valid':(jumlahSku>=25||g.totalQty>=400?'Padat':'Normal');return {...g,jumlahSku,skuKosong,status};});}
...
const STOK_MINUS_STATE={q:"",filter:"all",sort:"minusDesc",page:1,pageSize:25,selected:"",rows:[]};
...
function renderStokMinusPage(){const all=buildStokMinusRows();STOK_MINUS_STATE.rows=all;let rows=all.filter(r=>{const q=clean(STOK_MINUS_STATE.q);if(q&&!clean(`${r.sku} ${r.nama}`).includes(q))return false;if(STOK_MINUS_STATE.filter==="minus")return r.status==="Stok Minus";if(STOK_MINUS_STATE.filter==="keluar")return r.status==="Keluar Tanpa Masuk";return true;});rows=[...rows].sort(STOK_MINUS_STATE.sort==="keluarDesc"?(a,b)=>b.totalKeluar-a.totalKeluar:STOK_MINUS_STATE.sort==="skuAz"?(a,b)=>a.sku.localeCompare(b.sku):(a,b)=>a.stokEstimate-b.stokEstimate);const minusQty=all.reduce((n,r)=>n+(r.stokEstimate<0?Math.abs(r.stokEstimate):0),0);const onlyOut=all.filter(r=>r.status==="Keluar Tanpa Masuk").length;const topMinus=[...all].sort((a,b)=>a.stokEstimate-b.stokEstimate)[0];stokMinusSummary.innerHTML=[["Total SKU Minus",all.length],["Total Qty Minus",minusQty],["Keluar Tanpa Masuk",onlyOut],["Minus Terbesar",topMinus?`${topMinus.sku} (${topMinus.stokEstimate})`:"-"]].map(c=>`<div class='metric'><div class='k'>${c[0]}</div><div class='v'>${esc(c[1])}</div></div>`).join("");const max=Math.max(1,Math.ceil(rows.length/STOK_MINUS_STATE.pageSize));if(STOK_MINUS_STATE.page>max)STOK_MINUS_STATE.page=max;const pageRows=rows.slice((STOK_MINUS_STATE.page-1)*STOK_MINUS_STATE.pageSize,STOK_MINUS_STATE.page*STOK_MINUS_STATE.pageSize);stokMinusTable.innerHTML=`<div class='mv-toolbar stokminus-toolbar'><input id='minusSearch' class='search-lg' placeholder='Search SKU / nama barang' value='${esc(STOK_MINUS_STATE.q)}'><select id='minusFilter'><option value='all'>Semua</option><option value='minus'>Stok Minus</option><option value='keluar'>Keluar Tanpa Masuk</option></select><select id='minusSort'><option value='minusDesc'>Minus terbesar</option><option value='keluarDesc'>Keluar terbesar</option><option value='skuAz'>SKU A-Z</option></select><select id='minusSize'><option value='25'>25</option><option value='50'>50</option><option value='100'>100</option></select><button class='btn-ghost' onclick='exportStokMinusCsv()'>Export CSV</button></div><div class='table-wrap table-wrap-full'><table><thead><tr><th>SKU</th><th>Nama Barang</th><th>Total Masuk</th><th>Total Keluar</th><th>Selisih</th><th>Status</th><th>Action Lacak</th></tr></thead><tbody>${pageRows.map(r=>`<tr><td>${esc(r.sku)}</td><td>${esc(r.nama)}</td><td>${r.totalMasuk}</td><td>${r.totalKeluar}</td><td class='${r.stokEstimate<0?"txt-danger":""}'>${r.stokEstimate}</td><td><span class='badge ${r.status==="Stok Minus"?"b-out":"b-warn"}'>${esc(r.status)}</span></td><td><button class='btn-ghost' onclick="openStokMinusTrace('${encAttr(r.sku)}')">Lacak</button></td></tr>`).join("")||"<tr><td colspan='7'><div class='state'>Tidak ada data.</div></td></tr>"}</tbody></table></div><div class='mv-pagination'><span>Menampilkan ${rows.length?((STOK_MINUS_STATE.page-1)*STOK_MINUS_STATE.pageSize+1):0}–${Math.min(STOK_MINUS_STATE.page*STOK_MINUS_STATE.pageSize,rows.length)} dari ${rows.length} data</span><div class='row'><button class='btn-ghost' onclick='changeStokMinusPage(-1)'>Prev</button><button class='btn-ghost' onclick='changeStokMinusPage(1)'>Next</button></div></div>`;document.getElementById('minusFilter').value=STOK_MINUS_STATE.filter;document.getElementById('minusSort').value=STOK_MINUS_STATE.sort;document.getElementById('minusSize').value=String(STOK_MINUS_STATE.pageSize);document.getElementById('minusSearch')?.addEventListener('input',debounce(e=>{STOK_MINUS_STATE.q=e.target.value;STOK_MINUS_STATE.page=1;renderStokMinusPage();},180));document.getElementById('minusFilter')?.addEventListener('change',e=>{STOK_MINUS_STATE.filter=e.target.value;STOK_MINUS_STATE.page=1;renderStokMinusPage();});document.getElementById('minusSort')?.addEventListener('change',e=>{STOK_MINUS_STATE.sort=e.target.value;renderStokMinusPage();});document.getElementById('minusSize')?.addEventListener('change',e=>{STOK_MINUS_STATE.pageSize=Number(e.target.value)||25;STOK_MINUS_STATE.page=1;renderStokMinusPage();});if(STOK_MINUS_STATE.selected)openStokMinusTrace(encAttr(STOK_MINUS_STATE.selected),true);}
function changeStokMinusPage(delta){const max=Math.max(1,Math.ceil((STOK_MINUS_STATE.rows||[]).length/STOK_MINUS_STATE.pageSize));STOK_MINUS_STATE.page=Math.min(max,Math.max(1,STOK_MINUS_STATE.page+delta));renderStokMinusPage();}
...
const CYCLE_STORE_KEY="inventory_cycle_count_draft_v1";
const CYCLE_STATE={step:1,search:"",location:"all",category:"all",page:1,pageSize:20,session:createCycleSession(),rows:[],locked:false};
function createCycleSession(){const d=new Date();const day=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;return{name:"",date:day,pic:"",location:"",notes:"",status:"draft"};}
function getCycleCatalog(){const m=new Map();for(const r of (DATA["Kartu Stock"]||[])){const sku=String(getVal(r,["sku"])||"").trim();if(!sku)continue;const key=clean(sku);const nama=String(getVal(r,["nama barang","nama","item","description"])||"-").trim()||"-";const lokasi=String(getVal(r,["lokasi","location","rak","bin","area"])||"-").trim()||"-";const category=String(getVal(r,["kategori","category","jenis"])||"General").trim()||"General";const stokSistem=parseNumber(getVal(r,["stok akhir","qty","closing stock","ending stock","saldo akhir"]));if(!m.has(key))m.set(key,{sku,nama,lokasi,category,stokSistem:0});const it=m.get(key);it.stokSistem+=stokSistem;}return [...m.values()];}
function calculateDifference(row){const actual=Number.isFinite(Number(row.qtyAktual))?Number(row.qtyAktual):0;const selisih=actual-row.stokSistem;const status=selisih===0?"Cocok":(selisih>0?"Plus":"Minus");return {...row,qtyAktual:actual,selisih,status};}
function addSkuToSession(item){if(!item?.sku)return;const key=clean(item.sku);if(CYCLE_STATE.rows.some(r=>clean(r.sku)===key))return;CYCLE_STATE.rows.push(calculateDifference({sku:item.sku,nama:item.nama,lokasi:item.lokasi,stokSistem:item.stokSistem,qtyAktual:0,selisih:0,status:"Cocok",notes:"",category:item.category}));saveCycleDraft(false);}
function updateActualQty(sku,qty){const key=clean(sku);const row=CYCLE_STATE.rows.find(r=>clean(r.sku)===key);if(!row)return;Object.assign(row,calculateDifference({...row,qtyAktual:qty}));saveCycleDraft(false);}
function getCycleSummary(){const total=CYCLE_STATE.rows.length;const cocok=CYCLE_STATE.rows.filter(r=>r.status==="Cocok").length;const selisihSku=total-cocok;const totalSelisih=CYCLE_STATE.rows.reduce((n,r)=>n+r.selisih,0);const akurasi=total?((cocok/total)*100):0;return{total,cocok,selisihSku,totalSelisih,akurasi};}
function finalizeSession(){CYCLE_STATE.locked=true;CYCLE_STATE.session.status="selesai";saveCycleDraft(true);}
function exportCycleCountCsv(){const rows=CYCLE_STATE.rows;if(!rows.length)return toast("Belum ada SKU untuk diexport","warning");const head=["SKU","Nama Barang","Lokasi","Stok Sistem","Qty Aktual","Selisih","Status","Catatan"];const body=rows.map(r=>[r.sku,r.nama,r.lokasi,r.stokSistem,r.qtyAktual,r.selisih,r.status,r.notes||""]);const csv=[head,...body].map(cols=>cols.map(v=>`"${String(v??"").replaceAll('"','""')}"`).join(",")).join("
");downloadFile(`cycle-count-${Date.now()}.csv`,csv,"text/csv;charset=utf-8;");}
function saveCycleDraft(showToast=true){const payload={session:CYCLE_STATE.session,rows:CYCLE_STATE.rows,locked:CYCLE_STATE.locked,step:CYCLE_STATE.step};localStorage.setItem(CYCLE_STORE_KEY,JSON.stringify(payload));if(showToast)toast("Draft cycle count tersimpan","success",false);}
function loadCycleDraft(){try{const raw=localStorage.getItem(CYCLE_STORE_KEY);if(!raw)return;const d=JSON.parse(raw);if(d?.session)CYCLE_STATE.session={...createCycleSession(),...d.session};if(Array.isArray(d?.rows))CYCLE_STATE.rows=d.rows.map(calculateDifference);CYCLE_STATE.locked=!!d?.locked;CYCLE_STATE.step=Number(d?.step)||1;}catch(_){}}
const cycleSearchDebounce=debounce(v=>{CYCLE_STATE.search=v;CYCLE_STATE.page=1;renderCycleCountPage();},250);
function renderCycleCountPage(){if(!cycleCountPage)return;loadCycleDraft();const summary=getCycleSummary();const badge=CYCLE_STATE.session.status==="selesai"?"badge b-in":"badge b-warn";const stepTitles=["Setup","Pilih SKU","Hitung Aktual","Review"];const catalog=getCycleCatalog();let filtered=catalog.filter(x=>{if(CYCLE_STATE.search&&!clean(`${x.sku} ${x.nama}`).includes(clean(CYCLE_STATE.search)))return false;if(CYCLE_STATE.location!=="all"&&x.lokasi!==CYCLE_STATE.location)return false;if(CYCLE_STATE.category!=="all"&&x.category!==CYCLE_STATE.category)return false;return true;});const maxPage=Math.max(1,Math.ceil(filtered.length/CYCLE_STATE.pageSize));if(CYCLE_STATE.page>maxPage)CYCLE_STATE.page=maxPage;const paged=filtered.slice((CYCLE_STATE.page-1)*CYCLE_STATE.pageSize,CYCLE_STATE.page*CYCLE_STATE.pageSize);
cycleCountPage.innerHTML=`<div class='cycle-shell'><div class='cycle-header card'><div><h3 class='page-title'>Cycle Count</h3><p class='subtitle'>Hitung stok fisik dan bandingkan dengan stok sistem tanpa fetch ulang data.</p></div><span class='${badge}'>${CYCLE_STATE.session.status}</span></div><div class='grid dashboard cycle-summary'>${[["Total SKU",summary.total],["SKU Cocok",summary.cocok],["SKU Selisih",summary.selisihSku],["Total Selisih",summary.totalSelisih],["Akurasi",summary.akurasi.toFixed(2)+"%"]].map(x=>`<div class='metric'><div class='k'>${x[0]}</div><div class='v'>${x[1]}</div></div>`).join("")}</div><div class='cycle-steps'>${stepTitles.map((t,i)=>`<button class='cycle-step ${CYCLE_STATE.step===i+1?"active":""}' data-cycle-step='${i+1}'>${i+1}. ${t}</button>`).join("")}</div>${renderCycleStepContent(paged,filtered,catalog)}</div>`;
cycleCountPage.querySelectorAll('[data-cycle-step]').forEach(b=>b.onclick=()=>{CYCLE_STATE.step=Number(b.dataset.cycleStep);saveCycleDraft(false);renderCycleCountPage();});bindCycleEvents(catalog);
if(window.lucide)lucide.createIcons();}
function renderCycleStepContent(paged,filtered,catalog){if(CYCLE_STATE.step===1){return `<section class='card cycle-card'><div class='cycle-form'><input id='ccName' class='search-lg' placeholder='Nama sesi' value='${esc(CYCLE_STATE.session.name)}' ${CYCLE_STATE.locked?"disabled":""}><input id='ccDate' type='date' class='search-lg' value='${esc(CYCLE_STATE.session.date)}' ${CYCLE_STATE.locked?"disabled":""}><input id='ccPic' class='search-lg' placeholder='PIC' value='${esc(CYCLE_STATE.session.pic)}' ${CYCLE_STATE.locked?"disabled":""}><input id='ccLoc' class='search-lg' placeholder='Lokasi (opsional)' value='${esc(CYCLE_STATE.session.location)}' ${CYCLE_STATE.locked?"disabled":""}><textarea id='ccNotes' class='search-lg' placeholder='Catatan'>${esc(CYCLE_STATE.session.notes)}</textarea></div><div class='row'><button class='btn-ghost' id='ccSave'>Simpan Draft</button><button class='btn-primary' id='ccNext'>Lanjut</button></div></section>`;}
if(CYCLE_STATE.step===2){const locOpt=["all",...new Set(catalog.map(c=>c.lokasi))];const catOpt=["all",...new Set(catalog.map(c=>c.category))];return `<section class='card cycle-card'><div class='mv-toolbar'><input id='ccSearch' class='search-lg' placeholder='Search SKU / nama barang' value='${esc(CYCLE_STATE.search)}'><select id='ccFilterLoc'>${locOpt.map(x=>`<option value='${encAttr(x)}'>${esc(x)}</option>`).join("")}</select><select id='ccFilterCat'>${catOpt.map(x=>`<option value='${encAttr(x)}'>${esc(x)}</option>`).join("")}</select><button class='btn-ghost' id='ccAddMinus'>Ambil stok minus</button><button class='btn-ghost' id='ccAddAnomaly'>Ambil anomaly</button></div>${!filtered.length?"<div class='state'>Belum ada SKU. Coba ubah filter atau kata kunci.</div>":`<div class='table-wrap'><table><thead><tr><th>SKU</th><th>Nama</th><th>Lokasi</th><th>Kategori</th><th>Stok Sistem</th><th></th></tr></thead><tbody>${paged.map(r=>`<tr><td>${esc(r.sku)}</td><td>${esc(r.nama)}</td><td>${esc(r.lokasi)}</td><td>${esc(r.category)}</td><td>${r.stokSistem}</td><td><button class='btn-ghost' data-add-sku='${encAttr(r.sku)}'>Tambah</button></td></tr>`).join("")}</tbody></table></div><div class='mv-pagination'><span>${filtered.length} SKU</span><div class='row'><button class='btn-ghost' id='ccPrev'>Prev</button><button class='btn-ghost' id='ccNextPage'>Next</button></div></div>`}</section>`;}
if(CYCLE_STATE.step===3){return `<section class='card cycle-card'>${!CYCLE_STATE.rows.length?"<div class='state'>Belum ada SKU dipilih.</div>":`<div class='table-wrap'><table><thead><tr><th>SKU</th><th>Nama Barang</th><th>Lokasi</th><th>Stok Sistem</th><th>Qty Aktual</th><th>Selisih</th><th>Status</th><th>Catatan</th></tr></thead><tbody>${CYCLE_STATE.rows.slice((CYCLE_STATE.page-1)*CYCLE_STATE.pageSize,CYCLE_STATE.page*CYCLE_STATE.pageSize).map(r=>`<tr class='${r.selisih!==0?"cycle-row-diff":""}'><td>${esc(r.sku)}</td><td>${esc(r.nama)}</td><td>${esc(r.lokasi)}</td><td>${r.stokSistem}</td><td><input data-actual='${encAttr(r.sku)}' class='cc-qty' value='${r.qtyAktual}' ${CYCLE_STATE.locked?"disabled":""}></td><td>${r.selisih}</td><td><span class='badge ${r.status==="Cocok"?"b-in":(r.status==="Plus"?"b-warn":"b-out")}'>${r.status}</span></td><td><input data-note='${encAttr(r.sku)}' value='${esc(r.notes||"")}' ${CYCLE_STATE.locked?"disabled":""}></td></tr>`).join("")}</tbody></table></div>`}<div class='row'><button class='btn-ghost' id='ccSave2'>Simpan Draft</button><button class='btn-primary' id='ccToReview'>Lanjut</button></div></section>`;}
return `<section class='card cycle-card'><div class='row'><button class='btn-ghost' id='ccExport'>Export CSV</button><button class='btn-primary' id='ccFinalize' ${CYCLE_STATE.locked?"disabled":""}>Selesaikan Cycle Count</button></div></section>`;}
function bindCycleEvents(catalog){document.getElementById('ccSave')?.addEventListener('click',()=>{syncCycleSessionForm();saveCycleDraft(true);});document.getElementById('ccNext')?.addEventListener('click',()=>{syncCycleSessionForm();CYCLE_STATE.step=2;renderCycleCountPage();});document.getElementById('ccSearch')?.addEventListener('input',e=>cycleSearchDebounce(e.target.value));document.getElementById('ccFilterLoc')?.addEventListener('change',e=>{CYCLE_STATE.location=e.target.value;CYCLE_STATE.page=1;renderCycleCountPage();});document.getElementById('ccFilterCat')?.addEventListener('change',e=>{CYCLE_STATE.category=e.target.value;CYCLE_STATE.page=1;renderCycleCountPage();});document.getElementById('ccPrev')?.addEventListener('click',()=>{CYCLE_STATE.page=Math.max(1,CYCLE_STATE.page-1);renderCycleCountPage();});document.getElementById('ccNextPage')?.addEventListener('click',()=>{CYCLE_STATE.page++;renderCycleCountPage();});document.querySelectorAll('[data-add-sku]').forEach(b=>b.addEventListener('click',()=>{const item=catalog.find(c=>clean(c.sku)===clean(decodeURIComponent(b.dataset.addSku)));if(item)addSkuToSession(item);toast('SKU ditambahkan','success',false);}));document.getElementById('ccAddMinus')?.addEventListener('click',()=>{buildStokMinusRows().forEach(r=>addSkuToSession(r));toast('SKU stok minus ditambahkan','success',false);});document.getElementById('ccAddAnomaly')?.addEventListener('click',()=>{buildAnomalyRows().forEach(r=>addSkuToSession({sku:r.sku,nama:r.nama,lokasi:'-',category:'Anomaly',stokSistem:0}));toast('SKU anomaly ditambahkan','success',false);});document.querySelectorAll('[data-actual]').forEach(i=>i.addEventListener('input',e=>updateActualQty(decodeURIComponent(i.dataset.actual),parseNumber(e.target.value||0))));document.querySelectorAll('[data-note]').forEach(i=>i.addEventListener('input',e=>{const row=CYCLE_STATE.rows.find(r=>clean(r.sku)===clean(decodeURIComponent(i.dataset.note)));if(!row)return;row.notes=e.target.value;saveCycleDraft(false);}));document.getElementById('ccSave2')?.addEventListener('click',()=>saveCycleDraft(true));document.getElementById('ccToReview')?.addEventListener('click',()=>{CYCLE_STATE.step=4;renderCycleCountPage();});document.getElementById('ccExport')?.addEventListener('click',exportCycleCountCsv);document.getElementById('ccFinalize')?.addEventListener('click',()=>{finalizeSession();toast('Cycle count selesai','success');renderCycleCountPage();});}
function syncCycleSessionForm(){const map={name:'ccName',date:'ccDate',pic:'ccPic',location:'ccLoc',notes:'ccNotes'};Object.entries(map).forEach(([k,id])=>{const el=document.getElementById(id);if(el)CYCLE_STATE.session[k]=el.value;});}
window.exportCycleCountCsv=exportCycleCountCsv;
