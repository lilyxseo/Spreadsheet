...
const CYCLE_KEY='inventory_cycle_count_draft_v1';
const CYCLE_STATE={step:1,history:[],session:createCycleSession(),skuQuery:'',location:'all',category:'all',page:1,pageSize:12};
const debouncedCycleSearch=debounce((v)=>{CYCLE_STATE.skuQuery=v;CYCLE_STATE.page=1;renderCycleCountPage();},220);
function bindCycleCountEvents(){
  document.addEventListener('click',e=>{const btn=e.target.closest('[data-cycle-action]');if(!btn)return;const a=btn.dataset.cycleAction;
    if(a==='step'){CYCLE_STATE.step=Number(btn.dataset.step)||1;return renderCycleCountPage();}
    if(a==='add-manual'){const sku=(document.getElementById('ccManualSku')?.value||'').trim();if(sku)addSkuToSession({sku,nama:'Manual',lokasi:'-','stokSistem':0});return;}
    if(a==='add-stock-minus'){getStockMinusRows().slice(0,50).forEach(addSkuToSession);return;}
    if(a==='add-anomaly'){getAnomalyRows().slice(0,50).forEach(addSkuToSession);return;}
    if(a==='pick-sku'){const i=Number(btn.dataset.idx);const row=getSkuPool()[i];if(row)addSkuToSession(row);return;}
    if(a==='prev'||a==='next'){const max=Math.max(1,Math.ceil(CYCLE_STATE.session.items.length/CYCLE_STATE.pageSize));CYCLE_STATE.page=a==='prev'?Math.max(1,CYCLE_STATE.page-1):Math.min(max,CYCLE_STATE.page+1);return renderCycleCountPage();}
    if(a==='save-draft'){saveCycleDraft(true);}
    if(a==='continue'){CYCLE_STATE.step=Math.min(4,CYCLE_STATE.step+1);return renderCycleCountPage();}
    if(a==='finalize'){finalizeSession();return;}
    if(a==='export'){exportCycleCountCsv();}
  });
  document.addEventListener('input',e=>{if(e.target?.id==='ccSearchSku')debouncedCycleSearch(e.target.value);if(e.target?.matches('[data-cc-meta]')){CYCLE_STATE.session[e.target.dataset.ccMeta]=e.target.value;saveCycleDraft(false);}if(e.target?.matches('[data-cc-qty]'))updateActualQty(e.target.dataset.sku,e.target.value);if(e.target?.matches('[data-cc-note]')){const item=CYCLE_STATE.session.items.find(x=>x.sku===e.target.dataset.sku);if(item){item.catatan=e.target.value;saveCycleDraft(false);}}});
  document.addEventListener('change',e=>{if(e.target?.id==='ccFilterLokasi'){CYCLE_STATE.location=e.target.value;renderCycleCountPage();}if(e.target?.id==='ccFilterKategori'){CYCLE_STATE.category=e.target.value;renderCycleCountPage();}});
}
bindCycleCountEvents();loadCycleDraft();
function createCycleSession(){return {id:`CC-${Date.now()}`,namaSesi:'Cycle Count Harian',tanggal:new Date().toISOString().slice(0,10),pic:'',lokasi:'',catatan:'',status:'draft',items:[],locked:false};}
function getSkuPool(){const rows=[...(DATA['RPL']||[]),...(DATA['BULKY']||[]),...(DATA['Kartu Stock']||[])];const mapped=rows.map(r=>({sku:getVal(r,['sku']),nama:getVal(r,['nama barang','nama','item','description'])||'-',lokasi:getVal(r,['lokasi','location'])||'-',kategori:getVal(r,['kategori','category'])||'-',stokSistem:parseNumber(getVal(r,['stok global','stok akhir','qty','stok']))||0})).filter(r=>r.sku);
return mapped.filter(r=>(CYCLE_STATE.location==='all'||r.lokasi===CYCLE_STATE.location)&&(CYCLE_STATE.category==='all'||r.kategori===CYCLE_STATE.category)&&clean(`${r.sku} ${r.nama}`).includes(clean(CYCLE_STATE.skuQuery||''))).slice(0,80);
}
function getStockMinusRows(){return getSkuPool().filter(r=>r.stokSistem<0);} function getAnomalyRows(){return getSkuPool().filter(r=>Math.abs(r.stokSistem)>1000);} 
function addSkuToSession(row){if(!row?.sku)return;const exists=CYCLE_STATE.session.items.some(x=>x.sku===row.sku);if(exists)return;CYCLE_STATE.session.items.push({sku:row.sku,nama:row.nama,lokasi:row.lokasi,stokSistem:Number(row.stokSistem)||0,qtyAktual:0,selisih:0,status:'Cocok',catatan:''});calculateDifference();saveCycleDraft(true);}
function updateActualQty(sku,val){const item=CYCLE_STATE.session.items.find(x=>x.sku===sku);if(!item)return;item.qtyAktual=Number(val)||0;calculateDifference();saveCycleDraft(false);renderCycleCountPage();}
function calculateDifference(){CYCLE_STATE.session.items.forEach(it=>{it.selisih=(Number(it.qtyAktual)||0)-(Number(it.stokSistem)||0);it.status=it.selisih===0?'Cocok':(it.selisih>0?'Plus':'Minus');});}
function getCycleSummary(){const items=CYCLE_STATE.session.items,total=items.length,cocok=items.filter(x=>x.status==='Cocok').length,selisih=total-cocok,totalSelisih=items.reduce((n,x)=>n+x.selisih,0),akurasi=total?((cocok/total)*100):0;return{total,cocok,selisih,totalSelisih,akurasi};}
function finalizeSession(){CYCLE_STATE.session.locked=true;CYCLE_STATE.session.status='selesai';CYCLE_STATE.history.unshift(JSON.parse(JSON.stringify(CYCLE_STATE.session)));saveCycleDraft(true);renderCycleCountPage();}
function exportCycleCountCsv(){const rows=CYCLE_STATE.session.items;const header='SKU,Nama Barang,Lokasi,Stok Sistem,Qty Aktual,Selisih,Status,Catatan';const body=rows.map(r=>[r.sku,r.nama,r.lokasi,r.stokSistem,r.qtyAktual,r.selisih,r.status,(r.catatan||'').replaceAll(',', ';')].join(','));const csv=[header,...body].join('\n');const blob=new Blob([csv],{type:'text/csv;charset=utf-8;'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`cycle-count-${CYCLE_STATE.session.id}.csv`;a.click();URL.revokeObjectURL(a.href);} 
function saveCycleDraft(showToast){localStorage.setItem(CYCLE_KEY,JSON.stringify(CYCLE_STATE));if(showToast)toast('Draft cycle count tersimpan');}
function loadCycleDraft(){try{const raw=localStorage.getItem(CYCLE_KEY);if(!raw)return;const parsed=JSON.parse(raw);if(parsed?.session){Object.assign(CYCLE_STATE,parsed);calculateDifference();}}catch(_){}}
function renderCycleCountPage(){if(!cycleCountRoot)return;const s=CYCLE_STATE.session,sum=getCycleSummary();const pool=getSkuPool();const step=['Setup','Pilih SKU','Hitung Aktual','Review'];const start=(CYCLE_STATE.page-1)*CYCLE_STATE.pageSize,paged=s.items.slice(start,start+CYCLE_STATE.pageSize);
cycleCountRoot.innerHTML=`<div class='cc-wrap'><div class='cc-head'><div><h3 class='page-title'>Cycle Count</h3><div class='subtitle'>Hitung stok fisik vs stok sistem tanpa fetch ulang data.</div></div><span class='badge ${s.status==='selesai'?'b-kartu':'b-bulky'}'>${s.status==='selesai'?'Selesai':'Draft'}</span></div><div class='cc-summary'>${[['Total SKU',sum.total],['Cocok',sum.cocok],['Selisih',sum.selisih],['Akurasi',sum.akurasi.toFixed(1)+'%']].map(x=>`<div class='metric'><div class='k'>${x[0]}</div><div class='v'>${x[1]}</div></div>`).join('')}</div><div class='cc-steps'>${step.map((x,i)=>`<button class='btn-ghost ${CYCLE_STATE.step===i+1?'active':''}' data-cycle-action='step' data-step='${i+1}'>${i+1}. ${x}</button>`).join('')}</div>${CYCLE_STATE.step===1?renderCcSetup(s):''}${CYCLE_STATE.step===2?renderCcPick(pool):''}${CYCLE_STATE.step===3?renderCcCount(paged,sum,s.items.length):''}${CYCLE_STATE.step===4?renderCcReview(sum):''}</div>`;
}
function renderCcSetup(s){return `<div class='card cc-card'><div class='cc-grid'><input data-cc-meta='namaSesi' value='${esc(s.namaSesi)}' placeholder='Nama sesi'><input type='date' data-cc-meta='tanggal' value='${esc(s.tanggal)}'><input data-cc-meta='pic' value='${esc(s.pic)}' placeholder='PIC'><input data-cc-meta='lokasi' value='${esc(s.lokasi)}' placeholder='Lokasi (opsional)'><textarea data-cc-meta='catatan' placeholder='Catatan'>${esc(s.catatan)}</textarea></div><div class='row'><button class='btn-ghost' data-cycle-action='save-draft'>Simpan Draft</button><button class='btn-primary' data-cycle-action='continue'>Lanjut</button></div></div>`;}
function renderCcPick(pool){return `<div class='card cc-card'><div class='cc-toolbar'><input id='ccSearchSku' class='search-lg' placeholder='Search SKU / nama barang'><select id='ccFilterLokasi'><option value='all'>Semua Lokasi</option></select><select id='ccFilterKategori'><option value='all'>Semua Kategori</option></select><input id='ccManualSku' placeholder='Tambah SKU manual'><button class='btn-ghost' data-cycle-action='add-manual'>Tambah</button><button class='btn-ghost' data-cycle-action='add-stock-minus'>Ambil stok minus</button><button class='btn-ghost' data-cycle-action='add-anomaly'>Ambil anomaly</button></div><div class='table-wrap'>${pool.length?`<table><thead><tr><th>SKU</th><th>Nama</th><th>Lokasi</th><th></th></tr></thead><tbody>${pool.slice(0,20).map((r,i)=>`<tr><td>${esc(r.sku)}</td><td>${esc(r.nama)}</td><td>${esc(r.lokasi)}</td><td><button class='btn-ghost' data-cycle-action='pick-sku' data-idx='${i}'>Tambah</button></td></tr>`).join('')}</tbody></table>`:`<div class='state'>Belum ada SKU. Gunakan pencarian atau tambah manual.</div>`}</div><div class='row'><button class='btn-primary' data-cycle-action='continue'>Lanjut</button></div></div>`;}
function renderCcCount(rows,sum,totalRows){const start=(CYCLE_STATE.page-1)*CYCLE_STATE.pageSize;return `<div class='card cc-card'><div class='table-wrap table-wrap-full'><table><thead><tr><th>SKU</th><th>Nama</th><th>Lokasi</th><th>Sistem</th><th>Aktual</th><th>Selisih</th><th>Status</th><th>Catatan</th></tr></thead><tbody>${rows.map(r=>`<tr class='${r.selisih!==0?'row-mismatch-soft':''}'><td>${esc(r.sku)}</td><td>${esc(r.nama)}</td><td>${esc(r.lokasi)}</td><td>${r.stokSistem}</td><td><input class='cc-qty' data-cc-qty data-sku='${esc(r.sku)}' value='${r.qtyAktual}'></td><td>${r.selisih}</td><td><span class='badge ${r.status==='Cocok'?'cc-ok':r.status==='Plus'?'cc-plus':'cc-minus'}'>${r.status}</span></td><td><input data-cc-note data-sku='${esc(r.sku)}' value='${esc(r.catatan||'')}'></td></tr>`).join('')||`<tr><td colspan='8'><div class='state'>Belum ada SKU yang dipilih.</div></td></tr>`}</tbody></table></div><div class='mv-pagination'><span>Menampilkan ${totalRows?start+1:0}-${Math.min(start+CYCLE_STATE.pageSize,totalRows)} dari ${totalRows}</span><div class='row'><button class='btn-ghost' data-cycle-action='prev'>Prev</button><button class='btn-ghost' data-cycle-action='next'>Next</button></div></div><div class='row'><button class='btn-ghost' data-cycle-action='save-draft'>Simpan Draft</button><button class='btn-primary' data-cycle-action='continue'>Lanjut</button></div></div>`;}
function renderCcReview(sum){return `<div class='card cc-card'><div class='summary-grid'>${[['Total SKU dihitung',sum.total],['SKU cocok',sum.cocok],['SKU selisih',sum.selisih],['Total selisih qty',sum.totalSelisih],['Akurasi %',sum.akurasi.toFixed(2)]].map(x=>`<div class='summary-card'><div class='k'>${x[0]}</div><div class='v'>${x[1]}</div></div>`).join('')}</div><div class='row'><button class='btn-ghost' data-cycle-action='export'>Export CSV</button><button class='btn-primary' data-cycle-action='finalize'>Selesaikan Cycle Count</button></div></div>`;}
