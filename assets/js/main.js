...
const bulkDelete=e.target.closest('[data-mv-bulk-delete]');if(bulkDelete){const mode=bulkDelete.dataset.mvBulkDelete,rows=[...TABLE_STATE[mode].selected];if(!rows.length)return;showConfirmModal({title:'Bulk Delete',message:`Hapus ${rows.length} item?`,confirmText:'Hapus',cancelText:'Batal',type:'danger',onConfirm:async()=>{try{const res=await fetch(mode==='in'?'/api/barang-masuk/bulk-delete':'/api/barang-keluar/bulk-delete',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({rowNumbers:rows})});const out=await res.json();if(!res.ok||!out?.success)throw new Error(out?.message||'Gagal bulk delete');TABLE_STATE[mode].rows=TABLE_STATE[mode].rows.filter(r=>!TABLE_STATE[mode].selected.has(r.rowNumber));TABLE_STATE[mode].selected.clear();renderDataTablePage(mode,mode==='in'?'Barang Masuk':'Barang Keluar',true);logActivitySafe({action:mode==='in'?'BULK_DELETE_BARANG_MASUK':'BULK_DELETE_BARANG_KELUAR',module:mode,status:'SUCCESS'});}catch(err){toast(err?.message||'Gagal bulk delete','error');}}});return;}
const bulkEdit=e.target.closest('[data-mv-bulk-edit]');if(bulkEdit){const mode=bulkEdit.dataset.mvBulkEdit,rows=[...TABLE_STATE[mode].selected];const updates={};const status=prompt('Status baru (opsional):','');const pic=prompt('PIC baru (opsional):','');const keterangan=prompt('Keterangan baru (opsional):','');if(status)updates.status=status;if(pic)updates.pic=pic;if(keterangan)updates.keterangan=keterangan;if(!Object.keys(updates).length)return;fetch(mode==='in'?'/api/barang-masuk/bulk-update':'/api/barang-keluar/bulk-update',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({rowNumbers:rows,updates})}).then(async r=>({ok:r.ok,out:await r.json()})).then(({ok,out})=>{if(!ok||!out?.success)throw new Error(out?.message||'Gagal bulk edit');TABLE_STATE[mode].rows.forEach(r=>{if(TABLE_STATE[mode].selected.has(r.rowNumber))Object.assign(r,updates);});TABLE_STATE[mode].selected.clear();renderDataTablePage(mode,mode==='in'?'Barang Masuk':'Barang Keluar',true);logActivitySafe({action:mode==='in'?'BULK_EDIT_BARANG_MASUK':'BULK_EDIT_BARANG_KELUAR',module:mode,status:'SUCCESS'});}).catch(err=>toast(err?.message||'Gagal bulk edit','error'));return;}
...
document.addEventListener('change',e=>{const sel=e.target.closest('[data-mv-select]');if(sel){const mode=sel.dataset.mvSelect,row=Number(sel.dataset.row);if(sel.checked)TABLE_STATE[mode].selected.add(row);else TABLE_STATE[mode].selected.delete(row);renderDataTablePage(mode,mode==='in'?'Barang Masuk':'Barang Keluar',true);return;}const all=e.target.closest('[data-mv-select-all]');if(all){const mode=all.dataset.mvSelectAll;const st=TABLE_STATE[mode];const pageRows=st.filtered.slice((st.page-1)*st.pageSize,st.page*st.pageSize);pageRows.forEach(r=>all.checked?st.selected.add(r.rowNumber):st.selected.delete(r.rowNumber));renderDataTablePage(mode,mode==='in'?'Barang Masuk':'Barang Keluar',true);}});
...
const TABLE_STATE={in:{page:1,pageSize:25,rows:[],filtered:[],openFilterCol:"",selected:new Set()},out:{page:1,pageSize:25,rows:[],filtered:[],openFilterCol:"",selected:new Set()}};
