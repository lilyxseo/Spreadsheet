...
const BALIKAN_STATE={sheets:[],highlightRowNumber:null,scanAutoCheck:getBalikanAutoCheckDefault()};
window.BALIKAN_ROWS=[];
...
if(e.target.closest("[data-col-filter-menu]")){const menu=e.target.closest("[data-col-filter-menu]");const mode=menu.dataset.mode,col=menu.dataset.col;const st=mode==='balikan'?ensureBalikanFilterState():(TABLE_STATE[mode]||{});ensureColumnFilterState(mode);if(e.target.matches("[data-col-filter-clear]")){st.columnFilters[col]=[];st.openFilterCol=col;if(st.page)st.page=1;if(BALIKAN_STATE.filterState&&mode==='balikan')BALIKAN_STATE.filterState.page=1;rerenderTableByMode(mode,true);}if(e.target.matches("[data-col-filter-all]")){const rowsSource=mode==='balikan'?((BALIKAN_STATE.filterState?.rows)||[]):(st.rows||[]);const applier=mode==='balikan'?applyBalikanTableFilters:applyTableFilters;st.columnFilters[col]=getUniqueOptions(applier(rowsSource,mode,col),col);st.openFilterCol=col;if(st.page)st.page=1;if(BALIKAN_STATE.filterState&&mode==='balikan')BALIKAN_STATE.filterState.page=1;rerenderTableByMode(mode,true);}return;}
...
function applyBalikanTableFilters(rows,mode='balikan',omitCol=''){const st=ensureBalikanFilterState();const q=clean(window.balikanSearchKeyword||'');return rows.filter(r=>{if(BALIKAN_FILTERABLE_COLUMNS.some(col=>{if(col===omitCol)return false;const selected=st.columnFilters[col]||[];if(!selected.length)return false;return !selected.includes(sanitizeFilterValue(r[col]));}))return false;if(q&&!clean(`${r.sku} ${r.namaBarang} ${r.lokasi} ${r.status} ${r.keterangan} ${r.rakTujuan}`).includes(q))return false;return true;});}
function renderBalikanControlPanel(st){return `<div class="balikan-controls-extra"><div class="balikan-filter-panel"><div class="balikan-filter-grid"><label class="balikan-filter-item"><span class="balikan-filter-label">SKU</span><input id="balikanFilterSku" class="search-lg" placeholder="Masukkan SKU" value="${esc(st.scanFilters.sku||'')}"></label><label class="balikan-filter-item"><span class="balikan-filter-label">Barcode</span><input id="balikanFilterBarcode" class="search-lg" placeholder="Masukkan barcode" value="${esc(st.scanFilters.barcode||'')}"></label><label class="balikan-filter-item"><span class="balikan-filter-label">Judul Produk</span><input id="balikanFilterTitle" class="search-lg" placeholder="Masukkan judul produk" value="${esc(st.scanFilters.title||'')}"></label><label class="balikan-filter-item"><span class="balikan-filter-label">Status Ceklist</span><select id="balikanFilterChecklist"><option value="">Semua Status</option><option value="checked" ${st.scanFilters.status==='checked'?'selected':''}>Checked</option><option value="unchecked" ${st.scanFilters.status==='unchecked'?'selected':''}>Unchecked</option></select></label></div><div class="balikan-filter-actions"><button id="balikanResetFilters" class="btn-ghost balikan-btn-muted" type="button">Reset Filter</button><button id="balikanExportCsv" class="btn-primary" type="button">Export CSV</button></div><label class="balikan-toggle-row"><span>Scan Auto Ceklist</span><input id="balikanScanAutoCheck" type="checkbox" ${BALIKAN_STATE.scanAutoCheck?'checked':''}><span class="balikan-toggle-switch" aria-hidden="true"></span></label></div></div>`;}
...
async function handleBalikanScanResult(decodedText){if(navigator.vibrate)navigator.vibrate(100);const sku=cleanScannedSku(decodedText);window.balikanSearchKeyword=sku;const searchInput=document.querySelector("#balikanSearchInput");if(searchInput)searchInput.value=sku;const row=(window.BALIKAN_ROWS||[]).find(item=>String(item.sku||"").trim().toLowerCase()===String(sku||"").trim().toLowerCase());if(!row){BALIKAN_STATE.highlightRowNumber=null;toast("SKU tidak ditemukan di Balikan Store: "+sku,"error");renderBalikanTable();return;}BALIKAN_STATE.highlightRowNumber=Number(row.rowNumber);if(BALIKAN_STATE.scanAutoCheck){await toggleBalikanCheck(window.currentTripSheet,Number(row.rowNumber),true);}renderBalikanTable();setTimeout(()=>{const el=document.querySelector(`[data-row-number="${row.rowNumber}"]`);if(el){el.scrollIntoView({behavior:"smooth",block:"center"});el.classList.add("row-highlight");}},300);showToast(BALIKAN_STATE.scanAutoCheck?"SKU berhasil discan dan dicentang: "+sku:"SKU ditemukan: "+sku+" (ceklist manual)","success");}

document.addEventListener('input',e=>{if(e.target?.id==='balikanFilterSku'||e.target?.id==='balikanFilterBarcode'||e.target?.id==='balikanFilterTitle'){const st=ensureBalikanFilterState();if(!st.scanFilters)st.scanFilters={sku:'',barcode:'',title:'',status:''};st.scanFilters.sku=document.getElementById('balikanFilterSku')?.value||'';st.scanFilters.barcode=document.getElementById('balikanFilterBarcode')?.value||'';st.scanFilters.title=document.getElementById('balikanFilterTitle')?.value||'';renderBalikanTable(false);}});
document.addEventListener('change',e=>{if(e.target?.id==='balikanFilterChecklist'){const st=ensureBalikanFilterState();if(!st.scanFilters)st.scanFilters={sku:'',barcode:'',title:'',status:''};st.scanFilters.status=e.target.value||'';renderBalikanTable(false);return;}if(e.target?.id==='balikanScanAutoCheck'){setBalikanAutoCheck(e.target.checked);}});
document.addEventListener('click',e=>{if(e.target?.id==='balikanResetFilters'){const st=ensureBalikanFilterState();st.scanFilters={sku:'',barcode:'',title:'',status:''};renderBalikanTable(false);return;}if(e.target?.id==='balikanExportCsv'){exportBalikanCsv();return;}});
...
const BALIKAN_AUTOCHECK_KEY="balikan_scan_auto_check_v1";
function getBalikanAutoCheckDefault(){try{return localStorage.getItem(BALIKAN_AUTOCHECK_KEY)!=="false";}catch(_){return true;}}
function setBalikanAutoCheck(enabled){BALIKAN_STATE.scanAutoCheck=enabled===true;try{localStorage.setItem(BALIKAN_AUTOCHECK_KEY,BALIKAN_STATE.scanAutoCheck?"true":"false");}catch(_){}}
function normalizeBalikanScanTime(value){if(!value)return "-";const d=new Date(value);if(Number.isNaN(d.getTime()))return String(value);return d.toLocaleString('id-ID');}
function getBalikanFilteredRows(){const st=ensureBalikanFilterState();const baseRows=(window.BALIKAN_ROWS||[]).map(r=>({...r}));st.rows=baseRows;let rows=applyBalikanTableFilters(baseRows);
const f=st.scanFilters||{};
const skuQ=clean(f.sku||"");
const barcodeQ=clean(f.barcode||"");
const titleQ=clean(f.title||"");
const status=f.status||"";
if(skuQ)rows=rows.filter(r=>clean(r.sku||"").includes(skuQ));
if(barcodeQ)rows=rows.filter(r=>clean(r.barcode||r.sku||"").includes(barcodeQ));
if(titleQ)rows=rows.filter(r=>clean(r.namaBarang||"").includes(titleQ));
if(status==="checked")rows=rows.filter(r=>isCheckedValue(r.checked));
if(status==="unchecked")rows=rows.filter(r=>!isCheckedValue(r.checked));
return rows;
}
function exportBalikanCsv(){const st=ensureBalikanFilterState();const rows=getBalikanFilteredRows();const headers=["SKU","Barcode","Judul Produk","Status Ceklist","Waktu Scan"];
const toCsvCell=(v)=>`"${String(v??"").replace(/"/g,'""')}"`;
const csv=[headers,...rows.map(r=>[r.sku||"",r.barcode||r.sku||"",r.namaBarang||"",isCheckedValue(r.checked)?"Checked":"Unchecked",normalizeBalikanScanTime(r.scanTime||r.waktuScan||"")])].map(line=>line.map(toCsvCell).join(',')).join('\n');
const blob=new Blob([csv],{type:'text/csv;charset=utf-8;'});
const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`balikan-store-${new Date().toISOString().slice(0,10)}.csv`;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);toast(`Export ${rows.length} baris berhasil`,'success');
}
