...
if(page==="abc-analisis")renderAbcAnalysisPage();
...
const ABC_ANALYSIS_STATE={months:3,excludeTo:["REJECT","RUSAK"],rows:[],cacheKey:"ABC_ANALYSIS_CACHE",lastSync:0};
function parseDateLoose(v){if(!v)return null;const d=new Date(v);if(!Number.isNaN(d.getTime()))return d;const t=String(v).trim();const m=t.match(/^(\d{1,2})[\/-](\d{1,2})[\/-](\d{2,4})$/);if(!m)return null;const day=Number(m[1]),mon=Number(m[2])-1,year=Number(m[3].length===2?`20${m[3]}`:m[3]);const p=new Date(year,mon,day);return Number.isNaN(p.getTime())?null:p;}
function getInventoryBySku(){const map=new Map();for(const row of (DATA["Kartu Stock"]||[])){const sku=String(getVal(row,["sku"])||"").trim();if(!sku)continue;const key=clean(sku);const nama=String(getVal(row,["nama barang","nama","item","description"])||"-").trim()||"-";const stok=parseNumber(getVal(row,["stok akhir","qty","closing stock","ending stock","saldo akhir","stok"]));const minStok=parseNumber(getVal(row,["minimum stok","min stok","minimum","min","reorder point"]));if(!map.has(key))map.set(key,{sku,nama,stok:0,minStok:0});const it=map.get(key);it.stok+=stok;it.minStok=Math.max(it.minStok,minStok);}return map;}
function buildAbcAnalysisRows(months=3){const now=new Date();const start=new Date(now);start.setMonth(start.getMonth()-Math.max(1,months));const excludeSet=new Set((ABC_ANALYSIS_STATE.excludeTo||[]).map(v=>clean(v)));const grouped=new Map();const source=(DATA["Barang Keluar"]||[]);for(const row of source){try{const dt=parseDateLoose(getVal(row,["tanggal","date"]));if(!dt||dt<start||dt>now)continue;const toRaw=String(getVal(row,["to","tujuan","tujuan pengeluaran barang","destination","status"])||"").trim();if(excludeSet.has(clean(toRaw)))continue;const sku=String(getVal(row,["sku","sku / nama barang"])||"").trim();if(!sku)continue;const nama=String(getVal(row,["nama barang","nama","item","description"])||sku).trim()||sku;const qty=Math.abs(parseNumber(getVal(row,["qty","qty keluar","quantity"])));if(qty<=0)continue;const key=clean(sku);if(!grouped.has(key))grouped.set(key,{sku,nama,qty:0});grouped.get(key).qty+=qty;}catch(_err){console.warn('ABC_ANALISIS skip invalid row',row);}}
const rows=[...grouped.values()];const total=rows.reduce((n,r)=>n+r.qty,0);if(total<=0)return [];
const byContribution=[...rows].sort((a,b)=>b.qty-a.qty);let cum=0;const categoryMap=new Map();for(const r of byContribution){const c=(r.qty/total)*100;cum+=c;const cat=cum<=80?"A":cum<=95?"B":"C";categoryMap.set(clean(r.sku),{contrib:c,cumulative:cum,category:cat});}
const invMap=getInventoryBySku();
const out=rows.map(r=>{const calc=categoryMap.get(clean(r.sku))||{contrib:0,cumulative:0,category:"C"};const inv=invMap.get(clean(r.sku))||{stok:0,minStok:0};let rekom="NORMAL";if(calc.category==="A"&&inv.stok<inv.minStok)rekom="RESTOCK PRIORITAS";else if(calc.category==="A")rekom="AMAN";else if(calc.category==="C"&&inv.stok>inv.minStok*2&&inv.minStok>0)rekom="OVERSTOCK";else if(calc.category==="C"&&inv.stok<=inv.minStok)rekom="LOW PRIORITY";return {...r,contrib:calc.contrib,cumulative:calc.cumulative,category:calc.category,stok:inv.stok,minStok:inv.minStok,rekom};});
return out.sort((a,b)=>a.qty-b.qty||a.sku.localeCompare(b.sku));}
function renderAbcAnalysisPage(){if(!abcAnalysisApp)return;const rows=buildAbcAnalysisRows(ABC_ANALYSIS_STATE.months);ABC_ANALYSIS_STATE.rows=rows;ABC_ANALYSIS_STATE.lastSync=Date.now();localStorage.setItem(ABC_ANALYSIS_STATE.cacheKey,JSON.stringify({updatedAt:ABC_ANALYSIS_STATE.lastSync,months:ABC_ANALYSIS_STATE.months,rows}));if(!rows.length){abcAnalysisApp.innerHTML=`<div class='state'>Tidak ada data pada periode ini</div>`;return;}
const catCount={A:0,B:0,C:0};const catContrib={A:0,B:0,C:0};rows.forEach(r=>{catCount[r.category]++;catContrib[r.category]+=r.contrib;});const topFast=[...rows].sort((a,b)=>b.qty-a.qty).filter(r=>r.category==='A').slice(0,10);const topSlow=[...rows].filter(r=>r.category==='C').slice(0,10);
abcAnalysisApp.innerHTML=`<div class='card'><div class='mv-toolbar'><select id='abcMonths' class='search-lg'>${[1,2,3,4,5,6].map(n=>`<option value='${n}' ${n===ABC_ANALYSIS_STATE.months?'selected':''}>${n} bulan terakhir</option>`).join('')}</select><button id='abcRefresh' class='btn-primary'>Refresh</button><button id='abcExportCsv' class='btn-ghost'>Export CSV</button></div><div class='grid dashboard'>${['A','B','C'].map(c=>`<div class='metric'><div class='k'>Kategori ${c}</div><div class='v'>${catCount[c]} SKU • ${catContrib[c].toFixed(2)}%</div></div>`).join('')}</div><div class='table-wrap table-wrap-full'><table><thead><tr><th>SKU / Nama Barang</th><th>Total Qty Keluar</th><th>% Kontribusi</th><th>Cumulative %</th><th>Kategori</th><th>Stok Saat Ini</th><th>Minimum Stok</th><th>Status Rekomendasi</th></tr></thead><tbody>${rows.map(r=>`<tr><td><strong>${esc(r.sku)}</strong><div class='subtitle'>${esc(r.nama)}</div></td><td>${r.qty}</td><td>${r.contrib.toFixed(2)}%</td><td>${r.cumulative.toFixed(2)}%</td><td><span class='badge ${r.category==='A'?'b-out':r.category==='B'?'b-warn':'b-kartu'}'>${r.category}</span></td><td>${r.stok}</td><td>${r.minStok}</td><td>${esc(r.rekom)}</td></tr>`).join('')}</tbody></table></div><div class='grid dashboard'><div class='card'><h4>Top 10 Fast Moving (A)</h4>${topFast.map((r,i)=>`<div>${i+1}. ${esc(r.sku)} - ${r.qty}</div>`).join('')||'<div class="subtitle">Tidak ada data</div>'}</div><div class='card'><h4>Top 10 Slow Moving (C)</h4>${topSlow.map((r,i)=>`<div>${i+1}. ${esc(r.sku)} - ${r.qty}</div>`).join('')||'<div class="subtitle">Tidak ada data</div>'}</div></div></div>`;
const monthEl=document.getElementById('abcMonths');monthEl?.addEventListener('change',e=>{ABC_ANALYSIS_STATE.months=Number(e.target.value)||3;renderAbcAnalysisPage();});
document.getElementById('abcRefresh')?.addEventListener('click',()=>renderAbcAnalysisPage());
document.getElementById('abcExportCsv')?.addEventListener('click',()=>{const cols=['sku','nama','total_qty_keluar','kontribusi_pct','cumulative_pct','kategori','stok','minimum_stok','status_rekomendasi'];const lines=[cols.join(',')].concat((ABC_ANALYSIS_STATE.rows||[]).map(r=>[r.sku,r.nama,r.qty,r.contrib.toFixed(2),r.cumulative.toFixed(2),r.category,r.stok,r.minStok,r.rekom].map(v=>`"${String(v??'').replaceAll('"','""')}"`).join(',')));const blob=new Blob([lines.join('\n')],{type:'text/csv;charset=utf-8;'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='abc-analisis.csv';a.click();URL.revokeObjectURL(a.href);});}
