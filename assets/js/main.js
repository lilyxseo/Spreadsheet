...
const ASSET_STORE_STATE={sheetList:[],selectedSheet:"",cache:{},loadingList:false,loadingData:false,listError:"",dataError:"",searchInput:"",search:"",searchTimer:null,searchToken:0,minSearchLength:2,sortColumn:"",sortDirection:"asc",columnFilter:"all",page:1,pageSize:25,lastLoadedAt:{},saveStatus:{},loadingAll:false,allLoadError:""};
function bindAssetStoreEvents(){document.addEventListener("change",e=>{if(e.target?.id==="assetStoreSheetSelect")selectAssetStoreSheet(e.target.value);if(e.target?.id==="assetStoreSortColumn"){ASSET_STORE_STATE.sortColumn=e.target.value;ASSET_STORE_STATE.page=1;renderAssetStoreTableOnly();}if(e.target?.id==="assetStoreSortDir"){ASSET_STORE_STATE.sortDirection=e.target.value;ASSET_STORE_STATE.page=1;renderAssetStoreTableOnly();}if(e.target?.id==="assetStoreColumnFilter"){ASSET_STORE_STATE.columnFilter=e.target.value;ASSET_STORE_STATE.page=1;renderAssetStoreTableOnly();}});document.addEventListener("input",e=>{if(e.target?.id!=="assetStoreSearch")return;ASSET_STORE_STATE.searchInput=e.target.value||"";const debounceToken=++ASSET_STORE_STATE.searchToken;clearTimeout(ASSET_STORE_STATE.searchTimer);ASSET_STORE_STATE.searchTimer=setTimeout(()=>{if(debounceToken!==ASSET_STORE_STATE.searchToken)return;const rawInput=String(ASSET_STORE_STATE.searchInput||"").trim();ASSET_STORE_STATE.search=rawInput.length>=ASSET_STORE_STATE.minSearchLength?rawInput:"";ASSET_STORE_STATE.page=1;if(!ASSET_STORE_STATE.selectedSheet&&ASSET_STORE_STATE.search)loadAllAssetStoreSheets();else renderAssetStoreTableOnly();},300);});document.addEventListener("click",e=>{if(e.target?.closest("#assetStoreRefreshBtn"))refreshAssetStore();const pg=e.target?.closest("[data-asset-store-page]");if(pg){ASSET_STORE_STATE.page=Math.max(1,ASSET_STORE_STATE.page+(Number(pg.dataset.assetStorePage)||0));renderAssetStoreTableOnly();return;}const cell=e.target?.closest('[data-asset-edit]');if(cell){beginAssetStoreInlineEdit(cell);}});}
...
async function selectAssetStoreSheet(sheetName){ASSET_STORE_STATE.selectedSheet=sheetName||"";ASSET_STORE_STATE.page=1;ASSET_STORE_STATE.dataError="";ASSET_STORE_STATE.allLoadError="";if(!sheetName){renderAssetStorePage();if(clean(ASSET_STORE_STATE.search).length>=ASSET_STORE_STATE.minSearchLength)loadAllAssetStoreSheets();return;}if(ASSET_STORE_STATE.cache[sheetName]){renderAssetStorePage();return;}ASSET_STORE_STATE.loadingData=true;renderAssetStoreTableOnly();try{ASSET_STORE_STATE.cache[sheetName]=await fetchAssetStoreSheetData(sheetName);ASSET_STORE_STATE.lastLoadedAt[sheetName]=new Date().toISOString();}catch(err){ASSET_STORE_STATE.dataError=err.message||"Gagal memuat data sheet";}finally{ASSET_STORE_STATE.loadingData=false;renderAssetStoreTableOnly();}}
async function loadAllAssetStoreSheets({force=false}={}){if(ASSET_STORE_STATE.loadingAll)return;if(!ASSET_STORE_STATE.sheetList.length){await ensureAssetStoreList();}const sheets=ASSET_STORE_STATE.sheetList.map(s=>s.key).filter(Boolean);const missing=force?sheets:sheets.filter(sheet=>!ASSET_STORE_STATE.cache[sheet]);if(!missing.length){renderAssetStoreTableOnly();return;}ASSET_STORE_STATE.loadingAll=true;ASSET_STORE_STATE.allLoadError="";renderAssetStoreTableOnly();try{const results=await Promise.allSettled(missing.map(sheet=>fetchAssetStoreSheetData(sheet).then(data=>({sheet,data}))));const failed=[];results.forEach(result=>{if(result.status==='fulfilled'){ASSET_STORE_STATE.cache[result.value.sheet]=result.value.data;ASSET_STORE_STATE.lastLoadedAt[result.value.sheet]=new Date().toISOString();}else failed.push(result.reason?.message||'Gagal memuat sheet');});if(failed.length)ASSET_STORE_STATE.allLoadError=`${failed.length} sheet gagal dimuat: ${failed[0]}`;}finally{ASSET_STORE_STATE.loadingAll=false;renderAssetStoreTableOnly();}}
async function refreshAssetStore(){const sheet=ASSET_STORE_STATE.selectedSheet;if(!sheet){ASSET_STORE_STATE.sheetList=[];ASSET_STORE_STATE.cache={};ASSET_STORE_STATE.lastLoadedAt={};await ensureAssetStoreList();if(clean(ASSET_STORE_STATE.search).length>=ASSET_STORE_STATE.minSearchLength)await loadAllAssetStoreSheets({force:true});return;}delete ASSET_STORE_STATE.cache[sheet];ASSET_STORE_STATE.loadingData=true;ASSET_STORE_STATE.dataError="";renderAssetStoreTableOnly();try{ASSET_STORE_STATE.cache[sheet]=await fetchAssetStoreSheetData(sheet);ASSET_STORE_STATE.lastLoadedAt[sheet]=new Date().toISOString();}catch(err){ASSET_STORE_STATE.dataError=err.message||"Gagal refresh data";}finally{ASSET_STORE_STATE.loadingData=false;renderAssetStoreTableOnly();}}
...
const ASSET_STORE_RESULT_COLUMNS=[{key:'sheetName',label:'Sheet Sumber'},{key:'sku',label:'SKU / Kode Asset'},{key:'namaAsset',label:'Nama Asset'},{key:'lokasi',label:'Lokasi'},{key:'qty',label:'Qty / Stok'},{key:'keterangan',label:'Keterangan'},{key:'status',label:'Status'}];
const ASSET_STORE_FIELD_ALIASES={sku:['sku','kode asset','kode','asset code','kode barang'],namaAsset:['nama asset','nama barang','nama produk','asset','item','description'],lokasi:['lokasi','location','rack','bin','area'],qty:['qty','stok','stock','jumlah','quantity'],keterangan:['keterangan','remark','remarks','catatan','note','notes'],status:['status','condition','kondisi']};
function findAssetStoreColumn(columns,aliases){const normalized=columns.map(c=>({raw:c,norm:normalizeHeader(c)}));for(const alias of aliases){const a=normalizeHeader(alias);const exact=normalized.find(c=>c.norm===a);if(exact)return exact.raw;}for(const alias of aliases){const a=normalizeHeader(alias);const partial=normalized.find(c=>c.norm.includes(a)||a.includes(c.norm));if(partial)return partial.raw;}return '';}
function toAssetStoreResult(row,columns,sheetName){const mapped={sheetName};Object.entries(ASSET_STORE_FIELD_ALIASES).forEach(([key,aliases])=>{const col=findAssetStoreColumn(columns,aliases);mapped[key]=col?row[col]??'':'';});mapped.__rowNumber=row.__rowNumber;mapped.__sourceRow=row;return mapped;}
function assetStoreSearchMatch(result,query){const normalizedQuery=clean(query);if(!normalizedQuery)return true;const tokens=normalizedQuery.split(' ').filter(Boolean);const haystack=clean([result.sku,result.namaAsset,result.lokasi,result.keterangan].join(' '));return tokens.every(token=>haystack.includes(token));}
function getAssetStoreRows(){const selected=ASSET_STORE_STATE.selectedSheet;const q=clean(ASSET_STORE_STATE.search);const sheets=selected?[selected]:ASSET_STORE_STATE.sheetList.map(s=>s.key);let rows=[];let totalSource=0;let headerError=false;for(const sheetName of sheets){const parsed=ASSET_STORE_STATE.cache[sheetName]||{rows:[],columns:[],headerError:false};if(parsed.headerError)headerError=true;totalSource+=parsed.rows.length;rows.push(...parsed.rows.map(row=>toAssetStoreResult(row,parsed.columns||[],sheetName)));}if(q)rows=rows.filter(r=>assetStoreSearchMatch(r,q));if(ASSET_STORE_STATE.sortColumn){const col=ASSET_STORE_STATE.sortColumn;rows.sort((a,b)=>String(a[col]??"").localeCompare(String(b[col]??""),"id",{numeric:true,sensitivity:'base'}));if(ASSET_STORE_STATE.sortDirection==='desc')rows.reverse();}return {rows,columns:ASSET_STORE_RESULT_COLUMNS,headerError,totalSource};}
function renderAssetStoreTableOnly(){const host=document.getElementById("assetStoreDataSection");if(!host)return;const selected=ASSET_STORE_STATE.selectedSheet;const {rows,columns,headerError,totalSource}=getAssetStoreRows();const totalPage=Math.max(1,Math.ceil(rows.length/ASSET_STORE_STATE.pageSize));if(ASSET_STORE_STATE.page>totalPage)ASSET_STORE_STATE.page=totalPage;const start=(ASSET_STORE_STATE.page-1)*ASSET_STORE_STATE.pageSize;const pageRows=rows.slice(start,start+ASSET_STORE_STATE.pageSize);const activeSheetName=(ASSET_STORE_STATE.sheetList.find(s=>s.key===selected)||{}).name||selected||"Semua Sheet Asset";const loadedSheets=ASSET_STORE_STATE.sheetList.filter(s=>ASSET_STORE_STATE.cache[s.key]).length;const lastLoadedValues=Object.values(ASSET_STORE_STATE.lastLoadedAt);const lastLoaded=lastLoadedValues.length?new Date(lastLoadedValues.sort().at(-1)).toLocaleString("id-ID"):"-";const q=clean(ASSET_STORE_STATE.search);if(ASSET_STORE_STATE.loadingData||ASSET_STORE_STATE.loadingAll){host.innerHTML="<div class='card archive-section'><div class='state'>Memuat data sheet...</div></div>";return;}if(ASSET_STORE_STATE.dataError||ASSET_STORE_STATE.allLoadError){host.innerHTML=`<div class='card archive-section'><div class='state'>${esc(ASSET_STORE_STATE.dataError||ASSET_STORE_STATE.allLoadError)}</div></div>`;return;}if(headerError&&selected){host.innerHTML="<div class='card archive-section'><div class='state'>Header tidak terdeteksi</div></div>";return;}if(!selected&&!q){host.innerHTML="<div class='card archive-section'><div class='state'>Ketik kata kunci untuk mencari otomatis di semua sheet Asset Store, atau pilih sheet untuk membatasi pencarian.</div></div>";return;}host.innerHTML=`<div class='card archive-section'><div class='grid dashboard archive-summary'><div class='metric'><div class='k'>Total Baris Dimuat</div><div class='v'>${totalSource}</div></div><div class='metric'><div class='k'>Hasil Pencarian</div><div class='v'>${rows.length}</div></div><div class='metric'><div class='k'>Scope</div><div class='v'>${esc(activeSheetName)}</div></div><div class='metric'><div class='k'>Sheet Cache</div><div class='v'>${selected?'1':`${loadedSheets}/${ASSET_STORE_STATE.sheetList.length}`}</div></div><div class='metric'><div class='k'>Terakhir Dimuat</div><div class='v'>${esc(lastLoaded)}</div></div></div></div><div class='card archive-section archive-table-card'>${!rows.length?"<div class='state'>Tidak ada data asset</div>":`<div class='table-wrap table-wrap-full archive-table-wrap'><table><thead><tr>${columns.map(c=>`<th>${esc(c.label)}</th>`).join("")}</tr></thead><tbody>${pageRows.map(r=>`<tr>${columns.map(c=>`<td>${esc(r[c.key]??"")}</td>`).join("")}</tr>`).join("")}</tbody></table></div><div class='mv-pagination'><span>Menampilkan ${rows.length?start+1:0}-${Math.min(start+ASSET_STORE_STATE.pageSize,rows.length)} dari ${rows.length}</span><div class='row'><button class='btn-ghost' data-asset-store-page='-1' ${ASSET_STORE_STATE.page<=1?'disabled':''}>Prev</button><button class='btn-ghost' data-asset-store-page='1' ${ASSET_STORE_STATE.page>=totalPage?'disabled':''}>Next</button></div></div>`}</div>`;}
function renderAssetStorePage(){if(!assetStoreApp)return;if(!ASSET_STORE_STATE.sheetList.length&&!ASSET_STORE_STATE.loadingList)ensureAssetStoreList();const selected=ASSET_STORE_STATE.selectedSheet;const columns=ASSET_STORE_RESULT_COLUMNS;const sortOps=columns.map(c=>`<option value='${esc(c.key)}' ${ASSET_STORE_STATE.sortColumn===c.key?"selected":""}>${esc(c.label)}</option>`).join("");assetStoreApp.innerHTML=`<div class='archive-layout'><div class='card archive-section'><div class='section-header'><h3 class='page-title'>Asset Store</h3><div class='row'><select id='assetStoreSheetSelect'><option value=''>Semua Sheet Asset</option>${ASSET_STORE_STATE.sheetList.map(s=>`<option value='${esc(s.key)}' ${s.key===selected?"selected":""}>${esc(s.name)}</option>`).join("")}</select><button id='assetStoreRefreshBtn' class='btn-ghost'>Refresh Asset Store</button></div></div>${ASSET_STORE_STATE.loadingList?"<div class='state'>Memuat daftar sheet...</div>":ASSET_STORE_STATE.listError?`<div class='state'>${esc(ASSET_STORE_STATE.listError)}</div>`:""}</div><div class='card archive-section archive-filter-card'><div class='mv-filters open archive-filters'><input id='assetStoreSearch' class='search-lg' placeholder='Cari SKU, nama asset, lokasi, atau keterangan (min. 2 huruf)' value='${esc(ASSET_STORE_STATE.searchInput)}'/><select id='assetStoreSortColumn'><option value=''>Urutkan Kolom</option>${sortOps}</select><select id='assetStoreSortDir'><option value='asc' ${ASSET_STORE_STATE.sortDirection==='asc'?'selected':''}>ASC</option><option value='desc' ${ASSET_STORE_STATE.sortDirection==='desc'?'selected':''}>DESC</option></select></div></div><div id='assetStoreDataSection'></div></div>`;renderAssetStoreTableOnly();}
document.addEventListener("DOMContentLoaded",()=>{ensureCycleHistoryLoaded().finally(()=>renderCycleCountPage());});
