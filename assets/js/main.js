...
function parseCycleHistoryRows(rawValues){try{const rows=parseRows(rawValues);return {rows:rows.map(r=>({tanggal:r.tanggal,lokasi:r.lokasi,sku:r.sku,nama:r.nama_barang,bulky:r.bulky,retail:r.retail,aktualBulky:r.aktual_bulky,aktualRetail:r.aktual_retail,catatan:r.catatan,createdAt:r.created_at})),error:""};}catch(err){return {rows:[],error:err?.message||"Header Cycle Count tidak valid."};}}
...
const MOVEMENT_HISTORY_RANGE="Data Movement Barang!A1:ZZ";
...
function parseMovementHistoryRows(rawValues){try{const rows=parseRows(rawValues);return {rows:rows.map(r=>({tanggal:r.tanggal,from:r.lokasi,to:r.retail?String(r.retail):String((rawValues||[])[0]||""),sku:r.sku,nama:r.nama_barang,stok_lokasi_awal:r.bulky,stok_aktual:r.aktual_bulky,keterangan:r.catatan||""})),error:""};}catch(_){const values=Array.isArray(rawValues)?rawValues:[];if(!values.length)return {rows:[],error:""};const header=(values[0]||[]).map(x=>clean(String(x||"")));const idx={tanggal:header.indexOf("tanggal"),from:header.indexOf("from"),to:header.indexOf("to"),sku:header.indexOf("sku"),nama:header.indexOf("nama barang"),awal:header.indexOf("stok lokasi awal"),aktual:header.indexOf("stok aktual"),ket:header.indexOf("keterangan")};const required=["tanggal","from","to","sku","nama","awal","aktual"];const miss=required.filter(k=>idx[k]<0);if(miss.length)return {rows:[],error:`Header Movement tidak valid: ${miss.join(", ")}`};const rows=[];for(let i=1;i<values.length;i++){const r=values[i]||[];if(!r.some(c=>String(c||"").trim()))continue;rows.push({tanggal:String(r[idx.tanggal]||""),from:String(r[idx.from]||""),to:String(r[idx.to]||""),sku:String(r[idx.sku]||""),nama:String(r[idx.nama]||""),stok_lokasi_awal:parseNumber(r[idx.awal]??0),stok_aktual:parseNumber(r[idx.aktual]??0),keterangan:String(r[idx.ket]||"")});}return {rows,error:""};}}
}
