...
function renderBalikanControlPanel(st){return `<div class="balikan-controls-extra"><div class="balikan-filter-panel balikan-filter-panel-compact"><label class="balikan-filter-item balikan-sort-item"><span class="balikan-filter-label">Urutkan Berdasarkan</span><select id="balikanSortBy"><option value="default" ${st.sortBy==='default'?'selected':''}>Default</option><option value="sku" ${st.sortBy==='sku'?'selected':''}>SKU</option><option value="barcode" ${st.sortBy==='barcode'?'selected':''}>Barcode</option><option value="title" ${st.sortBy==='title'?'selected':''}>Judul Produk</option><option value="checklist" ${st.sortBy==='checklist'?'selected':''}>Status Ceklist</option></select></label><button id="balikanResetFilters" class="btn-ghost balikan-btn-muted" type="button">Reset Filter</button><button id="balikanExportCsv" class="btn-primary" type="button">Export CSV</button><label class="balikan-toggle-row"><span>Scan Auto Ceklist</span><input id="balikanScanAutoCheck" type="checkbox" ${BALIKAN_STATE.scanAutoCheck?'checked':''}><span class="balikan-toggle-switch" aria-hidden="true"></span></label></div></div>`;}
...
document.addEventListener('change',e=>{if(e.target?.id==='balikanSortBy'){const st=ensureBalikanFilterState();st.sortBy=e.target.value||'default';renderBalikanTable(false);return;}if(e.target?.id==='balikanScanAutoCheck'){setBalikanAutoCheck(e.target.checked);}});
document.addEventListener('click',e=>{if(e.target?.id==='balikanResetFilters'){const st=ensureBalikanFilterState();st.sortBy='default';renderBalikanTable(false);return;}if(e.target?.id==='balikanExportCsv'){exportBalikanCsv();return;}});
...
function getBalikanFilteredRows(){const st=ensureBalikanFilterState();const baseRows=(window.BALIKAN_ROWS||[]).map(r=>({...r}));st.rows=baseRows;const rows=applyBalikanTableFilters(baseRows);const key=st.sortBy||'default';if(key==='default')return rows;const sorted=[...rows];const val=(r)=>key==='sku'?String(r.sku||''):key==='barcode'?String(r.barcode||r.sku||''):key==='title'?String(r.namaBarang||''):key==='checklist'?(isCheckedValue(r.checked)?'1':'0'):'';sorted.sort((a,b)=>clean(val(a)).localeCompare(clean(val(b)),'id'));return sorted;
}
