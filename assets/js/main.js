...
async function updateBalikanCheck(sheetName,rowNumber,checked){const wrapper=document.querySelector('.balikan-table-wrapper');const scrollTop=wrapper?.scrollTop||0;const scrollLeft=wrapper?.scrollLeft||0;const pageScrollX=window.scrollX||window.pageXOffset||0;const pageScrollY=window.scrollY||window.pageYOffset||0;const targetSheet=sheetName||window.currentTripSheet;const res=await fetch('/api/balikan-store/check',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({sheetName:targetSheet,rowNumber,checked})});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal update centang');const row=(window.BALIKAN_ROWS||[]).find(r=>Number(r.rowNumber)===Number(rowNumber));const oldChecked=String(row?.checked).toUpperCase()==='TRUE';if(row)row.checked=checked?'TRUE':'FALSE';renderBalikanTable();await new Promise(resolve=>requestAnimationFrame(resolve));const newWrapper=document.querySelector('.balikan-table-wrapper');if(newWrapper){newWrapper.scrollTop=scrollTop;newWrapper.scrollLeft=scrollLeft;}window.scrollTo(pageScrollX,pageScrollY);toast('Centang berhasil diupdate','success');logActivitySafe({action:'UPDATE_BALIKAN_STORE_CHECK',module:'Balikan Store',detail:`[CHECK] SKU: ${row?.sku||'-'}\n${oldChecked?'FALSE':'TRUE'} → ${checked?'TRUE':'FALSE'}`,status:'SUCCESS',metadata:{sheetName:targetSheet,rowNumber,sku:row?.sku||'',oldValue:oldChecked,newValue:checked,source:'table_check'}});}
