...
function renderDataTablePage(mode,sheetName,keepPage=false,selectedCols){const isIn=mode==="in", resultEl=isIn?inResults:outResults, summaryEl=isIn?inSummary:outSummary;if(!resultEl)return;const st=TABLE_STATE[mode];if(!keepPage||!st.rows.length)st.rows=normalizeMovementRows(sheetName,isIn?"IN":"OUT");const rows=st.rows;const safeRows=Array.isArray(rows)?rows:[];if(!safeRows.length){resultEl.innerHTML='<div class="state">Belum ada data.</div>';summaryEl.textContent='0 data';return;}let orderedRows;try{orderedRows=safeRows.slice().reverse();}catch(e){console.error("REVERSE ERROR",e);orderedRows=safeRows||[];}if(!orderedRows.length){orderedRows=(safeRows||[]).slice(0,25);}const allCols=["tanggal","from","to","sku","nama","qty","status","pic","keterangan","lokasi"];st.columns=selectedCols||st.columns||["tanggal","from","to","sku","nama","qty","status","pic","keterangan"];let filteredRows=applyTableFilters(orderedRows,mode);if(!filteredRows||!filteredRows.length){filteredRows=orderedRows;}const sort=document.getElementById(`mv-sort-${mode}`)?.value||"latest";st.filtered=sortTableRows(filteredRows,sort);st.pageSize=Number(document.getElementById(`mv-size-${mode}`)?.value||25);if(![25,50].includes(st.pageSize))st.pageSize=25;if(!keepPage)st.page=1;const currentPage=st.page||1;const pageSize=st.pageSize;const PAGE_SIZE=pageSize||25;const start=(currentPage-1)*PAGE_SIZE;let pageRows;try{pageRows=st.filtered.slice(start,start+PAGE_SIZE);}catch(e){console.error("PAGINATION ERROR",e);pageRows=st.filtered.slice(0,25);}if(!pageRows||!pageRows.length){resultEl.innerHTML='<div class="state">Data tidak tersedia</div>';console.log("ROWS:",safeRows?.length);console.log("DISPLAY:",orderedRows?.length);console.log("FILTERED:",st.filtered?.length);console.log("PAGE:",pageRows?.length);return;}console.log("ROWS:",safeRows?.length);console.log("DISPLAY:",orderedRows?.length);console.log("FILTERED:",st.filtered?.length);console.log("PAGE:",pageRows?.length);const totalQty=st.filtered.reduce((n,r)=>n+r.qty,0),skuCount=new Set(st.filtered.map(r=>r.sku)).size,fromCount=new Set(st.filtered.map(r=>r.from)).size,toCount=new Set(st.filtered.map(r=>r.to)).size;
