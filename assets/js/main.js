...
document.addEventListener("change",e=>{const c=e.target.closest("[data-col-filter-option]");if(!c)return;setColumnFilter(c.dataset.mvMode,c.dataset.mvCol,c.value,c.checked);});
...
function setColumnFilter(mode,column,value,checked){const st=TABLE_STATE[mode];const curr=new Set(st.columnFilters[column]||[]);if(checked)curr.add(value);else curr.delete(value);st.columnFilters[column]=[...curr];st.page=1;renderDataTablePage(mode,mode==="in"?"Barang Masuk":"Barang Keluar",true);}
