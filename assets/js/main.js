...
const ANOMALY_CACHE_KEY="operationalWarningCacheV2";
...
function updateAnomalyTypeOptions(){if(!anomalyType)return;const options=['<option value="all">Semua Rule</option>'].concat(ANOMALY_STATE.types.map(t=>`<option value="${esc(t)}">${esc(t)}</option>`));anomalyType.innerHTML=options.join('');anomalyType.value=ANOMALY_STATE.type;}
function getAnomalyFilteredRows(){const q=clean(ANOMALY_STATE.q);return (ANOMALY_STATE.rows||[]).filter(r=>{if(ANOMALY_STATE.severity!=='all'&&r.severity!==ANOMALY_STATE.severity)return false;if(ANOMALY_STATE.type!=='all'&&r.rule!==ANOMALY_STATE.type)return false;if(q&&!clean(`${r.sku} ${r.nama} ${r.rule} ${r.problem} ${r.location} ${r.source}`).includes(q))return false;return true;});}
...
const tbody=document.querySelector('#anomalyTable tbody');if(tbody){tbody.innerHTML=pageRows.map((r,i)=>`<tr><td><span class='badge ${sevClass(r.severity)}'>${r.severity}</span></td><td>${esc(r.rule)}</td><td>${esc(r.sku)}</td><td>${esc(r.problem)}</td><td>${esc(r.location||'-')}</td><td>${esc(r.source)}</td><td><button class='btn-ghost' data-warning-detail='${start+i}'>Detail</button></td></tr>`).join('')||"<tr><td colspan='7'><div class='state'>Tidak ada data warning</div></td></tr>";tbody.querySelectorAll('[data-warning-detail]').forEach(btn=>btn.onclick=()=>openWarningDetail(rows[Number(btn.dataset.warningDetail)]));}
...
function renderAnomalySummary(){const rows=ANOMALY_STATE.filtered.length?ANOMALY_STATE.filtered:ANOMALY_STATE.rows;const count=prefixes=>rows.filter(r=>prefixes.some(p=>r.rule===p||r.rule.startsWith(p))).length;const cards=[["Salah Lokasi",count(['INVALID_','EMPTY_LOCATION','UNKNOWN_FORMAT'])],["SKU/Nama",count(['SKU_NOT_IN_MASTER','SKU_NAME_MISMATCH'])],["SKU Tidak Ada di Lokasi",count(['SKU_NOT_AT_LOCATION'])],["Tidak Balance",count(['OUTBOUND_NOT_BALANCED'])],["Input Bermasalah",count(['UNREADABLE_INPUT','DUPLICATE_TRANSACTION'])],["Critical",rows.filter(r=>r.severity==='CRITICAL').length]];anomalySummary.innerHTML=cards.map(c=>`<div class='metric'><div class='k'>${c[0]}</div><div class='v'>${c[1]}</div></div>`).join('');}
function openWarningDetail(r){if(!r)return;const old=document.getElementById('warningDetailPanel');old?.remove();const panel=document.createElement('div');panel.id='warningDetailPanel';panel.className='card warning-detail';panel.innerHTML=`<div class='section-header'><h4>Detail Warning</h4><button class='btn-ghost' type='button'>Tutup</button></div><div class='warning-detail-grid'>${[['Rule',r.rule],['Masalah',r.problem],['Lokasi',r.location||'-'],['Jenis Lokasi',r.locationType],['Source',r.source],['Nilai Sekarang',r.currentValue||'-'],['Nilai Seharusnya',r.expectedValue||'-'],['Suggestion',r.suggestion||'-'],['Confidence',r.confidence]].map(([k,v])=>`<div><small>${esc(k)}</small><strong>${esc(v)}</strong></div>`).join('')}</div>`;panel.querySelector('button').onclick=()=>panel.remove();anomalyTable.appendChild(panel);panel.scrollIntoView({behavior:'smooth',block:'nearest'});}
...
function renderAnomalyPage(){if(!ANOMALY_STATE.rendered){anomalyTable.innerHTML=`<div class='row anomaly-toolbar'><div id='anomalyInfo' class='mv-pagination-info'>Menampilkan 0 dari 0 data</div><div class='row'><select id='anomalySize'><option value='25'>25</option><option value='50'>50</option><option value='100'>100</option></select><button class='btn-ghost' onclick='changeAnomalyPage(-1)'>Prev</button><button class='btn-ghost' onclick='changeAnomalyPage(1)'>Next</button></div></div><div id='anomalyLoading' class='state hidden'>Memuat warning...</div><div class='table-wrap table-wrap-full anomaly-table-wrap'><table><thead><tr><th>Severity</th><th>Rule</th><th>SKU</th><th>Masalah</th><th>Lokasi</th><th>Source</th><th>Action</th></tr></thead><tbody><tr><td colspan='7'><div class='state'>Memuat data warning...</div></td></tr></tbody></table></div>`;document.getElementById('anomalySize')?.addEventListener('change',e=>{ANOMALY_STATE.pageSize=Number(e.target.value)||25;ANOMALY_STATE.page=1;renderAnomalyRowsOnly();});ANOMALY_STATE.rendered=true;}
if(anomalySearch&&anomalySearch.value!==ANOMALY_STATE.q)anomalySearch.value=ANOMALY_STATE.q;document.getElementById('anomalySize').value=String(ANOMALY_STATE.pageSize);applyAnomalyFilters(false);if(!(ANOMALY_STATE.rows||[]).length){const cacheRows=getAnomalyCache();if(cacheRows.length){ANOMALY_STATE.rows=cacheRows;ANOMALY_STATE.types=[...new Set(cacheRows.map(r=>r.rule))];updateAnomalyTypeOptions();applyAnomalyFilters(false);setAnomalyLoading(true,'Menampilkan cache, sinkronisasi data terbaru...');}else{setAnomalyLoading(true,'Memuat data warning...');}}
refreshAnomalyInBackground();}
function refreshAnomalyInBackground(){const token=++ANOMALY_STATE.lastRenderToken;const run=async()=>{const all=await buildWarningReport({'Kartu Stock':DATA['Kartu Stock']||[],'RPL':DATA.RPL||[],'BULKY':DATA.BULKY||[],'Barang Masuk':getBarangMasukRows(),'Barang Keluar':getBarangKeluarRows()},{chunkSize:1000});if(token!==ANOMALY_STATE.lastRenderToken)return;ANOMALY_STATE.rows=all;ANOMALY_STATE.types=[...new Set(all.map(r=>r.rule))];if(!ANOMALY_STATE.types.includes(ANOMALY_STATE.type))ANOMALY_STATE.type='all';updateAnomalyTypeOptions();setAnomalyCache(all);applyAnomalyFilters(false);setAnomalyLoading(false);};if(typeof requestIdleCallback==='function'){requestIdleCallback(()=>run(),{timeout:700});return;}setTimeout(run,0);}
