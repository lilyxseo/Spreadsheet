...
const mvCell=e.target.closest('[data-mv-cell]');if(mvCell){const mode=mvCell.dataset.mode,row=Number(mvCell.dataset.row),field=mvCell.dataset.field;const item=TABLE_STATE[mode].rows.find(r=>r.rowNumber===row);if(item)startInlineEdit(mvCell,item,field,item[field],{onSave:async({value,oldValue,td})=>{td.classList.add('is-saving');td.innerHTML="<span class='btn-spinner-inline' aria-hidden='true'></span>";let out={};let res;try{res=await fetch(mode==='in'?'/api/barang-masuk/bulk-update':'/api/barang-keluar/bulk-update',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({rowNumbers:[row],updates:{[field]:value}})});out=await res.json();if(!res.ok||!out?.success)throw new Error(out?.message||'Gagal update');item[field]=value;const sheetName=mode==='in'?'Barang Masuk':'Barang Keluar';const targetRows=mode==='in'?getBarangMasukRows():getBarangKeluarRows();const target=targetRows.find(x=>Number(x.rowNumber)===row);if(target)target[field]=value;if(mode==='in'){window.APP_STATE=window.APP_STATE||{};window.APP_STATE.barangMasuk=targetRows;setCacheSafe(MODULE_CACHE_KEYS.barangMasuk,targetRows);setModuleCache(MODULE_CACHE_KEYS.barangMasuk,targetRows);}else{window.APP_STATE=window.APP_STATE||{};window.APP_STATE.barangKeluar=targetRows;setCacheSafe(MODULE_CACHE_KEYS.barangKeluar,targetRows);setModuleCache(MODULE_CACHE_KEYS.barangKeluar,targetRows);}DATA[sheetName]=targetRows;saveCache(DATA);td.textContent=value||'-';renderDataTablePage(mode,sheetName,true);toast('Data berhasil diupdate','success');logActivitySafe({action:mode==='in'?'EDIT_BARANG_MASUK':'EDIT_BARANG_KELUAR',module:mode==='in'?'Barang Masuk':'Barang Keluar',detail:`Edit ${field} row ${row}`,status:'SUCCESS'});}finally{td.classList.remove('is-saving');}}});return;}
