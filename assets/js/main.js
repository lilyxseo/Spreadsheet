...
function movementItemKey(item){return `${clean(item.sku)}|${clean(item.lokasi)}|${clean(item.to||"")}`;}
...
function getMovementCandidates(query=""){const q=clean(query);const list=(DATA["Stock"]||[]).map(r=>{const lokasi=String(getVal(r,["lokasi","location"])||"").trim();const sku=String(getVal(r,["sku"])||"").trim();const nama=String(getVal(r,["nama barang","nama_barang","nama"])||"").trim();const qty=Number(getQty(r));return {lokasi,sku,nama,qty:Number.isFinite(qty)?qty:0};}).filter(r=>r.lokasi&&r.sku&&r.nama);const filtered=q?list.filter(r=>clean(`${r.lokasi} ${r.sku} ${r.nama}`).includes(q)):list;return filtered.filter(r=>!MOVEMENT_STATE.sessionItems.some(it=>clean(it.sku)===clean(r.sku)&&clean(it.lokasi)===clean(r.lokasi))).slice(0,200);}
function renderMovementSearchResults(){const tbody=document.querySelector('#mvSearchResultsBody');if(!tbody)return;const candidates=MOVEMENT_STATE.sessionActive?getMovementCandidates(MOVEMENT_STATE.search):[];if(!candidates.length){tbody.innerHTML="<tr><td colspan='5'><div class='state cc-state'>Cari SKU untuk menambah item movement.</div></td></tr>";return;}const frag=document.createDocumentFragment();candidates.forEach(c=>{const tr=document.createElement('tr');tr.innerHTML=`<td>${esc(c.lokasi)}</td><td>${esc(c.sku)}</td><td>${esc(c.nama)}</td><td>${esc(c.qty)}</td><td><button class='btn-ghost' data-mvm-action='add' data-sku='${encAttr(c.sku)}' data-lok='${encAttr(c.lokasi)}'>Tambah</button></td>`;frag.appendChild(tr);});tbody.replaceChildren(frag);}
...
function renderMovementPage(){if(!movementApp)return;movementApp.innerHTML=`<div class='card cc-card cc-section'><div class='section-header cc-main-header'><div class='cc-action-stack'>${MOVEMENT_STATE.sessionActive?"<button class='btn-ghost' onclick='mvCancelSession()'>Batal Movement</button>":""}<button class='btn-primary' onclick='mvStartSession()' ${MOVEMENT_STATE.sessionActive?"disabled":""}>Mulai Movement</button>${MOVEMENT_STATE.sessionActive?"<button id='mvSubmitBtn' class='btn-primary' onclick='mvSubmitSession()'>Selesai Movement</button>":""}</div></div>${MOVEMENT_STATE.sessionActive?`<div class='cc-section'><div class='mv-toolbar'><input id='mvSearch' class='search-lg' placeholder='Cari SKU / nama barang / lokasi' value='${esc(MOVEMENT_STATE.searchInput)}'></div></div><div class='cc-section'><div class='table-wrap cc-table-wrap cc-search-wrap'><table><thead><tr><th>Lokasi</th><th>SKU</th><th>Nama Barang</th><th>Qty</th><th>Aksi</th></tr></thead><tbody id='mvSearchResultsBody'></tbody></table></div></div><div class='cc-section'><h4>Movement Berjalan</h4><div class='table-wrap cc-table-wrap cc-session-wrap'><table><thead><tr><th>From</th><th>To</th><th>SKU</th><th>Nama Barang</th><th>Stok Lokasi Awal</th><th>Stok Aktual</th><th>Aksi</th></tr></thead><tbody id='mvSessionBody'></tbody></table></div></div>`:`<div class='cc-empty-state'><i data-lucide='arrow-right-left'></i><h4>Belum ada movement berjalan</h4><p>Klik Mulai Movement untuk memilih SKU dan input stok aktual.</p></div>`}</div>`;if(window.lucide)window.lucide.createIcons();if(MOVEMENT_STATE.sessionActive){renderMovementSearchResults();renderMovementSessionTable();}}
...
document.addEventListener('click',e=>{const btn=e.target.closest('[data-mvm-action]');if(!btn)return;const action=btn.dataset.mvmAction;if(action==='remove'){const idx=Number(btn.dataset.idx);MOVEMENT_STATE.sessionItems.splice(idx,1);renderMovementSessionTable();return;}if(action==='add'){const sku=decodeURIComponent(btn.dataset.sku||"");const lokasi=decodeURIComponent(btn.dataset.lok||"");const source=getMovementCandidates(MOVEMENT_STATE.search).find(r=>clean(r.sku)===clean(sku)&&clean(r.lokasi)===clean(lokasi));if(source)addMovementItem(source);}});
