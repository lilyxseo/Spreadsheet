...
window.addEventListener("DOMContentLoaded",async ()=>{applyTheme();bindNav();bindEvents();setupSidebar();renderFilters();initDashboard();document.getElementById("sheetInfo").textContent=SHEETS.join(", ");document.getElementById("spreadsheetInfo").textContent=SPREADSHEET_ID;
await restoreSession();
onAuthStateChanged((session)=>{
  const path=location.pathname;
  if(!session && path!=="/login") navigateTo("/login", { replace: true });
  if(session && path==="/login") navigateTo("/", { replace: true });
});
if(location.pathname !== "/login")initAppData();
renderRecentHistory();routeFromPath(location.pathname);if(window.lucide)lucide.createIcons();window.addEventListener("popstate",()=>routeFromPath(location.pathname));});
function bindNav(){
...
function navigateTo(path,{replace=false}={}){if(replace)history.replaceState({},"",path);else history.pushState({},"",path);routeFromPath(path);}
...
if(!enforceProtectedRoute(path,navigateTo))return;
if(path==="/login")return showPage("login");
...
async function handleLogin(){
  ...
  try{
    ...
    if(!window.__isDataReady) initAppData();
  }catch(err){
