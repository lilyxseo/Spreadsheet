...
function renderRejectMiniList(title,items,{empty='Belum ada data'}={}){return `<article class="card reject-insight-card reject-modern-card"><div class="reject-card-title"><i data-lucide="sparkles"></i><h4>${esc(title)}</h4></div><div class="reject-insight-list reject-insight-list--modern">${items.length?items.slice(0,5).map(item=>`<div class="reject-insight-row reject-insight-row--modern"><span class="reject-insight-icon"><i data-lucide="${rejectInsightIcon(item[0])}"></i></span><span>${esc(item[0])}</span>${item[1]!==''?`<strong class="badge b-warn">${esc(item[1])}</strong>`:''}</div>`).join(''):`<div class="state">${esc(empty)}</div>`}</div></article>`;}
function formatRejectDate(v){const d=new Date(v);return Number.isNaN(d.getTime())?String(v||'-'):d.toLocaleDateString('id-ID',{day:'numeric',month:'numeric',year:'numeric'});}
function renderRejectRecentActivity(){const latest=[...orderRejectSheetRows(BARANG_REJECT_STATE.masuk).slice(0,5).map(r=>['Masuk',r]),...orderRejectSheetRows(BARANG_REJECT_STATE.keluar).slice(0,5).map(r=>['Keluar',r])].sort((a,b)=>(new Date(b[1]?.tanggal).getTime()||Number(b[1]?.rowNumber)||0)-(new Date(a[1]?.tanggal).getTime()||Number(a[1]?.rowNumber)||0)).slice(0,6);return `<article class="card reject-insight-card reject-insight-card--wide reject-modern-card"><div class="reject-card-title"><i data-lucide="history"></i><h4>Aktivitas terakhir barang masuk/keluar</h4></div><div class="reject-activity-list reject-timeline-list">${latest.length?latest.map(([type,r])=>`<div class="reject-timeline-item"><span class="reject-timeline-dot ${type==='Masuk'?'in':'out'}"></span><div class="reject-timeline-body"><span class="badge ${type==='Masuk'?'b-in':'b-out'}">${type}</span><strong>${esc(r.sku||'-')}</strong><small>${esc(r.namaBarang||'-')}</small><span class="reject-activity-foot">Qty: ${Number(r.qty)||0} • ${esc(formatRejectDate(r.tanggal))}</span></div></div>`).join(''):`<div class="state">Belum ada aktivitas reject.</div>`}</div></article>`;}
function renderRejectDashboardDetails(){const stock=BARANG_REJECT_STATE.stock.filter(r=>(Number(r.qty)||0)>0);const skuSet=new Set(stock.map(r=>String(r.sku||'').trim()).filter(Boolean));const totalStock=sumQty(stock);const masukMonth=sumQty(thisMonthRows(BARANG_REJECT_STATE.masuk));const keluarMonth=sumQty(thisMonthRows(BARANG_REJECT_STATE.keluar));const delta=Math.abs(masukMonth-keluarMonth);const insights=[];if(keluarMonth>masukMonth)insights.push([`🚨 Barang keluar reject lebih tinggi ${delta.toLocaleString('id-ID')} pcs. Stok reject bulan ini berkurang.`, '']);else if(masukMonth>keluarMonth)insights.push([`📈 Barang masuk reject lebih tinggi ${delta.toLocaleString('id-ID')} pcs. Stok reject bulan ini bertambah.`, '']);else insights.push(['✅ Masuk dan keluar reject bulan ini seimbang.', '']);insights.push([`📦 ${skuSet.size.toLocaleString('id-ID')} SKU reject aktif. Total stok reject saat ini ${totalStock.toLocaleString('id-ID')} pcs.`, '']);topRejectEntries(stock,'lokasi',3).forEach(([lokasi,qty])=>insights.push([`📍 Lokasi ${lokasi} menyimpan stok reject terbanyak.`, `${qty.toLocaleString('id-ID')} pcs`]));return `<div class="reject-insight-grid">${renderRejectMiniList('Auto Insight Reject',insights.slice(0,5),{empty:'Belum ada insight'})}${renderRejectMiniList('Top 5 SKU Reject',topRejectEntries(stock,'sku',5).map(([sku,qty])=>[sku,`${qty.toLocaleString('id-ID')} pcs`]))}${renderRejectRecentActivity()}</div>`;}
...
const REJECT_IN_COLUMNS=[['Tanggal','tanggal'],['From','from'],['To','to'],['SKU','sku'],['Nama Barang','namaBarang'],['Qty','qty'],['Status','statusBarang','badge'],['PIC','pic'],['Keterangan','keterangan'],['No iSeller','noIseller'],['Netsuite','netsuite'],['Keterangan Lainnya','keteranganLainnya'],['Lokasi Surat Jalan','lokasiSuratJalan'],['STCKOUT Reject','stckoutReject']];
const REJECT_OUT_COLUMNS=[['Tanggal','tanggal'],['From','from'],['To','to'],['SKU','sku'],['Nama Barang','namaBarang'],['Qty','qty'],['Status Barang','statusBarang','badge'],['PIC','pic'],['Keterangan','keterangan'],['No iSeller','noIseller'],['Netsuite','netsuite'],['Keterangan Lainnya','keteranganLainnya'],['Status Proses','statusProses','badge'],['Lokasi Surat Jalan','lokasiSuratJalan']];
function renderRejectCell(r,col){const [label,key,type]=col;const val=key==='qty'?Number(r[key])||0:r[key];if(type==='badge')return rejectStatusBadge(val);if(key==='qty')return `<strong>${val}</strong>`;return esc(val);}
...
function renderRejectIn(){const f=BARANG_REJECT_STATE.filters;let rows=BARANG_REJECT_STATE.masuk.filter(r=>rejectTokensMatch(r,f.masukSearch)&&dateInRange(r.tanggal,f.masukTanggalFrom,f.masukTanggalTo)&&(!f.masukLokasi||clean([r.from,r.to,r.lokasi].join(' ')).includes(clean(f.masukLokasi))));rows=orderRejectSheetRows(rows);const pageRows=paginateReject(rows,'masuk');return `<div class="reject-page-title-row"><div><h3>Barang Masuk Reject</h3><small class="subtitle">Kolom mengikuti sheet Barang Masuk Reject. Search dan filter dibuat compact.</small></div><div class="row"><button class="btn-primary" data-reject-open-form="masuk" ${!canCrud()?'hidden':''}><i data-lucide="plus"></i>Tambah Data</button><button class="btn-ghost" data-reject-export="masuk">Export</button></div></div><div class="card reject-card"><div class="reject-filter-grid"><input class="search-lg reject-search-main" data-reject-filter="masukSearch" placeholder="Search SKU, nama, from, to, status, PIC, keterangan" value="${esc(f.masukSearch)}"><input type="date" data-reject-filter="masukTanggalFrom" value="${esc(f.masukTanggalFrom)}"><input type="date" data-reject-filter="masukTanggalTo" value="${esc(f.masukTanggalTo)}"><input class="search-lg" data-reject-filter="masukLokasi" placeholder="Filter from/to" value="${esc(f.masukLokasi)}"></div><div class="table-wrap reject-table-wrap"><table><thead><tr>${REJECT_IN_COLUMNS.map(c=>thRejectStatic(c[0])).join('')}${thRejectStatic('Aksi')}</tr></thead><tbody>${pageRows.length?renderRejectTableRows(pageRows,REJECT_IN_COLUMNS,'masuk'):`<tr><td colspan="15"><div class="state">Belum ada barang masuk reject.</div></td></tr>`}</tbody></table></div>${rejectPager('masuk',rows.length)}</div>`;}
function renderRejectOut(){const f=BARANG_REJECT_STATE.filters;let rows=BARANG_REJECT_STATE.keluar.filter(r=>rejectTokensMatch(r,f.keluarSearch)&&dateInRange(r.tanggal,f.keluarTanggalFrom,f.keluarTanggalTo)&&(!f.keluarStatus||clean([r.statusBarang,r.statusProses,r.status].join(' ')).includes(clean(f.keluarStatus)))&&(!f.keluarPic||clean(r.pic).includes(clean(f.keluarPic))));rows=orderRejectSheetRows(rows);const pageRows=paginateReject(rows,'keluar');return `<div class="reject-page-title-row"><div><h3>Barang Keluar Reject</h3><small class="subtitle">Dua kolom STATUS dibedakan menjadi Status Barang dan Status Proses.</small></div><div class="row"><button class="btn-primary" data-reject-open-form="keluar" ${!canCrud()?'hidden':''}><i data-lucide="plus"></i>Tambah Data</button><button class="btn-ghost" data-reject-export="keluar">Export</button></div></div><div class="card reject-card"><div class="reject-filter-grid"><input class="search-lg reject-search-main" data-reject-filter="keluarSearch" placeholder="Search SKU, nama, from, to, PIC, status, iSeller" value="${esc(f.keluarSearch)}"><input type="date" data-reject-filter="keluarTanggalFrom" value="${esc(f.keluarTanggalFrom)}"><input type="date" data-reject-filter="keluarTanggalTo" value="${esc(f.keluarTanggalTo)}"><input class="search-lg" data-reject-filter="keluarStatus" placeholder="Filter status" value="${esc(f.keluarStatus)}"><input class="search-lg" data-reject-filter="keluarPic" placeholder="Filter PIC" value="${esc(f.keluarPic)}"></div><div class="table-wrap reject-table-wrap"><table><thead><tr>${REJECT_OUT_COLUMNS.map(c=>thRejectStatic(c[0])).join('')}${thRejectStatic('Aksi')}</tr></thead><tbody>${pageRows.length?renderRejectTableRows(pageRows,REJECT_OUT_COLUMNS,'keluar'):`<tr><td colspan="15"><div class="state">Belum ada barang keluar reject.</div></td></tr>`}</tbody></table></div>${rejectPager('keluar',rows.length)}</div>`;}
