...
function closeColumnMenus(){document.querySelectorAll(".mv-columns.open").forEach(el=>el.classList.remove("open"));}
...
function toggleColumnMenu(mode){const target=document.getElementById(`mv-cols-${mode}`);if(!target)return;const willOpen=!target.classList.contains("open");closeColumnMenus();if(willOpen){target.classList.add("open");positionColumnMenu(mode);}}
...
const filterHtml=`<div class='mv-toolbar'><button class='btn-ghost' data-mv-action='reset' data-mv-mode='${mode}'>Reset Filter</button><button class='btn-ghost' data-mv-action='export' data-mv-mode='${mode}'>Export CSV</button><div class='mv-column-dropdown-wrap'><button class='btn-ghost' data-mv-action='columns' data-mv-mode='${mode}'>Kolom</button><div id='mv-cols-${mode}' class='mv-columns'><div class='mv-columns-head'><strong>Atur Kolom</strong><div class='mv-columns-actions'><button type='button' onclick='toggleAllColumns("${mode}",true)'>Pilih semua</button><button type='button' onclick='toggleAllColumns("${mode}",false)'>Reset</button></div></div><div class='mv-columns-list'>${allCols.map(c=>`<label><input type='checkbox' ${st.columns.includes(c)?"checked":""} value='${c}' onchange='toggleColumnVisibility("${mode}")'> <span>${c}</span></label>`).join("")}</div></div></div><select id='mv-sort-${mode}' data-mv-filter data-mv-mode='${mode}'><option value='latest'>Terbaru</option><option value='oldest'>Terlama</option><option value='sku'>SKU A-Z</option><option value='name'>Nama A-Z</option><option value='qtyDesc'>Qty terbesar</option><option value='qtyAsc'>Qty terkecil</option></select><select id='mv-size-${mode}' data-mv-filter data-mv-mode='${mode}'><option value='25'>25</option><option value='50'>50</option></select></div>`;
