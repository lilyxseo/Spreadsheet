...
function orderRejectSheetRows(rows){return [...(Array.isArray(rows)?rows:[])].sort((a,b)=>(new Date(b?.tanggal).getTime()||Number(b?.rowNumber)||0)-(new Date(a?.tanggal).getTime()||Number(a?.rowNumber)||0));}
...
function renderRejectRecentActivity(){const latest=[...orderRejectSheetRows(BARANG_REJECT_STATE.masuk).slice(0,5).map(r=>['Masuk',r]),...orderRejectSheetRows(BARANG_REJECT_STATE.keluar).slice(0,5).map(r=>['Keluar',r])].sort((a,b)=>(new Date(b[1]?.tanggal).getTime()||Number(b[1]?.rowNumber)||0)-(new Date(a[1]?.tanggal).getTime()||Number(a[1]?.rowNumber)||0)).slice(0,6);return `<article class="card reject-insight-card reject-insight-card--wide"><h4>Aktivitas terakhir barang masuk/keluar</h4><div class="reject-activity-list">${latest.length?latest.map(([type,r])=>`<div class="reject-activity-row"><span class="badge ${type==='Masuk'?'b-in':'b-out'}">${type}</span><div><strong>${esc(r.sku||'-')}</strong><small>${esc(r.namaBarang||r.lokasi||r.from||'-')}</small></div><div class="reject-activity-meta"><strong>${Number(r.qty)||0}</strong><small>${esc(r.tanggal||'-')}</small></div></div>`).join(''):`<div class="state">Belum ada aktivitas reject.</div>`}</div></article>`;}
function renderRejectDashboardDetails(){const stock=BARANG_REJECT_STATE.stock.filter(r=>(Number(r.qty)||0)>0);const masukMonth=sumQty(thisMonthRows(BARANG_REJECT_STATE.masuk));const keluarMonth=sumQty(thisMonthRows(BARANG_REJECT_STATE.keluar));const insight=masukMonth>keluarMonth?`Qty reject bulan ini naik ${masukMonth-keluarMonth} dari barang keluar.`:keluarMonth>masukMonth?`Barang keluar reject lebih tinggi ${keluarMonth-masukMonth}; stok reject berkurang.`:'Masuk dan keluar reject bulan ini seimbang.';return `<div class="reject-insight-grid">${renderRejectMiniList('Auto Insight Reject',[[insight,'']],{empty:'Belum ada insight'})}${renderRejectMiniList('Top 5 SKU Reject terbanyak',topRejectEntries(stock,'sku',5))}${renderRejectRecentActivity()}</div>`;}
...
function renderRejectDashboard(){return `${renderRejectCards()}${renderRejectDashboardDetails()}<div class="reject-chart-grid"><article class="card reject-chart reject-chart--combined"><h4>Grafik sederhana Masuk vs Keluar per bulan</h4><div class="reject-chart-grid reject-chart-grid--inner">${renderRejectChart(BARANG_REJECT_STATE.masuk,'Masuk','in')}${renderRejectChart(BARANG_REJECT_STATE.keluar,'Keluar','out')}</div></article></div>`;}
...
function rejectActionButtons(kind,r){if(!canCrud())return '';return `<td class="reject-actions"><button class="icon-btn" data-reject-edit="${kind}" data-row="${Number(r.rowNumber)||''}" title="Edit inline"><i data-lucide="pencil"></i></button></td>`;}
...
function renderRejectIn(){const f=BARANG_REJECT_STATE.filters;let rows=BARANG_REJECT_STATE.masuk.filter(r=>rejectTokensMatch(r,f.masukSearch)&&dateInRange(r.tanggal,f.masukTanggalFrom,f.masukTanggalTo)&&(!f.masukLokasi||clean(r.lokasi).includes(clean(f.masukLokasi))));rows=orderRejectSheetRows(rows);const pageRows=paginateReject(rows,'masuk');return `<div class="reject-page-title-row"><div><h3>Barang Masuk Reject</h3><small class="subtitle">Search SKU/nama/lokasi/keterangan, filter tanggal dan lokasi.</small></div><div class="row"><button class="btn-primary" data-reject-open-form="masuk" ${!canCrud()?'hidden':''}><i data-lucide="plus"></i>Tambah Data</button><button class="btn-ghost" data-reject-export="masuk">Export</button></div></div><div class="card reject-card"><div class="reject-filter-grid"><input class="search-lg reject-search-main" data-reject-filter="masukSearch" placeholder="Search SKU, nama, lokasi, keterangan" value="${esc(f.masukSearch)}"><input type="date" data-reject-filter="masukTanggalFrom" value="${esc(f.masukTanggalFrom)}"><input type="date" data-reject-filter="masukTanggalTo" value="${esc(f.masukTanggalTo)}"><input class="search-lg" data-reject-filter="masukLokasi" placeholder="Filter lokasi" value="${esc(f.masukLokasi)}"></div><div class="table-wrap reject-table-wrap"><table><thead><tr>${['Tanggal','Lokasi','SKU','Nama Barang','Qty','Keterangan','User','Aksi'].map(thRejectStatic).join('')}</tr></thead><tbody>${pageRows.map(r=>`<tr><td data-label="Tanggal">${esc(r.tanggal)}</td><td data-label="Lokasi">${esc(r.lokasi)}</td><td data-label="SKU">${esc(r.sku)}</td><td data-label="Nama Barang">${esc(r.namaBarang)}</td><td data-label="Qty"><strong>${Number(r.qty)||0}</strong></td><td data-label="Keterangan">${esc(r.keterangan)}</td><td data-label="User">${esc(r.user)}</td>${rejectActionButtons('masuk',r)||'<td data-label="Aksi">-</td>'}</tr>`).join('')||`<tr><td colspan="8"><div class="state">Belum ada barang masuk reject.</div></td></tr>`}</tbody></table></div>${rejectPager('masuk',rows.length)}</div>`;}
function renderRejectOut(){const f=BARANG_REJECT_STATE.filters;let rows=BARANG_REJECT_STATE.keluar.filter(r=>rejectTokensMatch(r,f.keluarSearch)&&dateInRange(r.tanggal,f.keluarTanggalFrom,f.keluarTanggalTo)&&(!f.keluarStatus||clean(r.status).includes(clean(f.keluarStatus)))&&(!f.keluarPic||clean(r.pic).includes(clean(f.keluarPic))));rows=orderRejectSheetRows(rows);const pageRows=paginateReject(rows,'keluar');return `<div class="reject-page-title-row"><div><h3>Barang Keluar Reject</h3><small class="subtitle">Search semua kolom penting, filter status dan PIC.</small></div><div class="row"><button class="btn-primary" data-reject-open-form="keluar" ${!canCrud()?'hidden':''}><i data-lucide="plus"></i>Tambah Data</button><button class="btn-ghost" data-reject-export="keluar">Export</button></div></div><div class="card reject-card"><div class="reject-filter-grid"><input class="search-lg reject-search-main" data-reject-filter="keluarSearch" placeholder="Search SKU, nama, from, to, PIC, status, iSeller" value="${esc(f.keluarSearch)}"><input type="date" data-reject-filter="keluarTanggalFrom" value="${esc(f.keluarTanggalFrom)}"><input type="date" data-reject-filter="keluarTanggalTo" value="${esc(f.keluarTanggalTo)}"><input class="search-lg" data-reject-filter="keluarStatus" placeholder="Filter status" value="${esc(f.keluarStatus)}"><input class="search-lg" data-reject-filter="keluarPic" placeholder="Filter PIC" value="${esc(f.keluarPic)}"></div><div class="table-wrap reject-table-wrap"><table><thead><tr>${['Tanggal','From','To','SKU','Nama Barang','Qty','Status','PIC','Keterangan','No iSeller','Netsuite','Lokasi Surat Jalan','Aksi'].map(thRejectStatic).join('')}</tr></thead><tbody>${pageRows.map(r=>`<tr><td data-label="Tanggal">${esc(r.tanggal)}</td><td data-label="From">${esc(r.from)}</td><td data-label="To">${esc(r.to)}</td><td data-label="SKU">${esc(r.sku)}</td><td data-label="Nama Barang">${esc(r.namaBarang)}</td><td data-label="Qty"><strong>${Number(r.qty)||0}</strong></td><td data-label="Status">${rejectStatusBadge(r.status)}</td><td data-label="PIC">${esc(r.pic)}</td><td data-label="Keterangan">${esc(r.keterangan)}</td><td data-label="No iSeller">${esc(r.noIseller)}</td><td data-label="Netsuite">${esc(r.netsuite)}</td><td data-label="Lokasi Surat Jalan">${esc(r.lokasiSuratJalan)}</td>${rejectActionButtons('keluar',r)||'<td data-label="Aksi">-</td>'}</tr>`).join('')||`<tr><td colspan="13"><div class="state">Belum ada barang keluar reject.</div></td></tr>`}</tbody></table></div>${rejectPager('keluar',rows.length)}</div>`;}
function todayIso(){return new Date().toISOString().slice(0,10);}
function renderRejectForm(kind){const isIn=kind==='masuk';return `<div class="reject-modal-backdrop" data-reject-close-form><aside class="reject-drawer" role="dialog" aria-modal="true" aria-label="Form ${isIn?'Barang Masuk':'Barang Keluar'} Reject" onclick="event.stopPropagation()"><div class="reject-drawer-head"><div><h3>${isIn?'Barang Masuk Reject':'Barang Keluar Reject'}</h3><small>Isi data tanpa pindah halaman.</small></div><button class="icon-btn" data-reject-close-form><i data-lucide="x"></i></button></div><form id="${isIn?'rejectInForm':'rejectOutForm'}" class="reject-form" data-crud-form><div class="reject-form-grid"><label class="reject-field"><span>Tanggal *</span><input type="date" name="tanggal" value="${todayIso()}" required></label>${isIn?'<label class="reject-field"><span>Lokasi *</span><input name="lokasi" required placeholder="AREA HOLD"></label>':'<label class="reject-field"><span>From *</span><input name="from" required placeholder="Lokasi asal"></label><label class="reject-field"><span>To</span><input name="to" placeholder="Tujuan"></label>'}<label class="reject-field"><span>SKU *</span><input name="sku" required placeholder="SKU"></label><label class="reject-field"><span>Nama Barang</span><input name="namaBarang" placeholder="Nama barang"></label><label class="reject-field"><span>Qty *</span><input type="number" name="qty" min="1" step="1" required></label>${isIn?'':'<label class="reject-field"><span>Status</span><input name="status" placeholder="Pending / Done"></label><label class="reject-field"><span>PIC</span><input name="pic" placeholder="PIC"></label><label class="reject-field"><span>No iSeller</span><input name="noIseller"></label><label class="reject-field"><span>Netsuite</span><input name="netsuite"></label><label class="reject-field"><span>Lokasi Surat Jalan</span><input name="lokasiSuratJalan"></label>'}<label class="reject-field reject-field--full"><span>Keterangan</span><textarea name="keterangan" rows="3"></textarea></label></div><div class="reject-form-actions"><button class="btn-primary" type="submit">Simpan</button><button class="btn-ghost" type="button" data-reject-close-form>Batal</button></div></form></aside></div>`;}
function renderBarangRejectPage(){const root=document.getElementById('barangRejectApp');if(!root)return;document.querySelectorAll('.side-link[data-page]').forEach(b=>b.classList.toggle('active',b.dataset.page===getBarangRejectNavPage()));document.querySelectorAll('[data-reject-tab]').forEach(b=>b.classList.toggle('active',b.dataset.rejectTab===BARANG_REJECT_STATE.activeTab));document.getElementById('rejectPermissionBadge')?.toggleAttribute('hidden',canCrud());syncActiveSidebarParent('barang-reject');ensureBarangRejectLoaded();const body=BARANG_REJECT_STATE.loading&&!BARANG_REJECT_STATE.stock.length?`<div class="card reject-skeleton"><div class="skeleton-line"></div><div class="skeleton-line"></div><div class="skeleton-line"></div></div>`:BARANG_REJECT_STATE.activeTab==='dashboard'?renderRejectDashboard():BARANG_REJECT_STATE.activeTab==='masuk'?renderRejectIn():BARANG_REJECT_STATE.activeTab==='keluar'?renderRejectOut():renderRejectDashboard();root.innerHTML=`<div class="reject-meta"><span class="subtitle">Sheet: KARTU STOCK KST • Barang Masuk • Barang Keluar</span><span class="subtitle">Last sync: ${BARANG_REJECT_STATE.lastSync?formatDateTime(BARANG_REJECT_STATE.lastSync):'-'}</span>${BARANG_REJECT_STATE.error?`<span class="state error">${esc(BARANG_REJECT_STATE.error)}</span>`:''}</div>${body}`;bindBarangRejectEvents();if(window.lucide)lucide.createIcons();}
function bindBarangRejectEvents(){document.getElementById('rejectRefreshBtn')?.addEventListener('click',()=>fetchBarangRejectData());document.querySelectorAll('[data-reject-tab]').forEach(btn=>btn.onclick=()=>{BARANG_REJECT_STATE.activeTab=btn.dataset.rejectTab;history.pushState({},'',btn.dataset.rejectTab==='dashboard'?'/barang-reject':`/barang-reject/${btn.dataset.rejectTab}`);renderBarangRejectPage();});document.querySelectorAll('[data-reject-filter]').forEach(input=>input.oninput=()=>{BARANG_REJECT_STATE.filters[input.dataset.rejectFilter]=input.value;BARANG_REJECT_STATE.page.stock=BARANG_REJECT_STATE.page.masuk=BARANG_REJECT_STATE.page.keluar=1;renderBarangRejectPage();});document.querySelectorAll('[data-reject-page]').forEach(btn=>btn.onclick=()=>{const k=btn.dataset.rejectPage;BARANG_REJECT_STATE.page[k]=Math.max(1,(BARANG_REJECT_STATE.page[k]||1)+(btn.dataset.dir==='next'?1:-1));renderBarangRejectPage();});document.querySelectorAll('[data-reject-export]').forEach(btn=>btn.onclick=()=>exportBarangRejectCsv(btn.dataset.rejectExport));document.querySelectorAll('[data-reject-open-form]').forEach(btn=>btn.onclick=()=>{if(!guardCrudAction('CREATE','/barang-reject'))return;document.body.insertAdjacentHTML('beforeend',renderRejectForm(btn.dataset.rejectOpenForm));bindRejectSubmitForms();if(window.lucide)lucide.createIcons();});document.querySelectorAll('[data-reject-edit]').forEach(btn=>btn.onclick=()=>toast('Inline edit mengikuti permission CRUD. Penyimpanan edit akan aktif saat endpoint update tersedia.','info'));}
function rejectFormPayload(form){return Object.fromEntries(new FormData(form).entries());}
async function submitRejectForm(kind,form){const isIn=kind==='masuk';const stateKey=isIn?'submittingIn':'submittingOut';if(BARANG_REJECT_STATE[stateKey])return;const payload=rejectFormPayload(form);payload.qty=Number(payload.qty);payload.action=kind;payload.user=currentUserIdentity().user_name||currentUserIdentity().user_id||'';if(!payload.tanggal||!payload.sku||!(payload.qty>0)||!(isIn?payload.lokasi:payload.from)){toast('Tanggal, SKU, Qty > 0, dan Lokasi/From wajib diisi.','error');return;}BARANG_REJECT_STATE[stateKey]=true;try{const res=await fetch('/api/barang-reject',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal submit Barang Reject');toast(data.message||'Data berhasil disimpan','success');document.querySelector('.reject-modal-backdrop')?.remove();await fetchBarangRejectData({background:true});BARANG_REJECT_STATE.activeTab=kind;history.replaceState({},'',`/barang-reject/${kind}`);}catch(err){toast(err?.message||'Gagal submit Barang Reject','error');}finally{BARANG_REJECT_STATE[stateKey]=false;renderBarangRejectPage();}}
...
function exportBarangRejectCsv(kind){const maps={masuk:{rows:BARANG_REJECT_STATE.masuk,headers:['Tanggal','Lokasi','SKU','Nama Barang','Qty','Keterangan','User'],cols:['tanggal','lokasi','sku','namaBarang','qty','keterangan','user']},keluar:{rows:BARANG_REJECT_STATE.keluar,headers:['Tanggal','From','To','SKU','Nama Barang','Qty','Status','PIC','Keterangan','No iSeller','Netsuite','Lokasi Surat Jalan'],cols:['tanggal','from','to','sku','namaBarang','qty','status','pic','keterangan','noIseller','netsuite','lokasiSuratJalan']}};const cfg=maps[kind]||maps.masuk;const lines=[cfg.headers.join(','),...cfg.rows.map(r=>cfg.cols.map(c=>`"${String(r[c]??'').replaceAll('"','""')}"`).join(','))];const blob=new Blob([lines.join('\n')],{type:'text/csv;charset=utf-8;'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`barang-reject-${kind}.csv`;a.click();URL.revokeObjectURL(a.href);toast('Export CSV Barang Reject berhasil','success');}
