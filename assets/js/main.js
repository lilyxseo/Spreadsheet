...
const BALIKAN_STATE={sheets:[],sheetName:"",rows:[],searchKeyword:"",highlightRowNumber:null};
let BALIKAN_ROWS=[];
let currentTripSheet="";
let balikanSearchKeyword="";
...
balikanSheetSelect?.addEventListener("change",async(e)=>{BALIKAN_STATE.sheetName=e.target.value||"";currentTripSheet=BALIKAN_STATE.sheetName;BALIKAN_STATE.highlightRowNumber=null;await loadBalikanRows();});
balikanSearchInput?.addEventListener("input",e=>{BALIKAN_STATE.searchKeyword=e.target.value||"";balikanSearchKeyword=BALIKAN_STATE.searchKeyword;renderBalikanTable();});
...
async function loadBalikanRows(){if(!BALIKAN_STATE.sheetName){balikanSummary.textContent='';balikanTable.innerHTML='';BALIKAN_ROWS=[];return;}try{currentTripSheet=BALIKAN_STATE.sheetName;const res=await fetch(`/api/balikan-store?sheetName=${encodeURIComponent(BALIKAN_STATE.sheetName)}`);const data=await res.json();if(!res.ok)throw new Error(data?.message||'Gagal memuat data Balikan Store');BALIKAN_STATE.rows=Array.isArray(data?.rows)?data.rows:[];BALIKAN_ROWS=BALIKAN_STATE.rows;balikanSearchKeyword=BALIKAN_STATE.searchKeyword;renderBalikanTable();balikanSummary.textContent=`${BALIKAN_STATE.rows.length} baris dari ${BALIKAN_STATE.sheetName}`;}catch(err){toast(err?.message||'Gagal memuat data Balikan Store','error');}}

function isCheckedValue(value){if(value===true)return true;const v=String(value??"").trim().toLowerCase();return v==="true"||v==="1"||v==="yes"||v==="ya"||v==="checked"||v==="✓"||v==="☑";}
...
function renderBalikanTable(){const rows=filterBalikanRows(BALIKAN_ROWS||[],balikanSearchKeyword);if(!rows.length){balikanTable.innerHTML='<div class="subtitle">Data kosong.</div>';return;}balikanTable.innerHTML=`<div class="balikan-table-wrapper"><table class="balikan-table"><thead><tr><th class="col-check">Centang</th><th>No</th><th>SKU</th><th>Nama Barang</th><th>Qty</th><th>Rak tujuan</th><th>Lokasi</th><th>Stok Bulky</th><th>Stok Retail</th><th>Status</th><th>Keterangan</th></tr></thead><tbody>${rows.map(r=>`<tr id="balikan-row-${Number(r.rowNumber)}" data-row-number="${Number(r.rowNumber)}" class="${BALIKAN_STATE.highlightRowNumber===Number(r.rowNumber)?'balikan-row-highlight':''}"><td class="col-check"><input type="checkbox" ${isCheckedValue(r.checked)?'checked':''} onchange="toggleBalikanCheck('${encAttr(currentTripSheet||BALIKAN_STATE.sheetName)}', ${Number(r.rowNumber)}, this.checked)"></td><td>${esc(r.no)}</td><td>${esc(r.sku)}</td><td>${esc(r.namaBarang)}</td><td>${esc(r.qty)}</td><td>${esc(r.rakTujuan)}</td><td>${esc(r.lokasi)}</td><td>${esc(r.stokBulky)}</td><td>${esc(r.stokRetail)}</td><td>${esc(r.status)}</td><td>${esc(r.keterangan)}</td></tr>`).join('')}</tbody></table></div>`;}

async function updateBalikanCheck(sheetName,rowNumber,checked,{refresh=true}={}){const row=(BALIKAN_ROWS||[]).find(r=>Number(r.rowNumber)===Number(rowNumber));const targetSheet=sheetName||currentTripSheet||BALIKAN_STATE.sheetName;const res=await fetch('/api/balikan-store/check',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({sheetName:targetSheet,rowNumber,checked})});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal update centang');if(row)row.checked=checked;toast('Centang berhasil diupdate','success');logActivitySafe({action:'CHECK_BALIKAN_STORE',module:'Balikan Store',detail:`Centang SKU ${row?.sku||'-'} di ${targetSheet}`,status:'SUCCESS',metadata:{sheetName:targetSheet,sku:row?.sku||'',checked,rowNumber}});if(refresh)await loadBalikanRows();}
window.toggleBalikanCheck=async(sheetName,rowNumber,checked)=>{try{await updateBalikanCheck(sheetName,Number(rowNumber),Boolean(checked));}catch(err){toast(err?.message||'Gagal update centang','error');renderBalikanTable();throw err;}};
...
async function handleBalikanScanResult(decodedText){if(navigator.vibrate)navigator.vibrate(100);const sku=cleanScannedSku(decodedText);BALIKAN_STATE.searchKeyword=sku;const searchInput=document.querySelector("#balikanSearchInput");if(searchInput)searchInput.value=sku;const row=(BALIKAN_STATE.rows||[]).find(item=>String(item.sku||"").trim().toLowerCase()===String(sku||"").trim().toLowerCase());if(!row){BALIKAN_STATE.highlightRowNumber=null;toast("SKU tidak ditemukan di Balikan Store: "+sku,"error");renderBalikanTable();return;}await toggleBalikanCheck(BALIKAN_STATE.sheetName,Number(row.rowNumber),true);await loadBalikanRows();BALIKAN_STATE.highlightRowNumber=Number(row.rowNumber);setTimeout(()=>{const el=document.querySelector(`[data-row-number="${row.rowNumber}"]`);if(el){el.scrollIntoView({behavior:"smooth",block:"center"});el.classList.add("row-highlight");}},300);showToast("SKU berhasil discan dan dicentang: "+sku,"success");}
...
document.addEventListener('click',async e=>{const b=e.target.closest('[data-mvh-action],[data-cch-action]');if(!b)return;try{if(b.dataset.mvhAction){const row=Number(b.dataset.row);const src=buildMovementHistoryRows().find(x=>x.rowNumber===row);if(b.dataset.mvhAction==='edit')HISTORY_EDIT_STATE.movement[row]={...src};if(b.dataset.mvhAction==='cancel')delete HISTORY_EDIT_STATE.movement[row];if(b.dataset.mvhAction==='save'){await updateHistoryRow('movement','Movement',row,HISTORY_EDIT_STATE.movement[row]);delete HISTORY_EDIT_STATE.movement[row];MOVEMENT_HISTORY_REMOTE.loaded=false;await ensureMovementHistoryLoaded();toast('Movement berhasil diupdate','success');logActivitySafe({action:'EDIT_MOVEMENT',module:'Movement',detail:`Edit movement row ${row}`,reference:`rowNumber ${row}`,status:'SUCCESS'});}if(b.dataset.mvhAction==='delete'){showConfirmModal({title:'Hapus Row Movement',message:'Yakin ingin menghapus row movement ini?',confirmText:'Hapus',cancelText:'Batal',type:'danger',onConfirm:async()=>{try{await deleteHistoryRow('movement','Movement',row);MOVEMENT_HISTORY_REMOTE.loaded=false;await ensureMovementHistoryLoaded();toast('Movement berhasil dihapus','success');logActivitySafe({action:'DELETE_MOVEMENT',module:'Movement',detail:`Delete movement row ${row}`,reference:`rowNumber ${row}`,status:'SUCCESS'});renderMovementHistory();}catch(err){toast(err?.message||'Aksi gagal','error');}}});return;}renderMovementHistory();}if(b.dataset.cchAction){const row=Number(b.dataset.row);const src=buildCycleHistoryRows().find(x=>x.rowNumber===row);if(b.dataset.cchAction==='edit')HISTORY_EDIT_STATE.cycle[row]={...src};if(b.dataset.cchAction==='cancel')delete HISTORY_EDIT_STATE.cycle[row];if(b.dataset.cchAction==='save'){await updateHistoryRow('cycle_count','Cycle Count',row,HISTORY_EDIT_STATE.cycle[row]);delete HISTORY_EDIT_STATE.cycle[row];CYCLE_HISTORY_REMOTE.loaded=false;await ensureCycleHistoryLoaded();toast('Cycle count berhasil diupdate','success');logActivitySafe({action:'EDIT_CYCLE_COUNT',module:'Cycle Count',detail:`Edit cycle count row ${row}`,reference:`rowNumber ${row}`,status:'SUCCESS'});}if(b.dataset.cchAction==='delete'){showConfirmModal({title:'Hapus Row Cycle Count',message:'Yakin ingin menghapus row cycle count ini?',confirmText:'Hapus',cancelText:'Batal',type:'danger',onConfirm:async()=>{try{await deleteHistoryRow('cycle_count','Cycle Count',row);CYCLE_HISTORY_REMOTE.loaded=false;await ensureCycleHistoryLoaded();toast('Cycle count berhasil dihapus','success');logActivitySafe({action:'DELETE_CYCLE_COUNT',module:'Cycle Count',detail:`Delete cycle count row ${row}`,reference:`rowNumber ${row}`,status:'SUCCESS'});renderCycleHistory();}catch(err){toast(err?.message||'Aksi gagal','error');}}});return;}renderCycleHistory();}}catch(err){toast(err?.message||'Aksi gagal','error');}});
