...
async function updateBalikanCell(sheetName,row,field,value,oldValue){const wrapper=document.querySelector('.balikan-table-wrapper');const scrollTop=wrapper?wrapper.scrollTop:0;const scrollLeft=wrapper?wrapper.scrollLeft:0;const res=await fetch('/api/balikan-store/cell',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({sheetName,rowNumber:row.rowNumber,field,value})});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal update data');await loadBalikanRows();requestAnimationFrame(()=>{const newWrapper=document.querySelector('.balikan-table-wrapper');if(newWrapper){newWrapper.scrollTop=scrollTop;newWrapper.scrollLeft=scrollLeft;}});showToast('Data berhasil diupdate','success');logActivitySafe({action:'EDIT_BALIKAN_STORE',module:'Balikan Store',detail:`Edit field ${field} SKU ${row?.sku||'-'} di ${sheetName}`,status:'SUCCESS',metadata:{sheetName,rowNumber:row.rowNumber,sku:row?.sku||'',field,oldValue:String(oldValue??''),newValue:String(value??'')}});}
