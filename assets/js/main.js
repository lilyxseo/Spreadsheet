...
function routeFromPath(path){if(!user)return showLoginView();if(path==="/")return showPage("dashboard");if(path==="/search")return showPage("search");if(path==="/barang-masuk")return showPage("barang-masuk");if(path==="/barang-keluar")return showPage("barang-keluar");if(path==="/accuracy-dashboard"||path==="/accuracy"||path==="/dashboard-akurasi")return showPage("stats");if(path==="/statistics"){history.replaceState({},"","/");return showPage("dashboard");}if(path==="/locations"||path==="/location")return showPage("locations");if(path==="/settings")return showPage("settings");if(path==="/sheet-input")return showPage("sheet-input");if(path==="/arsip")return showPage("arsip");if(path==="/asset-store")return showPage("asset-store");if(path==="/cycle-count")return showPage("cycle-count");if(path==="/movement")return showPage("movement");if(path==="/abc-analisis")return showPage("abc-analisis");if(path==="/balikan-store")return showPage("balikan-store");if(path==="/activity-log"){if(!isDeveloperUser()){history.replaceState({},"","/");return showPage("dashboard");}return showPage("activity-log");}if(path==="/anomaly"){history.replaceState({},"","/warning");return showPage("anomaly");}if(path==="/warning")return showPage("anomaly");if(path==="/stok-minus")return showPage("stok-minus");if(path.startsWith("/sku/")){currentSku=decodeURIComponent(path.split("/sku/")[1]||"");if(currentSku)showDetail(currentSku);return showPage("detail");}showPage("dashboard");}
...
const ABC_STATE={periodMonths:3,excludeTo:["REJECT","RUSAK"],rows:[],warehouses:new Set(),selectedWarehouse:"all",selectedCategory:"all",stats:{A:0,B:0,C:0}};
function parseDateLoose(v){const s=String(v||"").trim();if(!s)return null;const d=new Date(s);if(!Number.isNaN(d.getTime()))return d;const m=s.match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{2,4})$/);if(!m)return null;const y=Number(m[3]);return new Date(y<100?2000+y:y,Number(m[2])-1,Number(m[1]));}
function getAbcRekomendasi(item){const stok=Number(item.stokSaatIni)||0;const min=Number(item.minStok)||0;if(item.kategori==="A"&&stok<min)return "RESTOCK PRIORITAS";if(item.kategori==="A")return "AMAN";if(item.kategori==="C"&&stok>Math.max(min*2,10))return "OVERSTOCK";if(item.kategori==="C"&&stok<=min)return "LOW PRIORITY";return "NORMAL";}
function computeAbcAnalysis(){const end=new Date();const start=new Date(end);start.setMonth(start.getMonth()-ABC_STATE.periodMonths);const ex=new Set(ABC_STATE.excludeTo.map(v=>String(v).toUpperCase()));const m=new Map();ABC_STATE.warehouses=new Set();let total=0;getBarangKeluarRows().forEach(r=>{const dt=parseDateLoose(r?.tanggal);if(!dt||dt<start||dt>end)return;const to=String(r?.to||"").trim();if(ex.has(to.toUpperCase()))return;if(ABC_STATE.selectedWarehouse!=="all"&&to!==ABC_STATE.selectedWarehouse)return;const sku=String(r?.sku||r?.namaBarang||"").trim();if(!sku)return;const qty=Number(r?.qty)||0;if(qty<=0)return;ABC_STATE.warehouses.add(to||"-");const k=sku.toUpperCase();const prev=m.get(k)||{sku,namaBarang:String(r?.namaBarang||""),qty:0,stokSaatIni:0,minStok:0};prev.qty+=qty;m.set(k,prev);total+=qty;});const desc=[...m.values()].sort((a,b)=>b.qty-a.qty);let cum=0;desc.forEach(it=>{it.kontribusi=total?it.qty/total:0;cum+=it.kontribusi;it.cumulative=cum;it.kategori=cum<=0.8?"A":cum<=0.95?"B":"C";it.statusRekomendasi=getAbcRekomendasi(it);});ABC_STATE.rows=desc.sort((a,b)=>a.qty-b.qty).filter(r=>ABC_STATE.selectedCategory==="all"||r.kategori===ABC_STATE.selectedCategory);ABC_STATE.stats={A:desc.filter(r=>r.kategori==="A").length,B:desc.filter(r=>r.kategori==="B").length,C:desc.filter(r=>r.kategori==="C").length};}
function renderAbcAnalisisPage(){if(!abcAnalysisApp)return;computeAbcAnalysis();if(!ABC_STATE.rows.length){abcAnalysisApp.innerHTML="<div class='card'><div class='state'>Tidak ada data pada periode ini</div></div>";return;}const topA=[...ABC_STATE.rows].filter(r=>r.kategori==='A').sort((a,b)=>b.qty-a.qty).slice(0,10);const topC=[...ABC_STATE.rows].filter(r=>r.kategori==='C').sort((a,b)=>a.qty-b.qty).slice(0,10);abcAnalysisApp.innerHTML=`<div class='card'><div class='mv-filters open'><select id='abcPeriod'><option value='1'>1 bulan terakhir</option><option value='2'>2 bulan terakhir</option><option value='3'>3 bulan terakhir</option><option value='4'>4 bulan terakhir</option><option value='5'>5 bulan terakhir</option><option value='6'>6 bulan terakhir</option></select><select id='abcWarehouse'><option value='all'>Semua TO</option>${[...ABC_STATE.warehouses].sort().map(w=>`<option value='${esc(w)}'>${esc(w)}</option>`).join('')}</select><select id='abcCategory'><option value='all'>Semua Kategori</option><option value='A'>Kategori A</option><option value='B'>Kategori B</option><option value='C'>Kategori C</option></select><button id='abcRefreshBtn' class='btn-primary'>Refresh</button><button id='abcExportBtn' class='btn-ghost'>Export CSV</button></div><div class='grid dashboard' style='margin-top:10px'><div class='metric'><div class='k'>Total SKU A</div><div class='v'>${ABC_STATE.stats.A}</div></div><div class='metric'><div class='k'>Total SKU B</div><div class='v'>${ABC_STATE.stats.B}</div></div><div class='metric'><div class='k'>Total SKU C</div><div class='v'>${ABC_STATE.stats.C}</div></div></div><div class='table-wrap table-wrap-full abc-table-wrap'><table><thead><tr><th>SKU / Nama Barang</th><th>Total Qty Keluar</th><th>% Kontribusi</th><th>Cumulative %</th><th>Kategori</th><th>Stok Saat Ini</th><th>Minimum Stok</th><th>Status Rekomendasi</th></tr></thead><tbody>${ABC_STATE.rows.map(r=>`<tr><td>${esc(r.sku)}${r.namaBarang?`<div class='subtitle'>${esc(r.namaBarang)}</div>`:''}</td><td>${r.qty}</td><td>${(r.kontribusi*100).toFixed(2)}%</td><td>${(r.cumulative*100).toFixed(2)}%</td><td><span class='abc-cat abc-${r.kategori}'>${r.kategori}</span></td><td>${r.stokSaatIni}</td><td>${r.minStok}</td><td>${esc(r.statusRekomendasi)}</td></tr>`).join('')}</tbody></table></div><div class='grid dashboard' style='margin-top:10px'><div class='card'><strong>Top 10 Fast Moving</strong><ol>${topA.map(r=>`<li>${esc(r.sku)} (${r.qty})</li>`).join('')}</ol></div><div class='card'><strong>Top 10 Slow Moving</strong><ol>${topC.map(r=>`<li>${esc(r.sku)} (${r.qty})</li>`).join('')}</ol></div></div></div>`;document.getElementById('abcPeriod').value=String(ABC_STATE.periodMonths);document.getElementById('abcWarehouse').value=ABC_STATE.selectedWarehouse;document.getElementById('abcCategory').value=ABC_STATE.selectedCategory;}
document.addEventListener('change',e=>{if(e.target?.id==='abcPeriod'){ABC_STATE.periodMonths=Number(e.target.value)||3;renderAbcAnalisisPage();}if(e.target?.id==='abcWarehouse'){ABC_STATE.selectedWarehouse=e.target.value||'all';renderAbcAnalisisPage();}if(e.target?.id==='abcCategory'){ABC_STATE.selectedCategory=e.target.value||'all';renderAbcAnalisisPage();}});
document.addEventListener('click',e=>{if(e.target?.id==='abcRefreshBtn')renderAbcAnalisisPage();if(e.target?.id==='abcExportBtn'){const cols=['sku','nama_barang','qty_keluar','kontribusi','cumulative','kategori','stok_saat_ini','minimum_stok','status_rekomendasi'];const lines=[cols.join(',')].concat((ABC_STATE.rows||[]).map(r=>[r.sku,r.namaBarang,r.qty,(r.kontribusi*100).toFixed(2),(r.cumulative*100).toFixed(2),r.kategori,r.stokSaatIni,r.minStok,r.statusRekomendasi].map(v=>`"${String(v??'').replaceAll('"','""')}"`).join(',')));const blob=new Blob([lines.join('\n')],{type:'text/csv;charset=utf-8;'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='abc-analisis.csv';a.click();URL.revokeObjectURL(a.href);}});
