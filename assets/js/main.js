...
const CYCLE_STATE={step:1,page:1,pageSize:15,search:"",loc:"all",cat:"all",session:null,rows:[],history:[]};
function createCycleSession(payload={}){const t=new Date();return{id:`CC-${t.getFullYear()}${String(t.getMonth()+1).padStart(2,"0")}${String(t.getDate()).padStart(2,"0")}-${Math.random().toString(36).slice(2,6).toUpperCase()}`,name:payload.name||"",date:payload.date||t.toISOString().slice(0,10),pic:payload.pic||"",location:payload.location||"",note:payload.note||"",status:"draft",createdAt:t.toISOString(),completedAt:""};}
function getCycleSkuBase(){const rows=[...(DATA["Kartu Stock"]||[]),...(DATA["RPL"]||[]),...(DATA["BULKY"]||[])];const m=new Map();rows.forEach(r=>{const sku=getVal(r,["sku"]);if(!sku)return;const key=clean(sku);if(!m.has(key))m.set(key,{sku:String(sku),nama:getVal(r,["nama barang","nama","item","description"])||"-",lokasi:getVal(r,["lokasi","location","rak","bin"])||"-",kategori:getVal(r,["kategori","category"])||"Umum",stokSistem:0});const it=m.get(key);it.stokSistem+=parseNumber(getVal(r,["stok akhir","stok global","qty","stok"])||0);});return [...m.values()];}
function calculateDifference(r){const qty=Number(r.qtyAktual||0);const diff=qty-Number(r.stokSistem||0);const status=diff===0?"Cocok":(diff>0?"Plus":"Minus");return {...r,qtyAktual:qty,selisih:diff,status};}
function addSkuToSession(items=[]){items.forEach(it=>{if(!it?.sku)return; if(CYCLE_STATE.rows.some(r=>clean(r.sku)===clean(it.sku)))return;CYCLE_STATE.rows.push(calculateDifference({...it,qtyAktual:0,catatan:""}));});saveCycleDraft();}
function updateActualQty(sku,val){const i=CYCLE_STATE.rows.findIndex(r=>clean(r.sku)===clean(sku));if(i<0)return;CYCLE_STATE.rows[i]=calculateDifference({...CYCLE_STATE.rows[i],qtyAktual:Number(val||0)});saveCycleDraft();}
function getCycleSummary(){const total=CYCLE_STATE.rows.length;const cocok=CYCLE_STATE.rows.filter(r=>r.selisih===0).length;const selisih=total-cocok;const totalSel=CYCLE_STATE.rows.reduce((n,r)=>n+r.selisih,0);const akurasi=total?((cocok/total)*100):0;return{total,cocok,selisih,totalSel,akurasi};}
function finalizeSession(){if(!CYCLE_STATE.session)return;CYCLE_STATE.session.status="selesai";CYCLE_STATE.session.completedAt=new Date().toISOString();CYCLE_STATE.history.unshift({session:{...CYCLE_STATE.session},rows:[...CYCLE_STATE.rows]});saveCycleDraft();toast("Cycle Count selesai");renderCycleCountPage();}
function exportCycleCountCsv(){const cols=["SKU","Nama Barang","Lokasi","Stok Sistem","Qty Aktual","Selisih","Status","Catatan"];const lines=[cols.join(","),...CYCLE_STATE.rows.map(r=>[r.sku,r.nama,r.lokasi,r.stokSistem,r.qtyAktual,r.selisih,r.status,r.catatan||""].map(v=>`"${String(v??"").replaceAll('"','""')}"`).join(","))];const blob=new Blob([lines.join("
")],{type:"text/csv;charset=utf-8;"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`cycle-count-${(CYCLE_STATE.session?.id||"draft")}.csv`;a.click();URL.revokeObjectURL(a.href);}
function saveCycleDraft(){localStorage.setItem(CYCLE_KEY,JSON.stringify({state:CYCLE_STATE}));toast("Draft tersimpan");}
function loadCycleDraft(){try{const raw=JSON.parse(localStorage.getItem(CYCLE_KEY)||"{}").state;if(raw){Object.assign(CYCLE_STATE,raw);}}catch(_){}}
function renderCycleCountPage(){if(!cycleCountApp)return; if(!CYCLE_STATE.session)CYCLE_STATE.session=createCycleSession();const summary=getCycleSummary();const base=getCycleSkuBase();const q=clean(CYCLE_STATE.search);const options=base.filter(r=>(!q||clean(`${r.sku} ${r.nama}`).includes(q))&&(CYCLE_STATE.loc==="all"||r.lokasi===CYCLE_STATE.loc)&&(CYCLE_STATE.cat==="all"||r.kategori===CYCLE_STATE.cat));const maxPage=Math.max(1,Math.ceil(options.length/CYCLE_STATE.pageSize));if(CYCLE_STATE.page>maxPage)CYCLE_STATE.page=maxPage;const paged=options.slice((CYCLE_STATE.page-1)*CYCLE_STATE.pageSize,CYCLE_STATE.page*CYCLE_STATE.pageSize);const done=CYCLE_STATE.session.status==="selesai";
cycleCountApp.innerHTML=`<div class='cc-head card'><div><h3 class='page-title'>Cycle Count</h3><div class='subtitle'>Hitung stok fisik vs stok sistem secara bertahap.</div></div><span class='badge ${done?"b-kartu":"b-warn"}'>${done?"Selesai":"Draft"}</span></div><div class='cc-summary grid dashboard'>${[["Total SKU",summary.total],["SKU Cocok",summary.cocok],["SKU Selisih",summary.selisih],["Total Selisih",summary.totalSel],["Akurasi",`${summary.akurasi.toFixed(1)}%`]].map(c=>`<div class='metric'><div class='k'>${c[0]}</div><div class='v'>${c[1]}</div></div>`).join("")}</div><div class='cc-stepper'>${["Setup","Pilih SKU","Hitung Aktual","Review"].map((t,i)=>`<button class='btn-ghost ${CYCLE_STATE.step===i+1?"active":""}' onclick='setCycleStep(${i+1})'>${i+1}. ${t}</button>`).join("")}</div><div class='card cc-body'>${CYCLE_STATE.step===1?`<div class='mv-filters'><input id='ccName' placeholder='Nama sesi' value='${esc(CYCLE_STATE.session.name)}' ${done?"disabled":""}><input id='ccDate' type='date' value='${esc(CYCLE_STATE.session.date)}' ${done?"disabled":""}><input id='ccPic' placeholder='PIC' value='${esc(CYCLE_STATE.session.pic)}' ${done?"disabled":""}><input id='ccLoc' placeholder='Lokasi (opsional)' value='${esc(CYCLE_STATE.session.location)}' ${done?"disabled":""}><input id='ccNote' placeholder='Catatan' value='${esc(CYCLE_STATE.session.note)}' ${done?"disabled":""}></div>`:''}${CYCLE_STATE.step===2?`<div class='mv-toolbar'><input id='ccSearch' class='search-lg' placeholder='Search SKU / nama barang' value='${esc(CYCLE_STATE.search)}'><button class='btn-ghost' onclick='addMinusSku()' ${done?"disabled":""}>Ambil dari stok minus</button><button class='btn-ghost' onclick='addAnomalySku()' ${done?"disabled":""}>Ambil dari anomaly</button></div><div class='table-wrap table-wrap-full'><table><thead><tr><th>SKU</th><th>Nama</th><th>Lokasi</th><th>Kategori</th><th>Stok Sistem</th><th>Action</th></tr></thead><tbody>${paged.map(r=>`<tr><td>${esc(r.sku)}</td><td>${esc(r.nama)}</td><td>${esc(r.lokasi)}</td><td>${esc(r.kategori)}</td><td>${r.stokSistem}</td><td><button class='btn-ghost' onclick='addSkuToSession([${JSON.stringify(r).replaceAll("'","&#39;")}])' ${done?"disabled":""}>Tambah</button></td></tr>`).join("")||"<tr><td colspan='6'><div class='state'>Belum ada SKU.</div></td></tr>"}</tbody></table></div>`:''}${CYCLE_STATE.step===3?`<div class='table-wrap table-wrap-full'><table><thead><tr><th>SKU</th><th>Nama Barang</th><th>Lokasi</th><th>Stok Sistem</th><th>Qty Aktual</th><th>Selisih</th><th>Status</th><th>Catatan</th></tr></thead><tbody>${CYCLE_STATE.rows.map(r=>`<tr class='${r.selisih!==0?"cc-diff":""}'><td>${esc(r.sku)}</td><td>${esc(r.nama)}</td><td>${esc(r.lokasi)}</td><td>${r.stokSistem}</td><td><input class='cc-qty' type='number' value='${r.qtyAktual}' onchange='updateActualQty("${encAttr(r.sku)}",this.value)' ${done?"disabled":""}></td><td>${r.selisih}</td><td><span class='badge ${r.status==="Cocok"?"b-in":r.status==="Plus"?"b-warn":"b-out"}'>${r.status}</span></td><td><input value='${esc(r.catatan||"")}' onchange='updateCycleNote("${encAttr(r.sku)}",this.value)' ${done?"disabled":""}></td></tr>`).join("")||"<tr><td colspan='8'><div class='state'>Belum ada SKU dipilih.</div></td></tr>"}</tbody></table></div>`:''}${CYCLE_STATE.step===4?`<div class='stats-rank-grid'><div class='card'><strong>Review</strong><p>Total SKU: ${summary.total}</p><p>SKU Cocok: ${summary.cocok}</p><p>SKU Selisih: ${summary.selisih}</p><p>Total Selisih Qty: ${summary.totalSel}</p><p>Akurasi: ${summary.akurasi.toFixed(1)}%</p></div><div class='card'><strong>History</strong>${CYCLE_STATE.history.slice(0,5).map(h=>`<div>${esc(h.session.id)} - ${esc(h.session.status)}</div>`).join("")||"<div class='subtitle'>Belum ada history</div>"}</div></div>`:''}</div><div class='row' style='margin-top:12px'><button class='btn-ghost' onclick='saveCycleMeta()'>Simpan Draft</button><button class='btn-primary' onclick='nextCycleStep()'>Lanjut</button><button class='btn-primary' onclick='finalizeSession()' ${done?"disabled":""}>Selesaikan Cycle Count</button><button class='btn-ghost' onclick='exportCycleCountCsv()'>Export CSV</button></div>`;
if(CYCLE_STATE.step===2)document.getElementById('ccSearch')?.addEventListener('input',debounce(e=>{CYCLE_STATE.search=e.target.value;CYCLE_STATE.page=1;renderCycleCountPage();},220));}
function saveCycleMeta(){if(!CYCLE_STATE.session)CYCLE_STATE.session=createCycleSession();["name","date","pic","location","note"].forEach((k)=>{const id={name:"ccName",date:"ccDate",pic:"ccPic",location:"ccLoc",note:"ccNote"}[k];const el=document.getElementById(id);if(el)CYCLE_STATE.session[k]=el.value||"";});saveCycleDraft();}
function setCycleStep(step){CYCLE_STATE.step=step;renderCycleCountPage();}
function nextCycleStep(){saveCycleMeta();CYCLE_STATE.step=Math.min(4,CYCLE_STATE.step+1);renderCycleCountPage();}
function updateCycleNote(sku,val){const i=CYCLE_STATE.rows.findIndex(r=>clean(r.sku)===clean(sku));if(i<0)return;CYCLE_STATE.rows[i].catatan=val||"";saveCycleDraft();}
function addMinusSku(){addSkuToSession(buildStokMinusRows().slice(0,30).map(r=>({sku:r.sku,nama:r.nama,lokasi:"-",kategori:"Stok Minus",stokSistem:r.stokEstimate})));renderCycleCountPage();}
function addAnomalySku(){addSkuToSession((ANOMALY_STATE.rows||[]).slice(0,30).map(r=>({sku:r.sku,nama:r.nama,lokasi:"-",kategori:"Anomaly",stokSistem:0})));renderCycleCountPage();}
...
window.renderAnomalyPage=renderAnomalyPage;window.changeAnomalyPage=changeAnomalyPage;window.createCycleSession=createCycleSession;window.addSkuToSession=addSkuToSession;window.updateActualQty=updateActualQty;window.calculateDifference=calculateDifference;window.getCycleSummary=getCycleSummary;window.finalizeSession=finalizeSession;window.exportCycleCountCsv=exportCycleCountCsv;window.setCycleStep=setCycleStep;window.nextCycleStep=nextCycleStep;window.saveCycleMeta=saveCycleMeta;window.updateCycleNote=updateCycleNote;window.addMinusSku=addMinusSku;window.addAnomalySku=addAnomalySku;loadCycleDraft();
