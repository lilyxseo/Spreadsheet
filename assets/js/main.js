...
function updateLocalRow(moduleName,rowNumber,field,newValue){
const rows=moduleName==='barangMasuk'?getBarangMasukRows():getBarangKeluarRows();
const target=rows.find(x=>Number(x.rowNumber)===Number(rowNumber));
if(target)target[field]=newValue;
window.APP_STATE=window.APP_STATE||{};
if(moduleName==='barangMasuk')window.APP_STATE.barangMasuk=rows;
else window.APP_STATE.barangKeluar=rows;
return rows;
}
function updateCacheRow(moduleName,rowNumber,field,newValue){
const rows=updateLocalRow(moduleName,rowNumber,field,newValue);
if(moduleName==='barangMasuk'){setCacheSafe(MODULE_CACHE_KEYS.barangMasuk,rows);setModuleCache(MODULE_CACHE_KEYS.barangMasuk,rows);}
else{setCacheSafe(MODULE_CACHE_KEYS.barangKeluar,rows);setModuleCache(MODULE_CACHE_KEYS.barangKeluar,rows);}
return rows;
}
