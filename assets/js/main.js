...
const cards=[["Total SKU",skuSet.size],["Baris Kartu Stock",totals["Kartu Stock"]],["Baris RPL",totals["RPL"]],["Baris BULKY",totals["BULKY"]],["Barang Masuk",inSummary.totalQty,`+${inSummary.todayQty} hari ini`],["Barang Keluar",outSummary.totalQty,`+${outSummary.todayQty} hari ini`],["Lokasi terpakai",lokasiTerpakai],["Lokasi tersisa",lokasiTersisa]];
dashboardCards.innerHTML=cards.map(c=>`<div class='metric'><div class='k'>${c[0]}</div><div class='v'>${c[1]}</div>${c[2]?`<div class='muted'>${c[2]}</div>`:""}</div>`).join("");
...
if(!dateKey)continue;
totalQty+=qty;
if(dateKey===todayKey)todayQty+=qty;
}
return{totalQty,todayQty};
}
