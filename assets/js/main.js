...
function getCycleSourceRows(){return (DATA["Kartu Stock"]||[]).map(r=>({sku:String(getVal(r,["sku"])||"").trim(),nama:String(getVal(r,["nama barang","nama","item","description"])||"-").trim()||"-",lokasi:String(getVal(r,["lokasi","location","rak","bin","area"])||"-").trim()||"-",bulky:parseNumber(getVal(r,["stok bulky","bulky","qty bulky"])),retail:parseNumber(getVal(r,["stok retail","retail","qty retail"])),qtySystem:parseNumber(getVal(r,["stok akhir","current stock","closing stock","ending stock","saldo akhir","qty available","total stok","qty"]))})).filter(r=>r.sku);}
function getCycleSearchQty(item){const qtySystem=Number(item?.qtySystem);if(Number.isFinite(qtySystem))return qtySystem;const bulky=Number(item?.bulky)||0;const retail=Number(item?.retail)||0;if(bulky>0)return bulky;if(retail>0)return retail;return 0;}
...
window.ccAddItem=(skuEnc,lokEnc)=>{const sku=decodeURIComponent(skuEnc),lokasi=decodeURIComponent(lokEnc);const source=getCycleSourceRows().find(r=>clean(r.sku)===clean(sku)&&clean(r.lokasi)===clean(lokasi));if(!source)return;const newItem={...source,bulky:Number.isFinite(Number(source.bulky))?Number(source.bulky):0,retail:Number.isFinite(Number(source.retail))?Number(source.retail):0,aktualBulky:null,aktualRetail:null,catatan:""};const key=cycleItemKey(newItem);if(CYCLE_STATE.sessionItems.some(it=>cycleItemKey(it)===key)){toast('SKU/lokasi sudah ada di session.','error');return;}CYCLE_STATE.sessionItems.push(newItem);renderCycleSessionTable();toast('Item ditambahkan','success');};
