...
function beginAssetStoreInlineEdit(cell){const rowNumber=Number(cell.dataset.rowNumber);const field=cell.dataset.field||"";const sheetName=ASSET_STORE_STATE.selectedSheet;if(!sheetName||!rowNumber||!field)return;const parsed=ASSET_STORE_STATE.cache[sheetName];const row=(parsed?.rows||[]).find(r=>Number(r.__rowNumber)===rowNumber);if(!row)return;startInlineEdit(cell,row,field,row[field],{onSave:async({value,oldValue,td})=>{const key=`${sheetName}:${rowNumber}:${field}`;ASSET_STORE_STATE.saveStatus[key]='saving';td.dataset.saveState='saving';const old=String(oldValue??'');row[field]=value;td.textContent=value||'-';try{await fetch('/api/asset-store/cell',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({spreadsheetId:ASSET_STORE_SPREADSHEET_ID,sheetName,rowNumber,field,headers:parsed.columns,value})});ASSET_STORE_STATE.saveStatus[key]='saved';td.dataset.saveState='saved';setTimeout(()=>{if(td.dataset.saveState==='saved')td.dataset.saveState='';},1200);}catch(err){row[field]=old;td.textContent=old||'-';ASSET_STORE_STATE.saveStatus[key]='error';td.dataset.saveState='error';throw err;}}});}
