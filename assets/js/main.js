...
function rejectTokensMatch(row,query){const q=normalizeSearch(query);if(!q)return true;const hay=normalizeSearch([row.sku,row.namaBarang,row.lokasi,row.from,row.to,row.pic,row.status,row.keterangan,row.noIseller,row.netsuite,row.lokasiSuratJalan].join(' '));const skuText=normalizeSearch(row.sku);return skuText.includes(q)||q.split(/\s+/).filter(Boolean).every(tok=>hay.includes(tok));}
