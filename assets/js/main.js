...
let isSubmittingMovementScan=false;
...
function renderMovementSession(){return `<div class='cc-section'><div class='mv-toolbar'><input id='mvSearch' class='search-lg' placeholder='Cari SKU / nama barang / lokasi' value='${esc(MOVEMENT_STATE.searchInput)}'></div></div><div class='cc-section'><div class='row' style='gap:8px;flex-wrap:wrap'><input id='movementFrom' class='search-lg' placeholder='From (wajib dipilih)'><input id='movementTo' class='search-lg' placeholder='To (wajib dipilih)'><input id='movementQty' type='number' class='search-lg' placeholder='Qty / Stok Aktual' value='1' min='1'><input id='movementScanInput' class='search-lg' placeholder='Scan SKU...' autocomplete='off'></div></div><div class='cc-section'><div class='table-wrap cc-table-wrap cc-search-wrap'><table><thead><tr><th>Lokasi</th><th>SKU</th><th>Nama Barang</th><th>Qty</th><th>Aksi</th></tr></thead><tbody id='mvSearchResultsBody'></tbody></table></div></div><div class='cc-section'><h4>Movement Berjalan</h4><div class='table-wrap cc-table-wrap cc-session-wrap'><table><thead><tr><th>From</th><th>To</th><th>SKU</th><th>Nama Barang</th><th>Stok Lokasi Awal</th><th>Stok Aktual</th><th>Aksi</th></tr></thead><tbody id='mvSessionBody'></tbody></table></div></div>`;}
function renderMovementPage(){if(!movementApp)return;movementApp.innerHTML=`<div class='card cc-card cc-section'><div class='section-header cc-main-header'><div class='cc-action-stack'>${MOVEMENT_STATE.sessionActive?"<button class='btn-ghost' onclick='mvCancelSession()'>Batal Movement</button>":""}<button class='btn-primary' onclick='mvStartSession()' ${MOVEMENT_STATE.sessionActive?"disabled":""}>Mulai Movement</button>${MOVEMENT_STATE.sessionActive?"<button id='mvSubmitBtn' class='btn-primary' onclick='mvSubmitSession()'>Selesai Movement</button>":""}</div></div>${MOVEMENT_HISTORY_REMOTE.error?`<div class='state cc-state state-error'>${esc(MOVEMENT_HISTORY_REMOTE.error)}</div>`:""}${MOVEMENT_STATE.sessionActive?renderMovementSession():renderMovementEmptyState()}</div><div class='card cc-card cc-section mv-history-section'><div class='section-header'><h4>History Movement</h4></div><div class='table-wrap cc-table-wrap'><table><thead><tr><th>Tanggal</th><th>From</th><th>To</th><th>SKU</th><th>Nama Barang</th><th>Stok Lokasi Awal</th><th>Stok Aktual</th><th>Keterangan</th><th>Aksi</th></tr></thead><tbody id='mvHistoryBody'></tbody></table></div><div class='mv-pagination'><span id='mvHistoryInfo'>Menampilkan 0-0 dari 0 data</span><div class='row'><select id='mvHistoryPageSize'><option value='10'>10</option><option value='25'>25</option><option value='50'>50</option></select><button id='mvHistoryPrev' class='btn-ghost'>Prev</button><button id='mvHistoryNext' class='btn-ghost'>Next</button></div></div></div>`;if(window.lucide)window.lucide.createIcons();if(MOVEMENT_STATE.sessionActive){renderMovementSearchResults();renderMovementSessionTable();initMovementScanner();}renderMovementHistory();}

function findInventoryItemBySku(sku,from){const skuClean=clean(sku);const fromClean=clean(from);const rows=getMovementSourceRows();const preferred=rows.find(r=>clean(r.sku)===skuClean&&(!fromClean||clean(r.lokasi)===fromClean));if(preferred)return preferred;return rows.find(r=>clean(r.sku)===skuClean)||null;}
async function submitMovementScan(payload){const tanggal=formatTanggal(new Date());const rawRole=window.currentUser?.role||"";const normalizedRole=normalizeRole(rawRole);const pic=APP_CONFIG.PIC_BY_ROLE[normalizedRole]||APP_CONFIG.PIC_BY_ROLE.inventory||"ABI";const req={tanggal,from:payload.from,to:payload.to,sku:payload.sku,namaBarang:payload.namaBarang,qty:Number(payload.stokAktual),pic,stokDiLokasiAwal:Number(payload.stokDiLokasiAwal||0),stokAktual:Number(payload.stokAktual)};const res=await fetch('/api/movement/in',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(req)});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal menyimpan movement');logActivitySafe({action:'CREATE_MOVEMENT_SCAN',module:'Movement',detail:`[SCAN CREATE] Movement\nSKU: ${payload.sku}\nQty: ${payload.stokAktual}\nFrom: ${payload.from}\nTo: ${payload.to}`,status:'SUCCESS',metadata:{sku:payload.sku,namaBarang:payload.namaBarang,from:payload.from,to:payload.to,stokDiLokasiAwal:payload.stokDiLokasiAwal,stokAktual:payload.stokAktual,source:'scan'}});}
async function handleMovementScanSubmit(sku){if(isSubmittingMovementScan)return;isSubmittingMovementScan=true;try{const from=document.getElementById('movementFrom')?.value?.trim();const to=document.getElementById('movementTo')?.value?.trim();const qty=Number(document.getElementById('movementQty')?.value||1);if(!from||!to){showToast('Pilih From dan To dulu','error');return;}const item=findInventoryItemBySku(sku,from);if(!item){showToast('SKU tidak ditemukan','error');return;}await submitMovementScan({sku,namaBarang:item.nama,from,to,stokDiLokasiAwal:item.stok_akhir||'',stokAktual:Number.isFinite(qty)&&qty>0?qty:1,source:'scan'});MOVEMENT_HISTORY_REMOTE.loaded=false;await ensureMovementHistoryLoaded();if(typeof syncData==='function')syncData();showToast('Movement berhasil disimpan','success');}catch(err){showToast(err?.message||'Gagal menyimpan movement','error');}finally{isSubmittingMovementScan=false;}}
function initMovementScanner(){const input=document.getElementById('movementScanInput');if(!input||input.dataset.bound==='1')return;input.addEventListener('keydown',async(e)=>{if(e.key!=='Enter')return;e.preventDefault();const sku=input.value.trim();if(!sku)return;await handleMovementScanSubmit(sku);input.value='';input.focus();});input.dataset.bound='1';setTimeout(()=>input.focus(),0);}
