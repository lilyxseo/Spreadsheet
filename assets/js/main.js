...
header.dari=findVal('Dari');
header.kepada=findVal('Kepada');
...
try{for(const row of validRows){const payload={tanggal:new Date().toISOString().slice(0,10),sku:row.sku,nama_barang:row.namaProduk||'-',qty:Number(row.qty)||0,lokasi:PDF_TRANSFER_STATE.header.kepada||'-',keterangan:`Transfer ${PDF_TRANSFER_STATE.header.nomorTransfer||'-'}`};const res=await fetch('/api/sheet-input',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const out=await res.json();if(!res.ok||!out?.success)throw new Error(out?.message||'Gagal import');}
