...
if(skuIdx<0){
const isHeaderLike=/^(sku|nama produk|jumlah|diterima|batal|tolak|catatan)$/i.test(line);
const isShortNoteLike=/^[A-Za-z][A-Za-z\s\-\/]{1,24}$/.test(line)&&!/\d/.test(line)&&line.trim().split(/\s+/).length<=3;
if(parsed.length&&!isHeaderLike&&isShortNoteLike){
parsed[parsed.length-1].catatan=`${parsed[parsed.length-1].catatan||""} ${line}`.trim();
continue;
}
...
const qtyValues=numericIndices.slice(0,4).map(idx=>Number(after[idx]||0));
const jumlah=Number(qtyValues[0]||0),diterima=Number(qtyValues[1]||0),batal=Number(qtyValues[2]||0),tolak=Number(qtyValues[3]||0);
let catatan='';
if(numericIndices.length){
const lastQtyPos=numericIndices[Math.min(3,numericIndices.length-1)];
catatan=after.slice(lastQtyPos+1).join(' ').trim();
}
