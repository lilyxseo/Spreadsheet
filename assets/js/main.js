...
for(let i=0;i<itemRows.length;i++){const row=itemRows[i];if(/grand total|^total\b|status transfer|nomor referensi|perkiraan tanggal/i.test(row))break;const line=String(row||'').replace(/\s+/g,' ').trim();if(!line)continue;const tokens=line.split(' ').filter(Boolean);
const skuIdx=tokens.findIndex(tok=>{
if(!isSkuCandidate(tok))return false;
return skuMasterMap.has(clean(tok));
});
if(skuIdx<0){
const isLikelyNote=parsed.length&&/^[A-Za-z]/.test(line)&&line.length<=50&&!/^(sku|nama produk|jumlah|diterima|batal|tolak|catatan)$/i.test(line);
if(isLikelyNote){parsed[parsed.length-1].catatan=`${parsed[parsed.length-1].catatan||""} ${line}`.trim();continue;}
