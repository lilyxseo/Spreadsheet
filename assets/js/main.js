...
import { createCycleCountModule } from "./pages/cycle-count.js";
...
const cycleCountModule = createCycleCountModule({ getData:()=>DATA, toast:(m)=>toast(m) });
...
function showPage(page){document.querySelectorAll(".page").forEach(p=>p.classList.add("hidden"));document.getElementById(`page-${page}`).classList.remove("hidden");document.querySelectorAll(".side-link").forEach(b=>b.classList.toggle("active",b.dataset.page===page));closeSidebarMobile();if(page==="cycle-count"){cycleCountModule.mount();return;}if(!window.__isDataReady){console.log("DATA READY", window.__isDataReady);return;}rerenderCurrentPage();}
