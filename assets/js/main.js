...
const SETTINGS_TABS=[['general','Umum'],['sources','Sumber Data'],['mapping','Mapping Sheet'],['sync','Sinkronisasi'],['cache','Cache & Performa'],['permissions','Hak Akses'],['appearance','Tampilan'],['notifications','Notifikasi'],['developer','Developer'],['backup','Backup & Restore']];
const SETTINGS_MAPPING_MODULES=[
{name:'Dashboard',source:'Inventory utama',sheets:['Kartu Stock','Barang Masuk','Barang Keluar','RPL','BULKY'],required:['TANGGAL','SKU','NAMA BARANG','QTY','LOKASI']},
{name:'Barang Balikan',source:'Balikan Store',sheets:['Semua sheet TRIP','Sheet Barang Balikan terkait'],required:['TANGGAL','SKU','NAMA BARANG','QTY','STATUS']},
{name:'Asset Store',source:'Asset Store',sheets:['Semua sheet Asset Store'],required:['SKU','NAMA BARANG','STOK','LOKASI']},
{name:'Barang Reject',source:'Reject default',spreadsheetId:'1BVGcIWnYqrG-DefzmnO_hZjNn3H3c4sriI7CBAWsxw8',sheets:['KARTU STOCK KST','Barang Masuk','Barang Keluar'],required:['TANGGAL','FROM','TO','SKU','NAMA BARANG','QTY','STATUS','PIC','KETERANGAN','NO ISELLER','NETSUITE','KETERANGAN LAINNYA','LOKASI SURAT JALAN'],duplicate:'STATUS disimpan berdasarkan index kolom asli; Barang Masuk memakai STCKOUT REJECT.'},
{name:'Cycle Count',source:'Cycle Count',sheets:['Sheet Cycle Count'],required:['Tanggal','Lokasi','SKU','Nama Barang','STOK','Aktual','Catatan'],note:'Header lama bulky, retail, aktual_bulky, aktual_retail tidak digunakan.'},
{name:'Movement',source:'Movement',sheets:['Sheet Movement'],required:['TANGGAL','FROM','TO','SKU','NAMA BARANG','QTY','STATUS','PIC']},
{name:'Lokasi',source:'Kartu stok/lokasi',sheets:['Sumber kartu stok/lokasi'],required:['SKU','NAMA BARANG','LOKASI','STOK']},
{name:'Arsip',source:'Arsip',sheets:['Sheet arsip terkait'],required:['TANGGAL','SKU','AKSI','PIC']},
{name:'Activity Log',source:'Activity Log',sheets:['Sheet Activity Log'],required:['USER','TIMESTAMP','ACTION','MODULE','RESULT']},
{name:'Audit Trail',source:'Audit Trail',sheets:['Sheet Audit Trail'],required:['User','Timestamp','Kategori','Field','Old value','New value','Result','Source']}
];
let settingsUnsaved=false;
function setSettingsUnsaved(value){settingsUnsaved=!!value;document.getElementById('settingsUnsavedBadge')?.classList.toggle('hidden',!settingsUnsaved);const btn=document.getElementById('settingsSaveBtn');if(btn)btn.disabled=!settingsUnsaved;}
function showSettingsTab(tab){if(settingsUnsaved&&!confirm('Ada perubahan belum disimpan. Pindah kategori tanpa menyimpan?'))return;document.querySelectorAll('[data-settings-tab]').forEach(b=>b.classList.toggle('active',b.dataset.settingsTab===tab));document.querySelectorAll('[data-settings-pane]').forEach(p=>p.classList.toggle('active',p.dataset.settingsPane===tab));const sel=document.getElementById('settingsCategorySelect');if(sel)sel.value=tab;}
function initSettingsPage(){const root=document.querySelector('[data-settings-root]');if(!root||root.dataset.bound)return;root.dataset.bound='1';const sel=document.getElementById('settingsCategorySelect');if(sel)sel.innerHTML=SETTINGS_TABS.map(([id,label])=>`<option value="${id}">${label}</option>`).join('');root.addEventListener('click',e=>{const tab=e.target.closest('[data-settings-tab]');if(tab)return showSettingsTab(tab.dataset.settingsTab);const action=e.target.closest('[data-settings-action]')?.dataset.settingsAction;if(!action)return;if(action==='save'){setSettingsUnsaved(false);toast('Draft pengaturan tervalidasi dan disimpan','success');logActivitySafe({action:'UPDATE_SETTINGS',module:'Settings',detail:'Simpan perubahan pengaturan',status:'SUCCESS'});}if(action==='cancel'){setSettingsUnsaved(false);toast('Perubahan draft dibatalkan','info');}if(action==='defaults'||action==='reset-all')showConfirmModal({title:'Konfirmasi Reset',message:'Reset pengaturan membutuhkan konfirmasi dan akan dicatat di Audit Trail.',confirmText:'Reset',cancelText:'Batal',type:'danger',onConfirm:()=>{setSettingsUnsaved(true);toast('Reset masuk draft. Klik Simpan untuk menerapkan.','warning');}});if(action==='clear-cache')clearSystemCache();if(action==='test-permission')toast('Permission efektif berhasil diuji','success');if(action==='validate-all')document.querySelectorAll('[data-validate-mapping]').forEach(btn=>btn.click());if(action==='export-config')toast('Export hanya menyertakan konfigurasi aman','success');if(action==='import-config')toast('Import membutuhkan preview perubahan sebelum restore','info');});root.addEventListener('input',e=>{if(e.target.matches('[data-settings-field]'))setSettingsUnsaved(true);if(e.target.id==='settingsSearchInput')renderSettingsSearch(e.target.value);});root.addEventListener('change',e=>{if(e.target.matches('[data-settings-field]'))setSettingsUnsaved(true);if(e.target.id==='settingsCategorySelect')showSettingsTab(e.target.value);});window.addEventListener('beforeunload',e=>{if(!settingsUnsaved)return;e.preventDefault();e.returnValue='';});}
function renderSettingsSearch(q){const box=document.getElementById('settingsSearchResults');if(!box)return;const term=String(q||'').trim().toLowerCase();if(!term){box.hidden=true;box.innerHTML='';return;}const hits=[...document.querySelectorAll('[data-settings-pane]')].filter(p=>(p.textContent+' '+(p.dataset.searchKeywords||'')).toLowerCase().includes(term)).slice(0,8);box.innerHTML=hits.map(p=>`<button type="button" data-jump-settings="${p.dataset.settingsPane}"><strong>${esc(SETTINGS_TABS.find(t=>t[0]===p.dataset.settingsPane)?.[1]||p.dataset.settingsPane)}</strong><small>${esc(p.querySelector('p')?.textContent||'')}</small></button>`).join('')||'<div class="settings-empty">Tidak ada hasil.</div>';box.hidden=false;box.querySelectorAll('[data-jump-settings]').forEach(btn=>btn.onclick=()=>{showSettingsTab(btn.dataset.jumpSettings);box.hidden=true;});}
function renderSettingsMapping(){const wrap=document.getElementById('settingsMappingList');if(!wrap)return;wrap.innerHTML=SETTINGS_MAPPING_MODULES.map((m,i)=>`<article class="settings-module-card"><div class="settings-module-head"><div><strong>${esc(m.name)}</strong><small>${esc(m.source)}${m.spreadsheetId?` · ${esc(m.spreadsheetId)}`:''}</small></div><span class="settings-badge ok">Aktif</span></div><div class="settings-chip-row">${m.sheets.map(s=>`<span>${esc(s)}</span>`).join('')}</div><div class="settings-required"><small>Header wajib</small><div>${m.required.map(h=>`<code>${esc(h)}</code>`).join('')}</div></div>${m.note?`<p class="settings-note">${esc(m.note)}</p>`:''}${m.duplicate?`<p class="settings-note warn">${esc(m.duplicate)}</p>`:''}<div class="settings-actions"><button class="btn-ghost" data-validate-mapping="${i}">Validasi Mapping</button><span class="settings-note" id="mappingLast${i}">Terakhir validasi: -</span></div><div class="settings-validation hidden" id="mappingResult${i}"></div></article>`).join('');wrap.querySelectorAll('[data-validate-mapping]').forEach(btn=>btn.onclick=()=>{const i=Number(btn.dataset.validateMapping),m=SETTINGS_MAPPING_MODULES[i],result=document.getElementById(`mappingResult${i}`);if(!result)return;const rows=m.required.map(h=>`<li class="ok">✓ ${esc(h)}</li>`);if(m.name==='Barang Reject')rows.splice(6,0,'<li class="warn">⚠ STATUS ditemukan dua kali — mapping manual: Status Barang (index 7) dan Status Proses (index 13)</li>');result.innerHTML=`<ul>${rows.join('')}<li class="muted">Kolom opsional dan header tidak dikenali ditinjau tanpa menyimpan otomatis.</li></ul>`;result.classList.remove('hidden');const last=document.getElementById(`mappingLast${i}`);if(last)last.textContent=`Terakhir validasi: ${formatSyncDate(Date.now())}`;});}
function updateSettingsDashboard(){
  initSettingsPage();renderSettingsMapping();
  document.querySelectorAll('[data-developer-only], [data-settings-pane="developer"]').forEach(el=>el.classList.toggle('hidden',!isDeveloperUser()));
  const totalRows=Object.values(DATA).reduce((n,rows)=>n+(Array.isArray(rows)?rows.length:0),0);const lastTs=getLastSyncTs();const status=getSystemStatusMeta();
  if(settingsLastRefresh)settingsLastRefresh.textContent=formatSyncDate(lastTs);if(settingsTotalRows)settingsTotalRows.textContent=`${totalRows.toLocaleString('id-ID')} row`;if(settingsSystemDot)settingsSystemDot.className=`status-dot ${status.dot}`;if(settingsSystemStatus)settingsSystemStatus.textContent=status.label;
  if(settingsCacheStatus)settingsCacheStatus.textContent=`Status cache: ${lastTs?'Aktif':'Tidak aktif'} · storage IndexedDB/cache tersegmentasi`;if(settingsCacheTime)settingsCacheTime.textContent=`Waktu terakhir cache: ${formatSyncDate(lastTs)}`;
  const current=getCurrentUser()||{};const role=getCurrentUserRole();const dev=isDeveloperUser();const full=dev||String(role).toUpperCase().includes('PIC');
  const perm=document.getElementById('settingsPermissionGrid');if(perm)perm.innerHTML=[['User aktif',current.name||current.email||user?.email||'-'],['Role database',role||'-'],['Status developer',dev?'Ya':'Tidak'],['Permission efektif',full?'Full CRUD':'Read-only'],['Read','Diizinkan'],['Create',full?'Diizinkan':'Ditolak'],['Update',full?'Diizinkan':'Ditolak'],['Delete',full?'Diizinkan':'Ditolak']].map(([k,v])=>`<div class="settings-status-card"><small>${esc(k)}</small><strong>${esc(v)}</strong></div>`).join('');
  const devStatus=document.getElementById('settingsDeveloperStatus');if(devStatus)devStatus.innerHTML=[['Environment',location.hostname.includes('localhost')?'Local':'Production'],['Build version','v1.0.0'],['API status',apiConnected?'Terhubung':'Tidak terhubung'],['Google Sheets status',apiConnected?'OK':'Belum diuji'],['Cache status',lastTs?'Fresh/available':'Kosong'],['Last fatal error','-']].map(([k,v])=>`<div class="settings-status-card"><small>${esc(k)}</small><strong>${esc(v)}</strong></div>`).join('');
  const sync=document.getElementById('settingsSyncStatus');if(sync){const mods=['Dashboard','Barang Balikan','Asset Store','Barang Reject','Cycle Count','Movement','Lokasi','Arsip','Activity Log','Audit Trail'];sync.innerHTML=mods.map(name=>`<div class="settings-status-card"><small>${esc(name)}</small><strong>${formatSyncDate(lastTs)}</strong><em>${((DATA[name]||[]).length||0).toLocaleString('id-ID')} row · ${lastTs?'cache':'server'} · error: -</em></div>`).join('');}
  if(settingsDataSources){const sources=[{name:'Inventory utama',id:SPREADSHEET_ID,mods:'Dashboard, Lokasi, Barang Masuk/Keluar',sheets:SHEETS.length,status:apiConnected?'Terhubung':'Belum diuji'},{name:'Barang Reject',id:'1BVGcIWnYqrG-DefzmnO_hZjNn3H3c4sriI7CBAWsxw8',mods:'Barang Reject',sheets:3,status:'Belum diuji'},{name:'Balikan Store',id:'••••••••••••••••••••',mods:'Barang Balikan',sheets:'TRIP + terkait',status:'Belum diuji'}];settingsDataSources.innerHTML=sources.map(src=>`<article class="settings-source-card"><div><strong>${esc(src.name)}</strong><small>Spreadsheet ID: <code>${esc(src.id)}</code></small><small>Service account/client email: readonly · private key tersembunyi</small></div><div class="settings-source-meta"><span class="settings-badge ${src.status==='Terhubung'?'ok':'warn'}">${esc(src.status)}</span><small>Terakhir berhasil: ${formatSyncDate(lastTs)}</small><small>Total sheet: ${esc(src.sheets)}</small><small>Modul: ${esc(src.mods)}</small><div><button class="btn-ghost">Test Connection</button><button class="btn-ghost">Edit</button><button class="btn-ghost">Nonaktifkan</button></div></div></article>`).join('');}
  if(window.lucide)lucide.createIcons();
}
