...
function bindEvents(){const d=debounce(()=>runSearch(),250);searchInput.addEventListener("input",d);sortSearch.addEventListener("change",()=>renderResults(lastResults,lastQuery));statsFilter.addEventListener("change",updateStats);darkBtnHeader.addEventListener("click",toggleDark);const din=debounce(()=>renderDataTablePage("in","Barang Masuk"),220),dout=debounce(()=>renderDataTablePage("out","Barang Keluar"),220);inSearch?.addEventListener("input",din);outSearch?.addEventListener("input",dout);inDate?.addEventListener("change",()=>renderDataTablePage("in","Barang Masuk"));outDate?.addEventListener("change",()=>renderDataTablePage("out","Barang Keluar"));searchModalInput?.addEventListener("input",debounce(()=>runSearch(true),250));searchModalClose?.addEventListener("click",closeSearchModal);openSearchModalBtn?.addEventListener("click",openSearchModal);document.addEventListener("keydown",onSearchShortcut);document.addEventListener("change",e=>{const t=e.target;if(t?.matches("[data-mv-filter]")){const m=t.dataset.mvMode;debouncedTableRender(m);}});document.addEventListener("click",e=>{if(e.target?.matches("[data-search-close]"))return closeSearchModal();const btn=e.target.closest("[data-mv-action]");if(!btn)return;const mode=btn.dataset.mvMode;const action=btn.dataset.mvAction;if(action==="reset")return resetMovementFilter(mode);if(action==="export")return exportFilteredCsv(mode);if(action==="prev"||action==="next")return paginateRows(mode,action);if(action==="toggle-filter"){document.getElementById(`mv-filters-${mode}`)?.classList.toggle("open");}if(action==="columns"){document.getElementById(`mv-cols-${mode}`)?.classList.toggle("open");}});}
...
function runSearch(fromModal=false){const srcInput=fromModal?searchModalInput:searchInput;const targetId=fromModal?"searchModalResults":"results";const qRaw=srcInput?.value||"";const q=clean(qRaw);lastQuery=qRaw;if(!q){lastResults=[];if(fromModal){renderRecentSearches();return renderState(targetId,"Ketik kata kunci untuk mencari.");}return renderState(targetId,"Masukkan kata kunci pencarian.");}const words=q.split(" ").filter(Boolean);const out=[];for(const it of CACHE_SKU.values()){if(currentFilter!=="Semua"&&!it.sources.has(currentFilter))continue;const skuN=clean(it.sku),nameN=clean(it.nama),combined=`${skuN} ${nameN}`;let rank=99;if(skuN===q)rank=1;else if(/^\d{4}$/.test(q)&&skuN.endsWith(q))rank=2;else if(skuN.includes(q))rank=3;else if(nameN===q)rank=4;else if(words.length&&words.every(w=>combined.includes(w)))rank=5;else if(words.some(w=>combined.includes(w)))rank=6;if(rank<99)out.push({...it,sources:[...it.sources],rank});}
lastResults=out.sort((a,b)=>a.rank-b.rank||a.sku.localeCompare(b.sku)).slice(0,50);if(fromModal)saveRecentSearch(qRaw);renderResults(lastResults,qRaw,targetId,fromModal);}
function renderResults(items,query,targetId="results",fromModal=false){if(sortSearch.value==="sku")items=[...items].sort((a,b)=>a.sku.localeCompare(b.sku));if(sortSearch.value==="name")items=[...items].sort((a,b)=>a.nama.localeCompare(b.nama));if(!items.length) return renderState(targetId,"Data tidak ditemukan.");const resultsEl=`<div class='subtitle'>${items.length} hasil (maks 50).</div><div class='result-list'>`+items.map(r=>{const skuE=encAttr(r.sku);const action=fromModal?`closeSearchModal();showDetail(decodeURIComponent('${skuE}'));navigateTo('/sku/'+encodeURIComponent(decodeURIComponent('${skuE}')))`:`showDetail(decodeURIComponent('${skuE}'));navigateTo('/sku/'+encodeURIComponent(decodeURIComponent('${skuE}')))`;return `<div class='result-card'><div class='result-head'><div><strong>${highlight(r.nama,query)}</strong><div>SKU: ${highlight(r.sku,query)}</div></div><div>${r.sources.map(s=>`<span class='badge ${badgeClass(s)}'>${esc(s)}</span>`).join(" ")}</div></div><div class='row'><button class='btn-ghost' onclick="copySku(decodeURIComponent('${skuE}'))">Copy SKU</button><button class='btn-primary' onclick="${action}">Lihat Detail</button></div></div>`;}).join("")+"</div>";document.getElementById(targetId).innerHTML=resultsEl;}
function getRecentSearches(){try{return JSON.parse(localStorage.getItem(SEARCH_HISTORY_KEY)||"[]").filter(Boolean).slice(0,5);}catch(_){return [];}}
function saveRecentSearch(query){const q=String(query||"").trim();if(!q)return;const next=[q,...getRecentSearches().filter(x=>clean(x)!==clean(q))].slice(0,5);localStorage.setItem(SEARCH_HISTORY_KEY,JSON.stringify(next));renderRecentSearches();}
function clearSearchHistory(){localStorage.removeItem(SEARCH_HISTORY_KEY);renderRecentSearches();}
function renderRecentSearches(){if(!searchHistory)return;const items=getRecentSearches();if(!items.length){searchHistory.innerHTML="";return;}searchHistory.innerHTML=`<div class='history-head'><small>Recent Search</small><button class='btn-ghost' onclick='clearSearchHistory()'>Clear History</button></div><div class='chip-group'>${items.map(i=>`<button class='chip' onclick=\"searchFromHistory(decodeURIComponent('${encAttr(i)}'))\">${esc(i)}</button>`).join("")}</div>`;}
function searchFromHistory(q){searchModalInput.value=q;runSearch(true);}
function openSearchModal(){searchModal?.classList.remove("hidden");searchModal?.setAttribute("aria-hidden","false");renderRecentSearches();setTimeout(()=>searchModalInput?.focus(),10);if(window.lucide)lucide.createIcons();}
function closeSearchModal(){searchModal?.classList.add("hidden");searchModal?.setAttribute("aria-hidden","true");}
function onSearchShortcut(e){const isCmdK=(e.ctrlKey||e.metaKey)&&String(e.key).toLowerCase()==="k";if(isCmdK){e.preventDefault();navigateTo("/search");return openSearchModal();}if(e.key==="Escape"&&!searchModal?.classList.contains("hidden")){e.preventDefault();closeSearchModal();}}
