...
const toListHtml=toOptionFiltered.length?toOptionFiltered.map(v=>`<label style='display:flex;align-items:center;gap:8px;font-size:12px;padding:4px 0'><input type='checkbox' class='balikan-check' data-abc-to='${encAttr(v)}' ${selectedToSet.has(v)?"checked":""}><span>${esc(v)}</span></label>`).join(""):"<div class='state' style='font-size:12px;padding:6px 0'>Tujuan tidak ditemukan.</div>";
abcAnalisisApp.innerHTML=`<div class='card' style='overflow:visible'><div class='mv-filters open' style='display:flex;flex-wrap:wrap;gap:8px;align-items:center;overflow:visible'><select id='abcPeriod' class='search-lg' style='max-width:170px;height:34px;padding:5px 14px!important'>${periodOpt}</select><select id='abcOrderType' class='search-lg' style='max-width:160px;height:34px;padding:5px 14px!important'><option ${ABC_STATE.orderType==="Semua"?"selected":""}>Semua</option><option ${ABC_STATE.orderType==="Orderan GT"?"selected":""}>Orderan GT</option><option ${ABC_STATE.orderType==="Orderan Store"?"selected":""}>Orderan Store</option></select><div id='abcToWrap' style='position:relative;min-width:220px;flex:1 1 240px;max-width:360px'><button id='abcToToggle' class='btn-ghost abc-action-btn' style='height:34px;width:100%;text-align:left;display:flex;justify-content:space-between;align-items:center'><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(toSummary)}</span><span style="opacity:.85;font-size:12px">${ABC_STATE.toDropdownOpen?'▴':'▾'}</span></button><div id='abcToMenu' style='position:absolute;left:0;right:auto;top:38px;z-index:80;width:min(320px,calc(100vw - 24px));max-height:320px;overflow:auto;background:var(--panel);border:1px solid var(--line);border-radius:10px;padding:8px;box-shadow:0 10px 24px rgba(0,0,0,.18);${ABC_STATE.toDropdownOpen?'':'display:none'}'><input id='abcToSearch' class='search-lg' placeholder='Cari tujuan...' value='${esc(ABC_STATE.toSearch||"")}'><div style='display:flex;gap:8px;justify-content:space-between;margin:6px 0'><button id='abcToSelectAll' class='btn-ghost' style='height:28px;padding:0 10px'>Pilih semua</button><button id='abcToClear' class='btn-ghost' style='height:28px;padding:0 10px'>Hapus pilihan</button></div><div id='abcToOptions'>${toListHtml}</div></div></div><select id='abcSortBy' class='search-lg' style='max-width:190px;height:34px;padding:5px 14px!important'>${sortOpt}</select><select id='abcPageSize' class='search-lg' style='max-width:120px;height:34px;padding:5px 14px!important'><option value='25' ${ABC_STATE.pageSize===25?'selected':''}>25</option><option value='50' ${ABC_STATE.pageSize===50?'selected':''}>50</option><option value='all' ${ABC_STATE.pageSize===-1?'selected':''}>Semua</option></select><button id='abcResetBtn' class='btn-ghost abc-action-btn' style='height:34px'>Reset</button><button id='abcCopyBtn' class='btn-ghost abc-action-btn' style='height:34px'>Copy</button><button id='abcRefreshBtn' class='btn-primary abc-action-btn' style='height:34px'>Refresh</button><span id='abcLoadingBadge' class='subtitle' style='margin-left:auto'></span></div><div class='table-wrap' style='margin-top:14px;max-height:calc(100vh - 280px);overflow:auto'><table><thead><tr><th><input type='checkbox' id='abcSelectAll' class='balikan-check'></th><th>SKU</th><th>Nama Barang</th><th>Total Qty Keluar</th><th>% Kontribusi</th><th>Cumulative %</th><th>Kategori ABC</th><th>Stok Saat Ini</th><th>Priority Order</th><th>Status Rekomendasi</th></tr></thead><tbody id='abcTbody'></tbody></table></div><div id='abcPager' class='mv-pagination'></div></div><div class='card' style='margin-top:8px;padding:8px 10px'><ol id='abcTop10' style='margin:0;padding-left:16px;display:block'></ol></div>`;
...
document.getElementById("abcToSelectAll")?.addEventListener("click",e=>{e.preventDefault();e.stopPropagation();ABC_STATE.selectedTo=[...(ABC_STATE.toOptions||[])];ABC_STATE.page=1;renderAbcAnalisisPage(true);});
document.getElementById("abcToClear")?.addEventListener("click",e=>{e.preventDefault();e.stopPropagation();ABC_STATE.selectedTo=[];ABC_STATE.page=1;renderAbcAnalisisPage(true);});
document.querySelectorAll("[data-abc-to]").forEach(el=>{el.addEventListener("change",e=>{e.stopPropagation();const toValue=String(e.target?.dataset?.abcTo||"");if(!toValue)return;const next=new Set(ABC_STATE.selectedTo||[]);if(e.target.checked)next.add(toValue);else next.delete(toValue);ABC_STATE.selectedTo=[...next];ABC_STATE.page=1;renderAbcAnalisisPage(true);});});
