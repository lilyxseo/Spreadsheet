...
function renderCycleCountPage(){if(!cycleCountApp)return;cycleCountApp.innerHTML=`<div class='card cc-card cc-section'><div class='section-header'><h4>Cycle Count</h4><div class='row'>${CYCLE_STATE.sessionActive?"<button class='btn-ghost' onclick='ccCancelSession()'>Batal Cycle Count</button>":""}<button class='btn-primary' onclick='ccStartSession()' ${CYCLE_STATE.sessionActive?"disabled":""}>Mulai Cycle Count</button></div></div>${CYCLE_HISTORY_REMOTE.error?`<div class='state' style='color:#b91c1c;'>${esc(CYCLE_HISTORY_REMOTE.error)}</div>`:""}${CYCLE_STATE.sessionActive?`<div class='cc-section'><div class='mv-toolbar'><input id='ccSearch' class='search-lg' placeholder='Cari SKU / nama barang / lokasi' value='${esc(CYCLE_STATE.searchInput)}'></div></div><div class='cc-section'><div class='table-wrap cc-table-wrap cc-search-wrap'><table><thead><tr><th>Lokasi</th><th>SKU</th><th>Nama Barang</th><th>Aksi</th></tr></thead><tbody id='ccSearchResultsBody'></tbody></table></div></div><div class='cc-section'><h4>Cycle Count Berjalan</h4><div class='table-wrap cc-table-wrap cc-session-wrap'><table><thead><tr><th>Lokasi</th><th>SKU</th><th>Nama Barang</th><th>Stok Bulky</th><th>Stok Retail</th><th>Aktual Bulky</th><th>Aktual Retail</th><th>Catatan</th><th>Selisih Bulky</th><th>Selisih Retail</th><th>Status</th><th>Aksi</th></tr></thead><tbody id='ccSessionBody'></tbody></table></div><div class='row'><button id='ccSubmitBtn' class='btn-primary' onclick='ccSubmitSession()'>Selesai Cycle Count</button></div></div>`:""}</div><div class='card cc-card cc-section'><div class='section-header'><h4>History Cycle Count</h4></div><div class='table-wrap cc-table-wrap'><table><thead><tr><th>Tanggal</th><th>Lokasi</th><th>SKU</th><th>Nama Barang</th><th>Bulky</th><th>Retail</th><th>Aktual Bulky</th><th>Aktual Retail</th><th>Catatan</th><th>Status</th></tr></thead><tbody id='ccHistoryBody'></tbody></table></div></div>`;renderCycleSearchResults();renderCycleSessionTable();renderCycleHistory();}
...
window.ccAddItem=(skuEnc,lokEnc)=>{const sku=decodeURIComponent(skuEnc),lokasi=decodeURIComponent(lokEnc);const source=getCycleSourceRows().find(r=>clean(r.sku)===clean(sku)&&clean(r.lokasi)===clean(lokasi));if(!source)return;const newItem={...source,aktualBulky:null,aktualRetail:null,catatan:""};const key=cycleItemKey(newItem);if(CYCLE_STATE.sessionItems.some(it=>cycleItemKey(it)===key)){toast('SKU/lokasi sudah ada di session.','error');return;}CYCLE_STATE.sessionItems.push(newItem);renderCycleSessionTable();toast('Item ditambahkan','success');};
