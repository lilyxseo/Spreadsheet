...
const inRows=getLatestRows("Barang Masuk",null,false),outRows=getLatestRows("Barang Keluar",null,false);
...
function getLatestRows(sheetName,limit=null,requireSku=false){let rows=(DATA[sheetName]||[]).slice();if(requireSku)rows=rows.filter(r=>clean(getVal(r,["sku"])));rows=rows.reverse();return Number.isFinite(limit)?rows.slice(0,limit):rows;}
function renderDashboardTableSection(title,subtitle,rows,badgeClassName){const badgeText=`${rows.length} data`;return `<section class='dashboard-section'><div class='card'><div class='section-header'><div><h4>${esc(title)}</h4><small class='section-subtitle'>${esc(subtitle)}</small></div><span class='badge ${badgeClassName}'>${esc(badgeText)}</span></div>${renderDashboardSheetTable(rows,title)}</div></section>`;}
