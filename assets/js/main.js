...
hideInitialLoader();
setStatus("loading","Belum ada cache. Klik Refresh untuk memuat data.");
...
const TABLE_STATE={in:{page:1,pageSize:25,rows:[],filtered:[],filterCache:new Map(),filterKey:"",allRenderWarning:""},out:{page:1,pageSize:25,rows:[],filtered:[],filterCache:new Map(),filterKey:"",allRenderWarning:""}};
function getUniqueOptions(rows,key){return [...new Set(rows.map(r=>String(r[key]||"-")).filter(Boolean))].sort((a,b)=>a.localeCompare(b));}
function applyTableFilters(rows,mode){const q=clean((mode==="in"?inSearch:outSearch)?.value||"");const root=document.getElementById(`mv-filters-${mode}`);const get=name=>root?.querySelector(`[name="${name}"]`)?.value||"";const fields=["from","to","status","pic","lokasi","sku"];const filterState={q};fields.forEach(f=>filterState[f]=get(f));const cacheKey=JSON.stringify(filterState);const st=TABLE_STATE[mode];if(st.filterCache.has(cacheKey))return st.filterCache.get(cacheKey);const filtered=rows.filter(r=>{if(fields.some(f=>{const v=filterState[f];return v&&String(r[f]||"-")!==v;}))return false;if(q&&!clean(`${r.sku} ${r.nama} ${r.from} ${r.to} ${r.status} ${r.pic} ${r.keterangan}`).includes(q))return false;return true;});st.filterCache.set(cacheKey,filtered);if(st.filterCache.size>20){const firstKey=st.filterCache.keys().next().value;st.filterCache.delete(firstKey);}return filtered;}
...
summaryEl.innerHTML=`${st.allRenderWarning?`<div class='state'>${st.allRenderWarning}</div>`:''}<div class='summary-grid'><div class='summary-card'><div class='k'>Total Row</div><div class='v'>${st.filtered.length}</div></div><div class='summary-card'><div class='k'>Total Qty</div><div class='v'>${totalQty}</div></div><div class='summary-card'><div class='k'>SKU Unik</div><div class='v'>${skuCount}</div></div><div class='summary-card'><div class='k'>From Unik</div><div class='v'>${fromCount}</div></div><div class='summary-card'><div class='k'>To Unik</div><div class='v'>${toCount}</div></div></div>`;
const filterHtml=`<div class='mv-toolbar'><button class='btn-ghost mv-mobile-only' data-mv-action='toggle-filter' data-mv-mode='${mode}'>Filter</button><div id='mv-filters-${mode}' class='mv-filters'>${["from","to","status","pic","lokasi","sku"].map(k=>`<select data-mv-filter data-mv-mode='${mode}' name='${k}'><option value=''>${k.toUpperCase()} (Semua)</option>${getUniqueOptions(rows,k).map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join("")}</select>`).join("")}<select id='mv-sort-${mode}' data-mv-filter data-mv-mode='${mode}'><option value='latest'>Terbaru</option><option value='oldest'>Terlama</option><option value='sku'>SKU A-Z</option><option value='name'>Nama A-Z</option><option value='qtyDesc'>Qty terbesar</option><option value='qtyAsc'>Qty terkecil</option></select><select id='mv-size-${mode}' data-mv-filter data-mv-mode='${mode}'><option value='25'>25</option><option value='50'>50</option><option value='all'>Semua</option></select><button class='btn-ghost' data-mv-action='reset' data-mv-mode='${mode}'>Reset Filter</button><button class='btn-ghost' data-mv-action='export' data-mv-mode='${mode}'>Export CSV</button><button class='btn-ghost' data-mv-action='columns' data-mv-mode='${mode}'>Kolom</button></div><div id='mv-cols-${mode}' class='mv-columns'>${allCols.map(c=>`<label><input type='checkbox' ${st.columns.includes(c)?"checked":""} value='${c}' onchange='toggleColumnVisibility("${mode}")'> ${c}</label>`).join("")}</div></div>`;
const headers=st.columns.map(c=>`<th>${esc(c.toUpperCase())}</th>`).join("");const bodyFrag=document.createDocumentFragment();for(const r of pageRows){const tr=document.createElement('tr');for(const c of st.columns){const td=document.createElement('td');td.textContent=String(r[c]??'-');tr.appendChild(td);}bodyFrag.appendChild(tr);}const start=st.filtered.length?((st.page-1)*st.pageSize+1):0;const end=st.filtered.length?Math.min(st.page*st.pageSize,st.filtered.length):0;
resultEl.innerHTML=`${filterHtml}<div class='table-wrap table-wrap-full'><table><thead><tr>${headers}</tr></thead><tbody></tbody></table></div><div class='mv-pagination'><span>Menampilkan ${start}–${end} dari ${st.filtered.length} data</span><div class='row'><button class='btn-ghost' data-mv-action='prev' data-mv-mode='${mode}'>Prev</button><button class='btn-ghost' data-mv-action='next' data-mv-mode='${mode}'>Next</button><span class='badge ${isIn?"b-in":"b-out"}'>${isIn?"IN":"OUT"}</span></div></div>`;const tbody=resultEl.querySelector('tbody');if(!pageRows.length){const tr=document.createElement('tr');const td=document.createElement('td');td.colSpan=st.columns.length;td.innerHTML="<div class='state'>Tidak ada data.</div>";tr.appendChild(td);tbody.appendChild(tr);}else{tbody.appendChild(bodyFrag);}document.getElementById(`mv-size-${mode}`).value=selectedSize;}
function resetMovementFilter(mode){if(mode==="in"){if(inSearch)inSearch.value="";}else{if(outSearch)outSearch.value="";}const root=document.getElementById(`mv-filters-${mode}`);root?.querySelectorAll('select').forEach(el=>el.value="");TABLE_STATE[mode].filterCache.clear();TABLE_STATE[mode].page=1;renderDataTablePage(mode,mode==="in"?"Barang Masuk":"Barang Keluar",true);} 
