...
await saveCache(DATA);
console.log("UPDATE LAST SYNC TIME",localStorage.getItem(CACHE_KEYS.lastSync));
...
renderDataTablePage("out","Barang Keluar",true);
scheduleRenderDashboard();
if(dataChanged){await saveCache(DATA);console.log("UPDATE LAST SYNC TIME",localStorage.getItem(CACHE_KEYS.lastSync));}
}
setStatus('ok','');
return true;
}catch(err){
