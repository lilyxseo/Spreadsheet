...
function syncBalikanAutoCheckToggle(){if(!balikanAutoCheckToggle)return;const isOn=BALIKAN_STATE.autoCheckOnScan!==false;balikanAutoCheckToggle.classList.toggle('is-on',isOn);balikanAutoCheckToggle.classList.toggle('is-off',!isOn);balikanAutoCheckToggle.setAttribute('aria-pressed',isOn?'true':'false');balikanAutoCheckToggle.textContent=`Scan to checklist: ${isOn?'ON':'OFF'}`;}
