...
const totalSku=norm.length,skuAkurat=norm.filter(r=>r.selisih===0).length,skuTidakAkurat=totalSku-skuAkurat,akurasi=totalSku?((skuAkurat/totalSku)*100):0,selisihTotal=norm.reduce((n,r)=>n+(Number(r.selisih)||0),0);
...
statsChart.innerHTML=`<div class='mv-toolbar stats-toolbar stats-toolbar-sticky'><input id='statsSearch' placeholder='Search SKU/Nama/Lokasi' value='${esc(STATS_STATE.q)}'><select id='statsSort'><option value='absDesc'>Selisih terbesar</option><option value='absAsc'>Selisih terkecil</option><option value='sku'>SKU A-Z</option></select><select id='statsSize'><option value='25'>25</option><option value='50'>50</option><option value='100'>100</option></select></div>
<div class='table-wrap table-wrap-full'><table><thead><tr><th>LOKASI</th><th>SKU</th><th>NAMA BARANG</th><th>SELISIH</th><th>AKSI</th></tr></thead><tbody>${paged.map(r=>`<tr class='${r.selisih!==0?"row-mismatch":""}'><td class='stats-lokasi-cell' title='${esc(r.lokasiText)}'>${esc(r.lokasiText)}</td><td>${esc(r.sku)}</td><td>${esc(r.nama)}</td><td class='${r.selisih!==0?"txt-danger":""}'>${r.selisih}</td><td class='action-cell'><button class='detail-mini-btn' type='button' onclick="navigateToSku(decodeURIComponent('${encAttr(r.sku)}'))">Lihat SKU</button></td></tr>`).join("")||`<tr><td colspan='5'><div class='state'>Tidak ada data.</div></td></tr>`}</tbody></table></div>
