...
function renderBalikanTable(keepPage=true){const st=ensureBalikanFilterState();const baseRows=(window.BALIKAN_ROWS||[]).map(r=>({...r}));st.rows=baseRows;const rows=applyBalikanTableFilters(baseRows);if(!keepPage)st.page=1;const pageRows=rows;if(!rows.length){balikanTable.innerHTML='<div class="subtitle">Data kosong.</div>';return;}const filters=st.columnFilters;const headerWithFilter=(col)=>{const selected=filters[col]||[];const active=selected.length>0;const options=getUniqueOptions(applyBalikanTableFilters(baseRows,'balikan',col),col);return `<th><div class='th-filter-wrap'>${BALIKAN_FILTER_LABELS[col]||col}<button class='col-filter-btn ${active?'active':''}' type='button' data-col-filter-toggle data-mode='balikan' data-col='${col}' title='Filter ${BALIKAN_FILTER_LABELS[col]||col}'>▾</button><div class='col-filter-menu' data-col-filter-menu data-mode='balikan' data-col='${col}' hidden><input class='search-sm' data-col-filter-search placeholder='Cari...'><div class='col-filter-options'>${options.map(v=>`<label data-opt-item><input type='checkbox' value='${esc(v)}' ${selected.includes(v)?'checked':''}> <span>${esc(v)}</span></label>`).join('')||"<div class='subtitle'>Tidak ada opsi</div>"}</div><div class='col-filter-actions'><button type='button' class='btn-ghost btn-xs' data-col-filter-all>Pilih semua</button><button type='button' class='btn-ghost btn-xs' data-col-filter-clear>Hapus</button></div></div></div></th>`;};
