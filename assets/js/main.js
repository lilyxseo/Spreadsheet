...
const totalQty=(PDF_TRANSFER_STATE.items||[]).reduce((n,r)=>n+(Number(r.jumlah)||0),0);
root.innerHTML=`<div class='card'><div class='mv-filters open'><input id='pdfTransferFile' type='file' accept='application/pdf'/><button id='pdfTransferParseBtn' class='btn-primary' ${PDF_TRANSFER_STATE.isParsing?'disabled':''}>${PDF_TRANSFER_STATE.isParsing?'Parsing...':'Upload PDF'}</button><button id='pdfTransferImportBtn' class='btn-primary' ${PDF_TRANSFER_STATE.isImporting||!PDF_TRANSFER_STATE.items.length?'disabled':''}>${PDF_TRANSFER_STATE.isImporting?'Importing...':'Import'}</button><button id='pdfTransferResetBtn' class='btn-ghost'>Reset</button></div><div id='pdfTransferStatus' class='subtitle'></div></div><div class='card'><h4>Header Transfer</h4><div class='grid dashboard'>${[['Nomor Transfer','nomorTransfer'],['Tanggal','tanggal'],['Dari','dari'],['Kepada','kepada'],['Status','status'],['Nomor Referensi','nomorReferensi']].map(([label,key])=>`<label>${label}<input data-pdf-header='${key}' class='search-lg' value='${esc(PDF_TRANSFER_STATE.header?.[key]||'')}'/></label>`).join('')}</div></div><div class='card'><h4>Preview Item</h4><div class='subtitle'>Total SKU: ${PDF_TRANSFER_STATE.items.length} | Total Qty: ${totalQty}</div><div class='table-wrap'><table><thead><tr><th>SKU</th><th>Nama Produk</th><th>Jumlah</th><th>Diterima</th><th>Batal</th><th>Tolak</th><th>Catatan</th></tr></thead><tbody>${PDF_TRANSFER_STATE.items.map((it,idx)=>`<tr>${['sku','namaProduk','jumlah','diterima','batal','tolak','catatan'].map(k=>`<td contenteditable='true' data-pdf-item='${idx}' data-field='${k}'>${esc(String(it[k]??''))}</td>`).join('')}</tr>`).join('')||"<tr><td colspan='7'><div class='state'>Belum ada item.</div></td></tr>"}</tbody></table></div>${PDF_TRANSFER_STATE.warnings.length?`<div class='state'>Warning: ${PDF_TRANSFER_STATE.warnings.map(esc).join(' | ')}</div>`:''}</div><div class='card'><h4>Debug Log</h4><pre style='white-space:pre-wrap'>${esc((PDF_TRANSFER_STATE.logs||[]).join('\n'))}</pre>${PDF_TRANSFER_STATE.items.length?'':`<h5>Raw Text</h5><pre style='white-space:pre-wrap'>${esc(PDF_TRANSFER_STATE.rawText||'-')}</pre>`}</div>`;
...
header.nomorTransfer=findVal('Nomor Transfer|No Transfer');header.tanggal=findVal('Tanggal');header.dari=findVal('Dari');header.kepada=findVal('Kepada');header.status=findVal('Status');header.nomorReferensi=findVal('Nomor Referensi|No Referensi|Reference');
...
for(let i=0;i<itemRows.length;i++){const row=itemRows[i];if(/grand total|^total\b/i.test(row))break;const line=String(row||'').replace(/\s+/g,' ').trim();if(!line)continue;const tokens=line.split(' ').filter(Boolean);
const skuIdx=tokens.findIndex(tok=>{
if(!isSkuCandidate(tok))return false;
return skuMasterMap.has(clean(tok));
});
if(skuIdx<0){if(parsed.length&&/^[A-Za-z]/.test(line)){parsed[parsed.length-1].catatan=`${parsed[parsed.length-1].catatan||""} ${line}`.trim();continue;}skipped.push(`${line} => sku tidak ditemukan`);continue;}
const sku=tokens[skuIdx];const after=tokens.slice(skuIdx+1);
const numericTail=after.filter(x=>/^\d+$/.test(x)).slice(0,4);
const jumlah=Number(numericTail[0]||0),diterima=Number(numericTail[1]||0),batal=Number(numericTail[2]||0),tolak=Number(numericTail[3]||0);
const tailJoined=after.join(' ').trim();
let catatan=tailJoined;
if(numericTail.length){
const numericPrefixPattern=new RegExp(`^${numericTail.join('\\s+')}\\s*`,'i');
catatan=tailJoined.replace(numericPrefixPattern,'').trim();
}
const skuRef=skuMasterMap.get(clean(sku))||{};
const namaProduk=skuRef.nama||getNamaProdukBySkuFromKartuStock(sku)||'-';
parsed.push({sku:skuRef.sku||sku,namaProduk,jumlah,diterima,batal,tolak,catatan});
}
const warnings=[];parsed.forEach((r,ix)=>{if(!r.sku)warnings.push(`Row ${ix+1}: SKU kosong, tidak akan diimport`);if(r.jumlah===0)warnings.push(`Row ${ix+1}: qty kosong diset 0`);});
...
try{for(const row of validRows){const payload={tanggal:PDF_TRANSFER_STATE.header.tanggal||new Date().toISOString().slice(0,10),sku:row.sku,nama_barang:row.namaProduk||'-',qty:Number(row.jumlah)||0,lokasi:PDF_TRANSFER_STATE.header.kepada||'-',keterangan:`Transfer ${PDF_TRANSFER_STATE.header.nomorTransfer||'-'} | ${row.catatan||''}`};const res=await fetch('/api/sheet-input',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const out=await res.json();if(!res.ok||!out?.success)throw new Error(out?.message||'Gagal import');}
toast(`Import sukses ${validRows.length} item`,'success');
