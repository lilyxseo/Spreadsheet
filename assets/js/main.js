...
const ABC_ANALYSIS_STATE={months:3,excludeTo:["reject","rusak"],rows:[],filteredRows:[],summary:null};
function parseDateLike(value){const raw=String(value||"").trim();if(!raw)return null;const d=new Date(raw);if(!Number.isNaN(d.getTime()))return d;const m=raw.match(/^(\d{1,2})[\/-](\d{1,2})[\/-](\d{2,4})$/);if(!m)return null;const dd=Number(m[1]),mm=Number(m[2])-1,yy=Number(m[3].length===2?`20${m[3]}`:m[3]);const dt=new Date(yy,mm,dd);return Number.isNaN(dt.getTime())?null:dt;}
function getInventoryMap(){const rows=[];const buckets=[window.APP_STATE?.inventory,DATA["Kartu Stock"],DATA["RPL"],DATA["BULKY"];for(const b of buckets)if(Array.isArray(b))rows.push(...b);const map=new Map();rows.forEach(r=>{const sku=String(getVal(r,["sku"])||"").trim();if(!sku)return;const stok=Number(getVal(r,["stok akhir","stok","qty","quantity"])||0);const minimum=Number(getVal(r,["minimum stok","min stok","minimum","min"])||0);const nama=String(getVal(r,["nama barang","nama"])||"").trim();if(!map.has(sku))map.set(sku,{sku,nama,stok:0,minimum:0});const it=map.get(sku);if(Number.isFinite(stok))it.stok+=stok;if(Number.isFinite(minimum)&&minimum>0)it.minimum=minimum;if(!it.nama&&nama)it.nama=nama;});return map;}
function computeAbcAnalysis(){const months=Math.max(1,Math.min(6,Number(ABC_ANALYSIS_STATE.months)||3));const now=new Date();const start=new Date(now);start.setMonth(start.getMonth()-months);const excluded=new Set((ABC_ANALYSIS_STATE.excludeTo||[]).map(x=>clean(String(x||""))));const rawRows=getBarangKeluarRows();const agg=new Map();let totalQty=0;for(const row of rawRows){const tgl=parseDateLike(getVal(row,["tanggal","date"]));if(!tgl||tgl<start||tgl>now)continue;const to=clean(getVal(row,["to","tujuan","tujuan pengeluaran barang","lokasi tujuan"]));if(excluded.has(to))continue;const sku=String(getVal(row,["sku","nama barang","nama"])||"").trim();const qty=Number(getVal(row,["qty keluar","qty","quantity"])||0);if(!sku||!Number.isFinite(qty)||qty<=0)continue;const key=clean(sku);const rec=agg.get(key)||{sku,qty:0};rec.qty+=qty;agg.set(key,rec);totalQty+=qty;}
const rows=[...agg.values()].map(r=>({...r,contribution:totalQty?((r.qty/totalQty)*100):0}));const byDesc=[...rows].sort((a,b)=>b.qty-a.qty);let cumulative=0;const catMap=new Map();for(const r of byDesc){cumulative+=r.contribution;const category=cumulative<=80?"A":(cumulative<=95?"B":"C");catMap.set(clean(r.sku),{cumulative,category});}
const invMap=getInventoryMap();const merged=rows.map(r=>{const extra=catMap.get(clean(r.sku))||{cumulative:0,category:"C"};const inv=invMap.get(r.sku)||{stok:0,minimum:0,nama:""};const stock=Number(inv.stok)||0,min=Number(inv.minimum)||0;let status="NORMAL";if(extra.category==="A")status=stock<min?"RESTOCK PRIORITAS":"AMAN";else if(extra.category==="C")status=stock>min&&min>0?"OVERSTOCK":"LOW PRIORITY";return {...r,cumulative:extra.cumulative,category:extra.category,nama:inv.nama||r.sku,stok:stock,minimum:min,status};});
ABC_ANALYSIS_STATE.rows=merged.sort((a,b)=>a.qty-b.qty);ABC_ANALYSIS_STATE.filteredRows=ABC_ANALYSIS_STATE.rows;const sumCat={A:0,B:0,C:0,contribA:0,contribB:0,contribC:0};merged.forEach(r=>{sumCat[r.category]+=1;sumCat[`contrib${r.category}`]+=r.contribution;});ABC_ANALYSIS_STATE.summary={totalSku:merged.length,...sumCat};}
function renderAbcAnalysisPage(){if(!abcAnalysisApp)return;computeAbcAnalysis();const rows=ABC_ANALYSIS_STATE.filteredRows||[];if(!rows.length){abcAnalysisApp.innerHTML=`<div class='card'><div class='state'>Tidak ada data pada periode ini</div></div>`;return;}const s=ABC_ANALYSIS_STATE.summary||{};const topA=[...rows].filter(r=>r.category==='A').sort((a,b)=>b.qty-a.qty).slice(0,10);const slowC=[...rows].filter(r=>r.category==='C').sort((a,b)=>a.qty-b.qty).slice(0,10);abcAnalysisApp.innerHTML=`<div class='card'><div class='mv-filters open'><select id='abcPeriod'>${[1,2,3,4,5,6].map(m=>`<option value='${m}' ${m===ABC_ANALYSIS_STATE.months?'selected':''}>${m} bulan terakhir</option>`).join('')}</select><button id='abcRefresh' class='btn-primary'>Refresh</button><button id='abcExport' class='btn-ghost'>Export CSV</button></div><div class='grid dashboard'><div class='metric'><div class='k'>Total SKU</div><div class='v'>${s.totalSku||0}</div></div><div class='metric'><div class='k'>Kategori A/B/C</div><div class='v'>${s.A||0} / ${s.B||0} / ${s.C||0}</div></div><div class='metric'><div class='k'>Kontribusi A</div><div class='v'>${(s.contribA||0).toFixed(2)}%</div></div></div><div class='table-wrap table-wrap-full'><table><thead><tr><th>SKU / Nama Barang</th><th>Total Qty Keluar</th><th>% Kontribusi</th><th>Cumulative %</th><th>Kategori</th><th>Stok</th><th>Minimum</th><th>Status Rekomendasi</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.sku)}<div class='subtitle'>${esc(r.nama||'')}</div></td><td>${r.qty}</td><td>${r.contribution.toFixed(2)}%</td><td>${r.cumulative.toFixed(2)}%</td><td><span class='badge ${r.category==='A'?'b-out':(r.category==='B'?'b-warn':'b-kartu')}'>${r.category}</span></td><td>${r.stok}</td><td>${r.minimum}</td><td>${esc(r.status)}</td></tr>`).join('')}</tbody></table></div><div class='grid dashboard' style='margin-top:10px'><div class='card'><h4>Top 10 Fast Moving (A)</h4>${topA.map(r=>`<div>${esc(r.sku)} - ${r.qty}</div>`).join('')||'<div class="state">-</div>'}</div><div class='card'><h4>Top 10 Slow Moving (C)</h4>${slowC.map(r=>`<div>${esc(r.sku)} - ${r.qty}</div>`).join('')||'<div class="state">-</div>'}</div></div></div>`;}
document.addEventListener('change',e=>{if(e.target?.id==='abcPeriod'){ABC_ANALYSIS_STATE.months=Number(e.target.value)||3;renderAbcAnalysisPage();}});
document.addEventListener('click',e=>{if(e.target?.id==='abcRefresh'){renderAbcAnalysisPage();}if(e.target?.id==='abcExport'){const rows=ABC_ANALYSIS_STATE.filteredRows||[];const header=['SKU','Total Qty Keluar','% Kontribusi','Cumulative %','Kategori','Stok Saat Ini','Minimum Stok','Status Rekomendasi'];const lines=[header.join(','),...rows.map(r=>[r.sku,r.qty,r.contribution.toFixed(2),r.cumulative.toFixed(2),r.category,r.stok,r.minimum,r.status].map(v=>`"${String(v??'').replaceAll('"','""')}"`).join(','))];const blob=new Blob([lines.join('\n')],{type:'text/csv;charset=utf-8;'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='abc-analisis.csv';a.click();URL.revokeObjectURL(a.href);}});
