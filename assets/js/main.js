...
const ANOMALY_CACHE_KEY="warningReportCacheV2";
const ANOMALY_STATE={page:1,pageSize:25,rows:[],filtered:[],q:"",severity:"all",type:"all",rendered:false,isLoading:false,_searchDebounce:null,lastRenderToken:0,dataVersion:""};
...
function renderAnomalySummary(){const rows=ANOMALY_STATE.rows||[], count=types=>rows.filter(r=>types.has(r.type)).length;anomalySummary.innerHTML=[["Salah Lokasi",count(WARNING_GROUPS.LOCATION)],["SKU/Nama Tidak Sesuai",count(WARNING_GROUPS.SKU)],["Tidak Balance",count(WARNING_GROUPS.BALANCE)],["Input Tidak Terbaca",count(WARNING_GROUPS.FORMAT)],["Critical",rows.filter(r=>r.severity==='CRITICAL').length]].map(([k,v])=>`<div class='metric'><div class='k'>${k}</div><div class='v'>${v}</div></div>`).join('');}
