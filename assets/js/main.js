...
const selectedLabel=(ABC_STATE.selectedTo||[]).length?`${ABC_STATE.selectedTo.length} TO dipilih`:"Semua TO";
abcAnalisisApp.innerHTML=`<div class='card'><div class='mv-filters open' style='display:flex;flex-wrap:wrap;gap:8px;align-items:center'><select id='abcPeriod' style='max-width:170px'>${periodOpt}</select><select id='abcOrderType' style='max-width:160px'><option ${ABC_STATE.orderType==="Semua"?"selected":""}>Semua</option><option ${ABC_STATE.orderType==="Orderan GT"?"selected":""}>Orderan GT</option><option ${ABC_STATE.orderType==="Orderan Store"?"selected":""}>Orderan Store</option></select><select id='abcSortBy' style='max-width:190px'>${sortOpt}</select><div id='abcToWrap' style='position:relative;min-width:220px;max-width:320px;flex:1'><button id='abcToToggle' class='btn-ghost' type='button' style='width:100%;text-align:left'>${esc(selectedLabel)}</button><div id='abcToDropdown' ${ABC_STATE.toDropdownOpen?"":"hidden"} style='position:absolute;z-index:20;top:36px;left:0;right:0;background:var(--cardBg,#111827);border:1px solid var(--line,#2a3346);border-radius:10px;padding:8px;max-height:280px;overflow:auto'><input id='abcToSearch' class='search-lg' placeholder='Cari tujuan...' value='${encAttr(ABC_STATE.toSearch||"")}' style='margin-bottom:8px;height:34px'><div class='row' style='gap:8px;margin-bottom:8px'><button type='button' id='abcToSelectAll' class='btn-ghost'>Pilih semua</button><button type='button' id='abcToClear' class='btn-ghost'>Hapus</button></div>${toFiltered.map(v=>`<label style='display:flex;gap:8px;align-items:center;padding:4px 0'><input type='checkbox' data-abc-to='${encAttr(v)}' ${(ABC_STATE.selectedTo||[]).includes(v)?"checked":""}/> <span>${esc(v)}</span></label>`).join("")||"<div class='subtitle'>Tidak ada tujuan</div>"}</div></div><button id='abcRefreshBtn' class='btn-primary'>Refresh</button></div><div class='table-wrap'><table><thead><tr><th>Nama Barang</th><th>SKU</th><th>Total Qty Keluar</th><th>% Kontribusi</th><th>Cumulative %</th><th>Kategori ABC</th><th>Stok Saat Ini</th><th>Priority Order</th><th>Status Rekomendasi</th></tr></thead><tbody>${data.rows.map(r=>`<tr><td>${esc(r.namaBarang)}</td><td>${esc(r.sku)}</td><td>${r.qtyKeluar}</td><td>${(r.kontribusi*100).toFixed(2)}%</td><td>${(r.cum*100).toFixed(2)}%</td><td><span class='badge ${r.abc==="A"?"b-out":r.abc==="B"?"b-warn":"b-idle"}'>${r.abc}</span></td><td>${r.stokSaatIni}</td><td>${esc(r.priority)}</td><td>${esc(r.rekom)}</td></tr>`).join("")}</tbody></table></div></div><div class='card' style='margin-top:8px;padding:10px 12px'><div class='section-header' style='margin-bottom:6px'><h4 style='margin:0;font-size:14px'>Top 10 Fast Moving Items</h4></div><ol style='margin:0;padding-left:18px;display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:4px 14px'>${data.top10.map(r=>`<li style='margin:0'><button class='btn-link' data-abc-sku='${encAttr(r.sku)}' style='font-size:13px;line-height:1.2'>${esc(r.namaBarang)}</button></li>`).join("")}</ol></div>`;
