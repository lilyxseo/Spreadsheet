...
import { analyzeWarnings, mergeWarnings } from "./warning-engine.mjs";
...
const ANOMALY_CACHE_KEY="warningAnalysisV2";
const WARNING_REVIEW_KEY="warningReviewV1";
const ANOMALY_STATE={page:1,pageSize:25,rows:[],filtered:[],q:"",severity:"all",type:"all",module:"all",confidence:"all",risk:0,status:"all",sort:"risk",rendered:false,isLoading:false,selected:null,lastSourceVersion:"",lastRenderToken:0,_searchDebounce:null};
function warningSources(){return {stock:DATA["Kartu Stock"]||[],inbound:getBarangMasukRows(),outbound:getBarangKeluarRows(),movement:MOVEMENT_HISTORY_REMOTE?.rows||getModuleCache(MODULE_CACHE_KEYS.movement)||[],cycleCount:CYCLE_HISTORY_REMOTE?.rows||getModuleCache(MODULE_CACHE_KEYS.cycleCount)||[],returns:window.BALIKAN_ROWS||[],reject:BARANG_REJECT_STATE?.stock?.length?[...(BARANG_REJECT_STATE.masuk||[]),...(BARANG_REJECT_STATE.keluar||[])]:[],sourcesLoaded:{inbound:Array.isArray(getBarangMasukRows()),outbound:Array.isArray(getBarangKeluarRows())},sync:{KartuStok:REFRESH_STATE.modules?.kartuStock?.lastSync||REFRESH_STATE.lastRefreshAt,BarangMasuk:REFRESH_STATE.modules?.barangMasuk?.lastSync||REFRESH_STATE.lastRefreshAt,BarangKeluar:REFRESH_STATE.modules?.barangKeluar?.lastSync||REFRESH_STATE.lastRefreshAt,Movement:REFRESH_STATE.modules?.movement?.lastSync||REFRESH_STATE.lastRefreshAt,CycleCount:REFRESH_STATE.modules?.cycleCount?.lastSync||REFRESH_STATE.lastRefreshAt},mapping:{}}}
function warningVersion(s){return [REFRESH_STATE.dataVersion,...Object.values(s).filter(Array.isArray).map(x=>x.length),...Object.values(s.sync||{})].join('|')}
function getWarningReview(){try{return JSON.parse(localStorage.getItem(WARNING_REVIEW_KEY)||'{}')}catch{return {}}}function saveWarningReview(v){localStorage.setItem(WARNING_REVIEW_KEY,JSON.stringify(v))}
function reviewFor(id){return getWarningReview()[id]||{status:'Baru',note:'',reason:''}}
function setWarningReview(id,status){const reviews=getWarningReview(),old=reviews[id]||{};let reason=old.reason||'';if(status==='Diabaikan'){reason=prompt('Alasan mengabaikan warning (wajib):',reason)||'';if(!reason.trim()){toast('Alasan wajib diisi','warning');return}}reviews[id]={...old,status,reason,updatedAt:new Date().toISOString()};saveWarningReview(reviews);logActivitySafe({action:'WARNING_STATUS_CHANGED',module:'Warning',detail:`${id}: ${status}`,status:'SUCCESS'});renderAnomalyRowsOnly();if(ANOMALY_STATE.selected?.id===id)openWarningDetail(id)}
function getAnomalyCache(){try{return JSON.parse(localStorage.getItem(ANOMALY_CACHE_KEY)||'null')}catch{return null}}function setAnomalyCache(rows,version){try{localStorage.setItem(ANOMALY_CACHE_KEY,JSON.stringify({rows,version,updatedAt:Date.now()}))}catch{}}
function sevClass(s){return s==='CRITICAL'?'b-high':s==='HIGH'?'b-high':s==='MEDIUM'?'b-medium':'b-low'}
function setAnomalyLoading(on,msg='Memeriksa data terbaru…'){ANOMALY_STATE.isLoading=on;const el=document.getElementById('anomalyLoading');if(el){el.classList.toggle('hidden',!on);el.textContent=msg}}
function scheduleAnomalySearch(v=''){ANOMALY_STATE.q=v;clearTimeout(ANOMALY_STATE._searchDebounce);ANOMALY_STATE._searchDebounce=setTimeout(()=>applyAnomalyFilters(true),250)}
function getAnomalyFilteredRows(){const q=clean(ANOMALY_STATE.q);return (ANOMALY_STATE.rows||[]).filter(w=>{const review=reviewFor(w.id);return (ANOMALY_STATE.severity==='all'||w.severity===ANOMALY_STATE.severity)&&(ANOMALY_STATE.type==='all'||w.type===ANOMALY_STATE.type)&&(ANOMALY_STATE.module==='all'||w.module===ANOMALY_STATE.module)&&(ANOMALY_STATE.status==='all'||review.status===ANOMALY_STATE.status)&&w.riskScore>=ANOMALY_STATE.risk&&(ANOMALY_STATE.confidence==='all'||(ANOMALY_STATE.confidence==='high'?w.confidence>=80:ANOMALY_STATE.confidence==='medium'?w.confidence>=50&&w.confidence<80:w.confidence<50))&&(!q||clean(`${w.sku} ${w.itemName} ${w.location} ${w.title} ${w.description} ${w.module}`).includes(q))})}
function renderAnomalySummary(){const r=ANOMALY_STATE.filtered,items=[['Critical',r.filter(x=>x.severity==='CRITICAL').length],['High',r.filter(x=>x.severity==='HIGH').length],['Medium',r.filter(x=>x.severity==='MEDIUM').length],['Kemungkinan Minus',r.filter(x=>x.type==='possible_minus').length],['Tidak Balance',r.filter(x=>x.type==='stock_mismatch').length],['Data Bermasalah',r.filter(x=>['invalid_quantity','mapping_problem'].includes(x.type)).length]];anomalySummary.innerHTML=items.map(x=>`<div class='metric'><div class='k'>${x[0]}</div><div class='v'>${x[1]}</div></div>`).join('')}
function renderAnomalyRowsOnly(){let rows=getAnomalyFilteredRows();rows.sort(ANOMALY_STATE.sort==='time'?(a,b)=>new Date(b.detectedAt)-new Date(a.detectedAt):(a,b)=>b.riskScore-a.riskScore);ANOMALY_STATE.filtered=rows;renderAnomalySummary();const max=Math.max(1,Math.ceil(rows.length/ANOMALY_STATE.pageSize));ANOMALY_STATE.page=Math.min(max,ANOMALY_STATE.page);const start=(ANOMALY_STATE.page-1)*ANOMALY_STATE.pageSize,shown=rows.slice(start,start+ANOMALY_STATE.pageSize),tbody=document.querySelector('#anomalyTable tbody');if(tbody)tbody.innerHTML=shown.map(w=>{const rv=reviewFor(w.id),cause=w.possibleCause?.[0]?.title||'-';return `<tr><td data-label='Severity'><span class='badge ${sevClass(w.severity)}'>${w.severity}</span></td><td data-label='Risk'><strong>${w.riskScore}</strong></td><td data-label='Modul'>${esc(w.module)}</td><td data-label='SKU'>${esc(w.sku||'-')}</td><td data-label='Lokasi'>${esc(w.location||'-')}</td><td data-label='Masalah'><strong>${esc(w.title)}</strong><small>${esc(String(w.impact??'-'))}</small></td><td data-label='Penyebab'>${esc(cause)}</td><td data-label='Confidence'>${w.confidence}%</td><td data-label='Status'>${esc(rv.status)}</td><td data-label='Aksi'><button class='btn-ghost' onclick="openWarningDetail('${w.id}')">Detail</button></td></tr>`}).join('')||`<tr><td colspan='10'><div class='state'>Tidak ada warning pada filter ini.</div></td></tr>`;const info=document.getElementById('anomalyInfo');if(info)info.textContent=`${rows.length?start+1:0}–${Math.min(start+ANOMALY_STATE.pageSize,rows.length)} dari ${rows.length} warning`}
function applyAnomalyFilters(reset=false){if(reset)ANOMALY_STATE.page=1;for(const k of ['severity','type','module','confidence','status','sort']){const el=document.getElementById(`warning-${k}`);if(el)ANOMALY_STATE[k]=el.value}ANOMALY_STATE.risk=Number(document.getElementById('warning-risk')?.value)||0;renderAnomalyRowsOnly()}
function renderAnomalyPage(){if(!ANOMALY_STATE.rendered){anomalyTable.innerHTML=`<div class='warning-controls'><div id='anomalyLoading' class='warning-refresh hidden'>Memeriksa data terbaru…</div><div id='anomalyInfo'></div><button class='btn-ghost' onclick='refreshAnomalyInBackground(true)'>Refresh</button><button class='btn-ghost' onclick='exportWarnings()'>Export</button></div><div class='warning-advanced'><select id='warning-module'><option value='all'>Semua Modul</option></select><select id='warning-confidence'><option value='all'>Semua Confidence</option><option value='high'>Tinggi</option><option value='medium'>Sedang</option><option value='low'>Rendah</option></select><select id='warning-status'><option value='all'>Semua Status</option>${['Baru','Perlu Diperiksa','Sedang Diproses','Selesai','Diabaikan'].map(x=>`<option>${x}</option>`).join('')}</select><select id='warning-sort'><option value='risk'>Risk tertinggi</option><option value='time'>Terbaru</option></select><label>Risk ≥ <input id='warning-risk' type='number' min='0' max='100' value='0'></label></div><div class='table-wrap warning-table-wrap'><table><thead><tr><th>Severity</th><th>Risk</th><th>Modul</th><th>SKU</th><th>Lokasi</th><th>Masalah / Dampak</th><th>Kemungkinan Penyebab</th><th>Confidence</th><th>Status</th><th>Aksi</th></tr></thead><tbody></tbody></table></div><div class='mv-pagination'><button class='btn-ghost' onclick='changeAnomalyPage(-1)'>Prev</button><select id='anomalySize'><option>25</option><option>50</option><option>100</option></select><button class='btn-ghost' onclick='changeAnomalyPage(1)'>Next</button></div>`;document.querySelectorAll('#anomalyTable select,#anomalyTable input').forEach(el=>el.addEventListener('change',()=>applyAnomalyFilters(true)));ANOMALY_STATE.rendered=true}const cache=getAnomalyCache();if(!ANOMALY_STATE.rows.length&&cache?.rows){ANOMALY_STATE.rows=cache.rows;renderWarningOptions();renderAnomalyRowsOnly()}refreshAnomalyInBackground(false)}
function renderWarningOptions(){const types=[...new Map(ANOMALY_STATE.rows.map(w=>[w.type,w.title])).entries()],modules=[...new Set(ANOMALY_STATE.rows.map(w=>w.module))];anomalyType.innerHTML='<option value="all">Semua Jenis</option>'+types.map(([v,l])=>`<option value='${esc(v)}'>${esc(l)}</option>`).join('');anomalyType.value=ANOMALY_STATE.type;const m=document.getElementById('warning-module');if(m){m.innerHTML='<option value="all">Semua Modul</option>'+modules.map(x=>`<option>${esc(x)}</option>`).join('');m.value=ANOMALY_STATE.module}}
function refreshAnomalyInBackground(force=false){const source=warningSources(),version=warningVersion(source),cache=getAnomalyCache();if(!force&&version===ANOMALY_STATE.lastSourceVersion)return;if(!force&&cache?.version===version){ANOMALY_STATE.rows=cache.rows;ANOMALY_STATE.lastSourceVersion=version;renderWarningOptions();applyAnomalyFilters(false);return}setAnomalyLoading(true);const token=++ANOMALY_STATE.lastRenderToken,run=()=>{const fresh=analyzeWarnings(source);if(token!==ANOMALY_STATE.lastRenderToken)return;ANOMALY_STATE.rows=mergeWarnings(ANOMALY_STATE.rows,fresh);ANOMALY_STATE.lastSourceVersion=version;setAnomalyCache(ANOMALY_STATE.rows,version);renderWarningOptions();applyAnomalyFilters(false);setAnomalyLoading(false);if(force)logActivitySafe({action:'WARNING_REFRESH',module:'Warning',detail:'Refresh manual warning',status:'SUCCESS'})};typeof requestIdleCallback==='function'?requestIdleCallback(run,{timeout:800}):setTimeout(run,0)}
function openWarningDetail(id){const w=ANOMALY_STATE.rows.find(x=>x.id===id);if(!w)return;ANOMALY_STATE.selected=w;document.getElementById('warningDetail')?.remove();const rv=reviewFor(id),el=document.createElement('div');el.id='warningDetail';el.className='warning-detail-backdrop';el.innerHTML=`<aside class='warning-detail'><button class='warning-close' onclick="document.getElementById('warningDetail').remove()">×</button><p class='eyebrow'>${esc(w.module)} · ${esc(w.category)}</p><h3>${esc(w.title)}</h3><div class='warning-score'><span class='badge ${sevClass(w.severity)}'>${w.severity}</span><strong>Risk ${w.riskScore}</strong><span>Confidence ${w.confidence}%</span></div><dl><dt>SKU / Lokasi</dt><dd>${esc(w.sku||'-')} · ${esc(w.location||'-')}</dd><dt>Analisis</dt><dd>${esc(w.description)}</dd><dt>Perhitungan</dt><dd>${esc(w.calculation||'-')}</dd><dt>Evidence</dt><dd><ul>${w.evidence.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></dd><dt>Kemungkinan penyebab</dt><dd>${(w.possibleCause||[]).map(x=>`<p><strong>${esc(x.title)}</strong> (${x.confidence||w.confidence}%)<br><small>${esc(x.evidence.join(' · '))}</small></p>`).join('')||'Perlu diperiksa manual.'}</dd><dt>Rekomendasi</dt><dd>${esc(w.recommendation)}</dd></dl><label>Status<select id='warningReviewStatus'>${['Baru','Perlu Diperiksa','Sedang Diproses','Selesai','Diabaikan'].map(x=>`<option ${rv.status===x?'selected':''}>${x}</option>`).join('')}</select></label><div class='warning-actions'><button class='btn-primary' onclick="setWarningReview('${id}',document.getElementById('warningReviewStatus').value)">Simpan status</button><button class='btn-ghost' onclick="navigator.clipboard.writeText('${encAttr(w.sku)}')">Copy SKU</button><button class='btn-ghost' onclick="copyWarningAnalysis('${id}')">Copy analisis</button></div></aside>`;document.body.appendChild(el)}
function copyWarningAnalysis(id){const w=ANOMALY_STATE.rows.find(x=>x.id===id);if(w)navigator.clipboard.writeText(`${w.title}
${w.description}
Evidence: ${w.evidence.join('; ')}
Rekomendasi: ${w.recommendation}`)}
function exportWarnings(){const rows=getAnomalyFilteredRows(),cols=['severity','riskScore','module','sku','itemName','location','title','impact','confidence','status'];const csv=[cols.join(',')].concat(rows.map(w=>cols.map(k=>`"${String(k==='status'?reviewFor(w.id).status:w[k]??'').replaceAll('"','""')}"`).join(','))).join('
'),a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));a.download='warning-analysis.csv';a.click();URL.revokeObjectURL(a.href);logActivitySafe({action:'WARNING_EXPORT',module:'Warning',detail:`Export ${rows.length} warning`,status:'SUCCESS'})}
function changeAnomalyPage(n){ANOMALY_STATE.page=Math.max(1,ANOMALY_STATE.page+n);renderAnomalyRowsOnly()}
Object.assign(window,{renderAnomalyPage,changeAnomalyPage,applyAnomalyFilters,scheduleAnomalySearch,refreshAnomalyInBackground,openWarningDetail,setWarningReview,copyWarningAnalysis,exportWarnings});
