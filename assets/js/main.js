...
function formatLocationNumber(value){return new Intl.NumberFormat("id-ID").format(Number(value)||0);}
function resetLocationFilters(){locSearchInput.value="";locSkuSearchInput.value="";locStatusFilter.value="all";locTypeFilter.value="all";locSort.value="skuDesc";LOCATION_STATE.page=1;drawLocations();}
function drawLocations(){const rows=LOCATION_STATE.rows||[];const qLoc=clean(locSearchInput?.value||'');const qSku=clean(locSkuSearchInput?.value||'');const status=locStatusFilter?.value||'all';const locType=locTypeFilter?.value||'all';const sorter=locSort?.value||'skuDesc';const pageSize=Number(locPageSize?.value||25);LOCATION_STATE.pageSize=[25,50,100].includes(pageSize)?pageSize:25;let filtered=rows.filter(r=>{const rowType=getLocationTypeByDigit(r.lokasi);return (!qLoc||clean(r.lokasi).includes(qLoc))&&(!qSku||r.skus.some(s=>clean(`${s.sku} ${s.nama}`).includes(qSku)))&&(status==='all'||r.status===status)&&(locType==='all'||rowType===locType);});filtered=[...filtered].sort(sorter==='qtyDesc'?(a,b)=>b.totalQty-a.totalQty||b.jumlahSku-a.jumlahSku:sorter==='qtyAsc'?(a,b)=>a.totalQty-b.totalQty||a.jumlahSku-b.jumlahSku:sorter==='skuAsc'?(a,b)=>a.jumlahSku-b.jumlahSku||a.totalQty-b.totalQty:sorter==='az'?(a,b)=>a.lokasi.localeCompare(b.lokasi):(a,b)=>b.jumlahSku-a.jumlahSku||b.totalQty-a.totalQty);LOCATION_STATE.filtered=filtered;const totalLokasi=rows.length,totalSku=new Set(rows.flatMap(r=>r.skus.map(s=>clean(s.sku)))).size;locationsSummary.innerHTML=[["map-pin","Total Lokasi",totalLokasi,"lokasi aktif"],["package","Total SKU Berlokasi",totalSku,"SKU unik"]].map(c=>`<div class='metric location-summary-card'><span class='location-summary-icon'><i data-lucide='${c[0]}'></i></span><div><div class='k'>${c[1]}</div><div class='v'>${formatLocationNumber(c[2])}</div><div class='location-summary-note'>${c[3]}</div></div></div>`).join('');const max=Math.max(1,Math.ceil(filtered.length/LOCATION_STATE.pageSize));if(LOCATION_STATE.page>max)LOCATION_STATE.page=max;const start=(LOCATION_STATE.page-1)*LOCATION_STATE.pageSize;const pageRows=filtered.slice(start,LOCATION_STATE.page*LOCATION_STATE.pageSize);const emptyMessage=`<tr><td colspan='6'><div class='locations-no-result'><i data-lucide='search-x'></i><strong>Lokasi tidak ditemukan</strong><span>Tidak ada lokasi yang cocok dengan “${esc(locSearchInput?.value||locSkuSearchInput?.value||'filter aktif')}”.</span><button class='btn-ghost' type='button' onclick='resetLocationFilters()'>Reset filter</button></div></td></tr>`;locationsTable.innerHTML=`<div class='card locations-list-card'><div class='section-header'><div><h4><i data-lucide='map-pinned'></i> List Lokasi</h4><div class='subtitle'>Urutan lokasi sesuai filter dan sorting aktif</div></div><span class='location-count'>${formatLocationNumber(filtered.length)} lokasi</span></div><div class='table-wrap table-wrap-full'><table><thead><tr><th>No</th><th>Lokasi</th><th>Jumlah SKU</th><th>Total Qty</th><th>Status</th><th>Aksi</th></tr></thead><tbody>${pageRows.map((r,idx)=>`<tr class='location-row ${LOCATION_STATE.selected===r.lokasi?'is-selected':''}' data-location='${encAttr(r.lokasi)}' aria-selected='${LOCATION_STATE.selected===r.lokasi}'><td class='loc-row-no'>${start+idx+1}</td><td><span class='loc-name'><i data-lucide='map-pin'></i><strong>${esc(r.lokasi)}</strong></span></td><td>${renderLocationMetric('package','SKU',formatLocationNumber(r.jumlahSku),'loc-sku-metric')}</td><td>${renderLocationMetric('boxes','Qty',formatLocationNumber(r.totalQty),'loc-qty-metric')}</td><td>${renderLocationStatusBadge(r.status)}</td><td><button class='btn-ghost loc-action-btn' onclick="selectLocationDetail('${encAttr(r.lokasi)}')">Lihat SKU <i data-lucide='arrow-right'></i></button></td></tr>`).join('')||emptyMessage}</tbody></table></div><div class='mv-pagination'><span>${filtered.length?start+1:0}–${Math.min(LOCATION_STATE.page*LOCATION_STATE.pageSize,filtered.length)} dari ${formatLocationNumber(filtered.length)} lokasi</span><div class='location-page-nav'><button class='icon-btn' aria-label='Halaman sebelumnya' title='Halaman sebelumnya' onclick='changeLocationPage(-1)' ${LOCATION_STATE.page<=1?'disabled':''}><i data-lucide='chevron-left'></i></button><span>Halaman ${LOCATION_STATE.page} dari ${max}</span><button class='icon-btn' aria-label='Halaman berikutnya' title='Halaman berikutnya' onclick='changeLocationPage(1)' ${LOCATION_STATE.page>=max?'disabled':''}><i data-lucide='chevron-right'></i></button></div></div></div>`;if(LOCATION_STATE.selected)selectLocationDetail(encAttr(LOCATION_STATE.selected),true);renderEmptyLocationSection();if(window.lucide)window.lucide.createIcons();}
...
function renderEmptyLocationSection(){
  ...
  locationsEmpty.innerHTML=`<div class='card locations-empty-card'><div class='section-header'><div><h4><i data-lucide='map-pin-off'></i> Lokasi Kosong</h4><div class='subtitle'>Lokasi valid yang belum memiliki stok aktif.</div></div><span class='location-count'>${formatLocationNumber(empties.length)} lokasi</span></div><div class='empty-location-list'>${empties.slice(0,300).map(loc=>`<span><i data-lucide='map-pin'></i>${esc(loc)}</span>`).join('')}</div></div>`;if(window.lucide)window.lucide.createIcons();
}
...
function selectLocationDetail(locEncoded,keep=false){const lokasi=decodeURIComponent(locEncoded||'');LOCATION_STATE.selected=lokasi;document.querySelectorAll('#locationsTable .location-row').forEach(el=>{const selected=el.dataset.location===lokasi;el.classList.toggle('is-selected',selected);el.setAttribute('aria-selected',String(selected));});const row=(LOCATION_STATE.filtered||LOCATION_STATE.rows).find(r=>r.lokasi===lokasi);if(!row){if(!keep)locationDetail.innerHTML='';return;}const skuRows=[...row.skus].sort((a,b)=>b.qty-a.qty);locationDetail.innerHTML=`<div class='card location-detail-card'><div class='location-detail-header'><div><span class='location-detail-eyebrow'>Detail Lokasi</span><h4><i data-lucide='map-pin'></i>${esc(lokasi)}</h4></div><div class='location-detail-summary'><strong>${formatLocationNumber(row.jumlahSku)} <span>SKU</span></strong><strong>${formatLocationNumber(row.totalQty)} <span>pcs</span></strong>${renderLocationStatusBadge(row.status)}</div></div><div class='table-wrap table-wrap-full location-detail-table'><table><thead><tr><th>SKU</th><th>Nama Barang</th><th>Qty / Stok</th><th>Aksi</th></tr></thead><tbody>${skuRows.map(s=>`<tr><td data-label='SKU'><div class='copy-cell'><span class='copy-cell-text loc-sku-code'>${esc(s.sku)}</span><button class='copy-mini-btn loc-copy-btn' type='button' title='Copy SKU' aria-label='Copy SKU' onclick="copyText(decodeURIComponent('${encAttr(s.sku)}'),'SKU disalin',this)"><i data-lucide='copy'></i><span class='sr-only'>Copy</span></button></div></td><td data-label='Nama Barang'><div class='copy-cell'><span class='copy-cell-text'>${esc(s.nama)}</span><button class='copy-mini-btn loc-copy-btn' type='button' title='Copy nama barang' aria-label='Copy nama barang' onclick="copyText(decodeURIComponent('${encAttr(s.nama)}'),'Nama barang disalin',this)"><i data-lucide='copy'></i><span class='sr-only'>Copy</span></button></div></td><td data-label='Qty / Stok' class='location-qty'>${formatLocationNumber(s.qty)}</td><td data-label='Aksi'><button class='btn-ghost loc-detail-btn' onclick="navigateTo('/sku/'+encodeURIComponent(decodeURIComponent('${encAttr(s.sku)}')))" ${String(s.sku).trim()?"":"disabled"}><i data-lucide='eye'></i> Detail <i data-lucide='arrow-right'></i></button></td></tr>`).join('')||`<tr><td colspan='4'><div class='state'>Lokasi ini belum memiliki SKU aktif.</div></td></tr>`}</tbody></table></div></div>`;if(window.lucide)window.lucide.createIcons();if(!keep){locationDetail.scrollIntoView({behavior:'smooth',block:'start'});}}
