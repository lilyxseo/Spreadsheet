...
const DATA = {}; let CACHE_SKU = new Map(); let currentFilter="Semua", lastResults=[], lastQuery="", apiConnected=false, currentSku="", isSyncing=false, searchModalOpen=false, prevRouteBeforeSearch="/", autoRefreshTimer=null;
...
function applyTheme(){const saved=localStorage.getItem("theme");if(saved==="dark")document.body.classList.add("dark");syncThemeButton();syncAutoRefreshButton();}
...
function isAutoRefreshEnabled(){return localStorage.getItem("auto_refresh")==="on";}
function startAutoRefresh(){stopAutoRefresh();autoRefreshTimer=setInterval(()=>loadAllData(true),60000);}
function stopAutoRefresh(){if(autoRefreshTimer){clearInterval(autoRefreshTimer);autoRefreshTimer=null;}}
function toggleAutoRefresh(){const next=!isAutoRefreshEnabled();localStorage.setItem("auto_refresh",next?"on":"off");if(next){startAutoRefresh();loadAllData(true);}else stopAutoRefresh();syncAutoRefreshButton();toast(`Auto refresh ${next?"aktif":"nonaktif"}`);}
function syncAutoRefreshButton(){if(!refreshToggleHeader)return;const active=isAutoRefreshEnabled();refreshToggleHeader.innerHTML=`<i data-lucide="${active?"refresh-cw":"clock-3"}"></i>`;refreshToggleHeader.title=`Auto refresh ${active?"ON":"OFF"}`;refreshToggleHeader.setAttribute("aria-label",`Toggle auto refresh (${active?"ON":"OFF"})`);if(active&&!autoRefreshTimer)startAutoRefresh();if(!active&&autoRefreshTimer)stopAutoRefresh();if(window.lucide)lucide.createIcons();}
