...
function parseBalikanLocationEntries(lokasiValue,rowQty=0){
  const raw=String(lokasiValue??'').trim();
  if(!raw)return [];
  const fallbackQty=toNumberSafe(rowQty);
  return raw
    .split(/\r?\n|[;,|/]+|\s+dan\s+/i)
    .map(part=>String(part||'').trim())
    .filter(Boolean)
    .map(part=>{
      const qtyMatch=part.match(/(?:[:=\-–—]\s*)?(-?\d+(?:[.,]\d+)?)\s*(?:pcs?|pieces?|unit|qty)?\s*$/i);
      const hasQty=!!(qtyMatch&&(/^[\s:=\-–—]/.test(qtyMatch[0])||/(?:pcs?|pieces?|unit|qty)/i.test(qtyMatch[0])));
      const qty=hasQty?toNumberSafe(qtyMatch[1]):fallbackQty;
      const lokasi=hasQty?part.slice(0,qtyMatch.index).replace(/[:=\-–—\s]+$/,'').trim():part;
      return lokasi?{lokasi,qty}:null;
    })
    .filter(Boolean);
}
function getBalikanLocationSummary(rows=[]){
  ...
  (rows||[]).forEach(row=>{
    parseBalikanLocationEntries(row?.lokasi,row?.qty).forEach(({lokasi,qty})=>{
      totals.set(lokasi,(totals.get(lokasi)||0)+qty);
    });
  });
