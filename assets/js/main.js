...
import { EMPTY_FILTER_VALUE, isEmptyFilterValue, normalizeColumnSearch, serializeFilterValue, matchesColumnFilters } from "./column-filter.js";
...
function bindEvents(){searchInput?.addEventListener("input",e=>scheduleSearchFilter(e.target?.value||""));statsFilter?.addEventListener("change",updateStats);darkBtnHeader?.addEventListener("click",toggleDark);refreshToggleHeader?.addEventListener("click",triggerManualRefresh);bindDevAutoRefreshControls();const din=debounce(()=>renderDataTablePage("in","Barang Masuk"),250),dout=debounce(()=>renderDataTablePage("out","Barang Keluar"),250);inSearch?.addEventListener("input",din);outSearch?.addEventListener("input",dout);window.addEventListener("resize",()=>{document.querySelectorAll("[data-col-filter-menu]:not([hidden])").forEach(menu=>positionColumnFilterMenu(menu));document.querySelectorAll(".mv-columns.open").forEach(panel=>positionColumnMenu(panel.id.replace("mv-cols-","")));});document.addEventListener("change",e=>{const t=e.target;if(t?.matches("[data-mv-filter]")){const m=t.dataset.mvMode;debouncedTableRender(m);}if(t?.closest("[data-col-filter-menu]")&&t?.matches('input[type="checkbox"]')){const menu=t.closest("[data-col-filter-menu]");const mode=menu.dataset.mode,col=menu.dataset.col;const selected=[...menu.querySelectorAll('input[type="checkbox"]:checked')].map(x=>x.value);const st=mode==='balikan'?ensureBalikanFilterState():(TABLE_STATE[mode]||{});ensureColumnFilterState(mode);if(!st.columnFilters)st.columnFilters={};st.columnFilters[col]=selected;st.openFilterCol=col;if(mode==='balikan')scheduleBalikanRender(false,250);else rerenderTableWithScrollRestore(mode,true);}});document.addEventListener("input",e=>{const t=e.target;if(t?.matches("[data-col-filter-contains]")){const menu=t.closest("[data-col-filter-menu]");if(menu?.dataset.mode!=="balikan")return;const st=ensureBalikanFilterState();st.columnContains[menu.dataset.col]=t.value;st.openFilterCol=menu.dataset.col;clearTimeout(BALIKAN_STATE.filterContainsTimer);BALIKAN_STATE.filterContainsTimer=setTimeout(()=>scheduleBalikanRender(false,0),250);return;}if(!t?.matches("[data-col-filter-search]"))return;const q=clean(t.value);const menu=t.closest("[data-col-filter-menu]");menu?.querySelectorAll("[data-opt-item]").forEach(item=>{item.style.display=!q||clean(item.textContent).includes(q)?"":"none";});});document.addEventListener("click",e=>{const btn=e.target.closest("[data-search-page]");if(!btn)return;changeSearchPage(Number(btn.dataset.searchPage)||0);});anomalySeverity?.addEventListener("change",()=>applyAnomalyFilters(true));
...
if(e.target.closest("[data-col-filter-menu]")){const menu=e.target.closest("[data-col-filter-menu]");const mode=menu.dataset.mode,col=menu.dataset.col;const st=mode==='balikan'?ensureBalikanFilterState():(TABLE_STATE[mode]||{});ensureColumnFilterState(mode);if(e.target.matches("[data-col-filter-clear]")){st.columnFilters[col]=[];if(mode==='balikan'){st.columnContains[col]='';}st.openFilterCol=col;if(mode==='balikan')scheduleBalikanRender(false,0);else rerenderTableWithScrollRestore(mode,true);}if(e.target.matches("[data-col-filter-all]")){const rowsSource=mode==='balikan'?((BALIKAN_STATE.filterState?.rows)||[]):(st.rows||[]);const applier=mode==='balikan'?applyBalikanTableFilters:applyTableFilters;st.columnFilters[col]=getUniqueOptions(applier(rowsSource,mode,col),col);st.openFilterCol=col;if(mode==='balikan')scheduleBalikanRender(false,0);else rerenderTableWithScrollRestore(mode,true);}return;}
...
const BALIKAN_BASE_COLUMNS=[
  {key:'checked',label:'Centang',className:'col-check'},
  {key:'sheetName',label:'Sheet',value:row=>getBalikanActiveSheetName(row)},
  {key:'no',label:'No'},
  {key:'sku',label:'SKU',textSearch:true},
  {key:'namaBarang',label:'Nama Barang',textSearch:true},
  {key:'qty',label:'Qty'},
  {key:'rakTujuan',label:'Rak Tujuan',className:'column-rak-tujuan'},
  {key:'lokasi',label:'Lokasi',className:'column-lokasi'},
  {key:'stokBulky',label:'Stok Bulky'},
  {key:'stokRetail',label:'Stok Retail'},
  {key:'status',label:'Status'},
  {key:'keterangan',label:'Keterangan',className:'column-keterangan',textSearch:true}
];
function getBalikanTableColumns(){const dynamic=Array.isArray(window.BALIKAN_DYNAMIC_COLUMNS)?window.BALIKAN_DYNAMIC_COLUMNS:[];return [...BALIKAN_BASE_COLUMNS,...dynamic.map(col=>({key:col.key,label:col.header||col.key,className:getBalikanColumnClass(col.key,col.header),textSearch:true}))];}
function getBalikanColumnValue(row,column){return typeof column?.value==='function'?column.value(row):row?.[column?.key];}
function ensureBalikanFilterState(){if(!BALIKAN_STATE.filterState)BALIKAN_STATE.filterState={page:1,pageSize:50,openFilterCol:'',columnFilters:{},columnContains:{},rows:[],filtered:[]};const st=BALIKAN_STATE.filterState;st.columnContains=st.columnContains||{};if(![25,50,100].includes(Number(st.pageSize)))st.pageSize=50;if(!Number.isFinite(Number(st.page))||Number(st.page)<1)st.page=1;getBalikanTableColumns().forEach(({key})=>{if(!Array.isArray(st.columnFilters[key]))st.columnFilters[key]=[];});return st;}
...
function applyBalikanTableFilters(rows,mode='balikan',omitCol=''){const st=ensureBalikanFilterState();const columns=getBalikanTableColumns();const rawQuery=window.balikanSearchKeyword||'';const q=normalizeBalikanRawSearch(rawQuery);const qTokens=getBalikanSearchTokens(q);const isLocationQuery=isBalikanLocationSearchQuery(rawQuery)&&qTokens.length===1;const locationQuery=normalizeBalikanLocationSearchValue(rawQuery);const exactScanSku=normalizeBalikanRawSearch(BALIKAN_STATE.exactScanSku||'');const selectedSku=normalizeBalikanRawSearch(BALIKAN_STATE.selectedSkuValue||'');return rows.filter(r=>{if(!matchesColumnFilters(r,columns,st.columnFilters,st.columnContains,omitCol))return false;if(exactScanSku&&normalizeBalikanRawSearch(r.sku)!==exactScanSku)return false;if(selectedSku&&q===selectedSku&&normalizeBalikanRawSearch(r.sku)!==selectedSku)return false;if(qTokens.length){if(matchesBalikanSkuBarcodeRaw(r,q))return true;if(isLocationQuery){if(!matchesBalikanLocationQuery(r,locationQuery))return false;}else{const rowText=getBalikanSearchText(r);if(!qTokens.every(token=>rowText.includes(token)))return false;}}return true;});}
...
function resetBalikanFilter(){const st=ensureBalikanFilterState();st.columnFilters={};st.columnContains={};st.openFilterCol='';st.page=1;BALIKAN_STATE.filterOptionsCache.clear();renderBalikanTable(false);}
...
function buildBalikanHeaderHtml(baseRows){const st=ensureBalikanFilterState();const dataKey=checksumBalikanRows(baseRows,window.BALIKAN_DYNAMIC_COLUMNS||[]);const headerWithFilter=(column)=>{const col=column.key,selected=st.columnFilters[col]||[],contains=st.columnContains[col]||'',active=selected.length>0||!!contains;const cacheKey=`${dataKey}:${col}:${JSON.stringify(st.columnFilters)}:${JSON.stringify(st.columnContains)}`;let options=BALIKAN_STATE.filterOptionsCache.get(cacheKey);if(!options){options=[...new Set(applyBalikanTableFilters(baseRows,'balikan',col).map(row=>{const value=getBalikanColumnValue(row,column);return serializeFilterValue(value);}))].sort((a,b)=>a===EMPTY_FILTER_VALUE?-1:b===EMPTY_FILTER_VALUE?1:a.localeCompare(b,undefined,{numeric:true}));BALIKAN_STATE.filterOptionsCache.set(cacheKey,options);if(BALIKAN_STATE.filterOptionsCache.size>100)BALIKAN_STATE.filterOptionsCache.clear();}const label=column.label||col;return `<th class='${esc(column.className||'')}'><div class='th-filter-wrap'><span>${esc(label)}</span><button class='th-filter-btn ${active?'active':''}' type='button' data-col-filter-toggle data-mode='balikan' data-col='${esc(col)}' title='Filter ${esc(label)}' aria-label='Filter ${esc(label)}'><span class='th-filter-icon'>▾</span>${active?"<span class='th-filter-active-dot' aria-hidden='true'></span>":''}</button><div class='th-filter-dropdown' data-col-filter-menu data-mode='balikan' data-col='${esc(col)}' hidden>${column.textSearch?`<input class='th-filter-contains' data-col-filter-contains placeholder='Mengandung...' value='${esc(contains)}'>`:''}<input class='th-filter-search' data-col-filter-search placeholder='Cari pilihan...'><div class='th-filter-actions'><button type='button' data-col-filter-clear>Semua</button></div><div class='th-filter-options'>${options.map(v=>`<label data-opt-item><input type='checkbox' value='${esc(v)}' ${selected.includes(v)?'checked':''}> <span>${v===EMPTY_FILTER_VALUE?'(Kosong)':esc(v)}</span></label>`).join('')||"<div class='subtitle'>Tidak ada opsi</div>"}</div></div></div></th>`;};return `<div class="balikan-table-wrapper"><table class="data-table balikan-table"><thead><tr>${getBalikanTableColumns().map(headerWithFilter).join('')}</tr></thead><tbody></tbody></table></div><div id="balikanPager" class="mv-pagination"></div>`;}
function renderBalikanTable(keepPage=true){if(BALIKAN_STATE.isRendering){BALIKAN_STATE.pendingRender=true;return;}BALIKAN_STATE.isRendering=true;try{const st=ensureBalikanFilterState();if(!keepPage)st.page=1;const {baseRows,rows,pageRows,totalPages}=getBalikanVisibleRows();renderBalikanSummaryInfo(rows,baseRows);if(!baseRows.length){balikanTable.innerHTML='<div class="subtitle">Data kosong.</div>';BALIKAN_STATE.lastRenderedChecksum='';return;}const headerKey=checksumBalikanRows(baseRows,window.BALIKAN_DYNAMIC_COLUMNS||[])+JSON.stringify(st.columnFilters)+JSON.stringify(st.columnContains);if(BALIKAN_STATE.lastRenderedHeaderKey!==headerKey||!balikanTable.querySelector('tbody')){balikanTable.innerHTML=buildBalikanHeaderHtml(baseRows);BALIKAN_STATE.lastRenderedHeaderKey=headerKey;BALIKAN_STATE.lastRenderedChecksum='';if(st.openFilterCol){const menu=balikanTable.querySelector(`[data-col-filter-menu][data-mode='balikan'][data-col='${CSS.escape(st.openFilterCol)}']`);if(menu){menu.hidden=false;positionColumnFilterMenu(menu);}}}renderVisibleBalikanRows(pageRows,totalPages); }finally{BALIKAN_STATE.isRendering=false;if(BALIKAN_STATE.pendingRender){BALIKAN_STATE.pendingRender=false;scheduleBalikanRender(true,0);}}}
