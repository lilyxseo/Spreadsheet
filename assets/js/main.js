...
import { buildWarningReport } from "./warning-engine.js";
...
const ANOMALY_CACHE_KEY="warningReportCacheV3-retail-regex";
const ANOMALY_STATE={page:1,pageSize:25,rows:[],filtered:[],q:"",severity:"all",type:"all",rendered:false,isLoading:false,_searchDebounce:null,lastRenderToken:0,dataVersion:"",locationMasterReady:false,diagnostics:null};
const WARNING_GROUPS={LOCATION:new Set(["INVALID_LOCATION","EMPTY_LOCATION","MULTI_LOCATION","LOCATION_TYPE_MISMATCH","SKU_NOT_AT_LOCATION"]),SKU:new Set(["EMPTY_SKU","UNKNOWN_SKU","SKU_NAME_MISMATCH","NAME_MATCHES_OTHER_SKU"]),BALANCE:new Set(["STOCK_IMBALANCE","STOCK_NOT_UPDATED","DATA_NOT_SYNCED"]),FORMAT:new Set(["INVALID_QTY","NEGATIVE_QTY","SAME_FROM_TO"]),DUPLICATE:new Set(["DUPLICATE_REFERENCE"])};
function sevClass(s){return s==='CRITICAL'?'b-high':s==='WARNING'?'b-medium':'b-low';}
function anomalyVersion(){return `${REFRESH_STATE.dataVersion}|${(DATA["Kartu Stock"]||[]).length}|${getBarangKeluarRows().length}`;}
function setAnomalyCache(rows,version,diagnostics){try{localStorage.setItem(ANOMALY_CACHE_KEY,JSON.stringify({rows,version,diagnostics,updatedAt:Date.now()}));}catch(_){}}
function getAnomalyCache(){try{return JSON.parse(localStorage.getItem(ANOMALY_CACHE_KEY)||"null");}catch(_){return null;}}
...
function updateAnomalyTypeOptions(){if(anomalyType)anomalyType.value=ANOMALY_STATE.type;}
function getAnomalyFilteredRows(){const q=clean(ANOMALY_STATE.q);return (ANOMALY_STATE.rows||[]).filter(r=>{const filter=ANOMALY_STATE.severity;if(filter==='CRITICAL'&&r.severity!=='CRITICAL')return false;if(WARNING_GROUPS[filter]&&!WARNING_GROUPS[filter].has(r.type))return false;if(ANOMALY_STATE.type!=='all'&&r.locationType!==ANOMALY_STATE.type)return false;return !q||clean(`${r.sku} ${r.namaBarang} ${r.location} ${r.problem} ${r.source}`).includes(q);});}
function warningSuggestion(r){return Array.isArray(r.suggestion)?r.suggestion.join(' / '):r.suggestion;}
function renderAnomalyRowsOnly(){const rows=getAnomalyFilteredRows();ANOMALY_STATE.filtered=rows;const total=rows.length,max=Math.max(1,Math.ceil(total/ANOMALY_STATE.pageSize));ANOMALY_STATE.page=Math.min(ANOMALY_STATE.page,max);const start=(ANOMALY_STATE.page-1)*ANOMALY_STATE.pageSize,pageRows=rows.slice(start,start+ANOMALY_STATE.pageSize);const tbody=document.querySelector('#anomalyTable tbody');if(tbody)tbody.innerHTML=pageRows.map(r=>`<tr><td><span class='badge ${sevClass(r.severity)}'>${esc(r.severity)}</span></td><td>${esc(r.sku)}</td><td>${esc(r.namaBarang)}</td><td>${esc(r.location)}</td><td>${esc(r.locationType)}</td><td>${esc(r.problem)}</td><td>${esc(r.currentValue)}</td><td>${esc(warningSuggestion(r))}</td><td>${esc(r.source)}</td><td><button class='btn-ghost' onclick="openWarningDetail('${encAttr(r.id)}')">Detail</button></td></tr>`).join('')||"<tr><td colspan='10'><div class='state'>Tidak ada warning pada filter ini.</div></td></tr>";const info=document.getElementById('anomalyInfo');if(info)info.textContent=`Menampilkan ${pageRows.length} dari ${total.toLocaleString('id-ID')} warning`;}
function renderAnomalySummary(){const rows=ANOMALY_STATE.rows||[], count=types=>rows.filter(r=>types.has(r.type)).length;const locationValue=ANOMALY_STATE.locationMasterReady?count(WARNING_GROUPS.LOCATION):"—";anomalySummary.innerHTML=[["Salah Lokasi",locationValue],["SKU/Nama Tidak Sesuai",count(WARNING_GROUPS.SKU)],["Tidak Balance",count(WARNING_GROUPS.BALANCE)],["Input Tidak Terbaca",count(WARNING_GROUPS.FORMAT)],["Critical",rows.filter(r=>r.severity==='CRITICAL').length]].map(([k,v],index)=>`<div class='metric'><div class='k'>${k}</div><div class='v'>${v}</div>${index===0&&!ANOMALY_STATE.locationMasterReady?"<small>Master lokasi belum siap</small>":""}</div>`).join('');}
function applyAnomalyFilters(resetPage=false){if(resetPage)ANOMALY_STATE.page=1;ANOMALY_STATE.severity=anomalySeverity?.value||'all';ANOMALY_STATE.type=anomalyType?.value||'all';renderAnomalyRowsOnly();renderAnomalySummary();}
function renderAnomalyPage(){if(!ANOMALY_STATE.rendered){anomalyTable.innerHTML=`<div class='row anomaly-toolbar'><div id='anomalyInfo' class='mv-pagination-info'></div><div class='row'><select id='anomalySize'><option>25</option><option>50</option><option>100</option></select><button class='btn-ghost' onclick='changeAnomalyPage(-1)'>Prev</button><button class='btn-ghost' onclick='changeAnomalyPage(1)'>Next</button></div></div><div id='anomalyLoading' class='state hidden'></div><div class='table-wrap table-wrap-full anomaly-table-wrap'><table><thead><tr><th>Severity</th><th>SKU</th><th>Nama Barang</th><th>Lokasi</th><th>Jenis</th><th>Masalah</th><th>Nilai Saat Ini</th><th>Saran</th><th>Source</th><th>Aksi</th></tr></thead><tbody></tbody></table></div>`;document.getElementById('anomalySize').onchange=e=>{ANOMALY_STATE.pageSize=Number(e.target.value)||25;ANOMALY_STATE.page=1;renderAnomalyRowsOnly();};ANOMALY_STATE.rendered=true;}const cache=getAnomalyCache();if(!ANOMALY_STATE.rows.length&&cache?.rows?.length){ANOMALY_STATE.rows=cache.rows;applyAnomalyFilters();}refreshAnomalyInBackground();}
function refreshAnomalyInBackground(){const version=anomalyVersion();if(ANOMALY_STATE.dataVersion===version)return;const cached=getAnomalyCache();if(cached?.version===version){ANOMALY_STATE.rows=cached.rows;ANOMALY_STATE.locationMasterReady=Boolean(cached.diagnostics?.ready);ANOMALY_STATE.diagnostics=cached.diagnostics;ANOMALY_STATE.dataVersion=version;applyAnomalyFilters();setAnomalyLoading(false);return;}const token=++ANOMALY_STATE.lastRenderToken;setAnomalyLoading(true,'Menganalisis cache dalam batch ringan...');const run=()=>{const result=buildWarningReport({kartu:DATA["Kartu Stock"]||[],outbound:getBarangKeluarRows(),sync:{}});if(token!==ANOMALY_STATE.lastRenderToken)return;ANOMALY_STATE.rows=result.warnings;ANOMALY_STATE.locationMasterReady=result.locationMasterReady;ANOMALY_STATE.diagnostics=result.diagnostics;ANOMALY_STATE.dataVersion=version;setAnomalyCache(result.warnings,version,result.diagnostics);console.info('Warning location master diagnostic',result.diagnostics);applyAnomalyFilters();setAnomalyLoading(false);};typeof requestIdleCallback==='function'?requestIdleCallback(run,{timeout:700}):setTimeout(run,0);}
function openWarningDetail(id){const r=ANOMALY_STATE.rows.find(row=>row.id===decodeURIComponent(id));const dialog=document.getElementById('warningDetail');if(!r||!dialog)return;dialog.innerHTML=`<div class='section-header'><h3>Detail Warning</h3><button class='btn-ghost' onclick='warningDetail.close()'>Tutup</button></div><dl><dt>Masalah</dt><dd>${esc(r.problem)}</dd><dt>Lokasi ditemukan</dt><dd>${esc(r.location)}</dd><dt>Jenis lokasi</dt><dd>${esc(r.locationType)}</dd><dt>Nilai saat ini</dt><dd>${esc(r.currentValue)}</dd><dt>Nilai referensi</dt><dd>${esc(r.expectedValue)}</dd><dt>Saran</dt><dd>${esc(warningSuggestion(r))}</dd><dt>Source Sheet / Row</dt><dd>${esc(r.source)} • row ${esc(r.relatedRow||'-')}</dd><dt>SKU</dt><dd>${esc(r.sku)}</dd><dt>Nama Barang</dt><dd>${esc(r.namaBarang)}</dd></dl>`;dialog.showModal();}
function changeAnomalyPage(step){const max=Math.max(1,Math.ceil(ANOMALY_STATE.filtered.length/ANOMALY_STATE.pageSize));ANOMALY_STATE.page=Math.min(max,Math.max(1,ANOMALY_STATE.page+step));renderAnomalyRowsOnly();}
