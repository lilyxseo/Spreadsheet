...
function parseCycleHistoryRows(rawValues){const values=Array.isArray(rawValues)?rawValues:[];if(!values.length)return {rows:[],error:""};const header=(values[0]||[]).map(v=>clean(String(v||"")));const idx=(aliases)=>{for(const a of aliases){const i=header.indexOf(clean(a));if(i>=0)return i;}return -1;};const iT=idx(CYCLE_HEADER_ALIAS.tanggal),iL=idx(CYCLE_HEADER_ALIAS.lokasi),iS=idx(CYCLE_HEADER_ALIAS.sku),iN=idx(CYCLE_HEADER_ALIAS.nama),iB=idx(CYCLE_HEADER_ALIAS.bulky),iR=idx(CYCLE_HEADER_ALIAS.retail),iAB=idx(CYCLE_HEADER_ALIAS.aktualBulky),iAR=idx(CYCLE_HEADER_ALIAS.aktualRetail),iC=idx(CYCLE_HEADER_ALIAS.catatan),iCA=idx(CYCLE_HEADER_ALIAS.createdAt);if([iT,iL,iS,iN].some(i=>i<0))return {rows:[],error:"Header Cycle Count tidak valid."};const rows=[];for(let r=1;r<values.length;r++){const row=values[r]||[];if(!row.some(c=>String(c||"").trim()))continue;rows.push({tanggal:String(row[iT]||""),lokasi:String(row[iL]||""),sku:String(row[iS]||""),nama:String(row[iN]||""),bulky:parseNumber(row[iB]),retail:parseNumber(row[iR]),aktualBulky:parseNumber(row[iAB]),aktualRetail:parseNumber(row[iAR]),catatan:String(row[iC]||""),createdAt:String(row[iCA]||"")});}return {rows,error:""};}
...
function getCycleSourceRows(){return (DATA["Kartu Stock"]||[]).map(r=>({sku:String(getVal(r,["sku"])||"").trim(),nama:String(getVal(r,["nama barang","nama","item","description"])||"-").trim()||"-",lokasi:String(getVal(r,["lokasi","location","rak","bin","area"])||"-").trim()||"-",bulky:parseNumber(getVal(r,["bulky","stok bulky","qty bulky","stok akhir","stok global"])),retail:parseNumber(getVal(r,["retail","stok retail","qty retail"]))})).filter(r=>r.sku);}
...
function renderCycleCountPage(){if(!cycleCountApp)return;const candidates=CYCLE_STATE.sessionActive?getCycleCandidates(CYCLE_STATE.search):[];const history=buildCycleHistoryRows();const submitDisabled=!canSubmitCycle()||CYCLE_STATE.submitting;cycleCountApp.innerHTML=`<div class='card cc-card'><div class='section-header'><h4>Cycle Count</h4><div class='row'>${CYCLE_STATE.sessionActive?"<button class='btn-ghost' onclick='ccCancelSession()'>Batal Cycle Count</button>":""}<button class='btn-primary' onclick='ccStartSession()' ${CYCLE_STATE.sessionActive?"disabled":""}>Mulai Cycle Count</button></div></div>${CYCLE_HISTORY_REMOTE.error?`<div class='state' style='color:#b91c1c;'>${esc(CYCLE_HISTORY_REMOTE.error)}</div>`:""}${CYCLE_STATE.sessionActive?`<div class='mv-toolbar'><input id='ccSearch' class='search-lg' placeholder='Cari SKU / nama barang / lokasi' value='${esc(CYCLE_STATE.searchInput)}'></div>${candidates.length?`<div class='table-wrap'><table><thead><tr><th>Lokasi</th><th>SKU</th><th>Nama Barang</th><th>Aksi</th></tr></thead><tbody>${candidates.map(c=>`<tr><td>${esc(c.lokasi)}</td><td>${esc(c.sku)}</td><td>${esc(c.nama)}</td><td><button class='btn-ghost' onclick="ccAddItem('${encAttr(c.sku)}','${encAttr(c.lokasi)}')">Tambah</button></td></tr>`).join('')}</tbody></table></div>`:"<div class='state'>Cari SKU untuk menambah item.</div>"}<h4 style='margin-top:12px;'>Cycle Count Berjalan</h4><div class='table-wrap'><table><thead><tr><th>Lokasi</th><th>SKU</th><th>Nama Barang</th><th>Stok Bulky</th><th>Stok Retail</th><th>Aktual Bulky</th><th>Aktual Retail</th><th>Catatan</th><th>Selisih Bulky</th><th>Selisih Retail</th><th>Status</th><th>Aksi</th></tr></thead><tbody>${CYCLE_STATE.sessionItems.map((r,i)=>`<tr><td>${esc(r.lokasi)}</td><td>${esc(r.sku)}</td><td>${esc(r.nama)}</td><td>${r.bulky}</td><td>${r.retail}</td><td><input type='number' data-cc='ab' data-idx='${i}' value='${r.aktualBulky??""}'></td><td><input type='number' data-cc='ar' data-idx='${i}' value='${r.aktualRetail??""}'></td><td><input data-cc='ct' data-idx='${i}' value='${esc(r.catatan||"")}'></td><td>${(Number(r.aktualBulky)-Number(r.bulky))||0}</td><td>${(Number(r.aktualRetail)-Number(r.retail))||0}</td><td>${cycleDiffBadge(r)}</td><td><button class='btn-ghost' onclick='ccRemoveItem(${i})'>Hapus</button></td></tr>`).join('')||"<tr><td colspan='12'><div class='state'>Belum ada item dalam session.</div></td></tr>"}</tbody></table></div><div class='row'><button class='btn-primary' onclick='ccSubmitSession()' ${submitDisabled?"disabled":""}>${CYCLE_STATE.submitting?"Menyimpan...":"Selesai Cycle Count"}</button></div>`:""}</div><div class='card cc-card' style='margin-top:12px;'><div class='section-header'><h4>History Cycle Count</h4></div><div class='table-wrap'><table><thead><tr><th>Tanggal</th><th>Lokasi</th><th>SKU</th><th>Nama Barang</th><th>Bulky</th><th>Retail</th><th>Aktual Bulky</th><th>Aktual Retail</th><th>Catatan</th><th>Status</th></tr></thead><tbody>${history.map(r=>`<tr><td>${esc(r.tanggal||"-")}</td><td>${esc(r.lokasi)}</td><td>${esc(r.sku)}</td><td>${esc(r.nama)}</td><td>${r.bulky}</td><td>${r.retail}</td><td>${r.aktualBulky}</td><td>${r.aktualRetail}</td><td>${esc(r.catatan||"-")}</td><td>${cycleDiffBadge(r)}</td></tr>`).join('')||"<tr><td colspan='10'><div class='state'>Belum ada history cycle count.</div></td></tr>"}</tbody></table></div></div>`;}
...
window.ccAddItem=(skuEnc,lokEnc)=>{const sku=decodeURIComponent(skuEnc),lokasi=decodeURIComponent(lokEnc);const source=getCycleSourceRows().find(r=>clean(r.sku)===clean(sku)&&clean(r.lokasi)===clean(lokasi));if(!source)return;const newItem={...source,aktualBulky:null,aktualRetail:null,catatan:""};const key=cycleItemKey(newItem);if(CYCLE_STATE.sessionItems.some(it=>cycleItemKey(it)===key)){toast('SKU/lokasi sudah ada di session.','error');return;}CYCLE_STATE.sessionItems.push(newItem);renderCycleCountPage();};
window.ccRemoveItem=(idx)=>{CYCLE_STATE.sessionItems.splice(idx,1);renderCycleCountPage();};
window.ccSubmitSession=async()=>{if(!canSubmitCycle()){toast('Lengkapi aktual bulky dan aktual retail semua item.','error');return;}CYCLE_STATE.submitting=true;renderCycleCountPage();try{const tanggal=new Date().toISOString().slice(0,10);const payload={tanggal,items:CYCLE_STATE.sessionItems.map(it=>({lokasi:it.lokasi,sku:it.sku,nama_barang:it.nama,bulky:Number(it.bulky)||0,retail:Number(it.retail)||0,aktual_bulky:Number(it.aktualBulky),aktual_retail:Number(it.aktualRetail),catatan:it.catatan||""}))};const res=await fetch('/api/cycle-count',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal menyimpan cycle count');toast('Cycle count berhasil disimpan','success');CYCLE_STATE.sessionActive=false;CYCLE_STATE.searchInput="";CYCLE_STATE.search="";CYCLE_STATE.sessionItems=[];await ensureCycleHistoryLoaded();CYCLE_HISTORY_REMOTE.loaded=false;await ensureCycleHistoryLoaded();if(typeof syncData==='function')syncData();}catch(err){toast(err?.message||'Gagal menyimpan cycle count','error');}finally{CYCLE_STATE.submitting=false;renderCycleCountPage();}};
document.addEventListener('input',e=>{if(e.target?.id==='ccSearch'){CYCLE_STATE.searchInput=e.target.value;clearTimeout(CYCLE_STATE.searchTimer);CYCLE_STATE.searchTimer=setTimeout(()=>{CYCLE_STATE.search=CYCLE_STATE.searchInput;renderCycleCountPage();},250);return;}if(e.target?.dataset?.cc){const idx=Number(e.target.dataset.idx);const row=CYCLE_STATE.sessionItems[idx];if(!row)return;if(e.target.dataset.cc==='ab'){const n=Number(e.target.value);row.aktualBulky=Number.isFinite(n)?n:null;}if(e.target.dataset.cc==='ar'){const n=Number(e.target.value);row.aktualRetail=Number.isFinite(n)?n:null;}if(e.target.dataset.cc==='ct')row.catatan=e.target.value||"";renderCycleCountPage();}});
