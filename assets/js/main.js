...
const CYCLE_KEY="cycle_count_draft_v1";
const CYCLE_STATE={session:null,rows:[],step:1,query:"",page:1,pageSize:12,filters:{lokasi:"all",kategori:"all"},locked:false,lastToast:0};
function todayISO(){const d=new Date();return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;}
function createCycleSession(payload={}){CYCLE_STATE.session={id:Date.now(),nama:payload.nama||"",tanggal:payload.tanggal||todayISO(),pic:payload.pic||"",lokasi:payload.lokasi||"",catatan:payload.catatan||"",status:"draft"};CYCLE_STATE.locked=false;saveCycleDraft(true);}
function collectCycleCandidates(){const rows=[];for(const sheet of ["RPL","BULKY","Kartu Stock"]){for(const r of (DATA[sheet]||[])){const sku=getVal(r,["sku"]);if(!clean(sku))continue;rows.push({sku:String(sku).trim(),nama:getVal(r,["nama barang","nama","item","description"])||"-",lokasi:getVal(r,["lokasi","location","rak","bin","area"])||"-",kategori:sheet,stokSistem:parseNumber(getVal(r,["stok global","stok akhir","closing stock","qty","stok"]))||0});}}
return rows;}
function addSkuToSession(items=[]){const keyed=new Map(CYCLE_STATE.rows.map(r=>[clean(r.sku),r]));for(const it of items){if(!clean(it?.sku)||keyed.has(clean(it.sku)))continue;const row={...it,qtyAktual:0,selisih:0,status:"Cocok",catatan:""};keyed.set(clean(row.sku),row);}CYCLE_STATE.rows=[...keyed.values()];saveCycleDraft();}
function calculateDifference(stokSistem,qtyAktual){const diff=(Number(qtyAktual)||0)-(Number(stokSistem)||0);const status=diff===0?"Cocok":diff>0?"Plus":"Minus";return {diff,status};}
function updateActualQty(sku,qty){const row=CYCLE_STATE.rows.find(r=>clean(r.sku)===clean(sku));if(!row)return;const calc=calculateDifference(row.stokSistem,qty);row.qtyAktual=Number(qty)||0;row.selisih=calc.diff;row.status=calc.status;saveCycleDraft();}
function getCycleSummary(){const total=CYCLE_STATE.rows.length;const cocok=CYCLE_STATE.rows.filter(r=>r.status==="Cocok").length;const selisih=total-cocok;const totalSelisih=CYCLE_STATE.rows.reduce((n,r)=>n+r.selisih,0);const accuracy=total?((cocok/total)*100):0;return{total,cocok,selisih,totalSelisih,accuracy};}
function finalizeSession(){if(!CYCLE_STATE.session)return;CYCLE_STATE.session.status="selesai";CYCLE_STATE.locked=true;saveCycleDraft(true);toast("Cycle count selesai");renderCycleCountPage();}
function exportCycleCountCsv(){const header=["SKU","Nama Barang","Lokasi","Kategori","Stok Sistem","Qty Aktual","Selisih","Status","Catatan"];const lines=[header.join(",")].concat(CYCLE_STATE.rows.map(r=>[r.sku,r.nama,r.lokasi,r.kategori,r.stokSistem,r.qtyAktual,r.selisih,r.status,(r.catatan||"").replace(/,/g," ")].join(",")));const blob=new Blob([lines.join("\n")],{type:"text/csv;charset=utf-8;"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`cycle-count-${(CYCLE_STATE.session?.tanggal||todayISO())}.csv`;a.click();URL.revokeObjectURL(a.href);}
function saveCycleDraft(forceToast=false){localStorage.setItem(CYCLE_KEY,JSON.stringify({session:CYCLE_STATE.session,rows:CYCLE_STATE.rows,step:CYCLE_STATE.step,locked:CYCLE_STATE.locked}));if(forceToast||Date.now()-CYCLE_STATE.lastToast>3000){toast("Draft cycle count tersimpan");CYCLE_STATE.lastToast=Date.now();}}
function loadCycleDraft(){try{const raw=JSON.parse(localStorage.getItem(CYCLE_KEY)||"null");if(!raw)return;CYCLE_STATE.session=raw.session||null;CYCLE_STATE.rows=Array.isArray(raw.rows)?raw.rows:[];CYCLE_STATE.step=raw.step||1;CYCLE_STATE.locked=!!raw.locked;}catch(_){}}
const cycleSearchDebounced=debounce(v=>{CYCLE_STATE.query=v||"";CYCLE_STATE.page=1;renderCycleCountPage();},220);
function renderCycleCountPage(){if(!CYCLE_STATE.session&&!CYCLE_STATE.rows.length)loadCycleDraft();const summary=getCycleSummary();const status=CYCLE_STATE.session?.status||"draft";cycleHeader.innerHTML=`<div class='cc-header'><div><h3 class='page-title'>Cycle Count</h3><p class='subtitle'>Hitung stok fisik dan validasi terhadap stok sistem.</p></div><span class='badge ${status==="selesai"?"ok":"warn"}'>${status}</span></div>`;
cycleSummary.innerHTML=`<div class='cc-summary-grid'>${[["Total SKU",summary.total],["SKU Cocok",summary.cocok],["SKU Selisih",summary.selisih],["Total Selisih",summary.totalSelisih],["Akurasi",`${summary.accuracy.toFixed(1)}%`]].map(x=>`<div class='summary-card'><div class='k'>${x[0]}</div><div class='v'>${x[1]}</div></div>`).join("")}</div>`;
const candidates=collectCycleCandidates();const filtered=candidates.filter(r=>clean(`${r.sku} ${r.nama}`).includes(clean(CYCLE_STATE.query||""))).slice(0,80);
const maxPage=Math.max(1,Math.ceil(CYCLE_STATE.rows.length/CYCLE_STATE.pageSize));CYCLE_STATE.page=Math.min(CYCLE_STATE.page,maxPage);const paged=CYCLE_STATE.rows.slice((CYCLE_STATE.page-1)*CYCLE_STATE.pageSize,CYCLE_STATE.page*CYCLE_STATE.pageSize);
cycleStep.innerHTML=`<div class='cc-stepper'><button class='${CYCLE_STATE.step===1?"active":""}'>1.Setup</button><button class='${CYCLE_STATE.step===2?"active":""}'>2.Pilih SKU</button><button class='${CYCLE_STATE.step===3?"active":""}'>3.Hitung Aktual</button><button class='${CYCLE_STATE.step===4?"active":""}'>4.Review</button></div>
<div class='card cc-card'>${CYCLE_STATE.step===1?`<div class='cc-grid'><input id='ccNama' placeholder='Nama sesi' value='${esc(CYCLE_STATE.session?.nama||"")}'/><input id='ccTanggal' type='date' value='${esc(CYCLE_STATE.session?.tanggal||todayISO())}'/><input id='ccPic' placeholder='PIC' value='${esc(CYCLE_STATE.session?.pic||"")}'/><input id='ccLokasi' placeholder='Lokasi (opsional)' value='${esc(CYCLE_STATE.session?.lokasi||"")}'/><textarea id='ccCatatan' placeholder='Catatan'>${esc(CYCLE_STATE.session?.catatan||"")}</textarea></div><div class='row'><button class='btn-primary' data-cc='save-setup'>Simpan Draft</button><button class='btn-ghost' data-cc='next'>Lanjut</button></div>`:""}
${CYCLE_STATE.step===2?`<div class='cc-toolbar'><input id='ccSearch' placeholder='Search SKU / nama barang'/><button class='btn-ghost' data-cc='take-minus'>Ambil stok minus</button><button class='btn-ghost' data-cc='take-anomaly'>Ambil anomaly</button></div><div class='cc-list'>${filtered.map(r=>`<button data-cc='add-sku' data-sku='${encAttr(r.sku)}' class='cc-item'>${esc(r.sku)} - ${esc(r.nama)} <small>${esc(r.lokasi)} · ${esc(r.kategori)}</small></button>`).join("")||"<div class='state'>Tidak ada SKU</div>"}</div><div class='row'><button class='btn-primary' data-cc='next'>Lanjut</button></div>`:""}
${CYCLE_STATE.step===3?`${!CYCLE_STATE.rows.length?"<div class='state'>Belum ada SKU dipilih.</div>":`<div class='table-wrap cc-table'><table><thead><tr><th>SKU</th><th>Nama</th><th>Lokasi</th><th>Sistem</th><th>Aktual</th><th>Selisih</th><th>Status</th><th>Catatan</th></tr></thead><tbody>${paged.map(r=>`<tr class='${r.selisih!==0?"cc-row-soft":""}'><td>${esc(r.sku)}</td><td>${esc(r.nama)}</td><td>${esc(r.lokasi)}</td><td>${r.stokSistem}</td><td><input class='cc-qty' type='number' value='${r.qtyAktual||0}' data-cc='qty' data-sku='${encAttr(r.sku)}'/></td><td>${r.selisih}</td><td><span class='badge ${r.status==="Cocok"?"ok":r.status==="Plus"?"warn":"err"}'>${r.status}</span></td><td><input value='${esc(r.catatan||"")}' data-cc='note' data-sku='${encAttr(r.sku)}'/></td></tr>`).join("")}</tbody></table></div><div class='mv-pagination'><button class='btn-ghost' data-cc='prev'>Prev</button><span>Hal ${CYCLE_STATE.page}/${maxPage}</span><button class='btn-ghost' data-cc='next-page'>Next</button></div>`}<div class='row'><button class='btn-primary' data-cc='next'>Lanjut</button></div>`:""}
${CYCLE_STATE.step===4?`<div class='cc-review'><p>Total SKU dihitung: <strong>${summary.total}</strong></p><p>SKU cocok: <strong>${summary.cocok}</strong></p><p>SKU selisih: <strong>${summary.selisih}</strong></p><p>Total selisih qty: <strong>${summary.totalSelisih}</strong></p><p>Akurasi: <strong>${summary.accuracy.toFixed(2)}%</strong></p></div><div class='row'><button class='btn-primary' data-cc='final'>Selesaikan Cycle Count</button><button class='btn-ghost' data-cc='export'>Export CSV</button></div>`:""}
</div>`;
if(CYCLE_STATE.step===2){document.getElementById("ccSearch")?.addEventListener("input",e=>cycleSearchDebounced(e.target.value));}
}

document.addEventListener("click",e=>{const btn=e.target.closest("[data-cc]");if(!btn)return;const action=btn.dataset.cc;if(action==="save-setup"){createCycleSession({nama:document.getElementById("ccNama")?.value,tanggal:document.getElementById("ccTanggal")?.value,pic:document.getElementById("ccPic")?.value,lokasi:document.getElementById("ccLokasi")?.value,catatan:document.getElementById("ccCatatan")?.value});return renderCycleCountPage();}
if(action==="next"){CYCLE_STATE.step=Math.min(4,CYCLE_STATE.step+1);return renderCycleCountPage();}
if(action==="add-sku"){const sku=decodeURIComponent(btn.dataset.sku||"");const hit=collectCycleCandidates().find(x=>clean(x.sku)===clean(sku));if(hit)addSkuToSession([hit]);return renderCycleCountPage();}
if(action==="take-minus"){const rows=(window.__stokMinusRows||[]).map(r=>({sku:r.sku,nama:r.nama,lokasi:r.lokasi||"-",kategori:"Stok Minus",stokSistem:parseNumber(r.stok)||0}));addSkuToSession(rows);return renderCycleCountPage();}
if(action==="take-anomaly"){const rows=(window.__anomalyRows||[]).map(r=>({sku:r.sku,nama:r.nama,lokasi:r.lokasi||"-",kategori:"Anomaly",stokSistem:parseNumber(r.stokGlobal||r.stok)||0}));addSkuToSession(rows);return renderCycleCountPage();}
if(action==="prev"){CYCLE_STATE.page=Math.max(1,CYCLE_STATE.page-1);return renderCycleCountPage();}
if(action==="next-page"){CYCLE_STATE.page++;return renderCycleCountPage();}
if(action==="final")return finalizeSession();
if(action==="export")return exportCycleCountCsv();
});
document.addEventListener("input",e=>{if(e.target?.dataset?.cc==="qty")updateActualQty(decodeURIComponent(e.target.dataset.sku||""),e.target.value);if(e.target?.dataset?.cc==="note"){const row=CYCLE_STATE.rows.find(r=>clean(r.sku)===clean(decodeURIComponent(e.target.dataset.sku||"")));if(row){row.catatan=e.target.value||"";saveCycleDraft();}}});
