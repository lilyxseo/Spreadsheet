...
const BALIKAN_STATE={sheets:[],sheetsLoaded:false,sheetCache:{},sheetChecksums:{},dynamicColumnCache:{},sheetSelect:{selectedSheet:"",searchKeyword:"",isLoading:false,isOpen:false},allTripLoading:false,lastRefreshAt:0,highlightRowNumber:null,highlightSheetName:"",sortBy:"default",autoCheckOnScan:true,exactScanSku:"",selectedSkuRowNumber:null,selectedSkuSheetName:"",selectedSkuValue:"",lastCheckedRowId:null,lastCheckedSheetName:"",lastCheckedSku:"",lastCheckedVersion:0,lastCheckedFadeTimer:null,pendingEdits:{},pendingEditMeta:{},saveStatus:{},saveTimer:null,saveInProgress:false,saveRequested:false,isRendering:false,isRefreshing:false,pendingRender:false,lastRenderedChecksum:"",lastDataChecksum:"",lastRenderedHeaderKey:"",renderTimer:null,searchDebounceTimer:null};
...
balikanSheetSelect?.addEventListener("change",async(e)=>{const selected=e.target.value||"";BALIKAN_STATE.sheetSelect.selectedSheet=selected;window.currentTripSheet=selected;BALIKAN_STATE.highlightRowNumber=null;BALIKAN_STATE.highlightSheetName="";BALIKAN_STATE.selectedSkuRowNumber=null;BALIKAN_STATE.selectedSkuSheetName="";BALIKAN_STATE.selectedSkuValue="";await loadBalikanRows();});
...
function getBalikanSheetOptions(){return [{label:'📄 Semua sheet TRIP',value:''},...BALIKAN_STATE.sheets.map(name=>{const count=Array.isArray(BALIKAN_STATE.sheetCache[name])?BALIKAN_STATE.sheetCache[name].length:null;return {label:`📄 ${name}${count===null?'':` (${count})`}`,value:name,description:count===null?'Jumlah data belum tersedia':`${count} data`};})];}
function syncBalikanSheetOptions(){if(!balikanSheetSelect||typeof balikanSheetSelect.setOptions!=='function')return;const selected=BALIKAN_STATE.sheetSelect.selectedSheet||window.currentTripSheet||'';balikanSheetSelect.setOptions(getBalikanSheetOptions());balikanSheetSelect.setValue(selected);}
function initBalikanSheetSelect(){if(!balikanSheetSelect||balikanSheetSelect.dataset.virtualSelectReady==='1'||!window.VirtualSelect)return;window.VirtualSelect.init({ele:'#balikanSheetSelect',options:[],search:true,searchDelay:250,placeholder:'Pilih Sheet...',searchPlaceholderText:'Cari nama sheet...',noOptionsText:'Tidak ada sheet ditemukan.',noSearchResultsText:'Tidak ada sheet ditemukan.',showDropboxAsPopup:false,hasOptionDescription:true,optionHeight:'48px',optionsCount:8,disableSelectAll:true,allowNewOption:false,hideClearButton:false});balikanSheetSelect.dataset.virtualSelectReady='1';balikanSheetSelect.addEventListener('afterOpen',()=>{BALIKAN_STATE.sheetSelect.isOpen=true;});balikanSheetSelect.addEventListener('afterClose',()=>{BALIKAN_STATE.sheetSelect.isOpen=false;BALIKAN_STATE.sheetSelect.searchKeyword='';});balikanSheetSelect.addEventListener('input',e=>{if(e.target?.matches?.('.vscomp-search-input'))BALIKAN_STATE.sheetSelect.searchKeyword=e.target.value||'';});}
function setBalikanSheetLoading(isLoading){BALIKAN_STATE.sheetSelect.isLoading=isLoading===true;balikanSheetSelect?.setAttribute('aria-busy',String(BALIKAN_STATE.sheetSelect.isLoading));document.getElementById('balikanSheetSkeleton')?.toggleAttribute('hidden',!BALIKAN_STATE.sheetSelect.isLoading);if(typeof balikanSheetSelect?.disable==='function'&&isLoading)balikanSheetSelect.disable();if(typeof balikanSheetSelect?.enable==='function'&&!isLoading)balikanSheetSelect.enable();}
async function loadBalikanSheets(){if(BALIKAN_STATE.sheetsLoaded||BALIKAN_STATE.sheetSelect.isLoading)return;initBalikanSheetSelect();setBalikanSheetLoading(true);try{const res=await fetch('/api/balikan-store/sheets');const data=await res.json();if(!res.ok)throw new Error(data?.message||'Gagal memuat daftar sheet');BALIKAN_STATE.sheets=Array.isArray(data?.sheets)?data.sheets.filter(n=>String(n||'').toUpperCase().includes('TRIP')):[];BALIKAN_STATE.sheetsLoaded=true;syncBalikanSheetOptions();if(!window.currentTripSheet)loadAllBalikanTripRows({background:true,force:true});}catch(err){toast(err?.message||'Gagal memuat daftar sheet','error');}finally{setBalikanSheetLoading(false);}}
...
async function loadAllBalikanTripRows(options={}){const {background=false,force=false}=options||{};if(BALIKAN_STATE.allTripLoading)return;if(!BALIKAN_STATE.sheets.length){if(!background){balikanSummary.textContent='';balikanTable.innerHTML='<div class="subtitle">Tidak ada sheet TRIP.</div>';}return;}BALIKAN_STATE.allTripLoading=true;if(!background){balikanSummary.textContent='Memuat index semua sheet TRIP...';}try{const results=await Promise.allSettled(BALIKAN_STATE.sheets.map(sheet=>fetchBalikanSheetRows(sheet,{force})));const failed=results.filter(r=>r.status==='rejected');if(failed.length&&!background)toast(`${failed.length} sheet TRIP gagal dimuat`,'error');const allRows=BALIKAN_STATE.sheets.flatMap(sheet=>BALIKAN_STATE.sheetCache[sheet]||[]);setBalikanDisplayRows(allRows,mergeBalikanDynamicColumns(BALIKAN_STATE.sheets));syncBalikanSheetOptions();}catch(err){if(!background)toast(err?.message||'Gagal memuat semua sheet TRIP','error');}finally{BALIKAN_STATE.allTripLoading=false;}}
