...
if(!appInitialized){
bindNav();bindEvents();bindLogoutButtons();setupSidebar();syncDeveloperMenuVisibility();renderFilters();applyRoleBasedUi();setMainContentLoading(true);document.getElementById("sheetInfo").textContent=SHEETS.join(", ");document.getElementById("spreadsheetInfo").textContent=SPREADSHEET_ID;renderRecentHistory();routeFromPath(location.pathname);window.addEventListener("popstate",()=>routeFromPath(location.pathname));appInitialized=true;
}else{
syncDeveloperMenuVisibility();
}
...
const ASSET_STORE_STATE={sheetList:[],selectedSheet:"",cache:{},loadingList:false,loadingData:false,listError:"",dataError:"",searchInput:"",search:"",searchTimer:null,sortColumn:"",sortDirection:"asc",columnFilter:"all",page:1,pageSize:25,lastLoadedAt:{}};
function bindAssetStoreEvents(){document.addEventListener("change",e=>{if(e.target?.id==="assetStoreSheetSelect")selectAssetStoreSheet(e.target.value);if(e.target?.id==="assetStoreSortColumn"){ASSET_STORE_STATE.sortColumn=e.target.value;ASSET_STORE_STATE.page=1;renderAssetStorePage();}if(e.target?.id==="assetStoreSortDir"){ASSET_STORE_STATE.sortDirection=e.target.value;ASSET_STORE_STATE.page=1;renderAssetStorePage();}if(e.target?.id==="assetStoreColumnFilter"){ASSET_STORE_STATE.columnFilter=e.target.value;ASSET_STORE_STATE.page=1;renderAssetStorePage();}});document.addEventListener("input",e=>{if(e.target?.id!=="assetStoreSearch")return;ASSET_STORE_STATE.searchInput=e.target.value||"";clearTimeout(ASSET_STORE_STATE.searchTimer);ASSET_STORE_STATE.searchTimer=setTimeout(()=>{ASSET_STORE_STATE.search=ASSET_STORE_STATE.searchInput;ASSET_STORE_STATE.page=1;renderAssetStorePage();},250);});document.addEventListener("click",e=>{if(e.target?.closest("#assetStoreRefreshBtn"))refreshAssetStore();const pg=e.target?.closest("[data-asset-store-page]");if(pg){ASSET_STORE_STATE.page=Math.max(1,ASSET_STORE_STATE.page+(Number(pg.dataset.assetStorePage)||0));renderAssetStorePage();}});}
async function ensureAssetStoreList(){if(ASSET_STORE_STATE.sheetList.length||ASSET_STORE_STATE.loadingList)return;ASSET_STORE_STATE.loadingList=true;ASSET_STORE_STATE.listError="";renderAssetStorePage();try{const archiveSheets=await fetchArchiveSheetList();ASSET_STORE_STATE.sheetList=(archiveSheets||[]).filter(s=>clean(String(s?.name||s?.key||"" )).includes("asset"));}catch(err){ASSET_STORE_STATE.listError=err.message||"Gagal memuat daftar sheet";}finally{ASSET_STORE_STATE.loadingList=false;renderAssetStorePage();}}
async function selectAssetStoreSheet(sheetName){ASSET_STORE_STATE.selectedSheet=sheetName||"";ASSET_STORE_STATE.page=1;ASSET_STORE_STATE.dataError="";if(!sheetName){renderAssetStorePage();return;}if(ASSET_STORE_STATE.cache[sheetName]){renderAssetStorePage();return;}ASSET_STORE_STATE.loadingData=true;renderAssetStorePage();try{ASSET_STORE_STATE.cache[sheetName]=await fetchArchiveSheetData(sheetName,ASSET_STORE_STATE.sheetList);ASSET_STORE_STATE.lastLoadedAt[sheetName]=new Date().toISOString();}catch(err){ASSET_STORE_STATE.dataError=err.message||"Gagal memuat data sheet";}finally{ASSET_STORE_STATE.loadingData=false;renderAssetStorePage();}}
async function refreshAssetStore(){const sheet=ASSET_STORE_STATE.selectedSheet;if(!sheet){ASSET_STORE_STATE.sheetList=[];await ensureAssetStoreList();return;}delete ASSET_STORE_STATE.cache[sheet];ASSET_STORE_STATE.loadingData=true;ASSET_STORE_STATE.dataError="";renderAssetStorePage();try{ASSET_STORE_STATE.cache[sheet]=await fetchArchiveSheetData(sheet,ASSET_STORE_STATE.sheetList);ASSET_STORE_STATE.lastLoadedAt[sheet]=new Date().toISOString();}catch(err){ASSET_STORE_STATE.dataError=err.message||"Gagal refresh data";}finally{ASSET_STORE_STATE.loadingData=false;renderAssetStorePage();}}
function getAssetStoreRows(){const parsed=ASSET_STORE_STATE.cache[ASSET_STORE_STATE.selectedSheet]||{rows:[],columns:[],headerError:false};const cols=parsed.columns||[];const filtered=(parsed.rows||[]);const q=clean(ASSET_STORE_STATE.search);let out=filtered.filter(r=>!q||cols.some(c=>clean(String(r[c]??"")).includes(q)));if(ASSET_STORE_STATE.columnFilter!=="all"&&q)out=out.filter(r=>clean(String(r[ASSET_STORE_STATE.columnFilter]??"")).includes(q));const col=ASSET_STORE_STATE.sortColumn||cols[0]||"";if(col)out=[...out].sort((a,b)=>{const av=a[col],bv=b[col];const an=Number(av),bn=Number(bv),numA=Number.isFinite(an),numB=Number.isFinite(bn);let cmp=0;if(numA&&numB)cmp=an-bn;else cmp=String(av??"").localeCompare(String(bv??""),"id");return ASSET_STORE_STATE.sortDirection==="desc"?-cmp:cmp;});return {rows:out,columns:cols,headerError:!!parsed.headerError,totalSource:filtered.length};}
function renderAssetStorePage(){if(!assetStoreApp)return;if(!ASSET_STORE_STATE.sheetList.length&&!ASSET_STORE_STATE.loadingList)ensureAssetStoreList();const selected=ASSET_STORE_STATE.selectedSheet;const parsed=ASSET_STORE_STATE.cache[selected]||{rows:[],columns:[],headerError:false};const {rows,columns,headerError,totalSource}=selected?getAssetStoreRows():{rows:[],columns:[],headerError:false,totalSource:0};const totalPage=Math.max(1,Math.ceil(rows.length/ASSET_STORE_STATE.pageSize));if(ASSET_STORE_STATE.page>totalPage)ASSET_STORE_STATE.page=totalPage;const start=(ASSET_STORE_STATE.page-1)*ASSET_STORE_STATE.pageSize;const pageRows=rows.slice(start,start+ASSET_STORE_STATE.pageSize);const activeSheetName=(ASSET_STORE_STATE.sheetList.find(s=>s.key===selected)||{}).name||selected||"-";const lastLoaded=selected&&ASSET_STORE_STATE.lastLoadedAt[selected]?new Date(ASSET_STORE_STATE.lastLoadedAt[selected]).toLocaleString("id-ID"):"-";const sortOps=columns.map(c=>`<option value='${esc(c)}' ${ASSET_STORE_STATE.sortColumn===c?"selected":""}>${esc(c)}</option>`).join("");assetStoreApp.innerHTML=`<div class='archive-layout'><div class='card archive-section'><div class='section-header'><h3 class='page-title'>Asset Store</h3><div class='row'><select id='assetStoreSheetSelect'><option value=''>Pilih Sheet Arsip</option>${ASSET_STORE_STATE.sheetList.map(s=>`<option value='${esc(s.key)}' ${s.key===selected?"selected":""}>${esc(s.name)}</option>`).join("")}</select><button id='assetStoreRefreshBtn' class='btn-ghost'>Refresh Asset Store</button></div></div>${ASSET_STORE_STATE.loadingList?"<div class='state'>Memuat daftar sheet arsip...</div>":ASSET_STORE_STATE.listError?`<div class='state'>${esc(ASSET_STORE_STATE.listError)}</div>`:""}</div><div class='card archive-section archive-filter-card'><div class='mv-filters open archive-filters'><input id='assetStoreSearch' class='search-lg' placeholder='Cari di semua kolom' value='${esc(ASSET_STORE_STATE.searchInput)}'/><select id='assetStoreSortColumn'><option value=''>Urutkan Kolom</option>${sortOps}</select><select id='assetStoreSortDir'><option value='asc' ${ASSET_STORE_STATE.sortDirection==='asc'?'selected':''}>ASC</option><option value='desc' ${ASSET_STORE_STATE.sortDirection==='desc'?'selected':''}>DESC</option></select><select id='assetStoreColumnFilter'><option value='all'>Semua Kolom</option>${columns.map(c=>`<option value='${esc(c)}' ${ASSET_STORE_STATE.columnFilter===c?'selected':''}>Filter: ${esc(c)}</option>`).join('')}</select></div></div>${!selected?"<div class='card archive-section'><div class='state'>Pilih sheet arsip untuk melihat data</div></div>":ASSET_STORE_STATE.loadingData?"<div class='card archive-section'><div class='state'>Memuat data sheet...</div></div>":ASSET_STORE_STATE.dataError?`<div class='card archive-section'><div class='state'>${esc(ASSET_STORE_STATE.dataError)}</div></div>`:headerError?`<div class='card archive-section'><div class='state'>Header tidak terdeteksi</div></div>`:`<div class='card archive-section'><div class='grid dashboard archive-summary'><div class='metric'><div class='k'>Total Baris Asset</div><div class='v'>${totalSource}</div></div><div class='metric'><div class='k'>Total Kolom</div><div class='v'>${columns.length}</div></div><div class='metric'><div class='k'>Sheet Aktif</div><div class='v'>${esc(activeSheetName)}</div></div><div class='metric'><div class='k'>Terakhir Dimuat</div><div class='v'>${esc(lastLoaded)}</div></div></div></div><div class='card archive-section archive-table-card'>${!rows.length?"<div class='state'>Tidak ada data asset</div>":`<div class='table-wrap table-wrap-full archive-table-wrap'><table><thead><tr>${columns.map(c=>`<th>${esc(c)}</th>`).join("")}</tr></thead><tbody>${pageRows.map(r=>`<tr>${columns.map(c=>`<td>${esc(r[c]??"")}</td>`).join("")}</tr>`).join("")}</tbody></table></div><div class='mv-pagination'><span>Menampilkan ${rows.length?start+1:0}-${Math.min(start+ASSET_STORE_STATE.pageSize,rows.length)} dari ${rows.length}</span><div class='row'><button class='btn-ghost' data-asset-store-page='-1' ${ASSET_STORE_STATE.page<=1?'disabled':''}>Prev</button><button class='btn-ghost' data-asset-store-page='1' ${ASSET_STORE_STATE.page>=totalPage?'disabled':''}>Next</button></div></div>`}</div>`}</div>`;}
