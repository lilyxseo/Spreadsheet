...
function findMovementItemBySku(scannedSku){
  const normalized=clean(scannedSku);
  if(!normalized)return null;
  return getMovementSourceRows().find(it=>clean(it.sku)===normalized)||null;
}
function handleMovementBarcodeResult(scannedSku){
  const sku=cleanScannedSku(scannedSku);
  const item=findMovementItemBySku(sku);
  if(!item){showToast('SKU tidak ditemukan','error');return;}
  const inserted=addMovementItem({lokasi:item.lokasi,sku,nama:item.nama||item.namaBarang||item.name||'',stok_akhir:item.stok_akhir??item.stok??item.qty??''},{silentSuccess:true});
  if(inserted)showToast('SKU masuk list Movement','success');
}
function openMovementScanner(){
  if(!MOVEMENT_STATE.sessionActive){toast('Mulai Movement dulu sebelum scan.','error');return;}
  openBarcodeScanner('mvSearch',handleMovementBarcodeResult);
}
...
function renderMovementSession(){return `<div class='cc-section'><div class='mv-toolbar'><input id='mvSearch' class='search-lg' placeholder='Cari SKU / nama barang / lokasi' value='${esc(MOVEMENT_STATE.searchInput)}'><button id='movementOpenScannerBtn' class='icon-btn' title='Scan Barcode' aria-label='Scan Barcode'>📷</button></div></div><div class='cc-section'><div class='table-wrap cc-table-wrap cc-search-wrap'><table><thead><tr><th>Lokasi</th><th>SKU</th><th>Nama Barang</th><th>Qty</th><th>Aksi</th></tr></thead><tbody id='mvSearchResultsBody'></tbody></table></div></div><div class='cc-section'><h4>Movement Berjalan</h4><div class='table-wrap cc-table-wrap cc-session-wrap'><table><thead><tr><th>From</th><th>To</th><th>SKU</th><th>Nama Barang</th><th>Stok Lokasi Awal</th><th>Stok Aktual</th><th>Aksi</th></tr></thead><tbody id='mvSessionBody'></tbody></table></div></div>`;}
...
document.addEventListener('click',e=>{if(e.target?.id==='ccHistoryPrev'){cycleHistoryPage=Math.max(1,cycleHistoryPage-1);renderCycleHistory();return;}if(e.target?.id==='ccHistoryNext'){cycleHistoryPage+=1;renderCycleHistory();return;}if(e.target?.id==='movementOpenScannerBtn'){openMovementScanner();return;}if(e.target?.id==='mvHistoryPrev'){movementHistoryPage=Math.max(1,movementHistoryPage-1);renderMovementHistory();return;}if(e.target?.id==='mvHistoryNext'){movementHistoryPage+=1;renderMovementHistory();return;}const btn=e.target.closest('[data-mvm-action]');if(!btn)return;const action=btn.dataset.mvmAction;if(action==='remove'){const idx=Number(btn.dataset.idx);MOVEMENT_STATE.sessionItems.splice(idx,1);renderMovementSessionTable();return;}if(action==='add'){const sku=decodeURIComponent(btn.dataset.sku||"");const lokasi=decodeURIComponent(btn.dataset.lok||"");const source=getMovementSourceRows().find(r=>clean(r.sku)===clean(sku)&&clean(r.lokasi)===clean(lokasi));if(source)addMovementItem(source);}});
