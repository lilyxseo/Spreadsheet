...
function buildCycleHistoryRows(){return [...(CYCLE_HISTORY_REMOTE.rows||[])].sort((a,b)=>new Date(b.createdAt||b.tanggal||0)-new Date(a.createdAt||a.tanggal||0));}
...
window.ccCancelSession=()=>{if(!confirm('Batalkan cycle count berjalan?'))return;CYCLE_STATE.sessionActive=false;CYCLE_STATE.search="";CYCLE_STATE.searchInput="";CYCLE_STATE.sessionItems=[];CYCLE_STATE.submitting=false;renderCycleCountPage();};
...
function buildMovementHistoryRows(){return [...(MOVEMENT_HISTORY_REMOTE.rows||[])].sort((a,b)=>new Date(b.tanggal||0)-new Date(a.tanggal||0));}
...
function renderMovementHistory(){const tbody=document.querySelector('#mvHistoryBody');if(!tbody)return;const history=buildMovementHistoryRows();if(!history.length){tbody.innerHTML="<tr><td colspan='9'><div class='state'>Belum ada history movement.</div></td></tr>";return;}const frag=document.createDocumentFragment();history.forEach(r=>{const edit=HISTORY_EDIT_STATE.movement[r.rowNumber];const tr=document.createElement('tr');if(edit){tr.innerHTML=`<td><input data-mvh-edit='tanggal' data-row='${r.rowNumber}' value='${esc(edit.tanggal||"")}'></td><td><input data-mvh-edit='from' data-row='${r.rowNumber}' value='${esc(edit.from||"")}'></td><td><input data-mvh-edit='to' data-row='${r.rowNumber}' value='${esc(edit.to||"")}'></td><td><input data-mvh-edit='sku' data-row='${r.rowNumber}' value='${esc(edit.sku||"")}'></td><td><input data-mvh-edit='nama' data-row='${r.rowNumber}' value='${esc(edit.nama||"")}'></td><td><input type='number' data-mvh-edit='stok_lokasi_awal' data-row='${r.rowNumber}' value='${esc(edit.stok_lokasi_awal)}'></td><td><input type='number' data-mvh-edit='stok_aktual' data-row='${r.rowNumber}' value='${esc(edit.stok_aktual)}'></td><td><input data-mvh-edit='keterangan' data-row='${r.rowNumber}' value='${esc(edit.keterangan||"")}'></td><td><button class='btn-ghost' data-mvh-action='save' data-row='${r.rowNumber}'>Simpan</button> <button class='btn-ghost' data-mvh-action='cancel' data-row='${r.rowNumber}'>Batal</button></td>`;}else{tr.innerHTML=`<td>${esc(r.tanggal||"-")}</td><td>${esc(r.from||"-")}</td><td>${esc(r.to||"-")}</td><td>${esc(r.sku||"-")}</td><td>${esc(r.nama||"-")}</td><td>${esc(r.stok_lokasi_awal)}</td><td>${esc(r.stok_aktual)}</td><td>${esc(r.keterangan||"-")}</td><td><button class='btn-ghost' data-mvh-action='edit' data-row='${r.rowNumber}'>Edit</button> <button class='btn-ghost' data-mvh-action='delete' data-row='${r.rowNumber}'>Hapus</button></td>`;}frag.appendChild(tr);});tbody.replaceChildren(frag);}
function renderMovementPage(){if(!movementApp)return;movementApp.innerHTML=`<div class='card cc-card cc-section'><div class='section-header cc-main-header'><div class='cc-action-stack'>${MOVEMENT_STATE.sessionActive?"<button class='btn-ghost' onclick='mvCancelSession()'>Batal Movement</button>":""}<button class='btn-primary' onclick='mvStartSession()' ${MOVEMENT_STATE.sessionActive?"disabled":""}>Mulai Movement</button>${MOVEMENT_STATE.sessionActive?"<button id='mvSubmitBtn' class='btn-primary' onclick='mvSubmitSession()'>Selesai Movement</button>":""}</div></div>${MOVEMENT_HISTORY_REMOTE.error?`<div class='state cc-state state-error'>${esc(MOVEMENT_HISTORY_REMOTE.error)}</div>`:""}${MOVEMENT_STATE.sessionActive?`<div class='cc-section'><div class='mv-toolbar'><input id='mvSearch' class='search-lg' placeholder='Cari SKU / nama barang / lokasi' value='${esc(MOVEMENT_STATE.searchInput)}'></div></div><div class='cc-section'><div class='table-wrap cc-table-wrap cc-search-wrap'><table><thead><tr><th>Lokasi</th><th>SKU</th><th>Nama Barang</th><th>Qty</th><th>Aksi</th></tr></thead><tbody id='mvSearchResultsBody'></tbody></table></div></div><div class='cc-section'><h4>Movement Berjalan</h4><div class='table-wrap cc-table-wrap cc-session-wrap'><table><thead><tr><th>From</th><th>To</th><th>SKU</th><th>Nama Barang</th><th>Stok Lokasi Awal</th><th>Stok Aktual</th><th>Aksi</th></tr></thead><tbody id='mvSessionBody'></tbody></table></div></div>`:`<div class='cc-empty-state'><i data-lucide='arrow-right-left'></i><h4>Belum ada movement berjalan</h4><p>Klik Mulai Movement untuk memilih SKU dan input stok aktual.</p></div>`}</div><div class='card cc-card cc-section'><div class='section-header'><h4>History Movement</h4></div><div class='table-wrap cc-table-wrap'><table><thead><tr><th>Tanggal</th><th>From</th><th>To</th><th>SKU</th><th>Nama Barang</th><th>Stok Lokasi Awal</th><th>Stok Aktual</th><th>Keterangan</th><th>Aksi</th></tr></thead><tbody id='mvHistoryBody'></tbody></table></div></div>`;if(window.lucide)window.lucide.createIcons();if(MOVEMENT_STATE.sessionActive){renderMovementSearchResults();renderMovementSessionTable();}renderMovementHistory();}
...
window.mvCancelSession=()=>{if(!confirm('Batalkan movement berjalan?'))return;MOVEMENT_STATE.sessionActive=false;MOVEMENT_STATE.search="";MOVEMENT_STATE.searchInput="";MOVEMENT_STATE.sessionItems=[];MOVEMENT_STATE.submitting=false;renderMovementPage();};
...
document.addEventListener('input',e=>{if(e.target?.id==='mvSearch'){MOVEMENT_STATE.searchInput=e.target.value;clearTimeout(MOVEMENT_STATE.searchTimer);MOVEMENT_STATE.searchTimer=setTimeout(()=>{MOVEMENT_STATE.search=MOVEMENT_STATE.searchInput;renderMovementSearchResults();},280);return;}if(e.target?.dataset?.mvm){const idx=Number(e.target.dataset.idx);const row=MOVEMENT_STATE.sessionItems[idx];if(!row)return;if(e.target.dataset.mvm==='to')row.to=e.target.value||"";if(e.target.dataset.mvm==='akt'){const n=Number(e.target.value);row.stokAktual=Number.isFinite(n)?n:null;}const submitBtn=document.getElementById('mvSubmitBtn');if(submitBtn)submitBtn.disabled=!canSubmitMovement()||MOVEMENT_STATE.submitting;}});
document.addEventListener('click',e=>{const btn=e.target.closest('[data-mvm-action]');if(!btn)return;const action=btn.dataset.mvmAction;if(action==='remove'){const idx=Number(btn.dataset.idx);MOVEMENT_STATE.sessionItems.splice(idx,1);renderMovementSessionTable();return;}if(action==='add'){const sku=decodeURIComponent(btn.dataset.sku||"");const lokasi=decodeURIComponent(btn.dataset.lok||"");const source=getMovementSourceRows().find(r=>clean(r.sku)===clean(sku)&&clean(r.lokasi)===clean(lokasi));if(source)addMovementItem(source);}});
