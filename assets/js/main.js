...
function computeMovementSuggestions(){const source=getMovementSourceRows();const key=`${MOVEMENT_STATE.suggestionThreshold}|${getMovementSuggestionSourceKey(source)}`;if(MOVEMENT_STATE.suggestionCache.key===key)return MOVEMENT_STATE.suggestionCache;const grouped=new Map();for(const row of source){const sku=String(row.sku||"").trim();if(!sku)continue;const qty=Number(row.stok_akhir)||0;const sig=parseLokasiSignature(row.lokasi);if(!grouped.has(sku))grouped.set(sku,{sku,nama:row.nama||"-",locations:[],totalQty:0});const bucket=grouped.get(sku);bucket.locations.push({lokasi:row.lokasi||"-",qty,signature:sig.signature,digitCount:sig.digits});bucket.totalQty+=qty;}const suggestions=[];grouped.forEach(item=>{if(item.locations.length<=1)return;const groupsByType=new Map();item.locations.forEach(loc=>{if(!groupsByType.has(loc.signature))groupsByType.set(loc.signature,[]);groupsByType.get(loc.signature).push(loc);});groupsByType.forEach(groupLocs=>{if(groupLocs.length<=1)return;const sorted=[...groupLocs].sort((a,b)=>b.qty-a.qty);const target=sorted[0];if(!target||target.qty<=0)return;const totalDigit=groupLocs.reduce((acc,x)=>acc+(Number(x.digitCount)||0),0);for(let i=1;i<sorted.length;i++){const origin=sorted[i];if(origin.qty<=0||origin.qty>=MOVEMENT_STATE.suggestionThreshold)continue;if(item.totalQty<=origin.qty)continue;suggestions.push({sku:item.sku,nama:item.nama,fromLokasi:origin.lokasi,toLokasi:target.lokasi,toQty:target.qty,qtyMove:origin.qty,totalLokasi:groupLocs.length,totalDigit});}});});MOVEMENT_STATE.suggestionCache={key,rows:suggestions,count:suggestions.length};return MOVEMENT_STATE.suggestionCache;}
