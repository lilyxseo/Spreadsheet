...
function routeFromPath(path){if(!user)return showLoginView();if(path==="/")return showPage("dashboard");if(path==="/search")return showPage("search");if(path==="/barang-masuk")return showPage("barang-masuk");if(path==="/barang-keluar")return showPage("barang-keluar");if(path==="/accuracy-dashboard"||path==="/accuracy"||path==="/dashboard-akurasi")return showPage("stats");if(path==="/statistics"){history.replaceState({},"","/");return showPage("dashboard");}if(path==="/locations"||path==="/location")return showPage("locations");if(path==="/settings")return showPage("settings");if(path==="/sheet-input")return showPage("sheet-input");if(path==="/arsip")return showPage("arsip");if(path==="/asset-store")return showPage("asset-store");if(path==="/cycle-count")return showPage("cycle-count");if(path==="/movement")return showPage("movement");if(path==="/refill")return showPage("refill");if(path==="/balikan-store")return showPage("balikan-store");if(path==="/activity-log"){if(!isDeveloperUser()){history.replaceState({},"","/");return showPage("dashboard");}return showPage("activity-log");}if(path==="/anomaly"){history.replaceState({},"","/warning");return showPage("anomaly");}if(path==="/warning")return showPage("anomaly");if(path==="/stok-minus")return showPage("stok-minus");if(path.startsWith("/sku/")){currentSku=decodeURIComponent(path.split("/sku/")[1]||"");if(currentSku)showDetail(currentSku);return showPage("detail");}showPage("dashboard");}
...
if(page==="refill")renderRefillPage();
...
const REFILL_STATE={items:[],search:"",history:[],loading:false,submitting:false};
function isRefillSourceLokasi(lokasi){return /^\d{5}$/.test(String(lokasi||"").trim());}
function getRplAutoToMap(){const map=new Map();const rows=Array.isArray(DATA["RPL"])?DATA["RPL"]:[];rows.forEach(r=>{const sku=clean(r.sku||r.SKU||r["SKU"]||"");if(!sku)return;const vals=Object.values(r||{}).map(v=>String(v||"").trim());const to=vals.find(v=>/^\d{8}$/.test(v));if(to&&!map.has(sku))map.set(sku,to);});return map;}
function getRefillSourceRows(){const rplMap=getRplAutoToMap();return (getMovementSourceRows()||[]).filter(r=>isRefillSourceLokasi(r.lokasi)&&rplMap.has(clean(r.sku)));}
async function loadRefillHistory(){try{const r=await fetch('/api/refill?mode=history');const j=await r.json();if(r.ok&&j.success)REFILL_STATE.history=Array.isArray(j.data)?j.data:[];}catch(_e){}}
function renderRefillPage(){if(!refillApp)return;const keyword=clean(REFILL_STATE.search);const sourceRows=getRefillSourceRows();const rows=keyword?sourceRows.filter(r=>clean(`${r.sku} ${r.nama} ${r.lokasi}`).includes(keyword)):[];const history=[...REFILL_STATE.history].reverse().slice(0,100);refillApp.innerHTML=`<div class='card cc-card cc-section'><div class='mv-toolbar'><input id='refillSearchInput' class='search-lg' placeholder='Cari SKU / nama / lokasi asal' value='${esc(REFILL_STATE.search)}'></div><div class='table-wrap cc-table-wrap'><table><thead><tr><th>Lokasi Asal</th><th>SKU</th><th>Nama</th><th>Qty</th><th>Aksi</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.lokasi)}</td><td>${esc(r.sku)}</td><td>${esc(r.nama)}</td><td>${esc(r.stok_akhir)}</td><td><button class='btn-ghost' data-rf-add='1' data-sku='${encAttr(r.sku)}' data-lok='${encAttr(r.lokasi)}'>Tambah</button></td></tr>`).join('')||(keyword?"<tr><td colspan='5'><div class='state'>Tidak ada SKU refill yang cocok.</div></td></tr>":"<tr><td colspan='5'><div class='state'>Ketik SKU terlebih dahulu untuk menampilkan data.</div></td></tr>")}</tbody></table></div><h4>Refill Berjalan</h4><div class='table-wrap cc-table-wrap'><table><thead><tr><th>Lokasi Asal</th><th>Lokasi Tujuan</th><th>SKU</th><th>Nama Barang</th><th>Qty</th><th>Aksi</th></tr></thead><tbody>${REFILL_STATE.items.map((it,idx)=>`<tr><td>${esc(it.from)}</td><td><input data-rf='to' data-idx='${idx}' value='${esc(it.to||"")}'></td><td>${esc(it.sku)}</td><td>${esc(it.nama)}</td><td><input type='number' min='1' data-rf='qty' data-idx='${idx}' value='${esc(it.qty||"")}'></td><td><button class='btn-ghost' data-rf-remove='${idx}'>Hapus</button></td></tr>`).join('')||"<tr><td colspan='6'><div class='state'>Belum ada item.</div></td></tr>"}</tbody></table></div><div class='row'><button class='btn-primary' id='rfSubmitBtn' ${REFILL_STATE.submitting?"disabled":""}>Submit Refill</button></div></div><div class='card cc-card cc-section mv-history-section'><h4>History Refill</h4><div class='table-wrap cc-table-wrap'><table><thead><tr><th>Tanggal</th><th>Lokasi Asal</th><th>Lokasi Tujuan</th><th>SKU</th><th>NAMA BARANG</th><th>Qty</th></tr></thead><tbody>${history.map(r=>`<tr><td>${esc(r.tanggal||'-')}</td><td>${esc(r.from||r.lokasi_asal||'-')}</td><td>${esc(r.to||r.lokasi_tujuan||'-')}</td><td>${esc(r.sku||'-')}</td><td>${esc(r.nama_barang||r.namaBarang||'-')}</td><td>${esc(r.qty||'-')}</td></tr>`).join('')||"<tr><td colspan='6'><div class='state'>Belum ada history.</div></td></tr>"}</tbody></table></div></div>`;document.getElementById('rfSubmitBtn')?.addEventListener('click',submitRefillSession);}
async function submitRefillSession(){if(!REFILL_STATE.items.length){toast('Tambahkan item refill dulu.','error');return;}for(const it of REFILL_STATE.items){if(!it.sku||!it.from||!it.to||!it.qty||!Number.isFinite(Number(it.qty))){toast('SKU, lokasi asal, lokasi tujuan, qty wajib valid.','error');return;}}REFILL_STATE.submitting=true;renderRefillPage();try{const role=normalizeRole(getCurrentUserRole());const pic=String(APP_CONFIG.PIC_BY_ROLE[role]||APP_CONFIG.PIC_BY_ROLE.inventory).toUpperCase();const tanggal=formatTanggal(new Date());const items=REFILL_STATE.items.map(it=>({tanggal,from:it.from,to:it.to,sku:it.sku,namaBarang:it.nama,qty:Number(it.qty),pic,stokDiLokasiAwal:Number(it.qty),stokAktual:Number(it.qty)}));const r=await fetch('/api/refill',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({spreadsheetId:REFILL_SPREADSHEET_ID,items})});const j=await r.json();if(!r.ok||!j.success)throw new Error(j.message||'Gagal submit refill');toast('Refill berhasil disimpan','success');REFILL_STATE.items=[];await loadRefillHistory();}catch(e){toast(e.message||'Gagal submit refill','error');}finally{REFILL_STATE.submitting=false;renderRefillPage();}}
document.addEventListener('input',e=>{if(e.target?.id==='refillSearchInput'){REFILL_STATE.search=e.target.value;renderRefillPage();return;}if(e.target?.dataset?.rf){const idx=Number(e.target.dataset.idx);const row=REFILL_STATE.items[idx];if(!row)return;if(e.target.dataset.rf==='to')row.to=e.target.value||'';if(e.target.dataset.rf==='qty')row.qty=e.target.value;}});
document.addEventListener('click',e=>{const add=e.target.closest('[data-rf-add]');if(add){const sku=decodeURIComponent(add.dataset.sku||'');const lok=decodeURIComponent(add.dataset.lok||'');const src=getRefillSourceRows().find(r=>clean(r.sku)===clean(sku)&&clean(r.lokasi)===clean(lok));if(!src)return;const auto=getRplAutoToMap().get(clean(src.sku))||'';REFILL_STATE.items.push({from:src.lokasi,to:auto,sku:src.sku,nama:src.nama,qty:''});renderRefillPage();return;}const rem=e.target.closest('[data-rf-remove]');if(rem){REFILL_STATE.items.splice(Number(rem.dataset.rfRemove),1);renderRefillPage();}});
loadRefillHistory();
