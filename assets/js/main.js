...
function getBalikanStatusBadgeClass(status){
  ...
  if(normalized==='lebihkirim')return 'is-lebih-kirim';
  if(normalized==='kurangkirim')return 'is-kurang-kirim';
  return '';
}
function createBalikanStatusBadge(status){
  const value=String(status??'').trim();
  if(!value)return null;
  const badge=document.createElement('span');
  badge.className=`balikan-status-badge ${getBalikanStatusBadgeClass(value)}`.trim();
  badge.textContent=value;
  return badge;
}
function updateBalikanStatusBadge(rowNumber,status,sheetName=''){
  const rowEl=document.getElementById(getBalikanDomRowId(rowNumber,sheetName));
  const wrap=rowEl?.querySelector('.balikan-sheet-status-wrap');
  if(!wrap)return;
  wrap.querySelector('.balikan-status-badge')?.remove();
  const badge=createBalikanStatusBadge(status);
