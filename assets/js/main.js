...
const ASSET_STORE_RESULT_COLUMNS=["Sheet Sumber","SKU / Kode Asset","Nama Asset","Lokasi","Qty / Stok","Keterangan","Status"];
const ASSET_STORE_STATE={sheetList:[],selectedSheet:"",cache:{},loadingList:false,loadingData:false,loadingAll:false,listError:"",dataError:"",searchInput:"",search:"",searchTimer:null,searchToken:0,minSearchLength:0,sortColumn:"",sortDirection:"asc",columnFilter:"all",page:1,pageSize:25,lastLoadedAt:{},saveStatus:{},selectedSku:"",selectedLocation:""};
function bindAssetStoreEvents(){document.addEventListener("change",e=>{if(e.target?.id==="assetStoreSheetSelect")selectAssetStoreSheet(e.target.value);if(e.target?.id==="assetStoreSortColumn"){ASSET_STORE_STATE.sortColumn=e.target.value;ASSET_STORE_STATE.page=1;renderAssetStoreTableOnly();}if(e.target?.id==="assetStoreSortDir"){ASSET_STORE_STATE.sortDirection=e.target.value;ASSET_STORE_STATE.page=1;renderAssetStoreTableOnly();}if(e.target?.id==="assetStoreColumnFilter"){ASSET_STORE_STATE.columnFilter=e.target.value;ASSET_STORE_STATE.page=1;renderAssetStoreTableOnly();}if(e.target?.id==="assetStorePageSize"){ASSET_STORE_STATE.pageSize=Number(e.target.value)||25;ASSET_STORE_STATE.page=1;renderAssetStoreTableOnly();}});document.addEventListener("input",e=>{if(e.target?.id!=="assetStoreSearch")return;ASSET_STORE_STATE.searchInput=e.target.value||"";const debounceToken=++ASSET_STORE_STATE.searchToken;clearTimeout(ASSET_STORE_STATE.searchTimer);ASSET_STORE_STATE.searchTimer=setTimeout(async()=>{if(debounceToken!==ASSET_STORE_STATE.searchToken)return;ASSET_STORE_STATE.search=String(ASSET_STORE_STATE.searchInput||"").trim();ASSET_STORE_STATE.page=1;if(!ASSET_STORE_STATE.selectedSheet&&ASSET_STORE_STATE.search)await ensureAllAssetStoreSheetsLoaded();renderAssetStoreTableOnly();},300);});document.addEventListener("click",e=>{if(e.target?.closest("#assetStoreRefreshBtn"))refreshAssetStore();if(e.target?.closest("#assetStoreResetBtn")){resetAssetStoreFilter();return;}if(e.target?.closest("#assetStoreExportBtn")){exportAssetStoreCsv();return;}const sku=e.target?.closest('[data-asset-select-sku]');if(sku){ASSET_STORE_STATE.searchInput=sku.dataset.assetSelectSku||"";ASSET_STORE_STATE.search=ASSET_STORE_STATE.searchInput;ASSET_STORE_STATE.selectedSku=ASSET_STORE_STATE.search;ASSET_STORE_STATE.page=1;renderAssetStorePage();document.getElementById('assetStoreSearch')?.focus();return;}const loc=e.target?.closest('[data-asset-select-location]');if(loc){ASSET_STORE_STATE.searchInput=loc.dataset.assetSelectLocation||"";ASSET_STORE_STATE.search=ASSET_STORE_STATE.searchInput;ASSET_STORE_STATE.selectedLocation=ASSET_STORE_STATE.search;ASSET_STORE_STATE.page=1;renderAssetStorePage();document.getElementById('assetStoreSearch')?.focus();return;}const pg=e.target?.closest("[data-asset-store-page]");if(pg){ASSET_STORE_STATE.page=Math.max(1,ASSET_STORE_STATE.page+(Number(pg.dataset.assetStorePage)||0));renderAssetStoreTableOnly();return;}const cell=e.target?.closest('[data-asset-edit]');if(cell){beginAssetStoreInlineEdit(cell);}});}
...
function getAssetStoreCacheKey(sheetName){return `asset-store:${ASSET_STORE_SPREADSHEET_ID}:${String(sheetName||'')}`;}
function getAssetStoreSheetCache(sheetName){try{const parsed=JSON.parse(localStorage.getItem(getAssetStoreCacheKey(sheetName))||'null');return parsed&&Array.isArray(parsed.rows)&&Array.isArray(parsed.columns)?parsed:null;}catch(_err){return null;}}
function setAssetStoreSheetCache(sheetName,payload){try{localStorage.setItem(getAssetStoreCacheKey(sheetName),JSON.stringify({...payload,updatedAt:Date.now()}));}catch(_err){}}
async function ensureAssetStoreSheetLoaded(sheetName,{force=false}={}){if(!sheetName)return;if(ASSET_STORE_STATE.cache[sheetName]&&!force)return;const cached=!force?getAssetStoreSheetCache(sheetName):null;if(cached){ASSET_STORE_STATE.cache[sheetName]=cached;ASSET_STORE_STATE.lastLoadedAt[sheetName]=cached.updatedAt?new Date(cached.updatedAt).toISOString():new Date().toISOString();return;}ASSET_STORE_STATE.cache[sheetName]=await fetchAssetStoreSheetData(sheetName);ASSET_STORE_STATE.lastLoadedAt[sheetName]=new Date().toISOString();setAssetStoreSheetCache(sheetName,ASSET_STORE_STATE.cache[sheetName]);}
async function ensureAllAssetStoreSheetsLoaded({force=false}={}){if(!ASSET_STORE_STATE.sheetList.length)await ensureAssetStoreList();const missing=ASSET_STORE_STATE.sheetList.map(s=>s.key).filter(k=>force||!ASSET_STORE_STATE.cache[k]);if(!missing.length)return;ASSET_STORE_STATE.loadingAll=true;ASSET_STORE_STATE.loadingData=true;renderAssetStoreTableOnly();try{const results=await Promise.allSettled(missing.map(sheet=>ensureAssetStoreSheetLoaded(sheet,{force})));const failed=results.filter(r=>r.status==='rejected');if(failed.length)toast(`${failed.length} sheet Asset Store gagal dimuat`,'error');}finally{ASSET_STORE_STATE.loadingAll=false;ASSET_STORE_STATE.loadingData=false;renderAssetStoreTableOnly();}}
async function selectAssetStoreSheet(sheetName){ASSET_STORE_STATE.selectedSheet=sheetName||"";ASSET_STORE_STATE.page=1;ASSET_STORE_STATE.dataError="";if(!sheetName){if(ASSET_STORE_STATE.search)ensureAllAssetStoreSheetsLoaded();renderAssetStorePage();return;}if(ASSET_STORE_STATE.cache[sheetName]){renderAssetStorePage();return;}ASSET_STORE_STATE.loadingData=true;renderAssetStoreTableOnly();try{await ensureAssetStoreSheetLoaded(sheetName);}catch(err){ASSET_STORE_STATE.dataError=err.message||"Gagal memuat data sheet";}finally{ASSET_STORE_STATE.loadingData=false;renderAssetStoreTableOnly();}}
async function refreshAssetStore(){const sheet=ASSET_STORE_STATE.selectedSheet;ASSET_STORE_STATE.dataError="";if(!sheet){ASSET_STORE_STATE.cache={};ASSET_STORE_STATE.lastLoadedAt={};await ensureAllAssetStoreSheetsLoaded({force:true});return;}ASSET_STORE_STATE.loadingData=true;renderAssetStoreTableOnly();try{await ensureAssetStoreSheetLoaded(sheet,{force:true});}catch(err){ASSET_STORE_STATE.dataError=err.message||"Gagal refresh data";}finally{ASSET_STORE_STATE.loadingData=false;renderAssetStoreTableOnly();}}
function normalizeAssetHeader(v){return clean(String(v||'').replace(/[\/_-]+/g,' '));}
function findAssetColumn(columns,aliases){return (columns||[]).find(c=>aliases.some(a=>normalizeAssetHeader(c).includes(normalizeAssetHeader(a))))||'';}
function mapAssetStoreRow(row,columns,sheetName){const skuCol=findAssetColumn(columns,['sku','kode asset','kode','asset code']);const nameCol=findAssetColumn(columns,['nama asset','nama barang','nama item','asset name','nama']);const locCol=findAssetColumn(columns,['lokasi','location','rak','area']);const qtyCol=findAssetColumn(columns,['qty','stok','stock','jumlah']);const noteCol=findAssetColumn(columns,['keterangan','ket','catatan','note']);const statusCol=findAssetColumn(columns,['status','kondisi']);return {__sourceRow:row,__columns:columns,__sheetName:sheetName,__rowNumber:row.__rowNumber,sheet:sheetName,sku:skuCol?row[skuCol]:'',nama:nameCol?row[nameCol]:'',lokasi:locCol?row[locCol]:'',qty:qtyCol?row[qtyCol]:'',keterangan:noteCol?row[noteCol]:'',status:statusCol?row[statusCol]:'',__fieldMap:{sku:skuCol,nama:nameCol,lokasi:locCol,qty:qtyCol,keterangan:noteCol,status:statusCol}};}
function assetSearchText(r){return normalizeSearch(`${r.sku} ${r.nama} ${r.lokasi} ${r.keterangan}`);}
function assetSearchTokens(q){return normalizeSearch(q).split(/\s+/).filter(Boolean);}
function getAssetStoreRows(){const selected=ASSET_STORE_STATE.selectedSheet;const sources=selected?[selected]:ASSET_STORE_STATE.sheetList.map(s=>s.key);let mapped=[];let headerError=false,totalSource=0;const sourceColumns=new Set();sources.forEach(sheet=>{const parsed=ASSET_STORE_STATE.cache[sheet]||{rows:[],columns:[],headerError:false};if(parsed.headerError)headerError=true;totalSource+=parsed.rows?.length||0;(parsed.columns||[]).forEach(c=>sourceColumns.add(c));mapped.push(...(parsed.rows||[]).map(r=>mapAssetStoreRow(r,parsed.columns||[],sheet)));});const tokens=assetSearchTokens(ASSET_STORE_STATE.search);if(tokens.length)mapped=mapped.filter(r=>tokens.every(t=>assetSearchText(r).includes(t)));if(ASSET_STORE_STATE.columnFilter!=='all'){const key=ASSET_STORE_STATE.columnFilter;mapped=mapped.filter(r=>clean(r[key]).includes(clean(ASSET_STORE_STATE.search)));}if(ASSET_STORE_STATE.sortColumn){const col=ASSET_STORE_STATE.sortColumn;mapped.sort((a,b)=>String(a[col]??"").localeCompare(String(b[col]??""),"id",{numeric:true,sensitivity:'base'}));if(ASSET_STORE_STATE.sortDirection==='desc')mapped.reverse();}return {rows:mapped,columns:ASSET_STORE_RESULT_COLUMNS,sourceColumns:[...sourceColumns],headerError,totalSource};}
function getAssetSummary(rows){const qty=rows.reduce((sum,r)=>sum+toNumberSafe(r.qty),0);const locations=[...new Set(rows.map(r=>String(r.lokasi||'').trim()).filter(Boolean))];const sheets=[...new Set(rows.map(r=>String(r.sheet||'').trim()).filter(Boolean))];const notes=[...new Set(rows.map(r=>String(r.keterangan||'').trim()).filter(Boolean))].slice(0,4);return {qty,locations,sheets,notes};}
function beginAssetStoreInlineEdit(cell){const rowNumber=Number(cell.dataset.rowNumber);const fieldKey=cell.dataset.assetField||"";const sheetName=cell.dataset.sheetName||ASSET_STORE_STATE.selectedSheet;if(!sheetName||!rowNumber||!fieldKey)return;const parsed=ASSET_STORE_STATE.cache[sheetName];const mapped=getAssetStoreRows().rows.find(r=>Number(r.__rowNumber)===rowNumber&&r.__sheetName===sheetName);const field=mapped?.__fieldMap?.[fieldKey]||decodeURIComponent(cell.dataset.field||'');if(!parsed||!field)return;const row=(parsed.rows||[]).find(r=>Number(r.__rowNumber)===rowNumber);if(!row)return;startInlineEdit(cell,row,field,row[field],{onSave:async({value,oldValue,td})=>{const key=`${sheetName}:${rowNumber}:${field}`;ASSET_STORE_STATE.saveStatus[key]='saving';td.dataset.saveState='saving';const old=String(oldValue??'');row[field]=value;td.textContent=value||'-';try{const res=await fetch('/api/asset-store/cell',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({spreadsheetId:ASSET_STORE_SPREADSHEET_ID,sheetName,rowNumber,field,headers:parsed.columns,value})});const out=await res.json().catch(()=>({}));if(!res.ok||out?.success===false)throw new Error(out?.message||'Gagal update cell');ASSET_STORE_STATE.saveStatus[key]='saved';td.dataset.saveState='saved';td.innerHTML=`${esc(value||'')}<span class='asset-save-badge'>tersimpan</span>`;setTimeout(()=>{if(td.dataset.saveState==='saved'){td.dataset.saveState='';td.textContent=value||'-';}},1200);}catch(err){row[field]=old;td.textContent=old||'-';ASSET_STORE_STATE.saveStatus[key]='error';td.dataset.saveState='error';throw err;}}});}
function renderAssetCell(r,key){if(key==='sku')return `<button class='balikan-sku-select' type='button' data-asset-select-sku='${encAttr(r.sku)}'>${esc(r.sku||'-')}</button>`;if(key==='lokasi')return `<button class='btn-ghost' type='button' data-asset-select-location='${encAttr(r.lokasi)}'>${esc(r.lokasi||'-')}</button>`;const field=r.__fieldMap?.[key]||'';const state=ASSET_STORE_STATE.saveStatus[`${r.__sheetName}:${r.__rowNumber}:${field}`]||'';const badge=state==='saved'?"<span class='asset-save-badge'>tersimpan</span>":'';return `<span>${esc(r[key]??'')}</span>${badge}`;}
function renderAssetStoreTableOnly(){const host=document.getElementById("assetStoreDataSection");if(!host)return;const selected=ASSET_STORE_STATE.selectedSheet;const {rows,headerError,totalSource}=getAssetStoreRows();const totalPage=Math.max(1,Math.ceil(rows.length/ASSET_STORE_STATE.pageSize));if(ASSET_STORE_STATE.page>totalPage)ASSET_STORE_STATE.page=totalPage;const start=(ASSET_STORE_STATE.page-1)*ASSET_STORE_STATE.pageSize;const pageRows=rows.slice(start,start+ASSET_STORE_STATE.pageSize);const summary=getAssetSummary(rows);if(ASSET_STORE_STATE.loadingData){host.innerHTML=`<div class='card archive-section'><div class='state'>${ASSET_STORE_STATE.loadingAll?'Memuat cache semua sheet Asset Store...':'Memuat data sheet...'}</div></div>`;return;}if(ASSET_STORE_STATE.dataError){host.innerHTML=`<div class='card archive-section'><div class='state'>${esc(ASSET_STORE_STATE.dataError)}</div></div>`;return;}if(headerError){host.innerHTML="<div class='card archive-section'><div class='state'>Header tidak terdeteksi pada salah satu sheet</div></div>";return;}if(!selected&&!ASSET_STORE_STATE.search){host.innerHTML="<div class='card archive-section'><div class='state'>Pilih sheet untuk melihat data, atau ketik pencarian untuk mencari ke semua sheet Asset Store.</div></div>";return;}host.innerHTML=`<div class='card archive-section'><div class='grid dashboard archive-summary'><div class='metric'><div class='k'>Total Asset Ditemukan</div><div class='v'>${rows.length}</div></div><div class='metric'><div class='k'>Total Qty</div><div class='v'>${summary.qty.toLocaleString('id-ID')}</div></div><div class='metric'><div class='k'>Total Lokasi</div><div class='v'>${summary.locations.length}</div></div><div class='metric'><div class='k'>Sheet Sumber</div><div class='v'>${summary.sheets.length?esc(summary.sheets.join(', ')):esc(selected||'Semua Sheet')}</div></div><div class='metric'><div class='k'>Keterangan</div><div class='v'>${summary.notes.length?esc(summary.notes.join(' • ')):'-'}</div></div></div></div><div class='card archive-section archive-table-card'>${!rows.length?"<div class='state'>Tidak ada data asset</div>":`<div class='table-wrap table-wrap-full archive-table-wrap'><table><thead><tr>${ASSET_STORE_RESULT_COLUMNS.map(c=>`<th>${esc(c)}</th>`).join("")}</tr></thead><tbody>${pageRows.map(r=>`<tr><td><span class='balikan-sheet-badge'>${esc(r.sheet)}</span></td>${['sku','nama','lokasi','qty','keterangan','status'].map(k=>{const field=r.__fieldMap?.[k]||'';return `<td class='editable-cell' data-asset-edit='1' data-sheet-name='${encAttr(r.__sheetName)}' data-row-number='${r.__rowNumber}' data-asset-field='${k}' data-field='${encAttr(field)}'>${renderAssetCell(r,k)}</td>`;}).join('')}</tr>`).join("")}</tbody></table></div><div class='mv-pagination'><span>Menampilkan ${rows.length?start+1:0}-${Math.min(start+ASSET_STORE_STATE.pageSize,rows.length)} dari ${rows.length} data (${totalSource} baris sumber)</span><div class='row'><select id='assetStorePageSize'><option value='25' ${ASSET_STORE_STATE.pageSize===25?'selected':''}>25 row</option><option value='50' ${ASSET_STORE_STATE.pageSize===50?'selected':''}>50 row</option><option value='100' ${ASSET_STORE_STATE.pageSize===100?'selected':''}>100 row</option></select><button class='btn-ghost' data-asset-store-page='-1' ${ASSET_STORE_STATE.page<=1?'disabled':''}>Prev</button><span>Halaman ${ASSET_STORE_STATE.page} / ${totalPage}</span><button class='btn-ghost' data-asset-store-page='1' ${ASSET_STORE_STATE.page>=totalPage?'disabled':''}>Next</button></div></div>`}</div>`;}
function resetAssetStoreFilter(){ASSET_STORE_STATE.searchInput='';ASSET_STORE_STATE.search='';ASSET_STORE_STATE.selectedSku='';ASSET_STORE_STATE.selectedLocation='';ASSET_STORE_STATE.columnFilter='all';ASSET_STORE_STATE.sortColumn='';ASSET_STORE_STATE.page=1;renderAssetStorePage();setTimeout(()=>document.getElementById('assetStoreSearch')?.focus(),0);}
function exportAssetStoreCsv(){const {rows}=getAssetStoreRows();const csv=[ASSET_STORE_RESULT_COLUMNS.join(',')].concat(rows.map(r=>[r.sheet,r.sku,r.nama,r.lokasi,r.qty,r.keterangan,r.status].map(v=>`"${String(v??'').replace(/"/g,'""')}"`).join(','))).join('\n');const blob=new Blob([csv],{type:'text/csv;charset=utf-8'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`asset-store-${new Date().toISOString().slice(0,10)}.csv`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),500);}
function renderAssetStorePage(){if(!assetStoreApp)return;if(!ASSET_STORE_STATE.sheetList.length&&!ASSET_STORE_STATE.loadingList)ensureAssetStoreList();const selected=ASSET_STORE_STATE.selectedSheet;const sortOps=['sheet','sku','nama','lokasi','qty','keterangan','status'].map(c=>`<option value='${esc(c)}' ${ASSET_STORE_STATE.sortColumn===c?"selected":""}>${esc(c)}</option>`).join("");assetStoreApp.innerHTML=`<div class='archive-layout'><div class='card archive-section'><div class='section-header'><h3 class='page-title'>Asset Store</h3><div class='row'><select id='assetStoreSheetSelect'><option value=''>Semua Sheet Asset Store</option>${ASSET_STORE_STATE.sheetList.map(s=>`<option value='${esc(s.key)}' ${s.key===selected?"selected":""}>${esc(s.name)}</option>`).join("")}</select><button id='assetStoreRefreshBtn' class='btn-ghost'>Refresh Asset Store</button></div></div>${ASSET_STORE_STATE.loadingList?"<div class='state'>Memuat daftar sheet...</div>":ASSET_STORE_STATE.listError?`<div class='state'>${esc(ASSET_STORE_STATE.listError)}</div>`:""}</div><div class='card archive-section archive-filter-card'><div class='mv-filters open archive-filters'><input id='assetStoreSearch' class='search-lg' placeholder='Cari SKU, nama asset, lokasi, keterangan' value='${esc(ASSET_STORE_STATE.searchInput)}'/><select id='assetStoreSortColumn'><option value=''>Urutkan Kolom</option>${sortOps}</select><select id='assetStoreSortDir'><option value='asc' ${ASSET_STORE_STATE.sortDirection==='asc'?'selected':''}>ASC</option><option value='desc' ${ASSET_STORE_STATE.sortDirection==='desc'?'selected':''}>DESC</option></select><select id='assetStoreColumnFilter'><option value='all'>Semua Kolom Search</option><option value='sku' ${ASSET_STORE_STATE.columnFilter==='sku'?'selected':''}>SKU / Kode Asset</option><option value='nama' ${ASSET_STORE_STATE.columnFilter==='nama'?'selected':''}>Nama Asset</option><option value='lokasi' ${ASSET_STORE_STATE.columnFilter==='lokasi'?'selected':''}>Lokasi</option><option value='keterangan' ${ASSET_STORE_STATE.columnFilter==='keterangan'?'selected':''}>Keterangan</option></select><button id='assetStoreResetBtn' class='btn-ghost' type='button'>Reset Filter</button><button id='assetStoreExportBtn' class='btn-ghost' type='button'>Export CSV</button></div></div><div id='assetStoreDataSection'></div></div>`;renderAssetStoreTableOnly();}
document.addEventListener("DOMContentLoaded",()=>{ensureCycleHistoryLoaded().finally(()=>renderCycleCountPage());});
