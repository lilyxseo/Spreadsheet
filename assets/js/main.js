...
const BALIKAN_SHEET_LIST_CACHE_KEY="balikanStoreSheetListCache";
const BALIKAN_SHEET_LIST_TTL=20*60*1000;
const BALIKAN_DATA_CACHE_TTL=5*60*1000;
const BALIKAN_STATE={sheets:[],sheetList:[],sheetListLoaded:false,isSheetListLoading:false,sheetListLastFetch:0,sheetListError:"",sheetListFromCache:false,sheetListRequest:null,sheetListToastAt:0,sheetDataRequests:new Map(),sheetDataController:null,sheetDataVersion:0,sheetCache:{},sheetChecksums:{},dynamicColumnCache:{},allTripLoading:false,lastRefreshAt:0,highlightRowNumber:null,highlightSheetName:"",sortBy:"default",autoCheckOnScan:true,exactScanSku:"",selectedSkuRowNumber:null,selectedSkuSheetName:"",selectedSkuValue:"",lastCheckedRowId:null,lastCheckedSheetName:"",lastCheckedSku:"",lastCheckedVersion:0,lastCheckedFadeTimer:null,pendingEdits:{},pendingEditMeta:{},saveStatus:{},saveTimer:null,saveInProgress:false,saveRequested:false,isRendering:false,isRefreshing:false,pendingRender:false,lastRenderedChecksum:"",lastDataChecksum:"",lastRenderedHeaderKey:"",renderTimer:null,searchDebounceTimer:null};
const balikanDevLog=(...args)=>{if(getCurrentUser?.()?.isDeveloper===true)console.debug("[BalikanStore]",...args);};
...
balikanSheetSelect?.addEventListener("change",e=>handleBalikanSheetChange(e.target.value||""));
bindBalikanSheetCombobox();
...
function getBalikanSheetListCache(){try{const value=JSON.parse(localStorage.getItem(BALIKAN_SHEET_LIST_CACHE_KEY)||'null');return value&&Array.isArray(value.sheets)?value:null;}catch(_err){return null;}}
function setBalikanSheetListCache(sheets){try{localStorage.setItem(BALIKAN_SHEET_LIST_CACHE_KEY,JSON.stringify({sheets,updatedAt:Date.now()}));}catch(_err){}}
const waitBalikanRetry=ms=>new Promise(resolve=>setTimeout(resolve,ms));
function renderBalikanSheetList(){
  const select=document.getElementById('balikanSheetSelect'),trigger=document.getElementById('balikanSheetTrigger'),status=document.getElementById('balikanSheetStatus');
  if(!select||!trigger)return;
  select.innerHTML='<option value="">Pilih sheet TRIP</option>'+BALIKAN_STATE.sheetList.map(n=>`<option value="${esc(n)}">${esc(n)}</option>`).join('');
  select.value=window.currentTripSheet||'';
  trigger.disabled=BALIKAN_STATE.isSheetListLoading&&!BALIKAN_STATE.sheetList.length;
  trigger.querySelector('.balikan-sheet-trigger-label').textContent=trigger.disabled?'Memuat sheet…':(window.currentTripSheet||'Pilih sheet TRIP');
  if(status)status.innerHTML=BALIKAN_STATE.sheetListFromCache?'<span>Data sheet dari cache</span>':BALIKAN_STATE.sheetListError&&!BALIKAN_STATE.sheetList.length?'<span>Gagal memuat daftar sheet</span> <button type="button" data-balikan-sheet-retry>Coba lagi</button>':'';
  renderBalikanSheetOptions(document.getElementById('balikanSheetSearch')?.value||'');
}
async function fetchBalikanSheetListWithBackoff(){
  const delays=[0,2000,5000,10000];
  for(let attempt=0;attempt<delays.length;attempt++){
    if(delays[attempt])await waitBalikanRetry(delays[attempt]);
    const res=await fetch('/api/balikan-store/sheets',{cache:'no-store'});
    const data=await res.json().catch(()=>({}));
    if(res.ok)return Array.isArray(data.sheets)?data.sheets:[];
    const error=new Error(data?.message||'Gagal memuat daftar sheet');error.status=res.status;
    if(res.status!==429||attempt===delays.length-1)throw error;
    balikanDevLog('sheet-list error 429');
  }
  return [];
}
async function loadBalikanSheetList(force=false){
  const cached=getBalikanSheetListCache(),fresh=cached&&(Date.now()-Number(cached.updatedAt||0)<BALIKAN_SHEET_LIST_TTL);
  if(!force&&BALIKAN_STATE.sheetListLoaded&&BALIKAN_STATE.sheetListError)return BALIKAN_STATE.sheetList;
  if(!force&&BALIKAN_STATE.sheetListLoaded&&Date.now()-BALIKAN_STATE.sheetListLastFetch<BALIKAN_SHEET_LIST_TTL)return BALIKAN_STATE.sheetList;
  if(!force&&cached?.sheets?.length){BALIKAN_STATE.sheetList=cached.sheets;BALIKAN_STATE.sheets=cached.sheets;BALIKAN_STATE.sheetListLoaded=true;BALIKAN_STATE.sheetListLastFetch=Number(cached.updatedAt)||0;BALIKAN_STATE.sheetListFromCache=true;renderBalikanSheetList();balikanDevLog('sheet-list source CACHE');if(fresh)return BALIKAN_STATE.sheetList;}
  if(BALIKAN_STATE.sheetListRequest){balikanDevLog('sheet-list request DEDUPED');return BALIKAN_STATE.sheetListRequest;}
  BALIKAN_STATE.isSheetListLoading=!BALIKAN_STATE.sheetList.length;BALIKAN_STATE.sheetListError='';renderBalikanSheetList();balikanDevLog('sheet-list request START');
  BALIKAN_STATE.sheetListRequest=(async()=>{try{const sheets=(await fetchBalikanSheetListWithBackoff()).filter(n=>String(n||'').toUpperCase().includes('TRIP'));BALIKAN_STATE.sheetList=sheets;BALIKAN_STATE.sheets=sheets;BALIKAN_STATE.sheetListLoaded=true;BALIKAN_STATE.sheetListLastFetch=Date.now();BALIKAN_STATE.sheetListFromCache=false;setBalikanSheetListCache(sheets);balikanDevLog('sheet-list source API');return sheets;}catch(err){BALIKAN_STATE.sheetListError=err?.message||'Gagal memuat daftar sheet';BALIKAN_STATE.sheetListLoaded=true;if(cached?.sheets?.length){BALIKAN_STATE.sheetList=cached.sheets;BALIKAN_STATE.sheets=cached.sheets;BALIKAN_STATE.sheetListFromCache=true;if(err?.status===429&&Date.now()-BALIKAN_STATE.sheetListToastAt>60000){BALIKAN_STATE.sheetListToastAt=Date.now();toast('Batas request Google Sheets tercapai. Menggunakan data cache sementara.','error');}return BALIKAN_STATE.sheetList;}return [];}finally{BALIKAN_STATE.isSheetListLoading=false;BALIKAN_STATE.sheetListRequest=null;renderBalikanSheetList();}})();
  return BALIKAN_STATE.sheetListRequest;
}
async function loadBalikanSheets(){const sheets=await loadBalikanSheetList();if(!window.currentTripSheet&&sheets.length){window.currentTripSheet=sheets[0];renderBalikanSheetList();await loadBalikanRows();}}
function renderBalikanSheetOptions(query=''){
  const host=document.getElementById('balikanSheetOptions');if(!host)return;
  const q=clean(query),sheets=BALIKAN_STATE.sheetList.filter(name=>!q||clean(name).includes(q));
  host.innerHTML=sheets.length?sheets.map(name=>{const count=Array.isArray(BALIKAN_STATE.sheetCache[name])?BALIKAN_STATE.sheetCache[name].length:null;return `<button type="button" class="balikan-sheet-option${name===window.currentTripSheet?' is-selected':''}" role="option" aria-selected="${name===window.currentTripSheet}" data-balikan-sheet="${encAttr(name)}"><span class="balikan-sheet-option-name"><span aria-hidden="true">📄</span> ${esc(name)}</span>${count!==null?`<small>${count} data</small>`:''}</button>`;}).join(''):'<div class="balikan-sheet-empty">TIDAK ADA SHEET TRIP DITEMUKAN</div>';
}
function positionBalikanSheetMenu(){const trigger=document.getElementById('balikanSheetTrigger'),menu=document.getElementById('balikanSheetMenu');if(!trigger||!menu||menu.hidden)return;const rect=trigger.getBoundingClientRect(),gap=6,spaceBelow=innerHeight-rect.bottom-gap,openAbove=spaceBelow<220&&rect.top>spaceBelow;menu.style.left=`${Math.max(8,Math.min(rect.left,innerWidth-rect.width-8))}px`;menu.style.width=`${Math.min(rect.width,innerWidth-16)}px`;menu.style.top=openAbove?'auto':`${rect.bottom+gap}px`;menu.style.bottom=openAbove?`${innerHeight-rect.top+gap}px`:'auto';}
function closeBalikanSheetMenu(){const menu=document.getElementById('balikanSheetMenu'),trigger=document.getElementById('balikanSheetTrigger');if(menu)menu.hidden=true;if(trigger)trigger.setAttribute('aria-expanded','false');}
function bindBalikanSheetCombobox(){const root=document.getElementById('balikanSheetCombobox'),trigger=document.getElementById('balikanSheetTrigger'),menu=document.getElementById('balikanSheetMenu'),search=document.getElementById('balikanSheetSearch');if(!root||root.dataset.bound==='1')return;root.dataset.bound='1';trigger?.addEventListener('click',()=>{if(trigger.disabled)return;menu.hidden=!menu.hidden;trigger.setAttribute('aria-expanded',String(!menu.hidden));if(!menu.hidden){renderBalikanSheetOptions(search?.value||'');positionBalikanSheetMenu();requestAnimationFrame(()=>search?.focus());}});search?.addEventListener('input',()=>renderBalikanSheetOptions(search.value));root.addEventListener('click',e=>{const retry=e.target.closest('[data-balikan-sheet-retry]');if(retry){loadBalikanSheetList(true);return;}const option=e.target.closest('[data-balikan-sheet]');if(!option)return;const value=option.dataset.balikanSheet||'';balikanSheetSelect.value=value;balikanSheetSelect.dispatchEvent(new Event('change',{bubbles:true}));closeBalikanSheetMenu();});document.addEventListener('click',e=>{if(!root.contains(e.target))closeBalikanSheetMenu();});window.addEventListener('resize',positionBalikanSheetMenu);window.addEventListener('scroll',positionBalikanSheetMenu,true);}
async function handleBalikanSheetChange(sheetName){const next=String(sheetName||'').trim();if(!next||next===window.currentTripSheet){renderBalikanSheetList();return;}window.currentTripSheet=next;BALIKAN_STATE.highlightRowNumber=null;BALIKAN_STATE.highlightSheetName='';BALIKAN_STATE.selectedSkuRowNumber=null;BALIKAN_STATE.selectedSkuSheetName='';BALIKAN_STATE.selectedSkuValue='';ensureBalikanFilterState().page=1;renderBalikanSheetList();await loadBalikanRows();}
...
async function loadBalikanRows(options={}){const {background=false,force=false}=options||{};const targetSheet=String(window.currentTripSheet||'').trim();if(!targetSheet){await loadAllBalikanTripRows(options);return;}const version=++BALIKAN_STATE.sheetDataVersion;BALIKAN_STATE.sheetDataController?.abort();const controller=new AbortController();BALIKAN_STATE.sheetDataController=controller;try{BALIKAN_STATE.isRefreshing=true;if(balikanSortSelect)balikanSortSelect.value=BALIKAN_STATE.sortBy||'default';syncBalikanAutoCheckToggle();const {rows,dynamicColumns}=await fetchBalikanSheetRows(targetSheet,{force,signal:controller.signal});if(version!==BALIKAN_STATE.sheetDataVersion||targetSheet!==window.currentTripSheet)return;setBalikanDisplayRows(rows,dynamicColumns);}catch(err){if(err?.name!=='AbortError'&&!background)toast(err?.message||'Gagal memuat data Balikan Store','error');}finally{if(version===BALIKAN_STATE.sheetDataVersion){BALIKAN_STATE.isRefreshing=false;if(BALIKAN_STATE.sheetDataController===controller)BALIKAN_STATE.sheetDataController=null;}}}
