...
function toSheetColumnName(index){let n=Number(index)||0;let col='';while(n>0){const rem=(n-1)%26;col=String.fromCharCode(65+rem)+col;n=Math.floor((n-1)/26);}return col||'A';}
function beginAssetStoreInlineEdit(cell){const rowNumber=Number(cell.dataset.rowNumber);const rawField=cell.dataset.field||"";const field=decodeURIComponent(rawField);const sheetName=ASSET_STORE_STATE.selectedSheet;if(!sheetName||!rowNumber||!field)return;const parsed=ASSET_STORE_STATE.cache[sheetName];const row=(parsed?.rows||[]).find(r=>Number(r.__rowNumber)===rowNumber);if(!row)return;startInlineEdit(cell,row,field,row[field],{onSave:async({value,oldValue,td})=>{const key=`${sheetName}:${rowNumber}:${field}`;ASSET_STORE_STATE.saveStatus[key]='saving';td.dataset.saveState='saving';const old=String(oldValue??'');row[field]=value;td.textContent=value||'-';try{const columnIndex=(parsed?.columns||[]).findIndex(c=>String(c)===String(field));if(columnIndex<0)throw new Error('Kolom tidak ditemukan');const col=toSheetColumnName(columnIndex+1);const range=`${sheetName}!${col}${rowNumber}`;const url=`https://sheets.googleapis.com/v4/spreadsheets/${ASSET_STORE_SPREADSHEET_ID}/values/${encodeURIComponent(range)}?valueInputOption=USER_ENTERED&key=${API_KEY}`;const res=await fetch(url,{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify({values:[[value??'']]})});const out=await res.json();if(!res.ok||out?.error)throw new Error(out?.error?.message||'Gagal update cell');ASSET_STORE_STATE.saveStatus[key]='saved';td.dataset.saveState='saved';td.innerHTML=`${esc(value||'')}<span class='asset-save-badge'>tersimpan</span>`;setTimeout(()=>{if(td.dataset.saveState==='saved'){td.dataset.saveState='';td.textContent=value||'-';}},1200);}catch(err){row[field]=old;td.textContent=old||'-';ASSET_STORE_STATE.saveStatus[key]='error';td.dataset.saveState='error';throw err;}}});}
