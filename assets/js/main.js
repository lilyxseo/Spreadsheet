...
const CYCLE_STORAGE_KEY="cycle_count_draft_v1";
const CYCLE_STATE={step:1,list:[],search:"",session:createCycleSession()};
function createCycleSession(){return{id:`CC-${Date.now()}`,nama:"",tanggal:getTodayDateKey(),pic:"",lokasi:"",catatan:"",status:"draft",selected:{},actual:{},finalized:false};}
function addSkuToSession(sku,row){if(!sku)return;CYCLE_STATE.session.selected[sku]={sku,nama:row.nama||"-",lokasi:row.lokasi||"-",stokSistem:Number(row.stokSistem)||0,catatan:""};}
function removeSkuFromSession(sku){delete CYCLE_STATE.session.selected[sku];delete CYCLE_STATE.session.actual[sku];}
function calculateDifference(actual,system){return (Number(actual)||0)-(Number(system)||0);}
function updateActualQty(sku,qty){CYCLE_STATE.session.actual[sku]=Number(qty)||0;saveCycleDraft();}
function getCycleSummary(){const rows=Object.values(CYCLE_STATE.session.selected);const total=rows.length;let cocok=0,totalSelisih=0,plus=0,minus=0;rows.forEach(r=>{const sel=calculateDifference(CYCLE_STATE.session.actual[r.sku],r.stokSistem);totalSelisih+=sel;if(sel===0)cocok++;else if(sel>0)plus++;else minus++;});return{total,cocok,tidak:total-cocok,totalSelisih,akurasi:total?((cocok/total)*100):0,plus,minus};}
function finalizeSession(){CYCLE_STATE.session.finalized=true;CYCLE_STATE.session.status="selesai";saveCycleDraft();}
function exportCycleCsv(){const rows=Object.values(CYCLE_STATE.session.selected);const header="SKU,Nama,Lokasi,Stok Sistem,Qty Aktual,Selisih\n";const body=rows.map(r=>`${r.sku},${(r.nama||"").replaceAll(","," ")},${r.lokasi},${r.stokSistem},${Number(CYCLE_STATE.session.actual[r.sku]||0)},${calculateDifference(CYCLE_STATE.session.actual[r.sku],r.stokSistem)}`).join("\n");const blob=new Blob([header+body],{type:"text/csv;charset=utf-8;"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`cycle-count-${CYCLE_STATE.session.id}.csv`;a.click();URL.revokeObjectURL(a.href);}
function saveCycleDraft(){localStorage.setItem(CYCLE_STORAGE_KEY,JSON.stringify(CYCLE_STATE.session));toast("Draft cycle count tersimpan","success");}
function loadCycleDraft(){try{const d=JSON.parse(localStorage.getItem(CYCLE_STORAGE_KEY)||"null");if(d&&typeof d==="object")CYCLE_STATE.session={...createCycleSession(),...d};}catch(_){}}
function getCycleDataset(){const src=[...(DATA["RPL"]||[]),...(DATA["BULKY"]||[]),...(DATA["Kartu Stock"]||[])];const map=new Map();src.forEach(r=>{const sku=getVal(r,["sku"]);if(!clean(sku))return;if(map.has(sku))return;map.set(sku,{sku,nama:getVal(r,["nama barang","nama","item","description"])||"-",lokasi:getVal(r,["lokasi","location","rak","bin"])||"-",stokSistem:parseNumber(getVal(r,["stok global","stok akhir","kartu stok","stok"]))});});return [...map.values()];}
function renderCycleCountPage(){if(!CYCLE_STATE.__init){loadCycleDraft();CYCLE_STATE.list=getCycleDataset();CYCLE_STATE.__init=true;}const q=clean(CYCLE_STATE.search);const rows=CYCLE_STATE.list.filter(x=>!q||clean(`${x.sku} ${x.nama}`).includes(q)).slice(0,120);const selected=Object.values(CYCLE_STATE.session.selected);const summary=getCycleSummary();cycleCountRoot.innerHTML=`<div class='cc-wrap'><div class='cc-stepper'>${["Setup","Pilih SKU","Input Aktual","Review","Hasil"].map((s,i)=>`<button class='${CYCLE_STATE.step===i+1?"active":""}' data-cc-step='${i+1}'>${i+1}. ${s}</button>`).join("")}</div>${renderCycleStep(rows,selected,summary)}</div>`;}
function renderCycleStep(rows,selected,summary){if(CYCLE_STATE.step===1)return `<div class='card cc-card'><div class='cc-grid'><input id='ccNama' placeholder='Nama Sesi' value='${esc(CYCLE_STATE.session.nama)}'><input id='ccTanggal' type='date' value='${esc(CYCLE_STATE.session.tanggal)}'><input id='ccPic' placeholder='PIC' value='${esc(CYCLE_STATE.session.pic)}'><input id='ccLokasi' list='ccLokasiList' placeholder='Lokasi (opsional)' value='${esc(CYCLE_STATE.session.lokasi)}'><datalist id='ccLokasiList'>${[...new Set(CYCLE_STATE.list.map(r=>r.lokasi).filter(Boolean))].slice(0,100).map(x=>`<option value='${esc(x)}'>`).join("")}</datalist><textarea id='ccCatatan' placeholder='Catatan'>${esc(CYCLE_STATE.session.catatan)}</textarea></div><div class='row'><button class='btn-ghost' data-cc-save>Simpan Draft</button><button class='btn-primary' data-cc-next>Lanjut</button></div></div>`;
if(CYCLE_STATE.step===2)return `<div class='card cc-card'><div class='mv-toolbar'><input id='ccSearch' placeholder='Search SKU / Nama Barang' value='${esc(CYCLE_STATE.search)}'><div class='row'><button class='btn-ghost' data-cc-pick='minus'>Ambil stok minus</button><button class='btn-ghost' data-cc-pick='anomaly'>Ambil anomaly</button></div></div><div class='cc-chip'>${selected.map(s=>`<span>${s.sku}<button data-cc-del='${encAttr(s.sku)}'>×</button></span>`).join("")||"Belum ada SKU dipilih."}</div><div class='table-wrap'><table><thead><tr><th>SKU</th><th>Nama</th><th>Lokasi</th><th>Stok Sistem</th><th></th></tr></thead><tbody>${rows.map(r=>`<tr><td>${highlight(r.sku,CYCLE_STATE.search)}</td><td>${highlight(r.nama,CYCLE_STATE.search)}</td><td>${esc(r.lokasi)}</td><td>${r.stokSistem}</td><td><button class='btn-ghost' data-cc-add='${encAttr(r.sku)}'>Tambah</button></td></tr>`).join("")}</tbody></table></div><div class='row'><button class='btn-primary' data-cc-next>Lanjut</button></div></div>`;
if(CYCLE_STATE.step===3)return `<div class='card cc-card'><div class='table-wrap table-wrap-full'><table><thead><tr><th>SKU</th><th>Nama</th><th>Lokasi</th><th>Sistem</th><th>Aktual</th><th>Selisih</th><th>Status</th></tr></thead><tbody>${selected.map((r,i)=>{const act=Number(CYCLE_STATE.session.actual[r.sku]||0);const sel=calculateDifference(act,r.stokSistem);const cls=sel===0?"cc-ok":(sel>0?"cc-plus":"cc-minus");return `<tr class='${cls}'><td>${r.sku}</td><td>${esc(r.nama)}</td><td>${esc(r.lokasi)}</td><td>${r.stokSistem}</td><td><input data-cc-qty='${encAttr(r.sku)}' value='${act}' inputmode='numeric' data-idx='${i}'></td><td>${sel}</td><td>${sel===0?"Cocok":(sel>0?"Plus":"Minus")}</td></tr>`;}).join("")}</tbody></table></div><div class='row'><button class='btn-primary' data-cc-next>Lanjut</button></div></div>`;
if(CYCLE_STATE.step===4)return `<div class='card cc-card'><div class='grid dashboard'><div class='metric'><div class='k'>Total SKU</div><div class='v'>${summary.total}</div></div><div class='metric ok'><div class='k'>SKU Cocok</div><div class='v'>${summary.cocok}</div></div><div class='metric err'><div class='k'>Tidak Cocok</div><div class='v'>${summary.tidak}</div></div><div class='metric warn'><div class='k'>Akurasi</div><div class='v'>${summary.akurasi.toFixed(2)}%</div></div></div><div class='row'><button class='btn-primary' data-cc-next>Finalisasi</button></div></div>`;
return `<div class='card cc-card'><div class='subtitle'>${summary.akurasi.toFixed(0)}% SKU sudah sesuai. ${summary.tidak} SKU memiliki selisih.</div><div class='row'><button class='btn-ghost' data-cc-export>Export CSV</button></div></div>`;}
document.addEventListener("click",e=>{const step=e.target.closest("[data-cc-step]");if(step){CYCLE_STATE.step=Number(step.dataset.ccStep)||1;renderCycleCountPage();}if(e.target.matches("[data-cc-next]")){if(CYCLE_STATE.step===4)finalizeSession();CYCLE_STATE.step=Math.min(5,CYCLE_STATE.step+1);renderCycleCountPage();}if(e.target.matches("[data-cc-save]"))saveCycleDraft();const add=e.target.closest("[data-cc-add]");if(add){const sku=decodeURIComponent(add.dataset.ccAdd);const row=CYCLE_STATE.list.find(x=>x.sku===sku);if(row)addSkuToSession(sku,row);renderCycleCountPage();}const del=e.target.closest("[data-cc-del]");if(del){removeSkuFromSession(decodeURIComponent(del.dataset.ccDel));renderCycleCountPage();}if(e.target.matches("[data-cc-export]"))exportCycleCsv();if(e.target.matches("[data-cc-pick='minus']")){(DATA["RPL"]||[]).filter(r=>parseNumber(getVal(r,["selisih"]))<0).slice(0,40).forEach(r=>{const sku=getVal(r,["sku"]);const row=CYCLE_STATE.list.find(x=>x.sku===sku);if(row)addSkuToSession(sku,row);});renderCycleCountPage();}});
document.addEventListener("input",debounce(e=>{if(e.target?.id==="ccSearch"){CYCLE_STATE.search=e.target.value;renderCycleCountPage();}if(e.target?.id==="ccNama")CYCLE_STATE.session.nama=e.target.value;if(e.target?.id==="ccTanggal")CYCLE_STATE.session.tanggal=e.target.value;if(e.target?.id==="ccPic")CYCLE_STATE.session.pic=e.target.value;if(e.target?.id==="ccLokasi")CYCLE_STATE.session.lokasi=e.target.value;if(e.target?.id==="ccCatatan")CYCLE_STATE.session.catatan=e.target.value;if(e.target?.matches("[data-cc-qty]")){updateActualQty(decodeURIComponent(e.target.dataset.ccQty),e.target.value);const next=document.querySelector(`[data-idx='${Number(e.target.dataset.idx)+1}']`);if(next&&e.inputType==="insertText"&&e.data==="\n")next.focus();}},250));
