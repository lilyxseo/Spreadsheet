...
function getCycleSourceRows(){return (DATA["Kartu Stock"]||[]).map(r=>{const sku=String(getVal(r,["sku"])||"").trim();const nama=String(getVal(r,["nama barang","nama","item","description"])||"-").trim()||"-";const lokasi=String(getVal(r,["lokasi","location","rak","bin","area"])||"-").trim()||"-";const stok_akhir=getQty({stok_akhir:getVal(r,["stok akhir","stok_akhir","closing stock","ending stock","saldo akhir"]),stokAkhir:getVal(r,["stokAkhir"]),qty:getVal(r,["qty"]),availableQty:getVal(r,["availableQty"])});return {sku,nama,lokasi,stok_akhir};}).filter(r=>r.sku);}
