...
const ANOMALY_STATE={page:1,pageSize:25,rows:[],filtered:[],q:"",severity:"all",type:"all",period:"all",customStart:"",customEnd:"",headerFilters:{sku:"",lokasi:"",status:"",severity:"",cause:""},rendered:false,isLoading:false,_searchDebounce:null,lastRenderToken:0,lastCheckedAt:null,checked:{},notes:{}};
function normalizeSku(v){return clean(String(v||'').replace(/[–—]/g,'-').replace(/\s+/g,' ').trim().replace(/\s*[-]\s*/g,'-'));}
function normalizeSkuLoose(v){return normalizeSku(v).replace(/[-\s]/g,'');}
function normalizeLocation(v){let s=String(v||'').trim();try{s=decodeURIComponent(s);}catch(_){}return clean(s.replace(/\s+/g,' '));}
...
function refreshAnomalyInBackground(force=false){const token=++ANOMALY_STATE.lastRenderToken;setAnomalyLoading(true,'Menghitung warning terbaru...');logActivitySafe?.({action:'REFRESH_WARNING',module:'Warning',detail:'Refresh warning',status:'SUCCESS'});const run=()=>{const all=buildAnomalyReport();if(token!==ANOMALY_STATE.lastRenderToken)return;ANOMALY_STATE.rows=all;setAnomalyCache(all);applyAnomalyFilters(false);setAnomalyLoading(false);};if(typeof requestIdleCallback==='function'&&!force){requestIdleCallback(run,{timeout:700});return;}setTimeout(run,0);}
