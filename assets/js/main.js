...
document.addEventListener('click',e=>{const btn=e.target.closest('[data-mvm-action]');if(!btn)return;const action=btn.dataset.mvmAction;if(action==='remove'){const idx=Number(btn.dataset.idx);MOVEMENT_STATE.sessionItems.splice(idx,1);renderMovementSessionTable();return;}if(action==='add'){const sku=decAttr(btn.dataset.sku||"");const lokasi=decAttr(btn.dataset.lok||"");const source=getMovementCandidates(MOVEMENT_STATE.search).find(r=>clean(r.sku)===clean(sku)&&clean(r.lokasi)===clean(lokasi));if(source)addMovementItem(source);}});
