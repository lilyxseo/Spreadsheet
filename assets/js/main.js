...
import { logActivity, logActivityResult, logPageView, logLogin, logLogout, logImport, logExport } from "./activity-log.js";
...
const ACTIVITY_LOG_STATE={page:1,pageSize:25,filters:{module:"",action:"",user:"",status:"",search:"",date:""},rows:[],selected:null};
...
window.addEventListener("auth:logout",()=>{logLogout({user:currentUserIdentity().user_name});stopDevAutoRefresh({log:false});showLoginView();});
...
let lastLoggedPath=location.pathname;
function auditRoute(path){if(path===lastLoggedPath)return;const from=lastLoggedPath;lastLoggedPath=path;logPageView({from,to:path,module:"Navigasi"});}
function navigateTo(path){if(path===location.pathname)return;history.pushState({},"",path);routeFromPath(path);auditRoute(path);}
...
logImport({module:'Import PDF Transfer',importType:'PDF/CSV',sheet:out.sheetName||PDF_TRANSFER_STATE.selectedSheetName,validRows:valid.length,failedRows:0,durationMs:Math.round(performance.now()-activityStartedAt),result:'SUCCESS'});
}catch(err){logImport({module:'Import PDF Transfer',importType:'PDF/CSV',sheet:PDF_TRANSFER_STATE.selectedSheetName,validRows:0,failedRows:valid.length,durationMs:Math.round(performance.now()-activityStartedAt),result:'FAILED',description:`Import PDF ke ${PDF_TRANSFER_STATE.selectedSheetName} gagal: ${err.message||err}.`,details:{error:String(err.message||err)}});toast(`Import gagal: ${err.message||err}`,'error');}finally{PDF_TRANSFER_STATE.isImporting=false;renderImportPdfTransferPage();}
}
async function fetchActivityLogs(){const off=(ACTIVITY_LOG_STATE.page-1)*ACTIVITY_LOG_STATE.pageSize;const f=ACTIVITY_LOG_STATE.filters;const qs=new URLSearchParams({limit:String(ACTIVITY_LOG_STATE.pageSize),offset:String(off)});if(f.module)qs.set("module",f.module);if(f.action)qs.set("action",f.action);if(f.user)qs.set("user_name",f.user);if(f.status)qs.set("status",f.status);if(f.search)qs.set("search",f.search);if(f.date){qs.set("from",`${f.date}T00:00:00.000Z`);qs.set("to",`${f.date}T23:59:59.999Z`);}const headers=await getActivityLogAccessHeaders();const res=await fetch(`/api/activity-log?${qs}`,{headers});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||"Gagal memuat activity log");return {rows:Array.isArray(data.data)?data.data:[],hasMore:data.hasMore===true};}
const activityBadge=x=>`<span class="al-badge al-${esc(String(x||'unknown').toLowerCase().replaceAll('_','-'))}">${esc(x||'-')}</span>`;
const activityChanges=x=>{const changes=x.details?.changes||x.metadata?.details?.changes||[];return changes.length?`<div class="al-change-mini">${changes.slice(0,2).map(c=>`<b>${esc(c.field)}</b>: ${esc(c.oldValue??'kosong')} <span>→</span> ${esc(c.newValue??'kosong')}`).join('<br>')}</div>`:'';};
function closeActivityDrawer(){ACTIVITY_LOG_STATE.selected=null;document.getElementById('activityDrawer')?.remove();}
function openActivityDrawer(id){const x=ACTIVITY_LOG_STATE.rows.find(r=>String(r.id)===String(id));if(!x)return;ACTIVITY_LOG_STATE.selected=x;const changes=x.details?.changes||[];const masked=x.sessionId?`${String(x.sessionId).slice(0,6)}••••${String(x.sessionId).slice(-4)}`:'-';document.body.insertAdjacentHTML('beforeend',`<div id="activityDrawer" class="al-drawer-backdrop" onclick="if(event.target===this)closeActivityDrawer()"><aside class="al-drawer"><header><div><small>DETAIL AKTIVITAS</small><h3>${activityBadge(x.action)} ${esc(x.module||'-')}</h3></div><button class="btn-ghost" onclick="closeActivityDrawer()">✕</button></header><p class="al-description">${esc(x.description||'-')}</p><dl class="al-detail-grid"><div><dt>Activity ID</dt><dd>${esc(x.id||'-')}</dd></div><div><dt>Timestamp</dt><dd>${esc(new Date(x.timestamp||x.created_at).toLocaleString('id-ID'))}</dd></div><div><dt>User / Role</dt><dd>${esc(x.user||x.user_name||'-')} · ${esc(x.role||'-')}</dd></div><div><dt>Developer</dt><dd>${x.isDeveloper?'Ya':'Tidak'}</dd></div><div><dt>Session ID</dt><dd>${esc(masked)}</dd></div><div><dt>Page</dt><dd>${esc(x.page||'-')}</dd></div><div><dt>Entity</dt><dd>${esc(x.entityType||'-')} ${esc(x.entityId||'')}</dd></div><div><dt>Source / Result</dt><dd>${esc(x.source||'-')} · ${activityBadge(x.result)}</dd></div></dl>${changes.length?`<h4>Perubahan</h4><div class="al-changes">${changes.map(c=>`<div><b>${esc(c.field)}</b><span><small>Sebelum</small>${esc(c.oldValue??'kosong')}</span><i>→</i><span><small>Sesudah</small>${esc(c.newValue??'kosong')}</span></div>`).join('')}</div>`:''}${x.details?.error?`<div class="state error">${esc(x.details.error)}</div>`:''}</aside></div>`);}
window.openActivityDrawer=openActivityDrawer;window.closeActivityDrawer=closeActivityDrawer;
async function renderActivityLogPage(){const root=document.getElementById("activityLogApp");if(!root)return;let result={rows:[],hasMore:false},err="";try{result=await fetchActivityLogs();}catch(e){err=e?.message||"Gagal memuat activity log";}const rows=result.rows;ACTIVITY_LOG_STATE.rows=rows;const today=new Date().toDateString();const todayRows=rows.filter(x=>new Date(x.timestamp||x.created_at).toDateString()===today);const count=p=>todayRows.filter(p).length;const f=ACTIVITY_LOG_STATE.filters;const actions=['','LOGIN','AUTO_LOGIN','PAGE_VIEW','CREATE','UPDATE','DELETE','IMPORT','EXPORT','MOVEMENT','CYCLE_COUNT','UNDO','REDO','LOGOUT'];root.innerHTML=`<div class="al-summary"><div><small>Aktivitas Hari Ini</small><b>${todayRows.length}</b></div><div><small>Login</small><b>${count(x=>['LOGIN','AUTO_LOGIN'].includes(x.action))}</b></div><div><small>CRUD</small><b>${count(x=>x.category==='CRUD')}</b></div><div class="danger"><small>Gagal / Ditolak</small><b>${count(x=>['FAILED','DENIED'].includes(x.result))}</b></div></div><div class="card cc-card cc-section"><div class="al-quick-filters">${[['','Semua Aktivitas'],['LOGIN','Login'],['PAGE_VIEW','Navigasi'],['CREATE','Create'],['UPDATE','Update'],['DELETE','Delete'],['IMPORT','Import'],['EXPORT','Export'],['MOVEMENT','Movement'],['UNDO','Undo/Redo'],['LOGOUT','Logout'],['FAILED','Failed'],['DENIED','Denied']].map(([v,l])=>`<button class="${(f.action===v||f.status===v)?'active':''}" data-al-quick="${v}">${l}</button>`).join('')}</div><div class="mv-filters open al-filters"><input id="alSearch" class="search-lg" placeholder="Cari SKU, user, modul, deskripsi, lokasi, Activity ID" value="${esc(f.search)}"><input id="alUser" placeholder="User" value="${esc(f.user)}"><input id="alModule" placeholder="Modul" value="${esc(f.module)}"><input id="alDate" type="date" value="${esc(f.date)}"><select id="alAction">${actions.map(v=>`<option value="${v}">${v||'Semua Action'}</option>`).join('')}</select><select id="alStatus"><option value="">Semua Result</option><option>SUCCESS</option><option>FAILED</option><option>DENIED</option></select><button id="alApply" class="btn-primary">Terapkan</button></div>${err?`<div class='state error'>${esc(err)}</div>`:''}<div class="table-wrap al-table"><table><thead><tr><th>Waktu</th><th>User</th><th>Aktivitas</th><th>Modul</th><th>Detail</th><th>Result</th><th>Aksi</th></tr></thead><tbody>${rows.length?rows.map(x=>`<tr><td>${esc(formatDateTime(x.timestamp||x.created_at))}</td><td><b>${esc(x.user||x.user_name||'-')}</b><small>${esc(x.role||'-')}</small></td><td>${activityBadge(x.action)}</td><td>${esc(x.module||'-')}<small>${esc(x.page||'')}</small></td><td><div>${esc(x.description||x.detail||'-')}</div>${activityChanges(x)}</td><td>${activityBadge(x.result||x.status)}</td><td><button class="btn-ghost" onclick="openActivityDrawer('${encAttr(x.id)}')">Lihat</button></td></tr>`).join(''):`<tr><td colspan='7'><div class='state'>Belum ada aktivitas yang cocok.</div></td></tr>`}</tbody></table></div><div class='mv-pagination'><span>Halaman ${ACTIVITY_LOG_STATE.page}</span><div class='row'><button id='alPrev' class='btn-ghost' ${ACTIVITY_LOG_STATE.page===1?'disabled':''}>Sebelumnya</button><button id='alNext' class='btn-ghost' ${!result.hasMore?'disabled':''}>Berikutnya</button></div></div></div>`;document.getElementById('alAction').value=f.action;document.getElementById('alStatus').value=f.status;document.getElementById('alApply').onclick=()=>{ACTIVITY_LOG_STATE.filters={search:alSearch.value.trim(),user:alUser.value.trim(),module:alModule.value.trim(),date:alDate.value,action:alAction.value,status:alStatus.value};ACTIVITY_LOG_STATE.page=1;renderActivityLogPage();};root.querySelectorAll('[data-al-quick]').forEach(b=>b.onclick=()=>{const v=b.dataset.alQuick;f.action=['FAILED','DENIED'].includes(v)?'':v==='UNDO'?'UNDO':v;f.status=['FAILED','DENIED'].includes(v)?v:'';ACTIVITY_LOG_STATE.page=1;renderActivityLogPage();});document.getElementById('alPrev').onclick=()=>{ACTIVITY_LOG_STATE.page--;renderActivityLogPage();};document.getElementById('alNext').onclick=()=>{ACTIVITY_LOG_STATE.page++;renderActivityLogPage();};}
