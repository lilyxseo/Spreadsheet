...
async function loadMovementSheet({mode='full',limit=1000,preserveState=true,target='in'}={}){const endpoint=target==='in'?'/api/barang-masuk':'/api/barang-keluar';const qp=mode==='latest'?`?mode=latest&limit=${Number(limit)||1000}`:'?mode=full';const res=await fetch(endpoint+qp);const out=await res.json();if(!res.ok||!out?.success)throw new Error(out?.message||'Gagal memuat data');const sheetName=target==='in'?'Barang Masuk':'Barang Keluar';DATA[sheetName]=(out.rows||[]).map(r=>({tanggal:r.tanggal||'',from:r.from||'',to:r.to||'',sku:r.sku||'',namaBarang:r.namaBarang||'',qty:r.qty||'',status:r.status||'',pic:r.pic||'',keterangan:r.keterangan||'',rowNumber:r.rowNumber}));TABLE_STATE[target].rows=DATA[sheetName].map(r=>({...r,rowNumber:Number(r.rowNumber)||0}));renderDataTablePage(target,sheetName,!!preserveState);await saveCache(DATA);}
async function refreshMovementAfterCrud(target){await loadMovementSheet({mode:'latest',limit:1000,preserveState:true,target});}
