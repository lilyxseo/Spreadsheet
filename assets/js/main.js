...
const BALIKAN_STATE={sheets:[],highlightRowNumber:null,sortBy:"default",autoCheckOnScan:true,exactScanSku:"",selectedSkuRowNumber:null,selectedSkuValue:"",lastCheckedRowId:null,lastCheckedSku:"",lastCheckedVersion:0,lastCheckedFadeTimer:null,pendingEdits:{},pendingEditMeta:{},saveStatus:{},saveTimer:null,saveInProgress:false,saveRequested:false,isRendering:false,isRefreshing:false,pendingRender:false,lastRenderedChecksum:"",lastDataChecksum:"",lastRenderedHeaderKey:"",renderTimer:null,searchDebounceTimer:null};
...
balikanSheetSelect?.addEventListener("change",async(e)=>{window.currentTripSheet=e.target.value||"";BALIKAN_STATE.highlightRowNumber=null;BALIKAN_STATE.selectedSkuRowNumber=null;BALIKAN_STATE.selectedSkuValue="";await loadBalikanRows();});
balikanSearchInput?.addEventListener("input",e=>{window.balikanSearchKeyword=e.target.value||"";BALIKAN_STATE.exactScanSku="";BALIKAN_STATE.selectedSkuRowNumber=null;BALIKAN_STATE.selectedSkuValue="";scheduleBalikanRender(false,300);});
...
async function triggerManualRefresh(){logActivitySafe({action:'MANUAL_REFRESH',module:'System',detail:'Manual refresh dimulai',status:'SUCCESS'});if(getActivePage?.()==='balikan-store'&&window.currentTripSheet){loadBalikanRows({background:true,force:true});}if(isSyncing)return;await loadAllData(true);}
...
async function loadBalikanRows(options={}){const {background=false,force=false}=options||{};if(!window.currentTripSheet){balikanSummary.textContent='';balikanTable.innerHTML='';return;}if(BALIKAN_STATE.isRefreshing)return;const cache=getBalikanRowsCache(window.currentTripSheet);if(cache&&!force){window.BALIKAN_ROWS=Array.isArray(cache.rows)?cache.rows:[];window.BALIKAN_DYNAMIC_COLUMNS=Array.isArray(cache.dynamicColumns)?cache.dynamicColumns:[];BALIKAN_STATE.lastDataChecksum=cache.checksum||checksumBalikanRows(window.BALIKAN_ROWS,window.BALIKAN_DYNAMIC_COLUMNS);applyPendingBalikanEditsToRows();renderBalikanTable(true);}
try{BALIKAN_STATE.isRefreshing=true;if(balikanSortSelect)balikanSortSelect.value=BALIKAN_STATE.sortBy||'default';syncBalikanAutoCheckToggle();const res=await fetch(`/api/balikan-store?sheetName=${encodeURIComponent(window.currentTripSheet)}`,{cache:'no-store'});const data=await res.json();if(!res.ok)throw new Error(data?.message||'Gagal memuat data Balikan Store');const nextRows=Array.isArray(data?.rows)?data.rows:[];const nextDynamic=Array.isArray(data?.dynamicColumns)?data.dynamicColumns:[];const nextChecksum=checksumBalikanRows(nextRows,nextDynamic);if(background&&BALIKAN_STATE.lastDataChecksum&&BALIKAN_STATE.lastDataChecksum===nextChecksum){return;}window.BALIKAN_ROWS=nextRows;window.BALIKAN_DYNAMIC_COLUMNS=nextDynamic;window.currentTripSheet=window.currentTripSheet||"";BALIKAN_STATE.lastDataChecksum=nextChecksum;setBalikanRowsCache(window.currentTripSheet,{rows:nextRows,dynamicColumns:nextDynamic,checksum:nextChecksum,updatedAt:Date.now()});applyPendingBalikanEditsToRows();renderBalikanTable(true);}catch(err){if(!background)toast(err?.message||'Gagal memuat data Balikan Store','error');}finally{BALIKAN_STATE.isRefreshing=false;}}
...
function applyBalikanTableFilters(rows,mode='balikan',omitCol=''){const st=ensureBalikanFilterState();const q=clean(window.balikanSearchKeyword||'');const exactScanSku=clean(BALIKAN_STATE.exactScanSku||'');const selectedSku=clean(BALIKAN_STATE.selectedSkuValue||'');const dynamicCols=Array.isArray(window.BALIKAN_DYNAMIC_COLUMNS)?window.BALIKAN_DYNAMIC_COLUMNS:[];return rows.filter(r=>{if(BALIKAN_FILTERABLE_COLUMNS.some(col=>{if(col===omitCol)return false;const selected=st.columnFilters[col]||[];if(!selected.length)return false;return !selected.includes(sanitizeFilterValue(r[col]));}))return false;if(exactScanSku&&clean(r.sku)!==exactScanSku)return false;if(selectedSku&&q===selectedSku&&clean(r.sku)!==selectedSku)return false;const dynamicSearch=dynamicCols.map(col=>r?.[col.key]??'').join(' ');if(q&&!clean(`${r.sku} ${r.namaBarang} ${r.lokasi} ${r.status} ${r.keterangan} ${r.rakTujuan} ${dynamicSearch}`).includes(q))return false;return true;});}
...
function selectBalikanSkuRow(rowNumber){
  ...
  if(!nextRow)return;
  const selectedRow=(window.BALIKAN_ROWS||[]).find(row=>Number(row?.rowNumber)===nextRow);
  ...
  window.setTimeout(()=>scrollToBalikanRow(nextRow),0);
}
