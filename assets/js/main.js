...
function renderEditableCell(row,field,value){const td=document.createElement("td");td.className="editable-cell";td.textContent=value||"-";td.addEventListener("click",()=>{startInlineEdit(td,row,field,value);});return td;}
function startInlineEdit(td,row,field,oldValue){if(td.querySelector("input,select"))return;const input=document.createElement("input");input.value=oldValue||"";input.className="inline-edit-input";td.innerHTML="";td.appendChild(input);input.focus();input.select();let cancelled=false;async function save(){if(cancelled)return;const newValue=input.value.trim();if(newValue===String(oldValue||"")){td.textContent=oldValue||"-";return;}try{await updateBalikanCell(window.currentTripSheet,row,field,newValue,oldValue);}catch(err){toast(err?.message||"Gagal update data","error");td.textContent=oldValue||"-";}}input.addEventListener("blur",save);input.addEventListener("keydown",(e)=>{if(e.key==="Enter")input.blur();if(e.key==="Escape"){cancelled=true;td.textContent=oldValue||"-";}});} 
async function updateBalikanCell(sheetName,row,field,value,oldValue){const res=await fetch('/api/balikan-store/cell',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({sheetName,rowNumber:row.rowNumber,field,value})});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal update data');showToast('Data berhasil diupdate','success');logActivitySafe({action:'EDIT_BALIKAN_STORE',module:'Balikan Store',detail:`Edit field ${field} SKU ${row?.sku||'-'} di ${sheetName}`,status:'SUCCESS',metadata:{sheetName,rowNumber:row.rowNumber,sku:row?.sku||'',field,oldValue:String(oldValue??''),newValue:String(value??'')}});await loadBalikanRows();}
function renderBalikanTable(){const rows=filterBalikanRows(window.BALIKAN_ROWS||[],window.balikanSearchKeyword);if(!rows.length){balikanTable.innerHTML='<div class="subtitle">Data kosong.</div>';return;}balikanTable.innerHTML=`<div class="balikan-table-wrapper"><table class="balikan-table"><thead><tr><th class="col-check">Centang</th><th>No</th><th>SKU</th><th>Nama Barang</th><th>Qty</th><th>Rak tujuan</th><th>Lokasi</th><th>Stok Bulky</th><th>Stok Retail</th><th>Status</th><th>Keterangan</th></tr></thead><tbody></tbody></table></div>`;const tbody=balikanTable.querySelector("tbody");if(!tbody)return;const frag=document.createDocumentFragment();rows.forEach(row=>{const tr=document.createElement("tr");const rowNumber=Number(row.rowNumber);tr.id=`balikan-row-${rowNumber}`;tr.dataset.rowNumber=String(rowNumber);if(BALIKAN_STATE.highlightRowNumber===rowNumber)tr.className="balikan-row-highlight";const tdCheck=document.createElement("td");tdCheck.className="col-check";const checkbox=document.createElement("input");checkbox.type="checkbox";checkbox.className="balikan-check";checkbox.checked=isCheckedValue(row.checked);checkbox.addEventListener("change",()=>{toggleBalikanCheck(window.currentTripSheet,row.rowNumber,checkbox.checked);});tdCheck.appendChild(checkbox);tr.appendChild(tdCheck);[row.no,row.sku,row.namaBarang].forEach(value=>{const td=document.createElement("td");td.textContent=String(value??"");tr.appendChild(td);});const editableFields=['qty','rakTujuan','lokasi','stokBulky','stokRetail','status','keterangan'];editableFields.forEach(field=>{tr.appendChild(renderEditableCell(row,field,row[field]));});frag.appendChild(tr);});tbody.replaceChildren(frag);}
function renderBalikanTable(){const rows=filterBalikanRows(window.BALIKAN_ROWS||[],window.balikanSearchKeyword);if(!rows.length){balikanTable.innerHTML='<div class="subtitle">Data kosong.</div>';return;}balikanTable.innerHTML=`<div class="balikan-table-wrapper"><table class="balikan-table"><thead><tr><th class="col-check">Centang</th><th>No</th><th>SKU</th><th>Nama Barang</th><th>Qty</th><th>Rak tujuan</th><th>Lokasi</th><th>Stok Bulky</th><th>Stok Retail</th><th>Status</th><th>Keterangan</th></tr></thead><tbody></tbody></table></div>`;const tbody=balikanTable.querySelector("tbody");if(!tbody)return;const frag=document.createDocumentFragment();rows.forEach(row=>{const tr=document.createElement("tr");const rowNumber=Number(row.rowNumber);tr.id=`balikan-row-${rowNumber}`;tr.dataset.rowNumber=String(rowNumber);if(BALIKAN_STATE.highlightRowNumber===rowNumber)tr.className="balikan-row-highlight";const tdCheck=document.createElement("td");tdCheck.className="col-check";const checkbox=document.createElement("input");checkbox.type="checkbox";checkbox.className="balikan-check";checkbox.checked=isCheckedValue(row.checked);checkbox.addEventListener("change",()=>{toggleBalikanCheck(window.currentTripSheet,row.rowNumber,checkbox.checked);});tdCheck.appendChild(checkbox);tr.appendChild(tdCheck);const columns=[row.no,row.sku,row.namaBarang,row.qty,row.rakTujuan,row.lokasi,row.stokBulky,row.stokRetail,row.status,row.keterangan];columns.forEach(value=>{const td=document.createElement("td");td.textContent=String(value??"");tr.appendChild(td);});frag.appendChild(tr);});tbody.replaceChildren(frag);}
