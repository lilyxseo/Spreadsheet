...
const qtyRaw=String(row[map.qty]||'').trim().replace(/\./g,'').replace(',', '.');
const namaProduk=String(map.namaProduk!==undefined?row[map.namaProduk]||'':'').trim();
const diterima=String(map.diterima!==undefined?row[map.diterima]||'':'').trim();
const batal=String(map.batal!==undefined?row[map.batal]||'':'').trim();
const tolak=String(map.tolak!==undefined?row[map.tolak]||'':'').trim();
const catatan=String(map.catatan!==undefined?row[map.catatan]||'':'').trim();
const reasons=[];
if(!sku)reasons.push('SKU kosong');
const qty=Number(qtyRaw);
if(!qtyRaw||Number.isNaN(qty))reasons.push('Jumlah/Qty bukan angka');
if(reasons.length){failedRows.push({rowNumber:r+1,row,reason:reasons.join(', ')});continue;}
items.push({sku,namaProduk,qty,diterima,batal,tolak,catatan});
}
...
const expected=['sku','nama produk','jumlah','diterima','batal','tolak','catatan'];
for(let ri=0;ri<Math.min(pdfRows.length,60);ri++){
const row=pdfRows[ri];
const text=row.items.map(it=>it.str).join(' ');
const norm=normalizeTransferCsvHeader(text);
const score=expected.reduce((n,label)=>n+(norm.includes(label)?1:0),0);
if(score<3||!norm.includes('sku'))continue;
const anchors=[];
const used=new Set();
...
const one=normalizeTransferCsvHeader(row.items[i].str);
const two=normalizeTransferCsvHeader(`${row.items[i]?.str||''} ${row.items[i+1]?.str||''}`);
if(one===label||two===label||one.includes(label)||two.includes(label)||parts.includes(one)){found=i;break;}
}
if(found>=0){used.add(found);anchors.push({label,x:getPdfItemX(row.items[found])});}
}
if(anchors.length>=3){
anchors.sort((a,b)=>a.x-b.x);
return {headerRowIndex:ri,columns:anchors};
}
}
...
const csvRows=[columns.map(c=>c.label.replace(/\b\w/g,m=>m.toUpperCase()))];
for(let ri=detected.headerRowIndex+1;ri<pdfRows.length;ri++){
const row=pdfRows[ri];
const rowText=row.items.map(it=>it.str).join(' ').replace(/\s+/g,' ').trim();
if(!rowText)continue;
if(csvRows.length>1&&/^(grand\s+total|total|status\s+transfer|nomor\s+referensi)\b/i.test(rowText))break;
const cells=columns.map(()=>[]);
for(const item of row.items){
const x=getPdfItemX(item);
let ci=0;
for(let b=0;b<columns.length;b++){if(x>=boundaries[b]&&x<boundaries[b+1]){ci=b;break;}}
cells[ci].push(String(item.str||'').trim());
}
const out=cells.map(parts=>parts.join(' ').replace(/\s+/g,' ').trim());
if(out.some(Boolean))csvRows.push(out);
}
return csvRows;
}
...
const csvPreview=(PDF_TRANSFER_STATE.csvRows||[]).slice(0,20);
...
PDF_TRANSFER_STATE.isParsing=true;PDF_TRANSFER_STATE.logs=[];PDF_TRANSFER_STATE.warnings=[];PDF_TRANSFER_STATE.items=[];PDF_TRANSFER_STATE.failedRows=[];PDF_TRANSFER_STATE.detectedColumns=[];PDF_TRANSFER_STATE.csvRows=[];PDF_TRANSFER_STATE.csvText='';PDF_TRANSFER_STATE.lastFileName=file.name||'';renderImportPdfTransferPage();
try{
...
const csvRows=pdfRowsToCsvRows(pdfRows);
...
const parsed=parseTransferCsvRows(parseCsvText(PDF_TRANSFER_STATE.csvText));
