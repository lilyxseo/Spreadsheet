...
function preserveBalikanTableViewport(){
  const wrapper=document.querySelector('.balikan-table-wrapper');
  const activeElement=document.activeElement;
  const activeSelector=activeElement?.id?`#${activeElement.id}`:activeElement?.dataset?.rowNumber?`[data-row-number="${activeElement.dataset.rowNumber}"]`:activeElement?.className?`.${String(activeElement.className).trim().replace(/\s+/g,'.')}`:"";
  return {scrollTop:wrapper?.scrollTop||0,scrollLeft:wrapper?.scrollLeft||0,activeSelector};
}
function restoreBalikanTableViewport(viewport){
  const wrapper=document.querySelector('.balikan-table-wrapper');
  if(wrapper){
    wrapper.scrollTop=viewport?.scrollTop||0;
    wrapper.scrollLeft=viewport?.scrollLeft||0;
  }
  if(viewport?.activeSelector&&document.querySelector('#page-balikan-store')?.contains(document.activeElement)){
    const target=document.querySelector(viewport.activeSelector);
...
async function updateBalikanCheck(sheetName,rowNumber,checked){const viewport=preserveBalikanTableViewport();const targetSheet=sheetName||window.currentTripSheet;const res=await fetch('/api/balikan-store/check',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({sheetName:targetSheet,rowNumber,checked})});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal update centang');const row=(window.BALIKAN_ROWS||[]).find(r=>Number(r.rowNumber)===Number(rowNumber));const oldChecked=String(row?.checked).toUpperCase()==='TRUE';if(row)row.checked=checked?'TRUE':'FALSE';const checkbox=document.querySelector(`#balikan-row-${Number(rowNumber)} .balikan-check`);if(checkbox)checkbox.checked=!!checked;restoreBalikanTableViewport(viewport);toast('Centang berhasil diupdate','success');logActivitySafe({action:'UPDATE_BALIKAN_STORE_CHECK',module:'Balikan Store',detail:`[CHECK] SKU: ${row?.sku||'-'}\n${oldChecked?'FALSE':'TRUE'} → ${checked?'TRUE':'FALSE'}`,status:'SUCCESS',metadata:{sheetName:targetSheet,rowNumber,sku:row?.sku||'',oldValue:oldChecked,newValue:checked,source:'table_check'}});}
window.toggleBalikanCheck=async(sheetName,rowNumber,checked)=>{const normalizedChecked=checked===true;const targetCheckbox=document.querySelector(`#balikan-row-${Number(rowNumber)} .balikan-check`);try{await updateBalikanCheck(sheetName,Number(rowNumber),normalizedChecked);}catch(err){if(targetCheckbox)targetCheckbox.checked=!normalizedChecked;toast(err?.message||'Gagal update centang','error');throw err;}};
