...
import { buildAdditionalWarnings, deduplicateWarnings, normalizeProductName } from "./utils/warning-engine.js";
...
Object.keys(outTotals).forEach(sku=>{const names=[...(skuNameDisplay[sku]?.values()||[])];if(!inTotals[sku])rows.push({type:'OUTBOUND_WITHOUT_INBOUND',severity:'High',sku:getSkuDisplay(sku),nama:names[0]||'-',issue:'SKU keluar tanpa data masuk',source:'Barang Keluar',recommendation:'Verifikasi transaksi barang masuk sebelum pengeluaran.'});if(outTotals[sku]>(inTotals[sku]||0)){const diff=outTotals[sku]-(inTotals[sku]||0);rows.push({type:'OUTBOUND_EXCEEDS_INBOUND',severity:'High',sku:getSkuDisplay(sku),nama:names[0]||'-',issue:`Qty keluar ${outTotals[sku]} pcs > qty masuk ${inTotals[sku]||0} pcs (selisih ${diff})`,source:'Barang Masuk/Barang Keluar',recommendation:'Periksa transaksi Barang Masuk yang belum tercatat atau koreksi Qty Barang Keluar.',detail:{scope:'Perbandingan kumulatif',inboundQty:inTotals[sku]||0,outboundQty:outTotals[sku],difference:diff}});}});
...
const knownSpecialNames=new Set(["AREA OUTBOUND","EXPIRED","RUANG TR 1","RUANG TR 2","AREA HOLD","MASIH DI INBOUND","RAK RPL","REJECT"]);const specialLocations=[...new Set([...DATA["Kartu Stock"]||[],...rpl,...bulky].map(r=>String(getVal(r,["lokasi","location","rak","bin","area"])||"").trim().toUpperCase()).filter(value=>knownSpecialNames.has(value)))];const lastSyncAt=Number(localStorage.getItem(CACHE_KEYS.lastSync)||0),sourcesSynchronized=lastSyncAt>0&&Date.now()-lastSyncAt<AUTO_SYNC_INTERVAL_MS*2;const additional=buildAdditionalWarnings({stock:DATA["Kartu Stock"]||[],outbound:keluar,inbound:masuk,rpl,bulky,specialLocations,sourcesSynchronized});return deduplicateWarnings([...rows,...additional]);}
function sevClass(s){return s==='High'?'b-high':s==='Medium'?'b-medium':'b-low';}
function getWarningSourceVersion(){return [REFRESH_STATE.dataVersion,DATA["Kartu Stock"]?.length||0,getBarangMasukRows().length,getBarangKeluarRows().length,DATA["RPL"]?.length||0,DATA["BULKY"]?.length||0].join("|");}
...
function getAnomalyFilteredRows(){const q=clean(ANOMALY_STATE.q);return (ANOMALY_STATE.rows||[]).filter(r=>{if(ANOMALY_STATE.severity!=='all'&&r.severity!==ANOMALY_STATE.severity)return false;if(ANOMALY_STATE.type!=='all'&&anomalyTypeKey(r)!==ANOMALY_STATE.type)return false;if(q&&!clean(`${r.sku} ${r.nama} ${r.issue} ${r.source}`).includes(q))return false;return true;});}
