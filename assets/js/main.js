...
function buildStokMinusRows(){const map=new Map();const add=(sheet,type)=>{for(const row of (DATA[sheet]||[])){const sku=String(getVal(row,["sku"])||"").trim();if(!sku)continue;const key=clean(sku);const nama=String(getVal(row,["nama barang","nama","item","description"])||"-").trim()||"-";const qty=Math.abs(parseNumber(getVal(row,["qty"])));if(!map.has(key))map.set(key,{sku,nama,totalMasuk:0,totalKeluar:0,inRows:[],outRows:[]});const it=map.get(key);if(it.nama==="-"&&nama!=="-")it.nama=nama;if(type==="in"){it.totalMasuk+=qty;it.inRows.push(row);}else{it.totalKeluar+=qty;it.outRows.push(row);}}};add("Barang Masuk","in");add("Barang Keluar","out");const cards=(DATA["Kartu Stock"]||[]).concat(DATA["RPL"]||[],DATA["BULKY"]||[]);const bySku=(rows)=>rows.filter(r=>clean(getVal(r,["sku"])));const lookup={};["Kartu Stock","RPL","BULKY"].forEach(k=>lookup[k]=bySku(DATA[k]||[]));const out=[];for(const it of map.values()){const stokEstimate=it.totalMasuk-it.totalKeluar;const keluarTanpaMasuk=it.totalKeluar>0&&it.totalMasuk<=0;const isMinus=stokEstimate<0||it.totalKeluar>it.totalMasuk||keluarTanpaMasuk;if(!isMinus)continue;out.push({...it,stokEstimate,selisih:stokEstimate,status:keluarTanpaMasuk?"Keluar Tanpa Masuk":"Stok Minus",kartuRows:lookup["Kartu Stock"].filter(r=>clean(getVal(r,["sku"]))===clean(it.sku)),rplRows:lookup["RPL"].filter(r=>clean(getVal(r,["sku"]))===clean(it.sku)),bulkyRows:lookup["BULKY"].filter(r=>clean(getVal(r,["sku"]))===clean(it.sku))});}return out;}
