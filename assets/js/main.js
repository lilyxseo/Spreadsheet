...
abcAnalisisApp.querySelectorAll("[data-abc-copy-sku]").forEach(btn=>btn.onclick=()=>{const sku=btn.dataset.abcCopySku||"";if(!sku)return;navigator.clipboard.writeText(sku).then(()=>toast('SKU berhasil dicopy','success')).catch(()=>toast('Gagal copy SKU','error'));});
if(window.lucide)window.lucide.createIcons();
}
