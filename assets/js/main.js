...
function findInventoryItemBySku(sku){const normalizedSku=String(sku||"").trim();return (getMovementSourceRows()||[]).find(item=>String(item.sku||item.SKU||"").trim()===normalizedSku);}
async function submitMovement(payload){const res=await fetch('/api/movement/in',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal menyimpan barang masuk');return data;}
async function handleMovementScanSubmit(sku){if(isSubmittingMovementScan)return;isSubmittingMovementScan=true;try{const from=document.getElementById('movementFrom')?.value?.trim();const to=document.getElementById('movementTo')?.value?.trim();const qty=document.getElementById('movementQty')?.value;if(!from){showToast('From wajib diisi','error');return;}if(!to){showToast('To wajib diisi','error');return;}if(!qty||Number(qty)<=0){showToast('Qty wajib lebih dari 0','error');return;}const item=findInventoryItemBySku(sku);if(!item){showToast('SKU tidak ditemukan','error');return;}const rawRole=window.currentUser?.role||'';const normalizedRole=normalizeRole(rawRole);const pic=APP_CONFIG.PIC_BY_ROLE[normalizedRole]||APP_CONFIG.PIC_BY_ROLE.inventory||'ABI';const tanggal=formatTanggal(new Date());await submitMovement({tanggal,sku,namaBarang:item.nama||item.namaBarang||item.name||'',from,to,qty:Number(qty),pic,stokDiLokasiAwal:Number(item.stok_akhir??item.stok??item.qty??0),stokAktual:Number(qty),source:'scan'});logActivitySafe({action:'CREATE_MOVEMENT',module:'Movement',detail:`[CREATE] Movement Scan
SKU: ${sku}
Qty: ${Number(qty)}
Status: Barang Masuk`,status:'SUCCESS',metadata:{inventorySheet:'Movement',inventorySpreadsheet:'SHEET_ID_INVENTORY',syncedTo2026:true,tanggal,from,to,sku,namaBarang:item.nama||item.namaBarang||item.name||'',qty:Number(qty),status:'Barang Masuk',pic,keterangan:'INTERNAL STOCK TRANSFER',source:'scan'}});showToast('Movement berhasil disimpan','success');MOVEMENT_HISTORY_REMOTE.loaded=false;await ensureMovementHistoryLoaded();if(typeof syncData==='function')syncData();}catch(err){console.error(err);showToast('Gagal submit movement','error');}finally{isSubmittingMovementScan=false;}}
function initMovementScanner(){const input=document.getElementById('movementScanInput');if(!input||input.dataset.bound==='1')return;input.addEventListener('keydown',async e=>{if(e.key!=='Enter')return;e.preventDefault();const sku=input.value.trim();if(!sku)return;await handleMovementScanSubmit(sku);input.value='';input.focus();});input.dataset.bound='1';}
