...
const MOVEMENT_HISTORY_RANGE="Movement!A1:ZZ";
...
function findMovementHeaderRow(values){for(let i=0;i<values.length;i++){const norm=(values[i]||[]).map(normalizeMovementHeader);const hasTanggal=norm.includes("tanggal")||norm.includes("date");const hasFrom=norm.includes("from")||norm.includes("asal");const hasTo=norm.includes("to")||norm.includes("tujuan");if(hasTanggal&&hasFrom&&hasTo&&norm.includes("sku"))return i;}return -1;}
function mapMovementHeaderIndex(headerRow){const normalized=(headerRow||[]).map(normalizeMovementHeader);const aliases={tanggal:["tanggal","date"],from:["from","asal"],to:["to","tujuan"],sku:["sku"],nama:["nama","nama_barang","nama_item","item","description"],awal:["awal","stok_lokasi_awal","stok_awal","qty_awal"],aktual:["aktual","stok_aktual","qty","qty_aktual"],keterangan:["keterangan","catatan","note"]};const idx={tanggal:-1,from:-1,to:-1,sku:-1,nama:-1,awal:-1,aktual:-1,keterangan:-1};for(const key of Object.keys(idx)){for(const alias of aliases[key]||[]){const at=normalized.indexOf(alias);if(at>=0){idx[key]=at;break;}}}return idx;}
...
function createEditableCell(row,field,value,options){const td=document.createElement("td");td.className="editable-cell";td.textContent=value||"-";td.addEventListener("click",()=>{startInlineEdit(td,row,field,row?.[field],options);});return td;}
function startInlineEdit(cellEl,row,field,oldValue,options={}){const td=cellEl.closest('td')||cellEl;if(td.querySelector("input,select"))return;const saveDelayMs=Number(options.saveDelayMs||2000);const blurDelayMs=Number(options.blurDelayMs||1000);const input=document.createElement("input");const saveBtn=document.createElement("button");saveBtn.type="button";saveBtn.className="inline-edit-save-btn";saveBtn.textContent="Save";input.value=oldValue||"";input.className="inline-edit-input";td.classList.add("editing");td.innerHTML="";td.appendChild(input);td.appendChild(saveBtn);input.focus();input.select();let cancelled=false,saving=false;INLINE_EDIT_STATE.isEditing=true;INLINE_EDIT_STATE.editingRow=Number(row?.rowNumber)||null;INLINE_EDIT_STATE.editingField=String(field||"");const clearPending=()=>{if(INLINE_EDIT_STATE.pendingSave){clearTimeout(INLINE_EDIT_STATE.pendingSave);INLINE_EDIT_STATE.pendingSave=null;}};const finish=(value)=>{td.classList.remove("editing");td.textContent=value||"-";if(INLINE_EDIT_STATE.editingRow===(Number(row?.rowNumber)||null)&&INLINE_EDIT_STATE.editingField===String(field||"")){INLINE_EDIT_STATE.isEditing=false;INLINE_EDIT_STATE.editingRow=null;INLINE_EDIT_STATE.editingField="";clearPending();}};async function save(reason="manual"){if(cancelled||saving)return;const newValue=input.value.trim();if(newValue===String(oldValue||"")){finish(oldValue);return;}saving=true;try{await options.onSave?.({row,field,value:newValue,oldValue,td,reason});finish(newValue);}catch(err){finish(oldValue);toast(err?.message||"Gagal update data","error");}finally{saving=false;}}const queueAutoSave=(reason="typing")=>{clearPending();INLINE_EDIT_STATE.lastInputAt=Date.now();INLINE_EDIT_STATE.pendingSave=setTimeout(()=>{if(cancelled||saving)return;const idleFor=Date.now()-INLINE_EDIT_STATE.lastInputAt;if(idleFor<saveDelayMs-50)return;save(reason);},saveDelayMs);};input.addEventListener("input",()=>{queueAutoSave("typing-idle");});input.addEventListener("blur",()=>{clearPending();INLINE_EDIT_STATE.pendingSave=setTimeout(()=>{if(cancelled||saving)return;save("blur-delay");},blurDelayMs);});input.addEventListener("focus",()=>{clearPending();});saveBtn.addEventListener("click",()=>save("button"));input.addEventListener("keydown",(e)=>{if(e.key==="Enter"){e.preventDefault();clearPending();save("enter");}if(e.key==="Escape"){cancelled=true;finish(oldValue);}});}
