...
const ABC_STATE={periodMonths:3,orderType:"all",selectedTo:[],excludeTo:["REJECT","RUSAK"],rows:[],top:[]};
function parseDate(value){const d=new Date(value);return Number.isFinite(d.getTime())?d:null;}
function isStoreOrder(toValue){const to=String(toValue||"");return to.includes("100")||to.includes("-BT-");}
function normalizeToOptions(rows){return [...new Set(rows.map(r=>String(getVal(r,["to","tujuan"])||"").trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b));}
function renderAbcAnalysisPage(force=false){const root=document.getElementById("abcAnalysisApp");if(!root)return;const now=Date.now();if(!force){try{const cache=JSON.parse(localStorage.getItem(ABC_ANALYSIS_CACHE_KEY)||"null");if(cache&&cache.expiresAt>now&&cache.params&&JSON.stringify(cache.params)===JSON.stringify({m:ABC_STATE.periodMonths,t:ABC_STATE.orderType,to:ABC_STATE.selectedTo,x:ABC_STATE.excludeTo})){ABC_STATE.rows=cache.rows||[];ABC_STATE.top=cache.top||[];return drawAbcAnalysis();}}catch(_e){}}
const keluarRaw=window.APP_STATE?.barangKeluar||[];const masukRaw=window.APP_STATE?.barangMasuk||[];const stockRaw=(DATA["Kartu Stock"]||[]);
const start=new Date();start.setMonth(start.getMonth()-Number(ABC_STATE.periodMonths||1));
const filtered=keluarRaw.filter(r=>{const date=parseDate(getVal(r,["tanggal","date"]));if(!date||date<start)return false;const to=String(getVal(r,["to","tujuan"])||"").trim();if(!to)return false;if(ABC_STATE.excludeTo.some(x=>clean(to)===clean(x)))return false;const store=isStoreOrder(to);if(ABC_STATE.orderType==="store"&&!store)return false;if(ABC_STATE.orderType==="gt"&&store)return false;if(ABC_STATE.selectedTo.length&&!ABC_STATE.selectedTo.includes(to))return false;return true;});
const totalQty=filtered.reduce((n,r)=>n+parseNumber(getVal(r,["qty","qty keluar"])),0);
const bySku=new Map();filtered.forEach(r=>{const sku=String(getVal(r,["sku"])||"").trim();if(!sku)return;const qty=parseNumber(getVal(r,["qty","qty keluar"]));const nama=String(getVal(r,["nama barang","nama"])||sku);const item=bySku.get(sku)||{sku,nama,qty:0};item.qty+=qty;bySku.set(sku,item);});
const ranked=[...bySku.values()].sort((a,b)=>b.qty-a.qty);let cum=0;ranked.forEach(x=>{x.contribution=totalQty?x.qty/totalQty:0;cum+=x.contribution;x.cumulative=cum;x.abc=cum<=0.8?"A":cum<=0.95?"B":"C";});
const stockMap=new Map();stockRaw.forEach(r=>{const sku=String(getVal(r,["sku"])||"").trim();if(!sku)return;stockMap.set(sku,(stockMap.get(sku)||0)+parseNumber(getVal(r,["qty","stok akhir","stok"])));});
const masukFreq=new Map();masukRaw.forEach(r=>{const sku=String(getVal(r,["sku"])||"").trim();if(!sku)return;masukFreq.set(sku,(masukFreq.get(sku)||0)+1);});
const rows=ranked.map(x=>{const stok=stockMap.get(x.sku)||0;const freq=masukFreq.get(x.sku)||0;const priority=freq>=5&&stok<=5?"HIGH PRIORITY":freq>=3?"MEDIUM":"LOW";let status="NORMAL";if(priority==="HIGH PRIORITY"&&stok<=0)status="SEGERA ORDER";else if(x.abc==="A"&&stok<=5)status="RESTOCK PRIORITAS";else if(x.abc==="A")status="AMAN";else if(x.abc==="C"&&stok>50)status="OVERSTOCK";else if(x.abc==="C"&&stok<=5)status="LOW PRIORITY";return {...x,stok,priority,status};}).sort((a,b)=>a.qty-b.qty);
const top=[...ranked].filter(x=>x.abc==="A").sort((a,b)=>b.qty-a.qty).slice(0,10);
ABC_STATE.rows=rows;ABC_STATE.top=top;localStorage.setItem(ABC_ANALYSIS_CACHE_KEY,JSON.stringify({params:{m:ABC_STATE.periodMonths,t:ABC_STATE.orderType,to:ABC_STATE.selectedTo,x:ABC_STATE.excludeTo},rows,top,expiresAt:Date.now()+24*60*60*1000}));
drawAbcAnalysis();}
function drawAbcAnalysis(){const root=document.getElementById("abcAnalysisApp");if(!root)return;const toList=normalizeToOptions(window.APP_STATE?.barangKeluar||[]);const toOptions=toList.map(to=>`<option value="${esc(to)}" ${ABC_STATE.selectedTo.includes(to)?"selected":""}>${esc(to)}</option>`).join("");
if(!ABC_STATE.rows.length){root.innerHTML=`<div class='card'><div class='state'>Tidak ada data pada periode ini</div></div>`;return;}
root.innerHTML=`<div class='card'><div class='mv-filters open'><select id='abcPeriod'>${[1,2,3,4,5,6].map(m=>`<option value='${m}' ${ABC_STATE.periodMonths===m?"selected":""}>${m} bulan terakhir</option>`).join("")}</select><select id='abcType'><option value='all' ${ABC_STATE.orderType==="all"?"selected":""}>Semua</option><option value='gt' ${ABC_STATE.orderType==="gt"?"selected":""}>Orderan GT</option><option value='store' ${ABC_STATE.orderType==="store"?"selected":""}>Orderan Store</option></select><select id='abcTo' multiple size='4'>${toOptions}</select><button id='abcRefresh' class='btn-primary'>Refresh</button></div></div><div class='card'><div class='section-header'><h4>Top 10 Fast Moving Items</h4></div><div class='row' style='flex-wrap:wrap;gap:8px'>${ABC_STATE.top.map(x=>`<button class='chip' onclick="navigateTo('/sku/'+encodeURIComponent('${encAttr(x.sku)}'))">${esc(x.nama)}</button>`).join("")}</div></div><div class='card'><div class='table-wrap table-wrap-full'><table><thead><tr><th>Nama Barang</th><th>SKU</th><th>Total Qty Keluar</th><th>% Kontribusi</th><th>Cumulative %</th><th>Kategori ABC</th><th>Stok Saat Ini</th><th>Priority Order</th><th>Status Rekomendasi</th></tr></thead><tbody>${ABC_STATE.rows.map(r=>`<tr><td>${esc(r.nama)}</td><td>${esc(r.sku)}</td><td>${r.qty}</td><td>${(r.contribution*100).toFixed(2)}%</td><td>${(r.cumulative*100).toFixed(2)}%</td><td><span class='badge ${r.abc==="A"?"b-out":r.abc==="B"?"b-warn":""}'>${r.abc}</span></td><td>${r.stok}</td><td>${esc(r.priority)}</td><td>${esc(r.status)}</td></tr>`).join("")}</tbody></table></div></div>`;
document.getElementById('abcRefresh')?.addEventListener('click',()=>{ABC_STATE.periodMonths=Number(document.getElementById('abcPeriod')?.value||3);ABC_STATE.orderType=document.getElementById('abcType')?.value||'all';ABC_STATE.selectedTo=[...(document.getElementById('abcTo')?.selectedOptions||[])].map(o=>o.value);renderAbcAnalysisPage(true);});
}
