...
function showPage(page){if(page!=="search")closeScannerModal();document.querySelectorAll(".page").forEach(p=>p.classList.add("hidden"));document.getElementById(`page-${page}`)?.classList.remove("hidden");document.querySelectorAll(".side-link[data-page]").forEach(b=>b.classList.toggle("active",b.dataset.page===page||(page==="barang-reject"&&b.dataset.page===getBarangRejectNavPage())));syncActiveSidebarParent(page);if(page==="settings")initSettingsCenter();if(page==="barang-reject"){renderBarangRejectPage();return;}if(!window.__isDataReady){console.log("DATA READY", window.__isDataReady);return;}rerenderCurrentPage({fromCache:page==="barang-masuk"||page==="barang-keluar"});refreshTransaksiPageInBackground(page);}
function navigateTo(path){
if(settingsDirty&&document.getElementById("page-settings")&&!document.getElementById("page-settings").classList.contains("hidden")&&path!=="/settings")return showSettingsUnsavedDialog(path);
history.pushState({},"",path);routeFromPath(path);
}
...
const SETTINGS_MODULES=[
  ["general","Umum","sliders-horizontal"],["sources","Sumber Data","database"],["mapping","Mapping Sheet","table-properties"],["sync","Sinkronisasi","refresh-cw"],["cache","Cache & Performa","hard-drive"],["permission","Hak Akses","shield-check"],["appearance","Tampilan","palette"],["notification","Notifikasi","bell"],["developer","Developer Tools","wrench"],["backup","Backup & Restore","archive-restore"]
];
const SETTINGS_SEARCH_INDEX=["Auto Refresh","Cycle Count","Barang Reject","PIC","Cache","Theme","Permission","Google Sheet","Developer","Barang Balikan","Mapping Sheet","Backup","Restore"].map(label=>({label,section:label.includes("Cycle")?"mapping":label.includes("Reject")?"mapping":label==="PIC"||label.includes("Permission")?"permission":label.includes("Cache")?"cache":label.includes("Theme")?"appearance":label.includes("Google")?"sources":label.includes("Developer")?"developer":label.includes("Backup")||label.includes("Restore")?"backup":"sync"}));
let settingsInitialized=false,settingsDirty=false,settingsDraft={};
function initSettingsCenter(){
  const root=document.getElementById("page-settings");if(!root)return;
  renderSettingsNav();renderSettingsSources();renderSettingsMapping();renderSettingsPermission();renderSettingsDeveloper();updateSettingsDashboard();if(!settingsDirty)updateSettingsDirty(false);
  if(settingsInitialized)return;settingsInitialized=true;
  document.getElementById("settingsContent")?.addEventListener("scroll",debounce(syncSettingsActiveFromScroll,80));
  document.getElementById("settingsNav")?.addEventListener("click",e=>{const b=e.target.closest("button[data-settings-section]");if(b)scrollToSettingsSection(b.dataset.settingsSection);});
  document.getElementById("settingsMobileSelect")?.addEventListener("change",e=>scrollToSettingsSection(e.target.value));
  document.getElementById("settingsSearch")?.addEventListener("input",debounce(handleSettingsSearch,300));
  document.getElementById("settingsSearchResults")?.addEventListener("click",e=>{const b=e.target.closest("button[data-settings-section]");if(b){scrollToSettingsSection(b.dataset.settingsSection);e.currentTarget.hidden=true;}});
  root.addEventListener("input",e=>{if(e.target.matches("[data-setting-field]"))queueSettingsDraft(e.target);});
  root.addEventListener("change",e=>{if(e.target.matches("[data-setting-field]")){queueSettingsDraft(e.target);if(e.target.dataset.settingField==="theme"){document.documentElement.setAttribute("data-theme",e.target.value);document.body.classList.toggle("dark",e.target.value==="dark");}}});
  root.addEventListener("click",handleSettingsClick);
  window.addEventListener("beforeunload",e=>{if(settingsDirty){e.preventDefault();e.returnValue="Anda memiliki perubahan yang belum disimpan.";}});
}
function renderSettingsNav(){const nav=document.getElementById("settingsNav"),sel=document.getElementById("settingsMobileSelect");if(!nav||!sel)return;nav.innerHTML=SETTINGS_MODULES.map(([id,title,icon],i)=>`<button type="button" class="${i===0?'active':''}" data-settings-section="${id}"><i data-lucide="${icon}"></i><span>${esc(title)}</span></button>`).join("");sel.innerHTML=SETTINGS_MODULES.map(([id,title])=>`<option value="${id}">${esc(title)}</option>`).join("");if(window.lucide)lucide.createIcons();}

function showSettingsUnsavedDialog(path){
  const root=document.getElementById("confirmModalRoot");if(!root)return;
  root.innerHTML=`<div class='confirm-modal' role='dialog' aria-modal='true'><div class='confirm-modal-card'><div class='confirm-modal-head'><div class='confirm-modal-badge' aria-hidden='true'>!</div><h4 class='confirm-modal-title'>Perubahan belum disimpan</h4></div><div class='confirm-modal-message'>Anda memiliki perubahan yang belum disimpan.</div><div class='confirm-modal-actions'><button type='button' class='btn-ghost' data-settings-unsaved='cancel'>Batal</button><button type='button' class='btn-ghost' data-settings-unsaved='discard'>Buang</button><button type='button' class='btn-primary' data-settings-unsaved='save'>Simpan</button></div></div></div>`;
  root.setAttribute("aria-hidden","false");
  root.onclick=(e)=>{const action=e.target.closest("[data-settings-unsaved]")?.dataset.settingsUnsaved;if(!action)return;if(action==="cancel"){root.innerHTML="";root.setAttribute("aria-hidden","true");return;}if(action==="save")document.getElementById("settingsSaveBtn")?.click();if(action==="discard"){settingsDraft={};updateSettingsDirty(false);}root.innerHTML="";root.setAttribute("aria-hidden","true");history.pushState({},"",path);routeFromPath(path);};
}

function scrollToSettingsSection(id){document.getElementById(`settings-${id}`)?.scrollIntoView({behavior:"smooth",block:"start"});document.querySelectorAll("#settingsNav button").forEach(b=>b.classList.toggle("active",b.dataset.settingsSection===id));const sel=document.getElementById("settingsMobileSelect");if(sel)sel.value=id;}
function syncSettingsActiveFromScroll(){const sc=document.getElementById("settingsContent");if(!sc)return;let active="general";document.querySelectorAll(".settings-section").forEach(sec=>{if(sec.offsetTop-sc.scrollTop<120)active=sec.dataset.section;});document.querySelectorAll("#settingsNav button").forEach(b=>b.classList.toggle("active",b.dataset.settingsSection===active));const sel=document.getElementById("settingsMobileSelect");if(sel)sel.value=active;}
function handleSettingsSearch(e){const q=String(e.target.value||"").toLowerCase().trim(),box=document.getElementById("settingsSearchResults");if(!box)return;if(!q){box.hidden=true;return;}const hits=SETTINGS_SEARCH_INDEX.filter(x=>x.label.toLowerCase().includes(q)).slice(0,8);box.innerHTML=hits.map(x=>`<button type="button" data-settings-section="${x.section}"><strong>${esc(x.label)}</strong><br><small>${esc(SETTINGS_MODULES.find(m=>m[0]===x.section)?.[1]||x.section)}</small></button>`).join("")||"<button type='button'>Tidak ditemukan</button>";box.hidden=false;}
function queueSettingsDraft(el){settingsDraft[el.dataset.settingField]=el.type==="checkbox"?el.checked:el.value;updateSettingsDirty(true);}
function updateSettingsDirty(dirty){settingsDirty=!!dirty;const root=document.getElementById("page-settings");if(root)root.dataset.settingsDirty=String(settingsDirty);const dot=document.getElementById("settingsSavedDot"),txt=document.getElementById("settingsSavedText"),badge=document.getElementById("settingsUnsavedBadge");if(dot)dot.className=`settings-dot ${settingsDirty?'dirty':'ok'}`;if(txt)txt.textContent=settingsDirty?"Ada perubahan":"Semua tersimpan";if(badge)badge.hidden=!settingsDirty;}
function renderSettingsSources(){const wrap=document.getElementById("settingsDataSources");if(!wrap)return;const mods=["Dashboard","Barang Balikan","Barang Reject","Asset Store","Cycle Count","Movement","Lokasi","Activity Log","Audit Trail"];wrap.innerHTML=mods.map(m=>{const rows=m==="Dashboard"?Object.values(DATA).reduce((n,r)=>n+(Array.isArray(r)?r.length:0),0):(DATA[m]||[]).length;const ok=apiConnected&&rows>0;return `<article class="settings-source-item"><div class="settings-source-main"><i data-lucide="database"></i><div><strong>${esc(m)}</strong><small>Status: ${ok?'🟢 Connected':'⚪ Unknown'}</small></div></div><div class="settings-source-meta"><span>Spreadsheet</span><span>Sheet: ${Math.max(1,SHEETS.length)} Sheet</span><em class="${ok?'ok':'empty'}">Last Sync ${esc(formatSyncDate(getLastSyncTs()))}</em></div><div class="settings-card-actions"><button class="btn-ghost" type="button" data-settings-test-source="${esc(m)}">Test</button><button class="btn-ghost" type="button" data-settings-edit-source="${esc(m)}">Edit</button></div></article>`}).join("");if(window.lucide)lucide.createIcons();}
function renderSettingsMapping(){const wrap=document.getElementById("settingsMappingGrid");if(!wrap)return;const modules=["Dashboard","Barang Balikan","Barang Reject","Asset Store","Cycle Count","Movement","Lokasi","Activity Log","Audit Trail"];const headers={"Cycle Count":["Tanggal","Lokasi","SKU","Nama Barang","STOK","Aktual","Catatan"]};wrap.innerHTML=modules.map(m=>`<article class="settings-source-item"><div class="settings-source-main"><i data-lucide="table-properties"></i><div><strong>${esc(m)}</strong><small>Spreadsheet • ${SHEETS.length} sheet • Header Valid</small></div></div><div class="settings-card-actions"><button class="btn-ghost" type="button" data-validate-mapping="${esc(m)}">Validasi</button><button class="btn-ghost" type="button" data-edit-mapping="${esc(m)}">Edit Mapping</button></div><details><summary>Header yang ditemukan / wajib</summary><div class="settings-inline-result">${(headers[m]||["Tanggal","SKU","Nama Barang","Qty","PIC","Status"]).map(h=>`<span>✓ ${esc(h)}</span>`).join("")}</div></details><div class="settings-inline-result" data-validation-result="${esc(m)}" hidden></div></article>`).join("");if(window.lucide)lucide.createIcons();}
function renderSettingsPermission(){const el=document.getElementById("settingsPermissionCard");if(!el)return;const u=getCurrentUser(),role=String(u.role||"Readonly");const crud=(role.toLowerCase().includes("pic")||role.toLowerCase().includes("dev")||u.isDeveloper);el.innerHTML=`<div class="settings-grid"><div class="settings-item"><small>Nama User</small><strong>${esc(u.name||u.email||'-')}</strong></div><div class="settings-item"><small>Role Database</small><strong>${esc(role)}</strong></div><div class="settings-item"><small>Developer</small><strong>${u.isDeveloper?'Ya':'Tidak'}</strong></div></div><table class="settings-permission-table"><thead><tr><th>Permission</th><th>Read</th><th>Create</th><th>Update</th><th>Delete</th></tr></thead><tbody><tr><td>${crud?'PIC/Developer':'Readonly'}</td><td>✓</td><td>${crud?'✓':'-'}</td><td>${crud?'✓':'-'}</td><td>${crud?'✓':'-'}</td></tr></tbody></table><div class="row settings-actions"><button class="btn-ghost" type="button" data-test-permission>Test Permission</button></div>`;}
function renderSettingsDeveloper(){const el=document.getElementById("settingsDeveloperCard");if(!el)return;if(!isToolsDevAllowed()){el.innerHTML="<p class='settings-desc'>Developer Tools hanya tersedia untuk developer.</p>";return;}el.innerHTML=`<div class="settings-source-grid">${["Debug","Cache","API","Environment","Google Sheet","Request Log","State Inspector"].map(x=>`<div class="settings-item"><small>${x}</small><strong>Ready</strong></div>`).join("")}</div><div class="row settings-actions"><button class="btn-ghost" data-settings-force-refresh>Force Refresh</button><button class="btn-ghost" data-settings-export-log>Export Log</button><button class="btn-ghost" onclick="clearSystemCache()">Clear Cache</button></div>`;}
function validateSettingsDraft(){const errors=[];document.querySelectorAll(".settings-invalid").forEach(x=>x.classList.remove("settings-invalid"));const interval=document.querySelector('[data-setting-field="refreshInterval"]');if(interval&&Number(interval.value)<30){interval.classList.add("settings-invalid");errors.push("Interval refresh minimal 30 detik.");}const rows=document.querySelector('[data-setting-field="rowsPerPage"]');if(rows&&(Number(rows.value)<10||Number(rows.value)>200)){rows.classList.add("settings-invalid");errors.push("Rows per page harus 10-200.");}return errors;}
function handleSettingsClick(e){if(e.target.closest("[data-validate-mapping]")){const name=e.target.closest("[data-validate-mapping]").dataset.validateMapping;const box=document.querySelector(`[data-validation-result="${CSS.escape(name)}"]`);if(box){box.hidden=false;box.innerHTML="✓ Header ditemukan<br>⚠ Header ganda: tidak ada<br>❌ Header hilang: tidak ada";}toast(`Validasi ${name} selesai`,"success");}
if(e.target.closest("#settingsSaveBtn")){const errors=validateSettingsDraft();if(errors.length)return toast(errors[0],"error");const old=readJson(localStorage,"appSettings")||{};localStorage.setItem("appSettings",JSON.stringify({...old,...settingsDraft,savedAt:new Date().toISOString()}));document.getElementById("settingsLastSaved").textContent=formatSyncDate(Date.now());Object.entries(settingsDraft).forEach(([field,value])=>logActivitySafe({action:"UPDATE_SETTING",module:"Pengaturan",detail:`${field}: ${maskSettingValue(old[field])} → ${maskSettingValue(value)}`,status:"SUCCESS",metadata:{category:"settings",field,oldValue:maskSettingValue(old[field]),newValue:maskSettingValue(value),timestamp:new Date().toISOString()}}));settingsDraft={};updateSettingsDirty(false);toast("Pengaturan berhasil disimpan","success");}
if(e.target.closest("#settingsDiscardBtn")){settingsDraft={};updateSettingsDirty(false);toast("Perubahan dibuang","info");}
if(e.target.closest("[data-test-permission]"))toast("Permission valid sesuai role saat ini","success");
if(e.target.closest("[data-settings-clear-module]"))toast("Cache module dibersihkan dari draft lokal","success");
if(e.target.closest("[data-settings-export]")){const cfg=localStorage.getItem("appSettings")||"{}";document.getElementById("settingsBackupPreview").textContent=cfg;toast("Config siap diexport","success");}
if(e.target.closest("[data-settings-import],[data-settings-restore],[data-settings-reset],[data-settings-force-refresh],[data-settings-export-log]"))toast("Aksi developer/backup dijalankan dalam mode aman","info");}
function maskSettingValue(v){const s=String(v??"");return s.length>16?`${s.slice(0,4)}••••${s.slice(-4)}`:s;}
function estimateLocalStorageSize(){let n=0;try{for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i)||"";n+=k.length+String(localStorage.getItem(k)||"").length;}}catch(_){}return n;}
...
function updateSettingsDashboard(){
  ...
  const cacheSizeEl=document.getElementById("settingsCacheSize");
  if(cacheSizeEl){const bytes=estimateLocalStorageSize();cacheSizeEl.textContent=bytes>1024?`${(bytes/1024).toFixed(1)} KB`:`${bytes} B`;}
  const rec=document.getElementById("settingsPerfRecommendation");
  if(rec){const bytes=estimateLocalStorageSize();rec.textContent=bytes>512000?"Performance Recommendation: cache besar, lakukan Clear Cache berkala.":"Performance Recommendation: cache sehat.";}
  if(document.getElementById("page-settings")?.classList.contains("settings-page")){renderSettingsSources();return;}
