...
const bulkEdit=e.target.closest('[data-mv-bulk-edit]');if(bulkEdit){const mode=bulkEdit.dataset.mvBulkEdit,selectedSet=getSelectedSet(mode),rows=[...selectedSet];const status=prompt('Status baru (opsional):','');const pic=prompt('PIC baru (opsional):','');const keterangan=prompt('Keterangan baru (opsional):','');const updates={};if(status)updates.status=status;if(pic)updates.pic=pic;if(keterangan)updates.keterangan=keterangan;if(!rows.length||!Object.keys(updates).length)return;fetch(mode==='in'?'/api/barang-masuk/bulk-update':'/api/barang-keluar/bulk-update',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({rowNumbers:rows,updates})}).then(async r=>({ok:r.ok,out:await r.json()})).then(({ok,out})=>{if(!ok||!out?.success)throw new Error(out?.message||'Gagal bulk edit');TABLE_STATE[mode].rows.forEach(r=>{if(selectedSet.has(r.rowNumber))Object.assign(r,updates);});selectedSet.clear();renderDataTablePage(mode,mode==='in'?'Barang Masuk':'Barang Keluar',true);toast('Bulk edit berhasil','success');}).catch(err=>toast(err?.message||'Gagal bulk edit','error'));return;}
