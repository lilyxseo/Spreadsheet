...
const LOCATION_STATE={rows:[],filtered:[],page:1,pageSize:25,selected:"",status:"all",searchInput:"",skuSearchInput:"",_timer:null,_cacheKey:"",emptyLocations:[],activeSku:null};
function getLocationValue(row){return getVal(row,["lokasi","location","rak","bin","area","LOKASI"])||"";}
function getLocationRowSearchText(row){return clean(`${row.lokasi} ${row.skus.map(s=>`${s.sku} ${s.nama}`).join(" ")}`);}
function getEmptyLocationCount(){return LOCATION_STATE.emptyLocations?.length||0;}
function buildLocationRows(){const skuMap=new Map();for(const row of (DATA["Kartu Stock"]||[])){const sku=String(getVal(row,["sku"])||"").trim();if(!sku)continue;const nama=getVal(row,["nama barang","nama","item","description"])||"-";const loc=String(getLocationValue(row)||"").trim().toUpperCase();if(!loc)continue;const qty=parseNumber(getVal(row,["stok akhir","qty","closing stock","ending stock","saldo akhir"]));if(qty<=0)continue;const key=`${clean(loc)}__${clean(sku)}`;if(!skuMap.has(key))skuMap.set(key,{lokasi:loc,sku,nama,qty:0,invent:getVal(row,["invent","inventory","inv"])||"-",keterangan:getVal(row,["keterangan","note","notes","remark","remarks"])||"-",sourceRows:[]});const it=skuMap.get(key);it.qty+=qty;it.sourceRows.push(row);if((!it.invent||it.invent==="-")&&getVal(row,["invent","inventory","inv"]))it.invent=getVal(row,["invent","inventory","inv"]);if((!it.keterangan||it.keterangan==="-")&&getVal(row,["keterangan","note","notes","remark","remarks"]))it.keterangan=getVal(row,["keterangan","note","notes","remark","remarks"]);}const grouped={};for(const it of skuMap.values()){if(it.qty<=0)continue;if(!grouped[it.lokasi])grouped[it.lokasi]={lokasi:it.lokasi,skus:[],totalQty:0};grouped[it.lokasi].skus.push(it);grouped[it.lokasi].totalQty+=it.qty;}const rows=Object.values(grouped).filter(g=>g.totalQty>0&&g.skus.length>0).map(g=>{const jumlahSku=g.skus.length;const status=(jumlahSku>=25||g.totalQty>=400?'Padat':'Normal');return {...g,jumlahSku,skuKosong:0,status,_searchText:getLocationRowSearchText({...g,jumlahSku,status})};});const used=new Set(rows.map(r=>String(r.lokasi).trim()));LOCATION_STATE.emptyLocations=getAllValidLocations().filter(loc=>!used.has(loc));return rows;}
...
function renderLocationsPage(){LOCATION_STATE.rows=buildLocationRows();if(locStatusFilter)locStatusFilter.value=LOCATION_STATE.status;if(locPageSize)locPageSize.value=String(LOCATION_STATE.pageSize);if(locSearchInput)locSearchInput.value=LOCATION_STATE.searchInput;if(locSkuSearchInput)locSkuSearchInput.value=LOCATION_STATE.skuSearchInput;if(!locSearchInput?.dataset.bound){[locSearchInput,locSkuSearchInput].forEach(el=>el?.addEventListener('input',e=>{if(e.target===locSearchInput)LOCATION_STATE.searchInput=e.target.value||"";if(e.target===locSkuSearchInput)LOCATION_STATE.skuSearchInput=e.target.value||"";clearTimeout(LOCATION_STATE._timer);LOCATION_STATE._timer=setTimeout(()=>{LOCATION_STATE.page=1;drawLocations();},300);}));[locStatusFilter,locPageSize].forEach(el=>el?.addEventListener('change',e=>{if(e.target===locStatusFilter)LOCATION_STATE.status=e.target.value||'all';if(e.target===locPageSize)LOCATION_STATE.pageSize=Number(e.target.value)||25;LOCATION_STATE.page=1;drawLocations();}));locSearchInput.dataset.bound='1';}drawLocations();}
function normalizeLocationStatus(status){return status==='Terisi'?'Terisi':status==='Kosong'?'Kosong':status;}
function getFilteredLocations(){const rows=LOCATION_STATE.rows||[];const qLoc=clean(LOCATION_STATE.searchInput||locSearchInput?.value||'');const qSku=clean(LOCATION_STATE.skuSearchInput||locSkuSearchInput?.value||'');const status=normalizeLocationStatus(LOCATION_STATE.status||locStatusFilter?.value||'all');const key=`${qLoc}|${qSku}|${status}|${rows.length}`;if(LOCATION_STATE._cacheKey===key&&Array.isArray(LOCATION_STATE.filtered))return LOCATION_STATE.filtered;let filtered=rows.filter(r=>{const hitLoc=!qLoc||clean(r.lokasi).includes(qLoc);const hitSku=!qSku||r.skus.some(s=>clean(`${s.sku} ${s.nama}`).includes(qSku));const hitStatus=status==='all'||(status==='Terisi'&&r.totalQty>0)||(status==='Padat'&&r.status==='Padat')||(status==='Normal'&&r.status==='Normal')||(status==='Kosong'&&false);return hitLoc&&hitSku&&hitStatus;});filtered=[...filtered].sort((a,b)=>a.lokasi.localeCompare(b.lokasi,'id',{numeric:true})||b.totalQty-a.totalQty);LOCATION_STATE._cacheKey=key;LOCATION_STATE.filtered=filtered;return filtered;}
function setLocationStatus(status){LOCATION_STATE.status=status||'all';if(locStatusFilter)locStatusFilter.value=LOCATION_STATE.status;document.querySelectorAll('[data-loc-status]').forEach(btn=>btn.classList.toggle('is-active',btn.dataset.locStatus===LOCATION_STATE.status));LOCATION_STATE.page=1;drawLocations();}
function drawLocations(){document.querySelectorAll('[data-loc-status]').forEach(btn=>btn.classList.toggle('is-active',btn.dataset.locStatus===(LOCATION_STATE.status||'all')));const rows=LOCATION_STATE.rows||[];const filtered=getFilteredLocations();const totalSku=new Set(rows.flatMap(r=>r.skus.map(s=>clean(s.sku))).filter(Boolean)).size;locationsSummary.innerHTML=`<button class='metric location-summary-card' type='button' disabled><div class='k'>📍 Total Lokasi</div><div class='v'>${rows.length}</div></button><button class='metric location-summary-card' type='button' disabled><div class='k'>📦 Total SKU Berlokasi</div><div class='v'>${totalSku}</div></button><button class='metric location-summary-card location-empty-trigger' type='button' onclick='openEmptyLocationsModal()'><div class='k'>🚫 Lokasi Kosong</div><div class='v'>${getEmptyLocationCount()}</div></button>`;const max=Math.max(1,Math.ceil(filtered.length/LOCATION_STATE.pageSize));if(LOCATION_STATE.page>max)LOCATION_STATE.page=max;const start=(LOCATION_STATE.page-1)*LOCATION_STATE.pageSize;const pageRows=filtered.slice(start,start+LOCATION_STATE.pageSize);locationsTable.innerHTML=`<div class='location-list-head'><div><strong>Hasil Lokasi</strong><span>${filtered.length} hasil</span></div></div><div class='location-list' role='list'>${pageRows.map(r=>`<button type='button' class='location-list-item ${LOCATION_STATE.selected===r.lokasi?'is-active':''}' onclick="selectLocationDetail('${encAttr(r.lokasi)}')"><span class='location-list-main'><strong>${esc(r.lokasi)}</strong><small>${r.jumlahSku} SKU • ${r.totalQty} pcs</small></span><span class='location-status ${r.status==='Padat'?'is-dense':'is-normal'}'>${esc(r.status)}</span></button>`).join('')||"<div class='state'>Tidak ada lokasi yang cocok.</div>"}</div><div class='mv-pagination'><span>Menampilkan ${filtered.length?start+1:0}–${Math.min(start+LOCATION_STATE.pageSize,filtered.length)} dari ${filtered.length} data</span><div class='row'><button class='btn-ghost' onclick='changeLocationPage(-1)'>Prev</button><button class='btn-ghost' onclick='changeLocationPage(1)'>Next</button></div></div>`;locationsEmpty.innerHTML='';if(LOCATION_STATE.selected)selectLocationDetail(encAttr(LOCATION_STATE.selected),true);else locationDetail.innerHTML=`<div class='card location-detail-empty'><div class='state'>Pilih lokasi untuk melihat daftar SKU.</div></div>`;}
function renderEmptyLocationSection(){return;}
function changeLocationPage(step){const max=Math.max(1,Math.ceil((getFilteredLocations().length||0)/LOCATION_STATE.pageSize));LOCATION_STATE.page=Math.min(max,Math.max(1,LOCATION_STATE.page+step));drawLocations();}
function selectLocationDetail(locEncoded,keep=false){const lokasi=decodeURIComponent(locEncoded||'');LOCATION_STATE.selected=lokasi;const row=(LOCATION_STATE.filtered||LOCATION_STATE.rows).find(r=>r.lokasi===lokasi)||(LOCATION_STATE.rows||[]).find(r=>r.lokasi===lokasi);if(!row){if(!keep)locationDetail.innerHTML='';return;}const skuRows=[...row.skus].sort((a,b)=>b.qty-a.qty||String(a.sku).localeCompare(String(b.sku)));locationDetail.innerHTML=`<div class='card location-detail-card'><div class='location-detail-top'><div><h4>📍 ${esc(lokasi)}</h4><p>${row.jumlahSku} SKU • ${row.totalQty} pcs • Status: <strong>${esc(row.status)}</strong></p></div><div class='location-actions'><button class='copy-mini-btn' type='button' onclick="copyText(decodeURIComponent('${encAttr(lokasi)}'),'Lokasi disalin',this)">📋 Copy Lokasi</button><button class='copy-mini-btn' type='button' onclick='exportLocationCsv()'>📤 Export</button></div></div><div class='location-sku-table-wrap'><table class='location-sku-table'><thead><tr><th>SKU</th><th>Nama Barang</th><th>Qty</th><th>Aksi</th></tr></thead><tbody>${skuRows.map(s=>`<tr><td><span class='cell-clip'>${esc(s.sku)}</span></td><td><span class='cell-clip'>${esc(s.nama)}</span></td><td>${s.qty}</td><td class='location-icon-actions'><button class='icon-btn' title='Copy SKU' aria-label='Copy SKU' onclick="copyText(decodeURIComponent('${encAttr(s.sku)}'),'SKU disalin',this)">📋</button><button class='icon-btn' title='Detail SKU' aria-label='Detail SKU' onclick="openLocationSkuDetail('${encAttr(lokasi)}','${encAttr(s.sku)}')">🔍</button></td></tr>`).join('')}</tbody></table></div></div>`;drawLocationActiveState();if(!keep)locationDetail.scrollIntoView({behavior:'smooth',block:'nearest'});}
function drawLocationActiveState(){document.querySelectorAll('.location-list-item').forEach(el=>el.classList.toggle('is-active',clean(el.textContent).startsWith(clean(LOCATION_STATE.selected))));}
function openLocationSkuDetail(locEncoded,skuEncoded){const lokasi=decodeURIComponent(locEncoded||''),sku=decodeURIComponent(skuEncoded||'');const loc=(LOCATION_STATE.rows||[]).find(r=>r.lokasi===lokasi);const item=loc?.skus?.find(s=>clean(s.sku)===clean(sku));if(!item)return;LOCATION_STATE.activeSku={...item,lokasi};const html=`<div class='location-sku-backdrop' onclick='closeLocationSkuDetail()'></div><aside class='location-sku-drawer' role='dialog' aria-modal='true'><button class='icon-btn location-drawer-close' onclick='closeLocationSkuDetail()'>×</button><h3>${esc(item.sku)}</h3><p>${esc(item.nama)}</p><dl><dt>Lokasi</dt><dd>${esc(lokasi)}</dd><dt>Qty</dt><dd>${item.qty}</dd><dt>Invent</dt><dd>${esc(item.invent||'-')}</dd><dt>Keterangan</dt><dd>${esc(item.keterangan||'-')}</dd></dl><button class='btn-primary' onclick="navigateTo('/sku/'+encodeURIComponent(decodeURIComponent('${encAttr(item.sku)}')))">Buka halaman SKU</button></aside>`;document.body.insertAdjacentHTML('beforeend',`<div id='locationSkuOverlay' class='location-sku-overlay'>${html}</div>`);}
function closeLocationSkuDetail(){document.getElementById('locationSkuOverlay')?.remove();}
function openEmptyLocationsModal(){const list=LOCATION_STATE.emptyLocations||[];document.body.insertAdjacentHTML('beforeend',`<div id='emptyLocationModal' class='location-modal'><div class='location-sku-backdrop' onclick='closeEmptyLocationsModal()'></div><div class='location-empty-modal card' role='dialog' aria-modal='true'><div class='section-header'><h4>🚫 Lokasi Kosong</h4><button class='icon-btn' onclick='closeEmptyLocationsModal()'>×</button></div><div class='subtitle'>${list.length} lokasi kosong</div><div class='empty-location-grid'>${list.map(loc=>`<span>${esc(loc)}</span>`).join('')}</div></div></div>`);}
function closeEmptyLocationsModal(){document.getElementById('emptyLocationModal')?.remove();}
function exportLocationCsv(){const cols=["lokasi","jumlah_sku","total_qty","status"];const lines=[cols.join(',')].concat((LOCATION_STATE.filtered||getFilteredLocations()).map(r=>[r.lokasi,r.jumlahSku,r.totalQty,r.status].map(v=>`"${String(v??"").replaceAll('"','""')}"`).join(',')));const blob=new Blob([lines.join('\n')],{type:'text/csv;charset=utf-8;'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='locations-analysis.csv';a.click();URL.revokeObjectURL(a.href);}
window.renderLocationsPage=renderLocationsPage;window.setLocationStatus=setLocationStatus;window.changeLocationPage=changeLocationPage;window.selectLocationDetail=selectLocationDetail;window.openLocationSkuDetail=openLocationSkuDetail;window.closeLocationSkuDetail=closeLocationSkuDetail;window.openEmptyLocationsModal=openEmptyLocationsModal;window.closeEmptyLocationsModal=closeEmptyLocationsModal;window.exportLocationCsv=exportLocationCsv;
