...
function renderRejectDashboard(){return `${renderRejectCards()}${renderRejectDashboardDetails()}`;}
...
function renderRejectToolbar(kind,{title,subtitle,searchKey,fromKey,toKey,extra=[]}){const f=BARANG_REJECT_STATE.filters;return `<div class="reject-page-title-row"><div><h3>${esc(title)}</h3><small class="subtitle">${esc(subtitle)}</small></div><div class="row reject-title-actions"><button class="btn-primary" data-reject-open-form="${kind}" ${!canCrud()?'hidden':''}><i data-lucide="plus"></i>Tambah Data</button><button class="btn-ghost" data-reject-export="${kind}">Export</button><button class="btn-ghost" data-reject-reset="${kind}">Reset Filter</button></div></div><div class="reject-filter-panel"><input class="search-lg reject-search-main" data-reject-filter="${searchKey}" placeholder="Search semua kolom penting..." value="${esc(f[searchKey])}"><input type="date" data-reject-filter="${fromKey}" value="${esc(f[fromKey])}" title="Tanggal mulai"><input type="date" data-reject-filter="${toKey}" value="${esc(f[toKey])}" title="Tanggal akhir">${extra.map(item=>`<input class="reject-filter-compact" data-reject-filter="${item.key}" placeholder="${esc(item.placeholder)}" value="${esc(f[item.key])}">`).join('')}</div>`;}
function renderRejectIn(){const f=BARANG_REJECT_STATE.filters;let baseRows=BARANG_REJECT_STATE.masuk.filter(r=>rejectTokensMatch(r,f.masukSearch)&&dateInRange(r.tanggal,f.masukTanggalFrom,f.masukTanggalTo)&&(!f.masukLokasi||clean([r.from,r.to,r.lokasi].join(' ')).includes(clean(f.masukLokasi))));let rows=orderRejectSheetRows(baseRows).filter(r=>rejectColumnMatch(r,'masuk',REJECT_IN_COLUMNS));const pageRows=paginateReject(rows,'masuk');return `${renderRejectToolbar('masuk',{title:'Barang Masuk Reject',subtitle:'Filter utama bersih, filter detail tersedia di header tabel.',searchKey:'masukSearch',fromKey:'masukTanggalFrom',toKey:'masukTanggalTo',extra:[{key:'masukLokasi',placeholder:'From/To'}]})}<div class="card reject-card"><div class="table-wrap reject-table-wrap"><table><thead><tr>${REJECT_IN_COLUMNS.map(c=>thRejectFilter(c[0],'masuk',c,baseRows)).join('')}${thRejectStatic('Aksi')}</tr></thead><tbody>${pageRows.length?renderRejectTableRows(pageRows,REJECT_IN_COLUMNS,'masuk'):`<tr><td colspan="15"><div class="state">Belum ada barang masuk reject.</div></td></tr>`}</tbody></table></div>${rejectPager('masuk',rows.length)}</div>`;}
function renderRejectOut(){const f=BARANG_REJECT_STATE.filters;let baseRows=BARANG_REJECT_STATE.keluar.filter(r=>rejectTokensMatch(r,f.keluarSearch)&&dateInRange(r.tanggal,f.keluarTanggalFrom,f.keluarTanggalTo)&&(!f.keluarStatus||clean([r.statusBarang,r.statusProses,r.status].join(' ')).includes(clean(f.keluarStatus)))&&(!f.keluarPic||clean(r.pic).includes(clean(f.keluarPic))));let rows=orderRejectSheetRows(baseRows).filter(r=>rejectColumnMatch(r,'keluar',REJECT_OUT_COLUMNS));const pageRows=paginateReject(rows,'keluar');return `${renderRejectToolbar('keluar',{title:'Barang Keluar Reject',subtitle:'Dua kolom STATUS dibedakan menjadi Status Barang dan Status Proses.',searchKey:'keluarSearch',fromKey:'keluarTanggalFrom',toKey:'keluarTanggalTo',extra:[{key:'keluarStatus',placeholder:'Status'},{key:'keluarPic',placeholder:'PIC'}]})}<div class="card reject-card"><div class="table-wrap reject-table-wrap"><table><thead><tr>${REJECT_OUT_COLUMNS.map(c=>thRejectFilter(c[0],'keluar',c,baseRows)).join('')}${thRejectStatic('Aksi')}</tr></thead><tbody>${pageRows.length?renderRejectTableRows(pageRows,REJECT_OUT_COLUMNS,'keluar'):`<tr><td colspan="15"><div class="state">Belum ada barang keluar reject.</div></td></tr>`}</tbody></table></div>${rejectPager('keluar',rows.length)}</div>`;}
