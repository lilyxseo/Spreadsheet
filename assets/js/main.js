...
function showPage(page){if(page!=="search")closeScannerModal();document.querySelectorAll(".page").forEach(p=>p.classList.add("hidden"));document.getElementById(`page-${page}`)?.classList.remove("hidden");document.querySelectorAll(".side-link[data-page]").forEach(b=>b.classList.toggle("active",b.dataset.page===page));syncActiveSidebarParent(page);if(page==="barang-reject"){renderBarangRejectPage();return;}if(!window.__isDataReady){console.log("DATA READY", window.__isDataReady);return;}rerenderCurrentPage({fromCache:page==="barang-masuk"||page==="barang-keluar"});refreshTransaksiPageInBackground(page);}
...
const defaults={data:false,"monitoring-stok":false,inventory:false,sistem:false};
...
if(!["/search","/balikan-store","/movement","/barang-reject"].includes(location.pathname))return;
...
function renderRejectCards(){const stock=BARANG_REJECT_STATE.stock.filter(r=>(Number(r.qty)||0)>0);const skuSet=new Set(stock.map(r=>String(r.sku||'').trim()).filter(Boolean));const locSet=new Set(stock.map(r=>String(r.lokasi||'').trim()).filter(Boolean));const cards=[['Total SKU Reject',skuSet.size,'package-x'],['Total Qty Reject',sumQty(stock),'boxes'],['Total Lokasi Reject',locSet.size,'map-pin'],['Barang Masuk Bulan Ini',sumQty(thisMonthRows(BARANG_REJECT_STATE.masuk)),'package-plus'],['Barang Keluar Bulan Ini',sumQty(thisMonthRows(BARANG_REJECT_STATE.keluar)),'package-minus'],['SKU Terbanyak',topBy(stock,'sku'),'badge'],['Lokasi Terbanyak',topBy(stock,'lokasi'),'warehouse']];return `<div class="grid dashboard reject-dashboard-cards">${cards.map(([k,v,icon])=>`<div class="metric card"><div class="metric-head"><span class="k">${esc(k)}</span><i data-lucide="${icon}"></i></div><div class="v">${esc(v)}</div></div>`).join('')}</div>`;}
...
function renderRejectDashboard(){return `${renderRejectCards()}<div class="reject-chart-grid">${renderRejectChart(BARANG_REJECT_STATE.masuk,'Barang Masuk per bulan','in')}${renderRejectChart(BARANG_REJECT_STATE.keluar,'Barang Keluar per bulan','out')}</div>`;}
...
function renderRejectInputOut(){return `<div class="card reject-form-card"><form id="rejectOutForm" class="reject-form"><div class="reject-form-head"><div><h4>Input Barang Keluar Reject</h4><small>Kurangi stok reject dari lokasi asal; sistem menolak stok minus.</small></div></div><div class="reject-form-grid"><label class="reject-field"><span>Tanggal</span><input type="date" name="tanggal" value="${todayIso()}"></label><label class="reject-field"><span>From *</span><input name="from" required placeholder="Lokasi asal"></label><label class="reject-field"><span>To</span><input name="to" placeholder="Tujuan"></label><label class="reject-field"><span>SKU *</span><div class="reject-input-action"><input name="sku" required placeholder="Scan / ketik SKU"><button class="icon-btn reject-scan-btn" data-reject-scan-field="sku" type="button" title="Scan Barcode"><i data-lucide="scan-line"></i></button></div></label><label class="reject-field"><span>Nama Barang</span><input name="namaBarang" placeholder="Nama barang"></label><label class="reject-field"><span>Qty *</span><input type="number" name="qty" min="1" step="1" required placeholder="0"></label><label class="reject-field"><span>PIC</span><input name="pic" placeholder="PIC"></label><label class="reject-field"><span>No iSeller</span><input name="noIseller" placeholder="No iSeller"></label><label class="reject-field"><span>Netsuite</span><input name="netsuite" placeholder="Netsuite"></label><label class="reject-field"><span>Lokasi Surat Jalan</span><input name="lokasiSuratJalan" placeholder="Lokasi SJ"></label><label class="reject-field reject-field--full"><span>Keterangan</span><textarea name="keterangan" rows="3" placeholder="Opsional"></textarea></label></div><div class="reject-form-actions"><button class="btn-primary" type="submit" ${BARANG_REJECT_STATE.submittingOut?'disabled':''}>${BARANG_REJECT_STATE.submittingOut?'Menyimpan...':'Simpan Barang Keluar Reject'}</button></div></form></div>`;}
function renderBarangRejectPage(){const root=document.getElementById('barangRejectApp');if(!root)return;ensureBarangRejectLoaded();const tabs=[['dashboard','Dashboard Reject'],['stock','Kartu Stock Reject'],['masuk','Barang Masuk Reject'],['keluar','Barang Keluar Reject'],['inputMasuk','Input Barang Masuk Reject'],['inputKeluar','Input Barang Keluar Reject']];const body=BARANG_REJECT_STATE.loading&&!BARANG_REJECT_STATE.stock.length?`<div class="card"><div class="skeleton-line"></div><div class="skeleton-line"></div><div class="skeleton-line"></div></div>`:BARANG_REJECT_STATE.activeTab==='dashboard'?renderRejectDashboard():BARANG_REJECT_STATE.activeTab==='stock'?renderRejectStock():BARANG_REJECT_STATE.activeTab==='masuk'?renderRejectIn():BARANG_REJECT_STATE.activeTab==='keluar'?renderRejectOut():BARANG_REJECT_STATE.activeTab==='inputMasuk'?renderRejectInputIn():renderRejectInputOut();root.innerHTML=`<div class="reject-meta"><span class="subtitle">Sheet ID: 1BVGcIWnYqrG-DefzmnO_hZjNn3H3c4sriI7CBAWsxw8</span><span class="subtitle">Last sync: ${BARANG_REJECT_STATE.lastSync?formatDateTime(BARANG_REJECT_STATE.lastSync):'-'}</span>${BARANG_REJECT_STATE.error?`<span class="state error">${esc(BARANG_REJECT_STATE.error)}</span>`:''}</div><div class="reject-tabs">${tabs.map(([id,label])=>`<button class="btn-ghost ${BARANG_REJECT_STATE.activeTab===id?'active':''}" data-reject-tab="${id}">${esc(label)}</button>`).join('')}</div>${body}`;bindBarangRejectEvents();if(window.lucide)lucide.createIcons();}
function bindBarangRejectEvents(){document.querySelectorAll('[data-reject-tab]').forEach(btn=>btn.onclick=()=>{BARANG_REJECT_STATE.activeTab=btn.dataset.rejectTab;renderBarangRejectPage();});document.getElementById('rejectRefreshBtn')?.addEventListener('click',()=>fetchBarangRejectData());document.querySelectorAll('[data-reject-filter]').forEach(input=>input.oninput=()=>{BARANG_REJECT_STATE.filters[input.dataset.rejectFilter]=input.value;BARANG_REJECT_STATE.page.stock=BARANG_REJECT_STATE.page.masuk=BARANG_REJECT_STATE.page.keluar=1;renderBarangRejectPage();});document.querySelectorAll('[data-reject-sort]').forEach(btn=>btn.onclick=()=>{const key=btn.dataset.rejectSort;if(BARANG_REJECT_STATE.sort.key===key)BARANG_REJECT_STATE.sort.dir=BARANG_REJECT_STATE.sort.dir==='asc'?'desc':'asc';else BARANG_REJECT_STATE.sort={key,dir:'asc'};renderBarangRejectPage();});document.querySelectorAll('[data-reject-page]').forEach(btn=>btn.onclick=()=>{const k=btn.dataset.rejectPage;BARANG_REJECT_STATE.page[k]=Math.max(1,(BARANG_REJECT_STATE.page[k]||1)+(btn.dataset.dir==='next'?1:-1));renderBarangRejectPage();});document.querySelectorAll('[data-reject-export]').forEach(btn=>btn.onclick=()=>exportBarangRejectCsv(btn.dataset.rejectExport));document.querySelectorAll('[data-reject-scan]').forEach(btn=>btn.onclick=()=>{logActivitySafe({action:'SCAN_BARCODE_REJECT',module:'Barang Reject',detail:'Buka scanner Barang Reject',status:'SUCCESS'});openBarcodeScanner('',text=>{const sku=resolveScannedSku(text)?.sku||String(text||'').trim();BARANG_REJECT_STATE.filters[btn.dataset.rejectScan]=sku;renderBarangRejectPage();});});document.querySelectorAll('[data-reject-scan-field]').forEach(btn=>btn.onclick=()=>{const form=btn.closest('form');const field=btn.dataset.rejectScanField;logActivitySafe({action:'SCAN_BARCODE_REJECT',module:'Barang Reject',detail:'Buka scanner input Barang Reject',status:'SUCCESS'});openBarcodeScanner('',text=>{const sku=resolveScannedSku(text)?.sku||String(text||'').trim();const target=form?.querySelector(`[name="${field}"]`);if(target){target.value=sku;target.dispatchEvent(new Event('input',{bubbles:true}));}});});bindRejectSubmitForms();}
function rejectFormPayload(form){return Object.fromEntries(new FormData(form).entries());}
async function submitRejectForm(kind,form){const isIn=kind==='masuk';const stateKey=isIn?'submittingIn':'submittingOut';if(BARANG_REJECT_STATE[stateKey])return;const payload=rejectFormPayload(form);payload.qty=Number(payload.qty);payload.action=kind;payload.user=currentUserIdentity().user_name||currentUserIdentity().user_id||'';if(!payload.sku||!(payload.qty>0)||!(isIn?payload.lokasi:payload.from)){toast('SKU, Qty > 0, dan Lokasi/From wajib diisi.','error');return;}if(!isIn){const stock=BARANG_REJECT_STATE.stock.find(r=>clean(r.sku)===clean(payload.sku)&&clean(r.lokasi)===clean(payload.from));if(!stock||Number(stock.qty)<payload.qty){toast(`Stok tidak boleh minus. Stok tersedia: ${stock?.qty||0}`,'error');return;}}BARANG_REJECT_STATE[stateKey]=true;renderBarangRejectPage();try{const res=await fetch('/api/barang-reject',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal submit Barang Reject');toast(data.message||'Data berhasil disimpan','success');logActivitySafe({action:isIn?'INPUT_BARANG_MASUK_REJECT':'INPUT_BARANG_KELUAR_REJECT',module:isIn?'Barang Masuk Reject':'Barang Keluar Reject',detail:`${isIn?'Masuk':'Keluar'} Reject SKU ${payload.sku} Qty ${payload.qty}`,reference:payload.sku,status:'SUCCESS',metadata:{oldValue:'',newValue:JSON.stringify(payload)}});form.reset();await fetchBarangRejectData({background:true});BARANG_REJECT_STATE.activeTab=isIn?'masuk':'keluar';}catch(err){toast(err?.message||'Gagal submit Barang Reject','error');logActivitySafe({action:isIn?'INPUT_BARANG_MASUK_REJECT':'INPUT_BARANG_KELUAR_REJECT',module:isIn?'Barang Masuk Reject':'Barang Keluar Reject',detail:err?.message||'Gagal submit',reference:payload.sku,status:'FAILED'});}finally{BARANG_REJECT_STATE[stateKey]=false;renderBarangRejectPage();}}
function bindRejectSubmitForms(){document.getElementById('rejectInForm')?.addEventListener('submit',e=>{e.preventDefault();submitRejectForm('masuk',e.currentTarget);});document.getElementById('rejectOutForm')?.addEventListener('submit',e=>{e.preventDefault();submitRejectForm('keluar',e.currentTarget);});}
