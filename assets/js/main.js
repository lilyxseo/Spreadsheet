...
const CYCLE_STATE={step:1,sessionId:"",search:"",status:"all",rows:[],selected:new Set(),visible:0};
const cycleSessions=JSON.parse(localStorage.getItem(CYCLE_KEY)||"[]");
const cycleSkuPool=()=>[...(DATA["RPL"]||[]),...(DATA["BULKY"]||[])].map(r=>({sku:getVal(r,["sku"])||"",nama:getVal(r,["nama barang","nama"])||"-",lokasi:getVal(r,["lokasi"])||"-",stokSistem:parseNumber(getVal(r,["stok global","kartu stok","stok akhir"])),status:getVal(r,["status"])||""})).filter(r=>clean(r.sku));
function createCycleSession(payload){const id=`cc-${Date.now()}`;const draft={id,status:"draft",step:1,setup:payload,items:[],createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};cycleSessions.unshift(draft);localStorage.setItem(CYCLE_KEY,JSON.stringify(cycleSessions));return draft;}
function addSkuToSession(sessionId,items){const s=cycleSessions.find(x=>x.id===sessionId);if(!s)return;s.items=[...s.items,...items.filter(it=>clean(it.sku)&&!s.items.some(x=>x.sku===it.sku)).map(it=>({...it,qtyAktual:0,selisih:0,statusCount:"Cocok",catatan:""}))];s.updatedAt=new Date().toISOString();localStorage.setItem(CYCLE_KEY,JSON.stringify(cycleSessions));}
function removeSkuFromSession(sessionId,sku){const s=cycleSessions.find(x=>x.id===sessionId);if(!s)return;s.items=s.items.filter(it=>it.sku!==sku);localStorage.setItem(CYCLE_KEY,JSON.stringify(cycleSessions));}
function calculateDifference(stokSistem,qtyAktual){return (Number(qtyAktual)||0)-(Number(stokSistem)||0);}
function updateActualQty(sessionId,sku,qtyAktual){const s=cycleSessions.find(x=>x.id===sessionId);const row=s?.items.find(it=>it.sku===sku);if(!row)return;row.qtyAktual=Number(qtyAktual)||0;row.selisih=calculateDifference(row.stokSistem,row.qtyAktual);row.statusCount=row.selisih===0?"Cocok":row.selisih>0?"Plus":"Minus";localStorage.setItem(CYCLE_KEY,JSON.stringify(cycleSessions));}
function getCycleSummary(session){const total=session.items.length,cocok=session.items.filter(x=>x.selisih===0).length,plus=session.items.filter(x=>x.selisih>0).length,minus=session.items.filter(x=>x.selisih<0).length,totalSelisih=session.items.reduce((n,x)=>n+x.selisih,0),akurasi=total?((cocok/total)*100):0;return{total,cocok,plus,minus,totalSelisih,akurasi};}
function finalizeSession(sessionId){const s=cycleSessions.find(x=>x.id===sessionId);if(!s)return;s.status="selesai";s.step=5;localStorage.setItem(CYCLE_KEY,JSON.stringify(cycleSessions));}
function exportCycleCsv(sessionId){const s=cycleSessions.find(x=>x.id===sessionId);if(!s)return;const csv=["SKU,Nama,Lokasi,Stok Sistem,Qty Aktual,Selisih,Status,Catatan",...s.items.map(r=>[r.sku,r.nama,r.lokasi,r.stokSistem,r.qtyAktual,r.selisih,r.statusCount,(r.catatan||"").replaceAll(","," ")].join(","))].join("\n");downloadBlob(`cycle-count-${sessionId}.csv`,csv,"text/csv;charset=utf-8;");}
function renderCycleCountPage(){if(!cycleCountApp)return;const active=cycleSessions.find(s=>s.id===CYCLE_STATE.sessionId)||cycleSessions[0];if(active)CYCLE_STATE.sessionId=active.id;const summary=active?getCycleSummary(active):{total:0,cocok:0,plus:0,minus:0,totalSelisih:0,akurasi:0};const pool=cycleSkuPool();const q=clean(CYCLE_STATE.search);const filtered=pool.filter(r=>(!q||clean(`${r.sku} ${r.nama}`).includes(q))&&(CYCLE_STATE.status==="all"||clean(r.status).includes(clean(CYCLE_STATE.status))));CYCLE_STATE.rows=filtered.slice(0,120);cycleCountApp.innerHTML=`<div class='cc-shell'><h3 class='page-title'>Cycle Count</h3><div class='cc-stepper'>${["Setup","Pilih SKU","Input Aktual","Review","Hasil"].map((x,i)=>`<span class='${(active?.step||1)>=i+1?"active":""}'>${i+1}. ${x}</span>`).join("")}</div><div class='cc-grid'><div class='card cc-card'><h4>List Session</h4><div class='cc-list'>${cycleSessions.map(s=>`<button class='btn-ghost ${s.id===CYCLE_STATE.sessionId?"active":""}' data-cc-open='${s.id}'>${s.setup?.nama||"Untitled"} · ${s.status}</button>`).join("")||"<div class='state'>Belum ada session</div>"}</div><button class='btn-primary' data-cc-new>+ Session Baru</button></div><div class='card cc-card'><h4>Step ${(active?.step||1)}: ${(active?.step||1)===1?"Setup":(active?.step||1)===2?"Pilih SKU":(active?.step||1)===3?"Input Aktual":(active?.step||1)===4?"Review":"Hasil"}</h4>${renderCycleStep(active,summary)}<div class='row'><button class='btn-ghost' data-cc-prev>Back</button><button class='btn-primary' data-cc-next>Next</button></div></div></div><div class='card cc-card'><h4>Master SKU</h4><div class='mv-toolbar'><input id='ccSearch' placeholder='Cari SKU / nama barang'><select id='ccStatus'><option value='all'>Semua</option><option value='minus'>stok minus</option><option value='anomaly'>anomaly</option></select></div><div class='table-wrap'><table><thead><tr><th>SKU</th><th>Nama</th><th>Lokasi</th><th>Stok</th><th></th></tr></thead><tbody>${CYCLE_STATE.rows.map(r=>`<tr><td>${esc(r.sku)}</td><td>${highlight(r.nama,CYCLE_STATE.search)}</td><td>${esc(r.lokasi)}</td><td>${r.stokSistem}</td><td><button class='btn-ghost' data-cc-add='${encAttr(r.sku)}'>Tambah</button></td></tr>`).join("")}</tbody></table></div></div></div>`;bindCycleEvents(active);}
function renderCycleStep(active,summary){if(!active)return `<div class='state'>Buat session untuk mulai cycle count.</div>`;if(active.step===1)return `<div class='cc-form'><input id='ccNama' placeholder='Nama sesi' value='${esc(active.setup?.nama||"")}'><input id='ccTanggal' type='date' value='${esc(active.setup?.tanggal||new Date().toISOString().slice(0,10))}'><input id='ccPic' placeholder='PIC' value='${esc(active.setup?.pic||"")}'><input id='ccLokasi' placeholder='Lokasi (opsional)' value='${esc(active.setup?.lokasi||"")}'><textarea id='ccCatatan' placeholder='Catatan'>${esc(active.setup?.catatan||"")}</textarea></div>`;if(active.step===3||active.step===5)return `<div class='table-wrap table-wrap-full'><table><thead><tr><th>SKU</th><th>Nama</th><th>Stok Sistem</th><th>Qty Aktual</th><th>Selisih</th><th>Status</th></tr></thead><tbody>${active.items.map((r,i)=>`<tr class='cc-${clean(r.statusCount)}'><td>${r.sku}</td><td>${r.nama}</td><td>${r.stokSistem}</td><td><input data-cc-qty='${r.sku}' value='${r.qtyAktual||0}' ${active.status==="selesai"?"disabled":""}></td><td>${r.selisih||0}</td><td>${r.statusCount||"Cocok"}</td></tr>`).join("")}</tbody></table></div>`;if(active.step===4)return `<div class='grid dashboard'><div class='metric'><div class='k'>Total SKU</div><div class='v'>${summary.total}</div></div><div class='metric ok'><div class='k'>Cocok</div><div class='v'>${summary.cocok}</div></div><div class='metric warn'><div class='k'>Plus/Minus</div><div class='v'>${summary.plus+summary.minus}</div></div><div class='metric'><div class='k'>Akurasi</div><div class='v'>${summary.akurasi.toFixed(2)}%</div></div></div>`;return `<div class='state'>Pilih SKU untuk dihitung: ${active.items.length} item.</div>`;}
function bindCycleEvents(active){cycleCountApp.querySelector("[data-cc-new]")?.addEventListener("click",()=>{const s=createCycleSession({nama:`Cycle ${new Date().toLocaleDateString("id-ID")}`,tanggal:new Date().toISOString().slice(0,10),pic:"",lokasi:"",catatan:""});CYCLE_STATE.sessionId=s.id;renderCycleCountPage();toast("Draft session tersimpan","success");});cycleCountApp.querySelectorAll("[data-cc-open]").forEach(b=>b.addEventListener("click",()=>{CYCLE_STATE.sessionId=b.dataset.ccOpen;renderCycleCountPage();}));cycleCountApp.querySelectorAll("[data-cc-add]").forEach(b=>b.addEventListener("click",()=>{const sku=decodeURIComponent(b.dataset.ccAdd||"");const item=cycleSkuPool().find(x=>x.sku===sku);if(active&&item){addSkuToSession(active.id,[item]);toast("SKU ditambahkan","success");renderCycleCountPage();}}));cycleCountApp.querySelector("#ccSearch")?.addEventListener("input",debounce(e=>{CYCLE_STATE.search=e.target.value;renderCycleCountPage();},250));cycleCountApp.querySelector("#ccStatus")?.addEventListener("change",e=>{CYCLE_STATE.status=e.target.value;renderCycleCountPage();});cycleCountApp.querySelector("[data-cc-next]")?.addEventListener("click",()=>{if(!active)return;if(active.step===4){finalizeSession(active.id);toast("Session difinalisasi","success");}else active.step=Math.min(5,(active.step||1)+1);localStorage.setItem(CYCLE_KEY,JSON.stringify(cycleSessions));renderCycleCountPage();});cycleCountApp.querySelector("[data-cc-prev]")?.addEventListener("click",()=>{if(!active)return;active.step=Math.max(1,(active.step||1)-1);localStorage.setItem(CYCLE_KEY,JSON.stringify(cycleSessions));renderCycleCountPage();});cycleCountApp.querySelectorAll("[data-cc-qty]").forEach((inp,idx,arr)=>inp.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();arr[idx+1]?.focus();}}));cycleCountApp.querySelectorAll("[data-cc-qty]").forEach(inp=>inp.addEventListener("input",e=>{if(active)updateActualQty(active.id,inp.dataset.ccQty,e.target.value);}));}
