...
const startIdx=(textRows||[]).findIndex(row=>/\bSKU\b/i.test(row)&&/Nama\s*Produk/i.test(row)&&/Jumlah/i.test(row));
...
if(!line)continue;
if(csvRows.length>1&&/^(grand\s+total|total|status\s+transfer|nomor\s+referensi|perkiraan\s+tanggal)\b/i.test(line))break;
if(/\bSKU\b/i.test(line)&&/Nama\s*Produk/i.test(line))continue;
const split=splitCollapsedTransferCsvLine(line);
if(split)csvRows.push(split);
else csvRows.push([line,'','','','','','']);
}
