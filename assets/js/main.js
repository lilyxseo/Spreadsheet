...
function computeMovementSuggestions(){const source=getMovementSourceRows();const key=`${MOVEMENT_STATE.suggestionThreshold}|${getMovementSuggestionSourceKey(source)}`;if(MOVEMENT_STATE.suggestionCache.key===key)return MOVEMENT_STATE.suggestionCache;const grouped=new Map();for(const row of source){const sku=String(row.sku||"").trim();if(!sku)continue;const qty=Number(row.stok_akhir)||0;if(!grouped.has(sku))grouped.set(sku,{sku,nama:row.nama||"-",locations:[],totalQty:0});const bucket=grouped.get(sku);bucket.locations.push({lokasi:row.lokasi||"-",qty});bucket.totalQty+=qty;}const suggestions=[];grouped.forEach(item=>{if(item.locations.length<=1)return;const sorted=[...item.locations].sort((a,b)=>b.qty-a.qty);const target=sorted[0];if(!target||target.qty<=0)return;for(let i=1;i<sorted.length;i++){const origin=sorted[i];if(origin.qty<=0||origin.qty>=MOVEMENT_STATE.suggestionThreshold)continue;if(item.totalQty<=origin.qty)continue;suggestions.push({sku:item.sku,nama:item.nama,fromLokasi:origin.lokasi,toLokasi:target.lokasi,toQty:target.qty,qtyMove:origin.qty,totalLokasi:item.locations.length,totalQty:item.totalQty});}});MOVEMENT_STATE.suggestionCache={key,rows:suggestions,count:suggestions.length};return MOVEMENT_STATE.suggestionCache;}
