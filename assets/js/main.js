...
const STOCK_ACCURACY_CACHE_KEY="STOCK_ACCURACY_CACHE";
...
const cache=safeJsonParse(localStorage.getItem(STOCK_ACCURACY_CACHE_KEY),null);
if(cache?.rows?.length&&!STATS_STATE._normalizedRows){STATS_STATE._normalizedRows=cache.rows;renderStatsFromNormalized(cache.meta||{});}
setTimeout(()=>updateStatsBackground(),0);
}
...
localStorage.setItem(STOCK_ACCURACY_CACHE_KEY,JSON.stringify({rows:STATS_STATE._normalizedRows,meta:{totalSku,akurasi,skuTidakAkurat,selisihTotal,skuAkurat}}));
renderStatsFromNormalized({totalSku,akurasi,skuTidakAkurat,selisihTotal});}
function renderStatsFromNormalized(meta={}){
const totalSku=meta.totalSku??(STATS_STATE._normalizedRows||[]).length;
const akurasi=meta.akurasi??0;
const skuTidakAkurat=meta.skuTidakAkurat??0;
const selisihTotal=meta.selisihTotal??0;
statsCards.innerHTML=[["Total SKU",totalSku,""],["Akurat (%)",`${Number(akurasi).toFixed(2)}%`,"ok"],["Tidak Akurat",skuTidakAkurat,"err"],["Selisih Total",selisihTotal,"warn"]].map(c=>`<div class='metric ${c[2]||""}'><div class='k'>${c[0]}</div><div class='v'>${esc(c[1])}</div></div>`).join("");
...
const ABC_STATE={periodMonths:3,orderType:"Semua",selectedTo:[],excludeTo:["REJECT","RUSAK"],sortBy:"Prioritas Order",toSearch:"",toDropdownOpen:false,selectedRows:new Set(),page:1,pageSize:25,debounceTimer:null,isProcessing:false,lastKey:"",renderedRows:[]};
...
if(result.rows.length||result.sourceHasData)localStorage.setItem(ABC_ANALYSIS_CACHE_KEY,JSON.stringify(result));return result;
}
function renderAbcAnalisisPage(forceRefresh=false){
if(!abcAnalisisApp)return;
abcAnalisisApp.innerHTML="<div class='card'><div class='subtitle'>Memproses...</div><div class='state'>Menyiapkan analisis ABC...</div></div>";
const cached=!forceRefresh?safeJsonParse(localStorage.getItem(ABC_ANALYSIS_CACHE_KEY),null):null;
if(cached?.rows?.length){renderAbcAnalisisReady(cached);}
setTimeout(()=>{const data=buildAbcAnalysis({forceRefresh});renderAbcAnalisisReady(data);},0);
}
function renderAbcAnalisisReady(data){
