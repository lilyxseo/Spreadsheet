...
function normalizeChecked(val){return val===true||val==="TRUE"||val==="true";}
function renderBalikanTable(){const rows=BALIKAN_STATE.rows||[];if(!rows.length){balikanTable.innerHTML='<div class="subtitle">Data kosong.</div>';return;}balikanTable.innerHTML=`<div class="balikan-table-wrapper"><table class="balikan-table"><thead><tr><th class="col-check">Centang</th><th>No</th><th>SKU</th><th>Nama Barang</th><th>Qty</th><th>Rak tujuan</th><th>Lokasi</th><th>Stok Bulky</th><th>Stok Retail</th><th>Status</th><th>Keterangan</th></tr></thead><tbody>${rows.map(r=>`<tr><td class="col-check"><input type="checkbox" ${normalizeChecked(r.checked)?'checked':''} data-row="${Number(r.rowNumber)}" onchange="toggleBalikanCheck(${Number(r.rowNumber)})"></td><td>${esc(r.no)}</td><td>${esc(r.sku)}</td><td>${esc(r.namaBarang)}</td><td>${esc(r.qty)}</td><td>${esc(r.rakTujuan)}</td><td>${esc(r.lokasi)}</td><td>${esc(r.stokBulky)}</td><td>${esc(r.stokRetail)}</td><td>${esc(r.status)}</td><td>${esc(r.keterangan)}</td></tr>`).join('')}</tbody></table></div>`;}
...
window.toggleBalikanCheck=(rowNumber)=>{const input=document.querySelector(`input[onchange="toggleBalikanCheck(${Number(rowNumber)})"]`);if(!input)return;updateBalikanCheck(Number(rowNumber),input.checked).catch(err=>{toast(err?.message||'Gagal update centang','error');input.checked=!input.checked;});};
