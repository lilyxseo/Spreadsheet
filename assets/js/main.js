...
if(document.getElementById("abcToSearch"))document.getElementById("abcToSearch").oninput=e=>{const q=e.target.value||"";ABC_STATE.toSearch=q;const cq=clean(q);const list=document.getElementById('abcToList');if(list){const items=list.querySelectorAll('[data-abc-to-item]');let shown=0;items.forEach(item=>{const txt=item.textContent||'';const show=!cq||clean(txt).includes(cq);item.style.display=show?'':'none';if(show)shown++;});const empty=list.querySelector('[data-empty="1"]');if(empty)empty.remove();if(!shown){const d=document.createElement('div');d.className='subtitle';d.dataset.empty='1';d.textContent='Tidak ada tujuan';list.appendChild(d);}}};
