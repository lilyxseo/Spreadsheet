...
const totalRowCount=isIn?st.filtered.filter(r=>normalizeStatus(r._status)==='barang masuk').length:st.filtered.length;
...
const CYCLE_KEY="cycle_count_sessions_v1";
const CYCLE_STATE={step:1,activeId:"",search:"",filters:{lokasi:"all",kategori:"all",status:"all"},selected:new Set(),draft:{name:"",date:new Date().toISOString().slice(0,10),pic:"",lokasi:"",catatan:""}};
function getCycleRows(){const rows=(DATA["Kartu Stock"]||[]).map(r=>({sku:getVal(r,["sku"]).trim(),nama:getVal(r,["nama barang","nama","item","description"]).trim()||"-",lokasi:getVal(r,["lokasi","location","rak","bin","area"]).trim()||"-",stokSistem:parseNumber(getVal(r,["stok akhir","stok global","closing stock","ending stock","saldo akhir"])),kategori:getVal(r,["kategori","category"]).trim()||"General"})).filter(r=>r.sku);const by=new Map();rows.forEach(r=>{const k=clean(r.sku);if(!by.has(k))by.set(k,r);});return [...by.values()];}
function getSessions(){try{return JSON.parse(localStorage.getItem(CYCLE_KEY)||"[]");}catch(_){return []}}
function saveSessions(v){localStorage.setItem(CYCLE_KEY,JSON.stringify(v));toast("Draft cycle count tersimpan","success");}
function createCycleSession(payload){const all=getSessions();const id=`cc_${Date.now()}`;const ses={id,status:"draft",createdAt:new Date().toISOString(),setup:payload,items:[]};all.unshift(ses);saveSessions(all);return ses;}
function addSkuToSession(id,rows){const all=getSessions();const ses=all.find(x=>x.id===id);if(!ses)return;const seen=new Set(ses.items.map(i=>clean(i.sku)));rows.forEach(r=>{if(!r.sku||seen.has(clean(r.sku)))return;ses.items.push({...r,qtyAktual:0,selisih:0,statusHitung:"Cocok",catatan:""});});saveSessions(all);}
function removeSkuFromSession(id,sku){const all=getSessions();const ses=all.find(x=>x.id===id);if(!ses)return;ses.items=ses.items.filter(i=>clean(i.sku)!==clean(sku));saveSessions(all);}
function calculateDifference(item){const qty=Number(item.qtyAktual)||0;const diff=qty-(Number(item.stokSistem)||0);return {selisih:diff,statusHitung:diff===0?"Cocok":(diff>0?"Plus":"Minus")};}
function updateActualQty(id,sku,qty,catatan=""){const all=getSessions();const ses=all.find(x=>x.id===id);if(!ses)return;const row=ses.items.find(i=>clean(i.sku)===clean(sku));if(!row)return;row.qtyAktual=Number(qty)||0;row.catatan=catatan;Object.assign(row,calculateDifference(row));saveSessions(all);}
function getCycleSummary(ses){const total=ses.items.length||0,cocok=ses.items.filter(i=>i.statusHitung==="Cocok").length,plus=ses.items.filter(i=>i.statusHitung==="Plus").length,minus=ses.items.filter(i=>i.statusHitung==="Minus").length,totalSelisih=ses.items.reduce((n,i)=>n+i.selisih,0),akurasi=total?((cocok/total)*100):0;return{total,cocok,plus,minus,totalSelisih,akurasi};}
function finalizeSession(id){const all=getSessions();const ses=all.find(x=>x.id===id);if(!ses)return;ses.status="selesai";ses.finalizedAt=new Date().toISOString();saveSessions(all);}
function exportCycleCsv(id){const ses=getSessions().find(x=>x.id===id);if(!ses)return;const cols=["sku","nama","lokasi","stok_sistem","qty_aktual","selisih","status","catatan"];const lines=[cols.join(",")].concat(ses.items.map(r=>[r.sku,r.nama,r.lokasi,r.stokSistem,r.qtyAktual,r.selisih,r.statusHitung,r.catatan].map(v=>`"${String(v??"").replaceAll('"','""')}"`).join(",")));const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([lines.join("\\n")],{type:"text/csv;charset=utf-8;"}));a.download=`cycle-count-${id}.csv`;a.click();}
function renderCycleCountPage(){if(!cycleCountApp)return;const sessions=getSessions();const active=sessions.find(s=>s.id===CYCLE_STATE.activeId)||sessions[0];if(active)CYCLE_STATE.activeId=active.id;const ses=active;const summary=ses?getCycleSummary(ses):null;cycleCountApp.innerHTML=`<div class='cc-grid'><div class='card cc-card'><div class='section-header'><h4>List Session</h4><button class='btn-primary' onclick='ccNewSession()'>Session Baru</button></div><div class='cc-session-list'>${sessions.map(s=>`<button class='cc-session ${s.id===CYCLE_STATE.activeId?"active":""}' onclick="ccOpen('${s.id}')"><strong>${esc(s.setup?.name||'Tanpa Nama')}</strong><small>${s.status} · ${new Date(s.createdAt).toLocaleDateString('id-ID')}</small></button>`).join('')||"<div class='state'>Belum ada session.</div>"}</div></div><div class='card cc-card'>${ses?renderCycleWizard(ses,summary):"<div class='state'>Buat session baru untuk mulai audit.</div>"}</div></div>`;}
function renderCycleWizard(ses,sum){const step=CYCLE_STATE.step;return `<div class='cc-stepper'>${["Setup","Pilih SKU","Input Aktual","Review","Hasil"].map((s,i)=>`<span class='cc-step ${i+1<=step?"on":""}'>${i+1}. ${s}</span>`).join('')}</div>${step===1?renderStep1(ses):step===2?renderStep2(ses):step===3?renderStep3(ses):step===4?renderStep4(sum):renderStep5(ses,sum)}<div class='row' style='margin-top:12px'><button class='btn-ghost' ${step===1?'disabled':''} onclick='ccPrev()'>Back</button><button class='btn-primary' ${step===5?'disabled':''} onclick='ccNext()'>Next</button></div>`;}
function renderStep1(ses){const d=ses.setup||{};const loks=[...new Set(getCycleRows().map(r=>r.lokasi))].slice(0,300);return `<div class='cc-form'><input id='ccName' placeholder='Nama Sesi' value='${esc(d.name||"")}'><input id='ccDate' type='date' value='${esc(d.date||new Date().toISOString().slice(0,10))}'><input id='ccPic' placeholder='PIC' value='${esc(d.pic||"")}'><input list='ccLokasiList' id='ccLokasi' placeholder='Lokasi (opsional)' value='${esc(d.lokasi||"")}'><datalist id='ccLokasiList'>${loks.map(l=>`<option value='${esc(l)}'>`).join('')}</datalist><textarea id='ccCatatan' placeholder='Catatan'>${esc(d.catatan||"")}</textarea><button class='btn-ghost' onclick='ccSaveSetup()'>Simpan Draft</button></div>`;}
function renderStep2(ses){const all=getCycleRows();const q=clean(CYCLE_STATE.search);let rows=all.filter(r=>!q||clean(`${r.sku} ${r.nama}`).includes(q));if(CYCLE_STATE.filters.status==='minus')rows=rows.filter(r=>r.stokSistem<0);const preview=rows.slice(0,120);return `<div><div class='mv-toolbar'><input id='ccSearch' class='search-lg' placeholder='Search SKU/Nama' value='${esc(CYCLE_STATE.search)}'><button class='btn-ghost' onclick='ccPickMinus()'>Ambil stok minus</button></div><div class='cc-chip'>${[...CYCLE_STATE.selected].map(s=>`<span>${esc(s)} <button onclick="ccToggleSku('${encAttr(s)}')">×</button></span>`).join('')}</div><div class='table-wrap'><table><thead><tr><th></th><th>SKU</th><th>Nama</th><th>Lokasi</th><th>Stok Sistem</th></tr></thead><tbody>${preview.map(r=>`<tr><td><input type='checkbox' ${CYCLE_STATE.selected.has(r.sku)?'checked':''} onchange="ccToggleSku('${encAttr(r.sku)}')"></td><td>${highlight(r.sku,CYCLE_STATE.search)}</td><td>${highlight(r.nama,CYCLE_STATE.search)}</td><td>${esc(r.lokasi)}</td><td>${r.stokSistem}</td></tr>`).join('')}</tbody></table></div><button class='btn-primary' onclick='ccApplySelected()'>Tambah ke Session</button></div>`;}
function renderStep3(ses){const rows=(ses.items||[]).slice(0,200);return `<div class='table-wrap'><table><thead><tr><th>SKU</th><th>Nama</th><th>Lokasi</th><th>Sistem</th><th>Aktual</th><th>Selisih</th><th>Status</th><th>Catatan</th></tr></thead><tbody>${rows.map((r,i)=>`<tr class='cc-${clean(r.statusHitung)}'><td>${esc(r.sku)}</td><td>${esc(r.nama)}</td><td>${esc(r.lokasi)}</td><td>${r.stokSistem}</td><td><input data-idx='${i}' class='cc-qty' value='${r.qtyAktual??0}'></td><td>${r.selisih}</td><td>${r.statusHitung}</td><td><input data-note='${i}' value='${esc(r.catatan||"")}'></td></tr>`).join('')}</tbody></table></div>`;}
function renderStep4(sum){return `<div class='summary-grid'><div class='summary-card'><div class='k'>Total SKU</div><div class='v'>${sum.total}</div></div><div class='summary-card'><div class='k'>Cocok</div><div class='v'>${sum.cocok}</div></div><div class='summary-card'><div class='k'>Tidak Cocok</div><div class='v'>${sum.plus+sum.minus}</div></div><div class='summary-card'><div class='k'>Akurasi</div><div class='v'>${sum.akurasi.toFixed(2)}%</div></div></div>`;}
function renderStep5(ses,sum){const lock=ses.status==='selesai';return `<div><div class='row'><button class='btn-primary' ${lock?'disabled':''} onclick='ccFinalize()'>Finalisasi</button><button class='btn-ghost' onclick="exportCycleCsv('${ses.id}')">Export CSV</button></div>${renderStep4(sum)}</div>`;}
window.ccNewSession=()=>{const ses=createCycleSession({name:`Cycle Count ${new Date().toLocaleDateString('id-ID')}`,date:new Date().toISOString().slice(0,10),pic:'',lokasi:'',catatan:''});CYCLE_STATE.activeId=ses.id;CYCLE_STATE.step=1;renderCycleCountPage();};window.ccOpen=(id)=>{CYCLE_STATE.activeId=id;CYCLE_STATE.step=1;renderCycleCountPage();};window.ccPrev=()=>{CYCLE_STATE.step=Math.max(1,CYCLE_STATE.step-1);renderCycleCountPage();};window.ccNext=()=>{if(CYCLE_STATE.step===3){const ses=getSessions().find(s=>s.id===CYCLE_STATE.activeId);document.querySelectorAll('.cc-qty').forEach((el,i)=>updateActualQty(ses.id,ses.items[i].sku,el.value,document.querySelectorAll('[data-note]')[i]?.value||""));}CYCLE_STATE.step=Math.min(5,CYCLE_STATE.step+1);renderCycleCountPage();};window.ccSaveSetup=()=>{const all=getSessions();const ses=all.find(s=>s.id===CYCLE_STATE.activeId);if(!ses)return;ses.setup={name:ccName.value,date:ccDate.value,pic:ccPic.value,lokasi:ccLokasi.value,catatan:ccCatatan.value};saveSessions(all);renderCycleCountPage();};window.ccToggleSku=(skuEnc)=>{const sku=decodeURIComponent(skuEnc);if(CYCLE_STATE.selected.has(sku))CYCLE_STATE.selected.delete(sku);else CYCLE_STATE.selected.add(sku);renderCycleCountPage();};window.ccApplySelected=()=>{const rows=getCycleRows().filter(r=>CYCLE_STATE.selected.has(r.sku));addSkuToSession(CYCLE_STATE.activeId,rows);toast('SKU ditambahkan','success');};window.ccPickMinus=()=>{getCycleRows().filter(r=>r.stokSistem<0).slice(0,100).forEach(r=>CYCLE_STATE.selected.add(r.sku));renderCycleCountPage();};window.ccFinalize=()=>{finalizeSession(CYCLE_STATE.activeId);renderCycleCountPage();};document.addEventListener('input',e=>{if(e.target?.id==='ccSearch'){CYCLE_STATE.search=e.target.value;renderCycleCountPage();}});
window.exportCycleCsv=exportCycleCsv;


const ARCHIVE_SHEET_CANDIDATES=["SO JULI 2025"];
const ARCHIVE_STATE={sheetList:[],selectedSheet:"",cache:{},loadingList:false,loadingData:false,searchInput:"",search:"",searchTimer:null,location:"all",selisih:"all",stok:"all",sort:"nama_az",page:1,pageSize:25};

function bindArchiveEvents(){document.addEventListener("change",e=>{if(e.target?.id==="archiveSheetSelect"){selectArchiveSheet(e.target.value);}if(e.target?.id==="archiveLocationFilter"){ARCHIVE_STATE.location=e.target.value;ARCHIVE_STATE.page=1;renderArchivePage();}if(e.target?.id==="archiveSelisihFilter"){ARCHIVE_STATE.selisih=e.target.value;ARCHIVE_STATE.page=1;renderArchivePage();}if(e.target?.id==="archiveStokFilter"){ARCHIVE_STATE.stok=e.target.value;ARCHIVE_STATE.page=1;renderArchivePage();}if(e.target?.id==="archiveSort"){ARCHIVE_STATE.sort=e.target.value;renderArchivePage();}});document.addEventListener("input",e=>{if(e.target?.id!=="archiveSearch")return;ARCHIVE_STATE.searchInput=e.target.value||"";clearTimeout(ARCHIVE_STATE.searchTimer);ARCHIVE_STATE.searchTimer=setTimeout(()=>{ARCHIVE_STATE.search=ARCHIVE_STATE.searchInput;ARCHIVE_STATE.page=1;renderArchivePage();},300);});document.addEventListener("click",e=>{if(e.target?.closest("#archiveRefreshBtn"))refreshArchive();const pg=e.target?.closest("[data-archive-page]");if(pg){ARCHIVE_STATE.page=Math.max(1,ARCHIVE_STATE.page+(Number(pg.dataset.archivePage)||0));renderArchivePage();}});}
async function fetchArchiveSheetList(){const names=[];for(const name of ARCHIVE_SHEET_CANDIDATES){try{const values=await fetchSheet(name);if(Array.isArray(values)&&values.length)names.push(name);}catch(_){}}return names;}
async function fetchArchiveSheetData(sheetName){const raw=await fetchSheet(sheetName);return parseSheet(raw,sheetName);}
async function ensureArchiveList(){if(ARCHIVE_STATE.sheetList.length||ARCHIVE_STATE.loadingList)return;ARCHIVE_STATE.loadingList=true;renderArchivePage();ARCHIVE_STATE.sheetList=await fetchArchiveSheetList();ARCHIVE_STATE.loadingList=false;renderArchivePage();}
async function selectArchiveSheet(sheetName){ARCHIVE_STATE.selectedSheet=sheetName||"";ARCHIVE_STATE.page=1;if(!sheetName){renderArchivePage();return;}if(ARCHIVE_STATE.cache[sheetName]){renderArchivePage();return;}ARCHIVE_STATE.loadingData=true;renderArchivePage();try{ARCHIVE_STATE.cache[sheetName]=await fetchArchiveSheetData(sheetName);}finally{ARCHIVE_STATE.loadingData=false;renderArchivePage();}}
async function refreshArchive(){const sheet=ARCHIVE_STATE.selectedSheet;if(sheet)delete ARCHIVE_STATE.cache[sheet];ARCHIVE_STATE.sheetList=[];await ensureArchiveList();if(sheet)await selectArchiveSheet(sheet);}
function getArchiveRows(){const rows=ARCHIVE_STATE.cache[ARCHIVE_STATE.selectedSheet]||[];const withCalc=rows.map(r=>{const stok=parseNumber(getVal(r,["stok akhir"]));const c1=parseNumber(getVal(r,["count 1"]));const c2=parseNumber(getVal(r,["count 2"]));const s1=c1-stok,s2=c2-stok;let status="Cocok";if((!c1&&!c2))status="Belum Count";else if(s1!==0||s2!==0)status="Selisih";return {...r,__stok:stok,__c1:c1,__c2:c2,__s1:s1,__s2:s2,__status:status,__lokasi:getVal(r,["lokasi"]),__sku:getVal(r,["sku"]),__nama:getVal(r,["nama barang","nama"])};});
const q=clean(ARCHIVE_STATE.search);let out=withCalc.filter(r=>!q||clean(`${r.__sku} ${r.__nama} ${r.__lokasi}`).includes(q));if(ARCHIVE_STATE.location!=="all")out=out.filter(r=>r.__lokasi===ARCHIVE_STATE.location);if(ARCHIVE_STATE.selisih!=="all")out=out.filter(r=>ARCHIVE_STATE.selisih==="cocok"?(r.__s1===0&&r.__s2===0):ARCHIVE_STATE.selisih==="s1"?(r.__s1!==0&&r.__s2===0):ARCHIVE_STATE.selisih==="s2"?(r.__s1===0&&r.__s2!==0):ARCHIVE_STATE.selisih==="both"?(r.__s1!==0&&r.__s2!==0):(r.__s1!==0||r.__s2!==0));if(ARCHIVE_STATE.stok!=="all")out=out.filter(r=>ARCHIVE_STATE.stok==="zero"?r.__stok===0:ARCHIVE_STATE.stok==="pos"?r.__stok>0:r.__stok<0);out.sort((a,b)=>ARCHIVE_STATE.sort==="nama_az"?a.__nama.localeCompare(b.__nama):ARCHIVE_STATE.sort==="sku_az"?a.__sku.localeCompare(b.__sku):ARCHIVE_STATE.sort==="stok_desc"?b.__stok-a.__stok:Math.max(Math.abs(b.__s1),Math.abs(b.__s2))-Math.max(Math.abs(a.__s1),Math.abs(a.__s2)));return out;}
function renderArchivePage(){if(!archiveApp)return; if(!ARCHIVE_STATE.sheetList.length&&!ARCHIVE_STATE.loadingList)ensureArchiveList();const selected=ARCHIVE_STATE.selectedSheet;const rows=selected?getArchiveRows():[];const allRows=ARCHIVE_STATE.cache[selected]||[];const lokasiOps=[...new Set(allRows.map(r=>getVal(r,["lokasi"])).filter(Boolean))].sort((a,b)=>a.localeCompare(b));const totalPage=Math.max(1,Math.ceil(rows.length/ARCHIVE_STATE.pageSize));if(ARCHIVE_STATE.page>totalPage)ARCHIVE_STATE.page=totalPage;const start=(ARCHIVE_STATE.page-1)*ARCHIVE_STATE.pageSize;const pageRows=rows.slice(start,start+ARCHIVE_STATE.pageSize);const sum={sku:new Set(allRows.map(r=>getVal(r,["sku"])).filter(Boolean)).size,lokasi:new Set(allRows.map(r=>getVal(r,["lokasi"])).filter(Boolean)).size,s1:allRows.reduce((n,r)=>n+Math.abs(parseNumber(getVal(r,["count 1"]))-parseNumber(getVal(r,["stok akhir"]))),0),s2:allRows.reduce((n,r)=>n+Math.abs(parseNumber(getVal(r,["count 2"]))-parseNumber(getVal(r,["stok akhir"]))),0)};archiveApp.innerHTML=`<div class='card'><div class='section-header'><div><h3 class='page-title'>Arsip</h3></div><div class='row'><select id='archiveSheetSelect'><option value=''>Pilih Sheet Arsip</option>${ARCHIVE_STATE.sheetList.map(s=>`<option ${s===selected?"selected":""}>${esc(s)}</option>`).join("")}</select><button id='archiveRefreshBtn' class='btn-ghost'>Refresh Arsip</button></div></div><div class='mv-filters open'><input id='archiveSearch' class='search-lg' placeholder='Cari SKU / Nama Barang / Lokasi' value='${esc(ARCHIVE_STATE.searchInput)}'/><select id='archiveLocationFilter'><option value='all'>Semua Lokasi</option>${lokasiOps.map(l=>`<option value='${esc(l)}' ${ARCHIVE_STATE.location===l?"selected":""}>${esc(l)}</option>`).join("")}</select><select id='archiveSelisihFilter'><option value='all'>Semua Selisih</option><option value='cocok'>Cocok (stok = count)</option><option value='s1'>Selisih Count 1</option><option value='s2'>Selisih Count 2</option><option value='both'>Selisih Keduanya</option></select><select id='archiveStokFilter'><option value='all'>Semua Stok</option><option value='zero'>Stok kosong (0)</option><option value='pos'>Stok > 0</option><option value='neg'>Stok < 0</option></select><select id='archiveSort'><option value='nama_az'>Nama Barang A-Z</option><option value='sku_az'>SKU A-Z</option><option value='stok_desc'>Stok terbesar</option><option value='selisih_desc'>Selisih terbesar</option></select></div>${ARCHIVE_STATE.loadingList?"<div class='state'>Memuat daftar sheet arsip...</div>":!selected?"<div class='state'>Pilih sheet arsip untuk melihat data</div>":ARCHIVE_STATE.loadingData?"<div class='state'>Memuat data sheet...</div>":!rows.length?"<div class='state'>Tidak ada data</div>":`<div class='grid dashboard'><div class='metric'><div class='k'>Total SKU</div><div class='v'>${sum.sku}</div></div><div class='metric'><div class='k'>Total Lokasi</div><div class='v'>${sum.lokasi}</div></div><div class='metric'><div class='k'>Total Selisih Count 1</div><div class='v'>${sum.s1}</div></div><div class='metric'><div class='k'>Total Selisih Count 2</div><div class='v'>${sum.s2}</div></div></div><div class='table-wrap'><table><thead><tr><th>Lokasi</th><th>SKU</th><th>Nama Barang</th><th>Stok Akhir</th><th>Count 1</th><th>Count 2</th><th>Selisih 1</th><th>Selisih 2</th><th>Status</th></tr></thead><tbody>${pageRows.map(r=>`<tr><td>${esc(r.__lokasi)}</td><td>${esc(r.__sku)}</td><td>${esc(r.__nama)}</td><td>${r.__stok}</td><td>${r.__c1}</td><td>${r.__c2}</td><td>${r.__s1}</td><td>${r.__s2}</td><td>${r.__status}</td></tr>`).join("")}</tbody></table></div><div class='mv-pagination'><span>Menampilkan ${rows.length?start+1:0}-${Math.min(start+ARCHIVE_STATE.pageSize,rows.length)} dari ${rows.length}</span><div class='row'><button class='btn-ghost' data-archive-page='-1'>Prev</button><button class='btn-ghost' data-archive-page='1'>Next</button></div></div>`}</div>`;const sel=document.getElementById('archiveSelisihFilter');if(sel)sel.value=ARCHIVE_STATE.selisih;const stk=document.getElementById('archiveStokFilter');if(stk)stk.value=ARCHIVE_STATE.stok;const so=document.getElementById('archiveSort');if(so)so.value=ARCHIVE_STATE.sort;}
