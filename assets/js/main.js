...
function syncAutoRefreshButton(){if(!refreshToggleHeader)return;const active=isAutoRefreshEnabled();refreshToggleHeader.innerHTML=`<i data-lucide="${active?"refresh-cw":"pause"}"></i>`;refreshToggleHeader.title=`Auto refresh ${active?"ON":"OFF"}`;refreshToggleHeader.setAttribute("aria-label",`Toggle auto refresh (${active?"ON":"OFF"})`);if(active&&!autoRefreshTimer)startAutoRefresh();if(!active&&autoRefreshTimer)stopAutoRefresh();if(window.lucide)lucide.createIcons();}
