...
const CYCLE_KEY="inventory_cycle_count_v1";
const CYCLE_STATE={sessions:[],activeId:"",skuQuery:"",page:1,pageSize:25,__loaded:false};
function saveCycleState(){localStorage.setItem(CYCLE_KEY,JSON.stringify(CYCLE_STATE.sessions||[]));}
function loadCycleState(){try{CYCLE_STATE.sessions=JSON.parse(localStorage.getItem(CYCLE_KEY)||"[]");}catch(_){CYCLE_STATE.sessions=[];}}
function createCycleSession(payload){const id=`cc-${Date.now()}`;const item={id,name:String(payload?.name||"").trim(),date:payload?.date||getTodayDateKey(),pic:String(payload?.pic||"").trim(),location:String(payload?.location||""),notes:String(payload?.notes||""),status:"draft",locked:false,items:[]};if(!item.name||!item.pic)return null;CYCLE_STATE.sessions.unshift(item);CYCLE_STATE.activeId=id;saveCycleState();return item;}
function addSkuToSession(sessionId,rows=[]){const s=CYCLE_STATE.sessions.find(x=>x.id===sessionId);if(!s||s.locked)return;rows.forEach(r=>{if(!clean(r?.sku)||s.items.some(x=>clean(x.sku)===clean(r.sku)))return;s.items.push({...r,actualQty:Number(r.actualQty||0),notes:r.notes||"",diff:calculateDifference(Number(r.actualQty||0),Number(r.systemQty||0))});});saveCycleState();}
function updateActualQty(sessionId,sku,qty,notes=""){const s=CYCLE_STATE.sessions.find(x=>x.id===sessionId);if(!s||s.locked)return;const r=s.items.find(x=>clean(x.sku)===clean(sku));if(!r)return;r.actualQty=Number(qty||0);r.notes=String(notes||"");r.diff=calculateDifference(r.actualQty,Number(r.systemQty||0));saveCycleState();}
function calculateDifference(actual=0,system=0){return Number(actual||0)-Number(system||0);}function getCycleSummary(s){const t=(s?.items||[]).length,c=(s?.items||[]).filter(x=>x.diff===0).length;return{total:t,cocok:c,tidakCocok:t-c,totalSelisih:(s?.items||[]).reduce((n,x)=>n+x.diff,0),akurasi:t?((c/t)*100):0};}
function finalizeSession(id){const s=CYCLE_STATE.sessions.find(x=>x.id===id);if(!s)return;s.status="selesai";s.locked=true;saveCycleState();}
function renderCycleCountPage(){if(!CYCLE_STATE.__loaded){loadCycleState();CYCLE_STATE.__loaded=true;}const rows=[...(DATA["RPL"]||[]),...(DATA["BULKY"]||[]),...(DATA["Kartu Stock"]||[])].map(r=>({sku:getVal(r,["sku"]),nama:getVal(r,["nama barang","nama","item","description"])||"-",lokasi:getVal(r,["lokasi","location"])||"-",systemQty:parseNumber(getVal(r,["stok global","stok kartu","kartu stok","stok akhir"]))})).filter(r=>clean(r.sku));const sess=CYCLE_STATE.sessions.find(x=>x.id===CYCLE_STATE.activeId)||CYCLE_STATE.sessions[0];if(sess)CYCLE_STATE.activeId=sess.id;const q=clean(CYCLE_STATE.skuQuery);const filtered=rows.filter(r=>!q||clean(`${r.sku} ${r.nama}`).includes(q));const max=Math.max(1,Math.ceil(filtered.length/CYCLE_STATE.pageSize));const pageRows=filtered.slice((CYCLE_STATE.page-1)*CYCLE_STATE.pageSize,CYCLE_STATE.page*CYCLE_STATE.pageSize);const sum=getCycleSummary(sess);cycleCountApp.innerHTML=`<div class='card'><div class='mv-filters'><input id='ccName' placeholder='Nama sesi'><input id='ccDate' type='date' value='${getTodayDateKey()}'><input id='ccPic' placeholder='PIC'><input id='ccLoc' placeholder='Lokasi'><input id='ccNotes' placeholder='Catatan'><button id='ccCreate' class='btn-primary'>Buat Sesi</button></div><div class='mv-toolbar'><select id='ccSession'>${CYCLE_STATE.sessions.map(s=>`<option value='${s.id}'>${esc(s.name)} • ${s.status}</option>`).join('')}</select><input id='ccSearch' placeholder='Scan/ketik SKU' value='${esc(CYCLE_STATE.skuQuery)}'><button class='btn-ghost' id='ccAddMinus'>Ambil dari stok minus</button><button class='btn-ghost' id='ccAddAnom'>Ambil dari anomaly</button></div><div class='summary-grid'><div class='summary-card'><div class='k'>Total SKU dihitung</div><div class='v'>${sum.total||0}</div></div><div class='summary-card'><div class='k'>SKU cocok</div><div class='v'>${sum.cocok||0}</div></div><div class='summary-card'><div class='k'>SKU tidak cocok</div><div class='v'>${sum.tidakCocok||0}</div></div><div class='summary-card'><div class='k'>Akurasi</div><div class='v'>${(sum.akurasi||0).toFixed(2)}%</div></div></div><div class='table-wrap table-wrap-full'><table><thead><tr><th>SKU</th><th>Nama</th><th>Lokasi</th><th>Stok Sistem</th><th>Qty Aktual</th><th>Selisih</th><th>Catatan</th><th>Aksi</th></tr></thead><tbody>${pageRows.map(r=>{const it=sess?.items?.find(x=>clean(x.sku)===clean(r.sku));const aq=Number(it?.actualQty||0);const df=calculateDifference(aq,r.systemQty);const color=df===0?'#16a34a':df<0?'#dc2626':'#f97316';return`<tr class='${df!==0?'row-mismatch':''}'><td>${esc(r.sku)}</td><td>${esc(r.nama)}</td><td>${esc(r.lokasi)}</td><td>${r.systemQty}</td><td><input data-cc-qty='${encAttr(r.sku)}' type='number' value='${aq}' ${sess?.locked?'disabled':''}></td><td style='color:${color}'>${df}</td><td><input data-cc-note='${encAttr(r.sku)}' value='${esc(it?.notes||'')}' ${sess?.locked?'disabled':''}></td><td><button class='btn-ghost' data-cc-add='${encAttr(r.sku)}'>Tambah SKU</button></td></tr>`}).join('')}</tbody></table></div><div class='mv-pagination'><button class='btn-ghost' id='ccPrev'>Prev</button><button class='btn-ghost' id='ccNext'>Next</button><button class='btn-primary' id='ccDone' ${sess?.locked?'disabled':''}>Selesaikan Cycle Count</button></div></div>`;
document.getElementById('ccSession')&&(document.getElementById('ccSession').value=sess?.id||'');document.getElementById('ccCreate')?.addEventListener('click',()=>{if(createCycleSession({name:document.getElementById('ccName')?.value,date:document.getElementById('ccDate')?.value,pic:document.getElementById('ccPic')?.value,location:document.getElementById('ccLoc')?.value,notes:document.getElementById('ccNotes')?.value}))renderCycleCountPage();});document.getElementById('ccSession')?.addEventListener('change',e=>{CYCLE_STATE.activeId=e.target.value;renderCycleCountPage();});document.getElementById('ccSearch')?.addEventListener('input',debounce(e=>{CYCLE_STATE.skuQuery=e.target.value;renderCycleCountPage();},120));document.querySelectorAll('[data-cc-add]').forEach(b=>b.addEventListener('click',()=>{if(!sess)return;const sku=decodeURIComponent(b.dataset.ccAdd||'');const hit=rows.find(x=>clean(x.sku)===clean(sku));if(hit)addSkuToSession(sess.id,[hit]);renderCycleCountPage();}));document.querySelectorAll('[data-cc-qty]').forEach(i=>i.addEventListener('input',e=>{if(!sess)return;const sku=decodeURIComponent(e.target.dataset.ccQty||'');const note=document.querySelector(`[data-cc-note="${e.target.dataset.ccQty}"]`)?.value||'';updateActualQty(sess.id,sku,e.target.value,note);renderCycleCountPage();}));document.getElementById('ccPrev')?.addEventListener('click',()=>{CYCLE_STATE.page=Math.max(1,CYCLE_STATE.page-1);renderCycleCountPage();});document.getElementById('ccNext')?.addEventListener('click',()=>{CYCLE_STATE.page=Math.min(max,CYCLE_STATE.page+1);renderCycleCountPage();});document.getElementById('ccDone')?.addEventListener('click',()=>{if(sess){finalizeSession(sess.id);renderCycleCountPage();}});
}
