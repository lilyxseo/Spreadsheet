...
btnScanBalikan?.addEventListener("click",()=>openBarcodeScanner("balikanSearchInput",handleBalikanScanResult));
balikanSheetSelect?.addEventListener("change",async(e)=>{BALIKAN_STATE.sheetName=e.target.value||"";BALIKAN_STATE.highlightRowNumber=null;await loadBalikanRows();});
balikanSearchInput?.addEventListener("input",e=>{BALIKAN_STATE.searchKeyword=e.target.value||"";renderBalikanTable();});
...
function renderBalikanTable(){const rows=filterBalikanRows(BALIKAN_STATE.rows||[],BALIKAN_STATE.searchKeyword);if(!rows.length){balikanTable.innerHTML='<div class="subtitle">Data kosong.</div>';return;}balikanTable.innerHTML=`<div class="balikan-table-wrapper"><table class="balikan-table"><thead><tr><th class="col-check">Centang</th><th>No</th><th>SKU</th><th>Nama Barang</th><th>Qty</th><th>Rak tujuan</th><th>Lokasi</th><th>Stok Bulky</th><th>Stok Retail</th><th>Status</th><th>Keterangan</th></tr></thead><tbody>${rows.map(r=>`<tr id="balikan-row-${Number(r.rowNumber)}" class="${BALIKAN_STATE.highlightRowNumber===Number(r.rowNumber)?'balikan-row-highlight':''}"><td class="col-check"><input type="checkbox" ${isCheckedValue(r.checked)?'checked':''} onchange="toggleBalikanCheck('${encAttr(r.sheetName||BALIKAN_STATE.sheetName)}', ${Number(r.rowNumber)}, this.checked)"></td><td>${esc(r.no)}</td><td>${esc(r.sku)}</td><td>${esc(r.namaBarang)}</td><td>${esc(r.qty)}</td><td>${esc(r.rakTujuan)}</td><td>${esc(r.lokasi)}</td><td>${esc(r.stokBulky)}</td><td>${esc(r.stokRetail)}</td><td>${esc(r.status)}</td><td>${esc(r.keterangan)}</td></tr>`).join('')}</tbody></table></div>`;}

async function updateBalikanCheck(sheetName,rowNumber,checked,{refresh=true}={}){const row=(BALIKAN_STATE.rows||[]).find(r=>Number(r.rowNumber)===Number(rowNumber));const targetSheet=sheetName||BALIKAN_STATE.sheetName;const res=await fetch('/api/balikan-store/check',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({sheetName:targetSheet,rowNumber,checked})});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal update centang');if(row)row.checked=checked;toast('Centang berhasil diupdate','success');logActivitySafe({action:'CHECK_BALIKAN_STORE',module:'Balikan Store',detail:`Centang SKU ${row?.sku||'-'} di ${targetSheet}`,status:'SUCCESS',metadata:{sheetName:targetSheet,sku:row?.sku||'',checked,rowNumber}});if(refresh)await loadBalikanRows();}
window.toggleBalikanCheck=(sheetName,rowNumber,checked)=>{updateBalikanCheck(sheetName,Number(rowNumber),Boolean(checked)).catch(err=>{toast(err?.message||'Gagal update centang','error');renderBalikanTable();});};

async function handleBalikanScanResult(decodedText){if(navigator?.vibrate)navigator.vibrate(150);const sku=cleanScannedSku(decodedText);BALIKAN_STATE.searchKeyword=sku;if(balikanSearchInput)balikanSearchInput.value=sku;const row=(BALIKAN_STATE.rows||[]).find(item=>String(item.sku||"").trim()===sku);if(!row){BALIKAN_STATE.highlightRowNumber=null;renderBalikanTable();toast('SKU tidak ditemukan di Balikan Store','error');return;}await updateBalikanCheck(row.sheetName||BALIKAN_STATE.sheetName,Number(row.rowNumber),true,{refresh:false});BALIKAN_STATE.highlightRowNumber=Number(row.rowNumber);renderBalikanTable();scrollToBalikanRow(row.rowNumber);toast('SKU berhasil ditemukan dan dicentang: '+sku,'success');}
