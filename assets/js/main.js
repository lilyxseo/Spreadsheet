...
function openActivityDrawer(row){const m=activityMeta(row),changes=m.details?.changes||[];document.getElementById('activityDrawer')?.remove();document.body.insertAdjacentHTML('beforeend',`<div id="activityDrawer" class="activity-drawer-backdrop" role="presentation"><aside class="activity-drawer" role="dialog" aria-modal="true" aria-labelledby="activityDrawerTitle"><header><div><small>DETAIL AKTIVITAS</small><h3 id="activityDrawerTitle">${activityBadge(row.action)}</h3></div><button class="icon-btn activity-drawer-close" data-close-activity aria-label="Tutup detail aktivitas">×</button></header><p class="activity-description">${esc(row.detail||'-')}</p><dl><dt>Activity ID</dt><dd>${esc(row.reference||row.id||'-')}</dd><dt>Timestamp</dt><dd>${esc(new Date(row.created_at).toLocaleString('id-ID'))}</dd><dt>User / Role</dt><dd>${esc(row.user_name||'-')} · ${esc(row.role||'-')}</dd><dt>Developer</dt><dd>${m.isDeveloper?'Ya':'Tidak'}</dd><dt>Session</dt><dd>${esc(m.sessionId||'-')}</dd><dt>Module / Page</dt><dd>${esc(row.module||'-')} · ${esc(m.page||'-')}</dd><dt>Result</dt><dd>${activityBadge(row.status,'result')}</dd><dt>Entity</dt><dd>${esc(m.entityType||'-')} ${esc(m.entityId||'')}</dd><dt>Source</dt><dd>${esc(m.source||'-')}</dd></dl>${changes.length?`<section><h4>Perubahan</h4>${activityDetail(row)}</section>`:''}${m.details?.error?`<div class="state error">${esc(m.details.error)}</div>`:''}</aside></div>`);const drawer=document.getElementById('activityDrawer'),close=()=>drawer?.remove();drawer.onclick=e=>{if(e.target===drawer)close();};drawer.querySelector('[data-close-activity]').onclick=close;drawer.querySelector('[data-close-activity]').focus();}
