...
if(!hasCacheData){
setStatus("loading","Data belum dimuat. Tekan Refresh untuk sinkronisasi.");
}
...
const TABLE_STATE={in:{page:1,pageSize:25,rows:[],filtered:[],lastFilterKey:"",lastSort:"latest"},out:{page:1,pageSize:25,rows:[],filtered:[],lastFilterKey:"",lastSort:"latest"}};
...
function renderDataTablePage(mode,sheetName,keepPage=false,selectedCols){const isIn=mode==="in", resultEl=isIn?inResults:outResults, summaryEl=isIn?inSummary:outSummary;if(!resultEl)return;const st=TABLE_STATE[mode];if(!keepPage||!st.rows.length){st.rows=normalizeMovementRows(sheetName,isIn?"IN":"OUT");st.lastFilterKey="";st.lastSort="latest";}const rows=st.rows;if(!rows.length){resultEl.innerHTML='<div class="state">Belum ada data.</div>';summaryEl.textContent='0 data';return;}const allCols=["tanggal","from","to","sku","nama","qty","status","pic","keterangan","lokasi"];st.columns=selectedCols||st.columns||["tanggal","from","to","sku","nama","qty","status","pic","keterangan"];const filterRoot=document.getElementById(`mv-filters-${mode}`);const filterKey=JSON.stringify({q:clean((mode==="in"?inSearch:outSearch)?.value||""),from:filterRoot?.querySelector('[name="from"]')?.value||"",to:filterRoot?.querySelector('[name="to"]')?.value||"",status:filterRoot?.querySelector('[name="status"]')?.value||"",pic:filterRoot?.querySelector('[name="pic"]')?.value||"",lokasi:filterRoot?.querySelector('[name="lokasi"]')?.value||"",sku:filterRoot?.querySelector('[name="sku"]')?.value||""});if(filterKey!==st.lastFilterKey){st.filtered=applyTableFilters(rows,mode);st.lastFilterKey=filterKey;}const sort=document.getElementById(`mv-sort-${mode}`)?.value||"latest";if(sort!==st.lastSort){st.filtered=sortTableRows(st.filtered,sort);st.lastSort=sort;}const sizeValue=document.getElementById(`mv-size-${mode}`)?.value||"25";st.pageSize=sizeValue==="all"?100:Number(sizeValue||25);if(![25,50,100].includes(st.pageSize))st.pageSize=25;if(!keepPage)st.page=1;const size=st.pageSize;
...
const sizeWarning=sizeValue==="all"?"<div class='state'>Mode Semua dibatasi 100 row per halaman agar tetap ringan.</div>":"";
resultEl.innerHTML=`${filterHtml}${sizeWarning}<div class='table-wrap table-wrap-full'><table><thead><tr>${headers}</tr></thead><tbody>${body||`<tr><td colspan='${st.columns.length}'><div class='state'>Tidak ada data.</div></td></tr>`}</tbody></table></div><div class='mv-pagination'><span>Menampilkan ${start}–${end} dari ${st.filtered.length} data</span><div class='row'><button class='btn-ghost' data-mv-action='prev' data-mv-mode='${mode}'>Prev</button><button class='btn-ghost' data-mv-action='next' data-mv-mode='${mode}'>Next</button><span class='badge ${isIn?"b-in":"b-out"}'>${isIn?"IN":"OUT"}</span></div></div>`;}
