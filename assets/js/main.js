...
function renderBalikanKeteranganCardContent(rows=[]){
  ...
  const row=getBalikanLocationCardRow(list);
  if(!row){
    return `<div class='k'>Keterangan</div><div class='v balikan-keterangan-empty'>${list.length?'Pilih SKU di tabel':'Tidak ada keterangan'}</div><div class='subtitle'>${list.length?'Tap SKU untuk melihat keterangan row yang dipilih.':'Tidak ada baris hasil pencarian.'}</div>`;
  }
  const text=String(row?.keterangan??'').trim();
  return `<div class='k'>Keterangan</div><div class='v'>${esc(row.sku||'-')} <span class='balikan-sheet-badge'>${esc(getBalikanActiveSheetName(row)||'-')}</span></div><div class='balikan-keterangan-value ${text?'':'is-empty'}'>${text?esc(text):'Tidak ada keterangan'}</div>`;
}
