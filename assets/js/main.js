...
abcAnalisisApp.innerHTML=`<div class='card'><div class='mv-filters open' style='display:flex;flex-wrap:wrap;gap:8px;align-items:center'><select id='abcPeriod' style='max-width:170px;height:34px'>${periodOpt}</select><select id='abcOrderType' style='max-width:160px;height:34px'><option ${ABC_STATE.orderType==="Semua"?"selected":""}>Semua</option><option ${ABC_STATE.orderType==="Orderan GT"?"selected":""}>Orderan GT</option><option ${ABC_STATE.orderType==="Orderan Store"?"selected":""}>Orderan Store</option></select><select id='abcSortBy' style='max-width:190px;height:34px'>${sortOpt}</select><div id='abcToWrap' style='position:relative;min-width:180px;max-width:230px'><button id='abcToToggle' class='btn-ghost' type='button' style='height:34px;min-width:180px;max-width:230px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis'>${esc(selectedLabel)}</button><div id='abcToDropdown' ${ABC_STATE.toDropdownOpen?"":"hidden"} style='position:absolute;z-index:30;top:38px;left:0;width:265px;background:#071636;border:1px solid #2a3f6e;border-radius:12px;padding:8px;max-height:310px;overflow:auto;box-shadow:0 10px 25px rgba(0,0,0,.45)'><input id='abcToSearch' class='search-lg' placeholder='Cari...' value='${encAttr(ABC_STATE.toSearch||"")}' style='margin-bottom:8px;height:34px;background:#081a3f;border:1px solid #2f4d86;border-radius:10px;color:#dbe7ff'><div class='row' style='justify-content:space-between;gap:6px;margin-bottom:6px'><button type='button' id='abcToSelectAll' style='background:transparent;border:none;color:#ff4d5a;font-weight:700;cursor:pointer;padding:0'>Pilih semua</button><button type='button' id='abcToClear' style='background:transparent;border:none;color:#ff4d5a;font-weight:700;cursor:pointer;padding:0'>Hapus</button></div><div style='border-top:1px solid #1f3765;padding-top:6px'>${toFiltered.map(v=>`<label style='display:flex;gap:8px;align-items:center;padding:4px 0;font-size:12px;color:#ff4d5a;font-weight:700'><input type='checkbox' data-abc-to='${encAttr(v)}' ${(ABC_STATE.selectedTo||[]).includes(v)?"checked":""} style='width:16px;height:16px;accent-color:#ff4d5a'/> <span>${esc(v)}</span></label>`).join("")||"<div class='subtitle'>Tidak ada tujuan</div>"}</div></div></div><button id='abcResetBtn' class='btn-ghost' style='height:34px'>Reset</button><button id='abcCopyBtn' class='btn-ghost' style='height:34px'>Copy</button><button id='abcRefreshBtn' class='btn-primary' style='height:34px'>Refresh</button></div><div class='table-wrap'><table><thead><tr><th><input type='checkbox' id='abcSelectAll'></th><th>SKU</th><th>Nama Barang</th><th>Total Qty Keluar</th><th>% Kontribusi</th><th>Cumulative %</th><th>Kategori ABC</th><th>Stok Saat Ini</th><th>Priority Order</th><th>Status Rekomendasi</th></tr></thead><tbody>${data.rows.map(r=>`<tr><td><input type='checkbox' data-abc-row='${encAttr(r.sku)}' ${ABC_STATE.selectedRows.has(r.sku)?"checked":""}></td><td>${esc(r.sku)}</td><td><div style='display:flex;align-items:center;gap:6px;justify-content:space-between'><span>${esc(r.namaBarang)}</span><button class='btn-ghost' data-abc-sku='${encAttr(r.sku)}' style='padding:2px 6px;font-size:11px;line-height:1.1'>Detail</button></div></td><td>${r.qtyKeluar}</td><td>${(r.kontribusi*100).toFixed(2)}%</td><td>${(r.cum*100).toFixed(2)}%</td><td><span class='badge ${r.abc==="A"?"b-out":r.abc==="B"?"b-warn":"b-idle"}'>${r.abc}</span></td><td>${r.stokSaatIni}</td><td>${esc(r.priority)}</td><td>${esc(r.rekom)}</td></tr>`).join("")}</tbody></table></div></div><div class='card' style='margin-top:8px;padding:8px 10px'><div class='section-header' style='margin-bottom:4px'><h4 style='margin:0;font-size:13px'>Top 10 Fast Moving Items</h4></div><ol style='margin:0;padding-left:16px;display:block'>${data.top10.map(r=>`<li style='margin:0 0 2px 0'><button class='btn-link' data-abc-sku='${encAttr(r.sku)}' style='font-size:12px;line-height:1.2;padding:0'>${esc(r.namaBarang)}</button></li>`).join("")}</ol></div>`;
