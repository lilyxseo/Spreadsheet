...
function renderBalikanTableWhenIdle(){if(balikanTable?.querySelector('.inline-edit-input')){setTimeout(renderBalikanTableWhenIdle,500);return;}renderBalikanTable(true);}
...
function queueBalikanEdit(sheetName,row,field,value,oldValue){const rowNumber=Number(row?.rowNumber);if(!Number.isInteger(rowNumber)||rowNumber<=1)throw new Error('rowNumber tidak valid');const targetSheet=sheetName||window.currentTripSheet;if(!targetSheet)throw new Error('Sheet Balikan Store belum dipilih');if(!BALIKAN_STATE.pendingEdits[rowNumber])BALIKAN_STATE.pendingEdits[rowNumber]={};BALIKAN_STATE.pendingEdits[rowNumber][field]=value;const key=getBalikanEditKey(rowNumber,field);if(!BALIKAN_STATE.pendingEditMeta[key])BALIKAN_STATE.pendingEditMeta[key]={oldValue,rowSnapshot:{sku:row?.sku||''}};BALIKAN_STATE.pendingEditMeta[key]={...BALIKAN_STATE.pendingEditMeta[key],sheetName:targetSheet,rowNumber,field,value};setBalikanSaveStatus(rowNumber,field,'queued','Menunggu disimpan');updateBalikanLocalRow(rowNumber,field,value);queueBalikanSave();}
async function flushBalikanPendingEdits(){if(BALIKAN_STATE.saveInProgress){BALIKAN_STATE.saveRequested=true;return;}const pending=BALIKAN_STATE.pendingEdits||{};const edits=Object.entries(pending).map(([rowNumber,updates])=>({rowNumber:Number(rowNumber),updates:{...(updates||{})}})).filter(edit=>Number.isInteger(edit.rowNumber)&&Object.keys(edit.updates).length);if(!edits.length)return;BALIKAN_STATE.saveInProgress=true;BALIKAN_STATE.saveRequested=false;edits.forEach(edit=>Object.keys(edit.updates).forEach(field=>setBalikanSaveStatus(edit.rowNumber,field,'saving','Menyimpan')));try{const res=await fetch('/api/balikan-store/bulk-update',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sheetName:window.currentTripSheet,edits})});const data=await res.json().catch(()=>({}));if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal update data');const failed=new Map((data.failed||[]).map(item=>[`${Number(item.rowNumber)}:${String(item.field||'')}`,item]));let successCount=0,failedCount=0;edits.forEach(edit=>{Object.entries(edit.updates).forEach(([field,value])=>{const key=getBalikanEditKey(edit.rowNumber,field);const fail=failed.get(key);if(fail){failedCount++;setBalikanSaveStatus(edit.rowNumber,field,'error',fail.message||'Gagal update data');return;}const latestValue=BALIKAN_STATE.pendingEdits?.[edit.rowNumber]?.[field];if(latestValue===value){delete BALIKAN_STATE.pendingEdits[edit.rowNumber][field];delete BALIKAN_STATE.pendingEditMeta[key];if(!Object.keys(BALIKAN_STATE.pendingEdits[edit.rowNumber]).length)delete BALIKAN_STATE.pendingEdits[edit.rowNumber];setBalikanSaveStatus(edit.rowNumber,field,'saved','Tersimpan');setTimeout(()=>{const current=getBalikanSaveStatus(edit.rowNumber,field);if(current?.status==='saved')setBalikanSaveStatus(edit.rowNumber,field,'','');},1200);}updateBalikanLocalRow(edit.rowNumber,field,value);successCount++;});});if(successCount)showToast(`${successCount} perubahan Balikan Store tersimpan`,'success');if(failedCount)toast(`${failedCount} perubahan gagal disimpan. Cek baris bertanda gagal.`,'error');renderBalikanTableWhenIdle();}catch(err){edits.forEach(edit=>Object.keys(edit.updates).forEach(field=>setBalikanSaveStatus(edit.rowNumber,field,'error',err?.message||'Gagal update data')));toast(err?.message||'Gagal update data','error');renderBalikanTableWhenIdle();}finally{BALIKAN_STATE.saveInProgress=false;if(BALIKAN_STATE.saveRequested)queueBalikanSave(500);}}
async function updateBalikanCell(sheetName,row,field,value,oldValue){queueBalikanEdit(sheetName,row,field,value,oldValue);logActivitySafe({action:'EDIT_BALIKAN_STORE_QUEUE',module:'Balikan Store',detail:`[QUEUE EDIT] ${formatBalikanFieldName(field)}
SKU: ${row?.sku||'-'}
${oldValue||'-'} → ${value||'-'}`,status:'PENDING',metadata:{sheetName,rowNumber:row?.rowNumber,sku:row?.sku||'',field,oldValue,newValue:value}});}
