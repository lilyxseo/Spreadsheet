...
async function triggerManualRefresh(){logActivitySafe({action:'MANUAL_REFRESH',module:'System',detail:'Manual refresh dimulai',status:'SUCCESS'});if(isSyncing)return;await loadAllData(true);}
...
function createEditableCell(row,field,value,options){const td=document.createElement("td");td.className="editable-cell";td.textContent=value||"-";td.addEventListener("click",()=>{const latestValue=typeof options?.getLatestValue==="function"?options.getLatestValue({row,field,td}):row?.[field];startInlineEdit(td,row,field,latestValue,options);});return td;}
function startInlineEdit(cellEl,row,field,oldValue,options={}){const td=cellEl.closest('td')||cellEl;if(td.querySelector("input,select"))return;const saveDelayMs=Number(options.saveDelayMs||2000);const blurDelayMs=Number(options.blurDelayMs||1000);const input=document.createElement("input");const saveBtn=document.createElement("button");saveBtn.type="button";saveBtn.className="inline-edit-save-btn";saveBtn.textContent="Save";const prevValue=oldValue??"";input.value=String(prevValue);input.className="inline-edit-input";td.classList.add("editing");td.innerHTML="";td.appendChild(input);td.appendChild(saveBtn);input.focus();input.select();let cancelled=false,saving=false;INLINE_EDIT_STATE.isEditing=true;INLINE_EDIT_STATE.editingRow=Number(row?.rowNumber)||null;INLINE_EDIT_STATE.editingField=String(field||"");const clearPending=()=>{if(INLINE_EDIT_STATE.pendingSave){clearTimeout(INLINE_EDIT_STATE.pendingSave);INLINE_EDIT_STATE.pendingSave=null;}};const finish=(value)=>{td.classList.remove("editing");td.textContent=value||"-";if(INLINE_EDIT_STATE.editingRow===(Number(row?.rowNumber)||null)&&INLINE_EDIT_STATE.editingField===String(field||"")){INLINE_EDIT_STATE.isEditing=false;INLINE_EDIT_STATE.editingRow=null;INLINE_EDIT_STATE.editingField="";clearPending();}};async function save(reason="manual"){if(cancelled||saving)return;const newValue=input.value.trim();if(newValue===String(prevValue)){finish(prevValue);return;}saving=true;try{await options.onSave?.({row,field,value:newValue,oldValue:prevValue,td,reason});if(row&&field)row[field]=newValue;finish(newValue);}catch(err){if(row&&field)row[field]=prevValue;finish(prevValue);toast(err?.message||"Gagal update data","error");}finally{saving=false;}}const queueAutoSave=(reason="typing")=>{clearPending();INLINE_EDIT_STATE.lastInputAt=Date.now();INLINE_EDIT_STATE.pendingSave=setTimeout(()=>{if(cancelled||saving)return;const idleFor=Date.now()-INLINE_EDIT_STATE.lastInputAt;if(idleFor<saveDelayMs-50)return;save(reason);},saveDelayMs);};input.addEventListener("input",()=>{queueAutoSave("typing-idle");});input.addEventListener("blur",()=>{clearPending();INLINE_EDIT_STATE.pendingSave=setTimeout(()=>{if(cancelled||saving)return;save("blur-delay");},blurDelayMs);});input.addEventListener("focus",()=>{clearPending();});saveBtn.addEventListener("click",()=>save("button"));input.addEventListener("keydown",(e)=>{if(e.key==="Enter"){e.preventDefault();clearPending();save("enter");}if(e.key==="Escape"){cancelled=true;finish(prevValue);}});}
...
async function updateBalikanCell(sheetName,row,field,value,oldValue){const rowNumber=row?.rowNumber;const res=await fetch('/api/balikan-store/cell',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({sheetName,rowNumber,field,value})});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal update data');const targetRow=(window.BALIKAN_ROWS||[]).find(item=>Number(item.rowNumber)===Number(rowNumber));if(targetRow)targetRow[field]=value;if(row&&field)row[field]=value;const st=ensureBalikanFilterState();if(Array.isArray(st.rows)){const cachedRow=st.rows.find(item=>Number(item.rowNumber)===Number(rowNumber));if(cachedRow)cachedRow[field]=value;}st.openFilterCol='';showToast('Data berhasil diupdate','success');logActivitySafe({action:'EDIT_BALIKAN_STORE',module:'Balikan Store',detail:`[EDIT] ${formatBalikanFieldName(field)}\nSKU: ${row?.sku||'-'}\n${oldValue||'-'} → ${value||'-'}`,status:'SUCCESS',metadata:{sheetName,rowNumber:row.rowNumber,sku:row?.sku||'',field,oldValue,newValue:value}});} 
...
const IMPORT_PDF_TRANSFER_STATE={header:{},items:[],rawText:"",warnings:[],debug:{},isSubmitting:false,parseFailed:false,fileName:""};
function parseNumberLike(value){const num=Number(String(value??'').replace(/[^0-9-]/g,''));return Number.isFinite(num)?num:0;}
function isSkuToken(token){return /^[A-Za-z0-9-]{4,}$/.test(String(token||''));}
async function extractPdfRows(file){if(!window.pdfjsLib)throw new Error('pdf.js tidak tersedia');const ab=await file.arrayBuffer();const pdf=await window.pdfjsLib.getDocument({data:ab}).promise;const rows=[];let totalItems=0;let raw='';for(let p=1;p<=pdf.numPages;p++){const pg=await pdf.getPage(p);const tc=await pg.getTextContent();totalItems+=tc.items.length;const buckets=new Map();for(const it of tc.items){const y=(it.transform&&it.transform[5])?Math.round(it.transform[5]):0;const x=(it.transform&&it.transform[4])?it.transform[4]:0;const str=String(it.str||'').trim();if(!str)continue;raw+=str+'\n';if(!buckets.has(y))buckets.set(y,[]);buckets.get(y).push({x,str});}for(const [y,list] of buckets){list.sort((a,b)=>a.x-b.x);rows.push({y,texts:list.map(v=>v.str),text:list.map(v=>v.str).join(' ')})}}rows.sort((a,b)=>b.y-a.y);return {rows,pageCount:pdf.numPages,totalItems,rawText:raw};}
function parseTransferRows(rows){const warnings=[];const skipped=[];const header={nomorTransfer:'',tanggal:'',dari:'',kepada:'',status:'',nomorReferensi:''};const all=rows.map(r=>r.text);const findVal=(label)=>{const line=all.find(x=>new RegExp(label,'i').test(x));if(!line)return '';const m=line.split(':');return m.length>1?m.slice(1).join(':').trim():line.replace(new RegExp(label,'i'),'').trim();};header.nomorTransfer=findVal('Nomor Transfer');header.tanggal=findVal('Tanggal');header.dari=findVal('Dari');header.kepada=findVal('Kepada');header.status=findVal('Status');header.nomorReferensi=findVal('Nomor Referensi');const start=rows.findIndex(r=>/SKU/i.test(r.text)&&/Nama Produk/i.test(r.text)&&/Jumlah/i.test(r.text)&&/Diterima/i.test(r.text));if(start<0)return {header,items:[],warnings,skipped,parsedRows:0};const items=[];for(let i=start+1;i<rows.length;i++){const t=rows[i].texts;const line=rows[i].text.trim();if(!line)continue;if(/^(Total|Catatan Transfer|Ringkasan)/i.test(line))break;const skuIdx=t.findIndex(isSkuToken);if(skuIdx<0){if(items.length){items[items.length-1].namaProduk += ' '+line;skipped.push({line,reason:'lanjutan nama produk'});}else skipped.push({line,reason:'tidak ada SKU'});continue;}const sku=t[skuIdx]||'';const nums=t.filter(x=>/^\d+$/.test(x));const [jumlah,diterima,batal,tolak]=[nums[0],nums[1],nums[2],nums[3]].map(v=>v==null?0:parseNumberLike(v));const namaProduk=t.slice(skuIdx+1,t.length-nums.length).join(' ').trim()||'';if(!sku){skipped.push({line,reason:'SKU kosong'});continue;}if(!jumlah)warnings.push(`Qty kosong/0 untuk SKU ${sku}`);items.push({sku,namaProduk,jumlah,diterima,batal,tolak,catatan:''});}
return {header,items,warnings,skipped,parsedRows:items.length};}
function renderImportPdfTransferPage(){const root=document.getElementById('importPdfTransferApp');if(!root)return;const st=IMPORT_PDF_TRANSFER_STATE;const totalQty=st.items.reduce((n,i)=>n+parseNumberLike(i.jumlah),0);root.innerHTML=`<div class='card'><div class='row' style='gap:8px;flex-wrap:wrap'><input id='pdfTransferFile' type='file' accept='application/pdf'><button id='parsePdfTransferBtn' class='btn-primary'>Upload PDF</button><button id='importPdfTransferBtn' class='btn-primary' ${!st.items.length||st.isSubmitting?'disabled':''}>${st.isSubmitting?'Import...':'Import'}</button><button id='resetPdfTransferBtn' class='btn-ghost'>Reset</button></div><div class='subtitle'>Total SKU: ${st.items.length} | Total Qty: ${totalQty}</div><pre class='subtitle'>Debug: pages=${st.debug.pageCount||0}, textItems=${st.debug.totalItems||0}, rows=${st.debug.totalRows||0}, sku=${st.debug.totalSku||0}, parsedRows=${st.debug.parsedRows||0}, skipped=${st.debug.skipped||0}</pre><div class='table-wrap'><table><thead><tr><th>SKU</th><th>Nama Produk</th><th>Jumlah</th><th>Diterima</th><th>Batal</th><th>Tolak</th><th>Catatan</th></tr></thead><tbody>${st.items.map((it,idx)=>`<tr>${['sku','namaProduk','jumlah','diterima','batal','tolak','catatan'].map(k=>`<td contenteditable='true' data-pdf-edit='${idx}:${k}'>${esc(String(it[k]??''))}</td>`).join('')}</tr>`).join('')||"<tr><td colspan='7'><div class='state'>Belum ada data</div></td></tr>"}</tbody></table></div>${st.warnings.length?`<div class='state'>Warning: ${st.warnings.map(esc).join('<br>')}</div>`:''}${st.parseFailed?`<details open><summary>Debug raw text</summary><pre>${esc(st.rawText||'-')}</pre></details>`:''}</div>`;
const f=document.getElementById('pdfTransferFile'); if(f&&st.fileName){} 
document.getElementById('parsePdfTransferBtn')?.addEventListener('click',async()=>{const file=document.getElementById('pdfTransferFile')?.files?.[0];if(!file){toast('Pilih file PDF dulu','error');return;}try{const ex=await extractPdfRows(file);const parsed=parseTransferRows(ex.rows);Object.assign(IMPORT_PDF_TRANSFER_STATE,{header:parsed.header,items:parsed.items,warnings:parsed.warnings,rawText:ex.rawText,parseFailed:parsed.items.length===0,fileName:file.name,debug:{pageCount:ex.pageCount,totalItems:ex.totalItems,totalRows:ex.rows.length,totalSku:parsed.items.length,parsedRows:parsed.parsedRows,skipped:parsed.skipped.length}});console.log('PDF Debug',IMPORT_PDF_TRANSFER_STATE.debug,{skipped:parsed.skipped,parsedRows:parsed.items});renderImportPdfTransferPage();}catch(err){toast(err?.message||'Gagal parse PDF','error');}});
document.getElementById('resetPdfTransferBtn')?.addEventListener('click',()=>{IMPORT_PDF_TRANSFER_STATE.header={};IMPORT_PDF_TRANSFER_STATE.items=[];IMPORT_PDF_TRANSFER_STATE.warnings=[];IMPORT_PDF_TRANSFER_STATE.rawText='';IMPORT_PDF_TRANSFER_STATE.parseFailed=false;IMPORT_PDF_TRANSFER_STATE.debug={};renderImportPdfTransferPage();});
document.getElementById('importPdfTransferBtn')?.addEventListener('click',async()=>{if(IMPORT_PDF_TRANSFER_STATE.isSubmitting)return;const valid=IMPORT_PDF_TRANSFER_STATE.items.filter(it=>String(it.sku||'').trim());if(!valid.length){toast('SKU kosong, tidak bisa import','error');return;}IMPORT_PDF_TRANSFER_STATE.isSubmitting=true;renderImportPdfTransferPage();try{const res=await fetch('/api/import-pdf-transfer',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({header:IMPORT_PDF_TRANSFER_STATE.header,items:valid})});const out=await res.json();if(!res.ok||!out?.success)throw new Error(out?.message||'Import gagal');toast('Import sukses','success');}catch(err){toast(err?.message||'Import gagal','error');}finally{IMPORT_PDF_TRANSFER_STATE.isSubmitting=false;renderImportPdfTransferPage();}});
root.querySelectorAll('[data-pdf-edit]').forEach(td=>td.addEventListener('blur',()=>{const [idx,key]=td.dataset.pdfEdit.split(':');if(!IMPORT_PDF_TRANSFER_STATE.items[Number(idx)])return;IMPORT_PDF_TRANSFER_STATE.items[Number(idx)][key]=td.textContent.trim();}));
}
