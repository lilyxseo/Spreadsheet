...
function renderMovementSession(){return `<div class='cc-section'><div class='mv-toolbar'><input id='movementScanInput' class='form-input search-lg' placeholder='Scan SKU...' autocomplete='off'><input id='mvSearch' class='search-lg' placeholder='Cari SKU / nama barang / lokasi' value='${esc(MOVEMENT_STATE.searchInput)}'></div></div><div class='cc-section'><div class='table-wrap cc-table-wrap cc-search-wrap'><table><thead><tr><th>Lokasi</th><th>SKU</th><th>Nama Barang</th><th>Qty</th><th>Aksi</th></tr></thead><tbody id='mvSearchResultsBody'></tbody></table></div></div><div class='cc-section'><h4>Movement Berjalan</h4><div class='table-wrap cc-table-wrap cc-session-wrap'><table><thead><tr><th>From</th><th>To</th><th>SKU</th><th>Nama Barang</th><th>Stok Lokasi Awal</th><th>Qty Aktual</th><th>Aksi</th></tr></thead><tbody id='mvSessionBody'></tbody></table></div></div>`;}
...
function findMovementSourceByScannedSku(rawSku){
  const sku=cleanScannedSku(rawSku);
  if(!sku)return null;
  const rows=getMovementSourceRows();
  const exact=rows.find(r=>clean(String(r.sku||""))===clean(sku));
  if(exact)return exact;
  const digits=String(sku).replace(/\D/g,"");
  if(/^\d{4}$/.test(digits)){
    return rows.find(r=>String(r.sku||"").replace(/\D/g,"").endsWith(digits))||null;
  }
  return null;
}
function focusMovementRow(index){
  const target=document.querySelector(`#mvSessionBody [data-mvm='to'][data-idx='${index}']`);
  if(target){target.focus();target.scrollIntoView({behavior:'smooth',block:'center'});}
}
function handleMovementScanSubmit(rawSku){
  const source=findMovementSourceByScannedSku(rawSku);
  if(!source){toast('SKU tidak ditemukan','error');return;}
  const existingIdx=MOVEMENT_STATE.sessionItems.findIndex(it=>clean(it.sku)===clean(source.sku)&&clean(it.lokasi)===clean(source.lokasi));
  if(existingIdx>=0){toast('SKU sudah ada di list movement','success');focusMovementRow(existingIdx);return;}
  addMovementItem(source);
  focusMovementRow(MOVEMENT_STATE.sessionItems.length-1);
}
