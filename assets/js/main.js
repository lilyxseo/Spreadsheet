...
const PDF_TRANSFER_STATE={header:{},items:[],warnings:[],debugRows:[],rawText:"",csvText:"",csvRows:[],detectedColumns:[],failedRows:[],logs:[],isParsing:false,isImporting:false,lastFileName:"",selectedFile:null};
...
if(!appInitialized){
bindNav();bindEvents();bindLogoutButtons();setupSidebar();syncDeveloperMenuVisibility();renderFilters();applyRoleBasedUi();setMainContentLoading(true);document.getElementById("sheetInfo").textContent=SHEETS.join(", ");document.getElementById("spreadsheetInfo").textContent=SPREADSHEET_ID;renderRecentHistory();renderState("results",`Ketik minimal ${SEARCH_STATE.minChars} huruf untuk mencari.`);routeFromPath(location.pathname);window.addEventListener("popstate",()=>routeFromPath(location.pathname));appInitialized=true;
}else{
syncDeveloperMenuVisibility();
}
await initAppData();
...
balikanSearchInput?.addEventListener("input",e=>{window.balikanSearchKeyword=e.target.value||"";BALIKAN_STATE.exactScanSku="";BALIKAN_STATE.selectedSkuRowNumber=null;BALIKAN_STATE.selectedSkuSheetName="";BALIKAN_STATE.selectedSkuValue="";scheduleBalikanRender(false,300);});
...
const inventoryRes=await Promise.allSettled(INVENTORY_PRELOAD_SHEETS.map(sheet=>fetchSheet(sheet).then(raw=>parseSheet(raw,sheet))));
...
function runSearch(){const qRaw=SEARCH_STATE.filterValue||"";const q=clean(qRaw);const prevQuery=lastQuery;lastQuery=qRaw;const minChars=SEARCH_STATE.minChars||2;if(!q){lastResults=[];SEARCH_STATE.page=1;renderRecentHistory();return renderState("results",`Ketik minimal ${minChars} huruf untuk mencari.`);}if(q.length<minChars){lastResults=[];SEARCH_STATE.page=1;return renderState("results",`Ketik minimal ${minChars} huruf untuk mencari.`);}saveRecentSearch(qRaw);if(SEARCH_STATE.abortController)SEARCH_STATE.abortController.abort();SEARCH_STATE.abortController=new AbortController();const {signal}=SEARCH_STATE.abortController;const words=q.split(" ").filter(Boolean);const isMultiWord=words.length>1;const out=[];for(const it of CACHE_SKU.values()){if(signal.aborted)return;if(currentFilter!=="Semua"&&!it.sources.has(currentFilter))continue;const skuN=it._skuClean||clean(String(it.sku||""));const nameN=it._nameClean||clean(it.nama);const combined=it._searchText||clean(`${it.sku||""} ${it.nama||""}`);if(isMultiWord&&!words.every(word=>combined.includes(word)))continue;let rank=99;if(skuN===q)rank=1;else if(/^\d{4}$/.test(q)&&(it._skuDigits||"").endsWith(q))rank=2;else if(skuN.includes(q))rank=3;else if(nameN===q)rank=4;else if(words.length&&words.every(w=>combined.includes(w)))rank=5;else if(!isMultiWord&&words.some(w=>combined.includes(w)))rank=6;if(rank<99)out.push({...it,sources:[...it.sources],rank});}
...
const LOCATION_STATE={rows:[],filtered:[],page:1,pageSize:25,selected:"",cache:new Map(),emptyLocations:[],recent:[]};
function getLocationValue(row){return getVal(row,["lokasi","location","rak","bin","area","LOKASI"])||"";}
function getLocationStatus(jumlahSku,totalQty){if(totalQty<=0||jumlahSku<=0)return "Kosong";if(jumlahSku>=25||totalQty>=400)return "Padat";if(jumlahSku>=15||totalQty>=250)return "Hampir Penuh";return "Normal";}
function locationStatusBadge(status){const map={"Normal":["🟢","b-in"],"Hampir Penuh":["🟡","b-kartu"],"Padat":["🔴","b-out"],"Kosong":["⚫",""]};const it=map[status]||["•",""];return `<span class='badge location-status ${it[1]}'>${it[0]} ${esc(status)}</span>`;}
function buildLocationRows(){const skuMap=new Map();for(const row of (DATA["Kartu Stock"]||[])){const sku=String(getVal(row,["sku"])||"").trim();if(!sku)continue;const nama=getVal(row,["nama barang","nama","item","description"])||"-";const loc=String(getLocationValue(row)||"").trim().toUpperCase();if(!loc)continue;const qty=parseNumber(getVal(row,["stok akhir","qty","closing stock","ending stock","saldo akhir"]));if(qty<=0)continue;const key=`${clean(loc)}__${clean(sku)}`;if(!skuMap.has(key))skuMap.set(key,{lokasi:loc,sku,nama,qty:0});skuMap.get(key).qty+=qty;}const grouped={};for(const it of skuMap.values()){if(!grouped[it.lokasi])grouped[it.lokasi]={lokasi:it.lokasi,skus:[],totalQty:0};grouped[it.lokasi].skus.push(it);grouped[it.lokasi].totalQty+=it.qty;}return Object.values(grouped).filter(g=>g.totalQty>0&&g.skus.length>0).map(g=>{const jumlahSku=g.skus.length,skuKosong=0,status=getLocationStatus(jumlahSku,g.totalQty);return {...g,jumlahSku,skuKosong,status,searchText:clean(`${g.lokasi} ${g.lokasi.replace('-', ' ')} ${g.skus.map(s=>`${s.sku} ${s.nama}`).join(' ')}`)};});}
...
function smartLocationMatch(row,query){if(!query)return true;const q=clean(query);const loc=clean(row.lokasi);const spaced=clean(String(row.lokasi).replace('-', ' '));return loc.includes(q)||spaced.includes(q)||row.searchText.includes(q);}
function readLocationRecent(){try{return JSON.parse(localStorage.getItem('locationExplorerRecent')||'[]').filter(Boolean).slice(0,5);}catch(_){return [];}}
function saveLocationRecent(loc){const next=[loc,...readLocationRecent().filter(x=>x!==loc)].slice(0,5);localStorage.setItem('locationExplorerRecent',JSON.stringify(next));LOCATION_STATE.recent=next;renderLocationRecent();}
function renderLocationRecent(){const el=document.getElementById('locationsRecent');if(!el)return;const list=LOCATION_STATE.recent||readLocationRecent();el.innerHTML=list.length?`<div class='location-recent-title'>Terakhir Dilihat:</div><div class='location-chip-row'>${list.map(loc=>`<button class='loc-chip recent' type='button' onclick="selectLocationDetail('${encAttr(loc)}')">${esc(loc)}</button>`).join('')}</div>`:'';}
function renderLocationsPage(){LOCATION_STATE.rows=buildLocationRows();LOCATION_STATE.recent=readLocationRecent();if(!locSearchInput?.dataset.bound){const redraw=debounce(()=>{LOCATION_STATE.page=1;drawLocations();},300);[locSearchInput,locSkuSearchInput].forEach(el=>el?.addEventListener('input',redraw));[locSort,locPageSize].forEach(el=>el?.addEventListener('change',()=>{LOCATION_STATE.page=1;drawLocations();}));document.getElementById('locationsFilters')?.addEventListener('click',e=>{const btn=e.target.closest('[data-location-status]');if(!btn)return;locStatusFilter.value=btn.dataset.locationStatus;document.querySelectorAll('#locationsFilters .loc-chip').forEach(x=>x.classList.toggle('active',x===btn));LOCATION_STATE.page=1;drawLocations();});locSearchInput.dataset.bound='1';}drawLocations();renderLocationRecent();}
function drawLocations(){const rows=LOCATION_STATE.rows||[];const qLoc=locSearchInput?.value||'';const qSku=clean(locSkuSearchInput?.value||'');const status=locStatusFilter?.value||'all';const sorter=locSort?.value||'skuDesc';const pageSize=Number(locPageSize?.value||25);LOCATION_STATE.pageSize=[25,50,100].includes(pageSize)?pageSize:25;const usedNow=new Set(rows.map(r=>String(r.lokasi).trim()));const emptyRows=getAllValidLocations().filter(loc=>!usedNow.has(loc)).map(loc=>({lokasi:loc,skus:[],totalQty:0,jumlahSku:0,skuKosong:0,status:'Kosong',searchText:clean(`${loc} ${loc.replace('-', ' ')}`)}));const sourceRows=status==='Kosong'?emptyRows:rows;const cacheKey=JSON.stringify([qLoc,qSku,status,sorter,LOCATION_STATE.rows.length]);let filtered=LOCATION_STATE.cache.get(cacheKey);if(!filtered){filtered=sourceRows.filter(r=>smartLocationMatch(r,qLoc)&&(!qSku||r.skus.some(s=>clean(`${s.sku} ${s.nama}`).includes(qSku)))&&(status==='all'||(status==='Terisi'?r.totalQty>0:status==='Kosong'?r.status==='Kosong':r.status===status)));filtered=[...filtered].sort(sorter==='qtyDesc'?(a,b)=>b.totalQty-a.totalQty||b.jumlahSku-a.jumlahSku:sorter==='qtyAsc'?(a,b)=>a.totalQty-b.totalQty||a.jumlahSku-b.jumlahSku:sorter==='az'?(a,b)=>a.lokasi.localeCompare(b.lokasi):(a,b)=>b.jumlahSku-a.jumlahSku||b.totalQty-a.totalQty);LOCATION_STATE.cache.set(cacheKey,filtered);if(LOCATION_STATE.cache.size>30)LOCATION_STATE.cache.clear();}LOCATION_STATE.filtered=filtered;const used=new Set(rows.map(r=>String(r.lokasi).trim()));LOCATION_STATE.emptyLocations=emptyRows.map(r=>r.lokasi);const totalSku=new Set(rows.flatMap(r=>r.skus.map(s=>clean(s.sku)))).size;const densest=[...rows].sort((a,b)=>b.totalQty-a.totalQty)[0];locationsSummary.innerHTML=[["📍","Total Lokasi",rows.length],["📦","Total SKU Berlokasi",totalSku],["🚫","Lokasi Kosong",LOCATION_STATE.emptyLocations.length,'openEmptyLocationsDrawer()'],["🔥","Lokasi Terpadat",densest?`${densest.lokasi} • ${densest.totalQty} pcs`:'-']].map(c=>`<button class='location-summary-card' ${c[3]?`onclick='${c[3]}'`:''} type='button'><span>${c[0]}</span><small>${c[1]}</small><strong>${esc(c[2])}</strong></button>`).join('');const max=Math.max(1,Math.ceil(filtered.length/LOCATION_STATE.pageSize));if(LOCATION_STATE.page>max)LOCATION_STATE.page=max;const pageRows=filtered.slice((LOCATION_STATE.page-1)*LOCATION_STATE.pageSize,LOCATION_STATE.page*LOCATION_STATE.pageSize);locationsTable.innerHTML=`<div class='card location-results-card'><div class='section-header'><div><h4>Hasil Lokasi</h4><small class='section-subtitle'>${filtered.length} lokasi cocok. List dirender per halaman agar tetap cepat.</small></div><button class='btn-ghost' onclick='exportLocationCsv()'>Export hasil</button></div><div class='location-card-list'>${pageRows.map(r=>`<button class='location-result-card ${LOCATION_STATE.selected===r.lokasi?'active':''}' type='button' onclick="selectLocationDetail('${encAttr(r.lokasi)}')"><div><strong>📍 ${esc(r.lokasi)}</strong><span>${r.jumlahSku} SKU</span></div><div><strong>${esc(r.totalQty)} pcs</strong>${locationStatusBadge(r.status)}</div></button>`).join('')||"<div class='state'>Tidak ada lokasi yang cocok.</div>"}</div><div class='mv-pagination'><span>Menampilkan ${filtered.length?((LOCATION_STATE.page-1)*LOCATION_STATE.pageSize+1):0}–${Math.min(LOCATION_STATE.page*LOCATION_STATE.pageSize,filtered.length)} dari ${filtered.length}</span><div class='row'><button class='btn-ghost' onclick='changeLocationPage(-1)'>Prev</button><button class='btn-ghost' onclick='changeLocationPage(1)'>Next</button></div></div></div>`;if(LOCATION_STATE.selected)selectLocationDetail(encAttr(LOCATION_STATE.selected),true);if(window.lucide)window.lucide.createIcons();}
function renderEmptyLocationSection(){}
function openEmptyLocationsDrawer(){const list=LOCATION_STATE.emptyLocations||[];showConfirmModal({title:`Lokasi Kosong (${list.length})`,message:`<div class='location-empty-list'>${list.map(x=>`<button class='loc-chip' type='button' onclick="selectLocationDetail('${encAttr(x)}')">${esc(x)}</button>`).join('')}</div>`,allowHtmlMessage:true,confirmText:'Tutup',cancelText:'',onConfirm:()=>{}});}
...
function selectLocationDetail(locEncoded,keep=false){const lokasi=decodeURIComponent(locEncoded||'');LOCATION_STATE.selected=lokasi;const row=(LOCATION_STATE.rows||[]).find(r=>r.lokasi===lokasi);if(!row){locationDetail.innerHTML=`<div class='card location-detail-card'><div class='section-header'><h4>📍 ${esc(lokasi)}</h4>${locationStatusBadge('Kosong')}</div><div class='state'>Lokasi kosong atau belum ada stok aktif.</div></div>`;return;}if(!keep)saveLocationRecent(lokasi);const skuRows=[...row.skus].sort((a,b)=>b.qty-a.qty);locationDetail.innerHTML=`<div class='card location-detail-card'><div class='section-header'><div><h4>📍 ${esc(lokasi)}</h4><small class='section-subtitle'>Total SKU: ${row.jumlahSku} • Total Qty: ${row.totalQty}</small></div>${locationStatusBadge(row.status)}</div><div class='location-actions'><button class='btn-ghost' onclick="copyText(decodeURIComponent('${encAttr(lokasi)}'),'Lokasi disalin',this)">Copy lokasi</button><button class='btn-ghost' onclick="copyLocationSkus('${encAttr(lokasi)}',this)">Copy semua SKU lokasi</button><button class='btn-ghost' onclick="exportSelectedLocation('${encAttr(lokasi)}')">Export lokasi terpilih</button></div><div class='table-wrap'><table class='location-sku-table'><thead><tr><th>SKU</th><th>Nama Barang</th><th>Qty</th><th>Aksi</th></tr></thead><tbody>${skuRows.map(s=>`<tr><td>${esc(s.sku)}</td><td>${esc(s.nama)}</td><td>${s.qty}</td><td class='action-cell'><button class='icon-btn' title='Copy SKU' onclick="copyText(decodeURIComponent('${encAttr(s.sku)}'),'SKU disalin',this)">📋</button><button class='icon-btn' title='Detail SKU' onclick="openLocationSkuDrawer('${encAttr(lokasi)}','${encAttr(s.sku)}')">🔍</button></td></tr>`).join('')}</tbody></table></div></div>`;if(!keep){locationDetail.scrollIntoView({behavior:'smooth',block:'nearest'});drawLocations();}}
function copyLocationSkus(locEncoded,btn){const lokasi=decodeURIComponent(locEncoded||'');const row=(LOCATION_STATE.rows||[]).find(r=>r.lokasi===lokasi);copyText((row?.skus||[]).map(s=>s.sku).join('\n'),'Semua SKU lokasi disalin',btn);}
function exportSelectedLocation(locEncoded){const lokasi=decodeURIComponent(locEncoded||'');const row=(LOCATION_STATE.rows||[]).find(r=>r.lokasi===lokasi);const lines=['SKU,Nama Barang,Qty'].concat((row?.skus||[]).map(s=>[s.sku,s.nama,s.qty].map(v=>`"${String(v??'').replaceAll('"','""')}"`).join(',')));const blob=new Blob([lines.join('\n')],{type:'text/csv;charset=utf-8;'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`location-${lokasi}.csv`;a.click();URL.revokeObjectURL(a.href);}
function openLocationSkuDrawer(locEncoded,skuEncoded){const lokasi=decodeURIComponent(locEncoded||''),sku=decodeURIComponent(skuEncoded||'');const row=(LOCATION_STATE.rows||[]).find(r=>r.lokasi===lokasi);const item=row?.skus?.find(s=>clean(s.sku)===clean(sku));const raw=(DATA['Kartu Stock']||[]).find(r=>clean(getVal(r,['sku']))===clean(sku)&&clean(getLocationValue(r))===clean(lokasi))||{};const movement=parseNumber(getVal(raw,['pengeluaran','qty keluar','keluar','out']));const balikan=getVal(raw,['balikan','return','retur'])||'-';const ket=getVal(raw,['keterangan','notes','note'])||'-';const drawer=document.getElementById('locationSkuDrawer');if(!drawer)return;drawer.innerHTML=`<div class='location-drawer-backdrop' onclick='closeLocationSkuDrawer()'></div><aside class='location-drawer-panel'><button class='icon-btn close-side' onclick='closeLocationSkuDrawer()'>×</button><h4>Detail SKU</h4>${[['SKU',sku],['Nama Barang',item?.nama||'-'],['Lokasi',lokasi],['Qty',item?.qty||0],['Movement',movement||'-'],['Balikan',balikan],['Keterangan',ket]].map(([k,v])=>`<div class='location-drawer-row'><small>${esc(k)}</small><strong>${esc(v)}</strong></div>`).join('')}</aside>`;drawer.classList.add('show');drawer.setAttribute('aria-hidden','false');}
function closeLocationSkuDrawer(){const d=document.getElementById('locationSkuDrawer');d?.classList.remove('show');d?.setAttribute('aria-hidden','true');}
function exportLocationCsv(){const cols=["lokasi","jumlah_sku","total_qty","status"];const lines=[cols.join(',')].concat((LOCATION_STATE.filtered||[]).map(r=>[r.lokasi,r.jumlahSku,r.totalQty,r.status].map(v=>`"${String(v??"").replaceAll('"','""')}"`).join(',')));const blob=new Blob([lines.join('\n')],{type:'text/csv;charset=utf-8;'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='locations-analysis.csv';a.click();URL.revokeObjectURL(a.href);}
window.renderLocationsPage=renderLocationsPage;window.changeLocationPage=changeLocationPage;window.selectLocationDetail=selectLocationDetail;window.openEmptyLocationsDrawer=openEmptyLocationsDrawer;window.copyLocationSkus=copyLocationSkus;window.exportSelectedLocation=exportSelectedLocation;window.openLocationSkuDrawer=openLocationSkuDrawer;window.closeLocationSkuDrawer=closeLocationSkuDrawer;
...
function setAnomalyCache(rows=[]){try{localStorage.setItem(ANOMALY_CACHE_KEY,JSON.stringify({rows,updatedAt:Date.now()}));}catch(_){}}
function getAnomalyCache(){try{const parsed=JSON.parse(localStorage.getItem(ANOMALY_CACHE_KEY)||"null");return Array.isArray(parsed?.rows)?parsed.rows:[];}catch(_){return [];}}
...
function updateAnomalyTypeOptions(){if(!anomalyType)return;const options=['<option value="all">Semua Jenis Masalah</option>'].concat(ANOMALY_STATE.types.map(t=>`<option value="${esc(t)}">${esc(t)}</option>`));anomalyType.innerHTML=options.join('');anomalyType.value=ANOMALY_STATE.type;}
function getAnomalyFilteredRows(){const q=clean(ANOMALY_STATE.q);return (ANOMALY_STATE.rows||[]).filter(r=>{if(ANOMALY_STATE.severity!=='all'&&r.severity!==ANOMALY_STATE.severity)return false;if(ANOMALY_STATE.type!=='all'&&r.issue!==ANOMALY_STATE.type)return false;if(q&&!clean(`${r.sku} ${r.nama} ${r.issue} ${r.source}`).includes(q))return false;return true;});}
...
const tbody=document.querySelector('#anomalyTable tbody');if(tbody){tbody.innerHTML=pageRows.map(r=>`<tr><td><span class='badge ${sevClass(r.severity)}'>${r.severity}</span></td><td>${esc(r.sku)}</td><td>${esc(r.nama)}</td><td>${esc(r.issue)}</td><td>${esc(r.source)}</td><td>${esc(r.recommendation)}</td><td><button class='btn-ghost' onclick="navigateTo('/sku/'+encodeURIComponent('${encAttr(r.sku)}'))">Lihat Detail SKU</button></td></tr>`).join('')||"<tr><td colspan='7'><div class='state'>Tidak ada data warning</div></td></tr>";}
...
function renderAnomalySummary(){const rows=ANOMALY_STATE.filtered.length?ANOMALY_STATE.filtered:ANOMALY_STATE.rows;const cnt={High:rows.filter(r=>r.severity==='High').length,Medium:rows.filter(r=>r.severity==='Medium').length,Low:rows.filter(r=>r.severity==='Low').length};anomalySummary.innerHTML=[["Total warning",rows.length],["High severity",cnt.High],["Medium severity",cnt.Medium],["Low severity",cnt.Low]].map(c=>`<div class='metric'><div class='k'>${c[0]}</div><div class='v'>${c[1]}</div></div>`).join('');}
...
if(anomalySearch&&anomalySearch.value!==ANOMALY_STATE.q)anomalySearch.value=ANOMALY_STATE.q;document.getElementById('anomalySize').value=String(ANOMALY_STATE.pageSize);applyAnomalyFilters(false);if(!(ANOMALY_STATE.rows||[]).length){const cacheRows=getAnomalyCache();if(cacheRows.length){ANOMALY_STATE.rows=cacheRows;ANOMALY_STATE.types=[...new Set(cacheRows.map(r=>r.issue))];updateAnomalyTypeOptions();applyAnomalyFilters(false);setAnomalyLoading(true,'Menampilkan cache, sinkronisasi data terbaru...');}else{setAnomalyLoading(true,'Memuat data warning...');}}
refreshAnomalyInBackground();}
function refreshAnomalyInBackground(){const token=++ANOMALY_STATE.lastRenderToken;const run=()=>{const all=buildAnomalyReport();if(token!==ANOMALY_STATE.lastRenderToken)return;ANOMALY_STATE.rows=all;ANOMALY_STATE.types=[...new Set(all.map(r=>r.issue))];if(!ANOMALY_STATE.types.includes(ANOMALY_STATE.type))ANOMALY_STATE.type='all';updateAnomalyTypeOptions();setAnomalyCache(all);applyAnomalyFilters(false);setAnomalyLoading(false);};if(typeof requestIdleCallback==='function'){requestIdleCallback(run,{timeout:700});return;}setTimeout(run,0);}
...
window.renderAnomalyPage=renderAnomalyPage;window.changeAnomalyPage=changeAnomalyPage;window.applyAnomalyFilters=applyAnomalyFilters;window.scheduleAnomalySearch=scheduleAnomalySearch;
...
const ARCHIVE_STATE={sheetList:[],selectedSpreadsheetKey:"",selectedSheet:"",cache:{},loadingList:false,loadingData:false,listError:"",dataError:"",searchInput:"",search:"",searchTimer:null,searchToken:0,minSearchLength:2,sortColumn:"",sortDirection:"asc",columnFilter:"all",page:1,pageSize:25,lastLoadedAt:{}};
...
async function selectArchiveSpreadsheet(spreadsheetKey){ARCHIVE_STATE.selectedSpreadsheetKey=spreadsheetKey||"";ARCHIVE_STATE.selectedSheet="";ARCHIVE_STATE.sheetList=[];ARCHIVE_STATE.page=1;ARCHIVE_STATE.dataError="";ARCHIVE_STATE.listError="";if(!spreadsheetKey){renderArchivePage();return;}await ensureArchiveList();}
async function selectArchiveSheet(sheetName){ARCHIVE_STATE.selectedSheet=sheetName||"";ARCHIVE_STATE.page=1;ARCHIVE_STATE.dataError="";const source=getArchiveActiveSource();if(!sheetName||!source){renderArchivePage();return;}const cacheKey=buildArchiveCacheKey(source.spreadsheetId,sheetName);if(ARCHIVE_STATE.cache[cacheKey]){renderArchivePage();return;}ARCHIVE_STATE.loadingData=true;renderArchivePage();try{ARCHIVE_STATE.cache[cacheKey]=await fetchArchiveSheetData(source.spreadsheetId,sheetName);ARCHIVE_STATE.lastLoadedAt[cacheKey]=new Date().toISOString();}catch(err){ARCHIVE_STATE.dataError=err.message||"Gagal memuat data sheet";}finally{ARCHIVE_STATE.loadingData=false;renderArchivePage();}}
async function refreshArchive(){const source=getArchiveActiveSource();const sheet=ARCHIVE_STATE.selectedSheet;if(!source)return;if(!sheet){ARCHIVE_STATE.sheetList=[];await ensureArchiveList();return;}const cacheKey=buildArchiveCacheKey(source.spreadsheetId,sheet);delete ARCHIVE_STATE.cache[cacheKey];ARCHIVE_STATE.loadingData=true;ARCHIVE_STATE.dataError="";renderArchivePage();try{ARCHIVE_STATE.cache[cacheKey]=await fetchArchiveSheetData(source.spreadsheetId,sheet);ARCHIVE_STATE.lastLoadedAt[cacheKey]=new Date().toISOString();}catch(err){ARCHIVE_STATE.dataError=err.message||"Gagal refresh data";}finally{ARCHIVE_STATE.loadingData=false;renderArchivePage();}}
...
async function ensureAssetStoreList(){if(ASSET_STORE_STATE.sheetList.length||ASSET_STORE_STATE.loadingList)return;ASSET_STORE_STATE.loadingList=true;ASSET_STORE_STATE.listError="";renderAssetStorePage();try{ASSET_STORE_STATE.sheetList=await fetchAssetStoreSheetList();}catch(err){ASSET_STORE_STATE.listError=err.message||"Gagal memuat daftar sheet";}finally{ASSET_STORE_STATE.loadingList=false;renderAssetStorePage();if(ASSET_STORE_STATE.selectedSheet)selectAssetStoreSheet(ASSET_STORE_STATE.selectedSheet);}}
async function selectAssetStoreSheet(sheetName){ASSET_STORE_STATE.selectedSheet=sheetName||"";ASSET_STORE_STATE.page=1;ASSET_STORE_STATE.dataError="";if(!sheetName){renderAssetStorePage();return;}if(ASSET_STORE_STATE.cache[sheetName]){renderAssetStorePage();return;}ASSET_STORE_STATE.loadingData=true;renderAssetStoreTableOnly();try{ASSET_STORE_STATE.cache[sheetName]=await fetchAssetStoreSheetData(sheetName);ASSET_STORE_STATE.lastLoadedAt[sheetName]=new Date().toISOString();}catch(err){ASSET_STORE_STATE.dataError=err.message||"Gagal memuat data sheet";}finally{ASSET_STORE_STATE.loadingData=false;renderAssetStoreTableOnly();}}
async function refreshAssetStore(){const sheet=ASSET_STORE_STATE.selectedSheet;if(!sheet){ASSET_STORE_STATE.sheetList=[];await ensureAssetStoreList();return;}delete ASSET_STORE_STATE.cache[sheet];ASSET_STORE_STATE.loadingData=true;ASSET_STORE_STATE.dataError="";renderAssetStoreTableOnly();try{ASSET_STORE_STATE.cache[sheet]=await fetchAssetStoreSheetData(sheet);ASSET_STORE_STATE.lastLoadedAt[sheet]=new Date().toISOString();}catch(err){ASSET_STORE_STATE.dataError=err.message||"Gagal refresh data";}finally{ASSET_STORE_STATE.loadingData=false;renderAssetStoreTableOnly();}}
function beginAssetStoreInlineEdit(cell){const rowNumber=Number(cell.dataset.rowNumber);const rawField=cell.dataset.field||"";const field=decodeURIComponent(rawField);const sheetName=ASSET_STORE_STATE.selectedSheet;if(!sheetName||!rowNumber||!field)return;const parsed=ASSET_STORE_STATE.cache[sheetName];const row=(parsed?.rows||[]).find(r=>Number(r.__rowNumber)===rowNumber);if(!row)return;startInlineEdit(cell,row,field,row[field],{onSave:async({value,oldValue,td})=>{const key=`${sheetName}:${rowNumber}:${field}`;ASSET_STORE_STATE.saveStatus[key]='saving';td.dataset.saveState='saving';const old=String(oldValue??'');row[field]=value;td.textContent=value||'-';try{const res=await fetch('/api/asset-store/cell',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({spreadsheetId:ASSET_STORE_SPREADSHEET_ID,sheetName,rowNumber,field,headers:parsed.columns,value})});const out=await res.json().catch(()=>({}));if(!res.ok||out?.success===false)throw new Error(out?.message||'Gagal update cell');ASSET_STORE_STATE.saveStatus[key]='saved';td.dataset.saveState='saved';td.innerHTML=`${esc(value||'')}<span class='asset-save-badge'>tersimpan</span>`;setTimeout(()=>{if(td.dataset.saveState==='saved'){td.dataset.saveState='';td.textContent=value||'-';}},1200);}catch(err){row[field]=old;td.textContent=old||'-';ASSET_STORE_STATE.saveStatus[key]='error';td.dataset.saveState='error';throw err;}}});}
function getAssetStoreRows(){const selected=ASSET_STORE_STATE.selectedSheet;const parsed=ASSET_STORE_STATE.cache[selected]||{rows:[],columns:[],headerError:false};let rows=[...parsed.rows];const columns=parsed.columns||[];const q=clean(ASSET_STORE_STATE.search);if(q){rows=rows.filter(r=>{if(ASSET_STORE_STATE.columnFilter!=='all'){return clean(r[ASSET_STORE_STATE.columnFilter]).includes(q);}return columns.some(c=>clean(r[c]).includes(q));});}if(ASSET_STORE_STATE.sortColumn){const col=ASSET_STORE_STATE.sortColumn;rows.sort((a,b)=>String(a[col]??"").localeCompare(String(b[col]??""),"id",{numeric:true,sensitivity:'base'}));if(ASSET_STORE_STATE.sortDirection==='desc')rows.reverse();}return {rows,columns,headerError:parsed.headerError,totalSource:parsed.rows.length};}
function renderAssetStoreTableOnly(){const host=document.getElementById("assetStoreDataSection");if(!host)return;const selected=ASSET_STORE_STATE.selectedSheet;const {rows,columns,headerError,totalSource}=selected?getAssetStoreRows():{rows:[],columns:[],headerError:false,totalSource:0};const totalPage=Math.max(1,Math.ceil(rows.length/ASSET_STORE_STATE.pageSize));if(ASSET_STORE_STATE.page>totalPage)ASSET_STORE_STATE.page=totalPage;const start=(ASSET_STORE_STATE.page-1)*ASSET_STORE_STATE.pageSize;const pageRows=rows.slice(start,start+ASSET_STORE_STATE.pageSize);const activeSheetName=(ASSET_STORE_STATE.sheetList.find(s=>s.key===selected)||{}).name||selected||"-";const lastLoaded=selected&&ASSET_STORE_STATE.lastLoadedAt[selected]?new Date(ASSET_STORE_STATE.lastLoadedAt[selected]).toLocaleString("id-ID"):"-";if(!selected){host.innerHTML="<div class='card archive-section'><div class='state'>Pilih sheet untuk melihat data</div></div>";return;}if(ASSET_STORE_STATE.loadingData){host.innerHTML="<div class='card archive-section'><div class='state'>Memuat data sheet...</div></div>";return;}if(ASSET_STORE_STATE.dataError){host.innerHTML=`<div class='card archive-section'><div class='state'>${esc(ASSET_STORE_STATE.dataError)}</div></div>`;return;}if(headerError){host.innerHTML="<div class='card archive-section'><div class='state'>Header tidak terdeteksi</div></div>";return;}host.innerHTML=`<div class='card archive-section'><div class='grid dashboard archive-summary'><div class='metric'><div class='k'>Total Baris Asset</div><div class='v'>${totalSource}</div></div><div class='metric'><div class='k'>Total Kolom</div><div class='v'>${columns.length}</div></div><div class='metric'><div class='k'>Sheet Aktif</div><div class='v'>${esc(activeSheetName)}</div></div><div class='metric'><div class='k'>Terakhir Dimuat</div><div class='v'>${esc(lastLoaded)}</div></div></div></div><div class='card archive-section archive-table-card'>${!rows.length?"<div class='state'>Tidak ada data asset</div>":`<div class='table-wrap table-wrap-full archive-table-wrap'><table><thead><tr>${columns.map(c=>`<th>${esc(c)}</th>`).join("")}</tr></thead><tbody>${pageRows.map(r=>`<tr>${columns.map(c=>{const cellKey=`${selected}:${r.__rowNumber}:${c}`;const state=ASSET_STORE_STATE.saveStatus[cellKey]||'';const badge=state==='saved'?"<span class='asset-save-badge'>tersimpan</span>":'';return `<td class='editable-cell' data-asset-edit='1' data-row-number='${r.__rowNumber}' data-field='${encAttr(c)}' data-save-state='${state}'>${esc(r[c]??"")}${badge}</td>`;}).join("")}</tr>`).join("")}</tbody></table></div><div class='mv-pagination'><span>Menampilkan ${rows.length?start+1:0}-${Math.min(start+ASSET_STORE_STATE.pageSize,rows.length)} dari ${rows.length}</span><div class='row'><button class='btn-ghost' data-asset-store-page='-1' ${ASSET_STORE_STATE.page<=1?'disabled':''}>Prev</button><button class='btn-ghost' data-asset-store-page='1' ${ASSET_STORE_STATE.page>=totalPage?'disabled':''}>Next</button></div></div>`}</div>`;}
function renderAssetStorePage(){if(!assetStoreApp)return;if(!ASSET_STORE_STATE.sheetList.length&&!ASSET_STORE_STATE.loadingList)ensureAssetStoreList();const selected=ASSET_STORE_STATE.selectedSheet;const {columns}=selected?getAssetStoreRows():{columns:[]};const sortOps=columns.map(c=>`<option value='${esc(c)}' ${ASSET_STORE_STATE.sortColumn===c?"selected":""}>${esc(c)}</option>`).join("");assetStoreApp.innerHTML=`<div class='archive-layout'><div class='card archive-section'><div class='section-header'><h3 class='page-title'>Asset Store</h3><div class='row'><select id='assetStoreSheetSelect'><option value=''>Pilih Sheet</option>${ASSET_STORE_STATE.sheetList.map(s=>`<option value='${esc(s.key)}' ${s.key===selected?"selected":""}>${esc(s.name)}</option>`).join("")}</select><button id='assetStoreRefreshBtn' class='btn-ghost'>Refresh Asset Store</button></div></div>${ASSET_STORE_STATE.loadingList?"<div class='state'>Memuat daftar sheet...</div>":ASSET_STORE_STATE.listError?`<div class='state'>${esc(ASSET_STORE_STATE.listError)}</div>`:""}</div><div class='card archive-section archive-filter-card'><div class='mv-filters open archive-filters'><input id='assetStoreSearch' class='search-lg' placeholder='Cari di semua kolom (min. 2 huruf)' value='${esc(ASSET_STORE_STATE.searchInput)}'/><select id='assetStoreSortColumn'><option value=''>Urutkan Kolom</option>${sortOps}</select><select id='assetStoreSortDir'><option value='asc' ${ASSET_STORE_STATE.sortDirection==='asc'?'selected':''}>ASC</option><option value='desc' ${ASSET_STORE_STATE.sortDirection==='desc'?'selected':''}>DESC</option></select><select id='assetStoreColumnFilter'><option value='all'>Semua Kolom</option>${columns.map(c=>`<option value='${esc(c)}' ${ASSET_STORE_STATE.columnFilter===c?'selected':''}>Filter: ${esc(c)}</option>`).join('')}</select></div></div><div id='assetStoreDataSection'></div></div>`;renderAssetStoreTableOnly();}
document.addEventListener("DOMContentLoaded",()=>{ensureCycleHistoryLoaded().finally(()=>renderCycleCountPage());});
...
function createEditableCell(row,field,value,options){const td=document.createElement("td");td.className="editable-cell";const rowNumber=Number(row?.rowNumber);const sheetName=getBalikanActiveSheetName(row);const state=getBalikanSaveStatus(rowNumber,field,sheetName);td.dataset.saveState=state?.status||"";td.title=state?.message||"";const display=value||"-";td.innerHTML=`${esc(display)}${state?.status==="saving"?"<span class='balikan-save-badge'>menyimpan</span>":state?.status==="queued"?"<span class='balikan-save-badge'>pending</span>":state?.status==="saved"?"<span class='balikan-save-badge saved'>tersimpan</span>":state?.status==="error"?"<span class='balikan-save-badge error'>gagal</span>":""}`;td.addEventListener("click",()=>{const latestValue=typeof options?.getLatestValue==="function"?options.getLatestValue({row,field,td}):row?.[field];startInlineEdit(td,row,field,latestValue,options);});return td;}
function startInlineEdit(cellEl,row,field,oldValue,options={}){const td=cellEl.closest('td')||cellEl;if(td.querySelector("input,select"))return;const saveDelayMs=Number(options.saveDelayMs||2000);const blurDelayMs=Number(options.blurDelayMs||1000);const showSaveButton=options.showSaveButton!==false;const input=document.createElement("input");const saveBtn=document.createElement("button");saveBtn.type="button";saveBtn.className="inline-edit-save-btn";saveBtn.textContent="Save";const prevValue=oldValue??"";input.value=String(prevValue);input.className="inline-edit-input";td.classList.add("editing");td.innerHTML="";td.appendChild(input);if(showSaveButton)td.appendChild(saveBtn);input.focus();input.select();let cancelled=false,saving=false,pendingTimer=null,lastInputAt=0;INLINE_EDIT_STATE.isEditing=true;INLINE_EDIT_STATE.editingRow=Number(row?.rowNumber)||null;INLINE_EDIT_STATE.editingField=String(field||"");const clearPending=()=>{const timer=pendingTimer;if(timer){clearTimeout(timer);pendingTimer=null;}if(INLINE_EDIT_STATE.pendingSave===timer)INLINE_EDIT_STATE.pendingSave=null;};const finish=(value)=>{td.classList.remove("editing");td.textContent=value||"-";if(INLINE_EDIT_STATE.editingRow===(Number(row?.rowNumber)||null)&&INLINE_EDIT_STATE.editingField===String(field||"")){INLINE_EDIT_STATE.isEditing=false;INLINE_EDIT_STATE.editingRow=null;INLINE_EDIT_STATE.editingField="";clearPending();}};async function save(reason="manual"){if(cancelled||saving)return;const newValue=input.value.trim();if(newValue===String(prevValue)){finish(prevValue);return;}saving=true;try{await options.onSave?.({row,field,value:newValue,oldValue:prevValue,td,reason});if(row&&field)row[field]=newValue;finish(newValue);}catch(err){if(row&&field)row[field]=prevValue;finish(prevValue);toast(err?.message||"Gagal update data","error");}finally{saving=false;}}const queueAutoSave=(reason="typing")=>{clearPending();lastInputAt=Date.now();INLINE_EDIT_STATE.lastInputAt=lastInputAt;pendingTimer=setTimeout(()=>{if(cancelled||saving)return;const idleFor=Date.now()-lastInputAt;if(idleFor<saveDelayMs-50)return;save(reason);},saveDelayMs);INLINE_EDIT_STATE.pendingSave=pendingTimer;};input.addEventListener("input",()=>{queueAutoSave("typing-idle");});input.addEventListener("blur",()=>{clearPending();pendingTimer=setTimeout(()=>{if(cancelled||saving)return;save("blur-delay");},blurDelayMs);INLINE_EDIT_STATE.pendingSave=pendingTimer;});input.addEventListener("focus",()=>{clearPending();});if(showSaveButton)saveBtn.addEventListener("click",()=>save("button"));input.addEventListener("keydown",(e)=>{if(e.key==="Enter"){e.preventDefault();clearPending();save("enter");}if(e.key==="Escape"){cancelled=true;finish(prevValue);}});}
...
const BALIKAN_FILTERABLE_COLUMNS=['sku','namaBarang','lokasi','status','rakTujuan'];
const BALIKAN_FILTER_LABELS={sku:'SKU',namaBarang:'NAMA BARANG',lokasi:'LOKASI',status:'STATUS',rakTujuan:'RAK TUJUAN'};
function ensureBalikanFilterState(){if(!BALIKAN_STATE.filterState)BALIKAN_STATE.filterState={page:1,pageSize:50,openFilterCol:'',columnFilters:{},rows:[],filtered:[]};const st=BALIKAN_STATE.filterState;if(![25,50,100].includes(Number(st.pageSize)))st.pageSize=50;if(!Number.isFinite(Number(st.page))||Number(st.page)<1)st.page=1;BALIKAN_FILTERABLE_COLUMNS.forEach(k=>{if(!Array.isArray(st.columnFilters[k]))st.columnFilters[k]=[];});return st;}
...
async function updateBalikanCheck(sheetName,rowNumber,checked){const targetSheet=String(sheetName||window.currentTripSheet||'').trim();const res=await fetch('/api/balikan-store/check',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({sheetName:targetSheet,rowNumber,checked})});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal update centang');const row=(window.BALIKAN_ROWS||[]).find(r=>Number(r.rowNumber)===Number(rowNumber)&&(!targetSheet||getBalikanActiveSheetName(r)===targetSheet));const oldChecked=String(row?.checked).toUpperCase()==='TRUE';if(row)row.checked=checked?'TRUE':'FALSE';updateBalikanLocalRow(rowNumber,'checked',checked?'TRUE':'FALSE',targetSheet);const activeCheckbox=document.querySelector(`#${CSS.escape(getBalikanDomRowId(rowNumber,targetSheet))} .balikan-check`);if(activeCheckbox)activeCheckbox.checked=checked===true;if(checked===true)applyBalikanLastCheckedHighlight(Number(rowNumber),row?.sku,targetSheet);toast('Centang berhasil diupdate','success');logActivitySafe({action:'UPDATE_BALIKAN_STORE_CHECK',module:'Balikan Store',detail:`[CHECK]
Sheet: ${targetSheet||'-'}
SKU: ${row?.sku||'-'}
...
function getBarangRejectCache(){return safeJsonParse(localStorage.getItem(BARANG_REJECT_CACHE_KEY),null,false)||null;}
function setBarangRejectCache(payload){try{localStorage.setItem(BARANG_REJECT_CACHE_KEY,JSON.stringify({...payload,lastSync:Date.now()}));}catch(err){console.warn('Gagal simpan cache Barang Reject',err);}}
...
async function fetchBarangRejectData({background=false}={}){if(BARANG_REJECT_STATE.refreshing)return;BARANG_REJECT_STATE.refreshing=true;if(!background)BARANG_REJECT_STATE.loading=true;renderBarangRejectPage();try{const res=await fetch('/api/barang-reject',{cache:'no-store'});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal memuat Barang Reject');applyBarangRejectData(data);logActivitySafe({action:'REFRESH_BARANG_REJECT',module:'Barang Reject',detail:'Refresh data Barang Reject',status:'SUCCESS'});}catch(err){BARANG_REJECT_STATE.error=err?.message||'Gagal memuat Barang Reject';if(!background)toast(BARANG_REJECT_STATE.error,'error');logActivitySafe({action:'REFRESH_BARANG_REJECT',module:'Barang Reject',detail:BARANG_REJECT_STATE.error,status:'FAILED'});}finally{BARANG_REJECT_STATE.loading=false;BARANG_REJECT_STATE.refreshing=false;renderBarangRejectPage();}}
function ensureBarangRejectLoaded(){const cache=getBarangRejectCache();if(cache&&(!BARANG_REJECT_STATE.stock.length&&!BARANG_REJECT_STATE.masuk.length&&!BARANG_REJECT_STATE.keluar.length)){applyBarangRejectData(cache,{fromCache:true});queueMicrotask(()=>fetchBarangRejectData({background:true}));return;}if(!cache&&!BARANG_REJECT_STATE.loading&&!BARANG_REJECT_STATE.refreshing)fetchBarangRejectData();}
function rejectTokensMatch(row,query){const q=clean(query);if(!q)return true;const hay=clean([row.sku,row.namaBarang,row.lokasi,row.from,row.to,row.pic,row.status,row.keterangan,row.noIseller,row.netsuite,row.lokasiSuratJalan].join(' '));return q.split(/\s+/).filter(Boolean).every(tok=>hay.includes(tok));}
...
function renderRejectCards(){const stock=BARANG_REJECT_STATE.stock.filter(r=>(Number(r.qty)||0)>0);const skuSet=new Set(stock.map(r=>String(r.sku||'').trim()).filter(Boolean));const locSet=new Set(stock.map(r=>String(r.lokasi||'').trim()).filter(Boolean));const masukMonth=sumQty(thisMonthRows(BARANG_REJECT_STATE.masuk));const keluarMonth=sumQty(thisMonthRows(BARANG_REJECT_STATE.keluar));const latestIn=orderRejectSheetRows(BARANG_REJECT_STATE.masuk)[0];const latestOut=orderRejectSheetRows(BARANG_REJECT_STATE.keluar)[0];const zeroStock=BARANG_REJECT_STATE.stock.filter(r=>(Number(r.qty)||0)<=0).length;const cards=[['Total SKU Reject',skuSet.size,'package-x'],['Total Qty Reject',sumQty(stock),'boxes'],['Total Lokasi Reject',locSet.size,'map-pin'],['Barang Masuk Bulan Ini',masukMonth,'package-plus'],['Barang Keluar Bulan Ini',keluarMonth,'package-minus'],['Net Movement Bulan Ini',masukMonth-keluarMonth,'activity'],['SKU Terbanyak',topBy(stock,'sku'),'badge'],['Lokasi Terbanyak',topBy(stock,'lokasi'),'warehouse'],['Stok Kosong / 0',zeroStock,'circle-alert'],['Transaksi Terbaru',latestIn?.tanggal||latestOut?.tanggal||'-','clock']];return `<div class="grid dashboard reject-dashboard-cards">${cards.map(([k,v,icon])=>`<div class="metric card"><div class="metric-head"><span class="k">${esc(k)}</span><i data-lucide="${icon}"></i></div><div class="v">${esc(v)}</div></div>`).join('')}</div>`;}
...
function renderRejectMiniList(title,items,{empty='Belum ada data'}={}){return `<article class="card reject-insight-card"><h4>${esc(title)}</h4><div class="reject-insight-list">${items.length?items.map(item=>`<div class="reject-insight-row"><span>${esc(item[0])}</span><strong>${esc(item[1])}</strong></div>`).join(''):`<div class="state">${esc(empty)}</div>`}</div></article>`;}
function renderRejectRecentActivity(){const latest=[...orderRejectSheetRows(BARANG_REJECT_STATE.masuk).slice(0,4).map(r=>['Masuk',r]),...orderRejectSheetRows(BARANG_REJECT_STATE.keluar).slice(0,4).map(r=>['Keluar',r])].sort((a,b)=>(Number(b[1]?.rowNumber)||0)-(Number(a[1]?.rowNumber)||0)).slice(0,6);return `<article class="card reject-insight-card reject-insight-card--wide"><h4>Aktivitas Terbaru</h4><div class="reject-activity-list">${latest.length?latest.map(([type,r])=>`<div class="reject-activity-row"><span class="badge ${type==='Masuk'?'b-in':'b-out'}">${type}</span><div><strong>${esc(r.sku||'-')}</strong><small>${esc(r.namaBarang||'-')}</small></div><div class="reject-activity-meta"><strong>${Number(r.qty)||0}</strong><small>${esc(r.tanggal||'-')}</small></div></div>`).join(''):`<div class="state">Belum ada aktivitas reject.</div>`}</div></article>`;}
function renderRejectDashboardDetails(){const stock=BARANG_REJECT_STATE.stock.filter(r=>(Number(r.qty)||0)>0);const low=stock.filter(r=>(Number(r.qty)||0)<=2).sort((a,b)=>(Number(a.qty)||0)-(Number(b.qty)||0)).slice(0,5).map(r=>[`${r.sku} • ${r.lokasi}`,Number(r.qty)||0]);return `<div class="reject-insight-grid">${renderRejectMiniList('Top Lokasi Reject',topRejectEntries(stock,'lokasi'))}${renderRejectMiniList('Top SKU Reject',topRejectEntries(stock,'sku'))}${renderRejectMiniList('Stok Rendah (≤ 2)',low,{empty:'Tidak ada stok rendah'})}${renderRejectRecentActivity()}</div>`;}
...
function renderRejectDashboard(){return `${renderRejectCards()}${renderRejectDashboardDetails()}<div class="reject-chart-grid">${renderRejectChart(BARANG_REJECT_STATE.masuk,'Barang Masuk per bulan','in')}${renderRejectChart(BARANG_REJECT_STATE.keluar,'Barang Keluar per bulan','out')}</div>`;}
function sortRejectRows(rows,kind){const {key,dir}=BARANG_REJECT_STATE.sort;const factor=dir==='asc'?1:-1;return [...rows].sort((a,b)=>{const av=a[key],bv=b[key];const na=Number(av),nb=Number(bv);if(Number.isFinite(na)&&Number.isFinite(nb))return (na-nb)*factor;return String(av??'').localeCompare(String(bv??''),'id')*factor;});}
...
function thReject(label,key){const s=BARANG_REJECT_STATE.sort;return `<th><button class="reject-sort" data-reject-sort="${key}">${esc(label)}${s.key===key?(s.dir==='asc'?' ↑':' ↓'):''}</button></th>`;}
function thRejectStatic(label){return `<th>${esc(label)}</th>`;}
function renderRejectStock(){const f=BARANG_REJECT_STATE.filters;let rows=BARANG_REJECT_STATE.stock.filter(r=>rejectTokensMatch(r,f.stockSearch)&&rejectTokensMatch(r,f.stockSku)&&clean(r.namaBarang).includes(clean(f.stockNama))&&clean(r.lokasi).includes(clean(f.stockLokasi)));rows=sortRejectRows(rows,'stock');const pageRows=paginateReject(rows,'stock');return `<div class="card reject-card"><div class="reject-filter-grid"><div class="reject-search-with-scan"><input class="search-lg" data-reject-filter="stockSearch" placeholder="Cari besar: football blue, strike football, SKU, lokasi" value="${esc(f.stockSearch)}"><button class="icon-btn reject-scan-btn" data-reject-scan="stockSearch" type="button" title="Scan Barcode"><i data-lucide="scan-line"></i></button></div><input class="search-lg" data-reject-filter="stockSku" placeholder="Search SKU" value="${esc(f.stockSku)}"><input class="search-lg" data-reject-filter="stockNama" placeholder="Search Nama Barang" value="${esc(f.stockNama)}"><input class="search-lg" data-reject-filter="stockLokasi" placeholder="Search Lokasi" value="${esc(f.stockLokasi)}"><button class="btn-ghost" data-reject-export="stock">Export CSV</button></div><div class="table-wrap reject-table-wrap"><table><thead><tr>${thReject('Lokasi','lokasi')}${thReject('SKU','sku')}${thReject('Nama Barang','namaBarang')}${thReject('Qty','qty')}${thReject('Status','status')}${thReject('Last Update','lastUpdate')}</tr></thead><tbody>${pageRows.map(r=>`<tr><td>${esc(r.lokasi)}</td><td>${esc(r.sku)}</td><td>${esc(r.namaBarang)}</td><td>${Number(r.qty)||0}</td><td><span class="badge ${Number(r.qty)>0?'b-in':'b-out'}">${esc(r.status||'-')}</span></td><td>${esc(r.lastUpdate||formatDateTime(BARANG_REJECT_STATE.lastSync)||'-')}</td></tr>`).join('')||`<tr><td colspan="6"><div class="state">Tidak ada stok reject.</div></td></tr>`}</tbody></table></div>${rejectPager('stock',rows.length)}</div>`;}
function renderRejectIn(){const f=BARANG_REJECT_STATE.filters;let rows=BARANG_REJECT_STATE.masuk.filter(r=>rejectTokensMatch(r,f.masukSearch)&&dateInRange(r.tanggal,f.masukTanggalFrom,f.masukTanggalTo)&&(!f.masukLokasi||clean(r.lokasi).includes(clean(f.masukLokasi))));rows=orderRejectSheetRows(rows);const pageRows=paginateReject(rows,'masuk');return `<div class="card reject-card"><div class="reject-filter-grid"><input class="search-lg" data-reject-filter="masukSearch" placeholder="Search SKU, nama, lokasi, user" value="${esc(f.masukSearch)}"><input type="date" data-reject-filter="masukTanggalFrom" value="${esc(f.masukTanggalFrom)}"><input type="date" data-reject-filter="masukTanggalTo" value="${esc(f.masukTanggalTo)}"><input class="search-lg" data-reject-filter="masukLokasi" placeholder="Filter lokasi" value="${esc(f.masukLokasi)}"><button class="btn-ghost" data-reject-export="masuk">Export CSV</button></div><div class="table-wrap reject-table-wrap"><table><thead><tr>${thRejectStatic('Tanggal')}${thRejectStatic('Lokasi')}${thRejectStatic('SKU')}${thRejectStatic('Nama Barang')}${thRejectStatic('Qty')}${thRejectStatic('User')}${thRejectStatic('Keterangan')}</tr></thead><tbody>${pageRows.map(r=>`<tr><td>${esc(r.tanggal)}</td><td>${esc(r.lokasi)}</td><td>${esc(r.sku)}</td><td>${esc(r.namaBarang)}</td><td>${Number(r.qty)||0}</td><td>${esc(r.user)}</td><td>${esc(r.keterangan)}</td></tr>`).join('')||`<tr><td colspan="7"><div class="state">Belum ada barang masuk reject.</div></td></tr>`}</tbody></table></div>${rejectPager('masuk',rows.length)}</div>`;}
function renderRejectOut(){const f=BARANG_REJECT_STATE.filters;let rows=BARANG_REJECT_STATE.keluar.filter(r=>rejectTokensMatch(r,f.keluarSearch)&&dateInRange(r.tanggal,f.keluarTanggalFrom,f.keluarTanggalTo)&&(!f.keluarStatus||clean(r.status).includes(clean(f.keluarStatus)))&&(!f.keluarPic||clean(r.pic).includes(clean(f.keluarPic))));rows=orderRejectSheetRows(rows);const pageRows=paginateReject(rows,'keluar');return `<div class="card reject-card"><div class="reject-filter-grid"><input class="search-lg" data-reject-filter="keluarSearch" placeholder="Search SKU, nama, from, to, PIC, status" value="${esc(f.keluarSearch)}"><input type="date" data-reject-filter="keluarTanggalFrom" value="${esc(f.keluarTanggalFrom)}"><input type="date" data-reject-filter="keluarTanggalTo" value="${esc(f.keluarTanggalTo)}"><input class="search-lg" data-reject-filter="keluarStatus" placeholder="Filter status" value="${esc(f.keluarStatus)}"><input class="search-lg" data-reject-filter="keluarPic" placeholder="Filter PIC" value="${esc(f.keluarPic)}"><button class="btn-ghost" data-reject-export="keluar">Export CSV</button></div><div class="table-wrap reject-table-wrap"><table><thead><tr>${thRejectStatic('Tanggal')}${thRejectStatic('From')}${thRejectStatic('To')}${thRejectStatic('SKU')}${thRejectStatic('Nama Barang')}${thRejectStatic('Qty')}${thRejectStatic('PIC')}${thRejectStatic('Status')}${thRejectStatic('No iSeller')}${thRejectStatic('Netsuite')}${thRejectStatic('Keterangan')}${thRejectStatic('Lokasi Surat Jalan')}</tr></thead><tbody>${pageRows.map(r=>`<tr><td>${esc(r.tanggal)}</td><td>${esc(r.from)}</td><td>${esc(r.to)}</td><td>${esc(r.sku)}</td><td>${esc(r.namaBarang)}</td><td>${Number(r.qty)||0}</td><td>${esc(r.pic)}</td><td>${esc(r.status)}</td><td>${esc(r.noIseller)}</td><td>${esc(r.netsuite)}</td><td>${esc(r.keterangan)}</td><td>${esc(r.lokasiSuratJalan)}</td></tr>`).join('')||`<tr><td colspan="12"><div class="state">Belum ada barang keluar reject.</div></td></tr>`}</tbody></table></div>${rejectPager('keluar',rows.length)}</div>`;}
function todayIso(){return new Date().toISOString().slice(0,10);}
function renderRejectInputIn(){return `<div class="card reject-form-card"><form id="rejectInForm" class="reject-form"><div class="reject-form-head"><div><h4>Input Barang Masuk Reject</h4><small>Tambahkan stok reject ke lokasi tujuan.</small></div></div><div class="reject-form-grid"><label class="reject-field"><span>Tanggal</span><input type="date" name="tanggal" value="${todayIso()}"></label><label class="reject-field"><span>Lokasi *</span><input name="lokasi" required placeholder="Contoh: AREA HOLD"></label><label class="reject-field"><span>SKU *</span><div class="reject-input-action"><input name="sku" required placeholder="Scan / ketik SKU"><button class="icon-btn reject-scan-btn" data-reject-scan-field="sku" type="button" title="Scan Barcode"><i data-lucide="scan-line"></i></button></div></label><label class="reject-field"><span>Nama Barang</span><input name="namaBarang" placeholder="Nama barang"></label><label class="reject-field"><span>Qty *</span><input type="number" name="qty" min="1" step="1" required placeholder="0"></label><label class="reject-field reject-field--full"><span>Keterangan</span><textarea name="keterangan" rows="3" placeholder="Opsional"></textarea></label></div><div class="reject-form-actions"><button class="btn-primary" type="submit" ${BARANG_REJECT_STATE.submittingIn?'disabled':''}>${BARANG_REJECT_STATE.submittingIn?'Menyimpan...':'Simpan Barang Masuk Reject'}</button></div></form></div>`;}
function renderRejectInputCombined(){return `<div class="reject-input-stack reject-input-stack--single">${renderRejectInputIn()}</div>`;}
function renderBarangRejectPage(){const root=document.getElementById('barangRejectApp');if(!root)return;document.querySelectorAll('.side-link[data-page]').forEach(b=>b.classList.toggle('active',b.dataset.page===getBarangRejectNavPage()));syncActiveSidebarParent('barang-reject');ensureBarangRejectLoaded();const body=BARANG_REJECT_STATE.loading&&!BARANG_REJECT_STATE.stock.length?`<div class="card"><div class="skeleton-line"></div><div class="skeleton-line"></div><div class="skeleton-line"></div></div>`:BARANG_REJECT_STATE.activeTab==='dashboard'?renderRejectDashboard():BARANG_REJECT_STATE.activeTab==='masuk'?renderRejectIn():BARANG_REJECT_STATE.activeTab==='keluar'?renderRejectOut():BARANG_REJECT_STATE.activeTab==='input'?renderRejectInputCombined():renderRejectDashboard();root.innerHTML=`<div class="reject-meta"><span class="subtitle">Sheet ID: 1BVGcIWnYqrG-DefzmnO_hZjNn3H3c4sriI7CBAWsxw8</span><span class="subtitle">Last sync: ${BARANG_REJECT_STATE.lastSync?formatDateTime(BARANG_REJECT_STATE.lastSync):'-'}</span>${BARANG_REJECT_STATE.error?`<span class="state error">${esc(BARANG_REJECT_STATE.error)}</span>`:''}</div>${body}`;bindBarangRejectEvents();if(window.lucide)lucide.createIcons();}
function bindBarangRejectEvents(){document.getElementById('rejectRefreshBtn')?.addEventListener('click',()=>fetchBarangRejectData());document.querySelectorAll('[data-reject-filter]').forEach(input=>input.oninput=()=>{BARANG_REJECT_STATE.filters[input.dataset.rejectFilter]=input.value;BARANG_REJECT_STATE.page.stock=BARANG_REJECT_STATE.page.masuk=BARANG_REJECT_STATE.page.keluar=1;renderBarangRejectPage();});document.querySelectorAll('[data-reject-sort]').forEach(btn=>btn.onclick=()=>{const key=btn.dataset.rejectSort;if(BARANG_REJECT_STATE.sort.key===key)BARANG_REJECT_STATE.sort.dir=BARANG_REJECT_STATE.sort.dir==='asc'?'desc':'asc';else BARANG_REJECT_STATE.sort={key,dir:'asc'};renderBarangRejectPage();});document.querySelectorAll('[data-reject-page]').forEach(btn=>btn.onclick=()=>{const k=btn.dataset.rejectPage;BARANG_REJECT_STATE.page[k]=Math.max(1,(BARANG_REJECT_STATE.page[k]||1)+(btn.dataset.dir==='next'?1:-1));renderBarangRejectPage();});document.querySelectorAll('[data-reject-export]').forEach(btn=>btn.onclick=()=>exportBarangRejectCsv(btn.dataset.rejectExport));document.querySelectorAll('[data-reject-scan]').forEach(btn=>btn.onclick=()=>{logActivitySafe({action:'SCAN_BARCODE_REJECT',module:'Barang Reject',detail:'Buka scanner Barang Reject',status:'SUCCESS'});openBarcodeScanner('',text=>{const sku=resolveScannedSku(text)?.sku||String(text||'').trim();BARANG_REJECT_STATE.filters[btn.dataset.rejectScan]=sku;renderBarangRejectPage();});});document.querySelectorAll('[data-reject-scan-field]').forEach(btn=>btn.onclick=()=>{const form=btn.closest('form');const field=btn.dataset.rejectScanField;logActivitySafe({action:'SCAN_BARCODE_REJECT',module:'Barang Reject',detail:'Buka scanner input Barang Reject',status:'SUCCESS'});openBarcodeScanner('',text=>{const sku=resolveScannedSku(text)?.sku||String(text||'').trim();const target=form?.querySelector(`[name="${field}"]`);if(target){target.value=sku;target.dispatchEvent(new Event('input',{bubbles:true}));}});});bindRejectSubmitForms();}
function rejectFormPayload(form){return Object.fromEntries(new FormData(form).entries());}
async function submitRejectForm(kind,form){const isIn=kind==='masuk';const stateKey=isIn?'submittingIn':'submittingOut';if(BARANG_REJECT_STATE[stateKey])return;const payload=rejectFormPayload(form);payload.qty=Number(payload.qty);payload.action=kind;payload.user=currentUserIdentity().user_name||currentUserIdentity().user_id||'';if(!payload.sku||!(payload.qty>0)||!(isIn?payload.lokasi:payload.from)){toast('SKU, Qty > 0, dan Lokasi/From wajib diisi.','error');return;}if(!isIn){const stock=BARANG_REJECT_STATE.stock.find(r=>clean(r.sku)===clean(payload.sku)&&clean(r.lokasi)===clean(payload.from));if(!stock||Number(stock.qty)<payload.qty){toast(`Stok tidak boleh minus. Stok tersedia: ${stock?.qty||0}`,'error');return;}}BARANG_REJECT_STATE[stateKey]=true;renderBarangRejectPage();try{const res=await fetch('/api/barang-reject',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal submit Barang Reject');toast(data.message||'Data berhasil disimpan','success');logActivitySafe({action:isIn?'INPUT_BARANG_MASUK_REJECT':'INPUT_BARANG_KELUAR_REJECT',module:isIn?'Barang Masuk Reject':'Barang Keluar Reject',detail:`${isIn?'Masuk':'Keluar'} Reject SKU ${payload.sku} Qty ${payload.qty}`,reference:payload.sku,status:'SUCCESS',metadata:{oldValue:'',newValue:JSON.stringify(payload)}});form.reset();await fetchBarangRejectData({background:true});BARANG_REJECT_STATE.activeTab='masuk';history.replaceState({},'','/barang-reject/masuk');}catch(err){toast(err?.message||'Gagal submit Barang Reject','error');logActivitySafe({action:isIn?'INPUT_BARANG_MASUK_REJECT':'INPUT_BARANG_KELUAR_REJECT',module:isIn?'Barang Masuk Reject':'Barang Keluar Reject',detail:err?.message||'Gagal submit',reference:payload.sku,status:'FAILED'});}finally{BARANG_REJECT_STATE[stateKey]=false;renderBarangRejectPage();}}
function bindRejectSubmitForms(){document.getElementById('rejectInForm')?.addEventListener('submit',e=>{e.preventDefault();submitRejectForm('masuk',e.currentTarget);});}
function exportBarangRejectCsv(kind){const maps={stock:{rows:BARANG_REJECT_STATE.stock,headers:['Lokasi','SKU','Nama Barang','Qty','Status','Last Update'],cols:['lokasi','sku','namaBarang','qty','status','lastUpdate']},masuk:{rows:BARANG_REJECT_STATE.masuk,headers:['Tanggal','Lokasi','SKU','Nama Barang','Qty','User','Keterangan'],cols:['tanggal','lokasi','sku','namaBarang','qty','user','keterangan']},keluar:{rows:BARANG_REJECT_STATE.keluar,headers:['Tanggal','From','To','SKU','Nama Barang','Qty','PIC','Status','No iSeller','Netsuite','Keterangan','Lokasi Surat Jalan'],cols:['tanggal','from','to','sku','namaBarang','qty','pic','status','noIseller','netsuite','keterangan','lokasiSuratJalan']}};const cfg=maps[kind]||maps.stock;const lines=[cfg.headers.join(','),...cfg.rows.map(r=>cfg.cols.map(c=>`"${String(r[c]??'').replaceAll('"','""')}"`).join(','))];const blob=new Blob([lines.join('\n')],{type:'text/csv;charset=utf-8;'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`barang-reject-${kind}.csv`;a.click();URL.revokeObjectURL(a.href);toast('Export CSV Barang Reject berhasil','success');logActivitySafe({action:'EXPORT_CSV_BARANG_REJECT',module:'Barang Reject',detail:`Export ${kind} ${cfg.rows.length} row`,status:'SUCCESS'});}
...
document.getElementById('pdfTransferResetBtn')?.addEventListener('click',()=>{Object.assign(PDF_TRANSFER_STATE,{header:{},items:[],warnings:[],debugRows:[],rawText:'',csvText:'',csvRows:[],detectedColumns:[],failedRows:[],logs:[],isParsing:false,isImporting:false,lastFileName:'',selectedFile:null});renderImportPdfTransferPage();});
