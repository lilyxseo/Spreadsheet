...
const MOVEMENT_HISTORY_RANGE=`${MOVEMENT_HISTORY_SHEET_NAME}!A1:H`;
...
const url=`https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/${encodeURIComponent(MOVEMENT_HISTORY_RANGE)}?key=${API_KEY}&t=${Date.now()}`;
const res=await fetch(url,{cache:"no-store"});
const json=await res.json();
if(!res.ok||json.error)throw new Error((json.error&&json.error.message)||res.statusText||"Gagal memuat sheet Data Movement Barang");
const parsed=parseMovementHistoryRows(json.values||[]);
