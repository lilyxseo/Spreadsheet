...
const SCANNER_STATE={instance:null,active:false,isClosing:false,lastScan:""};
...
document.getElementById("btnScanSku")?.addEventListener("click",()=>{openScannerModal();});
...
if(!modal||SCANNER_STATE.active)return;
modal.hidden=false;
document.body.classList.add("scanner-modal-open");
if(window.lucide)window.lucide.createIcons();
try{
SCANNER_STATE.instance=SCANNER_STATE.instance||new window.Html5Qrcode(readerId);
SCANNER_STATE.active=true;
SCANNER_STATE.isClosing=false;
await SCANNER_STATE.instance.start({facingMode:{exact:"environment"}},{fps:10,qrbox:{width:260,height:160}},async(decodedText)=>{
if(SCANNER_STATE.isClosing||!decodedText||SCANNER_STATE.lastScan===decodedText)return;
SCANNER_STATE.lastScan=decodedText;
searchInput.value=decodedText;
SEARCH_STATE.inputValue=decodedText;
SEARCH_STATE.filterValue=decodedText;
runSearch();
...
if(SCANNER_STATE.instance&&SCANNER_STATE.active){try{await SCANNER_STATE.instance.stop();}catch(_){}
try{await SCANNER_STATE.instance.clear();}catch(_){} }
SCANNER_STATE.active=false;
SCANNER_STATE.isClosing=false;
modal.hidden=true;
document.body.classList.remove("scanner-modal-open");
