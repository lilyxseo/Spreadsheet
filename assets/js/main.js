...
const ACTION_HISTORY={undoStack:[],redoStack:[],running:false};
function syncActionHistoryButtons(){if(window.undoHeader)undoHeader.disabled=ACTION_HISTORY.running||!ACTION_HISTORY.undoStack.length;if(window.redoHeader)redoHeader.disabled=ACTION_HISTORY.running||!ACTION_HISTORY.redoStack.length;}
function pushActionHistory(action){if(!action||ACTION_HISTORY.running)return;ACTION_HISTORY.undoStack.push({...action,timestamp:action.timestamp||new Date().toISOString()});if(ACTION_HISTORY.undoStack.length>ACTION_HISTORY_LIMIT)ACTION_HISTORY.undoStack.shift();ACTION_HISTORY.redoStack=[];syncActionHistoryButtons();}
function movementMeta(mode){return {moduleName:mode==='in'?'barang-masuk':'barang-keluar',sheetName:mode==='in'?'Barang Masuk':'Barang Keluar',apiBase:mode==='in'?'/api/barang-masuk':'/api/barang-keluar'};}
function getMovementRowSnapshot(mode,rowNumber){const row=(TABLE_STATE?.[mode]?.rows||[]).find(r=>Number(r.rowNumber)===Number(rowNumber));return row?JSON.parse(JSON.stringify(row)):null;}
function updateMovementLocalCaches(mode,rowNumber,updates){const meta=movementMeta(mode);Object.entries(updates||{}).forEach(([field,value])=>{updateLocalRow(meta.moduleName,rowNumber,field,value);updateCacheRow(meta.moduleName,rowNumber,field,value);});const rows=Array.isArray(window.APP_STATE?.[meta.moduleName==='barang-masuk'?'barangMasuk':'barangKeluar'])?window.APP_STATE[meta.moduleName==='barang-masuk'?'barangMasuk':'barangKeluar']:[];DATA[meta.sheetName]=rows;saveCache(DATA);}
async function applyMovementUpdate(mode,rowNumbers,updates){const meta=movementMeta(mode);const res=await fetch(`${meta.apiBase}/bulk-update`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({rowNumbers,updates})});const out=await res.json();if(!res.ok||!out?.success)throw new Error(out?.message||'Gagal update');rowNumbers.forEach(row=>updateMovementLocalCaches(mode,row,updates));renderDataTablePage(mode,meta.sheetName,true);}
async function deleteMovementRows(mode,rowNumbers){const meta=movementMeta(mode);const res=await fetch(`${meta.apiBase}/bulk-delete`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({rowNumbers})});const out=await res.json();if(!res.ok||!out?.success)throw new Error(out?.message||'Gagal hapus');await refreshMovementTableData(mode,{deletedRowNumbers:rowNumbers});}
async function restoreMovementRows(mode,rows){const meta=movementMeta(mode);const res=await fetch('/api/action-history/movement-rows',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({mode,rows})});const out=await res.json();if(!res.ok||!out?.success)throw new Error(out?.message||'Gagal restore row');await refreshMovementTableData(mode);}
async function applyBalikanHistoryUpdate(action,isUndo){const value=isUndo?action.oldValue:action.newValue;const sheetName=action.sheetName;const rowNumber=Number(action.rowId||action.rowNumber);const field=action.field;const res=await fetch('/api/balikan-store/bulk-update',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sheetName,edits:[{rowNumber,updates:{[field]:value}}]})});const data=await res.json().catch(()=>({}));if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal update Balikan Store');updateBalikanLocalRow(rowNumber,field,value,sheetName);updateBalikanCellText(rowNumber,field,value,sheetName);renderBalikanTable(true);}
async function applyHistoryAction(action,direction){if(!action)return;const isUndo=direction==='undo';if(action.page==='movement'&&action.actionType==='update'){for(const change of (action.changes||[action]))await applyMovementUpdate(change.mode,[change.rowId||change.rowNumber],{[change.field]:isUndo?change.oldValue:change.newValue});return;}if(action.page==='movement'&&action.actionType==='delete'){if(isUndo)await restoreMovementRows(action.mode,action.rows||[]);else await deleteMovementRows(action.mode,(action.rows||[]).map(r=>r.rowNumber));return;}if(action.page==='balikan-store'&&action.actionType==='update'){await applyBalikanHistoryUpdate(action,isUndo);return;}}
async function runActionHistory(direction){if(ACTION_HISTORY.running)return;const from=direction==='undo'?ACTION_HISTORY.undoStack:ACTION_HISTORY.redoStack;const to=direction==='undo'?ACTION_HISTORY.redoStack:ACTION_HISTORY.undoStack;const action=from.pop();if(!action){syncActionHistoryButtons();return;}ACTION_HISTORY.running=true;syncActionHistoryButtons();try{await applyHistoryAction(action,direction);to.push(action);logActivitySafe({action:direction==='undo'?'UNDO':'REDO',module:action.sheetName||action.page||'Global',detail:`${direction.toUpperCase()} ${action.actionType||'aksi'}`,reference:String(action.rowId||''),status:'SUCCESS',metadata:{oldValue:action.oldValue,newValue:action.newValue,action}});toast(direction==='undo'?'Undo berhasil':'Redo berhasil','success');}catch(err){from.push(action);toast(err?.message||(direction==='undo'?'Undo gagal':'Redo gagal'),'error');logActivitySafe({action:direction==='undo'?'UNDO':'REDO',module:action.sheetName||action.page||'Global',detail:err?.message||'Gagal',status:'FAILED',metadata:{action}});}finally{ACTION_HISTORY.running=false;syncActionHistoryButtons();}}
...
const mvCell=e.target.closest('[data-mv-cell]');if(mvCell){const mode=mvCell.dataset.mode,row=Number(mvCell.dataset.row),field=mvCell.dataset.field;const item=TABLE_STATE[mode].rows.find(r=>r.rowNumber===row);if(item)startInlineEdit(mvCell,item,field,item[field],{showSaveButton:false,onSave:async({value,oldValue,td})=>{td.classList.add('is-saving');td.innerHTML="<span class='btn-spinner-inline' aria-hidden='true'></span>";let out={};let res;try{res=await fetch(mode==='in'?'/api/barang-masuk/bulk-update':'/api/barang-keluar/bulk-update',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({rowNumbers:[row],updates:{[field]:value}})});out=await res.json();if(!res.ok||!out?.success)throw new Error(out?.message||'Gagal update');item[field]=value;const moduleName=mode==='in'?'barang-masuk':'barang-keluar';const sheetName=mode==='in'?'Barang Masuk':'Barang Keluar';const localRows=updateLocalRow(moduleName,row,field,value);const targetRows=updateCacheRow(moduleName,row,field,value);DATA[sheetName]=localRows;saveCache(DATA);pushActionHistory({actionType:'update',page:'movement',mode,sheetName,rowId:row,rowIndex:row,field,oldValue,newValue:value});td.innerHTML='';td.textContent=value||'-';td.dataset.value=value||'';td.dataset.oldValue=value||'';console.log('INLINE SUCCESS UPDATE UI',{rowNumber:row,field,newValue:value,tdText:td.textContent});toast('Data berhasil diupdate','success');logActivitySafe({action:mode==='in'?'EDIT_BARANG_MASUK':'EDIT_BARANG_KELUAR',module:mode==='in'?'Barang Masuk':'Barang Keluar',detail:`Edit ${field} row ${row}`,status:'SUCCESS'});}finally{td.classList.remove('is-saving');}}});return;}
const mvDelete=e.target.closest('[data-mv-delete]');if(mvDelete){const mode=mvDelete.dataset.mvDelete,row=Number(mvDelete.dataset.row);showConfirmModal({title:'Hapus Data',message:'Yakin ingin hapus data ini?',confirmText:'Hapus',cancelText:'Batal',type:'danger',onConfirm:async()=>{const st=TABLE_STATE[mode];st.deletingRows.add(row);renderDataTablePage(mode,mode==='in'?'Barang Masuk':'Barang Keluar',true);try{const snapshot=getMovementRowSnapshot(mode,row);const res=await fetch(mode==='in'?`/api/barang-masuk/${row}`:`/api/barang-keluar/${row}`,{method:'DELETE'});const out=await res.json();if(!res.ok||!out?.success)throw new Error(out?.message||'Gagal hapus');await refreshMovementTableData(mode,{deletedRowNumbers:[row]});toast('Data berhasil dihapus','success');if(snapshot)pushActionHistory({actionType:'delete',page:'movement',mode,sheetName:mode==='in'?'Barang Masuk':'Barang Keluar',rowId:row,rowIndex:row,oldValue:snapshot,newValue:null,rows:[snapshot]});logActivitySafe({action:mode==='in'?'DELETE_BARANG_MASUK':'DELETE_BARANG_KELUAR',module:mode,status:'SUCCESS'});}catch(err){toast('Gagal menghapus data','error');}finally{st.deletingRows.delete(row);renderDataTablePage(mode,mode==='in'?'Barang Masuk':'Barang Keluar',true);}}});return;}
const bulkDelete=e.target.closest('[data-mv-bulk-delete]');if(bulkDelete){const mode=bulkDelete.dataset.mvBulkDelete,selectedSet=getSelectedSet(mode),rows=[...selectedSet];if(!rows.length)return;showConfirmModal({title:'Bulk Delete',message:`Hapus ${rows.length} item?`,confirmText:'Hapus',cancelText:'Batal',type:'danger',onConfirm:async()=>{const st=TABLE_STATE[mode];st.bulkDeleting=true;renderDataTablePage(mode,mode==='in'?'Barang Masuk':'Barang Keluar',true);try{const snapshots=rows.map(row=>getMovementRowSnapshot(mode,row)).filter(Boolean);const res=await fetch(mode==='in'?'/api/barang-masuk/bulk-delete':'/api/barang-keluar/bulk-delete',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({rowNumbers:rows})});const out=await res.json();if(!res.ok||!out?.success)throw new Error(out?.message||'Gagal bulk delete');await refreshMovementTableData(mode,{deletedRowNumbers:rows});if(snapshots.length)pushActionHistory({actionType:'delete',page:'movement',mode,sheetName:mode==='in'?'Barang Masuk':'Barang Keluar',rowId:rows.join(','),oldValue:snapshots,newValue:null,rows:snapshots,timestamp:new Date().toISOString()});toast('Data berhasil dihapus','success');}catch(err){toast('Gagal menghapus data','error');}finally{st.bulkDeleting=false;renderDataTablePage(mode,mode==='in'?'Barang Masuk':'Barang Keluar',true);}}});return;}
const bulkEdit=e.target.closest('[data-mv-bulk-edit]');if(bulkEdit){const mode=bulkEdit.dataset.mvBulkEdit,selectedSet=getSelectedSet(mode),rows=[...selectedSet];if(!rows.length)return;const formHtml=`<div class='mv-bulk-edit-modal'><div class='subtitle'>${rows.length} item dipilih</div><div class='mv-bulk-edit-grid'><label>Tanggal<input id='mvBulkTanggal' class='search-lg' placeholder='Tanggal'></label><label>From<input id='mvBulkFrom' class='search-lg' placeholder='From'></label><label>To<input id='mvBulkTo' class='search-lg' placeholder='To'></label><label>SKU<input id='mvBulkSku' class='search-lg' placeholder='SKU'></label><label>Nama Barang<input id='mvBulkNamaBarang' class='search-lg' placeholder='Nama Barang'></label><label>Qty<input id='mvBulkQty' class='search-lg' placeholder='Qty' type='number'></label><label>Status<input id='mvBulkStatus' class='search-lg' placeholder='Status'></label><label>PIC<input id='mvBulkPic' class='search-lg' placeholder='PIC'></label><label>Keterangan<input id='mvBulkKeterangan' class='search-lg' placeholder='Keterangan'></label></div></div>`;showConfirmModal({title:'Bulk Edit',message:formHtml,allowHtmlMessage:true,confirmText:'Simpan Perubahan',cancelText:'Batal',onConfirm:()=>{const updates={};const map=[['tanggal','mvBulkTanggal'],['from','mvBulkFrom'],['to','mvBulkTo'],['sku','mvBulkSku'],['namaBarang','mvBulkNamaBarang'],['qty','mvBulkQty'],['status','mvBulkStatus'],['pic','mvBulkPic'],['keterangan','mvBulkKeterangan']];for(const [key,id] of map){const raw=(document.getElementById(id)?.value??'');const value=String(raw).trim();if(value!=="")updates[key]=key==='qty'?Number(value):value;}if(!Object.keys(updates).length){toast('Isi minimal 1 field untuk update','error');return;}fetch(mode==='in'?'/api/barang-masuk/bulk-update':'/api/barang-keluar/bulk-update',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({rowNumbers:rows,updates})}).then(async r=>({ok:r.ok,out:await r.json()})).then(({ok,out})=>{if(!ok||!out?.success)throw new Error(out?.message||'Gagal bulk edit');const changes=[];TABLE_STATE[mode].rows.forEach(r=>{if(selectedSet.has(r.rowNumber)){Object.entries(updates).forEach(([field,newValue])=>changes.push({actionType:'update',page:'movement',mode,sheetName:mode==='in'?'Barang Masuk':'Barang Keluar',rowId:r.rowNumber,rowIndex:r.rowNumber,field,oldValue:r[field],newValue}));Object.assign(r,updates);updateMovementLocalCaches(mode,r.rowNumber,updates);}});pushActionHistory({actionType:'update',page:'movement',mode,sheetName:mode==='in'?'Barang Masuk':'Barang Keluar',changes,timestamp:new Date().toISOString()});selectedSet.clear();renderDataTablePage(mode,mode==='in'?'Barang Masuk':'Barang Keluar',true);toast('Bulk edit berhasil','success');}).catch(err=>toast(err?.message||'Gagal bulk edit','error'));}});return;}
...
async function updateBalikanCell(sheetName,row,field,value,oldValue){const targetSheet=String(sheetName||getBalikanActiveSheetName(row)||'').trim();queueBalikanEdit(targetSheet,row,field,value,oldValue);pushActionHistory({actionType:'update',page:'balikan-store',sheetName:targetSheet,rowId:Number(row?.rowNumber),rowIndex:Number(row?.rowNumber),field,oldValue,newValue:value,timestamp:new Date().toISOString()});logActivitySafe({action:'EDIT_BALIKAN_STORE_QUEUE',module:'Balikan Store',detail:`[QUEUE EDIT] ${formatBalikanFieldName(field)}
Sheet: ${targetSheet||'-'}
SKU: ${row?.sku||'-'}
...
async function updateBalikanCheck(sheetName,rowNumber,checked){const targetSheet=String(sheetName||window.currentTripSheet||'').trim();const row=(window.BALIKAN_ROWS||[]).find(r=>Number(r.rowNumber)===Number(rowNumber)&&(!targetSheet||getBalikanActiveSheetName(r)===targetSheet));const oldChecked=isCheckedValue(row?.checked);const activeCheckbox=document.querySelector(`#${CSS.escape(getBalikanDomRowId(rowNumber,targetSheet))} .balikan-check`);updateBalikanLocalRow(rowNumber,'checked',checked?'TRUE':'FALSE',targetSheet);if(activeCheckbox)activeCheckbox.checked=checked===true;try{const res=await fetch('/api/balikan-store/check',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({sheetName:targetSheet,rowNumber,checked})});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal update centang');updateBalikanLocalRow(rowNumber,'checked',checked?'TRUE':'FALSE',targetSheet);if(checked===true)applyBalikanLastCheckedHighlight(Number(rowNumber),row?.sku,targetSheet);pushActionHistory({actionType:'update',page:'balikan-store',sheetName:targetSheet,rowId:Number(rowNumber),rowIndex:Number(rowNumber),field:'checked',oldValue:oldChecked?'TRUE':'FALSE',newValue:checked?'TRUE':'FALSE',timestamp:new Date().toISOString()});toast('Centang berhasil diupdate','success');logActivitySafe({action:'UPDATE_BALIKAN_STORE_CHECK',module:'Balikan Store',detail:`[CHECK]
Sheet: ${targetSheet||'-'}
SKU: ${row?.sku||'-'}
