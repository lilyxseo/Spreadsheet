...
const skuIdx=tokens.findIndex((tok,idx)=>isSkuCandidate(tok)&&(/\d{6,}/.test(tok)||(/\d/.test(tok)&&idx>0)));
if(skuIdx<0){if(parsed.length&&/^[A-Za-z]/.test(line)){parsed[parsed.length-1].namaProduk=`${parsed[parsed.length-1].namaProduk} ${line}`.trim();continue;}skipped.push(`${line} => sku tidak ditemukan`);continue;}
const sku=tokens[skuIdx];const before=tokens.slice(0,skuIdx).join(' ').trim();const after=tokens.slice(skuIdx+1);
const numericTail=after.filter(x=>/^\d+$/.test(x));
...
const catatan=after.slice(numericTail.length).join(' ').trim();
parsed.push({sku,namaProduk:before||'-',jumlah,diterima,batal,tolak,catatan});
}
