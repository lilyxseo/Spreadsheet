...
const isIgnoredRow=(rowText)=>/^(transfer\s*#|dari\b|kepada\b|nomor\s+referensi|tanggal\s+dibuat|perkiraan\s+tanggal|status\s+transfer)/i.test(rowText)||isTransferHeaderLikeLine(rowText)&&/(kode\s+barang|sku|jumlah|qty)/i.test(rowText);
const skuRowIndexes=[];
