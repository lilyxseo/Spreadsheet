...
const CYCLE_STORAGE_KEY="inventory_cycle_count_state_v1";
...
const CYCLE_STATE={sessions:[],activeId:null,page:1,pageSize:50,skuQuery:"",filterStatus:"all"};
...
function saveCycleState(){localStorage.setItem(CYCLE_STORAGE_KEY,JSON.stringify({sessions:CYCLE_STATE.sessions,activeId:CYCLE_STATE.activeId}));}
function loadCycleState(){try{const raw=JSON.parse(localStorage.getItem(CYCLE_STORAGE_KEY)||"{}");CYCLE_STATE.sessions=Array.isArray(raw.sessions)?raw.sessions:[];CYCLE_STATE.activeId=raw.activeId||CYCLE_STATE.sessions[0]?.id||null;}catch(_){CYCLE_STATE.sessions=[];CYCLE_STATE.activeId=null;}}
function createCycleSession(payload={}){const id=`cc-${Date.now()}`;const ses={id,status:"draft",name:payload.name||`Sesi ${new Date().toLocaleDateString("id-ID")}`,date:payload.date||getTodayDateKey(),pic:payload.pic||"",location:payload.location||"",notes:payload.notes||"",items:[],createdAt:new Date().toISOString(),finalizedAt:""};CYCLE_STATE.sessions.unshift(ses);CYCLE_STATE.activeId=id;saveCycleState();return ses;}
function addSkuToSession(sessionId,sku){const ses=CYCLE_STATE.sessions.find(s=>s.id===sessionId&&s.status==="draft");if(!ses||!clean(sku))return false;const key=clean(sku);if(ses.items.some(i=>clean(i.sku)===key))return false;const system=getSystemStockBySku(sku);if(!system)return false;ses.items.push({sku:system.sku,nama:system.nama,lokasi:system.lokasi,systemQty:system.systemQty,stokBulky:system.stokBulky,stokRetail:system.stokRetail,stokGlobal:system.stokGlobal,actualQty:0,notes:"",diff:0,status:"Cocok"});saveCycleState();return true;}
function updateActualQty(sessionId,sku,actualQty,notes){const ses=CYCLE_STATE.sessions.find(s=>s.id===sessionId&&s.status==="draft");if(!ses)return;const it=ses.items.find(x=>clean(x.sku)===clean(sku));if(!it)return;it.actualQty=parseNumber(actualQty);it.notes=notes||it.notes||"";Object.assign(it,calculateDifference(it.actualQty,it.systemQty));saveCycleState();}
function calculateDifference(actualQty,systemQty){const diff=parseNumber(actualQty)-parseNumber(systemQty);return{diff,status:diff===0?"Cocok":(diff>0?"Lebih (Plus)":"Kurang (Minus)")};}
function getCycleSummary(session){const total=session?.items?.length||0;const match=(session?.items||[]).filter(i=>i.diff===0).length;const mismatch=total-match;const totalDiff=(session?.items||[]).reduce((n,i)=>n+i.diff,0);const accuracy=total?((match/total)*100):0;return{total,match,mismatch,totalDiff,accuracy};}
function finalizeSession(sessionId){const ses=CYCLE_STATE.sessions.find(s=>s.id===sessionId&&s.status==="draft");if(!ses)return;ses.status="selesai";ses.finalizedAt=new Date().toISOString();saveCycleState();}
function getSystemStockBySku(sku){const rows=[...(DATA["RPL"]||[]),...(DATA["BULKY"]||[]),...(DATA["Kartu Stock"]||[])].filter(r=>clean(getVal(r,["sku"]))===clean(sku));if(!rows.length)return null;const base=rows[0]||{};const nama=getVal(base,["nama barang","nama","item","description"])||"-";const lokasi=getVal(base,["lokasi","location","rak","bin","area"])||"-";const stokBulky=rows.reduce((n,r)=>n+parseNumber(getVal(r,["stok bulky"])),0);const stokRetail=rows.reduce((n,r)=>n+parseNumber(getVal(r,["stok retail"])),0);const stokGlobal=rows.reduce((n,r)=>n+parseNumber(getVal(r,["stok global"])),0);const kartu=rows.reduce((n,r)=>n+parseNumber(getVal(r,["stok kartu","kartu stok","stok akhir","saldo akhir"])),0);return{sku:getVal(base,["sku"]),nama,lokasi,stokBulky,stokRetail,stokGlobal,systemQty:kartu||stokGlobal||stokBulky+stokRetail};}
function renderCycleCountPage(){if(!cycleCountApp)return;loadCycleState();const active=CYCLE_STATE.sessions.find(s=>s.id===CYCLE_STATE.activeId)||null;const list=CYCLE_STATE.sessions.map(s=>`<tr><td>${esc(s.name)}</td><td>${esc(s.date)}</td><td>${esc(s.pic||"-")}</td><td>${esc(s.status)}</td><td><button class="btn-ghost" onclick="setCycleActive('${s.id}')">${s.status==="draft"?"Lanjutkan":"Lihat hasil"}</button><button class="btn-ghost" onclick="deleteCycleSession('${s.id}')">Hapus</button></td></tr>`).join("");
const activeItems=active?.items||[];const start=(CYCLE_STATE.page-1)*CYCLE_STATE.pageSize;const rows=activeItems.filter(i=>CYCLE_STATE.filterStatus==="all"||(CYCLE_STATE.filterStatus==="cocok"?i.diff===0:i.diff!==0)).filter(i=>!CYCLE_STATE.skuQuery||clean(`${i.sku} ${i.nama}`).includes(clean(CYCLE_STATE.skuQuery))).slice(start,start+CYCLE_STATE.pageSize);
const tableRows=rows.map(i=>`<tr class='${i.diff===0?"cycle-row-match":(i.diff>0?"cycle-row-plus":"cycle-row-minus")}'><td>${esc(i.sku)}</td><td>${esc(i.nama)}</td><td>${esc(i.lokasi)}</td><td>${i.systemQty}</td><td>${i.stokBulky}</td><td>${i.stokRetail}</td><td>${i.stokGlobal}</td><td><input type="number" ${active?.status==="selesai"?"disabled":""} value="${i.actualQty}" oninput="updateActualQty('${active?.id}','${encAttr(i.sku)}',this.value,'')"></td><td>${esc(i.status)}</td><td>${i.diff}</td></tr>`).join("");
const sum=getCycleSummary(active||{items:[]});
cycleCountApp.innerHTML=`<div class="cycle-grid"><div class="cycle-card"><h4>Buat Sesi Cycle Count</h4><div class="cycle-form"><input id="ccName" placeholder="Nama sesi"><input id="ccDate" type="date" value="${getTodayDateKey()}"><input id="ccPic" placeholder="PIC"><input id="ccLocation" placeholder="Lokasi (opsional)"><input id="ccNotes" placeholder="Catatan"></div><div class="row"><button class="btn-primary" onclick="createCycleSessionFromForm()">Simpan Sesi</button><button class="btn-ghost" onclick="addCycleFromSkuInput()">Tambah SKU</button><button class="btn-ghost" onclick="addFromStokMinus()">Ambil dari stok minus</button><button class="btn-ghost" onclick="addFromAnomaly()">Ambil dari anomaly</button><input id="ccSkuInput" placeholder="scan/ketik SKU"></div></div><div class="cycle-card"><h4>List Sesi</h4><div class="cycle-table-wrap table-wrap"><table><thead><tr><th>Sesi</th><th>Tanggal</th><th>PIC</th><th>Status</th><th>Aksi</th></tr></thead><tbody>${list||"<tr><td colspan='5'>Belum ada sesi.</td></tr>"}</tbody></table></div></div></div><div class="cycle-card"><div class="row"><strong>Detail Sesi:</strong> ${esc(active?.name||"-")} <span class="badge">${sum.total} SKU</span><span class="badge">Akurasi ${sum.accuracy.toFixed(2)}%</span><button class="btn-ghost" onclick="exportCycleCsv()">Export CSV</button>${active?.status==="draft"?"<button class='btn-primary' onclick=\"finalizeCycleSession()\">Selesaikan Cycle Count</button>":""}</div><div class="row"><input placeholder="Filter SKU/Nama" value="${esc(CYCLE_STATE.skuQuery)}" oninput="setCycleQuery(this.value)"><select onchange="setCycleStatusFilter(this.value)"><option value="all">Semua</option><option value="cocok">Cocok</option><option value="selisih">Selisih</option></select></div><div class="cycle-table-wrap table-wrap"><table><thead><tr><th>SKU</th><th>Nama</th><th>Lokasi</th><th>Kartu</th><th>Bulky</th><th>Retail</th><th>Global</th><th>Aktual</th><th>Status</th><th>Selisih</th></tr></thead><tbody>${tableRows||"<tr><td colspan='10'>Belum ada SKU dipilih.</td></tr>"}</tbody></table></div></div>`;
if(window.lucide)lucide.createIcons();
}
window.createCycleSessionFromForm=()=>{const payload={name:document.getElementById("ccName")?.value,date:document.getElementById("ccDate")?.value,pic:document.getElementById("ccPic")?.value,location:document.getElementById("ccLocation")?.value,notes:document.getElementById("ccNotes")?.value};createCycleSession(payload);renderCycleCountPage();};
window.addCycleFromSkuInput=()=>{const sku=document.getElementById("ccSkuInput")?.value||"";if(!CYCLE_STATE.activeId||!clean(sku))return;addSkuToSession(CYCLE_STATE.activeId,sku);renderCycleCountPage();};
window.addFromStokMinus=()=>{if(!CYCLE_STATE.activeId)return;const rows=(DATA["Kartu Stock"]||[]).filter(r=>parseNumber(getVal(r,["stok akhir","saldo akhir"]))<0);rows.forEach(r=>addSkuToSession(CYCLE_STATE.activeId,getVal(r,["sku"])));renderCycleCountPage();};
window.addFromAnomaly=()=>{if(!CYCLE_STATE.activeId)return;const rows=[...(DATA["RPL"]||[]),...(DATA["BULKY"]||[])].filter(r=>parseNumber(getVal(r,["selisih","selisih kartu stok"]))!==0);rows.forEach(r=>addSkuToSession(CYCLE_STATE.activeId,getVal(r,["sku"])));renderCycleCountPage();};
window.setCycleActive=(id)=>{CYCLE_STATE.activeId=id;saveCycleState();renderCycleCountPage();};
window.deleteCycleSession=(id)=>{CYCLE_STATE.sessions=CYCLE_STATE.sessions.filter(s=>s.id!==id);if(CYCLE_STATE.activeId===id)CYCLE_STATE.activeId=CYCLE_STATE.sessions[0]?.id||null;saveCycleState();renderCycleCountPage();};
window.setCycleQuery=(v)=>{CYCLE_STATE.skuQuery=v||"";CYCLE_STATE.page=1;renderCycleCountPage();};
window.setCycleStatusFilter=(v)=>{CYCLE_STATE.filterStatus=v||"all";CYCLE_STATE.page=1;renderCycleCountPage();};
window.finalizeCycleSession=()=>{if(!CYCLE_STATE.activeId)return;finalizeSession(CYCLE_STATE.activeId);renderCycleCountPage();};
window.exportCycleCsv=()=>{const ses=CYCLE_STATE.sessions.find(s=>s.id===CYCLE_STATE.activeId);if(!ses)return;const head=["SKU","Nama","Lokasi","SystemQty","ActualQty","Selisih","Status"];const lines=ses.items.map(i=>[i.sku,i.nama,i.lokasi,i.systemQty,i.actualQty,i.diff,i.status].map(v=>`"${String(v??"").replaceAll('"','""')}"`).join(","));downloadCsv([head.join(","),...lines].join("\n"),`cycle-count-${ses.id}.csv`);};
...
function downloadCsv(content,filename){const blob=new Blob([content],{type:"text/csv;charset=utf-8;"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=filename;a.click();URL.revokeObjectURL(a.href);}
