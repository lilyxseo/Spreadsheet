...
balikanSearchInput?.addEventListener("input",e=>{window.balikanSearchKeyword=e.target.value||"";BALIKAN_STATE.exactScanSku="";renderBalikanTable();});
balikanSortSelect?.addEventListener("change",e=>{BALIKAN_STATE.sortBy=e.target.value||"default";renderBalikanTable();});
...
const toggle=e.target.closest("[data-col-filter-toggle]");if(toggle){const mode=toggle.dataset.mode,col=toggle.dataset.col;document.querySelectorAll(`[data-col-filter-menu][data-mode="${mode}"]`).forEach(menu=>menu.hidden=true);const menu=document.querySelector(`[data-col-filter-menu][data-mode="${mode}"][data-col="${col}"]`);if(menu){menu.hidden=!menu.hidden;const st=TABLE_STATE[mode];if(st)st.openFilterCol=menu.hidden?"":col;if(mode==='balikan'&&BALIKAN_STATE.filterState)BALIKAN_STATE.filterState.openFilterCol=menu.hidden?'':col;if(!menu.hidden)positionColumnFilterMenu(menu);}return;}
if(e.target.closest("[data-col-filter-menu]")){const menu=e.target.closest("[data-col-filter-menu]");const mode=menu.dataset.mode,col=menu.dataset.col;const st=mode==='balikan'?ensureBalikanFilterState():(TABLE_STATE[mode]||{});ensureColumnFilterState(mode);if(e.target.matches("[data-col-filter-clear]")){st.columnFilters[col]=[];st.openFilterCol=col;rerenderTableWithScrollRestore(mode,true);}if(e.target.matches("[data-col-filter-all]")){const rowsSource=mode==='balikan'?((BALIKAN_STATE.filterState?.rows)||[]):(st.rows||[]);const applier=mode==='balikan'?applyBalikanTableFilters:applyTableFilters;st.columnFilters[col]=getUniqueOptions(applier(rowsSource,mode,col),col);st.openFilterCol=col;rerenderTableWithScrollRestore(mode,true);}return;}
...
const STOCK_ACCURACY_CACHE_KEY="STOCK_ACCURACY_CACHE_V1";
const STATS_STATE={page:1,pageSize:25,searchInputValue:"",debouncedSearchValue:"",sort:"absDesc",isFiltering:false,isProcessing:false,_normalizedRows:null,_debounceTimer:null,_idleTimer:null,_lastSourceKey:""};
...
function setStatsProcessing(flag,msg="Memproses..."){
  STATS_STATE.isProcessing=flag;
  const el=document.getElementById("statsProcessState");
  if(el)el.textContent=flag?msg:"";
}
function getStatsSourceKey(){
  return `${(DATA["RPL"]||[]).length}|${(DATA["BULKY"]||[]).length}`;
}
function getStatsCache(){return safeJsonParse(localStorage.getItem(STOCK_ACCURACY_CACHE_KEY),null);}
function setStatsCache(payload){if(payload&&Array.isArray(payload.rows)&&payload.rows.length)localStorage.setItem(STOCK_ACCURACY_CACHE_KEY,JSON.stringify(payload));}
function computeStatsPayload(){
const rows=[...(DATA["RPL"]||[]),...(DATA["BULKY"]||[])].filter(r=>clean(getVal(r,["sku"])));
if(!rows.length)return null;
const bySku=new Map();
let selisihTotal=0;
for(const r of rows){const sel=parseNumber(getVal(r,["selisih","selisih kartu stok","selisih kartu stock","selisih kartu stok vs iseller","selisih kartu stok vs netsuite"]));selisihTotal+=sel;const sku=getVal(r,["sku"])||"-";const key=clean(sku);if(!key)continue;const nama=getVal(r,["nama barang","nama"])||"-";const lokasi=getVal(r,["lokasi"])||"-";if(!bySku.has(key))bySku.set(key,{sku,nama,lokasiSet:new Set(),selisih:0,selisihAbs:0});const it=bySku.get(key);if(it.nama==="-"&&nama!=="-")it.nama=nama;if(lokasi&&lokasi!=="-")it.lokasiSet.add(String(lokasi));it.selisih+=sel;it.selisihAbs=Math.abs(it.selisih);}
const aggregated=[...bySku.values()].map(r=>({...r,lokasi:[...r.lokasiSet],lokasiText:[...r.lokasiSet].length?[...r.lokasiSet].join(", "):"-"}));
const totalSku=rows.length,skuAkurat=rows.reduce((n,r)=>n+(parseNumber(getVal(r,["selisih","selisih kartu stok","selisih kartu stock","selisih kartu stok vs iseller","selisih kartu stok vs netsuite"]))===0?1:0),0;
return {rows:aggregated,totalSku,skuAkurat,skuTidakAkurat:totalSku-skuAkurat,akurasi:totalSku?((skuAkurat/totalSku)*100):0,selisihTotal,sourceKey:getStatsSourceKey()};
}
function applyStatsPayload(payload){
  STATS_STATE._normalizedRows=(payload.rows||[]).map(r=>({...r,_searchText:clean(`${r.sku} ${r.nama} ${r.lokasiText}`)}));
  statsCards.innerHTML=[["Total SKU",payload.totalSku,""],["Akurat (%)",`${payload.akurasi.toFixed(2)}%`,"ok"],["Tidak Akurat",payload.skuTidakAkurat,"err"],["Selisih Total",payload.selisihTotal,"warn"]].map(c=>`<div class='metric ${c[2]||""}'><div class='k'>${c[0]}</div><div class='v'>${esc(c[1])}</div></div>`).join("");
  updateStatsTableOnly();
}
function updateStats(){
if(![...(DATA["RPL"]||[]),...(DATA["BULKY"]||[])].length){statsCards.innerHTML="";statsChart.innerHTML="<div class='state'>Sheet RPL/BULKY belum ada data.</div>";return;}
...
<select id='statsSize' aria-label='Jumlah data per halaman'><option value='25'>25 / halaman</option><option value='50'>50 / halaman</option><option value='100'>100 / halaman</option></select><small id='statsFilterState' class='stats-filtering-indicator'></small><small id='statsProcessState' class='stats-filtering-indicator'></small></div>
...
if(cache&&Array.isArray(cache.rows)&&cache.rows.length){applyStatsPayload(cache);}
setStatsProcessing(true,"Memproses...");
setTimeout(()=>{const payload=computeStatsPayload();if(payload){const same=payload.sourceKey===STATS_STATE._lastSourceKey&&Array.isArray(STATS_STATE._normalizedRows)&&STATS_STATE._normalizedRows.length===payload.rows.length;if(!same)applyStatsPayload(payload);setStatsCache(payload);STATS_STATE._lastSourceKey=payload.sourceKey;}setStatsProcessing(false,"");renderStatsFilteringState();},0);
renderStatsFilteringState();
}
...
const ARCHIVE_STATE={sheetList:[],selectedSheet:"",cache:{},loadingList:false,loadingData:false,listError:"",dataError:"",searchInput:"",search:"",searchTimer:null,searchToken:0,minSearchLength:2,sortColumn:"",sortDirection:"asc",columnFilter:"all",page:1,pageSize:25,lastLoadedAt:{}};
...
async function fetchArchiveSheetData(sheetKey,sheetList=ARCHIVE_STATE.sheetList){
...
function startInlineEdit(cellEl,row,field,oldValue,options={}){const td=cellEl.closest('td')||cellEl;if(td.querySelector("input,select"))return;const input=document.createElement("input");input.value=oldValue||"";input.className="inline-edit-input";td.classList.add("editing");td.innerHTML="";td.appendChild(input);input.focus();input.select();let cancelled=false;async function save(){if(cancelled)return;const newValue=input.value.trim();if(newValue===String(oldValue||"")){td.classList.remove("editing");td.textContent=oldValue||"-";return;}try{await options.onSave?.({row,field,value:newValue,oldValue,td});}catch(err){td.textContent=oldValue||"-";toast(err?.message||"Gagal update data","error");}finally{td.classList.remove("editing");}}input.addEventListener("blur",save);input.addEventListener("keydown",(e)=>{if(e.key==="Enter")input.blur();if(e.key==="Escape"){cancelled=true;td.classList.remove("editing");td.textContent=oldValue||"-";}});}
...
function ensureBalikanFilterState(){if(!BALIKAN_STATE.filterState)BALIKAN_STATE.filterState={page:1,pageSize:1000,openFilterCol:'',columnFilters:{},rows:[]};const st=BALIKAN_STATE.filterState;BALIKAN_FILTERABLE_COLUMNS.forEach(k=>{if(!Array.isArray(st.columnFilters[k]))st.columnFilters[k]=[];});return st;}
function applyBalikanTableFilters(rows,mode='balikan',omitCol=''){const st=ensureBalikanFilterState();const q=clean(window.balikanSearchKeyword||'');const exactScanSku=clean(BALIKAN_STATE.exactScanSku||'');return rows.filter(r=>{if(BALIKAN_FILTERABLE_COLUMNS.some(col=>{if(col===omitCol)return false;const selected=st.columnFilters[col]||[];if(!selected.length)return false;return !selected.includes(sanitizeFilterValue(r[col]));}))return false;if(exactScanSku&&clean(r.sku)!==exactScanSku)return false;if(q&&!clean(`${r.sku} ${r.namaBarang} ${r.lokasi} ${r.status} ${r.keterangan} ${r.rakTujuan}`).includes(q))return false;return true;});}
...
function resetBalikanFilter(){window.balikanSearchKeyword='';if(balikanSearchInput)balikanSearchInput.value='';if(balikanSortSelect)balikanSortSelect.value='default';BALIKAN_STATE.sortBy='default';BALIKAN_STATE.highlightRowNumber=null;BALIKAN_STATE.exactScanSku='';const st=ensureBalikanFilterState();st.columnFilters={};st.openFilterCol='';renderBalikanTable(false);}
function exportBalikanFilteredCsv(){const st=ensureBalikanFilterState();const baseRows=(window.BALIKAN_ROWS||[]).map(r=>({...r}));const filtered=sortBalikanRows(applyBalikanTableFilters(baseRows),BALIKAN_STATE.sortBy||'default');const cols=['no','sku','namaBarang','qty','rakTujuan','lokasi','stokBulky','stokRetail','status','keterangan','checked'];const header=['No','SKU','Nama Barang','Qty','Rak Tujuan','Lokasi','Stok Bulky','Stok Retail','Status','Keterangan','Centang'];const lines=[header.join(','),...filtered.map(row=>cols.map(c=>`"${String(row[c]??'').replaceAll('"','""')}"`).join(','))];const blob=new Blob([lines.join('\n')],{type:'text/csv;charset=utf-8;'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);const sheet=String(window.currentTripSheet||'balikan-store').replace(/[^a-z0-9-_]+/gi,'-');a.download=`${sheet}-filtered.csv`;a.click();URL.revokeObjectURL(a.href);toast('Export CSV berhasil','success');}
...
async function handleBalikanScanResult(decodedText){if(navigator.vibrate)navigator.vibrate(100);const sku=cleanScannedSku(decodedText);window.balikanSearchKeyword=sku;BALIKAN_STATE.exactScanSku=sku;const searchInput=document.querySelector("#balikanSearchInput");if(searchInput)searchInput.value=sku;const row=(window.BALIKAN_ROWS||[]).find(item=>String(item.sku||"").trim().toLowerCase()===String(sku||"").trim().toLowerCase());if(!row){BALIKAN_STATE.highlightRowNumber=null;toast("SKU tidak ditemukan di Balikan Store: "+sku,"error");renderBalikanTable(false);return;}BALIKAN_STATE.highlightRowNumber=Number(row.rowNumber);renderBalikanTable(false);if(BALIKAN_STATE.autoCheckOnScan!==false){await toggleBalikanCheck(window.currentTripSheet,Number(row.rowNumber),true);showToast('SKU berhasil discan dan dicentang: '+sku,'success');}else{showToast('SKU berhasil discan: '+sku,'success');}setTimeout(()=>{const el=document.querySelector(`[data-row-number="${row.rowNumber}"]`);if(el){el.scrollIntoView({behavior:"smooth",block:"center"});el.classList.add("row-highlight");}},300);}
...
const ABC_STATE={periodMonths:3,orderType:"Semua",selectedTo:[],excludeTo:["REJECT","RUSAK"],sortBy:"Prioritas Order",toSearch:"",toDropdownOpen:false,selectedRows:new Set(),page:1,pageSize:25,toSearchInput:"",toDebounceTimer:null,isProcessing:false,lastRenderedKey:""};
...
function buildAbcAnalysis({forceRefresh=false}={}){
const now=Date.now();if(!forceRefresh){const raw=localStorage.getItem(ABC_ANALYSIS_CACHE_KEY);const parsed=safeJsonParse(raw,null);if(parsed&&parsed.key&&parsed.ts&&now-parsed.ts<24*60*60*1000)return parsed;}
...
if(!abcAnalisisApp)return;const data=buildAbcAnalysis({forceRefresh});
if(!data.sourceHasData){abcAnalisisApp.innerHTML="<div class='card'><div class='state'>Tidak ada data pada periode ini</div></div>";return;}
if(!data.rows.length&&data.filteredCount===0){abcAnalisisApp.innerHTML="<div class='card'><div class='state'>Belum ada data pada hasil filter saat ini. Ubah periode/tipe/tujuan atau klik Reset.</div></div>";return;}
if(!data.rows.length){abcAnalisisApp.innerHTML="<div class='card'><div class='state'>Filter tidak valid, reset filter untuk melihat data</div></div>";return;}
...
const toOptionFiltered=(data.toOptions||[]).filter(v=>clean(v).includes(clean(ABC_STATE.toSearchInput||"")));
const toSummary=selectedToSet.size?`${selectedToSet.size} tujuan dipilih`:"Semua Tujuan";
const maxPage=Math.max(1,Math.ceil(data.rows.length/ABC_STATE.pageSize));if(ABC_STATE.page>maxPage)ABC_STATE.page=maxPage;
const pageRows=data.rows.slice((ABC_STATE.page-1)*ABC_STATE.pageSize,ABC_STATE.page*ABC_STATE.pageSize);
abcAnalisisApp.innerHTML=`<div class='card' style='overflow:visible'><div class='mv-filters open' style='display:flex;flex-wrap:wrap;gap:8px;align-items:center;overflow:visible'><select id='abcPeriod' class='search-lg' style='max-width:170px;height:34px;padding:5px 14px!important'>${periodOpt}</select><select id='abcOrderType' class='search-lg' style='max-width:160px;height:34px;padding:5px 14px!important'><option ${ABC_STATE.orderType==="Semua"?"selected":""}>Semua</option><option ${ABC_STATE.orderType==="Orderan GT"?"selected":""}>Orderan GT</option><option ${ABC_STATE.orderType==="Orderan Store"?"selected":""}>Orderan Store</option></select><div id='abcToWrap' style='position:relative;min-width:220px;flex:1 1 240px;max-width:360px'><button id='abcToToggle' class='btn-ghost abc-action-btn' style='height:34px;width:100%;text-align:left;display:flex;justify-content:space-between;align-items:center'><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(toSummary)}</span><span style="opacity:.85;font-size:12px">${ABC_STATE.toDropdownOpen?'▴':'▾'}</span></button><div id='abcToMenu' style='position:absolute;left:0;right:auto;top:38px;background:var(--panel,#fff);color:var(--text,#0f172a);border:1px solid var(--line,#d4d4d8);border-radius:10px;padding:8px;min-width:280px;width:min(360px,calc(100vw - 48px));z-index:25;box-shadow:0 12px 24px rgba(0,0,0,.18);${ABC_STATE.toDropdownOpen?'':'display:none'}'><input id='abcToSearch' class='search-lg' placeholder='Cari tujuan...' value='${esc(ABC_STATE.toSearchInput||"")}' style='width:100%;height:34px;margin-bottom:8px'><div style='display:flex;gap:6px;margin-bottom:8px'><button type='button' id='abcToSelectAll' class='btn-ghost abc-action-btn' style='height:30px;font-size:12px;flex:1'>Pilih semua</button><button type='button' id='abcToClear' class='btn-ghost abc-action-btn' style='height:30px;font-size:12px;flex:1'>Hapus</button></div><div style='max-height:240px;overflow:auto;padding-right:2px'>${toOptionFiltered.map(v=>`<label style='display:flex;gap:8px;align-items:flex-start;padding:5px 4px;border-radius:8px;cursor:pointer'><input type='checkbox' data-abc-to='${encAttr(v)}' ${selectedToSet.has(v)?'checked':''} style='margin-top:1px;width:18px;height:18px;accent-color:var(--primary,#e11d48);flex:0 0 18px'><span style='font-size:13px;line-height:1.35;word-break:break-word'>${esc(v)}</span></label>`).join('')||"<div class='subtitle'>Tidak ada opsi</div>"}</div></div></div><select id='abcSortBy' class='search-lg' style='max-width:190px;height:34px;padding:5px 14px!important'>${sortOpt}</select><select id='abcSize' class='search-lg' style='max-width:130px;height:34px'><option value='25'>25</option><option value='50'>50</option></select><small id='abcProcessState' class='stats-filtering-indicator'>${ABC_STATE.isProcessing?"Memproses...":""}</small><button id='abcResetBtn' class='btn-ghost abc-action-btn' style='height:34px'>Reset</button><button id='abcCopyBtn' class='btn-ghost abc-action-btn' style='height:34px'>Copy</button><button id='abcRefreshBtn' class='btn-primary abc-action-btn' style='height:34px'>Refresh</button></div><div class='table-wrap' style='margin-top:14px;max-height:calc(100vh - 280px);overflow:auto'><table><thead><tr><th><input type='checkbox' id='abcSelectAll' class='balikan-check'></th><th>SKU</th><th>Nama Barang</th><th>Total Qty Keluar</th><th>% Kontribusi</th><th>Cumulative %</th><th>Kategori ABC</th><th>Stok Saat Ini</th><th>Priority Order</th><th>Status Rekomendasi</th></tr></thead><tbody>${pageRows.map(r=>`<tr><td><input type='checkbox' class='balikan-check' data-abc-row='${encAttr(r.sku)}' ${ABC_STATE.selectedRows.has(r.sku)?"checked":""}></td><td>${esc(r.sku)}</td><td><div style='display:flex;align-items:center;gap:6px;justify-content:space-between'><span>${esc(r.namaBarang)}</span><button class='btn-ghost' data-abc-sku='${encAttr(r.sku)}' style='padding:2px 6px;font-size:11px;line-height:1.1;border-radius:8px'>Detail</button></div></td><td>${r.qtyKeluar}</td><td>${(r.kontribusi*100).toFixed(2)}%</td><td>${(r.cum*100).toFixed(2)}%</td><td><span class='badge ${r.abc==="A"?"b-out":r.abc==="B"?"b-warn":"b-idle"}'>${r.abc}</span></td><td>${r.stokSaatIni}</td><td>${esc(r.priority)}</td><td>${esc(r.rekom)}</td></tr>`).join("")}</tbody></table></div><div class='mv-pagination'><span>Menampilkan ${(data.rows.length?((ABC_STATE.page-1)*ABC_STATE.pageSize+1):0)}–${Math.min(ABC_STATE.page*ABC_STATE.pageSize,data.rows.length)} dari ${data.rows.length} data</span><div class='row'><button id='abcPrev' class='btn-ghost'>Prev</button><button id='abcNext' class='btn-ghost'>Next</button></div></div></div><div class='card' style='margin-top:8px;padding:8px 10px'><div class='section-header' style='margin-bottom:4px'><h4 style='margin:0;font-size:13px'>Top 10 Fast Moving Items</h4></div><ol style='margin:0;padding-left:16px;display:block'>${data.top10.map(r=>`<li style='margin:0 0 2px 0'><button class='btn-link' data-abc-sku='${encAttr(r.sku)}' style='font-size:12px;line-height:1.2;padding:0'>${esc(r.namaBarang)}</button></li>`).join("")}</ol></div>`;
...
const toSearchEl=document.getElementById("abcToSearch");if(toSearchEl){toSearchEl.oninput=e=>{e.stopPropagation();ABC_STATE.toSearchInput=e.target.value||"";clearTimeout(ABC_STATE.toDebounceTimer);ABC_STATE.toDebounceTimer=setTimeout(()=>renderAbcAnalisisPage(false),250);};toSearchEl.onclick=e=>e.stopPropagation();}
...
document.getElementById("abcSize").value=String(ABC_STATE.pageSize);
document.getElementById("abcSize").onchange=e=>{ABC_STATE.pageSize=Number(e.target.value)||25;ABC_STATE.page=1;renderAbcAnalisisPage(false);};
document.getElementById("abcPrev").onclick=()=>{ABC_STATE.page=Math.max(1,ABC_STATE.page-1);renderAbcAnalisisPage(false);};
document.getElementById("abcNext").onclick=()=>{ABC_STATE.page=Math.min(maxPage,ABC_STATE.page+1);renderAbcAnalisisPage(false);};
...
if(selAllEl)selAllEl.indeterminate=selectedInPage>0&&selectedInPage<data.rows.length;
if(selAllEl)selAllEl.onchange=e=>{const checked=!!e.target.checked;data.rows.forEach(r=>{if(checked)ABC_STATE.selectedRows.add(r.sku);else ABC_STATE.selectedRows.delete(r.sku);});abcAnalisisApp.querySelectorAll("[data-abc-row]").forEach(box=>{box.checked=checked;});};
abcAnalisisApp.querySelectorAll("[data-abc-row]").forEach(chk=>chk.onchange=e=>{const sku=e.target.dataset.abcRow||"";if(e.target.checked)ABC_STATE.selectedRows.add(sku);else ABC_STATE.selectedRows.delete(sku);const count=data.rows.filter(r=>ABC_STATE.selectedRows.has(r.sku)).length;if(selAllEl){selAllEl.checked=count===data.rows.length&&count>0;selAllEl.indeterminate=count>0&&count<data.rows.length;}});
document.getElementById("abcCopyBtn").onclick=()=>{const selected=data.rows.filter(r=>ABC_STATE.selectedRows.has(r.sku));if(!selected.length){toast("Pilih data dulu","error");return;}const header=["SKU","Nama Barang","Total Qty Keluar","% Kontribusi","Cumulative %","Kategori ABC","Stok Saat Ini","Priority Order","Status Rekomendasi"];const lines=[header.join("\t"),...selected.map(r=>[r.sku,r.namaBarang,r.qtyKeluar,`${(r.kontribusi*100).toFixed(2)}%`,`${(r.cum*100).toFixed(2)}%`,r.abc,r.stokSaatIni,r.priority,r.rekom].join("\t"))].join("\n");navigator.clipboard.writeText(lines).then(()=>toast("Data berhasil dicopy","success")).catch(()=>toast("Gagal copy data","error"));};
if(!window.__abcOutsideClickBound){
window.__abcOutsideClickBound=true;
document.addEventListener("click",(e)=>{const wrap=document.getElementById("abcToWrap");if(!wrap)return;const inWrap=wrap.contains(e.target);if(!inWrap&&ABC_STATE.toDropdownOpen){ABC_STATE.toDropdownOpen=false;renderAbcAnalisisPage(false);}});
abcAnalisisApp.querySelectorAll('#abcToMenu, #abcToMenu *').forEach(el=>el.addEventListener('click',ev=>ev.stopPropagation()));
}
abcAnalisisApp.querySelectorAll("[data-abc-sku]").forEach(btn=>btn.onclick=()=>navigateToSku(btn.dataset.abcSku||""));
}
