...
function getBalikanLocationSummary(rows=[]){
  ...
  (rows||[]).forEach(row=>{
    const locationNames=splitBalikanLocationNames(row?.rakTujuan||row?.lokasi);
    if(!locationNames.length)return;
    const qty=toNumberSafe(row?.qty);
    locationNames.forEach(lokasi=>{
