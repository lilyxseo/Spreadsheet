...
import { logAuditTrail, getAuditTrailLogs } from "./audit-trail.js";
...
const BALIKAN_STATE={sheets:[],highlightRowNumber:null,sortBy:"default",autoCheckOnScan:true,exactScanSku:"",lastCheckedRowId:null,lastCheckedSku:"",lastCheckedVersion:0,lastCheckedFadeTimer:null};
...
if(path==="/")return showPage("dashboard");if(path==="/search")return showPage("search");if(path==="/barang-masuk")return showPage("barang-masuk");if(path==="/barang-keluar")return showPage("barang-keluar");if(path==="/accuracy-dashboard"||path==="/accuracy"||path==="/dashboard-akurasi")return showPage("stats");if(path==="/abc-analisis")return showPage("abc-analisis");if(path==="/statistics"){history.replaceState({},"","/");return showPage("dashboard");}if(path==="/locations"||path==="/location")return showPage("locations");if(path==="/settings")return showPage("settings");if(path==="/sheet-input")return showPage("sheet-input");if(path==="/arsip")return showPage("arsip");if(path==="/asset-store")return showPage("asset-store");if(path==="/cycle-count")return showPage("cycle-count");if(path==="/movement")return showPage("movement");if(path==="/balikan-store")return showPage("balikan-store");if(path==="/import-pdf-transfer")return showPage("import-pdf-transfer");if(path==="/activity-log"){if(!isDeveloperUser()){history.replaceState({},"","/");return showPage("dashboard");}return showPage("activity-log");}if(path==="/audit-trail")return showPage("audit-trail");if(path==="/anomaly"){history.replaceState({},"","/warning");return showPage("anomaly");}if(path==="/warning")return showPage("anomaly");if(path==="/stok-minus")return showPage("stok-minus");if(path.startsWith("/sku/")){currentSku=decodeURIComponent(path.split("/sku/")[1]||"");if(currentSku)showDetail(currentSku);return showPage("detail");}showPage("dashboard");}
...
if(page==="audit-trail")renderAuditTrailPage();
...
function applyBalikanTableFilters(rows,mode='balikan',omitCol=''){const st=ensureBalikanFilterState();const q=clean(window.balikanSearchKeyword||'');const exactScanSku=clean(BALIKAN_STATE.exactScanSku||'');const dynamicCols=Array.isArray(window.BALIKAN_DYNAMIC_COLUMNS)?window.BALIKAN_DYNAMIC_COLUMNS:[];return rows.filter(r=>{if(BALIKAN_FILTERABLE_COLUMNS.some(col=>{if(col===omitCol)return false;const selected=st.columnFilters[col]||[];if(!selected.length)return false;return !selected.includes(sanitizeFilterValue(r[col]));}))return false;if(exactScanSku&&clean(r.sku)!==exactScanSku)return false;const dynamicSearch=dynamicCols.map(col=>r?.[col.key]??'').join(' ');if(q&&!clean(`${r.sku} ${r.namaBarang} ${r.lokasi} ${r.status} ${r.keterangan} ${r.rakTujuan} ${dynamicSearch}`).includes(q))return false;return true;});}
...
try{const res=await fetch('/api/import-pdf-transfer',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({header:PDF_TRANSFER_STATE.header,items:validRows})});const out=await res.json();if(!res.ok||!out?.success)throw new Error(out?.message||'Gagal import');validRows.forEach(r=>logAuditTrailSafe({page:'Movement',action:'IMPORT',entity:r.sku||'',sku:r.sku||'',field:'import',oldValue:null,newValue:r,qty:r.qty,location:r.to||r.from||'',referenceId:PDF_TRANSFER_STATE.header?.transferId||''}));
...
const AUDIT_TRAIL_STATE={rows:[],page:1,pageSize:25,filters:{q:'',user:'',page:'',sku:'',action:'',date:''}};
async function renderAuditTrailPage(){const root=document.getElementById('auditTrailApp');if(!root)return;AUDIT_TRAIL_STATE.rows=await getAuditTrailLogs();const f=AUDIT_TRAIL_STATE.filters;const rows=AUDIT_TRAIL_STATE.rows.filter(r=>(!f.q||JSON.stringify(r).toLowerCase().includes(f.q.toLowerCase()))&&(!f.user||String(r.user).includes(f.user))&&(!f.page||String(r.page).includes(f.page))&&(!f.sku||String(r.sku||'').includes(f.sku))&&(!f.action||r.action===f.action)&&(!f.date||String(r.timestamp).slice(0,10)===f.date));const paged=rows.slice(0,AUDIT_TRAIL_STATE.pageSize);root.innerHTML=`<div class='card'><div class='mv-filters open'><input id='auditSearch' class='search-lg' placeholder='Search' value='${f.q||''}'><input id='auditDate' type='date' class='search-lg' value='${f.date||''}'><input id='auditUser' class='search-lg' placeholder='User' value='${f.user||''}'><input id='auditPage' class='search-lg' placeholder='Halaman' value='${f.page||''}'><input id='auditSku' class='search-lg' placeholder='SKU' value='${f.sku||''}'><select id='auditAction' class='search-lg'><option value=''>Semua Aksi</option><option>UPDATE</option><option>CREATE</option><option>DELETE</option><option>CHECKLIST</option><option>IMPORT</option></select><select id='auditPageSize' class='search-lg'><option value='25'>25</option><option value='50'>50</option></select><button id='auditExport' class='btn-ghost'>Export CSV</button></div><div class='table-wrap'><table><thead><tr><th>Waktu</th><th>User</th><th>Halaman</th><th>SKU</th><th>Field</th><th>Perubahan</th><th>Aksi</th></tr></thead><tbody>${paged.map(r=>`<tr><td>${r.readableTime}</td><td>${r.user}</td><td>${r.page}</td><td>${r.sku||''}</td><td>${r.field||''}</td><td>${String(r.oldValue??'-')} → ${String(r.newValue??'-')}</td><td>${r.action}</td></tr>`).join('')||"<tr><td colspan='7'>Belum ada log</td></tr>"}</tbody></table></div></div>`;document.getElementById('auditExport').onclick=()=>{const head=['timestamp','user','page','action','sku','field','oldValue','newValue','location','qty','referenceId'];const lines=[head.join(','),...rows.map(r=>head.map(h=>`"${String(r[h]??'').replaceAll('"','""')}"`).join(','))];const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([lines.join('\n')],{type:'text/csv'}));a.download='audit-trail.csv';a.click();};['auditSearch','auditDate','auditUser','auditPage','auditSku','auditAction','auditPageSize'].forEach(id=>{const el=document.getElementById(id);if(!el)return;el.oninput=el.onchange=()=>{AUDIT_TRAIL_STATE.filters.q=document.getElementById('auditSearch').value;AUDIT_TRAIL_STATE.filters.date=document.getElementById('auditDate').value;AUDIT_TRAIL_STATE.filters.user=document.getElementById('auditUser').value;AUDIT_TRAIL_STATE.filters.page=document.getElementById('auditPage').value;AUDIT_TRAIL_STATE.filters.sku=document.getElementById('auditSku').value;AUDIT_TRAIL_STATE.filters.action=document.getElementById('auditAction').value;AUDIT_TRAIL_STATE.pageSize=Number(document.getElementById('auditPageSize').value)||25;renderAuditTrailPage();};});}
