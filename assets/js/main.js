...
const bulkEdit=e.target.closest('[data-mv-bulk-edit]');if(bulkEdit){const mode=bulkEdit.dataset.mvBulkEdit,selectedSet=getSelectedSet(mode),rows=[...selectedSet];if(!rows.length)return;const formHtml=`<div class='mv-bulk-edit-modal'><div class='subtitle'>${rows.length} item dipilih</div><label>Status<input id='mvBulkStatus' class='search-lg' placeholder='Status'></label><label>PIC<input id='mvBulkPic' class='search-lg' placeholder='PIC'></label><label>Keterangan<input id='mvBulkKet' class='search-lg' placeholder='Keterangan'></label></div>`;showConfirmModal({title:'Bulk Edit',message:formHtml,allowHtmlMessage:true,confirmText:'Simpan Perubahan',cancelText:'Batal',onConfirm:()=>{const updates={};const status=(document.getElementById('mvBulkStatus')?.value||'').trim();const pic=(document.getElementById('mvBulkPic')?.value||'').trim();const keterangan=(document.getElementById('mvBulkKet')?.value||'').trim();if(status)updates.status=status;if(pic)updates.pic=pic;if(keterangan)updates.keterangan=keterangan;if(!Object.keys(updates).length){toast('Isi minimal 1 field untuk update','error');return;}fetch(mode==='in'?'/api/barang-masuk/bulk-update':'/api/barang-keluar/bulk-update',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({rowNumbers:rows,updates})}).then(async r=>({ok:r.ok,out:await r.json()})).then(({ok,out})=>{if(!ok||!out?.success)throw new Error(out?.message||'Gagal bulk edit');TABLE_STATE[mode].rows.forEach(r=>{if(selectedSet.has(r.rowNumber))Object.assign(r,updates);});selectedSet.clear();renderDataTablePage(mode,mode==='in'?'Barang Masuk':'Barang Keluar',true);toast('Bulk edit berhasil','success');}).catch(err=>toast(err?.message||'Gagal bulk edit','error'));}});return;}
