...
const repaired=repairTransferCsvRow(cells);
...
let qtyRaw=String(cells[rowMap.qty]||'').trim().replace(/\./g,'').replace(',', '.');
...
if((!sku||!qtyRaw||Number.isNaN(qty))&&!repaired){
const split=splitCollapsedTransferCsvLine(rowText);
if(split){
cells=split;sku=split[0];namaProduk=split[1];qtyRaw=String(split[2]||'').replace(/\./g,'').replace(',', '.');diterima=split[3];batal=split[4];tolak=split[5];catatan=split[6];qty=Number(qtyRaw);
}
}
const reasons=[];
...
const records=[];
let current=null;
const pushCurrent=()=>{if(current&&outputFields.some(field=>String(current[field]||'').trim()))records.push(current);current=null;};
...
if(!rowText)continue;
if(records.length&&/^(grand\s+total|total)\b/i.test(rowText))break;
if(/^(transfer\s*#|dari\b|kepada\b|nomor\s+referensi|tanggal\s+dibuat|perkiraan\s+tanggal|status\s+transfer)/i.test(rowText))continue;
if(isTransferHeaderLikeLine(rowText)&&/(kode\s+barang|sku|jumlah|qty)/i.test(rowText))continue;
const rowCells={};
...
const hasRowNumber=/^\d+$/.test(String(rowCells.no||'').trim());
const hasSku=String(rowCells.sku||'').trim();
const hasQty=isTransferCsvNumber(rowCells.qty);
const currentHasData=current&&outputFields.some(field=>String(current[field]||'').trim());
const currentHasSku=current&&String(current.sku||'').trim();
const startsNew=(currentHasSku&&hasSku)||(!currentHasData&&(hasRowNumber||hasSku||hasQty));
if(startsNew){pushCurrent();current={};}
if(!current)current={};
for(const field of outputFields)appendPdfCell(current,field,rowCells[field]);
}
pushCurrent();
