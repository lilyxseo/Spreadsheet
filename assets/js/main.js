...
anomalySeverity?.addEventListener("change",()=>renderAnomalyPage());anomalyType?.addEventListener("change",()=>renderAnomalyPage());anomalySearch?.addEventListener("input",debounce(()=>renderAnomalyPage(),180));
...
document.addEventListener("click",e=>{const trigger=e.target.closest("[data-col-filter-trigger]");if(trigger){e.stopPropagation();return toggleHeaderFilter(trigger.dataset.mvMode,trigger.dataset.mvCol);}if(!e.target.closest(".mv-col-filter"))document.querySelectorAll(".mv-col-filter.open").forEach(el=>el.classList.remove("open"));});
document.addEventListener("input",e=>{const i=e.target.closest("[data-col-filter-search]");if(!i)return;const mode=i.dataset.mvMode,col=i.dataset.mvCol,q=clean(i.value);const root=document.getElementById(`mv-col-filter-${mode}-${col}`);root?.querySelectorAll("[data-col-option]").forEach(el=>{el.style.display=!q||clean(el.dataset.colOption).includes(q)?"":"none";});});
document.addEventListener("change",e=>{const c=e.target.closest("[data-col-filter-option]");if(!c)return;setColumnFilter(c.dataset.mvMode,c.dataset.mvCol,decodeURIComponent(c.value||""),c.checked);});
document.addEventListener("click",e=>{const act=e.target.closest("[data-col-filter-action]");if(!act)return;setColumnFilterAll(act.dataset.mvMode,act.dataset.mvCol,act.dataset.colFilterAction==="all");});}
...
const TABLE_STATE={in:{page:1,pageSize:25,rows:[],filtered:[],columnFilters:{}},out:{page:1,pageSize:25,rows:[],filtered:[],columnFilters:{}}};
function getUniqueOptions(rows,key){return [...new Set(rows.map(r=>String(r[key]??"").trim()).filter(v=>v&&v!=="-"))].sort((a,b)=>a.localeCompare(b));}
function applyTableFilters(rows,mode){const q=clean((mode==="in"?inSearch:outSearch)?.value||"");const selectedFilters=TABLE_STATE[mode].columnFilters||{};return rows.filter(r=>{if(FILTERABLE_COLUMNS.some(col=>{const selected=selectedFilters[col]||[];return selected.length&&!selected.includes(String(r[col]??"").trim());}))return false;if(q&&!clean(`${r.sku} ${r.nama} ${r.from} ${r.to} ${r.status} ${r.pic} ${r.keterangan}`).includes(q))return false;return true;});}
function toggleHeaderFilter(mode,column){const root=document.getElementById(`mv-col-filter-${mode}-${column}`);if(!root)return;document.querySelectorAll(".mv-col-filter.open").forEach(el=>{if(el!==root)el.classList.remove("open");});root.classList.toggle("open");}
function setColumnFilter(mode,column,value,checked){const st=TABLE_STATE[mode];const options=getUniqueOptions(st.rows||[],column);const curr=new Set(st.columnFilters[column]||[]);if(!curr.size){for(const opt of options)curr.add(opt);}if(checked)curr.add(value);else curr.delete(value);st.columnFilters[column]=curr.size===options.length?[]:[...curr];st.page=1;renderDataTablePage(mode,mode==="in"?"Barang Masuk":"Barang Keluar",true);}
function setColumnFilterAll(mode,column,selectAll){const rows=TABLE_STATE[mode].rows||[];TABLE_STATE[mode].columnFilters[column]=selectAll?getUniqueOptions(rows,column):[];TABLE_STATE[mode].page=1;renderDataTablePage(mode,mode==="in"?"Barang Masuk":"Barang Keluar",true);}
...
const filterHtml=`<div class='mv-toolbar'><div id='mv-filters-${mode}' class='mv-filters'><select id='mv-sort-${mode}' data-mv-filter data-mv-mode='${mode}'><option value='latest'>Terbaru</option><option value='oldest'>Terlama</option><option value='sku'>SKU A-Z</option><option value='name'>Nama A-Z</option><option value='qtyDesc'>Qty terbesar</option><option value='qtyAsc'>Qty terkecil</option></select><select id='mv-size-${mode}' data-mv-filter data-mv-mode='${mode}'><option value='25'>25</option><option value='50'>50</option></select><button class='btn-ghost' data-mv-action='reset' data-mv-mode='${mode}'>Reset Filter</button><button class='btn-ghost' data-mv-action='export' data-mv-mode='${mode}'>Export CSV</button><button class='btn-ghost' data-mv-action='columns' data-mv-mode='${mode}'>Kolom</button></div><div id='mv-cols-${mode}' class='mv-columns'>${allCols.map(c=>`<label><input type='checkbox' ${st.columns.includes(c)?"checked":""} value='${c}' onchange='toggleColumnVisibility("${mode}")'> ${c}</label>`).join("")}</div></div>`;
const headers=st.columns.map(c=>{if(!FILTERABLE_COLUMNS.includes(c))return `<th>${esc(c.toUpperCase())}</th>`;const selected=st.columnFilters[c]||[];const options=getUniqueOptions(rows,c);return `<th><div class='th-filter-wrap'><span>${esc(c.toUpperCase())}</span><button class='th-filter-btn ${selected.length?"active":""}' type='button' data-col-filter-trigger data-mv-mode='${mode}' data-mv-col='${c}'>⏷${selected.length?`<em>${selected.length}</em>`:""}</button><div class='mv-col-filter' id='mv-col-filter-${mode}-${c}'><input type='text' placeholder='Cari...' data-col-filter-search data-mv-mode='${mode}' data-mv-col='${c}'><div class='mv-col-filter-actions'><button type='button' data-col-filter-action='all' data-mv-mode='${mode}' data-mv-col='${c}'>Pilih semua</button><button type='button' data-col-filter-action='clear' data-mv-mode='${mode}' data-mv-col='${c}'>Clear</button></div><div class='mv-col-filter-list'>${options.map(v=>`<label data-col-option='${esc(v)}'><input type='checkbox' data-col-filter-option data-mv-mode='${mode}' data-mv-col='${c}' value='${encAttr(v)}' ${(selected.length?selected.includes(v):true)?"checked":""}> ${esc(v)}</label>`).join("")}</div></div></div></th>`;}).join("");const bodyRows=[];for(const r of pageRows){bodyRows.push(`<tr>${st.columns.map(c=>`<td>${esc(r[c]??"-")}</td>`).join("")}</tr>`);}const body=bodyRows.join("");const start=st.filtered.length?((st.page-1)*st.pageSize+1):0;const end=st.filtered.length?Math.min(st.page*st.pageSize,st.filtered.length):0;
...
function resetMovementFilter(mode){if(mode==="in"){if(inSearch)inSearch.value="";}else{if(outSearch)outSearch.value="";}const root=document.getElementById(`mv-filters-${mode}`);root?.querySelectorAll('select').forEach(el=>el.value="");TABLE_STATE[mode].columnFilters={};TABLE_STATE[mode].page=1;renderDataTablePage(mode,mode==="in"?"Barang Masuk":"Barang Keluar",true);} 
