...
const ids=["searchInput","sortSearch","statsFilter","refreshToggleHeader","darkBtnHeader","openSidebar","closeSidebar","sidebarOverlay","sheetInfo","spreadsheetInfo","dashboardCards","recentMove","statsCards","statsChart","loadedState","countPerSheet","filterRow","lastSync","settingsApiState","sidebarApi","detail","locationsSummary","locSearchInput","locSkuSearchInput","locStatusFilter","locSort","locPageSize","locationsTable","locationsEmpty","locationDetail","inSearch","inSummary","inResults","outSearch","outSummary","outResults","inFiltersToggle","outFiltersToggle","anomalySummary","anomalySeverity","anomalyType","anomalySearch","anomalyTable","stokMinusSummary","stokMinusPanel","stokMinusTable","abcAnalisisApp","cycleCountApp","movementApp","settingsLastRefresh","settingsTotalRows","settingsSystemStatus","settingsSystemDot","settingsDataSources","settingsCacheStatus","settingsCacheTime","archiveApp","assetStoreApp","mainContentSkeleton","mainContentPages","sidebarToggle","balikanSheetSelect","balikanSearchInput","balikanSummary","balikanTable","btnScanBalikan","balikanSortSelect","btnResetBalikanFilter","btnExportBalikanCsv","balikanAutoCheckToggle"];
...
function routeFromPath(path){if(!user)return showLoginView();if(path==="/")return showPage("dashboard");if(path==="/search")return showPage("search");if(path==="/barang-masuk")return showPage("barang-masuk");if(path==="/barang-keluar")return showPage("barang-keluar");if(path==="/accuracy-dashboard"||path==="/accuracy"||path==="/dashboard-akurasi")return showPage("stats");if(path==="/statistics"){history.replaceState({},"","/");return showPage("dashboard");}if(path==="/locations"||path==="/location")return showPage("locations");if(path==="/settings")return showPage("settings");if(path==="/sheet-input")return showPage("sheet-input");if(path==="/arsip")return showPage("arsip");if(path==="/asset-store")return showPage("asset-store");if(path==="/cycle-count")return showPage("cycle-count");if(path==="/movement")return showPage("movement");if(path==="/balikan-store")return showPage("balikan-store");if(path==="/abc-analisis")return showPage("abc-analisis");if(path==="/activity-log"){if(!isDeveloperUser()){history.replaceState({},"","/");return showPage("dashboard");}return showPage("activity-log");}if(path==="/anomaly"){history.replaceState({},"","/warning");return showPage("anomaly");}if(path==="/warning")return showPage("anomaly");if(path==="/stok-minus")return showPage("stok-minus");if(path.startsWith("/sku/")){currentSku=decodeURIComponent(path.split("/sku/")[1]||"");if(currentSku)showDetail(currentSku);return showPage("detail");}showPage("dashboard");}
...
const ABC_STATE={periodMonths:3,excludeTo:["REJECT","RUSAK"],rows:[],updatedAt:0,error:""};
function parseDateLoose(value){if(!value)return null;const asDate=new Date(value);if(!Number.isNaN(asDate.getTime()))return asDate;const s=String(value).trim();const m=s.match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{2,4})$/);if(!m)return null;const y=Number(m[3].length===2?`20${m[3]}`:m[3]);return new Date(y,Number(m[2])-1,Number(m[1]));}
function loadAbcCache(){const parsed=safeJsonParse(localStorage.getItem(ABC_ANALYSIS_CACHE_KEY),null);if(!parsed)return;ABC_STATE.periodMonths=Number(parsed.periodMonths)||3;ABC_STATE.excludeTo=Array.isArray(parsed.excludeTo)?parsed.excludeTo:["REJECT","RUSAK"];ABC_STATE.rows=Array.isArray(parsed.rows)?parsed.rows:[];ABC_STATE.updatedAt=Number(parsed.updatedAt)||0;}
function saveAbcCache(){localStorage.setItem(ABC_ANALYSIS_CACHE_KEY,JSON.stringify({periodMonths:ABC_STATE.periodMonths,excludeTo:ABC_STATE.excludeTo,rows:ABC_STATE.rows,updatedAt:Date.now()}));}
function computeAbcAnalysis({months=3,force=false}={}){
if(!force&&ABC_STATE.rows.length&&ABC_STATE.periodMonths===months)return ABC_STATE.rows;
const rows=getBarangKeluarRows();
const end=new Date();const start=new Date(end);start.setMonth(start.getMonth()-Number(months||3));
const excluded=new Set((ABC_STATE.excludeTo||[]).map(x=>clean(x)));
const agg=new Map();let invalidRows=0;
rows.forEach(r=>{const sku=String(getVal(r,["sku","nama barang","nama","item"])||"").trim();const qty=parseNumber(getVal(r,["qty","qty keluar"]))||0;const to=String(getVal(r,["to","tujuan","tujuan pengeluaran barang","status"])||"").trim();const dt=parseDateLoose(getVal(r,["tanggal","date"]));if(!sku||!dt||qty<=0){invalidRows++;return;}if(dt<start||dt>end)return;if(excluded.has(clean(to)))return;const prev=agg.get(sku)||{sku,totalQty:0};prev.totalQty+=qty;agg.set(sku,prev);});
const list=[...agg.values()];const totalAll=list.reduce((n,r)=>n+r.totalQty,0);
if(!totalAll){ABC_STATE.rows=[];ABC_STATE.error="Tidak ada data pada periode ini";saveAbcCache();return [];}
const desc=[...list].sort((a,b)=>b.totalQty-a.totalQty);let cumulative=0;const meta=new Map();
desc.forEach(r=>{const contrib=(r.totalQty/totalAll)*100;cumulative+=contrib;const category=cumulative<=80?"A":(cumulative<=95?"B":"C");meta.set(r.sku,{contrib,cumulative,category});});
ABC_STATE.rows=list.map(r=>({sku:r.sku,totalQty:r.totalQty,contrib:meta.get(r.sku)?.contrib||0,cumulative:meta.get(r.sku)?.cumulative||0,category:meta.get(r.sku)?.category||"C"})).sort((a,b)=>a.totalQty-b.totalQty);
ABC_STATE.error=invalidRows?`Sebagian data tidak valid dilewati (${invalidRows} baris).`:"";
ABC_STATE.periodMonths=months;ABC_STATE.updatedAt=Date.now();saveAbcCache();return ABC_STATE.rows;
}
function renderAbcAnalisisPage(){
if(!abcAnalisisApp)return;loadAbcCache();const rows=computeAbcAnalysis({months:ABC_STATE.periodMonths});
const summary=rows.reduce((acc,r)=>{acc[r.category]=(acc[r.category]||0)+1;acc.contrib[r.category]=(acc.contrib[r.category]||0)+r.contrib;return acc;},{A:0,B:0,C:0,contrib:{A:0,B:0,C:0}});
const topFast=[...rows].filter(r=>r.category==="A").sort((a,b)=>b.totalQty-a.totalQty).slice(0,10);
const topSlow=[...rows].filter(r=>r.category==="C").slice(0,10);
abcAnalisisApp.innerHTML=`<div class='card'><div class='mv-toolbar'><select id='abcPeriodSelect'>${[1,2,3,4,5,6].map(m=>`<option value='${m}' ${ABC_STATE.periodMonths===m?"selected":""}>${m} bulan terakhir</option>`).join("")}</select><button class='btn-primary' onclick='refreshAbcAnalisis()'>Refresh</button><button class='btn-ghost' onclick='exportAbcCsv()'>Export CSV</button></div><div class='subtitle'>Cache: ${ABC_STATE.updatedAt?new Date(ABC_STATE.updatedAt).toLocaleString("id-ID"):"belum ada"} ${ABC_STATE.error?` • ${esc(ABC_STATE.error)}`:""}</div><div class='grid dashboard'><div class='summary-card'><div class='k'>Kategori A</div><div class='v'>${summary.A} SKU (${summary.contrib.A.toFixed(1)}%)</div></div><div class='summary-card'><div class='k'>Kategori B</div><div class='v'>${summary.B} SKU (${summary.contrib.B.toFixed(1)}%)</div></div><div class='summary-card'><div class='k'>Kategori C</div><div class='v'>${summary.C} SKU (${summary.contrib.C.toFixed(1)}%)</div></div></div><div class='table-wrap' style='margin-top:10px'><table><thead><tr><th>SKU / Nama Barang</th><th>Total Qty Keluar</th><th>% Kontribusi</th><th>Cumulative %</th><th>Kategori</th><th>Status Rekomendasi</th></tr></thead><tbody>${rows.length?rows.map(r=>{const rec=getAbcRecommendation(r);return `<tr><td>${esc(r.sku)}</td><td>${r.totalQty}</td><td>${r.contrib.toFixed(2)}%</td><td>${r.cumulative.toFixed(2)}%</td><td><span class='badge ${r.category==="A"?"b-out":(r.category==="B"?"b-warn":"b-kartu")}'>${r.category}</span></td><td>${esc(rec)}</td></tr>`;}).join(""):"<tr><td colspan='6'><div class='state'>Tidak ada data pada periode ini</div></td></tr>"}</tbody></table></div><div class='grid dashboard' style='margin-top:10px'><div class='card'><h4>Top 10 Fast Moving</h4><ol>${topFast.map(x=>`<li>${esc(x.sku)} (${x.totalQty})</li>`).join("")||"<li>-</li>"}</ol></div><div class='card'><h4>Top 10 Slow Moving</h4><ol>${topSlow.map(x=>`<li>${esc(x.sku)} (${x.totalQty})</li>`).join("")||"<li>-</li>"}</ol></div></div></div>`;
document.getElementById("abcPeriodSelect")?.addEventListener("change",e=>{ABC_STATE.periodMonths=Number(e.target.value)||3;computeAbcAnalysis({months:ABC_STATE.periodMonths,force:true});renderAbcAnalisisPage();});
}
function getInventorySnapshotBySku(sku){const key=clean(sku);for(const sheet of INVENTORY_PRELOAD_SHEETS){for(const r of (DATA[sheet]||[])){if(clean(getVal(r,["sku"]))!==key)continue;return {stok:parseNumber(getVal(r,["stok akhir","qty","stok"]))||0,min:parseNumber(getVal(r,["minimum stok","min stok","rop"]))||0};}}return {stok:0,min:0};}
function getAbcRecommendation(row){const inv=getInventorySnapshotBySku(row.sku);if(row.category==="A"&&inv.stok<inv.min)return"RESTOCK PRIORITAS";if(row.category==="A")return"AMAN";if(row.category==="C"&&inv.stok>inv.min&&inv.min>0)return"OVERSTOCK";if(row.category==="C")return"LOW PRIORITY";return"NORMAL";}
window.refreshAbcAnalisis=()=>{computeAbcAnalysis({months:ABC_STATE.periodMonths,force:true});renderAbcAnalisisPage();};
window.exportAbcCsv=()=>{const cols=["sku","total_qty_keluar","persen_kontribusi","cumulative_persen","kategori","status_rekomendasi"];const lines=[cols.join(",")].concat((ABC_STATE.rows||[]).map(r=>[r.sku,r.totalQty,r.contrib.toFixed(4),r.cumulative.toFixed(4),r.category,getAbcRecommendation(r)].map(v=>`"${String(v??"").replaceAll('"','""')}"`).join(",")));const blob=new Blob([lines.join("\n")],{type:"text/csv;charset=utf-8;"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`abc-analisis-${ABC_STATE.periodMonths}bulan.csv`;a.click();URL.revokeObjectURL(a.href);};
