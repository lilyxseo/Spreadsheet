...
const selectedSet=new Set(ABC_STATE.selectedTo||[]);
...
filtered.forEach(r=>{const sku=String(r.sku||"").trim();if(!sku)return;const qty=toNum(r.qty);if(qty<=0)return;const obj=bySku.get(sku)||{sku,namaBarang:String(r.namaBarang||sku),qtyKeluar:0};obj.qtyKeluar+=qty;bySku.set(sku,obj);total+=qty;});
const stockMap=new Map();stokRows.forEach(r=>{const sku=String(getVal(r,["sku","kode sku","item code"])||r?.sku||"").trim();if(!sku)return;const qty=parseNumber(getVal(r,["qty","stok akhir","closing stock","ending stock","saldo akhir","stok","quantity"]));if(!Number.isFinite(qty))return;stockMap.set(sku,(stockMap.get(sku)||0)+qty);});
const freqMap=new Map();masuk.forEach(r=>{const sku=String(r.sku||"").trim();if(!sku)return;freqMap.set(sku,(freqMap.get(sku)||0)+1);});
...
if(!data.rows.length&&data.filteredCount===0){abcAnalisisApp.innerHTML="<div class='card'><div class='state'>Tidak ada data yang sesuai dengan filter</div></div>";return;}
...
const toFiltered=(data.toOptions||[]).filter(v=>clean(v).includes(clean(ABC_STATE.toSearch||"")));
...
const selectedCount=(ABC_STATE.selectedTo||[]).length;
const allSelected=selectedCount>0&&selectedCount===data.toOptions.length;
const selectedLabel=allSelected||selectedCount===0?"Semua TO":`${selectedCount} TO dipilih`;
abcAnalisisApp.innerHTML=`<div class='card' style='overflow:visible'><div class='mv-filters open' style='display:flex;flex-wrap:wrap;gap:8px;align-items:center;overflow:visible'><select id='abcPeriod' class='search-lg' style='max-width:170px;height:34px;padding:5px 14px!important'>${periodOpt}</select><select id='abcOrderType' class='search-lg' style='max-width:160px;height:34px;padding:5px 14px!important'><option ${ABC_STATE.orderType==="Semua"?"selected":""}>Semua</option><option ${ABC_STATE.orderType==="Orderan GT"?"selected":""}>Orderan GT</option><option ${ABC_STATE.orderType==="Orderan Store"?"selected":""}>Orderan Store</option></select><select id='abcSortBy' class='search-lg' style='max-width:190px;height:34px;padding:5px 14px!important'>${sortOpt}</select><div id='abcToWrap' style='position:relative;min-width:180px;max-width:230px'><button id='abcToToggle' class='search-lg' type='button' style='height:34px;min-width:180px;max-width:230px;text-align:center;white-space:nowrap;overflow:hidden;text-overflow:ellipsis'>${esc(selectedLabel)}</button><div id='abcToDropdown' ${ABC_STATE.toDropdownOpen?"":"hidden"} style='position:absolute;z-index:30;top:38px;left:0;width:265px;background:#071636;border:1px solid #2a3f6e;border-radius:12px;padding:8px;max-height:310px;overflow:auto;box-shadow:0 10px 25px rgba(0,0,0,.45)'><input id='abcToSearch' class='search-lg' placeholder='Cari...' value='${encAttr(ABC_STATE.toSearch||"")}' style='margin-bottom:8px;height:34px'><div class='row' style='justify-content:space-between;gap:6px;margin-bottom:6px'><button type='button' id='abcToSelectAll' style='background:transparent;border:none;color:#ff4d5a;font-weight:700;cursor:pointer;padding:0'>Pilih semua</button><button type='button' id='abcToClear' style='background:transparent;border:none;color:#ff4d5a;font-weight:700;cursor:pointer;padding:0'>Hapus</button></div><div style='border-top:1px solid #1f3765;padding-top:6px'>${toFiltered.map(v=>`<label style='display:flex;gap:8px;align-items:center;padding:4px 0;font-size:12px;color:#ff4d5a;font-weight:700'><input type='checkbox' data-abc-to='${encAttr(v)}' ${(ABC_STATE.selectedTo||[]).includes(v)?"checked":""} style='width:16px;height:16px;appearance:auto;-webkit-appearance:checkbox;cursor:pointer'/> <span>${esc(v)}</span></label>`).join("")||"<div class='subtitle'>Tidak ada tujuan</div>"}</div></div></div><button id='abcResetBtn' class='btn-ghost abc-action-btn' style='height:34px'>Reset</button><button id='abcCopyBtn' class='btn-ghost abc-action-btn' style='height:34px'>Copy</button><button id='abcRefreshBtn' class='btn-primary abc-action-btn' style='height:34px'>Refresh</button></div><div class='table-wrap' style='margin-top:14px;max-height:calc(100vh - 280px);overflow:auto'><table><thead><tr><th><input type='checkbox' id='abcSelectAll' class='abc-check-input'></th><th>SKU</th><th>Nama Barang</th><th>Total Qty Keluar</th><th>% Kontribusi</th><th>Cumulative %</th><th>Kategori ABC</th><th>Stok Saat Ini</th><th>Priority Order</th><th>Status Rekomendasi</th></tr></thead><tbody>${data.rows.map(r=>`<tr><td><input type='checkbox' data-abc-row='${encAttr(r.sku)}' ${ABC_STATE.selectedRows.has(r.sku)?"checked":""} class='abc-check-input'></td><td><div class='abc-sku-cell'><span>${esc(r.sku)}</span><button type='button' class='abc-mini-icon-btn' data-abc-copy-sku='${encAttr(r.sku)}' aria-label='Copy SKU ${encAttr(r.sku)}' title='Copy SKU'><i data-lucide='copy'></i></button></div></td><td><div class='abc-name-cell'><span>${esc(r.namaBarang)}</span><button type='button' class='abc-mini-icon-btn abc-detail-link-btn' data-abc-sku='${encAttr(r.sku)}' aria-label='Buka detail SKU ${encAttr(r.sku)}' title='Buka detail SKU'><i data-lucide='arrow-up-right'></i></button></div></td><td>${r.qtyKeluar}</td><td>${(r.kontribusi*100).toFixed(2)}%</td><td>${(r.cum*100).toFixed(2)}%</td><td><span class='badge ${r.abc==="A"?"b-out":r.abc==="B"?"b-warn":"b-idle"}'>${r.abc}</span></td><td>${r.stokSaatIni}</td><td>${esc(r.priority)}</td><td>${esc(r.rekom)}</td></tr>`).join("")}</tbody></table></div></div><div class='card' style='margin-top:8px;padding:8px 10px'><div class='section-header' style='margin-bottom:4px'><h4 style='margin:0;font-size:13px'>Top 10 Fast Moving Items</h4></div><ol style='margin:0;padding-left:16px;display:block'>${data.top10.map(r=>`<li style='margin:0 0 2px 0'><button class='btn-link' data-abc-sku='${encAttr(r.sku)}' style='font-size:12px;line-height:1.2;padding:0'>${esc(r.namaBarang)}</button></li>`).join("")}</ol></div>`;
document.getElementById("abcPeriod").onchange=e=>{ABC_STATE.periodMonths=Number(e.target.value)||3;renderAbcAnalisisPage(true);};
document.getElementById("abcOrderType").onchange=e=>{ABC_STATE.orderType=e.target.value;renderAbcAnalisisPage(true);};
document.getElementById("abcSortBy").onchange=e=>{ABC_STATE.sortBy=e.target.value||"Prioritas Order";renderAbcAnalisisPage(true);};
document.getElementById("abcToToggle").onclick=()=>{ABC_STATE.toDropdownOpen=!ABC_STATE.toDropdownOpen;renderAbcAnalisisPage(false);};
if(document.getElementById("abcToSearch"))document.getElementById("abcToSearch").oninput=e=>{ABC_STATE.toSearch=e.target.value||"";renderAbcAnalisisPage(false);};
if(document.getElementById("abcToSelectAll"))document.getElementById("abcToSelectAll").onclick=()=>{ABC_STATE.selectedTo=[...data.toOptions];renderAbcAnalisisPage(true);};
if(document.getElementById("abcToClear"))document.getElementById("abcToClear").onclick=()=>{ABC_STATE.selectedTo=[];renderAbcAnalisisPage(true);};
abcAnalisisApp.querySelectorAll("[data-abc-to]").forEach(chk=>chk.onchange=e=>{const v=e.target.dataset.abcTo||"";const set=new Set(ABC_STATE.selectedTo||[]);if(e.target.checked)set.add(v);else set.delete(v);ABC_STATE.selectedTo=[...set];renderAbcAnalisisPage(true);});
...
abcAnalisisApp.querySelectorAll("[data-abc-copy-sku]").forEach(btn=>btn.onclick=()=>{const sku=btn.dataset.abcCopySku||"";if(!sku)return;navigator.clipboard.writeText(sku).then(()=>toast("SKU disalin","success")).catch(()=>toast("Gagal copy SKU","error"));});
