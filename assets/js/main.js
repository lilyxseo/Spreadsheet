...
const raw=Number(devAutoRefreshInterval?.value)||30;
const seconds=Math.max(5,raw);
if(devAutoRefreshInterval&&String(seconds)!==String(raw))devAutoRefreshInterval.value=String(seconds);
return seconds*1000;
}
...
if(!devAutoRefreshStatus)return;
...
devAutoRefreshStatus.textContent=allowed?status:"Restricted";
devAutoRefreshLastTime.textContent=formatDevAutoRefreshTime(devAutoRefreshLastTs);
devAutoRefreshCountValue.textContent=String(devAutoRefreshCount);
devAutoRefreshLastError.textContent=devAutoRefreshLastError||"-";
if(devAutoRefreshStart)devAutoRefreshStart.disabled=!allowed||isDevAutoRefreshRunning;
if(devAutoRefreshStop)devAutoRefreshStop.disabled=!allowed||(!isDevAutoRefreshRunning&&!devAutoRefreshTimer&&!isDevAutoRefreshingNow);
if(devAutoRefreshInterval)devAutoRefreshInterval.disabled=!allowed||isDevAutoRefreshRunning;
...
devAutoRefreshStart?.addEventListener('click',startDevAutoRefresh);
devAutoRefreshStop?.addEventListener('click',()=>stopDevAutoRefresh());
devAutoRefreshInterval?.addEventListener('input',renderToolsDevPage);
renderToolsDevPage();
}
