...
const MOVEMENT_HISTORY_SHEET_NAME="Movement";
const MOVEMENT_HISTORY_SPREADSHEET_ID=SPREADSHEET_ID;
const MOVEMENT_HISTORY_RANGE=`${MOVEMENT_HISTORY_SHEET_NAME}!A:Z`;
...
function parseMovementHistoryRows(rawValues){const values=Array.isArray(rawValues)?rawValues:[];if(!values.length)return {rows:[],error:"data kosong dari API"};const headerRow=0;const idx=mapMovementHeaderIndex(values[headerRow]||[]);const required=["tanggal","from","to","sku"];const miss=required.filter(k=>idx[k]<0);if(miss.length)return {rows:[],error:`sheet tidak ditemukan / header tidak valid (${miss.join(", ")})`};const parsedRows=parseMovementRows(values,idx,headerRow+1);if(!parsedRows.length)return {rows:[],error:"parser menghasilkan 0 row"};return {rows:parsedRows,error:""};}
async function fetchMovementHistoryRemote(){
const sheetName=MOVEMENT_HISTORY_SHEET_NAME;
const spreadsheetId=MOVEMENT_HISTORY_SPREADSHEET_ID;
console.log("MOVEMENT SPREADSHEET ID:", spreadsheetId);
console.log("MOVEMENT SHEET:", sheetName);
const url=`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${encodeURIComponent(MOVEMENT_HISTORY_RANGE)}?key=${API_KEY}&t=${Date.now()}`;
const res=await fetch(url,{cache:"no-store"});
const json=await res.json();
if(!res.ok||json.error){const msg=String((json.error&&json.error.message)||res.statusText||"fetch gagal");if(msg.toLowerCase().includes("unable to parse range"))throw new Error("sheet tidak ditemukan");throw new Error(`fetch gagal: ${msg}`);}
const values=Array.isArray(json.values)?json.values:[];
console.log("RAW MOVEMENT VALUES:", values);
console.log("MOVEMENT RAW ROWS:", values.length);
console.log("MOVEMENT HEADER:", values[0]);
console.log("MOVEMENT FIRST DATA:", values[1]);
if(!values.length)throw new Error("data kosong dari API (cek spreadsheet ID, sheet name, permission sheet/API)");
const parsed=parseMovementHistoryRows(values);
return parsed;
}
async function ensureMovementHistoryLoaded(){if(MOVEMENT_HISTORY_REMOTE.loaded)return;try{const parsed=await fetchMovementHistoryRemote();MOVEMENT_HISTORY_REMOTE={rows:parsed.rows,error:parsed.error||"",loaded:true};}catch(err){console.error("Gagal fetch Movement history",err);MOVEMENT_HISTORY_REMOTE={rows:[],error:`Gagal fetch Movement: ${err.message||"unknown error"}`,loaded:true};}}
