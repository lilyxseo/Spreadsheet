...
const mvCell=e.target.closest('[data-mv-cell]');if(mvCell){const mode=mvCell.dataset.mode,row=Number(mvCell.dataset.row),field=mvCell.dataset.field;const moduleName=mode==='in'?'barang-masuk':'barang-keluar';const item=TABLE_STATE[mode].rows.find(r=>r.rowNumber===row);if(item)startInlineEdit(mvCell,item,field,item[field],{onSave:async({value,td})=>{const success=await handleInlineEdit(moduleName,row,field,value,td);if(success)logActivitySafe({action:mode==='in'?'EDIT_BARANG_MASUK':'EDIT_BARANG_KELUAR',module:mode==='in'?'Barang Masuk':'Barang Keluar',detail:`Edit ${field} row ${row}`,status:'SUCCESS'});}});return;}
...
async function patchInlineCell(moduleName,rowNumber,field,newValue){const endpoint=moduleName==='barang-masuk'?'/api/barang-masuk/bulk-update':'/api/barang-keluar/bulk-update';const res=await fetch(endpoint,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({rowNumbers:[Number(rowNumber)],updates:{[field]:newValue}})});const data=await res.json().catch(()=>({}));return{success:Boolean(res.ok&&data?.success),message:data?.message||data?.error||'Gagal update data',data};}
function setCellLoading(cellEl,isLoading){if(!cellEl)return;cellEl.classList.toggle('is-saving',Boolean(isLoading));}
function updateLocalRow(moduleName,rowNumber,field,value){const mode=moduleName==='barang-masuk'?'in':'out';const rows=TABLE_STATE?.[mode]?.rows||[];const row=rows.find(r=>Number(r.rowNumber)===Number(rowNumber));if(row)row[field]=value;const sheetName=moduleName==='barang-masuk'?'Barang Masuk':'Barang Keluar';const dataRows=DATA?.[sheetName]||[];const dataRow=dataRows.find(r=>Number(r.rowNumber)===Number(rowNumber));if(dataRow)dataRow[field]=value;}
function updateCacheRow(moduleName,rowNumber,field,value){const cacheKey=moduleName==='barang-masuk'?MODULE_CACHE_KEYS.barangMasuk:MODULE_CACHE_KEYS.barangKeluar;const rows=getCache(cacheKey)||[];const row=rows.find(r=>Number(r.rowNumber)===Number(rowNumber));if(row){row[field]=value;setCacheSafe(cacheKey,rows);setModuleCache(cacheKey,rows);}if(DATA)saveCache(DATA);}
async function handleInlineEdit(moduleName,rowNumber,field,newValue,cellEl){const oldValue=cellEl?.dataset?.oldValue??"";if(String(oldValue)===String(newValue))return;setCellLoading(cellEl,true);try{const res=await patchInlineCell(moduleName,rowNumber,field,newValue);if(!res.success)throw new Error(res.message);updateLocalRow(moduleName,rowNumber,field,newValue);updateCacheRow(moduleName,rowNumber,field,newValue);if(cellEl){cellEl.textContent=newValue;cellEl.dataset.oldValue=String(newValue);}showToast('Data berhasil diperbarui','success');return true;}catch(err){if(cellEl)cellEl.textContent=oldValue||"-";showToast('Gagal memperbarui data','error');return false;}finally{setCellLoading(cellEl,false);}}
...
function startInlineEdit(td,row,field,oldValue,options={}){if(td.querySelector("input,select"))return;const normalizedOldValue=String(oldValue??"");td.dataset.oldValue=normalizedOldValue;const input=document.createElement("input");input.value=normalizedOldValue;input.className="inline-edit-input";td.classList.add("editing");td.innerHTML="";td.appendChild(input);input.focus();input.select();let cancelled=false;async function save(){if(cancelled)return;const newValue=input.value.trim();if(newValue===normalizedOldValue){td.classList.remove("editing");td.textContent=normalizedOldValue||"-";return;}try{await options.onSave?.({row,field,value:newValue,oldValue:normalizedOldValue,td});}catch(err){toast(err?.message||"Gagal update data","error");}finally{td.classList.remove("editing");}}input.addEventListener("blur",save);input.addEventListener("keydown",(e)=>{if(e.key==="Enter")input.blur();if(e.key==="Escape"){cancelled=true;td.classList.remove("editing");td.textContent=normalizedOldValue||"-";}});}
