...
const BALIKAN_STATE={sheets:[],sheetName:"",rows:[]};
...
btnScanBalikan?.addEventListener("click",()=>openBarcodeScanner("searchInput",handleBalikanScanResult));
balikanSheetSelect?.addEventListener("change",async(e)=>{BALIKAN_STATE.sheetName=e.target.value||"";await loadBalikanRows();});
...
function renderBalikanTable(){const rows=BALIKAN_STATE.rows||[];if(!rows.length){balikanTable.innerHTML='<div class="subtitle">Data kosong.</div>';return;}balikanTable.innerHTML=`<div class="table-wrap table-wrap-full balikan-table-wrapper"><table class="balikan-table"><thead><tr><th class="col-check">Centang</th><th>No</th><th>SKU</th><th class="col-name">Nama Barang</th><th>Qty</th><th>Rak tujuan</th><th>Lokasi</th><th>Stok Bulky</th><th>Stok Retail</th><th>Status</th><th>Keterangan</th></tr></thead><tbody>${rows.map(r=>`<tr><td class="col-check"><input type="checkbox" data-balikan-check="1" data-row="${r.rowNumber}" ${r.checked?'checked':''}></td><td>${esc(r.no)}</td><td>${esc(r.sku)}</td><td class="col-name">${esc(r.namaBarang)}</td><td>${esc(r.qty)}</td><td>${esc(r.rakTujuan)}</td><td>${esc(r.lokasi)}</td><td>${esc(r.stokBulky)}</td><td>${esc(r.stokRetail)}</td><td>${esc(r.status)}</td><td>${esc(r.keterangan)}</td></tr>`).join('')}</tbody></table></div>`;}

async function updateBalikanCheck(rowNumber,checked){const row=(BALIKAN_STATE.rows||[]).find(r=>r.rowNumber===rowNumber);const res=await fetch('/api/balikan-store/check',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({sheetName:BALIKAN_STATE.sheetName,rowNumber,checked})});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal update centang');toast('Centang berhasil diupdate','success');logActivitySafe({action:'CHECK_BALIKAN_STORE',module:'Balikan Store',detail:`Centang SKU ${row?.sku||'-'} di ${BALIKAN_STATE.sheetName}`,status:'SUCCESS',metadata:{sheetName:BALIKAN_STATE.sheetName,sku:row?.sku||'',checked,rowNumber}});await loadBalikanRows();}

function handleBalikanScanResult(raw){const sku=String(raw||'').trim();if(navigator?.vibrate)navigator.vibrate(150);const hit=(BALIKAN_STATE.rows||[]).find(r=>clean(r.sku)===clean(sku));if(!hit){toast('SKU tidak ditemukan di Balikan Store','error');return;}updateBalikanCheck(hit.rowNumber,true).then(()=>toast(`SKU ${sku} berhasil dicentang`,`success`)).catch(err=>toast(err?.message||'Gagal centang SKU','error'));}
