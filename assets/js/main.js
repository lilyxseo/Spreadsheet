...
const BALIKAN_EDITABLE_FIELDS=["qty","rakTujuan","lokasi","stokBulky","stokRetail","status","keterangan"];
function createEditableCell(row,field,value){const td=document.createElement("td");td.className="editable-cell";const displayValue=String(value??"").trim();td.textContent=displayValue||"-";td.addEventListener("click",()=>{startInlineEdit(td,row,field);});return td;}
function setBalikanRowValue(row,field,value){if(!row||!field)return;row[field]=value;}
async function startInlineEdit(td,row,field){if(td.classList.contains("editing"))return;const oldValue=String(row?.[field]??"");td.classList.add("editing");const input=document.createElement("input");input.type="text";input.value=oldValue;input.className="inline-edit-input";td.replaceChildren(input);input.focus();input.select();let finished=false;const cancel=()=>{if(finished)return;finished=true;td.classList.remove("editing");td.textContent=oldValue||"-";};const save=async()=>{if(finished)return;finished=true;const newValue=String(input.value??"").trim();td.classList.remove("editing");if(newValue===oldValue){td.textContent=oldValue||"-";return;}try{await updateBalikanCell(window.currentTripSheet,row.rowNumber,field,newValue);setBalikanRowValue(row,field,newValue);td.textContent=newValue||"-";}catch(err){td.textContent=oldValue||"-";showToast(err?.message||"Gagal update","error");}};input.addEventListener("blur",save);input.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();input.blur();}if(e.key==="Escape"){e.preventDefault();cancel();}});}
function renderBalikanTable(){const rows=filterBalikanRows(window.BALIKAN_ROWS||[],window.balikanSearchKeyword);if(!rows.length){balikanTable.innerHTML='<div class="subtitle">Data kosong.</div>';return;}balikanTable.innerHTML=`<div class="balikan-table-wrapper"><table class="balikan-table"><thead><tr><th class="col-check">Centang</th><th>No</th><th>SKU</th><th>Nama Barang</th><th>Qty</th><th>Rak tujuan</th><th>Lokasi</th><th>Stok Bulky</th><th>Stok Retail</th><th>Status</th><th>Keterangan</th></tr></thead><tbody></tbody></table></div>`;const tbody=balikanTable.querySelector("tbody");if(!tbody)return;const frag=document.createDocumentFragment();rows.forEach(row=>{const tr=document.createElement("tr");const rowNumber=Number(row.rowNumber);tr.id=`balikan-row-${rowNumber}`;tr.dataset.rowNumber=String(rowNumber);if(BALIKAN_STATE.highlightRowNumber===rowNumber)tr.className="balikan-row-highlight";const tdCheck=document.createElement("td");tdCheck.className="col-check";const checkbox=document.createElement("input");checkbox.type="checkbox";checkbox.className="balikan-check";checkbox.checked=isCheckedValue(row.checked);checkbox.addEventListener("change",()=>{toggleBalikanCheck(window.currentTripSheet,row.rowNumber,checkbox.checked);});tdCheck.appendChild(checkbox);tr.appendChild(tdCheck);const cols=[{field:"no",value:row.no},{field:"sku",value:row.sku},{field:"namaBarang",value:row.namaBarang},{field:"qty",value:row.qty},{field:"rakTujuan",value:row.rakTujuan},{field:"lokasi",value:row.lokasi},{field:"stokBulky",value:row.stokBulky},{field:"stokRetail",value:row.stokRetail},{field:"status",value:row.status},{field:"keterangan",value:row.keterangan}];cols.forEach(col=>{if(BALIKAN_EDITABLE_FIELDS.includes(col.field)){tr.appendChild(createEditableCell(row,col.field,col.value));return;}const td=document.createElement("td");td.textContent=String(col.value??"");tr.appendChild(td);});frag.appendChild(tr);});tbody.replaceChildren(frag);}


async function updateBalikanCell(sheetName,rowNumber,field,value){const res=await fetch('/api/balikan-store/cell',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({sheetName,rowNumber,field,value})});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal update data');showToast('Berhasil diupdate','success');await loadBalikanRows();}

async function updateBalikanCheck(sheetName,rowNumber,checked,{refresh=true}={}){const row=(window.BALIKAN_ROWS||[]).find(r=>Number(r.rowNumber)===Number(rowNumber));const targetSheet=sheetName||window.currentTripSheet;const res=await fetch('/api/balikan-store/check',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({sheetName:targetSheet,rowNumber,checked})});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal update centang');if(row)row.checked=checked;toast('Centang berhasil diupdate','success');logActivitySafe({action:'CHECK_BALIKAN_STORE',module:'Balikan Store',detail:`Centang SKU ${row?.sku||'-'} di ${targetSheet}`,status:'SUCCESS',metadata:{sheetName:targetSheet,sku:row?.sku||'',checked,rowNumber}});if(refresh)await loadBalikanRows();}
