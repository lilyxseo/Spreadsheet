...
function isCheckedValue(value) {
  ...
  return value === true || v === "true" || v === "1" || v === "yes" || v === "ya";
}
...
function renderBalikanTable(){const rows=filterBalikanRows(window.BALIKAN_ROWS||[],window.balikanSearchKeyword);if(!rows.length){balikanTable.innerHTML='<div class="subtitle">Data kosong.</div>';return;}balikanTable.innerHTML=`<div class="balikan-table-wrapper"><table class="balikan-table"><thead><tr><th class="col-check">Centang</th><th>No</th><th>SKU</th><th>Nama Barang</th><th>Qty</th><th>Rak tujuan</th><th>Lokasi</th><th>Stok Bulky</th><th>Stok Retail</th><th>Status</th><th>Keterangan</th></tr></thead><tbody>${rows.map(r=>`<tr id="balikan-row-${Number(r.rowNumber)}" data-row-number="${Number(r.rowNumber)}" class="${BALIKAN_STATE.highlightRowNumber===Number(r.rowNumber)?'balikan-row-highlight':''}"><td class="col-check"><input type="checkbox" ${isCheckedValue(r.checked) ? "checked" : ""} onchange="toggleBalikanCheck(window.currentTripSheet, ${Number(r.rowNumber)}, this.checked)"></td><td>${esc(r.no)}</td><td>${esc(r.sku)}</td><td>${esc(r.namaBarang)}</td><td>${esc(r.qty)}</td><td>${esc(r.rakTujuan)}</td><td>${esc(r.lokasi)}</td><td>${esc(r.stokBulky)}</td><td>${esc(r.stokRetail)}</td><td>${esc(r.status)}</td><td>${esc(r.keterangan)}</td></tr>`).join('')}</tbody></table></div>`;}
