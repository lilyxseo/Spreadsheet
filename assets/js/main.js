...
abcAnalisisApp.innerHTML=`<div class='card' style='overflow:visible'><div class='mv-filters open' style='display:flex;flex-wrap:wrap;gap:8px;align-items:center;overflow:visible'><select id='abcPeriod' class='search-lg' style='max-width:170px;height:34px;padding:5px 14px!important'>${periodOpt}</select><select id='abcOrderType' class='search-lg' style='max-width:160px;height:34px;padding:5px 14px!important'><option ${ABC_STATE.orderType==="Semua"?"selected":""}>Semua</option><option ${ABC_STATE.orderType==="Orderan GT"?"selected":""}>Orderan GT</option><option ${ABC_STATE.orderType==="Orderan Store"?"selected":""}>Orderan Store</option></select><div id='abcToWrap' style='position:relative;min-width:220px'><button id='abcToToggle' class='btn-ghost abc-action-btn' style='height:34px;min-width:220px;text-align:left;display:flex;justify-content:space-between;align-items:center'><span>${esc(toSummary)}</span><span>▾</span></button><div id='abcToMenu' style='position:absolute;left:0;top:38px;background:var(--card,#fff);border:1px solid var(--line,#d4d4d8);border-radius:10px;padding:8px;min-width:260px;z-index:25;box-shadow:0 8px 24px rgba(0,0,0,.15);${ABC_STATE.toDropdownOpen?'':'display:none'}'><input id='abcToSearch' class='search-lg' placeholder='Cari tujuan...' value='${esc(ABC_STATE.toSearch||"")}' style='width:100%;height:32px;margin-bottom:6px'><div style='display:flex;gap:6px;margin-bottom:6px'><button type='button' id='abcToSelectAll' class='btn-ghost abc-action-btn' style='height:30px;font-size:12px'>Pilih semua</button><button type='button' id='abcToClear' class='btn-ghost abc-action-btn' style='height:30px;font-size:12px'>Hapus</button></div><div style='max-height:230px;overflow:auto'>${toOptionFiltered.map(v=>`<label style='display:flex;gap:6px;align-items:center;padding:3px 2px'><input type='checkbox' data-abc-to='${encAttr(v)}' ${selectedToSet.has(v)?'checked':''}><span>${esc(v)}</span></label>`).join('')||"<div class='subtitle'>Tidak ada opsi</div>"}</div></div></div><select id='abcSortBy' class='search-lg' style='max-width:190px;height:34px;padding:5px 14px!important'>${sortOpt}</select><button id='abcResetBtn' class='btn-ghost abc-action-btn' style='height:34px'>Reset</button><button id='abcCopyBtn' class='btn-ghost abc-action-btn' style='height:34px'>Copy</button><button id='abcRefreshBtn' class='btn-primary abc-action-btn' style='height:34px'>Refresh</button></div><div class='table-wrap' style='margin-top:14px;max-height:calc(100vh - 280px);overflow:auto'><table><thead><tr><th><input type='checkbox' id='abcSelectAll' class='balikan-check'></th><th>SKU</th><th>Nama Barang</th><th>Total Qty Keluar</th><th>% Kontribusi</th><th>Cumulative %</th><th>Kategori ABC</th><th>Stok Saat Ini</th><th>Priority Order</th><th>Status Rekomendasi</th></tr></thead><tbody>${data.rows.map(r=>`<tr><td><input type='checkbox' class='balikan-check' data-abc-row='${encAttr(r.sku)}' ${ABC_STATE.selectedRows.has(r.sku)?"checked":""}></td><td>${esc(r.sku)}</td><td><div style='display:flex;align-items:center;gap:6px;justify-content:space-between'><span>${esc(r.namaBarang)}</span><button class='btn-ghost' data-abc-sku='${encAttr(r.sku)}' style='padding:2px 6px;font-size:11px;line-height:1.1;border-radius:8px'>Detail</button></div></td><td>${r.qtyKeluar}</td><td>${(r.kontribusi*100).toFixed(2)}%</td><td>${(r.cum*100).toFixed(2)}%</td><td><span class='badge ${r.abc==="A"?"b-out":r.abc==="B"?"b-warn":"b-idle"}'>${r.abc}</span></td><td>${r.stokSaatIni}</td><td>${esc(r.priority)}</td><td>${esc(r.rekom)}</td></tr>`).join("")}</tbody></table></div></div><div class='card' style='margin-top:8px;padding:8px 10px'><div class='section-header' style='margin-bottom:4px'><h4 style='margin:0;font-size:13px'>Top 10 Fast Moving Items</h4></div><ol style='margin:0;padding-left:16px;display:block'>${data.top10.map(r=>`<li style='margin:0 0 2px 0'><button class='btn-link' data-abc-sku='${encAttr(r.sku)}' style='font-size:12px;line-height:1.2;padding:0'>${esc(r.namaBarang)}</button></li>`).join("")}</ol></div>`;
...
const toToggle=document.getElementById("abcToToggle");if(toToggle)toToggle.onclick=()=>{ABC_STATE.toDropdownOpen=!ABC_STATE.toDropdownOpen;renderAbcAnalisisPage(false);};
const toSearchEl=document.getElementById("abcToSearch");if(toSearchEl)toSearchEl.oninput=e=>{ABC_STATE.toSearch=e.target.value||"";renderAbcAnalisisPage(false);};
const toSelectAll=document.getElementById("abcToSelectAll");if(toSelectAll)toSelectAll.onclick=()=>{ABC_STATE.selectedTo=[...(data.toOptions||[])];ABC_STATE.toDropdownOpen=true;renderAbcAnalisisPage(true);};
const toClear=document.getElementById("abcToClear");if(toClear)toClear.onclick=()=>{ABC_STATE.selectedTo=[];ABC_STATE.toDropdownOpen=true;renderAbcAnalisisPage(true);};
abcAnalisisApp.querySelectorAll("[data-abc-to]").forEach(chk=>chk.onchange=e=>{const toVal=e.target.dataset.abcTo||"";const set=new Set(ABC_STATE.selectedTo||[]);if(e.target.checked)set.add(toVal);else set.delete(toVal);ABC_STATE.selectedTo=[...set];ABC_STATE.toDropdownOpen=true;renderAbcAnalisisPage(true);});
