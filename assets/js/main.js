...
const PDF_TRANSFER_STATE={header:{},items:[],warnings:[],debugRows:[],rawText:"",csvText:"",csvRows:[],detectedColumns:[],failedRows:[],logs:[],isParsing:false,isImporting:false,isLoadingSheets:false,sheetsLoaded:false,sheets:[],selectedSheetName:"",sheetError:"",lastFileName:"",selectedFile:null};
...
const SETTINGS_CONFIG_KEY="wms_settings_config";
const SETTINGS_SAVED_AT_KEY="wms_settings_saved_at";
const SETTINGS_BACKUP_AT_KEY="wms_settings_backup_at";
const SETTINGS_DEFAULTS={refreshMinutes:5};
let settingsDraft={};
function readSettingsConfig(){try{return {...SETTINGS_DEFAULTS,...JSON.parse(localStorage.getItem(SETTINGS_CONFIG_KEY)||"{}")};}catch(_){return {...SETTINGS_DEFAULTS};}}
function relativeSettingsTime(ts){if(!ts)return "Belum pernah";const seconds=Math.max(0,Math.floor((Date.now()-ts)/1000));if(seconds<60)return "baru saja";if(seconds<3600)return `${Math.floor(seconds/60)} menit lalu`;if(seconds<86400)return `${Math.floor(seconds/3600)} jam lalu`;return formatSyncDate(ts);}
function settingsRow(label,value,state=""){return `<div class="settings-status-row"><span>${esc(label)}</span><span class="${state?`settings-state ${state}`:""}"><b>${esc(value)}</b></span></div>`;}
function getLocalStorageSize(){let bytes=0;try{for(let i=0;i<localStorage.length;i++){const key=localStorage.key(i)||"";bytes+=key.length+(localStorage.getItem(key)||"").length;}}catch(_){}return bytes*2;}
function getSettingsPermission(){const current=getCurrentUser()||{};const role=String(current.role||devProfile?.role||"");const developer=Boolean(current.isDeveloper||user?.id==="developer"||role.toLowerCase().includes("developer"));const crud=developer||role.toLowerCase().includes("pic");return {type:developer?"Developer":(role||"User"),role:role||"Tidak tersedia",developer,crud,source:developer?"Developer Session":"Database Role"};}
function renderSettingsAncillary(totalRows,lastTs){
  const config=readSettingsConfig();const permission=getSettingsPermission();const storageBytes=getLocalStorageSize();const storageMb=storageBytes/1024/1024;
  const available=SHEETS.filter(key=>(DATA[key]||[]).length>0);const invalid=SHEETS.filter(key=>(DATA[key]||[]).length===0);const issueCount=(!apiConnected?1:0)+invalid.length;
  const overview=document.getElementById("settingsOverviewList");if(overview)overview.innerHTML=[settingsRow("Google Sheets",apiConnected?"Terhubung":"Tidak terhubung",apiConnected?"":"error"),settingsRow("Auto Refresh",`Aktif • ${config.refreshMinutes} menit`),settingsRow("Cache",lastTs?`Aktif • ${storageMb.toFixed(1)} MB`:"Belum tersedia",lastTs?"":"neutral"),settingsRow("Mapping Sheet",`${available.length}/${SHEETS.length} valid`,invalid.length?"warn":""),settingsRow("Permission",`${permission.type} • ${permission.crud?"CRUD":"Read Only"}`),settingsRow("Last Sync",relativeSettingsTime(lastTs),lastTs?"":"neutral")].join("");
  const headline=document.getElementById("settingsHeadline");if(headline)headline.textContent=issueCount?`${issueCount} konfigurasi perlu diperiksa`:"Semua konfigurasi normal";
  const summary=document.getElementById("settingsOverviewSummary");if(summary)summary.textContent=issueCount?`${issueCount} item memerlukan perhatian`:"Semua layanan yang terpantau dalam kondisi normal";
  const warnings=document.getElementById("settingsWarnings");if(warnings){warnings.classList.toggle("hidden",!issueCount);warnings.innerHTML=issueCount?`<h5>Perlu Diperiksa</h5>${!apiConnected?`<button class="settings-warning" data-settings-target="settings-sources">⚠ Koneksi Google Sheets tidak tersedia</button>`:""}${invalid.map(s=>`<button class="settings-warning" data-settings-target="settings-mapping">⚠ Mapping ${esc(s)} belum dapat divalidasi</button>`).join("")}`:"";}
  const map=document.getElementById("settingsMappingList");if(map)map.innerHTML=SHEETS.map(name=>{const rows=DATA[name]||[];const headers=rows[0]?Object.keys(rows[0]).length:0;return `<details class="settings-mapping-item"><summary><strong>${esc(name)}</strong><span class="${rows.length?"mapping-ok":"mapping-warn"}">${rows.length?`✓ ${headers} header terbaca`:"⚠ Belum divalidasi"}</span></summary><p>${rows.length?`${rows.length.toLocaleString("id-ID")} row aktif • struktur header tersedia • validasi dari data cache saat ini`:`Data belum tersedia. Refresh sumber data untuk melakukan validasi.`}</p></details>`;}).join("");
  const mappingSummary=document.getElementById("settingsMappingSummary");if(mappingSummary)mappingSummary.textContent=`${SHEETS.length} modul terdaftar • ${available.length} valid • ${invalid.length} perlu diperiksa`;
  const sourceSummary=document.getElementById("settingsSourcesSummary");if(sourceSummary)sourceSummary.textContent=`${available.length} dari ${SHEETS.length} sumber memiliki data • sinkron ${relativeSettingsTime(lastTs)}`;
  const syncSummary=document.getElementById("settingsSyncSummary");if(syncSummary)syncSummary.textContent=`Auto refresh aktif setiap ${config.refreshMinutes} menit • sinkron terakhir ${relativeSettingsTime(lastTs)}`;
  const cacheSummary=document.getElementById("settingsCacheSummary");if(cacheSummary)cacheSummary.textContent=`Cache ${lastTs?"aktif":"belum tersedia"} • ${storageMb.toFixed(1)} MB digunakan`;
  const performance=document.getElementById("settingsPerformance");if(performance)performance.innerHTML=[settingsRow("UI Responsiveness","Baik"),settingsRow("Penyimpanan cache",`${storageMb.toFixed(1)} MB`,storageMb>4?"warn":""),settingsRow("Rows Cached",totalRows.toLocaleString("id-ID")),settingsRow("Background Refresh","Aktif"),settingsRow("Virtual Table","Aktif")].join("");
  const perm=document.getElementById("settingsPermission");if(perm)perm.innerHTML=`<div class="permission-block"><small>User Type</small><strong>${esc(permission.type)}</strong></div><div class="permission-block"><small>Database Role</small><strong>${esc(permission.role)}</strong></div><div class="permission-block"><small>Developer Session</small><strong>${permission.developer?"Aktif":"Tidak aktif"}</strong></div><div class="permission-block"><small>Source</small><strong>${esc(permission.source)}</strong></div><div class="permission-block"><small>Effective Permission</small><div class="permission-crud"><span class="allowed">Read ✓</span><span class="${permission.crud?"allowed":""}">Create ${permission.crud?"✓":"—"}</span><span class="${permission.crud?"allowed":""}">Update ${permission.crud?"✓":"—"}</span><span class="${permission.crud?"allowed":""}">Delete ${permission.crud?"✓":"—"}</span></div></div>`;
  const dev=document.getElementById("settingsDeveloperStatus");if(dev)dev.innerHTML=[settingsRow("Environment",location.hostname.includes("localhost")?"Development":"Production"),settingsRow("Build","v1.0.0"),settingsRow("API",apiConnected?"Healthy":"Unavailable",apiConnected?"":"error"),settingsRow("Google Sheets",apiConnected?"Connected":"Disconnected",apiConnected?"":"error"),settingsRow("Cache",lastTs?"Healthy":"Empty",lastTs?"":"neutral"),settingsRow("Credential",apiConnected?"Configured ✓":"Not connected",apiConnected?"":"neutral"),settingsRow("Last Fatal Error","None")].join("");
  const backupTs=Number(localStorage.getItem(SETTINGS_BACKUP_AT_KEY)||0);const backup=document.getElementById("settingsBackupInfo");if(backup)backup.innerHTML=[settingsRow("Last Backup",backupTs?formatSyncDate(backupTs):"Belum ada backup konfigurasi",backupTs?"":"neutral"),settingsRow("Backup Status",backupTs?"Success":"Belum tersedia",backupTs?"":"neutral"),settingsRow("Configuration Version","v1")].join("");
  document.querySelectorAll("[data-health]").forEach(dot=>{const key=dot.dataset.health;dot.className="health-indicator"+(key==="sources"&&!apiConnected?" error":key==="mapping"&&invalid.length?" warn":(["developer","backup"].includes(key)?" neutral":""));});
  const lastSaved=Number(localStorage.getItem(SETTINGS_SAVED_AT_KEY)||0);const savedEl=document.getElementById("settingsLastSaved");if(savedEl)savedEl.textContent=lastSaved?formatSyncDate(lastSaved):"Belum ada perubahan";
}
function downloadSettingsJson(name,data){const blob=new Blob([JSON.stringify(data,null,2)],{type:"application/json"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=name;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),500);}
function updateSettingsDraftUi(){const active=readSettingsConfig();const changes=Object.keys(settingsDraft).filter(k=>String(settingsDraft[k])!==String(active[k]));const bar=document.getElementById("settingsUnsavedBar");bar?.classList.toggle("hidden",!changes.length);const badge=document.getElementById("settingsSavedBadge");if(badge){badge.textContent=changes.length?`${changes.length} perubahan belum disimpan`:"Semua tersimpan";badge.className=`settings-badge ${changes.length?"warn":"ok"}`;}const count=document.getElementById("settingsUnsavedCount");if(count)count.textContent=changes.length;const details=document.getElementById("settingsUnsavedDetails");if(details)details.textContent=changes.map(k=>`${k}: ${active[k]} → ${settingsDraft[k]}`).join(" • ");document.querySelectorAll("[data-recommend-for]").forEach(el=>el.classList.toggle("custom",Number(settingsDraft[el.dataset.recommendFor]??active[el.dataset.recommendFor])!==SETTINGS_DEFAULTS[el.dataset.recommendFor]));}
function bindSettingsControlCenter(){if(window.__settingsControlBound)return;window.__settingsControlBound=true;settingsDraft=readSettingsConfig();
  document.getElementById("page-settings")?.addEventListener("click",event=>{const btn=event.target.closest("[data-settings-target]");if(!btn)return;document.getElementById(btn.dataset.settingsTarget)?.scrollIntoView({behavior:"smooth",block:"start"});document.querySelectorAll(".settings-nav button").forEach(n=>n.classList.toggle("active",n.dataset.settingsTarget===btn.dataset.settingsTarget));});
  document.querySelectorAll("[data-settings-target]").forEach(btn=>btn.addEventListener("click",()=>{const id=btn.dataset.settingsTarget;document.getElementById(id)?.scrollIntoView({behavior:"smooth",block:"start"});document.querySelectorAll(".settings-nav button").forEach(n=>n.classList.toggle("active",n===btn));}));
  document.querySelectorAll("[data-setting-key]").forEach(input=>{input.value=String(settingsDraft[input.dataset.settingKey]);input.addEventListener("change",()=>{settingsDraft[input.dataset.settingKey]=Number(input.value);updateSettingsDraftUi();});});
  document.getElementById("settingsDiscard")?.addEventListener("click",()=>{settingsDraft=readSettingsConfig();document.querySelectorAll("[data-setting-key]").forEach(i=>i.value=String(settingsDraft[i.dataset.settingKey]));updateSettingsDraftUi();});
  document.getElementById("settingsSave")?.addEventListener("click",()=>{localStorage.setItem(SETTINGS_CONFIG_KEY,JSON.stringify(settingsDraft));localStorage.setItem(SETTINGS_SAVED_AT_KEY,String(Date.now()));updateSettingsDashboard();updateSettingsDraftUi();toast("Pengaturan berhasil disimpan","success");});
  const search=document.getElementById("settingsSearch"),results=document.getElementById("settingsSearchResults");search?.addEventListener("input",()=>{const q=clean(search.value);if(!q){results?.classList.add("hidden");return;}const items=[...document.querySelectorAll(".settings-section")].flatMap(section=>[...section.querySelectorAll("[data-search-setting],.settings-head")].map(node=>({title:node.querySelector("strong,h4")?.textContent||"Pengaturan",section:section.querySelector("h4")?.textContent||"Umum",value:node.querySelector(".settings-control-value span")?.textContent||section.querySelector(".settings-head p")?.textContent||"Detail",id:section.id,text:clean(node.textContent)}))).filter(x=>x.text.includes(q)).slice(0,6);results.innerHTML=items.map(x=>`<button data-settings-target="${x.id}"><strong>${esc(x.title)}</strong><small>${esc(x.section)} • ${esc(x.value)}</small></button>`).join("")||`<button disabled><strong>Tidak ditemukan</strong><small>Coba kata kunci lain</small></button>`;results.classList.remove("hidden");results.querySelectorAll("[data-settings-target]").forEach(b=>b.addEventListener("click",()=>{document.getElementById(b.dataset.settingsTarget)?.scrollIntoView({behavior:"smooth"});results.classList.add("hidden");}));});
  document.getElementById("settingsHealthCheck")?.addEventListener("click",e=>{e.currentTarget.textContent="Health check selesai";updateSettingsDashboard();toast("Health check menggunakan state dan metadata lokal","success");});
  document.getElementById("settingsExportDiagnostics")?.addEventListener("click",()=>downloadSettingsJson("wms-diagnostics.json",{generatedAt:new Date().toISOString(),apiConnected,lastSync:getLastSyncTs(),rowCounts:Object.fromEntries(SHEETS.map(s=>[s,(DATA[s]||[]).length])),cacheVersion:CACHE_VERSION}));
  document.getElementById("settingsExportConfig")?.addEventListener("click",()=>{localStorage.setItem(SETTINGS_BACKUP_AT_KEY,String(Date.now()));downloadSettingsJson("wms-config.json",{version:1,exportedAt:new Date().toISOString(),settings:readSettingsConfig()});updateSettingsDashboard();});
  document.getElementById("settingsImportConfig")?.addEventListener("change",async e=>{try{if(!e.target.files?.[0])return;const data=JSON.parse(await e.target.files[0].text());settingsDraft={...SETTINGS_DEFAULTS,...data.settings};document.querySelectorAll("[data-setting-key]").forEach(i=>i.value=String(settingsDraft[i.dataset.settingKey]));updateSettingsDraftUi();toast("Konfigurasi dimuat. Simpan untuk menerapkan.");}catch(_){toast("File konfigurasi tidak valid","error");}});
}
function updateSettingsDashboard(){
  ...
  settingsDataSources.innerHTML=Object.entries(sheetMap).map(([label,key])=>{
    ...
    return `<div class='settings-source-item'><div class='settings-source-main'><i data-lucide='${icons[label]||"database"}'></i><div><strong>${label}</strong><small>${active}</small><div class="settings-source-detail"><span>${count?"1 sheet ditemukan":"Sheet belum dimuat"}</span><span>Last sync: ${esc(relativeSettingsTime(lastTs))}</span><span>Source: ${lastTs?"Local cache":"Network"}</span></div></div></div><div class='settings-source-meta'><span>${count.toLocaleString("id-ID")} row</span><em class='${tag}'>${txt}</em><div class="settings-source-actions"><button class="btn-ghost" onclick="loadAllData(true)">Test</button><button class="btn-ghost" onclick="loadAllData(true)">Refresh</button><button class="btn-ghost" data-settings-target="settings-mapping">Detail</button></div></div></div>`;
  }).join("");
  renderSettingsAncillary(totalRows,lastTs);bindSettingsControlCenter();updateSettingsDraftUi();
  if(window.lucide)lucide.createIcons();
}
...
async function loadImportPdfSheets(){
if(PDF_TRANSFER_STATE.isLoadingSheets||PDF_TRANSFER_STATE.sheetsLoaded)return;
PDF_TRANSFER_STATE.isLoadingSheets=true;PDF_TRANSFER_STATE.sheetError="";renderImportPdfTransferPage();
try{const res=await fetch('/api/import-pdf-transfer');const out=await res.json();if(!res.ok||!out?.success)throw new Error(out?.message||'Gagal memuat daftar sheet');PDF_TRANSFER_STATE.sheets=Array.isArray(out.sheets)?out.sheets:[];PDF_TRANSFER_STATE.sheetsLoaded=true;if(PDF_TRANSFER_STATE.selectedSheetName&&!PDF_TRANSFER_STATE.sheets.includes(PDF_TRANSFER_STATE.selectedSheetName))PDF_TRANSFER_STATE.selectedSheetName="";
}catch(err){PDF_TRANSFER_STATE.sheetError=err?.message||'Gagal memuat daftar sheet';}finally{PDF_TRANSFER_STATE.isLoadingSheets=false;renderImportPdfTransferPage();}
}
function renderImportPdfTransferPage(){
const root=document.getElementById("importPdfTransferApp");if(!root)return;
if(!PDF_TRANSFER_STATE.sheetsLoaded&&!PDF_TRANSFER_STATE.isLoadingSheets&&!PDF_TRANSFER_STATE.sheetError)queueMicrotask(loadImportPdfSheets);
...
document.getElementById('pdfTransferSheetSelect')?.addEventListener('change',e=>{PDF_TRANSFER_STATE.selectedSheetName=e.target.value;renderImportPdfTransferPage();});
...
async function importPdfTransferData(){if(PDF_TRANSFER_STATE.isImporting)return;if(!PDF_TRANSFER_STATE.selectedSheetName){toast('Pilih sheet tujuan terlebih dahulu.','error');return;}const {valid,failed}=getValidatedPdfTransferRowsForImport();if(failed.length){PDF_TRANSFER_STATE.failedRows=failed;PDF_TRANSFER_STATE.warnings=[`Masih ada ${failed.length} row preview gagal validasi. Perbaiki sebelum import.`];renderImportPdfTransferPage();toast('Perbaiki row yang gagal validasi sebelum import','error');return;}if(!valid.length){toast('Tidak ada row CSV valid untuk import','error');return;}PDF_TRANSFER_STATE.isImporting=true;renderImportPdfTransferPage();
try{const res=await fetch('/api/import-pdf-transfer',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({sheetName:PDF_TRANSFER_STATE.selectedSheetName,header:PDF_TRANSFER_STATE.header,rows:valid,columnMapping:{sku:'sku',namaProduk:'namaProduk',qty:'qty',diterima:'diterima',batal:'batal',tolak:'tolak',catatan:'catatan'},source:'csv'})});const out=await res.json();if(!res.ok||!out?.success)throw new Error(out?.message||'Gagal import');
toast(`Import sukses ke sheet ${out.sheetName||PDF_TRANSFER_STATE.selectedSheetName}`,'success');
...
function openActivityDrawer(row){const m=activityMeta(row),changes=m.details?.changes||[];document.getElementById('activityDrawer')?.remove();document.body.insertAdjacentHTML('beforeend',`<div id="activityDrawer" class="activity-drawer-backdrop"><aside class="activity-drawer"><header><div><small>DETAIL AKTIVITAS</small><h3>${activityBadge(row.action)}</h3></div><button class="icon-btn" data-close-activity>×</button></header><p class="activity-description">${esc(row.detail||'-')}</p><dl><dt>Activity ID</dt><dd>${esc(row.reference||row.id||'-')}</dd><dt>Timestamp</dt><dd>${esc(new Date(row.created_at).toLocaleString('id-ID'))}</dd><dt>User / Role</dt><dd>${esc(row.user_name||'-')} · ${esc(row.role||'-')}</dd><dt>Developer</dt><dd>${m.isDeveloper?'Ya':'Tidak'}</dd><dt>Session</dt><dd>${esc(m.sessionId||'-')}</dd><dt>Module / Page</dt><dd>${esc(row.module||'-')} · ${esc(m.page||'-')}</dd><dt>Result</dt><dd>${activityBadge(row.status,'result')}</dd><dt>Entity</dt><dd>${esc(m.entityType||'-')} ${esc(m.entityId||'')}</dd><dt>Source</dt><dd>${esc(m.source||'-')}</dd></dl>${changes.length?`<section><h4>Perubahan</h4>${activityDetail(row)}</section>`:''}${m.details?.error?`<div class="state error">${esc(m.details.error)}</div>`:''}</aside></div>`);document.querySelectorAll('[data-close-activity],#activityDrawer').forEach(el=>el.onclick=e=>{if(e.target===el)document.getElementById('activityDrawer')?.remove();});}
async function renderActivityLogPage(){const root=document.getElementById('activityLogApp');if(!root)return;root.innerHTML='<div class="state">Memuat aktivitas…</div>';let rows=[],err='';try{rows=await fetchActivityLogs();}catch(e){err=e.message;}const today=rows.filter(x=>new Date(x.created_at).toDateString()===new Date().toDateString()),login=today.filter(x=>['LOGIN','AUTO_LOGIN','LOGIN_SUCCESS','LOGIN_DEVELOPER'].includes(x.action)).length,crud=today.filter(x=>['CREATE','UPDATE','DELETE','BATCH_UPDATE'].includes(x.action)).length,bad=today.filter(x=>['FAILED','DENIED'].includes(x.status)).length;const actions=['','LOGIN','AUTO_LOGIN','PAGE_VIEW','CREATE','UPDATE','DELETE','CHECKLIST','UNCHECK','IMPORT','EXPORT','MOVEMENT','BATCH_UPDATE','UNDO','REDO','REFRESH','LOGOUT','SESSION_EXPIRED'];root.innerHTML=`<div class="activity-summary"><div><b>${today.length}</b><span>Aktivitas Hari Ini</span></div><div><b>${login}</b><span>Login</span></div><div><b>${crud}</b><span>CRUD</span></div><div><b>${bad}</b><span>Gagal / Ditolak</span></div></div><div class="card activity-panel"><div class="activity-filters"><input id="alSearch" placeholder="Cari SKU, user, lokasi, sheet, Activity ID…" value="${esc(ACTIVITY_LOG_STATE.filters.search)}"><select id="alAction">${actions.map(a=>`<option value="${a}">${a||'Semua Aktivitas'}</option>`).join('')}</select><input id="alModule" placeholder="Modul" value="${esc(ACTIVITY_LOG_STATE.filters.module)}"><input id="alUser" placeholder="User" value="${esc(ACTIVITY_LOG_STATE.filters.user)}"><select id="alStatus"><option value="">Semua Result</option><option>SUCCESS</option><option>FAILED</option><option>DENIED</option></select><input id="alFrom" type="date" value="${esc(ACTIVITY_LOG_STATE.filters.from)}"><button id="alApply" class="btn-primary">Terapkan</button></div>${err?`<div class="state error">${esc(err)}</div>`:''}<div class="table-wrap"><table class="activity-table"><thead><tr><th>Waktu</th><th>User</th><th>Aktivitas</th><th>Modul</th><th>Detail</th><th>Result</th><th>Aksi</th></tr></thead><tbody>${rows.length?rows.map((x,i)=>`<tr><td>${esc(formatDateTime(x.created_at))}</td><td><b>${esc(x.user_name||'-')}</b><small>${esc(x.role||'')}</small></td><td>${activityBadge(x.action)}</td><td>${esc(x.module||'-')}</td><td>${activityDetail(x)}</td><td>${activityBadge(x.status,'result')}</td><td><button class="btn-ghost" data-view-activity="${i}">Lihat</button></td></tr>`).join(''):`<tr><td colspan="7"><div class="state">Belum ada aktivitas.</div></td></tr>`}</tbody></table></div><div class="mv-pagination"><span>Halaman ${ACTIVITY_LOG_STATE.page}</span><div class="row"><button id="alPrev" class="btn-ghost">Sebelumnya</button><button id="alNext" class="btn-ghost">Berikutnya</button></div></div></div>`;for(const id of ['Action','Status'])document.getElementById(`al${id}`).value=ACTIVITY_LOG_STATE.filters[id.toLowerCase()];document.getElementById('alApply').onclick=()=>{ACTIVITY_LOG_STATE.filters={...ACTIVITY_LOG_STATE.filters,search:alSearch.value.trim(),action:alAction.value,module:alModule.value.trim(),user:alUser.value.trim(),status:alStatus.value,from:alFrom.value};ACTIVITY_LOG_STATE.page=1;renderActivityLogPage();};document.getElementById('alPrev').onclick=()=>{ACTIVITY_LOG_STATE.page=Math.max(1,ACTIVITY_LOG_STATE.page-1);renderActivityLogPage();};document.getElementById('alNext').onclick=()=>{if(rows.length===ACTIVITY_LOG_STATE.pageSize)ACTIVITY_LOG_STATE.page++;renderActivityLogPage();};document.querySelectorAll('[data-view-activity]').forEach(b=>b.onclick=()=>openActivityDrawer(rows[Number(b.dataset.viewActivity)]));}
