...
header.nomorTransfer=findVal('Nomor Transfer');header.tanggal=findVal('Tanggal');header.dari=findVal('Dari');header.kepada=findVal('Kepada');header.status=findVal('Status');header.nomorReferensi=findVal('Nomor Referensi|No Referensi|Reference');
const startIdx=rows.findIndex(r=>/SKU/i.test(r)&&/Nama Produk/i.test(r)&&/Jumlah/i.test(r)&&/Diterima/i.test(r)&&/Batal/i.test(r)&&/Tolak/i.test(r));
const parsed=[];const skipped=[];
if(startIdx>=0){for(let i=startIdx+1;i<rows.length;i++){const row=rows[i];if(/total|grand total/i.test(row))break;const parts=row.split(/\s{2,}|	/).filter(Boolean);if(!parts.length)continue;const sku=parts[0];if(!isSkuCandidate(sku)){if(parsed.length){parsed[parsed.length-1].namaProduk+=(parsed[parsed.length-1].namaProduk?' ':'')+row;continue;}skipped.push(`${row} => sku tidak valid`);continue;}const nums=parts.filter(x=>/^\d+$/.test(x));const jumlah=Number(nums[0]||0),diterima=Number(nums[1]||0),batal=Number(nums[2]||0),tolak=Number(nums[3]||0);const namaProduk=parts.slice(1,Math.max(2,parts.length-4)).join(' ').trim();parsed.push({sku,namaProduk,jumlah,diterima,batal,tolak,catatan:parts.slice(parts.length-1).join(' ')});}
}
