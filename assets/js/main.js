...
function runSearch(){const qRaw=searchInput?.value||"";const q=clean(qRaw);lastQuery=qRaw;if(!q){lastResults=[];renderRecentSearches();return renderState("results","Masukkan kata kunci pencarian.");}const words=q.split(" ").filter(Boolean);const out=[];for(const it of CACHE_SKU.values()){if(currentFilter!=="Semua"&&!it.sources.has(currentFilter))continue;const skuN=clean(it.sku),nameN=clean(it.nama),combined=`${skuN} ${nameN}`;let rank=99;if(skuN===q)rank=1;else if(/^\d{4}$/.test(q)&&skuN.endsWith(q))rank=2;else if(skuN.includes(q))rank=3;else if(nameN===q)rank=4;else if(words.length&&words.every(w=>combined.includes(w)))rank=5;else if(words.some(w=>combined.includes(w)))rank=6;if(rank<99)out.push({...it,sources:[...it.sources],rank});}
lastResults=out.sort((a,b)=>a.rank-b.rank||a.sku.localeCompare(b.sku)).slice(0,50);saveRecentSearch(qRaw);renderResults(lastResults,qRaw);}
function renderResults(items,query,targetId="results"){if(sortSearch.value==="sku")items=[...items].sort((a,b)=>a.sku.localeCompare(b.sku));if(sortSearch.value==="name")items=[...items].sort((a,b)=>a.nama.localeCompare(b.nama));if(!items.length) return renderState(targetId,"Data tidak ditemukan.");const resultsEl=`<div class='subtitle'>${items.length} hasil (maks 50).</div><div class='result-list'>`+items.map(r=>{const skuE=encAttr(r.sku);return `<div class='result-card'><div class='result-head'><div><strong>${highlight(r.nama,query)}</strong><div>SKU: ${highlight(r.sku,query)}</div></div><div>${r.sources.map(s=>`<span class='badge ${badgeClass(s)}'>${esc(s)}</span>`).join(" ")}</div></div><div class='row'><button class='btn-ghost' onclick="copySku(decodeURIComponent('${skuE}'))">Copy SKU</button><button class='btn-primary' onclick="showDetail(decodeURIComponent('${skuE}'));navigateTo('/sku/'+encodeURIComponent(decodeURIComponent('${skuE}')))">Lihat Detail</button></div></div>`;}).join("")+"</div>";document.getElementById(targetId).innerHTML=resultsEl;}
...
function searchFromHistory(q){searchInput.value=q;runSearch();searchInput.focus();}
function onSearchShortcut(e){const isCmdK=(e.ctrlKey||e.metaKey)&&String(e.key).toLowerCase()==="k";if(isCmdK){e.preventDefault();navigateTo("/search");setTimeout(()=>searchInput?.focus(),0);return renderRecentSearches();}if(e.key==="Escape"&&document.activeElement===searchInput){e.preventDefault();searchInput.blur();}}
