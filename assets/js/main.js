...
function runSearch(){const qRaw=SEARCH_STATE.filterValue||"";const q=clean(qRaw);const prevQuery=lastQuery;if(clean(prevQuery)!==q)selectedQuickSku="";lastQuery=qRaw;const minChars=SEARCH_STATE.minChars||2;if(!q){lastResults=[];SEARCH_STATE.page=1;renderRecentHistory();renderQuickResultCard(null,qRaw,"hint");return renderState("results",`Ketik minimal ${minChars} huruf untuk mencari.`);}if(q.length<minChars){lastResults=[];SEARCH_STATE.page=1;renderQuickResultCard(null,qRaw,"hint");return renderState("results",`Ketik minimal ${minChars} huruf untuk mencari.`);}saveRecentSearch(qRaw);if(SEARCH_STATE.abortController)SEARCH_STATE.abortController.abort();SEARCH_STATE.abortController=new AbortController();const {signal}=SEARCH_STATE.abortController;const words=q.split(" ").filter(Boolean);const isMultiWord=words.length>1;const out=[];for(const it of CACHE_SKU.values()){if(signal.aborted)return;if(currentFilter!=="Semua"&&!it.sources.has(currentFilter))continue;const skuN=it._skuClean||clean(String(it.sku||""));const nameN=it._nameClean||clean(it.nama);const combined=it._searchText||clean(`${it.sku||""} ${it.nama||""}`);if(isMultiWord&&!words.every(word=>combined.includes(word)))continue;let rank=99;if(skuN===q)rank=1;else if(/^\d{4}$/.test(q)&&(it._skuDigits||"").endsWith(q))rank=2;else if(skuN.includes(q))rank=3;else if(nameN===q)rank=4;else if(words.length&&words.every(w=>combined.includes(w)))rank=5;else if(!isMultiWord&&words.some(w=>combined.includes(w)))rank=6;if(rank<99)out.push({...it,sources:[...it.sources],rank});}
...
const CYCLE_SESSIONS_KEY="cycle_count_sessions_v1";
const CYCLE_LAST_SESSION_KEY="cycle_count_last_session_v1";
const CYCLE_HISTORY_RANGE="Cycle Count!A1:ZZ";
const CYCLE_STATE={currentSessionId:"",searchInput:"",search:"",searchTimer:null,submitting:false,autoSaveTimer:null,syncing:false};
...
function cycleSessions(){try{return JSON.parse(localStorage.getItem(CYCLE_SESSIONS_KEY)||"[]");}catch(_){return[];}}
function saveCycleSessions(rows){localStorage.setItem(CYCLE_SESSIONS_KEY,JSON.stringify(rows));}
function cycleNow(){return new Date().toISOString();}
function newCycleSessionId(){return `CC-${Date.now()}-${Math.random().toString(36).slice(2,8).toUpperCase()}`;}
function getCycleSession(id=CYCLE_STATE.currentSessionId){return cycleSessions().find(s=>s.sessionId===id)||null;}
function upsertCycleSession(session){const rows=cycleSessions();const i=rows.findIndex(s=>s.sessionId===session.sessionId);const next={...session,updatedAt:cycleNow()};if(i>=0)rows[i]={...rows[i],...next};else rows.unshift(next);saveCycleSessions(rows);localStorage.setItem(CYCLE_LAST_SESSION_KEY,next.sessionId);return next;}
function cycleStatusBadge(status){const cls=status==='Selesai'?'b-ok':status==='Aktif'?'b-warn':'b-low';return `<span class='badge ${cls}'>${esc(status)}</span>`;}
function normalizeCycleSessionStatus(s){return s?.status||(s?.completedAt?'Selesai':((s?.items||[]).length?'Aktif':'Draft'));}
function cycleDiffBadge(item){if(item?.stokFisik===''||item?.stokFisik===null||item?.stokFisik===undefined)return "<span class='badge b-low'>Belum dihitung</span>";const d=(Number(item.stokFisik)-Number(item.stokSistem));return d===0?"<span class='badge b-ok'>Sesuai</span>":"<span class='badge b-warn'>Ada Selisih</span>";}
function canSubmitCycle(session=getCycleSession()){return session&&session.items?.length>0&&session.items.every(it=>it.stokFisik!==''&&it.stokFisik!==null&&it.stokFisik!==undefined&&Number.isFinite(Number(it.stokFisik)));}
function scheduleCycleAutoSave(session){clearTimeout(CYCLE_STATE.autoSaveTimer);CYCLE_STATE.autoSaveTimer=setTimeout(()=>{upsertCycleSession(session);renderCycleSaveState('Tersimpan lokal');},500);renderCycleSaveState('Menyimpan otomatis...');}
function renderCycleSaveState(text){const el=document.getElementById('ccSaveState');if(el)el.textContent=text;}
function buildCycleHistoryRows(){return [...(CYCLE_HISTORY_REMOTE.rows||[])];}
function renderCycleHistory(){/* History lama tetap tersedia di backend, halaman Cycle Count kini berfokus pada daftar sesi. */}
function renderCycleCountPage(){if(!cycleCountApp)return;const isSessionRoute=location.pathname.startsWith('/cycle-count/session/');if(isSessionRoute)return renderCycleSessionPage();const sessions=cycleSessions().map(s=>({...s,status:normalizeCycleSessionStatus(s)})).sort((a,b)=>String(b.updatedAt).localeCompare(String(a.updatedAt)));const active=sessions.filter(s=>s.status!=='Selesai');const done=sessions.filter(s=>s.status==='Selesai');const last=localStorage.getItem(CYCLE_LAST_SESSION_KEY);cycleCountApp.innerHTML=`<div class='card cc-card cc-section'><div class='section-header'><div><h4>Daftar Sesi Cycle Count</h4><p class='muted'>Kelola sesi audit. Proses hitung stok dilakukan di halaman Sesi Cycle Count.</p></div><div class='cc-action-stack'><button class='btn-ghost' ${last?`onclick="ccOpenSession('${encAttr(last)}')"`:"disabled"}>Resume sesi terakhir</button><button class='btn-primary' onclick='ccStartSession()'>Mulai Sesi Baru</button></div></div>${CYCLE_HISTORY_REMOTE.error?`<div class='state cc-state state-error'>${esc(CYCLE_HISTORY_REMOTE.error)}</div>`:""}</div><div class='card cc-card cc-section'><h4>Sesi Aktif</h4>${renderCycleSessionList(active,'Belum ada sesi aktif.')}</div><div class='card cc-card cc-section'><h4>Sesi Selesai</h4>${renderCycleSessionList(done,'Belum ada sesi selesai.')}</div>`;if(window.lucide?.createIcons)window.lucide.createIcons();}
function renderCycleSessionList(rows,empty){if(!rows.length)return `<div class='cc-empty-state'><i data-lucide='clipboard-check'></i><h4>${empty}</h4></div>`;return `<div class='table-wrap cc-table-wrap'><table><thead><tr><th>Session ID</th><th>Status</th><th>Item</th><th>Dibuat</th><th>Update</th><th>Aksi</th></tr></thead><tbody>${rows.map(s=>`<tr><td>${esc(s.sessionId)}</td><td>${cycleStatusBadge(s.status)}</td><td>${(s.items||[]).length}</td><td>${esc(formatDateTime(s.createdAt))}</td><td>${esc(formatDateTime(s.updatedAt))}</td><td><button class='btn-ghost' onclick="ccOpenSession('${encAttr(s.sessionId)}')">${s.status==='Selesai'?'Lihat':'Lanjutkan'}</button></td></tr>`).join('')}</tbody></table></div>`;}
function renderCycleSessionPage(){const session=getCycleSession();if(!session){cycleCountApp.innerHTML=`<div class='card cc-card cc-section'><div class='state cc-state state-error'>Sesi tidak ditemukan.</div><button class='btn-ghost' onclick="navigateTo('/cycle-count')">Kembali</button></div>`;return;}const readOnly=normalizeCycleSessionStatus(session)==='Selesai';cycleCountApp.innerHTML=`<div class='card cc-card cc-section'><div class='section-header'><div><h4>Sesi Cycle Count</h4><p class='muted'>${esc(session.sessionId)} · ${cycleStatusBadge(normalizeCycleSessionStatus(session))} · <span id='ccSaveState'>Tersimpan lokal</span></p></div><div class='cc-action-stack'><button class='btn-ghost' onclick='ccSaveDraft()' ${readOnly?'disabled':''}>Simpan Draft</button><button class='btn-primary' onclick='ccSubmitSession()' ${readOnly||!canSubmitCycle(session)||CYCLE_STATE.submitting?'disabled':''}>Selesaikan Sesi</button><button class='btn-ghost' onclick="navigateTo('/cycle-count')">Keluar Sesi</button></div></div>${!readOnly?`<div class='mv-toolbar'><input id='ccSearch' class='search-lg' placeholder='Cari SKU / nama barang / lokasi' value='${esc(CYCLE_STATE.searchInput)}'></div><div class='table-wrap cc-table-wrap cc-search-wrap'><table><thead><tr><th>Lokasi</th><th>SKU</th><th>Nama Barang</th><th>Stok Sistem</th><th>Aksi</th></tr></thead><tbody id='ccSearchResultsBody'></tbody></table></div>`:''}<div class='cc-section'><div class='table-wrap cc-table-wrap cc-session-wrap'><table><thead><tr><th>Lokasi</th><th>SKU</th><th>Nama Barang</th><th>Stok Sistem</th><th>Stok Fisik</th><th>Selisih</th><th>Status</th><th>Catatan</th><th>Aksi</th></tr></thead><tbody id='ccSessionBody'></tbody></table></div></div></div>`;if(!readOnly)renderCycleSearchResults();renderCycleSessionTable(session,readOnly);}
function renderCycleSearchResults(){const tbody=document.querySelector('#ccSearchResultsBody');if(!tbody)return;const candidates=getCycleCandidates(CYCLE_STATE.search);if(!candidates.length){tbody.innerHTML="<tr><td colspan='5'><div class='state cc-state'>Cari SKU untuk menambah item.</div></td></tr>";return;}tbody.innerHTML=candidates.map(c=>`<tr><td>${esc(c.lokasi)}</td><td>${esc(c.sku)}</td><td>${esc(c.nama)}</td><td>${esc(c.stok_akhir)}</td><td><button class='btn-ghost' data-cc-action='add' data-sku='${encAttr(c.sku)}' data-lok='${encAttr(c.lokasi)}'>Tambah</button></td></tr>`).join('');}
function renderCycleSessionTable(session=getCycleSession(),readOnly=normalizeCycleSessionStatus(session)==='Selesai'){const tbody=document.querySelector('#ccSessionBody');if(!tbody)return;const items=session?.items||[];if(!items.length){tbody.innerHTML="<tr><td colspan='9'><div class='state cc-state'>Belum ada item dalam sesi ini.</div></td></tr>";return;}tbody.innerHTML=items.map((r,i)=>{const fisik=r.stokFisik??"";const selisih=Number.isFinite(Number(fisik))?Number(fisik)-Number(r.stokSistem):"-";return `<tr><td>${esc(r.lokasi)}</td><td>${esc(r.sku)}</td><td>${esc(r.nama)}</td><td>${esc(r.stokSistem)}</td><td>${readOnly?esc(fisik):`<input type='number' data-cc='fisik' data-idx='${i}' value='${esc(fisik)}'>`}</td><td data-cc-cell='diff'>${esc(selisih)}</td><td data-cc-cell='st'>${cycleDiffBadge(r)}</td><td>${readOnly?esc(r.catatan||''):`<input data-cc='ct' data-idx='${i}' value='${esc(r.catatan||"")}'>`}</td><td>${readOnly?'-':`<button class='btn-ghost' data-cc-action='remove' data-idx='${i}'>Hapus</button>`}</td></tr>`;}).join('');}
window.ccOpenSession=(id)=>{navigateTo(`/cycle-count/session/${encodeURIComponent(id)}`);};
window.ccStartSession=()=>{const now=cycleNow();const session=upsertCycleSession({sessionId:newCycleSessionId(),status:'Draft',items:[],createdAt:now,updatedAt:now,synced:false,syncError:''});logActivitySafe({action:'START_CYCLE_COUNT_SESSION',module:'Cycle Count',detail:`Mulai sesi ${session.sessionId}`,status:'SUCCESS',reference:session.sessionId});navigateTo(`/cycle-count/session/${encodeURIComponent(session.sessionId)}`);};
function addCycleItem(source){const session=getCycleSession();if(!session||normalizeCycleSessionStatus(session)==='Selesai')return;const newItem={lokasi:source.lokasi,sku:source.sku,nama:source.nama,stokSistem:getQty(source),stokFisik:null,selisih:null,status:'Belum dihitung',catatan:""};if((session.items||[]).some(it=>cycleItemKey(it)===cycleItemKey(newItem))){toast('SKU/lokasi sudah ada di sesi ini.','error');return;}session.items=[...(session.items||[]),newItem];session.status='Aktif';scheduleCycleAutoSave(session);renderCycleSessionTable(session,false);toast('Item ditambahkan','success');}
window.ccAddItem=(skuEnc,lokEnc)=>{const sku=decodeURIComponent(skuEnc),lokasi=decodeURIComponent(lokEnc);const source=getCycleSourceRows().find(r=>clean(r.sku)===clean(sku)&&clean(r.lokasi)===clean(lokasi));if(source)addCycleItem(source);};
window.ccRemoveItem=(idx)=>{const session=getCycleSession();if(!session||normalizeCycleSessionStatus(session)==='Selesai')return;session.items.splice(idx,1);session.status=session.items.length?'Aktif':'Draft';upsertCycleSession(session);renderCycleSessionPage();};
window.ccSaveDraft=()=>{const session=getCycleSession();if(!session)return;upsertCycleSession({...session,status:session.items?.length?'Aktif':'Draft'});toast('Draft sesi tersimpan lokal','success');renderCycleSaveState('Draft tersimpan');};
async function syncCycleSession(session){const payload={tanggal:formatTanggal(new Date()),sessionId:session.sessionId,items:(session.items||[]).map(it=>({lokasi:it.lokasi,sku:it.sku,nama_barang:it.nama,bulky:Number(it.stokSistem)||0,retail:0,aktual_bulky:Number(it.stokFisik)||0,aktual_retail:0,catatan:it.catatan||""}))};const res=await fetch('/api/cycle-count',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const data=await res.json();if(!res.ok||!data?.success)throw new Error(data?.message||'Gagal sync cycle count');return data;}
async function syncPendingCycleSessions(){if(CYCLE_STATE.syncing)return;const pending=cycleSessions().filter(s=>normalizeCycleSessionStatus(s)==='Selesai'&&!s.synced);if(!pending.length)return;CYCLE_STATE.syncing=true;try{for(const session of pending){try{await syncCycleSession(session);upsertCycleSession({...session,synced:true,syncedAt:cycleNow(),syncError:''});}catch(err){upsertCycleSession({...session,synced:false,syncError:err?.message||'Gagal sync'});}}}finally{CYCLE_STATE.syncing=false;if(getActivePage()==='cycle-count')renderCycleCountPage();}}
window.addEventListener('online',()=>syncPendingCycleSessions());
setTimeout(()=>syncPendingCycleSessions(),2000);

window.ccSubmitSession=async()=>{let session=getCycleSession();if(!canSubmitCycle(session)){toast('Lengkapi stok fisik semua item.','error');return;}CYCLE_STATE.submitting=true;renderCycleSessionPage();try{session={...session,status:'Selesai',completedAt:cycleNow(),syncError:''};upsertCycleSession(session);await syncCycleSession(session);upsertCycleSession({...session,synced:true,syncedAt:cycleNow()});toast('Sesi selesai dan berhasil disync','success');logActivitySafe({action:'SUBMIT_CYCLE_COUNT',module:'Cycle Count',detail:`Selesaikan sesi ${session.sessionId} (${session.items.length} item)`,status:'SUCCESS',reference:session.sessionId,metadata:{items:session.items.length}});CYCLE_HISTORY_REMOTE.loaded=false;ensureCycleHistoryLoaded();}catch(err){upsertCycleSession({...session,synced:false,syncError:err?.message||'Gagal sync'});toast('Sesi selesai tersimpan lokal. Sync akan dicoba saat koneksi berhasil.','error');}finally{CYCLE_STATE.submitting=false;renderCycleSessionPage();}};
...
document.addEventListener('input',e=>{if(e.target?.id==='ccSearch'){CYCLE_STATE.searchInput=e.target.value;clearTimeout(CYCLE_STATE.searchTimer);CYCLE_STATE.searchTimer=setTimeout(()=>{CYCLE_STATE.search=CYCLE_STATE.searchInput;renderCycleSearchResults();},280);return;}if(e.target?.id==='ccHistoryPageSize'){cycleHistoryPageSize=Number(e.target.value)||10;cycleHistoryPage=1;renderCycleHistory();return;}if(e.target?.dataset?.cc){const session=getCycleSession();if(!session||normalizeCycleSessionStatus(session)==='Selesai')return;const idx=Number(e.target.dataset.idx);const row=session.items?.[idx];if(!row)return;if(e.target.dataset.cc==='fisik'){const n=Number(e.target.value);row.stokFisik=Number.isFinite(n)?n:null;row.selisih=Number.isFinite(n)?n-Number(row.stokSistem):null;row.status=Number.isFinite(n)?(row.selisih===0?'Sesuai':'Ada Selisih'):'Belum dihitung';}if(e.target.dataset.cc==='ct')row.catatan=e.target.value||"";session.status=session.items?.length?'Aktif':'Draft';scheduleCycleAutoSave(session);const tr=e.target.closest('tr');if(tr){const diffCell=tr.querySelector("[data-cc-cell='diff']"),stCell=tr.querySelector("[data-cc-cell='st']");if(diffCell)diffCell.textContent=Number.isFinite(Number(row.stokFisik))?String(Number(row.stokFisik)-Number(row.stokSistem)):'-';if(stCell)stCell.innerHTML=cycleDiffBadge(row);}const submitBtn=document.querySelector("button[onclick='ccSubmitSession()']");if(submitBtn)submitBtn.disabled=!canSubmitCycle(session)||CYCLE_STATE.submitting;}});
