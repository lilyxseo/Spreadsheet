...
anomalyType?.addEventListener("change",()=>applyAnomalyFilters(true));anomalyPeriod?.addEventListener("change",e=>{ANOMALY_STATE.period=e.target.value||"all";refreshAnomalyInBackground(true);});anomalyStart?.addEventListener("change",e=>{ANOMALY_STATE.customStart=e.target.value||"";ANOMALY_STATE.period="custom";if(anomalyPeriod)anomalyPeriod.value="custom";refreshAnomalyInBackground(true);});anomalyEnd?.addEventListener("change",e=>{ANOMALY_STATE.customEnd=e.target.value||"";ANOMALY_STATE.period="custom";if(anomalyPeriod)anomalyPeriod.value="custom";refreshAnomalyInBackground(true);});anomalySearch?.addEventListener("input",e=>scheduleAnomalySearch(e.target?.value||""));
bindSheetInputForm();
bindArchiveEvents();
...
const FINAL_OUTBOUND_STATUSES=["close","closed","selesai","completed","barang keluar"];
const NON_FINAL_OUTBOUND_STATUSES=["draft","pending","open"];
const CANCELLED_OUTBOUND_STATUSES=["cancel","cancelled","batal","ditolak"];
const ANOMALY_STATE={page:1,pageSize:25,rows:[],filtered:[],q:"",severity:"all",type:"all",period:"all",customStart:"",customEnd:"",headerFilters:{sku:"",lokasi:"",status:"",severity:"",cause:""},rendered:false,isLoading:false,_searchDebounce:null,lastRenderToken:0,lastCheckedAt:null,lastSourceVersion:"",checked:{},notes:{}};
function canonicalText(v,{keepDash=false}={}){let s=String(v||'').trim();try{s=decodeURIComponent(s);}catch(_){}s=s.replace(/[–—]/g,'-').replace(/\s+/g,' ').toLowerCase();return keepDash?s.replace(/\s*-\s*/g,'-'):s.replace(/[-_]+/g,' ').replace(/\s+/g,' ');}
function normalizeSku(v){return canonicalText(v,{keepDash:false});}
function normalizeSkuLoose(v){return canonicalText(v,{keepDash:false}).replace(/[-\s]/g,'');}
function normalizeLocation(v){return canonicalText(v,{keepDash:true});}
function displayLocation(v){let s=String(v||'').trim();try{s=decodeURIComponent(s);}catch(_){}return s.replace(/\s+/g,' ').toUpperCase()||'-';}
function isValidSku(sku){const value=String(sku||"").trim().toLowerCase();return value&&value!=="-"&&value!=="null"&&value!=="undefined";}
function getSkuName(row){return String(getVal(row,["nama barang","nama","nama produk","item","description"])||"").trim();}
function rowDate(row){const raw=getVal(row,["tanggal","date","tgl","last update","updated at","last_update"]);const d=raw?new Date(raw):null;return d&&!isNaN(d)?d:null;}
function getAnomalyRange(){const now=new Date(),start=new Date(now),end=new Date(now);end.setHours(23,59,59,999);if(ANOMALY_STATE.period==='today'){start.setHours(0,0,0,0);return {start,end,label:'Hari ini'};}if(ANOMALY_STATE.period==='7d'){start.setDate(start.getDate()-6);start.setHours(0,0,0,0);return {start,end,label:'7 hari terakhir'};}if(ANOMALY_STATE.period==='month'){start.setDate(1);start.setHours(0,0,0,0);return {start,end,label:'Bulan ini'};}if(ANOMALY_STATE.period==='lastmonth'){const s=new Date(now.getFullYear(),now.getMonth()-1,1),e=new Date(now.getFullYear(),now.getMonth(),0,23,59,59,999);return {start:s,end:e,label:'Bulan lalu'};}if(ANOMALY_STATE.period==='custom'){const s=ANOMALY_STATE.customStart?new Date(ANOMALY_STATE.customStart):null,e=ANOMALY_STATE.customEnd?new Date(ANOMALY_STATE.customEnd):null;if(e)e.setHours(23,59,59,999);return {start:s,end:e,label:'Custom'};}return {start:null,end:null,label:'Semua data'};}
function inAnomalyPeriod(row,hasDate){const range=getAnomalyRange();if(!range.start&&!range.end)return true;if(!hasDate)return true;const d=rowDate(row);if(!d)return false;return (!range.start||d>=range.start)&&(!range.end||d<=range.end);}
function anomalyKey(sku,loc,hasLoc){return `${normalizeSku(sku)}__${hasLoc?normalizeLocation(loc):'ALL'}`;}
function normalizeQtyText(raw){const text=String(raw??'').trim().replace(/\s/g,'');if(!text)return '';if(/,/.test(text)&&/\./.test(text)){const lastComma=text.lastIndexOf(','),lastDot=text.lastIndexOf('.');return lastComma>lastDot?text.replace(/\./g,'').replace(',','.'):text.replace(/,/g,'');}if(/,/.test(text)){const parts=text.split(',');return parts.length===2&&parts[1].length===3?text.replace(/,/g,''):text.replace(',','.');}if(/\./.test(text)){const parts=text.split('.');return parts.length===2&&parts[1].length===3?text.replace(/\./g,''):text;}return text;}
function parseQtyStrict(raw){const text=String(raw??'').trim();if(!text)return {valid:false,qty:0,reason:'QTY kosong'};const normalized=normalizeQtyText(text);if(!/^-?\d+(\.\d+)?$/.test(normalized))return {valid:false,qty:0,reason:'QTY bukan angka / format angka salah'};const qty=Number(normalized);if(!Number.isFinite(qty))return {valid:false,qty:0,reason:'QTY bukan angka'};if(qty<0)return {valid:false,qty,reason:'QTY negatif'};if(qty===0)return {valid:false,qty,reason:'QTY nol'};return {valid:true,qty};}
function qtyCheck(row,fieldNames){return parseQtyStrict(getVal(row,fieldNames));}
function outboundStatus(row){const s=clean(getVal(row,["status"]));if(CANCELLED_OUTBOUND_STATUSES.includes(s))return 'cancelled';if(NON_FINAL_OUTBOUND_STATUSES.includes(s))return 'non-final';if(!s||FINAL_OUTBOUND_STATUSES.includes(s))return 'final';return 'final';}
function makeAnomalyRow(base){const abs=Math.abs(base.selisih||0);const severity=base.minusStatus==='KEMUNGKINAN MINUS'||abs>=10||base.invalidCount?'Critical':(abs>0||base.duplicateCount||base.nonFinalQty||base.locationMismatch?'Warning':'Info');return {id:`${base.skuKey}_${base.locationKey}_${base.issue}`,...base,severity};}
function getAnomalySourceVersion(){const kartu=DATA["Kartu Stock"]||DATA["Kartu Stok"]||[],keluar=getBarangKeluarRows();return [kartu.length,keluar.length,sheetChecksum(kartu),sheetChecksum(keluar),ANOMALY_STATE.period,ANOMALY_STATE.customStart,ANOMALY_STATE.customEnd].join('|');}
function buildAnomalyReport(){
 const kartu=DATA["Kartu Stock"]||DATA["Kartu Stok"]||[],keluar=getBarangKeluarRows();
 const kartuHasLoc=kartu.some(r=>getVal(r,["lokasi","location","rak","bin","area"]));const keluarHasLoc=keluar.some(r=>getVal(r,["from","lokasi","lokasi asal"]));const hasLoc=kartuHasLoc&&keluarHasLoc;
 const kartuHasDate=kartu.some(r=>rowDate(r));const keluarHasDate=keluar.some(r=>rowDate(r));const periodLimited=ANOMALY_STATE.period!=='all';
 const groups=new Map(),skuLoose=new Map(),dups=new Map();
 const ensure=(sku,loc)=>{const skuKey=normalizeSku(sku),locKey=hasLoc?normalizeLocation(loc):'ALL',key=`${skuKey}__${locKey}`;if(!groups.has(key))groups.set(key,{sku:String(sku||'').trim(),skuKey,location:hasLoc?displayLocation(loc):'Semua Lokasi',locationKey:locKey,nama:'-',kartuPengeluaran:0,barangKeluarQty:0,stokAwal:0,pemasukan:0,stokSaatIni:null,kartuRows:[],outRows:[],invalidRows:[],nonFinalRows:[],cancelledRows:[],duplicateRows:[],notes:[],noRefs:new Set(),searchText:''});return groups.get(key);};
 for(const r of kartu){const sku=getVal(r,["sku"]);if(!isValidSku(sku))continue;if(!inAnomalyPeriod(r,kartuHasDate))continue;const loc=getVal(r,["lokasi","location","rak","bin","area"]);const g=ensure(sku,loc);g.nama=g.nama==='-'?(getSkuName(r)||'-'):g.nama;g.kartuRows.push(r);g.kartuPengeluaran+=Math.abs(parseNumber(getVal(r,["pengeluaran","qty keluar","keluar","out"]))||0);g.stokAwal+=parseNumber(getVal(r,["stok awal","opening stock","beginning stock","saldo awal"]))||0;g.pemasukan+=parseNumber(getVal(r,["pemasukan","qty masuk","masuk","in"]))||0;const stok=getVal(r,["stok akhir","stok","stock","closing stock","ending stock","saldo akhir"]);if(stok!==undefined&&stok!==null&&String(stok).trim()!=='')g.stokSaatIni=(g.stokSaatIni||0)+(parseNumber(stok)||0);const loose=normalizeSkuLoose(sku);if(!skuLoose.has(loose))skuLoose.set(loose,new Set());skuLoose.get(loose).add(String(sku).trim());}
 for(const r of keluar){const sku=getVal(r,["sku"]);if(!isValidSku(sku))continue;if(!inAnomalyPeriod(r,keluarHasDate))continue;const loc=getVal(r,["from","lokasi","lokasi asal"]);const g=ensure(sku,loc);g.nama=g.nama==='-'?(getSkuName(r)||'-'):g.nama;g.outRows.push(r);['no iseller','netsuite','keterangan','keterangan lainnya'].forEach(k=>{const v=getVal(r,[k]);if(v)g.noRefs.add(v);});const qc=qtyCheck(r,["qty","quantity","jumlah"]),st=outboundStatus(r);if(!qc.valid){g.invalidRows.push({...r,_reason:qc.reason});continue;}if(st==='cancelled'){g.cancelledRows.push(r);continue;}if(st==='non-final'){g.nonFinalRows.push(r);continue;}g.barangKeluarQty+=Math.abs(qc.qty);const dk=[rowDate(r)?.toISOString().slice(0,10)||'',normalizeSku(sku),qc.qty,normalizeLocation(getVal(r,["from"])),normalizeLocation(getVal(r,["to"])),clean(getVal(r,["no iseller"])),clean(getVal(r,["netsuite"]))].join('|');if(!dups.has(dk))dups.set(dk,[]);dups.get(dk).push(r);}
 for(const list of dups.values())if(list.length>1){const r=list[0],g=ensure(getVal(r,["sku"]),getVal(r,["from"]));g.duplicateRows.push(...list);}
 const rows=[];for(const g of groups.values()){const selisih=g.kartuPengeluaran-g.barangKeluarQty;let stokPerkiraan=null,minusStatus='Tidak dapat dihitung',minusReason='Kolom stok tidak cukup untuk menghitung stok perkiraan.';if(g.stokAwal||g.pemasukan) {stokPerkiraan=g.stokAwal+g.pemasukan-g.barangKeluarQty;minusReason='stokAwal + pemasukan - qty barang keluar';} else if(g.stokSaatIni!==null){stokPerkiraan=g.stokSaatIni+g.kartuPengeluaran-g.barangKeluarQty;minusReason='stok saat ini + pengeluaran kartu stok - qty barang keluar';}if(stokPerkiraan!==null)minusStatus=stokPerkiraan<0?'KEMUNGKINAN MINUS':stokPerkiraan<=3?'RISIKO MINUS':'AMAN';
  const abs=Math.abs(selisih), reasons=[], rec=[];let issue='BALANCE',confidence='Rendah';if(Math.abs(selisih)>0.0001){issue=selisih>0?'KARTU STOK LEBIH BESAR':'BARANG KELUAR LEBIH BESAR';confidence=abs>=5?'Tinggi':'Sedang';reasons.push(selisih<0?'Ditemukan qty keluar yang belum tercermin di kolom Pengeluaran Kartu Stok.':'Pengeluaran Kartu Stok lebih besar daripada transaksi Barang Keluar. Periksa input manual atau transaksi yang terhapus.');rec.push(selisih<0?'Periksa row Barang Keluar yang belum memiliki pasangan di Kartu Stok.':'Periksa input manual Kartu Stok atau transaksi Barang Keluar yang terhapus.');}
  if(g.duplicateRows.length){reasons.push(`Kemungkinan transaksi duplikat ditemukan (${g.duplicateRows.length} row dicurigai).`);rec.push('Periksa transaksi duplikat dengan tanggal, SKU, qty, FROM/TO, No iSeller, dan Netsuite yang sama.');}
  if(g.nonFinalRows.length){reasons.push('Sebagian transaksi belum berstatus final.');rec.push('Periksa transaksi yang masih berstatus Draft/Pending/Open sebelum dihitung final.');}
  if(g.invalidRows.length){reasons.push(`${g.invalidRows.length} row memiliki QTY invalid.`);rec.push('Perbaiki QTY kosong/bukan angka/negatif/nol sebelum audit stok.');}
  if(minusStatus==='KEMUNGKINAN MINUS'){reasons.push(`Kemungkinan stok minus (${stokPerkiraan}).`);rec.push('Audit SKU sebelum stok menjadi minus dan cek transaksi keluar terbaru.');}
  const sameSku=[...groups.values()].filter(x=>x.skuKey===g.skuKey&&x.locationKey!==g.locationKey&&((x.kartuPengeluaran&&g.barangKeluarQty)||(x.barangKeluarQty&&g.kartuPengeluaran)));if(hasLoc&&sameSku.length&&abs>0){reasons.push('Transaksi ditemukan, tetapi lokasi asal tidak cocok.');rec.push('Verifikasi lokasi asal transaksi Barang Keluar terhadap lokasi Kartu Stok.');g.locationMismatch=true;}
  if(periodLimited&&!kartuHasDate){reasons.push('Tanggal Kartu Stok tidak tersedia/rinci; perbandingan periode hanya dapat dianalisis kumulatif.');rec.push('Tambahkan tanggal transaksi Kartu Stok untuk validasi periode yang akurat.');}
  if(reasons.length||issue==='BALANCE')rows.push(makeAnomalyRow({...g,issue,selisih,stokPerkiraan,minusStatus,minusReason,confidence,cause:reasons[0]||'Semua data Kartu Stok dan Barang Keluar pada periode ini terdeteksi balance.',causes:reasons,recommendation:rec[0]||'Tidak ada aksi; data terdeteksi balance.',recommendations:rec,duplicateCount:g.duplicateRows.length,invalidCount:g.invalidRows.length,nonFinalQty:g.nonFinalRows.length}));}
 for(const variants of skuLoose.values())if(variants.size>1){const sku=[...variants][0],g=ensure(sku,'');rows.push(makeAnomalyRow({...g,issue:'SKU TIDAK KONSISTEN',selisih:0,stokPerkiraan:null,minusStatus:'Tidak dapat dihitung',confidence:'Sedang',cause:'Kode SKU memiliki format berbeda dan kemungkinan tidak terkelompok dengan benar.',causes:[`Varian SKU: ${[...variants].join(', ')}`],recommendation:'Standarisasi format SKU tanpa mengubah data otomatis.',recommendations:['Audit format SKU pada kedua sheet.']}));}
 ANOMALY_STATE.lastCheckedAt=new Date().toISOString();const severityRank={Critical:0,Warning:1,Info:2};return rows.sort((a,b)=>(severityRank[a.severity]??9)-(severityRank[b.severity]??9)||Math.abs(b.selisih)-Math.abs(a.selisih));}
function sevClass(s){return s==='Critical'?'b-high':s==='Warning'?'b-medium':'b-low';}
function setAnomalyCache(rows=[]){try{localStorage.setItem(ANOMALY_CACHE_KEY,JSON.stringify({rows,updatedAt:Date.now(),period:ANOMALY_STATE.period}));}catch(_){}}
...
function scheduleAnomalySearch(value=""){ANOMALY_STATE.q=value;clearTimeout(ANOMALY_STATE._searchDebounce);ANOMALY_STATE._searchDebounce=setTimeout(()=>applyAnomalyFilters(true),250);}
function isActionableAnomaly(r){return r.issue!=='BALANCE'||r.minusStatus==='KEMUNGKINAN MINUS'||r.duplicateCount||r.invalidCount||r.nonFinalQty||r.locationMismatch;}
function getAnomalyFilteredRows(){const q=clean(ANOMALY_STATE.q),hf=ANOMALY_STATE.headerFilters;return (ANOMALY_STATE.rows||[]).filter(r=>{if(ANOMALY_STATE.type==='all'&&!isActionableAnomaly(r))return false;if(ANOMALY_STATE.severity!=='all'&&r.severity!==ANOMALY_STATE.severity)return false;if(ANOMALY_STATE.type==='not_balance'&&r.issue==='BALANCE')return false;if(ANOMALY_STATE.type==='minus'&&r.minusStatus!=='KEMUNGKINAN MINUS')return false;if(ANOMALY_STATE.type==='kartu'&&r.issue!=='KARTU STOK LEBIH BESAR')return false;if(ANOMALY_STATE.type==='keluar'&&r.issue!=='BARANG KELUAR LEBIH BESAR')return false;if(ANOMALY_STATE.type==='dupe'&&!r.duplicateCount)return false;if(ANOMALY_STATE.type==='invalid'&&!r.invalidCount)return false;if(ANOMALY_STATE.type==='loc'&&!r.locationMismatch)return false;if(ANOMALY_STATE.type==='non_final'&&!r.nonFinalQty)return false;const hay=`${r.sku} ${r.nama} ${r.location} ${[...r.noRefs||[]].join(' ')} ${r.cause} ${r.recommendation}`;if(q&&!clean(hay).includes(q))return false;if(hf.sku&&!clean(r.sku).includes(clean(hf.sku)))return false;if(hf.lokasi&&!clean(r.location).includes(clean(hf.lokasi)))return false;if(hf.status&&!clean(r.issue).includes(clean(hf.status)))return false;if(hf.severity&&!clean(r.severity).includes(clean(hf.severity)))return false;if(hf.cause&&!clean(r.cause).includes(clean(hf.cause)))return false;return true;});}
function fmtQty(n){return n===null||n===undefined?'-':Number(n).toLocaleString('id-ID',{maximumFractionDigits:3});}
function renderAnomalyRowsOnly(){const rows=getAnomalyFilteredRows();ANOMALY_STATE.filtered=rows;const total=rows.length,max=Math.max(1,Math.ceil(total/ANOMALY_STATE.pageSize));if(ANOMALY_STATE.page>max)ANOMALY_STATE.page=max;const start=(ANOMALY_STATE.page-1)*ANOMALY_STATE.pageSize,pageRows=rows.slice(start,start+ANOMALY_STATE.pageSize);const tbody=document.querySelector('#anomalyTable tbody');if(tbody)tbody.innerHTML=pageRows.map(r=>`<tr><td><span class='badge ${sevClass(r.severity)}'>${esc(r.severity)}</span></td><td>${esc(r.sku)}</td><td>${esc(r.nama)}</td><td>${esc(r.location)}</td><td>${fmtQty(r.kartuPengeluaran)}</td><td>${fmtQty(r.barangKeluarQty)}</td><td class='${r.selisih<0?'txt-danger':r.selisih>0?'txt-warn':''}'>${fmtQty(r.selisih)}</td><td>${fmtQty(r.stokSaatIni)}</td><td>${fmtQty(r.stokPerkiraan)}</td><td>${esc(r.issue)}</td><td>${esc(r.cause)}</td><td>${esc(r.confidence)}</td><td><button class='btn-ghost' onclick="openAnomalyDetail('${encAttr(r.id)}')">Detail</button></td></tr>`).join('')||`<tr><td colspan='13'><div class='state'>Semua data Kartu Stok dan Barang Keluar pada periode ini terdeteksi balance.<br>Total SKU diperiksa: ${new Set((ANOMALY_STATE.rows||[]).map(x=>x.skuKey)).size} • Total qty dibandingkan: ${fmtQty((ANOMALY_STATE.rows||[]).reduce((n,x)=>n+(x.barangKeluarQty||0),0))} • Waktu pemeriksaan: ${esc(new Date(ANOMALY_STATE.lastCheckedAt||Date.now()).toLocaleString('id-ID'))}</div></td></tr>`;const info=document.getElementById('anomalyInfo');if(info)info.textContent=`Menampilkan ${pageRows.length?start+1:0}-${Math.min(start+pageRows.length,total)} dari ${total.toLocaleString('id-ID')} data`;}
function renderAnomalySummary(){const all=ANOMALY_STATE.rows||[];const cards=[['Total SKU Diperiksa',new Set(all.map(r=>r.skuKey)).size],['Balance',all.filter(r=>r.issue==='BALANCE').length],['Tidak Balance',all.filter(r=>r.issue!=='BALANCE').length],['Kemungkinan Minus',all.filter(r=>r.minusStatus==='KEMUNGKINAN MINUS').length],['Duplikasi Terindikasi',all.reduce((n,r)=>n+(r.duplicateCount?1:0),0)],['Data Invalid',all.reduce((n,r)=>n+(r.invalidCount||0),0)]];anomalySummary.innerHTML=cards.map(c=>`<div class='metric anomaly-metric'><div class='k'>${c[0]}</div><div class='v'>${esc(c[1])}</div></div>`).join('');}
function applyAnomalyFilters(resetPage=false){if(resetPage)ANOMALY_STATE.page=1;ANOMALY_STATE.severity=document.getElementById('anomalySeverity')?.value||'all';ANOMALY_STATE.type=document.getElementById('anomalyType')?.value||'all';renderAnomalyRowsOnly();renderAnomalySummary();}
function renderAnomalyPage(){if(!ANOMALY_STATE.rendered){anomalyTable.innerHTML=`<div class='mv-toolbar anomaly-toolbar'><div id='anomalyInfo' class='mv-pagination-info'>Menampilkan 0 data</div><select id='anomalySize'><option value='25'>25</option><option value='50'>50</option><option value='100'>100</option></select><button class='btn-ghost' onclick='refreshAnomalyInBackground(true)'>Refresh</button><button class='btn-ghost' onclick='exportAnomalyCsv()'>Export</button><button class='btn-ghost' onclick='changeAnomalyPage(-1)'>Prev</button><button class='btn-ghost' onclick='changeAnomalyPage(1)'>Next</button></div><div id='anomalyLoading' class='state hidden'>Memuat warning...</div><div class='table-wrap table-wrap-full anomaly-table-wrap'><table><thead><tr><th>Severity<br><input data-af='severity' placeholder='Filter'></th><th>SKU<br><input data-af='sku' placeholder='SKU'></th><th>Nama Barang</th><th>Lokasi<br><input data-af='lokasi' placeholder='Lokasi'></th><th>Pengeluaran Kartu Stok</th><th>Qty Barang Keluar</th><th>Selisih</th><th>Stok Saat Ini</th><th>Stok Perkiraan</th><th>Status<br><input data-af='status' placeholder='Status'></th><th>Kemungkinan Penyebab<br><input data-af='cause' placeholder='Penyebab'></th><th>Confidence</th><th>Aksi</th></tr></thead><tbody></tbody></table></div><div id='anomalyDetailHost'></div>`;document.getElementById('anomalySize')?.addEventListener('change',e=>{ANOMALY_STATE.pageSize=Number(e.target.value)||25;ANOMALY_STATE.page=1;renderAnomalyRowsOnly();});document.querySelectorAll('[data-af]').forEach(el=>el.addEventListener('input',e=>{ANOMALY_STATE.headerFilters[e.target.dataset.af]=e.target.value;applyAnomalyFilters(true);}));ANOMALY_STATE.rendered=true;}document.getElementById('anomalySize').value=String(ANOMALY_STATE.pageSize);applyAnomalyFilters(false);if(!(ANOMALY_STATE.rows||[]).length){const cacheRows=getAnomalyCache();if(cacheRows.length){ANOMALY_STATE.rows=cacheRows;applyAnomalyFilters(false);setAnomalyLoading(true,'Menampilkan cache, sinkronisasi data terbaru...');}else setAnomalyLoading(true,'Memuat data warning...');}refreshAnomalyInBackground();}
function refreshAnomalyInBackground(force=false){const token=++ANOMALY_STATE.lastRenderToken,sourceVersion=getAnomalySourceVersion();if(!force&&ANOMALY_STATE.lastSourceVersion===sourceVersion&&ANOMALY_STATE.rows.length){applyAnomalyFilters(false);setAnomalyLoading(false);return;}setAnomalyLoading(true,'Menghitung warning terbaru...');logActivitySafe?.({action:'REFRESH_WARNING',module:'Warning',detail:'Refresh warning',status:'SUCCESS'});const run=()=>{const all=buildAnomalyReport();if(token!==ANOMALY_STATE.lastRenderToken)return;ANOMALY_STATE.rows=all;ANOMALY_STATE.lastSourceVersion=sourceVersion;setAnomalyCache(all);applyAnomalyFilters(false);setAnomalyLoading(false);};if(typeof requestIdleCallback==='function'&&!force){requestIdleCallback(run,{timeout:700});return;}setTimeout(run,0);}
...
function openAnomalyDetail(idEnc){const id=decodeURIComponent(idEnc||''),r=(ANOMALY_STATE.rows||[]).find(x=>x.id===id);if(!r)return;logActivitySafe?.({action:'OPEN_WARNING_DETAIL',module:'Warning',detail:r.sku,status:'SUCCESS'});const host=document.getElementById('anomalyDetailHost');host.innerHTML=`<div class='anomaly-detail-backdrop' onclick='closeAnomalyDetail()'></div><aside class='anomaly-detail'><button class='btn-ghost' onclick='closeAnomalyDetail()'>Tutup</button><h3>${esc(r.sku)} • ${esc(r.location)}</h3><p><strong>Masalah:</strong> ${esc(r.issue)} (${esc(r.severity)})</p><div class='detail-note'><div class='note-box'><div class='note-title'>Perhitungan</div><div class='note-value'>Pengeluaran Kartu Stok: ${fmtQty(r.kartuPengeluaran)} • Barang Keluar: ${fmtQty(r.barangKeluarQty)} • Selisih: ${fmtQty(r.selisih)} • Stok Perkiraan: ${fmtQty(r.stokPerkiraan)}</div></div><div class='note-box'><div class='note-title'>Kemungkinan penyebab (${esc(r.confidence)})</div><div class='note-value'>${(r.causes||[r.cause]).map(esc).join('<br>')}</div></div><div class='note-box'><div class='note-title'>Rekomendasi</div><div class='note-value'>${(r.recommendations||[r.recommendation]).map(esc).join('<br>')}</div></div></div>${renderTraceTable('Kartu Stock terkait',r.kartuRows||[])}${renderTraceTable('Barang Keluar terkait',r.outRows||[])}${renderTraceTable('QTY invalid',r.invalidRows||[])}<div class='row'><button class='btn-ghost' onclick="copyText(decodeURIComponent('${encAttr(r.sku)}'),'SKU disalin')">Copy SKU</button><button class='btn-ghost' onclick="copyText(decodeURIComponent('${encAttr(`${r.sku} ${r.issue} ${r.cause}`)}'),'Analisis disalin')">Copy hasil analisis</button><button class='btn-ghost' onclick="markAnomalyChecked('${encAttr(r.id)}')">Tandai sudah diperiksa</button></div><textarea id='anomalyNote' class='search-lg' placeholder='Catatan pemeriksaan'>${esc(ANOMALY_STATE.notes[r.id]||'')}</textarea><button class='btn-primary' onclick="saveAnomalyNote('${encAttr(r.id)}')">Simpan catatan</button></aside>`;}
function closeAnomalyDetail(){const h=document.getElementById('anomalyDetailHost');if(h)h.innerHTML='';}
function markAnomalyChecked(idEnc){const id=decodeURIComponent(idEnc||'');ANOMALY_STATE.checked[id]=new Date().toISOString();logActivitySafe?.({action:'MARK_WARNING_CHECKED',module:'Warning',detail:id,status:'SUCCESS'});toast('Warning ditandai sudah diperiksa','success');}
function saveAnomalyNote(idEnc){const id=decodeURIComponent(idEnc||'');ANOMALY_STATE.notes[id]=document.getElementById('anomalyNote')?.value||'';logActivitySafe?.({action:'ADD_WARNING_NOTE',module:'Warning',detail:id,status:'SUCCESS'});toast('Catatan disimpan','success');}
function exportAnomalyCsv(){const cols=['severity','sku','nama','lokasi','kartu_pengeluaran','barang_keluar_qty','selisih','stok_saat_ini','stok_perkiraan','status','penyebab','confidence'];const lines=[cols.join(',')].concat((ANOMALY_STATE.filtered||ANOMALY_STATE.rows||[]).map(r=>[r.severity,r.sku,r.nama,r.location,r.kartuPengeluaran,r.barangKeluarQty,r.selisih,r.stokSaatIni,r.stokPerkiraan,r.issue,r.cause,r.confidence].map(v=>`"${String(v??'').replaceAll('"','""')}"`).join(',')));const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([lines.join('\n')],{type:'text/csv;charset=utf-8;'}));a.download='warning-kartu-stok-vs-barang-keluar.csv';a.click();URL.revokeObjectURL(a.href);logActivitySafe?.({action:'EXPORT_WARNING',module:'Warning',detail:`${lines.length-1} rows`,status:'SUCCESS'});}
window.renderAnomalyPage=renderAnomalyPage;window.changeAnomalyPage=changeAnomalyPage;window.applyAnomalyFilters=applyAnomalyFilters;window.scheduleAnomalySearch=scheduleAnomalySearch;window.refreshAnomalyInBackground=refreshAnomalyInBackground;window.openAnomalyDetail=openAnomalyDetail;window.closeAnomalyDetail=closeAnomalyDetail;window.markAnomalyChecked=markAnomalyChecked;window.saveAnomalyNote=saveAnomalyNote;window.exportAnomalyCsv=exportAnomalyCsv;
...
function applyBalikanTableFilters(rows,mode='balikan',omitCol=''){const st=ensureBalikanFilterState();const rawQuery=window.balikanSearchKeyword||'';const q=normalizeBalikanRawSearch(rawQuery);const qTokens=getBalikanSearchTokens(q);const isLocationQuery=isBalikanLocationSearchQuery(rawQuery)&&qTokens.length===1;const locationQuery=normalizeBalikanLocationSearchValue(rawQuery);const exactScanSku=normalizeBalikanRawSearch(BALIKAN_STATE.exactScanSku||'');const selectedSku=normalizeBalikanRawSearch(BALIKAN_STATE.selectedSkuValue||'');return rows.filter(r=>{if(BALIKAN_FILTERABLE_COLUMNS.some(col=>{if(col===omitCol)return false;const selected=st.columnFilters[col]||[];if(!selected.length)return false;return !selected.includes(sanitizeFilterValue(r[col]));}))return false;if(exactScanSku&&normalizeBalikanRawSearch(r.sku)!==exactScanSku)return false;if(selectedSku&&q===selectedSku&&normalizeBalikanRawSearch(r.sku)!==selectedSku)return false;if(qTokens.length){if(matchesBalikanSkuBarcodeRaw(r,q))return true;if(isLocationQuery){if(!matchesBalikanLocationQuery(r,locationQuery))return false;}else{const rowText=getBalikanSearchText(r);if(!qTokens.every(token=>rowText.includes(token)))return false;}}return true;});}
