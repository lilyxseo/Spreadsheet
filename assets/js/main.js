...
function isPicRole(){
  return String(getCurrentUserRole()||"").trim().toLowerCase()==="pic";
}
function getPermissions(){
  const canRead=true;
  const canCreate=isPicRole();
...
function renderReadOnlyBadge(){
  ...
  const badgeHtml=`<span class="badge-readonly">Read Only</span>`;
  pageTitles.forEach((el)=>{
    if(isPicRole()){el.innerHTML=el.innerHTML.replace(badgeHtml,"");return;}
    if(!el.innerHTML.includes("badge-readonly"))el.innerHTML+=` ${badgeHtml}`;
  });
}
function applyRoleBasedUi(){
  const perms=getPermissions();
  const hideForReadOnly=["sheetSubmitBtn","balikanAutoCheckToggle"];
  hideForReadOnly.forEach(id=>{const el=document.getElementById(id);if(el)el.style.display=perms.canUpdate?"":"none";});
  document.querySelectorAll("[data-mv-action='delete'],[data-cch-action='delete']").forEach(el=>{el.style.display=perms.canDelete?"":"none";});
  renderReadOnlyBadge();
}
...
balikanAutoCheckToggle?.addEventListener("click",()=>toggleBalikanAutoCheck());
...
window.ccStartSession=()=>{if(!getPermissions().canCreate){toast('Mode Read Only: tidak bisa mulai cycle count','error');return;}CYCLE_STATE.sessionActive=true;CYCLE_STATE.search="";CYCLE_STATE.searchInput="";CYCLE_STATE.sessionItems=[];renderCycleCountPage();};
...
window.mvStartSession=()=>{if(!getPermissions().canCreate){toast('Mode Read Only: tidak bisa mulai movement','error');return;}MOVEMENT_STATE.sessionActive=true;MOVEMENT_STATE.search="";MOVEMENT_STATE.searchInput="";MOVEMENT_STATE.sessionItems=[];renderMovementPage();};
...
function startInlineEdit(cellEl,row,field,oldValue,options={}){if(!getPermissions().canUpdate){toast("Mode Read Only: tidak bisa edit data","error");return;}const td=cellEl.closest('td')||cellEl;if(td.querySelector("input,select"))return;const input=document.createElement("input");input.value=oldValue||"";input.className="inline-edit-input";td.classList.add("editing");td.innerHTML="";td.appendChild(input);input.focus();input.select();let cancelled=false;async function save(){if(cancelled)return;const newValue=input.value.trim();if(newValue===String(oldValue||"")){td.classList.remove("editing");td.textContent=oldValue||"-";return;}try{await options.onSave?.({row,field,value:newValue,oldValue,td});}catch(err){td.textContent=oldValue||"-";toast(err?.message||"Gagal update data","error");}finally{td.classList.remove("editing");}}input.addEventListener("blur",save);input.addEventListener("keydown",(e)=>{if(e.key==="Enter")input.blur();if(e.key==="Escape"){cancelled=true;td.classList.remove("editing");td.textContent=oldValue||"-";}});}
...
function syncBalikanAutoCheckToggle(){if(!balikanAutoCheckToggle)return;const perms=getPermissions();const isOn=BALIKAN_STATE.autoCheckOnScan!==false;balikanAutoCheckToggle.classList.toggle('is-on',isOn);balikanAutoCheckToggle.classList.toggle('is-off',!isOn);balikanAutoCheckToggle.setAttribute('aria-pressed',isOn?'true':'false');balikanAutoCheckToggle.textContent=`Scan to checklist: ${isOn?'ON':'OFF'}`;balikanAutoCheckToggle.style.display=perms.canUpdate?"":"none";}
function toggleBalikanAutoCheck(){BALIKAN_STATE.autoCheckOnScan=!(BALIKAN_STATE.autoCheckOnScan!==false);syncBalikanAutoCheckToggle();}
