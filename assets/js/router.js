...
export function ensureDataReady(state, onLoading){
  if(state?.isDataReady) return true;
  if(typeof onLoading==='function') onLoading();
  return false;
}

export function renderCurrentRoute(path, showDetail, setCurrentSku){
  ...
  if(path.startsWith('/sku/')){const sku=decodeURIComponent(path.split('/sku/')[1]||''); if(sku){setCurrentSku(sku);showDetail(sku);} return showPage('detail');}
  showPage('dashboard');
}

export function routeFromPath(path,showDetail,setCurrentSku,state,onLoading){
  if(!ensureDataReady(state,onLoading)) return;
  renderCurrentRoute(path,showDetail,setCurrentSku);
}
